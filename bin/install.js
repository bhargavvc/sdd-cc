...
const SDD_CODEX_MARKER = '# SDD Agent Configuration \u2014 managed by sdd installer';
const SDD_CODEX_HOOKS_OWNERSHIP_PREFIX = '# SDD codex_hooks ownership: ';
...
const SDD_COPILOT_INSTRUCTIONS_MARKER = '<!-- SDD Configuration \u2014 managed by sdd installer -->';
const SDD_COPILOT_INSTRUCTIONS_CLOSE_MARKER = '<!-- /SDD Configuration -->';

const CODEX_AGENT_SANDBOX = {
  'sdd-executor': 'workspace-write',
  'sdd-planner': 'workspace-write',
  'sdd-phase-researcher': 'workspace-write',
  'sdd-project-researcher': 'workspace-write',
  'sdd-research-synthesizer': 'workspace-write',
  'sdd-verifier': 'workspace-write',
  'sdd-codebase-mapper': 'workspace-write',
  'sdd-roadmapper': 'workspace-write',
  'sdd-debugger': 'workspace-write',
  'sdd-plan-checker': 'read-only',
  'sdd-integration-checker': 'read-only',
};
...
} = require(path.join(_sddLibDir, 'installer-migrations.cjs'));
const {
  assertInstallerMigrationsUnblocked,
  resolveInstallerMigrationPromptsForNonTty,
  summarizeInstallerMigrationResult,
} = require(path.join(_sddLibDir, 'installer-migration-report.cjs'));
...
// Note: when re-running as `sdd update` the marker is read later (after
...
Then re-run: npx @bhargavvc/sdd-cc@latest
`);
    process.exit(1);
  }
}
...
function resolveBashRunner(opts) {
  ...
  const candidates = [];
  if (env.SDD_BASH_PATH) candidates.push(env.SDD_BASH_PATH);
...
function buildCodexHookBlock(targetDir, opts) {
  ...
  const platform = (opts && opts.platform) || process.platform;
  const updateCheckScript = path.resolve(targetDir, 'hooks', 'sdd-check-update.js');
 ...
 * sdd-check-update hook entry, while preserving user-owned entries.
 *
 * Codex accepts hook config from hooks.json and config.toml. To avoid the
 * startup warning for mixed representations in the same layer, SDD now stores
...
function ensureCodexHooksJsonSessionStart(targetDir, opts = {}) {
  ...
  const managedCommand = projectManagedHookCommand({
    absoluteRunner,
    scriptPath: path.resolve(targetDir, 'hooks', 'sdd-check-update.js'),
    runtime: 'codex',
    platform,
  });
...
function convertClaudeToCopilotContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.github/');
  c = c.replace(/\.claude\//g, '.github/');
  // CONV-07: Command name conversion (all sdd: references → sdd-)
  c = c.replace(/sdd:/g, 'sdd-');
...
function convertClaudeToAntigravityContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.agent/');
  c = c.replace(/\.claude\//g, '.agent/');
  // Command name conversion (all sdd: references → sdd-)
  c = c.replace(/sdd:/g, 'sdd-');
  // Runtime-neutral agent name replacement (#766)
...
function convertSlashCommandsToCursorSkillMentions(content) {
  // Keep leading "/" for slash commands; only normalize sdd: -> sdd-.
  // This preserves rendered "next step" commands like "/sdd-execute-phase 17".
...
## C. Tool Usage
Use these Windsurf tools when executing SDD workflows:
- \`Shell\` for running commands (terminal operations)
- \`StrReplace\` for editing existing files
- \`Read\`, \`Write\`, \`Glob\`, \`Grep\`, \`Task\`, \`WebSearch\`, \`WebFetch\`, \`TodoWrite\` as needed
...
- Resolved \`reasoning_effort="low|medium|high|xhigh"\` (\`xhigh\` is a SDD/Codex tier, not a generic runtime enum) → pass \`reasoning_effort\`
...
function stripSddFromCodexConfig(content) {
  ...
  if (markerIndex !== -1) {
    // Has SDD marker — remove everything from marker to EOF
    let before = content.substring(0, markerIndex);
    before = stripCodexHooksFeatureAssignments(before, codexHooksOwnership);
    // Also strip SDD-injected feature keys above the marker (Case 3 inject)
  ...
  // No marker but may have SDD-injected feature keys
  let cleaned = content;
...
function getManagedCodexHooksOwnership(content) {
  ...
  const afterMarker = content.slice(markerIndex + SDD_CODEX_MARKER.length);
  const match = afterMarker.match(/^\r?\n# SDD codex_hooks ownership: (section|root_dotted)\r?\n/);
...
function setManagedCodexHooksOwnership(content, ownership) {
  ...
  const eol = detectLineEnding(content);
  const markerEnd = markerIndex + SDD_CODEX_MARKER.length;
  ...
  return content.slice(0, markerEnd) +
    eol +
    `${SDD_CODEX_HOOKS_OWNERSHIP_PREFIX}${ownership}${eol}` +
    remainder;
}
...
/**
 * Repair config.toml files corrupted by pre-#1346 SDD installs.
...
/**
 * Merge SDD instructions into copilot-instructions.md.
 ...
 * @param {string} sddContent - Template content (without markers)
 */
function mergeCopilotInstructions(filePath, sddContent) {
  const sddBlock = SDD_COPILOT_INSTRUCTIONS_MARKER + '\n' +
    sddContent.trim() + '\n' +
    SDD_COPILOT_INSTRUCTIONS_CLOSE_MARKER;

  // Case 1: No file — create fresh
  if (!fs.existsSync(filePath)) {
    fs.writeFileSync(filePath, sddBlock + '\n');
    return;
  }

  const existing = fs.readFileSync(filePath, 'utf8');
  const openIndex = existing.indexOf(SDD_COPILOT_INSTRUCTIONS_MARKER);
  const closeIndex = existing.indexOf(SDD_COPILOT_INSTRUCTIONS_CLOSE_MARKER);

  // Case 2: Has SDD markers — replace between markers
  if (openIndex !== -1 && closeIndex !== -1) {
    const before = existing.substring(0, openIndex).trimEnd();
    const after = existing.substring(closeIndex + SDD_COPILOT_INSTRUCTIONS_CLOSE_MARKER.length).trimStart();
    let newContent = '';
...
/**
 * Strip SDD section from copilot-instructions.md content.
 * Returns cleaned content, or null if file should be deleted (was SDD-only).
...
function stripSddFromCopilotInstructions(content) {
  ...
  if (openIndex !== -1 && closeIndex !== -1) {
    ...
    const after = content.substring(closeIndex + SDD_COPILOT_INSTRUCTIONS_CLOSE_MARKER.length).trimStart();
...
function installCodexConfig(targetDir, agentsSrc) {
  ...
  const agentEntries = fs.readdirSync(agentsSrc).filter(f => f.startsWith('sdd-') && f.endsWith('.md'));
  const agents = [];
  ...
  const sddBlock = generateCodexConfigBlock(agents, targetDir);
  mergeCodexConfig(configPath, sddBlock);
 ...
 * OpenCode expects: command/sdd-help.md (invoked as /sdd-help)
 * Source structure: commands/sdd/help.md
 * 
 * @param {string} srcDir - Source directory (e.g., commands/sdd/)
...
/**
 * Recursively install SDD commands as Antigravity skills.
...
function copyCommandsAsAntigravitySkills(srcDir, skillsDir, prefix, isGlobal = false) {
  ...
  // Remove previous SDD Antigravity skills
  const existing = fs.readdirSync(skillsDir, { withFileTypes: true });
  for (const entry of existing) {
    if (entry.isDirectory() && entry.name.startsWith(`${prefix}-`)) {
      fs.rmSync(path.join(skillsDir, entry.name), { recursive: true });
...
function cleanupOrphanedHooks(settings) {
  const orphanedHookPatterns = [
    'sdd-notify.sh',  // Removed in v1.6.x
    'hooks/statusline.js',  // Renamed to sdd-statusline.js in v1.9.0
    'sdd-intel-index.js',  // Removed in v1.9.2
    'sdd-intel-session.js',  // Removed in v1.9.2
    'sdd-intel-prune.js',  // Removed in v1.9.2
  ];
  ...
  // Fix #330: Update statusLine if it points to old SDD statusline.js path
  ...
  if (settings.statusLine && settings.statusLine.command &&
    ...
    console.log(`  ${green}✓${reset} Updated statusline path (hooks/statusline.js → hooks/sdd-statusline.js)`);
  }
...
/**
 * Uninstall SDD from the specified directory for a specific runtime
 * Removes only SDD-specific files/directories, preserves user content
...
function uninstall(isGlobal, runtime = 'claude') {
  ...
  try {
    fs.unlinkSync(path.join(targetDir, '.sdd-profile'));
    removedCount++;
  } catch {}
  ...
  if (isOpencode || isKilo) {
    ...
    // Copilot: clean SDD section from copilot-instructions.md
    ...
    if (fs.existsSync(instructionsPath)) {
      ...
      if (cleaned === null) {
        ...
        removedCount++;
        console.log(`  ${green}✓${reset} Removed copilot-instructions.md (was SDD-only)`);
      } else if (cleaned !== content) {
        fs.writeFileSync(instructionsPath, cleaned);
        removedCount++;
        console.log(`  ${green}✓${reset} Cleaned SDD section from copilot-instructions.md`);
      }
    }
  } else if (isAntigravity) {
    // Antigravity: remove skills/sdd-*/ directories (same layout as Copilot skills)
    const skillsDir = path.join(targetDir, 'skills');
    if (fs.existsSync(skillsDir)) {
      let skillCount = 0;
      const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
  ...
  if (fs.existsSync(agentsDir)) {
    ...
    if (agentCount > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed ${agentCount} SDD agents`);
    }
  }
  ...
  if (fs.existsSync(hooksDir)) {
    ...
    if (hookCount > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed ${hookCount} SDD hooks`);
    }
  }

  // 5. Remove SDD package.json (CommonJS mode marker)
  ...
  if (fs.existsSync(pkgJsonPath)) {
    try {
      ...
      if (content === '{"type":"commonjs"}') {
        ...
        removedCount++;
        console.log(`  ${green}✓${reset} Removed SDD package.json`);
      }
    } catch (e) {
      // Ignore read errors
    }
  ...
  // 6. Clean up settings.json (remove SDD hooks and statusline)
  ...
  console.log(`
  ${green}Done!${reset} SDD has been uninstalled from ${runtimeLabel}.
...
const PATCHES_DIR_NAME = 'sdd-local-patches';
const MANIFEST_NAME = 'sdd-file-manifest.json';
...
function install(isGlobal, runtime = 'claude', options = {}) {
  ...
  // Path prefix for file references in markdown content (e.g. sdd-tools.cjs).
  // Replaces $HOME/.claude/ or ~/.claude/ so the result is <pathPrefix>sdd/bin/...
  ...
  // sdd update marker re-application (ADR-0010 Deviation 2):
  ...
  const _commandsDir = path.join(src, 'commands', 'sdd');
  ...
  function _stageSkills(commandsSddDir) {
    if (_isCoreProfileAlias) return stageSkillsForMode(commandsSddDir, _effectiveInstallMode);
    return stageSkillsForProfile(commandsSddDir, _resolvedProfile);
  }
  ...
  // directories and files SDD will mutate so that any post-install validation
  ...
  // reflect the true pre-SDD state. Non-Codex runtimes skip this block.
  //
  // Snapshot contents:
  //   codexPreInstallSkillNames  — Set of sdd-* skill dir names that existed
  //   codexPreInstallSkillContents — Map<skillName, Map<relPath, Buffer>> of
  //       the full file tree of each pre-existing sdd-* skill dir, so that
  //       overwritten dirs can be fully restored on rollback (not just removed).
  //   codexPreInstallAgentFiles  — Set of sdd-*.{md,toml} filenames in agents/
  ...
  //   codexPreInstallVersionBytes — Buffer (or null) of sdd/VERSION
  //
  ...
  // pre-existing sdd-* skill directory. Best-effort: read errors are silently
  ...
  // Map<filename, Buffer> — content snapshot of each pre-existing sdd-* agent file.
  ...
  if (isCodex && !isMinimalMode(_effectiveInstallMode)) {
    ...
    if (fs.existsSync(_preAgentsDir)) {
      for (const file of fs.readdirSync(_preAgentsDir)) {
        if (file.startsWith('sdd-') && (file.endsWith('.md') || file.endsWith('.toml'))) {
    ...
    const _preVersionPath = path.join(targetDir, 'sdd', 'VERSION');
  ...
  // config.toml is touched: skills/, agents/, sdd/VERSION, and orphaned
  ...
  const _codexPreConfigRollback = !isCodex || isMinimalMode(_effectiveInstallMode) ? null : () => {
    ...
    // skills/sdd-* — pass 1: restore snapshot entries (may be absent if deleted mid-install).
    ...
    // skills/sdd-* — pass 2: remove any newly-created dirs not in the snapshot.
    if (fs.existsSync(_earlySkillsDir)) {
      try {
        for (const entry of fs.readdirSync(_earlySkillsDir, { withFileTypes: true })) {
          if (entry.isDirectory() && entry.name.startsWith('sdd-') && !codexPreInstallSkillNames.has(entry.name)) {
    ...
    // agents/sdd-* — pass 1: restore snapshot entries.
    ...
    // agents/sdd-* — pass 2: remove any newly-created files not in the snapshot.
    if (fs.existsSync(_earlyAgentsDir)) {
      try {
        for (const file of fs.readdirSync(_earlyAgentsDir)) {
          if (file.startsWith('sdd-') && (file.endsWith('.md') || file.endsWith('.toml')) && !codexPreInstallAgentFiles.has(file)) {
    ...
    // sdd/VERSION
    const _earlyVersionPath = path.join(targetDir, 'sdd', 'VERSION');
  ...
  // #3541: non-interactive runs (typical /sdd-update via Claude Code) have
  ...
  // artifacts → remove; user-facing skills → keep; bundled SDD hooks →
  ...
  // For unambiguous categories (e.g. `hooks/sdd-*` bundled hooks left
  // behind by a previous version), there is no actual "user choice" to
  // make — the file is a known SDD-managed artifact and the installer is
  // about to write the fresh bundled version. Gating the resolver on
  // `!isTTY` made `npx @bhargavvc/sdd-cc@latest --codex` hard-abort with
  // 12 blocked bundled hooks. The env-override branch (operator-supplied
  // SDD_INSTALLER_MIGRATION_RESOLVE) still applies only in non-TTY mode.
  ...
  if (isOpencode || isKilo) {
    ...
    const sddSrc = _stageSkills(_commandsDir);
    ...
    if (verifyInstalled(commandDir, 'command/sdd-*')) {
      ...
      failures.push('command/sdd-*');
    }
  } else if (isCodex) {
    // Codex CLI (0.130.0 at time of #3562) does NOT auto-discover commands
    // from sdd/workflows/*.md or agents/*.md. It only registers
    ...
    // no $sdd-* entrypoints. Regenerate the skill surface the same way the
    // other runtimes do — copyCommandsAsCodexSkills() rewrites each
    // commands/sdd/*.md as ~/.codex/skills/sdd-<name>/SKILL.md and converts
  ...
  // #3288 — Copy sdk/shared/model-catalog.json into the sdd payload
  // at the co-located path that model-catalog.cjs resolves first:
  //   sdd/bin/shared/model-catalog.json
  //
  // The install copies sdd/ but NOT sdk/ — the CJS module's legacy
  ...
  if (fs.existsSync(modelCatalogSrc)) {
    ...
    if (verifyFileInstalled(modelCatalogDest, 'sdd/bin/shared/model-catalog.json')) {
      console.log(`  ${green}✓${reset} Installed sdd/bin/shared/model-catalog.json`);
    } else {
      failures.push('sdd/bin/shared/model-catalog.json');
    }
  } else {
  ...
  const changelogDest = path.join(targetDir, 'sdd', 'CHANGELOG.md');
  ...
  // Write VERSION file
  const versionDest = path.join(targetDir, 'sdd', 'VERSION');
    ...
    if (fs.existsSync(hooksSrc)) {
      ...
      for (const entry of hookEntries) {
        ...
        if (fs.statSync(srcFile).isFile()) {
          ...
          // and stamp the current SDD version into the hook version header
  ...
  if (isCodex && !isMinimalMode(_effectiveInstallMode)) {
    // Capture pre-install snapshots before ANY SDD mutation
    ...
    //   skills/sdd-* — restore pre-existing dirs from content snapshot; remove
    //                   newly-created dirs (i.e. those not in the pre-install Set)
    //   agents/sdd-* — restore pre-existing files from content snapshot; remove
    //                   newly-created files
    //   sdd/VERSION — restore or remove
    ...
    // default to empty Set / null). Does NOT touch non-sdd-* user content.
    const restoreCodexSnapshot = () => {
      ...
      //     (copyCommandsAsCodexSkills removes pre-existing sdd-* dirs before re-writing)
      ...
      // Pass 2 — remove any newly-created sdd-* dirs (not in the pre-install snapshot).
      if (fs.existsSync(_rollbackSkillsDir)) {
        try {
          for (const entry of fs.readdirSync(_rollbackSkillsDir, { withFileTypes: true })) {
            if (!entry.isDirectory() || !entry.name.startsWith('sdd-')) continue;
      ...
      // Pass 2 — remove any newly-created sdd-* agent files (not in the pre-install snapshot).
      if (fs.existsSync(_rollbackAgentsDir)) {
        try {
          for (const file of fs.readdirSync(_rollbackAgentsDir)) {
            if (!file.startsWith('sdd-') || (!file.endsWith('.md') && !file.endsWith('.toml'))) continue;
      ...
      // 4. sdd/VERSION
      const _rollbackVersionPath = path.join(targetDir, 'sdd', 'VERSION');
      if (codexPreInstallVersionBytes !== null) {
    ...
    // sdd-check-update.js through hooks config — the file must physically exist.
    ...
    try {
      ...
      // SDD-managed Codex hook payloads now live in hooks.json to avoid mixed
      ...
      if (hasEnabledCodexHooksFeature(configContent)) {
        ...
        if (!fs.existsSync(checkUpdateFile)) {
          console.warn(`  ${yellow}⚠${reset}  Skipped Codex SessionStart hook registration — sdd-check-update.js not found at target`);
  ...
  if (isCopilot) {
    ...
    const templatePath = path.join(targetDir, 'sdd', 'templates', 'copilot-instructions.md');
    const instructionsPath = path.join(targetDir, 'copilot-instructions.md');
  ...
  if (!isOpencode && !isKilo) {
    ...
    if (!hasContextMonitorHook && fs.existsSync(contextMonitorFile) && contextMonitorCommand) {
      ...
      for (const entry of settings.hooks[postToolEvent]) {
        if (entry.hooks && entry.hooks.some(h => h.command && h.command.includes('sdd-context-monitor'))) {
          let migrated = false;
          if (!entry.matcher) {
          ...
          for (const h of entry.hooks) {
            if (h.command && h.command.includes('sdd-context-monitor') && !h.timeout) {
              h.timeout = 10;
              migrated = true;
            }
          }
    ...
    const sessionStateCommand = isGlobal
      ...
      : localShellCmd('sdd-session-state.sh');
    ...
    const phaseBoundaryCommand = isGlobal
      ...
      : localShellCmd('sdd-phase-boundary.sh');
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = 'claude', isGlobal = true, configDir = null, bannerOpts = {}) {
  ...
  // See #1156.
  if (runtime !== 'claude') {
    const sddDir = path.join(os.homedir(), '.sdd');
    const defaultsPath = path.join(sddDir, 'defaults.json');
    try {
      fs.mkdirSync(sddDir, { recursive: true });
      let defaults = {};
      ...
      if (defaults.resolve_model_ids !== 'omit') {
        ...
        console.log(`  ${green}✓${reset} Set resolve_model_ids: "omit" in ~/.sdd/defaults.json`);
      }
    } catch (e) {
      console.log(`  ${yellow}⚠${reset} Could not write ~/.sdd/defaults.json: ${e.message}`);
    }
  }
...
function handleStatusline(settings, isInteractive, callback) {
  ...
  SDD includes a statusline showing:
    • Model name
    • Current task (from todo list)
    • Context window usage (color-coded)

  ${cyan}1${reset}) Keep existing
  ${cyan}2${reset}) Replace with SDD statusline
`);
...
function installSdkIfNeeded(opts) {
  ...
  if (!fs.existsSync(sdkCliPath)) {
    ...
    if (opts.throwOnFailure) {
      const error = new Error(`SDD SDK prebuilt artifact missing: ${sdkCliPath}`);
      error.code = 'SDD_SDK_MISSING_DIST';
      error.exitCode = 1;
      throw error;
    }
    process.exit(1);
  ...
  // shell. A sdd-sdk found there must NOT count as "on PATH".
  const shimSrc = path.resolve(__dirname, 'sdd-sdk.js');
  const persistentPath = filterNpxFromPath(process.env.PATH || '');
  let resolvedSdkPath = findSddSdkOnPath(persistentPath);
  ...
  if (!onPath) {
    ...
    if (linked) {
      ...
      resolvedSdkPath = findSddSdkOnPath(persistentPath);
  ...
  if (onPath && userShellPath !== null) {
    ...
    const userSdkPath = findSddSdkOnPath(persistentUserShellPath);
    if (!userSdkPath) {
      onPath = false;
  ...
  if (onPath) {
    const versionReport = buildSddSdkVersionMismatchReport(resolvedSdkPath, pkg.version);
    if (versionReport) {
      renderSddSdkVersionMismatchReport(versionReport);
    } else {
...
/**
 * #3231 helper: detect whether a `sdd-sdk` binary is the legacy deprecated
 * shim pointing at `sdd-tools.cjs`.
 *
 * Reads the first 512 bytes of the file and looks for the `@deprecated`
 * marker alongside a `sdd-tools.cjs` reference — the fingerprint that
 ...
 * shim in place).
 */
function isLegacySddSdkShim(filePath) {
  const fs = require('fs');
  try {
    ...
    // The legacy binary contains "@deprecated" AND "sdd-tools.cjs" within
    // its first 512 bytes.
    return header.includes('@deprecated') && header.includes('sdd-tools.cjs');
  } catch {
    return false;
  }
}
 ...
 * evidence that `sdd-sdk` is durably reachable.
 *
...
/**
 * #2775 helper: find a callable `sdd-sdk` on a PATH.
 *
 ...
 * isLegacySddSdkShim — a symlink pointing at the deprecated sdd-tools.cjs
 * binary must NOT be treated as "on PATH" even if it is executable.
 */
function findSddSdkOnPath(pathString) {
  const path = require('path');
  const fs = require('fs');
  ...
  for (const seg of pathEnv.split(path.delimiter)) {
    ...
    for (const ext of exts) {
      ...
      try {
        ...
        if (st.isFile()) {
          if (process.platform === 'win32') {
            if (!isLegacySddSdkShim(candidate)) return candidate;
            ...
            if (!isLegacySddSdkShim(target)) return candidate;
          }
        }
      } catch {
...
function isSddSdkOnPath(pathString) {
  return !!findSddSdkOnPath(pathString);
}

function parseSddSdkVersion(text) {
  const match = String(text || '').match(/\bv?(\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?)\b/);
  return match ? match[1] : null;
}

function readSddSdkVersion(sdkPath) {
  ...
  try {
    ...
    return parseSddSdkVersion(`${result.stdout || ''}\n${result.stderr || ''}`);
  } catch {
    return null;
  }
}

function buildSddSdkVersionMismatchReport(sdkPath, expectedVersion) {
  const actualVersion = readSddSdkVersion(sdkPath);
  if (!actualVersion || !expectedVersion) return null;
  if (actualVersion === expectedVersion) return null;
  return {
    ok: false,
    reason: 'sdd_sdk_version_mismatch',
    sdk_path: sdkPath,
...
function renderSddSdkVersionMismatchReport(ir) {
  console.log('');
  console.log(`  ${yellow}⚠${reset} ${bold}sdd-sdk version mismatch${reset} — PATH resolves a stale SDK.`);
  console.log(`    Resolved sdd-sdk: ${ir.sdk_path}`);
  console.log(`    Resolved version: ${ir.actual_version}`);
  console.log(`    Installer version: ${ir.expected_version}`);
  console.log(`    Workflows that call ${cyan}sdd-sdk query …${reset} will use the stale executable first.`);
 ...
 * warning when sdd-sdk is in a machine-level bin dir (e.g. C:\Program Files\nodejs).
 *
...
function getUserShellWindowsPersistentPath() {
  ...
  try {
    ...
    // warning when sdd-sdk is installed in a machine-level bin dir
    // (e.g. C:\Program Files\nodejs).
    const out = execFile(
      'powershell.exe',
      [
...
function installAllRuntimes(runtimes, isGlobal, isInteractive) {
  ...
  const installerMigrations = discoverInstallerMigrations({
    migrationsDir: path.join(_sddLibDir, 'installer-migrations'),
  });
