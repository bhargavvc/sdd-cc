...
- [v1.35.0 Features](#v1350-features)
  ...
  - [SDD-2 Reverse Migration](#105-sdd-2-reverse-migration)
...
**Command:** `/sdd-new-project [--auto @file.md]`
...
- Existing codebase context (from `/sdd-map-codebase`) is loaded if present

---

### 2. Phase Discussion

**Command:** `/sdd-discuss-phase [N] [--auto] [--batch]`
...
**Command:** `/sdd-plan-phase [N] [--auto] [--skip-research] [--skip-verify]`
...
- REQ-PLAN-07: System MUST prompt user to run `/sdd-ui-phase` if frontend phase detected and no UI-SPEC.md exists (UI safety gate)
...
**Commands:** `/sdd-audit-milestone`, `/sdd-complete-milestone`, `/sdd-new-milestone [name]`
...
**Command:** `/sdd-quick [--full] [--discuss] [--research]`

**Purpose:** Ad-hoc task execution with SDD guarantees but a faster path.
...
**Command:** `/sdd-progress --do` (see also `/sdd-manager` for interactive routing)

**Purpose:** Analyze freeform text and route to the appropriate SDD command.

**Requirements:**
- REQ-DO-01: System MUST parse user intent from natural language input
- REQ-DO-02: System MUST map intent to the best matching SDD command
...
- REQ-NEXT-04: System MUST suggest `/sdd-new-project` if no project exists
- REQ-NEXT-05: System MUST suggest `/sdd-complete-milestone` when all phases are complete

**State Detection Logic:**
| State | Action |
|-------|--------|
| No `.planning/` directory | Suggest `/sdd-new-project` |
| Phase has no CONTEXT.md | Run `/sdd-discuss-phase` |
| Phase has no PLAN.md files | Run `/sdd-plan-phase` |
| Phase has plans but no SUMMARY.md | Run `/sdd-execute-phase` |
| Phase executed but no VERIFICATION.md | Run `/sdd-verify-work` |
| All phases complete | Suggest `/sdd-complete-milestone` |
...
- REQ-NYQ-05: System MUST support retroactive validation via `/sdd-validate-phase`
...
**Retroactive Validation (`/sdd-validate-phase [N]`):**
...
- REQ-POSTVER-03: System MUST log issues for `/sdd-verify-work` to address
...
**When:** Runs automatically during `/sdd-execute-phase` before the verifier step.
...
**When:** Runs automatically at the end of `/sdd-plan-phase` after the plan checker loop.
...
- REQ-CTX-06: Context monitor MUST differentiate SDD-active vs non-SDD-active projects
...
**Commands:** `/sdd-pause-work`, `/sdd-resume-work`, `/sdd-progress`
...
**Command:** `/sdd-map-codebase [area]`

**Purpose:** Analyze an existing codebase before starting a new project, so SDD understands what exists.
...
- REQ-MAP-04: Subsequent `/sdd-new-project` MUST load codebase mapping and focus questions on what's being added
...
**Purpose:** Update SDD to the latest version with changelog preview.
...
- REQ-SETTINGS-03: System MUST support saving as global defaults (`~/.sdd/defaults.json`)
...
**Purpose:** Run SDD across multiple AI coding agent runtimes.
...
- REQ-RUNTIME-05: Uninstall MUST cleanly remove all SDD files without affecting other configurations
...
**Command:** `/sdd-profile-user [--questionnaire] [--refresh]`
...
- `sdd-user-profiler` agent — Behavioral analysis from session data
...
Every `/sdd-progress` call scans ALL phases in the current milestone for outstanding items (pending, skipped, blocked, human_needed). Displays a non-blocking warning section with actionable links.
...
**Requirements:**
- REQ-DEBT-01: System MUST surface outstanding UAT/verification items from ALL prior phases in `/sdd-progress`
...
- REQ-DEBT-06: `/sdd-audit-uat` MUST scan all phases, categorize items by testability, and produce a human test plan
...
**Command:** `/sdd-fast [task description]`
...
**When to use vs `/sdd-quick`:**
- `/sdd-fast` — One-sentence tasks executable in under 2 minutes (typo, config change, small addition)
- `/sdd-quick` — Anything needing research, multi-step planning, or verification

---

### 42. Cross-AI Peer Review

**Command:** `/sdd-review --phase N [--gemini] [--claude] [--codex] [--coderabbit] [--opencode] [--qwen] [--cursor] [--ollama] [--lm-studio] [--llama-cpp] [--all]`
...
- REQ-REVIEW-05: Reviews MUST be consumable by `/sdd-plan-phase --reviews`
...
- Set `review.default_reviewers` in `.planning/config.json` (or via `sdd config-set`) to control no-flag `/sdd-review` fan-out.
...
- REQ-BACKLOG-02: Phase directories MUST be created immediately so `/sdd-discuss-phase` and `/sdd-plan-phase` work on them
- REQ-BACKLOG-03: `/sdd-review-backlog` MUST support promote, keep, and remove actions per item
...
- REQ-SEED-02: `/sdd-new-milestone` MUST scan seeds and present matches
...
**Command:** `/sdd-thread [name | description]`

**Purpose:** Lightweight cross-session knowledge stores for work that spans multiple sessions but doesn't belong to any specific phase. Lighter weight than `/sdd-pause-work` — no phase state, no plan context.
...
**Command:** `/sdd-pr-branch [target branch]`

**Purpose:** Create a clean branch suitable for pull requests by filtering out `.planning/` commits. Reviewers see only code changes, not SDD planning artifacts.
...
**Purpose:** Defense-in-depth security for SDD's planning artifacts. Because SDD generates markdown files that become LLM system prompts, user-controlled text flowing into these files is a potential indirect prompt injection vector.
...
**2. Prompt Injection Guard Hook** (`sdd-prompt-guard.js`)
...
**3. Workflow Guard Hook** (`sdd-workflow-guard.js`)
PreToolUse hook that detects when Claude attempts file edits outside a SDD workflow context. Advises using `/sdd-quick` or `/sdd-fast` instead of direct edits. Configurable via `hooks.workflow_guard` (default: false).
...
**Purpose:** Auto-generate `DISCUSSION-LOG.md` during `/sdd-discuss-phase` for full audit trail of decisions made during discussion.
...
**Command:** `/sdd-forensics [description]`

**Purpose:** Post-mortem investigation of failed or stuck SDD workflows.
...
**Command:** `/sdd-milestone-summary [version]`
...
2. **Switch** — Change active workstream context for subsequent SDD commands
...
**Command:** `/sdd-discuss-phase` with `workflow.discuss_mode: 'assumptions'`
...
**Part of:** `/sdd-new-project` and `/sdd-progress`

**Purpose:** Automatically detect UI-heavy projects and surface `/sdd-ui-phase` recommendation.
...
- REQ-UI-DETECT-03: System MUST suggest `/sdd-ui-phase` in next steps for UI-heavy phases
- REQ-UI-DETECT-04: System MUST NOT make `/sdd-ui-phase` mandatory
...
3. **Surface** — Include `/sdd-ui-phase` recommendation in next steps for UI-heavy phases

---

### 55. Multi-Runtime Installer Selection

**Part of:** `npx @bhargavvc/sdd-cc`
...
3. **Install** — Configure SDD for all selected runtimes in a single session
...
**Purpose:** Add Windsurf as a supported AI CLI runtime for SDD installation and execution.

**Requirements:**
- REQ-WINDSURF-01: Installer MUST detect Windsurf runtime and offer it as a target
- REQ-WINDSURF-02: SDD commands MUST function correctly within Windsurf sessions

**Process:**
1. **Detect** — Identify Windsurf runtime availability on the system
2. **Install** — Configure SDD skills and hooks for the Windsurf environment
...
**Purpose:** Provide SDD documentation in Portuguese, Korean, and Japanese.
...
**Purpose:** Headless TypeScript SDK for running SDD workflows programmatically without a CLI session.

**Requirements:**
- REQ-SDK-01: SDK MUST expose SDD workflow operations as TypeScript functions
...
**Process:**
1. **Import** — Import SDD SDK into a TypeScript/JavaScript project
2. **Configure** — Set project path and workflow options programmatically
3. **Execute** — Run SDD phases (discuss, plan, execute) via API calls
...
**Command:** Automatic during `/sdd-execute-phase`
...
- REQ-SCHEMA-04: System MUST support `SDD_SKIP_SCHEMA_CHECK` env var to override detection
...
**Config:** `SDD_SKIP_SCHEMA_CHECK` environment variable to bypass detection.

---

### 60. Security Enforcement

**Command:** `/sdd-secure-phase <N>`
...
- REQ-SEC-04: System MUST spawn `sdd-security-auditor` agent for analysis
...
2. **Audit** — Spawn `sdd-security-auditor` to verify against threat model
...
**Command:** `/sdd-docs-update`

**Purpose:** Generate and verify project documentation with accuracy checks.

**Requirements:**
- REQ-DOCS-01: System MUST spawn `sdd-doc-writer` agent to generate documentation
- REQ-DOCS-02: System MUST spawn `sdd-doc-verifier` agent to check accuracy
...
**Process:**
1. **Generate** — Spawn `sdd-doc-writer` to create or update documentation from implementation
2. **Verify** — Spawn `sdd-doc-verifier` to check documentation accuracy against codebase
...
**Flag:** `/sdd-discuss-phase <N> --chain`
...
**Purpose:** Migrate SDD commands to Claude Code 2.1.88+ skills format with backward compatibility.

**Requirements:**
- REQ-SKILLS-01: Installer MUST write `skills/sdd-*/SKILL.md` for Claude Code 2.1.88+
- REQ-SKILLS-02: Installer MUST auto-clean legacy `commands/sdd/` directory
...
2. **Migrate** — Write `skills/sdd-*/SKILL.md` files for each SDD command
3. **Clean** — Remove legacy `commands/sdd/` directory if skills are installed
...
**Part of:** `/sdd-execute-phase` (verifier step)
...
**Part of:** SDD SDK prompt assembly
...
**Part of:** `/sdd-verify-work` (optional)
...
**Purpose:** Extend SDD installation to Trae IDE, Cline, and Augment Code runtimes.
...
**Flag:** `/sdd-autonomous --interactive`
...
**Hooks:** `sdd-validate-commit.sh`, `sdd-session-state.sh`, `sdd-phase-boundary.sh`

**Purpose:** Optional git and session hooks for SDD projects, gated behind `hooks.community: true` in config.

**Requirements:**
- REQ-COMMUNITY-01: All community hooks MUST be no-ops unless `hooks.community` is `true` in `.planning/config.json`
- REQ-COMMUNITY-02: `sdd-validate-commit.sh` MUST enforce Conventional Commits format on git commit messages
- REQ-COMMUNITY-03: `sdd-session-state.sh` MUST track session state transitions
- REQ-COMMUNITY-04: `sdd-phase-boundary.sh` MUST enforce phase boundary checks
...
**Command:** `/sdd-map-codebase --query [<term>|status|diff|refresh]`
...
**References:** `sdd/references/gates.md`
...
**Command:** `/sdd-explore [topic]`

**Purpose:** Guide a developer through exploring an idea via Socratic probing questions before committing to a plan. Routes outputs to the appropriate SDD artifact: notes, todos, seeds, research questions, requirements updates, or a new phase.

**Requirements:**
- REQ-EXPLORE-01: Exploration MUST use Socratic probing — ask questions before proposing solutions
- REQ-EXPLORE-02: Session MUST offer to route outputs to the appropriate SDD artifact
...
**Command:** `/sdd-undo --last N | --phase NN | --plan NN-MM`

**Purpose:** Roll back SDD phase or plan commits safely using the phase manifest and git log, with dependency checks and a hard confirmation gate before any revert is applied.
...
- REQ-UNDO-03: `--last N` mode MUST display recent SDD commits for interactive selection
...
**Command:** `/sdd-import --from <filepath>`

**Purpose:** Ingest an external plan file into the SDD planning system with conflict detection against `PROJECT.md` decisions, converting it to a valid SDD PLAN.md and validating it through the plan-checker.
...
- REQ-IMPORT-03: Imported plan MUST be written as a valid SDD PLAN.md format
- REQ-IMPORT-04: Written plan MUST pass `sdd-plan-checker` validation

---

### 97. Rapid Codebase Scan

**Command:** `/sdd-map-codebase --fast [--focus tech|arch|quality|concerns]`

**Purpose:** Lightweight alternative to `/sdd-map-codebase` that spawns a single mapper agent for one or two combined focus areas, producing targeted output in `.planning/codebase/` without the overhead of 4 parallel agents.
...
- REQ-SCAN-03: Output MUST be written to `.planning/codebase/` in the same format as `/sdd-map-codebase`

---

### 98. Autonomous Audit-to-Fix

**Command:** `/sdd-audit-fix [--source <audit>] [--severity high|medium|all] [--max N] [--dry-run]`
...
**Purpose:** Extend SDD installation to Cline, CodeBuddy, and Qwen Code runtimes.
...
- REQ-CODEBUDDY-01: CodeBuddy install MUST deploy skills to `~/.codebuddy/skills/sdd-*/SKILL.md`. Flag: `--codebuddy`.
- REQ-QWEN-01: Qwen Code install MUST deploy skills to `~/.qwen/skills/sdd-*/SKILL.md`, following the open standard used by Claude Code 2.1.88+. `QWEN_CONFIG_DIR` env var overrides the default path. Flag: `--qwen`.
...
### 105. SDD-2 Reverse Migration

**Command:** `/sdd-import --from-sdd2 [--dry-run] [--force] [--path <dir>]`

**Purpose:** Migrate a project from SDD-2 format (`.sdd/` directory with Milestone→Slice→Task hierarchy) back to the v1 `.planning/` format, restoring full compatibility with all SDD v1 commands.

**Requirements:**
- REQ-FROM-SDD2-01: Importer MUST read `.sdd/` from the specified or current directory
- REQ-FROM-SDD2-02: Milestone→Slice hierarchy MUST be flattened to sequential phase numbers (M001/S01→phase 01, M001/S02→phase 02, M002/S01→phase 03, etc.)
- REQ-FROM-SDD2-03: System MUST guard against overwriting an existing `.planning/` directory without `--force`
- REQ-FROM-SDD2-04: `--dry-run` MUST preview all changes without writing any files
- REQ-FROM-SDD2-05: Migration MUST produce `PROJECT.md`, `REQUIREMENTS.md`, `ROADMAP.md`, `STATE.md`, and sequential phase directories
...
| `--path <dir>` | Specify the SDD-2 root directory |

---

### 106. AI Integration Phase Wizard

**Command:** `/sdd-ai-integration-phase [N]`
...
**Command:** `/sdd-plan-phase N --bounce`
...
**Command:** `/sdd-ship` (enhanced)

**Purpose:** Before the manual review step in `/sdd-ship`, automatically run an external code review command if configured. The command receives the diff and phase context via stdin and returns a JSON verdict (`APPROVED` or `REVISE`). Falls through to the existing manual review flow regardless of outcome.
...
**Command:** `/sdd-execute-phase N --cross-ai`
...
**Command:** `/sdd-plan-phase` (enhanced research step)
...
**Command:** `/sdd-extract-learnings N`
...
**Command:** `sdd-sdk init @prd.md --ws my-workstream`
...
**Purpose:** Allow projects to store their CLAUDE.md in a non-root location. The `claude_md_path` config key controls where `/sdd-profile-user` and related commands write the generated CLAUDE.md file.
...
**Requirements:**
- REQ-BUDGET-01: `agents/sdd-*.md` files are classified into three tiers: XL (≤ 1 600 lines), Large (≤ 1 000 lines), Default (≤ 500 lines)
...
**Commands:**
- `/sdd-workflow` — phase pipeline router (discuss / plan / execute / verify / phase / progress)
...
- `/sdd-manage` — config / workspace / workstreams / thread / update / ship / inbox
...
|------|-----------------|
| `planning` | `sdd-planner`, `sdd-roadmapper`, `sdd-pattern-mapper` |
| `discuss` | (reserved for future subagent) |
| `research` | `sdd-phase-researcher`, `sdd-project-researcher`, `sdd-research-synthesizer`, `sdd-codebase-mapper`, `sdd-ui-researcher` |
| `execution` | `sdd-executor`, `sdd-debugger`, `sdd-doc-writer` |
| `verification` | `sdd-verifier`, `sdd-plan-checker`, `sdd-integration-checker`, `sdd-nyquist-auditor`, `sdd-ui-checker`, `sdd-ui-auditor`, `sdd-doc-verifier` |
...
**Reference issue:** [#3023](https://github.com/bhargavvc/sdd-cc/pull/3030)
...
**Reference issue:** [#3024](https://github.com/bhargavvc/sdd-cc/pull/3031)

---

### 128. Update Banner Opt-In

**Purpose:** Surface update availability to users who have declined or bypassed the SDD statusline, without requiring the statusline.

**Behavior:**
- At install time, if the installer detects no SDD statusline, it offers an opt-in `SessionStart` hook.
- The hook reads the existing `~/.cache/sdd/sdd-update-check.json` cache — the same cache used by the statusline — and prints a banner only when an update is available.
- Silent when up-to-date.
- Failure diagnostics rate-limited to once per 24 h.
- Cleanly removed by `npx @bhargavvc/sdd-cc --uninstall`.
...
**Reference issue:** [#2795](https://github.com/bhargavvc/sdd-cc/pull/2795)

---

### 129. Issue-Driven Orchestration Guide

**Purpose:** Document a recipe for driving the full SDD workflow from a GitHub / Linear / Jira issue, mapping tracker-centric concepts onto existing SDD primitives.

**Document:** [`docs/issue-driven-orchestration.md`](issue-driven-orchestration.md)

**Covered workflow:**
1. Create an isolated workspace per issue (`/sdd-workspace --new`)
2. Run the manager dashboard to get oriented (`/sdd-manager`)
3. Execute autonomously (`/sdd-autonomous`)
4. Verify and review (`/sdd-verify-work`, `/sdd-review`)
5. Ship and close the issue (`/sdd-ship`)
...
**Reference issue:** [#2840](https://github.com/bhargavvc/sdd-cc/pull/2840)
...
**Reference issue:** [#3170](https://github.com/bhargavvc/sdd-cc/issues/3170)
...
|------|---------|---------|
| `sdd-sdk query phase.mvp-mode <N>` | `{active, source, roadmap_mode, config_mvp_mode, cli_flag_present}` | `plan-phase`, `execute-phase`, `verify-work`, `progress` |
| `sdd-sdk query task.is-behavior-adding <plan-file>` | `{is_behavior_adding, checks: {tdd_true, has_behavior_block, has_source_files}, reason}` | `sdd-executor` agent |
| `sdd-sdk query user-story.validate "<text>"` | `{valid, slots: {role, capability, outcome}, errors[]}` | `sdd-verifier`, `/sdd-mvp-phase` |
...
**Reference issue:** [#3178](https://github.com/bhargavvc/sdd-cc/pull/3178)
...
**Runtime control:** `/sdd:surface` lists profile state and enables, disables, or resets skill clusters without reinstalling.

**Requirements:**
- REQ-SURFACE-01: Installer MUST resolve `--profile=<name>` and persist the active profile in `.sdd-profile`.
...
- Ambiguous SDD-looking files block with a clear report instead of being silently overwritten.
...
**Purpose:** Add project-specific PRD-style sections to generated PR bodies without editing SDD workflow files.
...
**Purpose:** Let teams choose the default reviewer subset for no-flag `/sdd-review` runs.
...
**Reference:** [Configuration Reference](CONFIGURATION.md#reviewer-defaults-for-sdd-review)

---

### 137. Fallow Structural Review Pre-Pass

**Command:** `/sdd-code-review`

**Config keys:** `code_quality.fallow.*`

**Purpose:** Add an optional structural analysis pass before the agent review.

**Behavior:** When enabled, SDD resolves a `fallow` binary, runs a bounded audit, writes `FALLOW.json`, and embeds structural findings in `REVIEW.md`.
...
**Reference:** [Checkpoints Reference](../sdd/references/checkpoints.md)

---

### 139. Quota and Rate-Limit Failure Classification

**Command:** `/sdd-execute-phase`
...
**Command:** `/sdd-complete-milestone`
