...
The **canary** dist-tag is SDD's earliest preview channel. It exists so contributors and willing early adopters can exercise in-flight features against the long-lived `dev` integration branch before they have any expectation of stability.

## Stream policy

SDD ships through three npm dist-tags, each fed by exactly one git branch. **Streams do not mix.**
...
npm install -g @bhargavvc/sdd-cc@canary

# Pin to an exact canary version
npm install -g @bhargavvc/sdd-cc@1.50.0-canary.1
```

The CC installer's defensive purge rewrites stale config blocks left by older SDD versions, so reinstalling on top of an existing project is safe.
...
- A machine where rolling back means recreating SDD state (use a profile or a workspace instead)
...
If you have a local project that interacted with canary-only features (for instance, an MVP-mode phase planned by 1.50.0-canary), the planner artifacts in `.planning/` remain valid — older SDD versions will just ignore the `**Mode:** mvp` field on phases.

## Reporting issues against canary

File against the [issue tracker](https://github.com/bhargavvc/sdd-cc/issues) with the `bug` template. Include the exact canary version (`@bhargavvc/sdd-cc --version` reports it) so triage can route the report back into the `dev` stream rather than the stable stream.
...
- Stream architecture rationale: discussed across [#2727](https://github.com/bhargavvc/sdd-cc/issues/2727), [#2773](https://github.com/bhargavvc/sdd-cc/issues/2773) (codex schema-break and the resulting promotion bottleneck that motivated explicit stream isolation)
