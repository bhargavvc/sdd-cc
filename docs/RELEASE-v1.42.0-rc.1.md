...
# or pin exact:
npm install -g @bhargavvc/sdd-cc@1.42.0-rc1
```
...
This RC also rolls up every fix that shipped in [v1.41.1](https://github.com/bhargavvc/sdd-cc/releases/tag/v1.41.1). Those fixes are listed in the v1.41.1 notes and on the GitHub release page; this document is scoped to the **new features** in 1.42.0.
...
#### Package legitimacy gate against slopsquatting ([#3215](https://github.com/bhargavvc/sdd-cc/pull/3215))

A three-layer defense across the research → plan → execute pipeline. Before this release, a hallucinated package name that passed `npm view` could flow undetected into `sdd-executor` running `npm install <malicious-pkg>` with no human gate. The gate closes that path:

- **Layer 1 — Researcher (`agents/sdd-phase-researcher.md`).** A new `<package_legitimacy_protocol>` block runs `slopcheck install <pkgs> --json` over every recommended package, performs ecosystem-specific verification (`pip index versions` / `npm view` / `cargo search`), and emits a `## Package Legitimacy Audit` table to `RESEARCH.md` with Package, Registry, Age, Downloads, Source Repo, slopcheck, and Disposition columns. Packages discovered solely through WebSearch are tagged `[ASSUMED]` — never `[VERIFIED]`. `[SLOP]` packages are removed from RESEARCH.md and listed under "Packages removed due to slopcheck."
- **Layer 2 — Planner (`agents/sdd-planner.md`).** Reads the Audit table and inserts a `checkpoint:human-verify` task before any install whose package is tagged `[ASSUMED]` or `[SUS]`. Plans that introduce installs gain a `T-{phase}-SC` Tampering / supply-chain row in their `<threat_model>` template.
- **Layer 3 — Executor (`agents/sdd-executor.md`).** RULE 3 amended: package installs (`npm`/`pip`/`cargo`) are excluded from auto-fix scope. Failed installs become `checkpoint:human-verify` with a slopsquatting-risk rationale instead of being silently retried.
...
**Documentation.** `docs/USER-GUIDE.md` has a new "Package Legitimacy Gate" subsection in the Security section; `docs/COMMANDS.md` notes the gate on `/sdd-plan-phase`; `docs/ARCHITECTURE.md` documents the gate before the Security Hooks section and updates the plan-phase pipeline diagram with the gate steps.

Closes [#2827](https://github.com/bhargavvc/sdd-cc/issues/2827).
...
#### SDK package seam deepened; runtime-global skills policy converged ([#3238](https://github.com/bhargavvc/sdd-cc/pull/3238))
...
Closes [#3237](https://github.com/bhargavvc/sdd-cc/issues/3237). Refs [#3234](https://github.com/bhargavvc/sdd-cc/issues/3234).

#### Phase lifecycle seams deepened ([#3267](https://github.com/bhargavvc/sdd-cc/pull/3267))
...
Closes [#3270](https://github.com/bhargavvc/sdd-cc/issues/3270).

---

## What was in 1.41.x

- **[v1.41.1](https://github.com/bhargavvc/sdd-cc/releases/tag/v1.41.1)** — 14-fix hotfix: phase-plan-index DAG correctness, state-snapshot YAML frontmatter precedence, code-review SUMMARY parser hardening (`BL-` / `blocker:` accepted as Critical-tier), Codex install TOML floats + idempotent rollback, persistent SDK reachability probe, shared model-catalog source of truth (ADR-0003), and more.
- **[v1.41.0](https://github.com/bhargavvc/sdd-cc/releases/tag/v1.41.0)** — six namespace meta-skills, `/sdd-health --context` utilization guard, `--minimal` install flag, `/sdd-edit-phase`, post-merge build & test gate, manual canary release workflow, and 25+ correctness fixes. See [`RELEASE-v1.41.0.md`](RELEASE-v1.41.0.md).
