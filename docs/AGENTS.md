...
SDD uses a multi-agent architecture where thin orchestrators (workflow files) spawn specialized agents with fresh context windows. Each agent has a focused role, limited tool access, and produces specific artifacts.
...
**Role:** Executes SDD plans with atomic commits, deviation handling, and checkpoint protocols.
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/sdd-map-codebase`, post-execute drift gate in `/sdd-execute-phase` |
...
Twelve additional agents ship under `agents/sdd-*.md` and are used by specialty workflows (`/sdd-ai-integration-phase`, `/sdd-eval-review`, `/sdd-code-review`, `/sdd-code-review --fix`, `/sdd-debug`, `/sdd-map-codebase --query`, `/sdd-select-framework`, `/sdd-ingest-docs`) and by the planner pipeline. Each carries full frontmatter in its agent file; the stubs below are concise by design. The authoritative roster (with spawner and primary-doc status per agent) lives in [`docs/INVENTORY.md`](INVENTORY.md).
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/sdd-plan-phase` (between research and planning) |
| **Parallelism** | Single instance |
...
### sdd-debug-session-manager

**Role:** Runs the full `/sdd-debug` checkpoint-and-continuation loop in an isolated context so the orchestrator's main context stays lean; spawns `sdd-debugger` agents, dispatches specialist skills, and handles user checkpoints via AskUserQuestion.

| Property | Value |
|----------|-------|
| **Spawned by** | `/sdd-debug` |
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/sdd-code-review --fix` |
| **Parallelism** | Single instance |
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/sdd-ai-integration-phase` |
...
**Required reading:** `sdd/references/ai-evals.md` (evaluation framework).
...
**Required reading:** `sdd/references/ai-evals.md`.
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/sdd-ai-integration-phase`, `/sdd-select-framework` |
| **Parallelism** | Single instance (interactive) |
...
**Required reading:** `sdd/references/ai-frameworks.md` (decision matrix).
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/sdd-map-codebase --query` (refresh / update flows) |
| **Parallelism** | Single instance |
| **Tools** | Read, Write, Bash, Glob, Grep |
| **Model (balanced)** | Sonnet |
| **Color** | Cyan |
| **Produces** | `.planning/intel/*.json` (and companion Markdown) consumed by `sdd-sdk query intel` |

**Key behaviors:**
- Writes current state only — no temporal language, every claim references an actual file path
- Uses Glob / Read / Grep for cross-platform correctness; Bash is reserved for `sdd-sdk query intel` CLI calls

---

### sdd-doc-classifier

**Role:** Classifies a single planning document as ADR, PRD, SPEC, DOC, or UNKNOWN. Extracts title, scope summary, and cross-references. Writes a JSON classification file used by `sdd-doc-synthesizer` to build a consolidated context.

| Property | Value |
|----------|-------|
| **Spawned by** | `/sdd-ingest-docs` (parallel fan-out over the doc corpus) |
...
| Property | Value |
|----------|-------|
| **Spawned by** | `/sdd-ingest-docs` (after classifier fan-in) |
| **Parallelism** | Single instance |
...
- Follows the `references/doc-conflict-engine.md` contract so `/sdd-import` and `/sdd-ingest-docs` produce consistent conflict reports

---

## Agent Tool Permissions Summary

> **Scope:** this table covers the 21 primary agents only. The 12 advanced/specialized agents listed above carry their own tool surfaces in their `agents/sdd-*.md` frontmatter (summarized in the per-agent stubs above and in [`docs/INVENTORY.md`](INVENTORY.md)).
