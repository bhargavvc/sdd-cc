...
```
  ...
  └─────────────────────────┬────────────────────────┘
             ...
             │                            │
             │  ┌────────────────────┐    │
             │  │ /sdd-discuss-phase │    │  <- 锁定偏好
             │  └──────────┬─────────┘    │
             │             │              │
             │  ┌──────────▼─────────┐    │
             │  │ /sdd-plan-phase    │    │  <- 研究 + 规划 + 验证
             │  └──────────┬─────────┘    │
             │             │              │
             │  ┌──────────▼─────────┐    │
             │  │ /sdd-execute-phase │    │  <- 并行执行
             │  └──────────┬─────────┘    │
             │             │              │
             │  ┌──────────▼─────────┐    │
             │  │ /sdd-verify-work   │    │  <- 手动 UAT
             │  └──────────┬─────────┘    │
             │             │              │
             │     下一阶段?────────────┘
             │             │ 否
...
在 plan-phase 研究期间，SDD 现在在任何代码编写之前将自动化测试覆盖率映射到每个阶段需求。这确保当 Claude 的执行者提交任务时，反馈机制已经存在可以在几秒钟内验证它。
...
**禁用：** 在 `/sdd-settings` 中设置 `workflow.nyquist_validation: false`，用于测试基础设施不是重点的快速原型阶段。

### 追溯验证 (`/sdd-validate-phase`)
...
**何时使用：** 在启用了 Nyquist 之前规划的阶段执行后，或在 `/sdd-audit-milestone` 发现 Nyquist 合规缺口后。
...
| 命令 | 用途 | 何时使用 |
|---------|---------|-------------|
| `/sdd-new-project` | 完整项目初始化：提问、研究、需求、路线图 | 新项目开始时 |
| `/sdd-new-project --auto @idea.md` | 从文档自动初始化 | 有现成的 PRD 或想法文档 |
| `/sdd-discuss-phase [N] [--chain] [--power]` | 捕获实现决策（`--chain` 自动链式，`--power` 文件批量输入） | 规划前，塑造构建方式 |
| `/sdd-plan-phase [N]` | 研究 + 规划 + 验证 | 执行阶段前 |
| `/sdd-execute-phase <N>` | 在并行波次中执行所有计划 | 规划完成后 |
| `/sdd-verify-work [N]` | 带自动诊断的手动 UAT | 执行完成后 |
| `/sdd-audit-milestone` | 验证里程碑达到其完成定义 | 完成里程碑前 |
| `/sdd-complete-milestone` | 归档里程碑，标记发布 | 所有阶段已验证 |
| `/sdd-new-milestone [name]` | 开始下一个版本周期 | 完成里程碑后 |

### 导航

| 命令 | 用途 | 何时使用 |
|---------|---------|-------------|
| `/sdd-progress` | 显示状态和下一步 | 任何时候 -- "我在哪?" |
| `/sdd-resume-work` | 从上次会话恢复完整上下文 | 开始新会话 |
...
SDD 在 `.planning/config.json` 中存储项目设置。在 `/sdd-new-project` 期间配置或稍后用 `/sdd-settings` 更新。
...
| `git.phase_branch_template` | 模板字符串 | `sdd/phase-{phase}-{slug}` | 阶段策略的分支名 |
| `git.milestone_branch_template` | 模板字符串 | `sdd/{milestone}-{slug}` | 里程碑策略的分支名 |
...
|-------|-----------|------------|----------|
| sdd-planner | Opus | Opus | Sonnet |
| sdd-roadmapper | Opus | Sonnet | Sonnet |
| sdd-executor | Opus | Sonnet | Sonnet |
| sdd-phase-researcher | Opus | Sonnet | Haiku |
| sdd-project-researcher | Opus | Sonnet | Haiku |
| sdd-research-synthesizer | Sonnet | Sonnet | Haiku |
| sdd-debugger | Opus | Sonnet | Sonnet |
| sdd-codebase-mapper | Sonnet | Haiku | Haiku |
| sdd-verifier | Sonnet | Sonnet | Haiku |
| sdd-plan-checker | Sonnet | Sonnet | Haiku |
| sdd-integration-checker | Sonnet | Sonnet | Haiku |
...
/sdd-new-project            # 回答问题，配置，批准路线图
/clear
/sdd-discuss-phase 1        # 锁定你的偏好
/sdd-plan-phase 1           # 研究 + 规划 + 验证
/sdd-execute-phase 1        # 并行执行
/sdd-verify-work 1          # 手动 UAT
/clear
/sdd-discuss-phase 2        # 对每个阶段重复
...
/sdd-audit-milestone        # 检查所有内容已发布
...
```bash
/sdd-new-project --auto @prd.md   # 从你的文档自动运行研究/需求/路线图
/clear
/sdd-discuss-phase 1               # 从这里开始正常流程
```

### 现有代码库

```bash
/sdd-map-codebase           # 分析现有内容（并行代理）
/sdd-new-project            # 问题聚焦于你正在添加的内容
# （从这里开始正常阶段工作流）
```
...
```bash
/sdd-progress               # 查看你停在哪和接下来做什么
# 或
/sdd-resume-work            # 从上次会话完整恢复上下文
```
...
你运行了 `/sdd-new-project` 但 `.planning/PROJECT.md` 已存在。这是安全检查。如果你想重新开始，先删除 `.planning/` 目录。

### 长会话期间上下文退化

在主要命令之间清除上下文窗口：Claude Code 中的 `/clear`。SDD 设计围绕全新上下文 —— 每个子代理获得干净的 200K 窗口。如果主会话质量下降，清除并使用 `/sdd-resume-work` 或 `/sdd-progress` 恢复状态。
...
运行 `/sdd-progress`。它读取所有状态文件，准确告诉你位置和下一步。

### 执行后需要更改某些内容

不要重新运行 `/sdd-execute-phase`。使用 `/sdd-quick` 进行针对性修复，或用 `/sdd-verify-work` 通过 UAT 系统识别和修复问题。
...
```bash
node sdd-tools.cjs state validate          # 检测 STATE.md 与文件系统之间的偏差
node sdd-tools.cjs state sync --verify     # 预览 sync 将更改的内容
node sdd-tools.cjs state sync              # 从磁盘重建 STATE.md
```
...
`/sdd-plan-phase` 在规划开始前会检查 RESEARCH.md 是否存在未解决的开放问题。如果存在未解决的问题，规划将被阻止，系统会显示需要解决的具体问题。这防止了基于不完整信息构建计划。
...
在 `/sdd-new-project` 期间或通过 `/sdd-settings` 设置 `commit_docs: false`。将 `.planning/` 添加到 `.gitignore`。规划工件保留在本地，从不接触 git。
...
存在 Claude Code 分类 bug 的已知解决方法。SDD 的编排器（execute-phase、quick）在报告失败前抽查实际输出。如果你看到失败消息但提交已创建，检查 `git log` —— 工作可能已成功。
...
.planning/
  ...
  research/               # 来自 /sdd-new-project 的领域研究
  todos/
    pending/              # 等待处理的捕获想法
    done/                 # 已完成的待办事项
  debug/                  # 活跃调试会话
    resolved/             # 已归档的调试会话
  codebase/               # 现有代码库映射（来自 /sdd-map-codebase）
  phases/
    XX-phase-name/
