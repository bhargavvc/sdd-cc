...
| `git.phase_branch_template` | `"sdd/phase-{phase}-{slug}"` | 阶段策略的分支模板 |
| `git.milestone_branch_template` | `"sdd/{milestone}-{slug}"` | 里程碑策略的分支模板 |
</config_schema>
...
**注意：** 大多数 SDD 操作使用直接文件读取或显式路径，无论 gitignore 状态如何都有效。
...
- 分支名来自 `phase_branch_template`（如 `sdd/phase-03-authentication`）
- 所有计划提交到该分支
- 阶段完成后用户手动合并分支
...
- 分支名来自 `milestone_branch_template`（如 `sdd/v1.0-mvp`）
- 里程碑中所有阶段提交到同一分支
