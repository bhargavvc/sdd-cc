...
如果你想在讨论期间更快速地输入，使用 `/sdd-discuss-phase <n> --batch` 一次回答一组小问题，而不是一个一个来。使用 `--chain` 可以自动链式执行从讨论到规划+执行，中间不停顿。
...
**`--full` 标志：** 启用所有阶段 —— 讨论 + 研究 + 计划检查 + 验证。快速任务形式的完整 SDD 管道。
...
| `/sdd-discuss-phase [N] [--auto] [--chain] [--power]` | 在规划前捕获实现决策（`--chain` 自动链式执行规划+执行，`--power` 文件批量输入） |
...
OpenCode、Gemini CLI、Kilo 和 Codex 现在通过 `npx @bhargavvc/sdd-cc` 原生支持。
