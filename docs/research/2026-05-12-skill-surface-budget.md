...
**Date:** 2026-05-12
**Author:** triage analysis for issue [#3408](https://github.com/bhargavvc/sdd-cc/issues/3408)
...
| SDD share of a 200k 1% budget | ~60% |

SDD on its own consumes roughly 60% of the default skill-listing budget. When the user stacks any other plugin of comparable size (the reporter's `/doctor` shows several), the budget is breached and skills get dropped. This is not a SDD-only problem, but **SDD is the single biggest contributor in the typical install**, so we are the natural place for ecosystem-wide remediation to start.

## 2. What's already in place

SDD has done one consolidation pass and shipped one install-time lever:

- **`--minimal` / `--core-only` install flag** (`bin/install.js:123`, `sdd/bin/lib/install-profiles.cjs`). Stages a filtered copy of `commands/sdd/` into a temp dir before each runtime-specific copy step. Reduces ~12k tokens of cold-start overhead to ~700.
...
- **`sdd update` (without `--minimal`)** as the documented upgrade path from minimal → full.

The 100-char cap means **shrinking descriptions further is not a viable lever** — average is already 72.5 chars and the rare 99-char outliers exist because they earn their length (e.g. `sdd:progress`, `sdd:inbox`). Any future budget relief has to come from **emitting fewer skills**, not shorter ones.
...
`phase` and `review` are the two skills whose absence would silently break dozens of others. **`phase` is referenced by 38 other skills but is not in the current minimal allowlist** — a latent gap worth raising. Confirm with a `--minimal` install + a `/sdd:audit-fix` invocation whether it still works.
...
```
sdd install --profile=core            # current --minimal (6 skills, 0 agents)
sdd install --profile=standard        # +phase, +review, +config, +progress (~14 skills)
sdd install --profile=full            # current default (66 skills, 33 agents)
sdd install --profile=core,audit,ui   # composable feature tags
```
...
A `/sdd:surface` (or `sdd surface` CLI) command that toggles which skills are visible to the runtime without touching installed files:

```
/sdd:surface list                # show enabled/disabled
/sdd:surface disable ui audit    # hide a cluster
/sdd:surface profile standard    # apply a named profile
```

Implementation: write enable/disable state to `~/.claude/skills/<name>/SKILL.md.disabled` (rename) or maintain a `sdd-surface.json` manifest the installer reads on every `update`.

| Dimension | Assessment |
|---|---|
| User UX | Discoverable through `/sdd:help`. Lower commit than reinstall. Mirrors VS Code's enable/disable extension UX. |
| Implementation cost | **Medium.** Need persistent state separate from install files, plus a re-apply loop on `sdd update`. |
...
**Cons:** state lives outside the installer's idempotent model, so `sdd update` migrations get more complex.
...
```text
sdd-phase           → keeps existing
sdd-milestone {new|complete|summary|audit}        # was 4 skills (hypothetical)
sdd-research  {sketch|spike|forensics|explore}    # was 4 skills (hypothetical)
sdd-workspace {pause|resume|capture|inbox|thread} # was 5 skills (hypothetical)
```
...
| Implementation cost | **Low for SDD** — add a `teaser:` frontmatter field. **High for Anthropic** — harness changes. |
...
This is the architecturally correct long-term answer. SDD can't ship it alone.
...
|---|---|
| User UX | Invisible. |
| Implementation cost | **Low for SDD** — declare quota in `package.json` / plugin manifest. **Medium for Anthropic** — quota arithmetic and tie-breaking in the harness. |
...
Publish SDD as multiple npm packages: `@bhargavvc/sdd-cc-core`, `@bhargavvc/sdd-cc-milestones`, `@bhargavvc/sdd-cc-research`, etc. Users install only what they need.

| Dimension | Assessment |
|---|---|
| User UX | Reasonable for advanced users; confusing for first-time installers. Needs a meta-package (`@bhargavvc/sdd-cc`) that depends on the slim ones — analogous to VS Code extension packs. |
...
3. **B follows A naturally.** Once profiles exist, the `/sdd:surface` command is "apply a profile to a live install plus persist deltas." Without A, B has no profiles to apply.
4. **C is independent and orthogonal.** It can happen in parallel as IA cleanup; it should not block A.
5. **D and E are platform-level.** SDD ships A regardless; D/E are documented as cooperative asks so Anthropic sees them in context.
...
5. `sdd update` re-applies the recorded profile from a small marker file (`~/.claude/skills/.sdd-profile`).

Phase 2 — runtime surface command (follow-up ADR or amendment):

1. `/sdd:surface` command writes to the profile marker and re-runs the staging step for the active runtime.
...
- **The `phase` dispatcher gap in the existing minimal allowlist.** Confirm whether a fresh `--minimal` install + the documented main loop actually works end-to-end. If `discuss-phase`/`plan-phase`/`execute-phase` silently fall back to `/sdd:phase`, the minimal allowlist is currently broken. Track as a separate bug if confirmed.
...
- **Discoverability of disabled skills.** If `sdd:audit-fix` isn't surfaced, a user asking "audit my project" won't get it suggested. `/sdd:help` should list installed-but-not-surfaced skills with a one-line upgrade hint.
- **Telemetry blind spot.** SDD doesn't currently know which skills users invoke, so "drop the long tail" is theoretical. Survey or self-reporting may be needed before drawing the `standard` profile line.

## 10. References

- Issue: [#3408](https://github.com/bhargavvc/sdd-cc/issues/3408)
- Existing seam: `sdd/bin/lib/install-profiles.cjs`
...
- Audit data: [`docs/research/data/2026-05-12-skill-audit.json`](data/2026-05-12-skill-audit.json) (per-skill dep graph, description sizes, and cluster mapping — reproducible from `commands/sdd/` and `agents/`)
