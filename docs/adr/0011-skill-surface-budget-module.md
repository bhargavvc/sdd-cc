...
Every installed `sdd-*` skill costs eager system-prompt tokens: runtimes (Claude Code, opencode, and others) enumerate all skill descriptions in `<available_skills>` on every turn. With 66 skills and 33 agents, SDD alone consumes roughly 60% of the default 1%-of-context skill-listing budget, causing descriptions to drop when users stack multiple plugins (#3408).
...
- Add a **Skill Surface Budget Module** under `sdd/bin/lib/install-profiles.cjs` as the single owner for which skills and agents are written to runtime config directories.
...
- Persist the chosen profile in a `.sdd-profile` marker file in each runtime config directory; `sdd update` reads the marker to honor the profile on re-install.
...
- Add a `/sdd:surface` slash command with the following sub-commands:
  ...
  - `reset` — clear surface state and re-apply the active profile from `.sdd-profile`
- Implement the runtime surface engine in `sdd/bin/lib/surface.cjs`, consuming `stageSkillsForProfile` and `stageAgentsForProfile` from the Phase 1 module without duplicating staging logic.
- Persist per-runtime surface state in `<runtimeConfigDir>/.sdd-surface.json`, independent from `.sdd-profile`. The profile marker owns install-time identity; the surface JSON owns session-scope cluster toggles.
- Source cluster taxonomy from the research memo §3.2 (2026-05-12-skill-surface-budget.md). Define clusters in `sdd/bin/lib/clusters.cjs` — a separate module so the surface engine and future SDK callers can import cluster definitions without loading the full profile module.
...
- Relationship to Anthropic platform asks: Asks D (native per-skill toggle API) and E (budget-fraction negotiation) remain filed separately. The `/sdd:surface` command is a unilateral SDD-side workaround that does not depend on those platform changes.

## Status — Phase 1 shipped

Phase 1 artifacts landed on `feat/3408-skills-description-dropped-due-to-size`:

- `sdd/bin/lib/install-profiles.cjs` — `PROFILES` map, `resolveProfile`, `loadSkillsManifest`, `stageSkillsForProfile`, `stageAgentsForProfile`, `readActiveProfile`, `writeActiveProfile`, `mostRestrictiveProfile`, `resolveEffectiveProfile`
- `requires:` frontmatter added to 64 skills in `commands/sdd/*.md`
- `scripts/lint-skill-deps.cjs` — CI gate for `requires:` integrity, wired into `pretest`
- `bin/install.js` — `--profile=<name>` flag (composable); `--minimal`/`--core-only` as aliases; `.sdd-profile` marker write on install; `sdd update` re-reads marker
...
- `commands/sdd/surface.md` — `/sdd:surface` slash command runbook (sub-commands: `list`, `status`, `profile <name>`, `disable <cluster>`, `enable <cluster>`, `reset`)
- `sdd/bin/lib/surface.cjs` — runtime engine (`readSurface`, `writeSurface`, `resolveSurface`, `applySurface`, `listSurface`); reuses `stageSkillsForProfile` / `stageAgentsForProfile` from Phase 1
- `sdd/bin/lib/clusters.cjs` — 10-cluster taxonomy covering all installed skill stems
...
- Persistent surface state: `<runtimeConfigDir>/.sdd-surface.json` (independent from `.sdd-profile`)

## Open questions

- Whether the `requires:` field should also be consumed by `/sdd:help` to annotate dependency chains in help output (follow-up).
...
- Users with constrained context budgets can install `--profile=core` and expand incrementally via `/sdd:surface enable <cluster>` without a full reinstall.
