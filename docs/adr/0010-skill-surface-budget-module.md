...
We propose extending the existing install profile seam (`sdd/bin/lib/install-profiles.cjs`) into a **Skill Surface Budget Module** that owns which subset of SDD's 66 skills is written to the runtime config dirs, and that owns the per-skill `requires:` dependency manifest used to keep that subset closed under cross-skill references. SDD currently ships a binary `--minimal` / full toggle; runtimes that enumerate skills (Claude Code, OpenCode, etc.) cap the `<available_skills>` system-prompt block at `skillListingBudgetFraction` of the context window (default 1% = ~2k tokens at 200k), and SDD alone consumes ~60% of that cap (#3408). Further description shrinkage is unavailable — `scripts/lint-descriptions.cjs` already enforces a hard 100-char ceiling and the mean is 72.5 chars. The remaining lever is surfacing fewer skills, which requires a typed profile model plus a dependency manifest, not more ad-hoc allowlists.

## Decision

- Add a **Skill Surface Budget Module** by extending `sdd/bin/lib/install-profiles.cjs` as the single owner for which `commands/sdd/*.md` and `agents/sdd-*.md` files are staged into the per-runtime copy pipeline.
...
- Add a `requires:` frontmatter field to every skill whose body references another SDD skill. The dependency graph in the research memo (`docs/research/2026-05-12-skill-surface-budget.md` §3.1) is the migration spec for this pass.
...
- Persist the active profile to `~/.claude/skills/.sdd-profile` (and runtime-equivalent locations) so `sdd update` re-applies the same profile instead of expanding silently to full.
...
5. Profile marker persistence + `sdd update` re-application.

It should **not** in the first pass:

- Build a runtime enable/disable surface (`/sdd:surface`). Track as a follow-up ADR (see "Open questions").
- Split SDD into multiple npm packages. The packaging-level alternative was considered and rejected — see research memo §4 Option F.
...
### `sdd/bin/lib/install-profiles.cjs`
...
- Add `requires:` field to every skill in `commands/sdd/*.md` whose body references another SDD skill. Audit data lists the full set (`docs/research/2026-05-12-skill-surface-budget.md` §3.1). Estimate: 25-30 files touched in Phase 1.
- Field is optional. Absence = "no SDD-skill dependencies." `lint-skill-deps.cjs` enforces consistency, not presence.

### New: `scripts/lint-skill-deps.cjs`

- Walks `commands/sdd/*.md`, parses `requires:`, walks the body for `sdd:<name>` or `\b<stem>\b` references to other skills (same matching rules documented in `docs/research/2026-05-12-skill-surface-budget.md` §3.1).
...
- New `~/.claude/skills/.sdd-profile` (and per-runtime equivalents under `.codex/`, `.cursor/`, etc. as enumerated in `install.js`) containing the active profile name.
...
- New `tests/skill-surface-budget-*.test.cjs` covering:
  ...
  - marker persistence: `sdd install --profile=standard` followed by `sdd update` preserves `standard`
...
name: sdd:plan-phase
description: ...
requires: [phase, discuss-phase]   # SDD skills only; not Claude Code primitives
---
```

`requires:` lists *SDD* skills (file stems). It does not include Claude Code built-ins (`Read`, `Bash`, etc.) — those continue to live in `allowed-tools:` per existing convention.
...
- `sdd update` stops silently re-expanding a `--minimal` install to full — a current foot-gun documented inline in `install-profiles.cjs` (its module-level comment recommends `sdd update` without `--minimal` to "expand to the full surface"; that path remains available, but the default `sdd update` now respects the recorded profile).
- The `requires:` manifest creates a new authoring obligation (~30 files in Phase 1), enforced by CI. Skill authors who add a `/sdd:phase` reference in a new skill body have to update `requires:`. The lint script keeps drift low-cost.
...
- If accepted, `CONTEXT.md` should gain a canonical **Skill Surface Budget Module** entry alongside the existing seam entries, and future architecture reviews should treat ad-hoc `commands/sdd/` filtering outside this seam as drift.

## Open questions

- Whether the Phase-2 runtime `/sdd:surface` command (research memo §4 Option B) should be its own ADR or an amendment to this one. Leaning **separate ADR** because it introduces persistent runtime state outside the install pipeline.
...
- Whether the `requires:` field should also be consumed by `/sdd:help` to render a "skills you have installed and what depends on what" graph. Likely yes, but out of scope for this ADR.
...
- Existing seam being extended: `sdd/bin/lib/install-profiles.cjs`
