# PRD — `review.default_reviewers` config key for `/sdd-review` reviewer selection
...
- Cut median wall-clock time of a no-flag `/sdd-review` on multi-CLI machines (target: ≥40% reduction for users with ≥4 detected CLIs).
- Keep the change non-breaking. Absent config = today's behavior; nothing changes for the install-day experience.
- Match SDD's established config conventions (`review.models.*`, `review.*_host`, **absent = enabled**, namespacing under `review.*`) so users don't have to learn a new pattern.
...
- **GUI / TUI editing of the key in v1.** Editing the JSON file directly (or via `/sdd-settings` / `/sdd-config --integrations` if maintainers choose to support it later) is sufficient.

## Users & Use Cases

### Primary persona: "Multi-CLI power user"

A developer who has installed multiple coding CLIs (e.g., Claude Code, Codex, Gemini CLI, Cursor, OpenCode, Kilo) plus one or more local inference servers. They run `/sdd-review` frequently — sometimes dozens of times per day during a sprint — and have a stable mental model of which 1–3 reviewers actually add signal for their day-to-day work.

### Secondary persona: "Single-CLI user with a sometimes-on local server"

Has Claude + ollama installed. Wants reviews from Claude every time, and from ollama only when explicitly asked. Today, every `/sdd-review` pays the ollama timeout cost when ollama isn't running.
...
- "I want today's behavior just this once" on a configured project → `/sdd-review --all`.
...
- As a multi-CLI user, I want to declare which reviewers run by default so that `/sdd-review` doesn't probe backends I don't use.
- As a multi-CLI user, I want my preference to survive `/sdd-update` so I don't have to keep re-patching `workflows/review.md`.
...
- **P0-2. No-flag honors the key.** Given the key is `["gemini", "codex"]` and both are detected, running `/sdd-review` invokes only Gemini and Codex.
- **P0-3. Absent key preserves current behavior.** Given the key is unset, running `/sdd-review` runs every detected reviewer, identical to today.
- **P0-4. `--all` overrides the config.** Given the key is `["gemini"]`, running `/sdd-review --all` invokes every detected reviewer. Verbose mode shows which reviewers came from `--all` vs. the default.
- **P0-5. Individual flags override the config.** Given the key is `["gemini"]`, running `/sdd-review --cursor` invokes only Cursor. Running `/sdd-review --gemini --codex` invokes exactly those two regardless of the default.
...
- **P1-1.** `/sdd-config --integrations` extends to set `review.default_reviewers` interactively, aligning with the existing interactive config flow for reviewers.
...
- **P1-3.** Verbose-mode "selection source" line in `/sdd-review` output: `Running reviewers (default): gemini, codex (set in .planning/config.json)`.
- **P1-4.** Per-command override env var (`SDD_REVIEW_DEFAULT=...`) for CI scenarios where mutating `config.json` is undesirable.
...
- One line per `/sdd-review` start identifying the source of selection (default config / `--all` / explicit flags / no config). Surfaced under `--verbose` per Q-5.
...
- **Adoption proxy.** Count of `.planning/config.json` files containing `review.default_reviewers` (only countable if/when SDD ever ships opt-in telemetry; otherwise qualitative via Discussions).
...
- **Median wall-clock per `/sdd-review`** on machines with ≥4 detected CLIs (self-reported or telemetry). Target: ≥40% reduction for users who opt in.
...
SDD doesn't ship usage telemetry today. Most of these metrics rely on qualitative signal: issue activity, Discussion threads, and a follow-up on `#3079`. That's appropriate for a config addition of this size — we don't need a metrics pipeline to validate it.
...
- **Q-3. `/sdd-config --integrations` integration.** Land in this pass or as a fast follow? *Proposal: fast follow; depends on contributor bandwidth.*
...
6. **Optional fast follow** — P1-1 (`/sdd-config --integrations` integration) if there's contributor bandwidth.
...
With this config, `/sdd-review` invokes only Gemini and Codex. `/sdd-review --all` invokes every detected reviewer. `/sdd-review --cursor` invokes only Cursor.

## Appendix B: Glossary

- **Reviewer / CLI / backend.** Any code-review-capable CLI or model server SDD can invoke (Claude, Codex, Cursor, Gemini, OpenCode, Kilo, ollama, lm-studio, llama.cpp, …).
...
- **"Absent = enabled" pattern.** SDD's convention that missing config keys default to a sensible enabled state. Here, missing `review.default_reviewers` means "all detected."
