...
1. `sdd/bin/lib/model-profiles.cjs` — agent → profile alias map, phase-type map, dynamic-routing default tiers
2. `sdk/src/query/config-query.ts` — stale 18-agent copy of `MODEL_PROFILES`
3. `sdd/workflows/settings-advanced.md` — runtime → built-in model-id table
...
- repo root package (`@bhargavvc/sdd-cc`) includes it
- standalone SDK package (`@bhargavvc/sdk`) includes it
...
- tests validate:
  - every `agents/sdd-*.md` file exists in the catalog
