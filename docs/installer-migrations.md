...
This document defines the migration layer for SDD installs and upgrades.
It is for contributors who need to retire files, move install surfaces,
rewrite runtime config, or preserve user data while changing how SDD is
installed.
...
- replacing SDD-managed command, skill, agent, hook, and engine files
...
inside a SDD-managed directory.
...
2. Remove stale SDD-managed files when a feature is retired.
...
A file that SDD installed and recorded in the install manifest. Managed files
...
files must never be removed just because they sit under a SDD directory.
...
SDD version, install mode, and hashes for distribution-owned files.
...
Remove a path only when it is known to be SDD-managed and unchanged from the
previous manifest, or when the migration provides a purpose-built detector for
an old SDD-owned shape.
...
purpose-built detector that proves SDD ownership.
...
Use this for legacy JSON config cleanup such as Codex `hooks.json`, where SDD
...
retired SDD-generated artifacts that need an explicit user choice.
...
contains a specific detector proving the file is a stale SDD artifact.
...
Runtime config is mixed ownership. SDD may own marker blocks, generated agent
sections, or hook entries, but it does not own the whole file unless the file
was created as a SDD-only file. Config migrations should remove or rewrite
...
- **What:** the SDD invocation, agent, skill, rule, hook, or config surface
...
- **Why:** the upstream loader contract or current SDD compatibility shim
...
| Runtime | What SDD installs | Where SDD installs it | Config ownership boundary | Upstream contract snapshot |
| --- | --- | --- | --- | --- |
| Claude Code | Global skills in `skills/sdd-*/SKILL.md`; local slash commands in `commands/sdd/*.md`; agents in `agents/sdd-*.md`; hooks in `hooks/`; `settings.json` registrations | Global `CLAUDE_CONFIG_DIR` or `~/.claude`; local `./.claude` | SDD owns only generated skills, local commands, `sdd-*` agents, hook files, and SDD hook/statusLine entries in `settings.json` | [Slash commands](https://docs.anthropic.com/en/docs/claude-code/slash-commands), [settings](https://docs.anthropic.com/en/docs/claude-code/settings), [hooks](https://docs.anthropic.com/en/docs/claude-code/hooks), [subagents](https://docs.anthropic.com/en/docs/claude-code/sub-agents); docs not versioned, checked 2026-05-11 |
| OpenCode | Flat markdown commands in `command/sdd-*.md`; agents in `agents/sdd-*.md`; config updates in `opencode.json` or `opencode.jsonc` | Global `OPENCODE_CONFIG_DIR`, `dirname(OPENCODE_CONFIG)`, `XDG_CONFIG_HOME/opencode`, or `~/.config/opencode`; local `./.opencode` | SDD owns generated command/agent files and SDD entries in structured config only | [Config](https://opencode.ai/docs/config/); docs published 2026-05, checked 2026-05-11 |
| Kilo | OpenCode-style flat markdown commands in `command/sdd-*.md`; agents in `agents/sdd-*.md`; config updates in `kilo.json` or `kilo.jsonc` | Global `KILO_CONFIG_DIR`, `dirname(KILO_CONFIG)`, `XDG_CONFIG_HOME/kilo`, or `~/.config/kilo`; local `./.kilo` | SDD owns generated command/agent files and SDD entries in structured config only | [Custom subagents](https://docs.kilo.ai/docs/customize/custom-subagents); docs not versioned, checked 2026-05-11 |
| Gemini CLI | TOML slash commands in `commands/sdd/*.toml`; agents in `agents/sdd-*.md`; `settings.json` feature flag, hooks, and statusline | Global `GEMINI_CONFIG_DIR` or `~/.gemini`; local `./.gemini` | SDD owns generated commands/agents/hooks and only SDD settings entries; local command copy may be skipped when global SDD commands already exist | [Custom commands](https://google-gemini.github.io/gemini-cli/docs/cli/custom-commands.html), [configuration](https://google-gemini.github.io/gemini-cli/docs/cli/configuration.html); docs checked 2026-05-11 |
| Codex | Skills in `skills/sdd-*/SKILL.md`; agents as source markdown plus per-agent TOML in `agents/`; `[agents.sdd-*]` and hooks in `config.toml` | Global `CODEX_HOME` or `~/.codex`; local `./.codex` | SDD owns generated skills, generated agent TOML, `agents.sdd-*` config sections, `[features].hooks` when added by SDD (canonical; legacy alias `codex_hooks` is recognized and migrated forward, #3566), and SDD hook entries | [Codex config schema](https://developers.openai.com/codex/config-schema.json), [Codex developer docs](https://developers.openai.com/codex/); docs not versioned, checked 2026-05-15; installer compatibility sentinel: Codex 0.130.0 features.hooks key (legacy `codex_hooks` recognized) |
| GitHub Copilot | Skills in `skills/sdd-*/SKILL.md`; agents as `.agent.md`; repository instructions in `copilot-instructions.md` | Global `COPILOT_CONFIG_DIR` or `~/.copilot`; local `./.github` | SDD owns generated skill/agent files and SDD-authored instruction files; no hook/statusline ownership | [Repository custom instructions](https://docs.github.com/en/copilot/how-tos/configure-custom-instructions/add-repository-instructions), [Copilot CLI custom instructions](https://docs.github.com/en/copilot/how-tos/copilot-cli/add-custom-instructions); GitHub Docs product docs, checked 2026-05-11 |
| Antigravity | Skills in `skills/sdd-*/SKILL.md`; agents in `agents/`; Gemini-style `settings.json` hooks when installed by SDD | Global `ANTIGRAVITY_CONFIG_DIR` or `~/.gemini/antigravity`; local `./.agent` | SDD owns generated skills/agents/hooks and SDD settings entries only | Public Antigravity install/config docs for this file layout were not stable or complete as of 2026-05-11; installer compatibility therefore uses SDD's Gemini-compatible settings policy, documented shim baseline. |
| Cursor | Skills in `skills/sdd-*/SKILL.md`; agents in `agents/`; rule references under `rules/` | Global `CURSOR_CONFIG_DIR` or `~/.cursor`; local `./.cursor` | SDD owns generated skills/agents and SDD rule files or references; no hook/statusline ownership | [Cursor rules](https://docs.cursor.com/context/rules); docs not versioned, checked 2026-05-11 |
| Windsurf | Skills in `skills/sdd-*/SKILL.md`; agents in `agents/`; rule references under `rules/` | Global `WINDSURF_CONFIG_DIR` or `~/.codeium/windsurf`; local `./.windsurf` | SDD owns generated skills/agents and SDD rule files or references; no hook/statusline ownership | Windsurf public rule docs were source-limited in search results as of 2026-05-11; installer targets the common workspace rules convention `./.windsurf/rules` and must be rechecked before migrations rewrite rules |
| Augment Code | Skills in `skills/sdd-*/SKILL.md`; agents in `agents/` | Global `AUGMENT_CONFIG_DIR` or `~/.augment`; local `./.augment` | SDD owns generated skills/agents only; no hook/statusline ownership | [Augment Agent Skills](https://docs.augmentcode.com/cli/skills), [Augment IDE skills](https://docs.augmentcode.com/using-augment/skills); IDE skills public beta in VS Code 0.789.0+, checked 2026-05-11 |
| Trae | Skills in `skills/sdd-*/SKILL.md`; agents in `agents/`; rule references under `rules/` | Global `TRAE_CONFIG_DIR` or `~/.trae`; local `./.trae` | SDD owns generated skills/agents and SDD rule files or references; no hook/statusline ownership | Public Trae docs expose AI settings and `.rules` announcements, but no stable skills/config API was found as of 2026-05-11; migrations must treat this row as source-limited |
| Qwen Code | Claude-compatible skills in `skills/sdd-*/SKILL.md`; agents in `agents/`; optional common hook/settings integration through SDD | Global `QWEN_CONFIG_DIR` or `~/.qwen`; local `./.qwen` | SDD owns generated skills/agents/hooks and SDD settings entries only | [Qwen commands and skills](https://qwenlm.github.io/qwen-code-docs/en/users/features/commands/); docs last updated 2026-05-06 |
| Hermes Agent | Category skills under `skills/sdd/` with `DESCRIPTION.md` plus nested `sdd-*/SKILL.md`; agents in `agents/`; optional common hook/settings integration through SDD | Global `HERMES_HOME` or `~/.hermes`; local `./.hermes` | SDD owns generated `skills/sdd/` category content, generated agents, and SDD settings entries only | [Hermes configuration](https://hermes-agent.nousresearch.com/docs/user-guide/configuration), [Hermes skills](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/skills), [working with skills](https://hermes-agent.nousresearch.com/docs/guides/work-with-skills); docs checked 2026-05-11 |
| CodeBuddy | Skills in `skills/sdd-*/SKILL.md`; agents in `agents/`; optional common hook/settings integration through SDD | Global `CODEBUDDY_CONFIG_DIR` or `~/.codebuddy`; local `./.codebuddy` | SDD owns generated skills/agents/hooks and SDD settings entries only | [CodeBuddy CLI skills](https://www.codebuddy.ai/docs/cli/skills), [CodeBuddy IDE skills](https://www.codebuddy.ai/docs/ide/Features/Skills); docs checked 2026-05-11 |
| Cline | Rule-based integration via `.clinerules` for current installer output | Global `CLINE_CONFIG_DIR` or `~/.cline`; local project root `.clinerules` | SDD owns the generated `.clinerules` file only when it created or manifest-tracked it; no hooks/statusline ownership | [Cline rules](https://docs.cline.bot/customization/cline-rules); docs prefer `.clinerules/` directory and still detect legacy rule files, checked 2026-05-11 |
...
- Preserve unknown user config, even when it sits inside a SDD-managed runtime
  root.
...
4. report stale SDD-looking files that are not in the current manifest
...
- `baseline-preserve-user` for known user-owned files and unknown files that do
  not look like stale SDD-generated artifacts
- `prompt-user` for stale SDD-looking artifacts that are not manifest-proven
...
- How do we prove it is SDD-owned?
...
- unknown file under a SDD directory
