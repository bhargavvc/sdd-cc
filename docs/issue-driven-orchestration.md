# Issue-Driven Orchestration with SDD
...
through SDD's existing primitives.

## What this guide is

A recipe for combining commands SDD already ships into an issue-tracker
...
referenced below already exists in SDD today.
...
([repository](https://github.com/openai/symphony)). SDD does not vendor or
wrap Symphony. The orchestration *concepts* map cleanly onto primitives
SDD already exposes; this guide just spells the mapping out so you can
adopt the pattern without writing glue code or bypassing SDD's safety
gates.

## Why this exists

SDD has the building blocks for issue-driven AI development —
`/sdd-workspace --new`, `/sdd-manager`, `/sdd-autonomous`, `/sdd-verify-work`,
`/sdd-review`, `/sdd-ship`, plus `STATE.md` and the phase artifact suite
...
- Underuse: developers run discuss/plan/execute manually and never reach
  for `/sdd-manager` or `/sdd-autonomous` even when their work pattern
  fits.
...
Each row maps a Symphony-style orchestration concept to the SDD primitive
...
| Symphony concept | SDD primitive |
|---|---|
...
| One isolated agent workspace per task | `/sdd-workspace --new --strategy worktree` |
| Agent dispatch and concurrency | `/sdd-manager` (interactive dashboard), `/sdd-autonomous` (unattended) |
| Per-phase plan and discuss steps | `/sdd-discuss-phase` → `/sdd-plan-phase` → `/sdd-execute-phase` |
...
2. **Map to a SDD phase.** If the issue maps onto an existing phase in
   `ROADMAP.md`, select it. If not, run `/sdd-new-milestone` (for a new
   milestone of related issues) or open a phase via `/sdd-phase` /
   `/sdd-phase --insert`. Capture the tracker issue URL in the phase's
   `CONTEXT.md` so traceability survives compaction.
3. **Create an isolated workspace.** Run
   `/sdd-workspace --new --strategy worktree <slug>` to spin up a git
...
4. **Run discuss → plan → execute through SDD.** From inside the
   workspace, run `/sdd-discuss-phase` to clarify ambiguities,
   `/sdd-plan-phase` to produce `PLAN.md`, and either `/sdd-manager`
   (interactive dashboard) or `/sdd-execute-phase` / `/sdd-autonomous`
   (unattended) to implement. Avoid driving raw `claude` invocations
   from outside SDD — that bypasses `STATE.md` updates and the phase
   manifest.
5. **Demand proof-of-work.** Run `/sdd-verify-work` to walk the user
   ...
   `/sdd-plan-phase --gaps` when verification surfaces missed scope.
6. **Pass through the review and ship gates.** Run `/sdd-review` to get
...
7. **Capture follow-up work explicitly.** Use `/sdd-note` for inline
   ...
   confirmation — SDD does not post to remote trackers automatically.
...
- **Isolated worktrees.** Every issue runs in a `/sdd-workspace --new`
  worktree, so partial work, aborted plans, and exploratory commits
  never touch `main`. `sdd-local-patches/` is the recovery surface if a
  worktree's hand-edits need to come back across an update.
- **Explicit human review.** `/sdd-review` and `/sdd-ship` both stop for
  ...
  something SDD opts into for you.
- **No automatic public posting.** SDD never opens, comments on, or
...
- **Verification before ship.** `/sdd-verify-work`'s UAT.md must record
  evidence before `/sdd-ship` is run. The recommended discipline is to
...
against the worktree, skipping `/sdd-verify-work`, or scripting issue
...
- **No vendoring or copying Symphony code.** SDD reuses its own
  primitives. The mapping above is conceptual; no Symphony-derived
  source ships in this repo.
- **No long-running daemon.** SDD does not poll GitHub or Linear. The
...
- **No mandatory tracker dependency.** The loop works without any
  ...
  the URL goes into `CONTEXT.md`. SDD has no opinion about which
  tracker you use, or whether you use one at all.
- **No bypass of verification, review, or human decision gates.** Even
  when running `/sdd-autonomous`, the verification and review gates
...
- Importing one GitHub or Linear issue into a SDD workspace / phase.
- Exporting `UAT.md` evidence as a comment on the source issue.
- Generating follow-up tracker issues from `/sdd-capture --seed` output.
...
- [docs/COMMANDS.md](COMMANDS.md) — full reference for `/sdd-*`
  commands.
