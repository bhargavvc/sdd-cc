...
- **Per-phase-type model selection (`models` block)** — express "Opus for planning,
  ...
  backward compatible.
  ([#3023](https://github.com/bhargavvc/sdd-cc/pull/3030))
...
- **Dynamic routing with failure-tier escalation (`dynamic_routing` block)** — start
  ...
  ([#3024](https://github.com/bhargavvc/sdd-cc/pull/3031))

- **Optional update banner for non-SDD statusline users** — when the installer detects
  no SDD statusline, it offers an opt-in `SessionStart` hook that surfaces update
  availability via the existing `~/.cache/sdd/sdd-update-check.json` cache. Silent when
  up-to-date; removed cleanly by `--uninstall`.
  ([#2795](https://github.com/bhargavvc/sdd-cc/pull/2795))

- **Issue-driven orchestration guide** — new
  [`docs/issue-driven-orchestration.md`](issue-driven-orchestration.md) recipe that maps
  tracker issues (GitHub / Linear / Jira) onto existing SDD primitives: workspace →
  discuss → plan → execute → verify → review → ship.
  ([#2840](https://github.com/bhargavvc/sdd-cc/pull/2840))
...
- **MVP mode SDK resolution layer — three canonical query verbs** — three new verbs
  ...
  `sdd-sdk query phase.mvp-mode <N>` (precedence resolver), `task.is-behavior-adding`
  ...
  ([#3178](https://github.com/bhargavvc/sdd-cc/pull/3178))

- **`/sdd-graphify status` surfaces commit-based staleness** — reads `built_at_commit`
  ...
  ([#3170](https://github.com/bhargavvc/sdd-cc/issues/3170))
...
- **MVP concept index and domain glossary** — seven MVP-related terms added to
  ...
  No behavior change.
  ([#3176](https://github.com/bhargavvc/sdd-cc/pull/3176))
...
- **Stable node path on Homebrew** — `resolveNodeRunner()` now maps versioned Cellar
  ...
  ([#3181](https://github.com/bhargavvc/sdd-cc/issues/3181))
...
- **Milestone-archive layout support** — `validate consistency`, `validate health`, and
  ...
  ([#3164](https://github.com/bhargavvc/sdd-cc/issues/3164))

- **`/sdd-graphify build` runs inline instead of spawning a sub-agent** — the
  ...
  ([#3166](https://github.com/bhargavvc/sdd-cc/issues/3166))

- **Planner directive language restored** — 10 `CRITICAL`/`MANDATORY`/`MUST` emphasis
  markers were silently removed from `sdd-planner.md` in v1.38.4, weakening planner
  adherence to user decisions and requirement coverage. All restored.
  ([#3138](https://github.com/bhargavvc/sdd-cc/issues/3087))
...
- **`secure-phase` retroactive-STRIDE mode for legacy phases** — phases with no
  ...
  ([#3142](https://github.com/bhargavvc/sdd-cc/issues/3120))
...
- **Global skills resolution now uses the correct runtime home directory** —
  ...
  directory.
  ([#3126](https://github.com/bhargavvc/sdd-cc/issues/3126))
...
- **`state.begin-phase` is now idempotent** — wave-resume calls no longer overwrite
  ...
  ([#3127](https://github.com/bhargavvc/sdd-cc/issues/3127))

- **`sdd-validate-commit.sh` hook catches all git commit forms** — the previous bash
  ...
  forms correctly.
  ([#3141](https://github.com/bhargavvc/sdd-cc/issues/3129))

- **`/sdd-plan-phase` no longer auto-dispatches to a subagent on OpenCode** — the
  `agent: sdd-planner` frontmatter directive caused OpenCode to run the orchestrator in
  a context where the `Agent` tool is unavailable. Directive removed.
  ([#3156](https://github.com/bhargavvc/sdd-cc/issues/3156))

- **`/sdd-quick` worktree-merge resurrection guard** — the inverted `PRE_MERGE_FILES`
  ...
  ([#3195](https://github.com/bhargavvc/sdd-cc/issues/3195))

- **`sdd-health` no longer raises W019 for `RETROSPECTIVE.md`** — registered in
  `CANONICAL_EXACT` in `artifacts.cjs` to match its established status as a milestone
  completion artifact.
  ([#3200](https://github.com/bhargavvc/sdd-cc/issues/3198))
...
- **`--sdk` flag now wired into SDK deployment** — `hasSdk` was parsed but never
  ...
  ([#3033](https://github.com/bhargavvc/sdd-cc/issues/3033))

- **Installer shell-path probe for SDK shim** — no longer prints "✓ SDD SDK ready"
  ...
  ([#3028](https://github.com/bhargavvc/sdd-cc/issues/3020))

- **Windows update-check no longer silently fails** — passes `shell: true` on Windows
  so `npm.cmd` resolves via PATHEXT; without this the statusline "⬆ /sdd-update"
  indicator never rendered on Windows.
  ([#3102](https://github.com/bhargavvc/sdd-cc/issues/3103))
...
- **Community `.sh` hooks use `#!/usr/bin/env bash`** — the previous `#!/bin/bash`
  ...
  ([#3194](https://github.com/bhargavvc/sdd-cc/issues/3194))

- **Gemini local install no longer duplicates `/sdd:*` commands** — when SDD is
  ...
  ([#3037](https://github.com/bhargavvc/sdd-cc/issues/3037))

- **Workstream resolution in `init.milestone-op` and `roadmap.analyze`** — both
  handlers now respect `--ws`, `SDD_WORKSTREAM`, and `.planning/active-workstream`.
  Workstream-scoped repos no longer exit with "Nothing left to do" from reading the
  root `.planning/` directory.
  ([#3196](https://github.com/bhargavvc/sdd-cc/issues/3196),
  [#3207](https://github.com/bhargavvc/sdd-cc/pull/3207))

- **`sdd-tools config-set workflow._auto_chain_active` no longer rejected** — the key
  was added to the SDK schema but not mirrored to `config-schema.cjs`; users routed
  through `sdd-tools` saw "Unknown config key."
  ([#3197](https://github.com/bhargavvc/sdd-cc/issues/3197))
...
- **Statusline state rendering is type-robust and YAML-list compatible** — milestone
  ...
  ([#3153](https://github.com/bhargavvc/sdd-cc/issues/3153))
...
- **Codex SessionStart hook uses absolute Node binary path** — bare `node` in
  ...
  ([#3022](https://github.com/bhargavvc/sdd-cc/issues/3017))
...
- **`config-set resolve_model_ids` and `workflow._auto_chain_active` accepted** — both
  ...
  ([#3162](https://github.com/bhargavvc/sdd-cc/issues/3162))
...
(86 → 59, [#2790](https://github.com/bhargavvc/sdd-cc/issues/2790)), six
namespace meta-skills ([#2792](https://github.com/bhargavvc/sdd-cc/issues/2792)),
`/sdd-health --context` utilization guard, phase-lifecycle status-line read-side
([#2833](https://github.com/bhargavvc/sdd-cc/issues/2833)), and Gemini
...
```bash
# npm (global)
npm install -g @bhargavvc/sdd-cc@latest

# npx (one-shot)
npx @bhargavvc/sdd-cc@latest

# Pin to this exact version
npm install -g @bhargavvc/sdd-cc@1.41.0
```
