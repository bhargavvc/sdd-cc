...
- **Six namespace meta-skills with keyword-tag descriptions** — replace the flat
  ...
  namespace routers (`sdd:workflow`, `sdd:project`, `sdd:review`, `sdd:context`,
  `sdd:manage`, `sdd:ideate`) instead of 86 entries; selects a namespace, then routes
  to the sub-skill. Existing sub-skills are unchanged and still invocable directly.
  ([#2792](https://github.com/bhargavvc/sdd-cc/issues/2792))

- **`/sdd-health --context` utilization guard** — context-window quality guard with
  two thresholds: 60 % warns ("consider `/sdd-thread`"), 70 % is critical ("reasoning
  quality may degrade"). Also exposed as `sdd-tools validate context`.
  ([#2792](https://github.com/bhargavvc/sdd-cc/issues/2792))
...
- **Phase-lifecycle status-line — read-side** — `parseStateMd()` now reads four new
  ...
  `progress`. `formatSddState()` gains scenes for in-flight, idle, and progress
...
- **`--minimal` install flag** (alias `--core-only`) — writes only the six core
  skills needed for the main workflow loop; no `sdd-*` subagents. Drops cold-start
  overhead from ~12k tokens to ~700. Useful for local LLMs with 32K–128K context.
  ([#2762](https://github.com/bhargavvc/sdd-cc/issues/2762))
...
- **Skill surface consolidated 86 → 59 `commands/sdd/*.md` entries** — four new
  ...
  ([#2790](https://github.com/bhargavvc/sdd-cc/issues/2790))
...
- **PRs missing `Closes #NNN` are auto-closed** — the `Issue link required`
  ...
  ([#2872](https://github.com/bhargavvc/sdd-cc/issues/2872))

### Fixed

- **Gemini slash commands now namespaced as `/sdd:<cmd>` instead of `/sdd-<cmd>`** —
  ...
  ([#2768](https://github.com/bhargavvc/sdd-cc/issues/2768),
  [#2783](https://github.com/bhargavvc/sdd-cc/issues/2783))

- **SDD slash-command namespace drift cleaned up across docs, workflows, and
  autocomplete** — remaining stale `/sdd:<cmd>` references now use canonical
  `/sdd-<cmd>`; `scripts/fix-slash-commands.cjs` rewrites retired colon syntax.
  ([#2858](https://github.com/bhargavvc/sdd-cc/pull/2858))
...
- **`SKILL.md` description quoted for Copilot / Antigravity / Trae / CodeBuddy** —
  ...
  ([#2876](https://github.com/bhargavvc/sdd-cc/issues/2876))
...
- **`sdd-tools` invocations use the absolute installed path** — bare `sdd-tools …`
  ...
  ([#2851](https://github.com/bhargavvc/sdd-cc/issues/2851))
...
- **Codex installer preserves trailing newline when stripping legacy hooks** — the
  ...
  downstream parsers.
  ([#2866](https://github.com/bhargavvc/sdd-cc/issues/2866))
...
fix ([#2787](https://github.com/bhargavvc/sdd-cc/issues/2787)), `audit-uat`
frontmatter parse fix ([#2788](https://github.com/bhargavvc/sdd-cc/issues/2788)),
skill description budget + lint gate ([#2789](https://github.com/bhargavvc/sdd-cc/issues/2789)),
`sdd-sdk` workstream + binary-collision fixes ([#2791](https://github.com/bhargavvc/sdd-cc/issues/2791)),
