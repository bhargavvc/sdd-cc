...
SDD はプロジェクト設定を `.planning/config.json` に保存します。`/sdd-new-project` 実行時に作成され、`/sdd-settings` で更新できます。
...
| `workflow.ui_safety_gate` | boolean | `true` | plan-phase 中にフロントエンドフェーズに対して /sdd-ui-phase の実行を促すプロンプトを表示 |
...
| `workflow.discuss_mode` | string | `'discuss'` | `/sdd-discuss-phase` のコンテキスト収集方法を制御。`'discuss'`（デフォルト）は質問を1つずつ行います。`'assumptions'` はまずコードベースを読み取り、信頼度レベル付きの構造化された仮説を生成し、誤っている点のみ修正を求めます。v1.28 で追加 |
| `workflow.skip_discuss` | boolean | `false` | `true` の場合、`/sdd-autonomous` は discuss-phase を完全にスキップし、ROADMAP のフェーズ目標から最小限の CONTEXT.md を作成します。開発者の要望が PROJECT.md/REQUIREMENTS.md に十分に記載されているプロジェクトに適しています。v1.28 で追加 |
...
| `hooks.workflow_guard` | boolean | `false` | SDD ワークフローのコンテキスト外でファイル編集が行われた場合に警告（`/sdd-quick` または `/sdd-fast` の使用を推奨） |

プロンプトインジェクションガードフック（`sdd-prompt-guard.js`）は常に有効であり、無効にすることはできません。これはワークフロートグルではなく、セキュリティ機能です。
...
| `git.phase_branch_template` | string | `sdd/phase-{phase}-{slug}` | phase 戦略のブランチ名テンプレート |
| `git.milestone_branch_template` | string | `sdd/{milestone}-{slug}` | milestone 戦略のブランチ名テンプレート |
| `git.quick_branch_template` | string or null | `null` | `/sdd-quick` タスク用のオプションのブランチ名テンプレート |
...
SDD が非 Claude ランタイム向けにインストールされると、インストーラーは自動的に `~/.sdd/defaults.json` に `resolve_model_ids: "omit"` を設定します。これにより SDD はすべてのエージェントに対して空のモデルパラメータを返し、各エージェントはランタイムで設定されたモデルを使用します。デフォルトの場合、追加のセットアップは不要です。
...
| `SDD_SKIP_SCHEMA_CHECK` | スキーマドリフト検出をバイパス (v1.31) |
...
**保存場所:** `~/.sdd/defaults.json`

`/sdd-new-project` が新しい `config.json` を作成する際、グローバルデフォルトを読み込み、初期設定としてマージします。プロジェクトごとの設定は常にグローバル設定を上書きします。
