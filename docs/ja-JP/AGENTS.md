...
SDD はマルチエージェントアーキテクチャを採用しており、軽量なオーケストレーター（ワークフローファイル）が新しいコンテキストウィンドウを持つ専門エージェントをスポーンします。各エージェントは特定の役割に特化し、限定的なツールアクセス権を持ち、特定の成果物を生成します。
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/sdd-new-project`, `/sdd-new-milestone` |
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/sdd-new-project`（4つのリサーチャー完了後） |
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/sdd-plan-phase`, `/sdd-quick` |
| **並列数** | 単一インスタンス |
...
**役割:** アトミックコミット、逸脱処理、チェックポイントプロトコルを使用して SDD 計画を実行する。

| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/sdd-execute-phase`, `/sdd-quick` |
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/sdd-plan-phase`（検証ループ、最大3回の反復） |
| **並列数** | 単一インスタンス（反復型） |
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/sdd-ui-phase`（検証ループ、最大2回の反復） |
| **並列数** | 単一インスタンス |
| **ツール** | Read, Bash, Glob, Grep |
| **モデル (balanced)** | Sonnet |
| **カラー** | `#22D3EE`（シアン） |
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/sdd-execute-phase`（すべてのエグゼキューター完了後） |
| **並列数** | 単一インスタンス |
...
- `/sdd-verify-work` で対処すべき問題をログに記録
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/sdd-debug`, `/sdd-verify-work`（失敗時） |
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/sdd-profile-user` |
| **並列数** | 単一インスタンス |
| **ツール** | Read |
| **モデル (balanced)** | Sonnet |
| **カラー** | Magenta |
| **生成物** | `USER-PROFILE.md`、`/sdd-dev-preferences`、`CLAUDE.md` プロファイルセクション |
