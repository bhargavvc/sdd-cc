...
**コマンド:** `/sdd-new-project [--auto @file.md]`
...
- 既存のコードベースコンテキスト（`/sdd-map-codebase` から取得）がある場合は読み込む

---

### 2. フェーズディスカッション

**コマンド:** `/sdd-discuss-phase [N] [--auto] [--batch]`
...
**コマンド:** `/sdd-plan-phase [N] [--auto] [--skip-research] [--skip-verify]`
...
- REQ-PLAN-07: フロントエンドフェーズが検出され UI-SPEC.md が存在しない場合、システムはユーザーに `/sdd-ui-phase` の実行を促さなければならない（UI セーフティゲート）
...
**コマンド:** `/sdd-audit-milestone`、`/sdd-complete-milestone`、`/sdd-new-milestone [name]`
...
**コマンド:** `/sdd-quick [--full] [--discuss] [--research]`

**目的:** SDD の保証を維持しながら、より高速なパスでアドホックなタスクを実行します。
...
**目的:** 自由形式のテキストを分析し、適切な SDD コマンドにルーティングします。

**要件:**
- REQ-DO-01: システムは自然言語入力からユーザーの意図を解析しなければならない
- REQ-DO-02: システムは意図を最も適切な SDD コマンドにマッピングしなければならない
...
- REQ-NEXT-04: プロジェクトが存在しない場合、システムは `/sdd-new-project` を提案しなければならない
- REQ-NEXT-05: すべてのフェーズが完了している場合、システムは `/sdd-complete-milestone` を提案しなければならない

**状態検出ロジック:**
| 状態 | アクション |
|------|----------|
| `.planning/` ディレクトリなし | `/sdd-new-project` を提案 |
| フェーズに CONTEXT.md がない | `/sdd-discuss-phase` を実行 |
| フェーズに PLAN.md ファイルがない | `/sdd-plan-phase` を実行 |
| プランはあるが SUMMARY.md がない | `/sdd-execute-phase` を実行 |
| 実行済みだが VERIFICATION.md がない | `/sdd-verify-work` を実行 |
| すべてのフェーズが完了 | `/sdd-complete-milestone` を提案 |
...
- REQ-POSTVER-03: システムは `/sdd-verify-work` が対処すべき問題をログに記録しなければならない
...
**実行タイミング:** `/sdd-execute-phase` の検証ステップの前に自動実行されます。
...
**実行タイミング:** `/sdd-plan-phase` の末尾、プランチェッカーループの後に自動実行されます。
...
- REQ-CTX-06: コンテキストモニターは SDD アクティブ vs 非 SDD アクティブプロジェクトを区別しなければならない
...
**コマンド:** `/sdd-pause-work`、`/sdd-resume-work`、`/sdd-progress`
...
**コマンド:** `/sdd-map-codebase [area]`

**目的:** 新しいプロジェクトを開始する前に既存のコードベースを分析し、SDD が既存の構成を理解できるようにします。
...
- REQ-MAP-04: 後続の `/sdd-new-project` はコードベースマッピングを読み込み、追加する内容に焦点を当てた質問を行わなければならない
...
**目的:** SDD を最新バージョンに更新し、チェンジログのプレビューを表示します。
...
- REQ-SETTINGS-03: システムはグローバルデフォルト（`~/.sdd/defaults.json`）としての保存をサポートしなければならない
...
**目的:** 複数の AI コーディングエージェントランタイムで SDD を実行します。
...
- REQ-RUNTIME-05: アンインストールは他の設定に影響を与えることなく、すべての SDD ファイルをクリーンに削除しなければならない
...
**コマンド:** `/sdd-profile-user [--questionnaire] [--refresh]`
...
- `/sdd-dev-preferences` コマンド — 任意のセッションで好みを読み込み
...
- `sdd-user-profiler` エージェント — セッションデータからの行動分析
...
すべての `/sdd-progress` 呼び出しで、現在のマイルストーンのすべてのフェーズの未解決項目（pending、skipped、blocked、human_needed）をスキャンします。アクション可能なリンク付きのノンブロッキング警告セクションを表示します。
...
**要件:**
- REQ-DEBT-01: システムは `/sdd-progress` ですべての過去フェーズの未解決 UAT/検証項目を表面化しなければならない
...
- REQ-DEBT-06: `/sdd-audit-uat` はすべてのフェーズをスキャンし、項目をテスト可能性別にカテゴリ分けし、人的テストプランを生成しなければならない
...
**コマンド:** `/sdd-fast [task description]`
...
**`/sdd-quick` との使い分け:**
- `/sdd-fast` — 2分以内に実行可能な一文のタスク（タイポ修正、設定変更、小規模な追加）
- `/sdd-quick` — リサーチ、マルチステッププランニング、または検証が必要なもの

---

### 42. クロス AI ピアレビュー

**コマンド:** `/sdd-review --phase N [--gemini] [--claude] [--codex] [--coderabbit] [--opencode] [--qwen] [--cursor] [--all]`
...
- REQ-REVIEW-05: レビューは `/sdd-plan-phase --reviews` で使用可能でなければならない
...
- REQ-BACKLOG-02: `/sdd-discuss-phase` と `/sdd-plan-phase` が動作するよう、フェーズディレクトリは即座に作成されなければならない
- REQ-BACKLOG-03: `/sdd-review-backlog` は項目ごとにプロモート、維持、削除のアクションをサポートしなければならない
- REQ-BACKLOG-04: プロモートされた項目はアクティブなマイルストーンシーケンスに再番号付けされなければならない
- REQ-SEED-01: シードは完全な WHY と表面化条件の WHEN をキャプチャしなければならない
- REQ-SEED-02: `/sdd-new-milestone` はシードをスキャンして一致するものを提示しなければならない
...
**コマンド:** `/sdd-thread [name | description]`

**目的:** 複数セッションにまたがるが特定のフェーズには属さない作業のための、軽量なクロスセッションナレッジストア。`/sdd-pause-work` よりも軽量 — フェーズ状態やプランコンテキストは不要です。
...
**コマンド:** `/sdd-pr-branch [target branch]`

**目的:** `.planning/` のコミットを除外して、プルリクエストに適したクリーンなブランチを作成します。レビュアーにはコード変更のみが表示され、SDD プランニング成果物は表示されません。
...
**目的:** SDD のプランニング成果物に対する多層防御セキュリティ。SDD は LLM のシステムプロンプトとなるマークダウンファイルを生成するため、これらのファイルに流入するユーザー制御テキストは間接的なプロンプトインジェクションの潜在的なベクターです。
...
**2. プロンプトインジェクションガードフック**（`sdd-prompt-guard.js`）
`.planning/` を対象とする Write/Edit 呼び出しをインジェクションパターンでスキャンする PreToolUse フック。アドバイザリーのみ — 正当な操作をブロックせず、検出を認識のためにログ記録します。

**3. ワークフローガードフック**（`sdd-workflow-guard.js`）
Claude が SDD ワークフローコンテキスト外でファイル編集を試行した際に検出する PreToolUse フック。直接編集の代わりに `/sdd-quick` や `/sdd-fast` の使用をアドバイスします。`hooks.workflow_guard`（デフォルト: false）で設定可能です。
...
**目的:** `/sdd-discuss-phase` 中に `DISCUSSION-LOG.md` を自動生成し、ディスカッション中の決定事項の完全な監査証跡を残します。
...
**コマンド:** `/sdd-forensics [description]`

**目的:** 失敗または停滞した SDD ワークフローのポストモーテム調査。
...
**コマンド:** `/sdd-milestone-summary [version]`
...
2. **切り替え** — 後続の SDD コマンドのためにアクティブなワークストリームコンテキストを変更
...
**コマンド:** `/sdd-discuss-phase`（`workflow.discuss_mode: 'assumptions'` 設定時）
...
**対象:** `/sdd-new-project` および `/sdd-progress`

**目的:** UI 重視のプロジェクトを自動検出し、`/sdd-ui-phase` の推奨を表面化します。
...
- REQ-UI-DETECT-03: システムは UI 重視フェーズのネクストステップに `/sdd-ui-phase` を提案しなければならない
- REQ-UI-DETECT-04: システムは `/sdd-ui-phase` を必須にしてはならない

**プロセス:**
1. **検出** — プロジェクト説明と技術スタックの UI シグナル（キーワード、フレームワーク参照）をスキャン
2. **アノテーション** — ROADMAP.md の該当フェーズに `ui_hint` マーカーを追加
3. **表面化** — UI 重視フェーズのネクストステップに `/sdd-ui-phase` の推奨を含める

---

### 55. マルチランタイムインストーラー選択

**対象:** `npx @bhargavvc/sdd-cc`
...
3. **インストール** — 1回のセッションで選択されたすべてのランタイムに対して SDD を設定
...
2. **変換** — SDD プロンプトを Windsurf ルール形式に変換
3. **インストール** — Windsurf 設定ディレクトリに SDD を設定
...
**目的:** SDD ドキュメントをポルトガル語、韓国語、日本語で提供します。
...
**目的:** CLI セッションなしでプログラムから SDD ワークフローを実行するためのヘッドレス TypeScript SDK。

**要件:**
- REQ-SDK-01: SDK は SDD ワークフロー操作を TypeScript 関数として公開しなければならない
- REQ-SDK-02: SDK はインタラクティブプロンプトなしのヘッドレス実行をサポートしなければならない
- REQ-SDK-03: SDK は CLI 駆動ワークフローと同一のアーティファクトを生成しなければならない

**プロセス:**
1. **インポート** — SDD SDK を TypeScript/JavaScript プロジェクトにインポート
2. **設定** — プロジェクトパスとワークフローオプションをプログラムから設定
3. **実行** — API コールで SDD フェーズ（discuss、plan、execute）を実行
...
- REQ-SCHEMA-04: 検出をオーバーライドする `SDD_SKIP_SCHEMA_CHECK` 環境変数をサポートしなければならない
...
**設定:** `SDD_SKIP_SCHEMA_CHECK` 環境変数で検出をバイパス。

---

### 60. セキュリティエンフォースメント

**コマンド:** `/sdd-secure-phase <N>`
...
- REQ-SEC-04: 分析のために `sdd-security-auditor` エージェントを起動しなければならない
...
2. **監査** — `sdd-security-auditor` を起動して脅威モデルに対して検証
...
**コマンド:** `/sdd-docs-update`

**目的:** 正確性チェック付きのプロジェクトドキュメントを生成・検証します。

**要件:**
- REQ-DOCS-01: システムはドキュメント生成のために `sdd-doc-writer` エージェントを起動しなければならない
- REQ-DOCS-02: システムは正確性チェックのために `sdd-doc-verifier` エージェントを起動しなければならない
...
**プロセス:**
1. **生成** — `sdd-doc-writer` を起動して実装からドキュメントを作成・更新
2. **検証** — `sdd-doc-verifier` を起動してコードベースに対するドキュメント正確性をチェック
...
**目的:** SDD コマンドを Claude Code 2.1.88+ のスキル形式に後方互換性を維持してマイグレーションします。

**要件:**
- REQ-SKILLS-01: インストーラーは Claude Code 2.1.88+ 向けに `skills/sdd-*/SKILL.md` を書き込まなければならない
- REQ-SKILLS-02: インストーラーはレガシー `commands/sdd/` ディレクトリを自動クリーンしなければならない
- REQ-SKILLS-03: Gemini パスを通じて古い Claude Code バージョンとの後方互換性を維持しなければならない

**プロセス:**
1. **検出** — Claude Code のバージョンをチェックしてスキルサポートを判定
2. **マイグレーション** — 各 SDD コマンドに対して `skills/sdd-*/SKILL.md` ファイルを書き込み
3. **クリーン** — スキルがインストールされた場合、レガシー `commands/sdd/` ディレクトリを削除
...
**対象:** `/sdd-execute-phase`（ベリファイヤーステップ）
...
**目的:** Trae IDE、Cline、Augment Code ランタイムへの SDD インストールを拡張します。
