...
ワークフローファイル（`sdd/workflows/*.md`）は重い処理を行いません。以下の役割に徹します：
- `sdd-tools.cjs init <workflow>` でコンテキストを読み込む
...
- **Claude Code:** カスタムスラッシュコマンド（`/sdd-command-name`）
- **OpenCode / Kilo:** スラッシュコマンド（`/sdd-command-name`）
- **Codex:** スキル（`$sdd-command-name`）
- **Copilot:** スラッシュコマンド（`/sdd-command-name`）
- **Antigravity:** スキル

**コマンド総数:** 44

### ワークフロー（`sdd/workflows/*.md`）

コマンドが参照するオーケストレーションロジックです。以下を含むステップバイステップのプロセスが記述されています：
- `sdd-tools.cjs init` によるコンテキスト読み込み
- モデル解決を伴うエージェント起動の指示
- ゲート/チェックポイントの定義
- 状態更新パターン
- エラーハンドリングとリカバリー
...
### テンプレート（`sdd/templates/`）

すべてのプランニングアーティファクト用のMarkdownテンプレートです。`sdd-tools.cjs template fill` および `scaffold` コマンドにより、事前構造化されたファイルを作成するために使用されます：
...
17のドメインモジュールを持つNode.js CLIユーティリティ（`sdd-tools.cjs`）：
...
| カテゴリ | エージェント | 並列実行 |
|----------|--------|-------------|
| **リサーチャー** | sdd-project-researcher, sdd-phase-researcher, sdd-ui-researcher, sdd-advisor-researcher | 4並列（stack、features、architecture、pitfalls）; advisorはdiscuss-phase中に起動 |
| **シンセサイザー** | sdd-research-synthesizer | 逐次（リサーチャー完了後） |
| **プランナー** | sdd-planner, sdd-roadmapper | 逐次 |
| **チェッカー** | sdd-plan-checker, sdd-integration-checker, sdd-ui-checker, sdd-nyquist-auditor | 逐次（検証ループ、最大3回反復） |
| **エグゼキューター** | sdd-executor | ウェーブ内は並列、ウェーブ間は逐次 |
| **ベリファイアー** | sdd-verifier | 逐次（全エグゼキューター完了後） |
| **マッパー** | sdd-codebase-mapper | 4並列（tech、arch、quality、concerns） |
| **デバッガー** | sdd-debugger | 逐次（インタラクティブ） |
| **オーディター** | sdd-ui-auditor | 逐次 |
...
├── research/               # /sdd-new-project によるドメインリサーチ
│   ├── SUMMARY.md
│   ├── STACK.md
│   ├── FEATURES.md
│   ├── ARCHITECTURE.md
│   └── PITFALLS.md
├── codebase/               # ブラウンフィールドマッピング（/sdd-map-codebase から）
│   ├── STACK.md
│   ├── ARCHITECTURE.md
│   ├── CONVENTIONS.md
│   ├── CONCERNS.md
...
├── ui-reviews/             # /sdd-ui-review からのスクリーンショット（gitignore対象）
...
|---------|---------------|--------------|-----------------|
| Claude Code | `/sdd-command` | Task起動 | `~/.claude/` |
| OpenCode | `/sdd-command` | サブエージェントモード | `~/.config/opencode/` |
| Kilo | `/sdd-command` | サブエージェントモード | `~/.config/kilo/` |
| Gemini CLI | `/sdd-command` | Task起動 | `~/.gemini/` |
| Codex | `$sdd-command` | スキル | `~/.codex/` |
| Copilot | `/sdd-command` | エージェント委譲 | `~/.github/` |
