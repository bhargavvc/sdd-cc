# Multi-Project Workspaces (`/sdd-new-workspace`)
...
SDD is tied to one `.planning/` directory per working directory. Users with multiple independent projects (monorepo-style setups with 20+ child repos) or users needing feature branch isolation in the same repo cannot run parallel SDD sessions without manual cloning and state management.
...
| `--path` | No | `~/sdd-workspaces/<name>` | Target directory |
...
Scans `~/sdd-workspaces/*/WORKSPACE.md` for workspace manifests. Displays table with name, path, repo count, SDD status (has PROJECT.md, current phase).
...
```
~/sdd-workspaces/feature-b/          # workspace root
├── WORKSPACE.md                      # manifest
├── .planning/                        # independent SDD planning directory
│   ├── PROJECT.md                    # (if user ran /sdd-new-project)
│   ├── STATE.md
│   └── config.json
...
- `WORKSPACE.md` is the only SDD-specific file at the root (besides `.planning/`)
...
### `/sdd-new-workspace` Workflow Steps
...
8. **Offer /sdd-new-project** — Ask if user wants to run project initialization in the new workspace
...
- Default workspace base dir (`~/sdd-workspaces/`)
...
- **`~/sdd-workspaces/` doesn't exist** — "No workspaces found"
...
| Component | Path |
|-----------|------|
| Command: new-workspace | `commands/sdd/new-workspace.md` |
| Command: list-workspaces | `commands/sdd/list-workspaces.md` |
| Command: remove-workspace | `commands/sdd/remove-workspace.md` |
| Workflow: new-workspace | `sdd/workflows/new-workspace.md` |
| Workflow: list-workspaces | `sdd/workflows/list-workspaces.md` |
| Workflow: remove-workspace | `sdd/workflows/remove-workspace.md` |
| Init function | `sdd/bin/lib/init.cjs` (add `cmdInitNewWorkspace`, `cmdInitListWorkspaces`, `cmdInitRemoveWorkspace`) |
| Routing | `sdd/bin/sdd-tools.cjs` (add cases to init switch) |
...
| Decision | Rationale |
|----------|-----------|
| Physical directories over logical registry | Filesystem is source of truth — matches SDD's existing cwd-based detection pattern |
| Worktree as default strategy | Lightweight (shared .git objects), fast to create, easy to clean up |
| `.planning/` at workspace root | Gives full isolation from individual repo planning. Each workspace is an independent SDD project |
...
| Default path `~/sdd-workspaces/<name>` | Predictable location for `list-workspaces` to scan, keeps workspaces out of source repos |
