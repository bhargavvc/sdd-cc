# JSON Error Mode — `sdd-tools` Structured Errors

## Overview

`sdd-tools` supports a **JSON error mode** that emits all errors as structured
...
node sdd-tools.cjs --json-errors <command> [args]

# Env var (preferred for shell wrappers and CI):
SDD_JSON_ERRORS=1 node sdd-tools.cjs <command> [args]
```
...
Codes are frozen constants in `sdd/bin/lib/core.cjs` under
`ERROR_REASON`.  Tests must assert on `reason` values (stable), not `message`
text (unstable).

### Dispatch errors (sdd-tools routing layer)

| Code | When emitted |
|------|-------------|
| `sdk_unknown_command` | Unknown top-level command (`sdd-tools bogus-cmd`) |
| `sdk_unknown_command` | Unknown dotted command (`sdd-tools foo.bar` where `foo` is not a known command) |
| `sdk_unknown_command` | Unknown subcommand within a domain (e.g. `sdd-tools intel bogus-sub`) |
...
| `usage` | Version flag (`--version`, `-v`) which sdd-tools never accepts |
...
const result = runSddTools(['--json-errors', 'bogus-command'], tmpDir);
...
1. Add the constant to `ERROR_REASON` in
   `sdd/bin/lib/core.cjs` (snake\_case, prefixed by subsystem).
