...
framework** — three changes that make SDD safer to install, safer to update, and easier
to run in constrained contexts. The release also ships configurable `/sdd-ship` PR body
sections, `/sdd-review` reviewer defaults, optional fallow structural review, and
...
- **Package legitimacy gate against slopsquatting** — researchers audit external
  ...
  ([#3215](https://github.com/bhargavvc/sdd-cc/pull/3215))

- **Skill surface budgeting** — install with `--profile=core`, `--profile=standard`,
  or the default `full`; profiles persist in `.sdd-profile`. Use `/sdd:surface` to
  ...
  ([#3408](https://github.com/bhargavvc/sdd-cc/pull/3456))
...
- **Installer migration framework** — install now has explicit migration records,
  ...
  ([#3398](https://github.com/bhargavvc/sdd-cc/pull/3398),
  [#3399](https://github.com/bhargavvc/sdd-cc/pull/3399),
  [#3400](https://github.com/bhargavvc/sdd-cc/pull/3400),
  [#3402](https://github.com/bhargavvc/sdd-cc/pull/3402),
  [#3404](https://github.com/bhargavvc/sdd-cc/pull/3404))

- **Configurable `/sdd-ship` PR body sections** — `ship.pr_body_sections` appends
  ...
  ([#3391](https://github.com/bhargavvc/sdd-cc/pull/3391))

- **`review.default_reviewers`** — no-flag `/sdd-review` can default to a configured
  reviewer subset. Explicit reviewer flags and `--all` still take precedence.
  ([#3464](https://github.com/bhargavvc/sdd-cc/pull/3464))

- **Optional fallow structural review pre-pass** — `code_quality.fallow.*` runs a
  structural pass before `/sdd-code-review`, writes `FALLOW.json`, and embeds
  structural findings in `REVIEW.md`.
  ([#3424](https://github.com/bhargavvc/sdd-cc/pull/3486))

- **Structured CLI error mode** — `sdd-tools --json-errors` returns machine-readable
  error envelopes for automation and SDK callers while preserving human-readable output
  by default.
  ([#3255](https://github.com/bhargavvc/sdd-cc/pull/3304))
...
- **Human verification defaults to end-of-phase** — `workflow.human_verify_mode:
  ...
  checkpoint behavior.
  ([#3309](https://github.com/bhargavvc/sdd-cc/pull/3325))
...
- **Quota and rate-limit failures get a distinct recovery path** — execute-phase
  ...
  ([#3095](https://github.com/bhargavvc/sdd-cc/pull/3490))
...
- **Milestone tags can be disabled** — `git.create_tag: false` lets projects with
  ...
  ([#3086](https://github.com/bhargavvc/sdd-cc/pull/3508))
...
- **Statusline context meter can move to the front** — `statusline.context_position:
  ...
  terminals.
  ([#2937](https://github.com/bhargavvc/sdd-cc/pull/3515))
...
- **Reasoning effort is transported with resolved model IDs** — runtime-aware model
  ...
  ([#3474](https://github.com/bhargavvc/sdd-cc/pull/3483))
...
- **Shell command projection and SDK architecture seams deepened** — hook commands,
  ...
  ([#3238](https://github.com/bhargavvc/sdd-cc/pull/3238),
  [#3316](https://github.com/bhargavvc/sdd-cc/pull/3316),
  [#3470](https://github.com/bhargavvc/sdd-cc/pull/3470),
  [#3476](https://github.com/bhargavvc/sdd-cc/pull/3476),
  [#3481](https://github.com/bhargavvc/sdd-cc/pull/3481),
  [#3484](https://github.com/bhargavvc/sdd-cc/pull/3484))

### Fixed

- **`project_code` phase directory prefixes apply consistently** — first-touch
  `/sdd-discuss-phase`, `/sdd-plan-phase`, import, gap-planning, and backlog creation
  paths now create prefixed phase directories consistently.
  ([#3287](https://github.com/bhargavvc/sdd-cc/pull/3292),
  [#3298](https://github.com/bhargavvc/sdd-cc/pull/3306))
...
- **Phase completion is idempotent and refreshes state** — `state complete-phase` and
  ...
  ([#3489](https://github.com/bhargavvc/sdd-cc/pull/3499),
  [#3517](https://github.com/bhargavvc/sdd-cc/pull/3520))

- **Nested git worktrees are detected** — `/sdd-new-project` and ingest flows avoid
  creating nested `.git` directories when run inside an existing repository or
  worktree.
  ([#3491](https://github.com/bhargavvc/sdd-cc/pull/3502))
...
- **Codex install and hook migration are safer** — AoT hooks use event-name leaf keys,
  ...
  ([#3346](https://github.com/bhargavvc/sdd-cc/pull/3505),
  [#3357](https://github.com/bhargavvc/sdd-cc/pull/3380),
  [#3360](https://github.com/bhargavvc/sdd-cc/pull/3380))

- **SDK install readiness is durable** — `--sdk` now forces SDK deployment, stale shims
  are detected, Windows PATH probing is hardened, and "SDD SDK ready" only prints when
  the shim is reachable.
  ([#3033](https://github.com/bhargavvc/sdd-cc/issues/3033),
  [#3211](https://github.com/bhargavvc/sdd-cc/pull/3282),
  [#3231](https://github.com/bhargavvc/sdd-cc/pull/3249),
  [#3359](https://github.com/bhargavvc/sdd-cc/pull/3380))
...
- **User custom skills are preserved during update detection** — `detect-custom-files`
  ...
  patch preservation.
  ([#3317](https://github.com/bhargavvc/sdd-cc/pull/3318))
...
- **Decimal-phase `depends_on` references resolve correctly** — SDK phase indexing now
  ...
  references.
  ([#3488](https://github.com/bhargavvc/sdd-cc/pull/3501))

- **`sdd-sdk query commit --files --respect-staged` preserves interactive staging** —
  ...
  ([#3522](https://github.com/bhargavvc/sdd-cc/pull/3528))
...
npm install -g @bhargavvc/sdd-cc@1.42.1
```
