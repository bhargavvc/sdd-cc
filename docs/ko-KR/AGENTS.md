...
SDD는 멀티 에이전트 아키텍처를 사용합니다. 가벼운 오케스트레이터(워크플로우 파일)가 전문화된 에이전트를 새로운 컨텍스트 윈도우로 생성합니다. 각 에이전트는 집중된 역할, 제한된 도구 접근 권한을 가지며 특정 결과물을 생성합니다.
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/sdd-new-project`, `/sdd-new-milestone` |
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/sdd-new-project` (4개 조사자 완료 후) |
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/sdd-plan-phase`, `/sdd-quick` |
| **병렬성** | 단일 인스턴스 |
...
**역할:** 원자적 커밋, 이탈 처리, 체크포인트 프로토콜로 SDD 계획을 실행합니다.

| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/sdd-execute-phase`, `/sdd-quick` |
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/sdd-plan-phase` (검증 루프, 최대 3회 반복) |
| **병렬성** | 단일 인스턴스 (반복적) |
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/sdd-ui-phase` (검증 루프, 최대 2회 반복) |
| **병렬성** | 단일 인스턴스 |
| **도구** | Read, Bash, Glob, Grep |
| **모델 (balanced)** | Sonnet |
| **색상** | `#22D3EE` (cyan) |
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/sdd-execute-phase` (모든 executor 완료 후) |
| **병렬성** | 단일 인스턴스 |
...
- `/sdd-verify-work`가 처리할 문제를 기록합니다
...
| 속성 | 값 |
|----------|-------|
| **생성 주체** | `/sdd-debug`, `/sdd-verify-work` (실패 시) |
| **병렬성** | 단일 인스턴스 (대화형) |
