...
**비활성화:** 테스트 인프라가 중요하지 않은 빠른 프로토타이핑 페이즈에서는 `/sdd-settings`에서 `workflow.nyquist_validation: false`로 설정하세요.

### 소급 검증 (`/sdd-validate-phase`)
...
**사용 시점:** Nyquist가 활성화되기 전에 계획된 페이즈를 실행한 후 또는 `/sdd-audit-milestone`에서 Nyquist 준수 갭이 발견된 후에 사용합니다.
...
`/sdd-ui-phase`는 계획 전에 설계 계약을 확정합니다. `/sdd-ui-review`는 실행 후 결과를 감사합니다.

### 명령어

| 명령어 | 설명 |
|--------|------|
| `/sdd-ui-phase [N]` | 프론트엔드 페이즈를 위한 UI-SPEC.md 설계 계약 생성 |
| `/sdd-ui-review [N]` | 구현된 UI의 6개 기둥 기반 시각적 감사 소급 수행 |

### 워크플로우: `/sdd-ui-phase`

**실행 시점:** `/sdd-discuss-phase` 이후, `/sdd-plan-phase` 이전 — 프론트엔드/UI 작업이 포함된 페이즈.
...
**실행 시점:** `/sdd-execute-phase` 또는 `/sdd-verify-work` 이후 — 프론트엔드 코드가 있는 모든 프로젝트.

**독립 실행:** 모든 프로젝트에서 작동하며 SDD 관리 프로젝트가 아니어도 됩니다. UI-SPEC.md가 없으면 추상적인 6개 기둥 기준으로 감사합니다.
...
| `workflow.ui_safety_gate` | `true` | plan-phase가 프론트엔드 페이즈에서 /sdd-ui-phase 실행을 유도합니다 |

두 설정 모두 부재 시 활성화 패턴을 따릅니다. `/sdd-settings`에서 비활성화할 수 있습니다.
...
`/sdd-ui-review`는 Playwright CLI를 통해 `.planning/ui-reviews/`에 스크린샷을 캡처합니다. 바이너리 파일이 git에 포함되지 않도록 `.gitignore`가 자동으로 생성됩니다. 스크린샷은 `/sdd-complete-milestone` 실행 시 정리됩니다.
...
`/sdd-review-backlog`으로 **검토 및 승격**합니다 — 모든 백로그 항목을 표시하고 승격 (활성 순서로 이동), 유지 (백로그에 남김), 또는 제거 (삭제)를 선택할 수 있습니다.
...
| 명령어 | 목적 |
|--------|------|
| `/sdd-workstreams create <name>` | 격리된 계획 상태로 새 워크스트림 생성 |
| `/sdd-workstreams switch <name>` | 활성 컨텍스트를 다른 워크스트림으로 전환 |
| `/sdd-workstreams list` | 모든 워크스트림과 활성 워크스트림 표시 |
| `/sdd-workstreams complete <name>` | 워크스트림을 완료로 표시하고 상태 아카이브 |
...
**런타임 훅.**
- `sdd-prompt-guard.js` — `.planning/`에 대한 Write/Edit 호출에서 인젝션 패턴 스캔 (항상 활성, 권고만)
- `sdd-workflow-guard.js` — SDD 워크플로우 컨텍스트 밖의 파일 편집 시 경고 (`hooks.workflow_guard`로 선택적 활성화)
...
| 명령어 | 목적 | 사용 시점 |
|--------|------|----------|
| `/sdd-new-project` | 전체 프로젝트 초기화: 질문, 조사, 요구사항, 로드맵 | 새 프로젝트 시작 시 |
| `/sdd-new-project --auto @idea.md` | 문서에서 자동 초기화 | PRD나 아이디어 문서가 준비된 경우 |
| `/sdd-discuss-phase [N]` | 구현 결정사항 캡처 | 계획 전 구축 방식을 결정할 때 |
| `/sdd-ui-phase [N]` | UI 설계 계약 생성 | discuss-phase 이후, plan-phase 이전 (프론트엔드 페이즈) |
| `/sdd-plan-phase [N]` | 조사 + 계획 + 검증 | 페이즈 실행 전 |
| `/sdd-execute-phase <N>` | 병렬 웨이브로 모든 계획 실행 | 계획이 완료된 후 |
...
| `/sdd-new-milestone [name]` | 다음 버전 사이클 시작 | 마일스톤 완료 후 |
...
| 명령어 | 목적 | 사용 시점 |
|--------|------|----------|
| `/sdd-map-codebase` | 기존 코드베이스 분석 | 기존 코드에서 `/sdd-new-project` 실행 전 |
...
| 명령어 | 목적 | 사용 시점 |
|--------|------|----------|
| `/sdd-review --phase N` | 외부 CLI를 통한 교차 AI 동료 리뷰 | 실행 전 계획 검증 시 |
| `/sdd-pr-branch` | `.planning/` 커밋을 필터링한 깔끔한 PR 브랜치 | 계획 없는 diff로 PR 생성 전 |
| `/sdd-audit-uat` | 모든 페이즈의 검증 부채 감사 | 마일스톤 완료 전 |
...
| `workflow.ui_safety_gate` | `true`, `false` | `true` | plan-phase가 프론트엔드 페이즈에서 /sdd-ui-phase 실행을 유도합니다 |
...
| `hooks.workflow_guard` | `true`, `false` | `false` | SDD 워크플로우 컨텍스트 밖의 파일 편집 시 경고 |
...
| `git.phase_branch_template` | 템플릿 문자열 | `sdd/phase-{phase}-{slug}` | phase 전략의 브랜치 이름 |
| `git.milestone_branch_template` | 템플릿 문자열 | `sdd/{milestone}-{slug}` | milestone 전략의 브랜치 이름 |
| `git.quick_branch_template` | 템플릿 문자열 또는 `null` | `null` | `/sdd-quick` 작업의 선택적 브랜치 이름 |
...
**자율 모드에서 discuss-phase 건너뛰기:** PROJECT.md에 선호도가 이미 충분히 캡처된 `yolo` 모드에서 실행할 때 `/sdd-settings`에서 `workflow.skip_discuss: true`로 설정하세요. 이렇게 하면 discuss-phase를 완전히 우회하고 ROADMAP 페이즈 목표에서 파생된 최소한의 CONTEXT.md를 작성합니다. PROJECT.md와 관례가 충분히 포괄적이어서 토론이 새로운 정보를 제공하지 않을 때 유용합니다.
...
격리된 SDD 상태로 여러 저장소나 기능을 병렬로 작업합니다.
...
`.planning/PROJECT.md`가 이미 존재하는데 `/sdd-new-project`를 실행했습니다. 이것은 안전 검사입니다. 처음부터 다시 시작하려면 먼저 `.planning/` 디렉터리를 삭제하세요.
...
`/sdd-progress`를 실행하세요. 모든 상태 파일을 읽고 현재 위치와 다음에 할 일을 정확히 알려줍니다.

### 실행 후 변경이 필요한 경우

`/sdd-execute-phase`를 다시 실행하지 마세요. 목표를 정확히 수정하려면 `/sdd-quick`을 사용하거나 UAT를 통해 체계적으로 문제를 식별하고 수정하려면 `/sdd-verify-work`를 사용하세요.
...
`/sdd-new-project` 중에 또는 `/sdd-settings`에서 `commit_docs: false`로 설정하세요. `.planning/`을 `.gitignore`에 추가하세요. 계획 아티팩트는 로컬에 유지되며 git에 절대 포함되지 않습니다.
...
워크플로우가 명확하지 않은 방식으로 실패할 때 — 계획이 존재하지 않는 파일을 참조하거나 실행이 예상치 못한 결과를 생성하거나 상태가 손상된 것 같을 때 — `/sdd-forensics`를 실행하여 진단 보고서를 생성하세요.
...
병렬 실행을 완전히 비활성화하려면: `/sdd-settings` → `parallelization.enabled`를 `false`로 설정합니다.
