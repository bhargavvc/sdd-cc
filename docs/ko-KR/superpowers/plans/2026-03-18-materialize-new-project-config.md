...
**목표:** `/sdd-new-project`가 `.planning/config.json`을 생성할 때, 파일에 사용자가 선택한 6개 키만이 아닌 모든 유효한 기본값이 포함되도록 하여 개발자가 소스 코드를 읽지 않고도 모든 설정을 확인할 수 있게 합니다.
...
**기술 스택:** Node.js/CommonJS, 기존 sdd-tools CLI, 테스트에는 `node:test`.
...
| 파일 | 액션 | 목적 |
|------|--------|---------|
| `sdd/bin/lib/config.cjs` | 수정 | `buildNewProjectConfig()` + `cmdConfigNewProject()` 추가 |
| `sdd/bin/sdd-tools.cjs` | 수정 | `config-new-project` case 등록 + usage 문자열 업데이트 |
| `sdd/workflows/new-project.md` | 수정 | Steps 2a + 5: 인라인 JSON 작성을 CLI 호출로 교체 |
...
describe('config-new-project command', () => {
  ...
  test('works with empty choices — all defaults materialized', () => {
    const result = runSddTools(['config-new-project', '{}'], tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`sdd/bin/lib/config.cjs`에서 `validateKnownConfigKeyPath` 함수 뒤(약 35번째 줄)와 `ensureConfigFile` 앞에 다음을 추가하세요:
 ...
 * 다음 우선순위 순서로 병합합니다:
 *   1. 하드코딩된 기본값
 *   2. ~/.sdd/defaults.json의 사용자 수준 기본값(있는 경우)
...
function buildNewProjectConfig(cwd, userChoices) {
  ...
  // 사용 가능한 경우 ~/.sdd/defaults.json에서 사용자 수준 기본값 로드
 ...
 *
 * 사용자가 선택한 설정을 JSON 문자열로 받습니다(/sdd-new-project 중 명시적으로
 * 구성한 키들). 나머지 키들은 하드코딩된 기본값과 선택적 ~/.sdd/defaults.json에서 채워집니다.
 *
...
## 작업 2: sdd-tools.cjs에 `config-new-project` 등록

**파일.**

- 수정: `sdd/bin/sdd-tools.cjs`

- [ ] **Step 2.1: sdd-tools.cjs의 switch에 case 추가**
...
- 수정: `sdd/workflows/new-project.md`
...
**문제:**
`/sdd-new-project`는 온보딩 중 사용자가 명시적으로 선택한 6개 키만으로
...
2. **암묵적 확장**: `/sdd-settings` 또는 `config-set`이 처음으로 config에
   기록할 때도 해당 키들이 추가되지 않습니다. config는 유효한 구성의
   일부만 반영합니다.

**해결책:**
`sdd-tools.cjs`에 `config-new-project` CLI 명령어를 추가합니다. 이 명령어는:
...
`git.branching_strategy: "none"`을 확인하고 SDD 소스를 읽지 않고도
