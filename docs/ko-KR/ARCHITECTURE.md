...
워크플로우 파일(`sdd/workflows/*.md`)은 무거운 작업을 직접 수행하지 않습니다. 다음 작업만 담당합니다.
- `sdd-tools.cjs init <workflow>`로 컨텍스트를 로드합니다
...
- **Claude Code:** 커스텀 슬래시 명령어 (`/sdd-command-name`)
- **OpenCode / Kilo:** 슬래시 명령어 (`/sdd-command-name`)
- **Codex:** Skills (`$sdd-command-name`)
- **Copilot:** 슬래시 명령어 (`/sdd-command-name`)
- **Antigravity:** Skills
...
- `sdd-tools.cjs init`을 통한 컨텍스트 로드
...
모든 계획 아티팩트를 위한 Markdown 템플릿입니다. `sdd-tools.cjs template fill`과 `scaffold` 명령어가 사전 구조화된 파일을 생성하는 데 사용합니다.
...
| 훅 | 이벤트 | 목적 |
|------|-------|---------|
| `sdd-statusline.js` | `statusLine` | 모델, 작업, 디렉터리, 컨텍스트 사용 바 표시 |
| `sdd-context-monitor.js` | `PostToolUse` / `AfterTool` | 잔여 35%/25% 시점에 에이전트 대면 컨텍스트 경고 주입 |
| `sdd-check-update.js` | `SessionStart` | 새 SDD 버전을 백그라운드에서 확인 |
| `sdd-prompt-guard.js` | `PreToolUse` | `.planning/` 쓰기 작업에서 프롬프트 인젝션 패턴 스캔 (권고용) |
| `sdd-workflow-guard.js` | `PreToolUse` | SDD 워크플로우 컨텍스트 외부의 파일 편집 감지 (권고용, `hooks.workflow_guard`로 활성화) |

### CLI Tools (`sdd/bin/`)

17개의 도메인 모듈을 포함하는 Node.js CLI 유틸리티(`sdd-tools.cjs`)입니다.
...
| 카테고리 | 에이전트 | 병렬성 |
|----------|--------|-------------|
| **Researchers** | sdd-project-researcher, sdd-phase-researcher, sdd-ui-researcher, sdd-advisor-researcher | 4개 병렬 (stack, features, architecture, pitfalls); advisor는 discuss-phase 중 생성됨 |
| **Synthesizers** | sdd-research-synthesizer | 순차적 (조사자 완료 후) |
| **Planners** | sdd-planner, sdd-roadmapper | 순차적 |
| **Checkers** | sdd-plan-checker, sdd-integration-checker, sdd-ui-checker, sdd-nyquist-auditor | 순차적 (검증 루프, 최대 3회 반복) |
| **Executors** | sdd-executor | 웨이브 내 병렬, 웨이브 간 순차적 |
| **Verifiers** | sdd-verifier | 순차적 (모든 executor 완료 후) |
| **Mappers** | sdd-codebase-mapper | 4개 병렬 (tech, arch, quality, concerns) |
| **Debuggers** | sdd-debugger | 순차적 (대화형) |
| **Auditors** | sdd-ui-auditor | 순차적 |
...
├── commands/sdd/*.md               # 37개 슬래시 명령어
├── sdd/
│   ├── bin/sdd-tools.cjs           # CLI 유틸리티
...
├── hooks/
│   ├── sdd-statusline.js           # 상태표시줄 훅
│   ├── sdd-context-monitor.js      # 컨텍스트 경고 훅
│   └── sdd-check-update.js         # 업데이트 확인 훅
├── settings.json                   # 훅 등록
└── VERSION                         # 설치된 버전 번호
```
...
├── research/               # /sdd-new-project의 도메인 조사
│   ├── SUMMARY.md
│   ├── STACK.md
│   ├── FEATURES.md
│   ├── ARCHITECTURE.md
│   └── PITFALLS.md
├── codebase/               # 브라운필드 매핑 (/sdd-map-codebase에서)
│   ├── STACK.md
│   ├── ARCHITECTURE.md
│   ├── CONVENTIONS.md
│   ├── CONCERNS.md
...
├── ui-reviews/             # /sdd-ui-review의 스크린샷 (gitignored)
...
- SDD 워크플로우 컨텍스트 외부의 편집을 감지합니다 (활성 `/sdd-` 명령어 또는 Task 서브에이전트 없음)
- 상태 추적 변경을 위해 `/sdd-quick` 또는 `/sdd-fast` 사용을 권고합니다
...
|---------|---------------|--------------|-----------------|
| Claude Code | `/sdd-command` | Task 생성 | `~/.claude/` |
| OpenCode | `/sdd-command` | Subagent 모드 | `~/.config/opencode/` |
| Kilo | `/sdd-command` | Subagent 모드 | `~/.config/kilo/` |
| Gemini CLI | `/sdd-command` | Task 생성 | `~/.gemini/` |
| Codex | `$sdd-command` | Skills | `~/.codex/` |
| Copilot | `/sdd-command` | 에이전트 위임 | `~/.github/` |
