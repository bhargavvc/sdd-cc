...
**명령어:** `/sdd-new-project [--auto @file.md]`
...
- 기존 코드베이스 컨텍스트(`/sdd-map-codebase`에서)가 있으면 로드됩니다.

---

### 2. Phase Discussion

**명령어:** `/sdd-discuss-phase [N] [--auto] [--batch]`
...
**명령어:** `/sdd-plan-phase [N] [--auto] [--skip-research] [--skip-verify]`
...
- REQ-PLAN-07: 프론트엔드 페이즈가 감지되고 UI-SPEC.md가 없는 경우 `/sdd-ui-phase` 실행을 촉구해야 합니다(UI 안전 게이트).
...
**명령어:** `/sdd-audit-milestone`, `/sdd-complete-milestone`, `/sdd-new-milestone [name]`
...
**명령어:** `/sdd-quick [--full] [--discuss] [--research]`

**목적:** SDD 보증을 제공하지만 더 빠른 경로로 임시 작업을 실행합니다.
...
**목적:** 자유형 텍스트를 분석하고 적절한 SDD 명령어로 라우팅합니다.

**요구사항.**
- REQ-DO-01: 자연어 입력에서 사용자 의도를 파악해야 합니다.
- REQ-DO-02: 의도를 가장 적합한 SDD 명령어에 매핑해야 합니다.
...
- REQ-NEXT-04: 프로젝트가 없으면 `/sdd-new-project`를 제안해야 합니다.
- REQ-NEXT-05: 모든 페이즈가 완료되면 `/sdd-complete-milestone`을 제안해야 합니다.

**상태 감지 로직.**
| 상태 | 액션 |
|-------|--------|
| `.planning/` 디렉토리 없음 | `/sdd-new-project` 제안 |
| 페이즈에 CONTEXT.md 없음 | `/sdd-discuss-phase` 실행 |
| 페이즈에 PLAN.md 파일 없음 | `/sdd-plan-phase` 실행 |
| 계획 있지만 SUMMARY.md 없음 | `/sdd-execute-phase` 실행 |
| 실행되었지만 VERIFICATION.md 없음 | `/sdd-verify-work` 실행 |
...
- REQ-POSTVER-03: `/sdd-verify-work`가 처리할 문제를 기록해야 합니다.
...
**실행 시점.** `/sdd-execute-phase` 중 검증자 단계 전에 자동으로 실행됩니다.
...
**실행 시점.** `/sdd-plan-phase`의 계획 검사기 루프 후 자동으로 실행됩니다.
...
- REQ-CTX-06: 컨텍스트 모니터는 SDD 활성 프로젝트와 비활성 프로젝트를 구분해야 합니다.
...
**명령어:** `/sdd-pause-work`, `/sdd-resume-work`, `/sdd-progress`
...
- REQ-MAP-04: 이후 `/sdd-new-project`는 코드베이스 매핑을 로드하고 추가하는 내용에 대한 질문에 집중해야 합니다.
...
- REQ-SETTINGS-03: 전역 기본값으로 저장하는 것을 지원해야 합니다(`~/.sdd/defaults.json`).
...
- REQ-RUNTIME-05: 제거는 다른 구성에 영향을 주지 않고 모든 SDD 파일을 깔끔하게 제거해야 합니다.
...
**명령어:** `/sdd-profile-user [--questionnaire] [--refresh]`
...
- `/sdd-dev-preferences` 명령어 — 모든 세션에서 선호도 로드
...
- `sdd-user-profiler` 에이전트 — 세션 데이터에서 행동 분석
...
모든 `/sdd-progress` 호출은 현재 마일스톤의 모든 페이즈에서 미결 항목(pending, skipped, blocked, human_needed)을 스캔합니다. 실행 가능한 링크가 포함된 비차단 경고 섹션을 표시합니다.
...
**요구사항.**
- REQ-DEBT-01: `/sdd-progress`에서 모든 이전 페이즈의 미결 UAT/검증 항목을 표시해야 합니다.
...
- REQ-DEBT-06: `/sdd-audit-uat`는 모든 페이즈를 스캔하고 테스트 가능성별로 항목을 분류하며 사람 테스트 계획을 생성해야 합니다.
...
**`/sdd-quick`과 비교하여 사용 시점.**
- `/sdd-fast` — 2분 이내에 실행 가능한 한 문장 작업(오타, 구성 변경, 작은 추가)
- `/sdd-quick` — 연구, 다단계 계획, 또는 검증이 필요한 모든 것

---

### 42. Cross-AI Peer Review

**명령어:** `/sdd-review --phase N [--gemini] [--claude] [--codex] [--coderabbit] [--opencode] [--qwen] [--cursor] [--all]`
...
- REQ-REVIEW-05: 검토는 `/sdd-plan-phase --reviews`가 사용할 수 있어야 합니다.
...
- REQ-BACKLOG-02: `/sdd-discuss-phase`와 `/sdd-plan-phase`가 작동할 수 있도록 페이즈 디렉토리를 즉시 생성해야 합니다.
- REQ-BACKLOG-03: `/sdd-review-backlog`는 항목별로 승격, 유지, 제거 액션을 지원해야 합니다.
- REQ-BACKLOG-04: 승격된 항목은 활성 마일스톤 순서로 번호가 다시 매겨져야 합니다.
- REQ-SEED-01: 시드는 표시 조건에 대한 전체 이유와 시기를 캡처해야 합니다.
- REQ-SEED-02: `/sdd-new-milestone`은 시드를 스캔하고 일치하는 항목을 표시해야 합니다.
...
**명령어:** `/sdd-thread [name | description]`

**목적:** 여러 세션에 걸쳐 있지만 특정 페이즈에 속하지 않는 작업을 위한 가벼운 교차 세션 지식 저장소입니다. `/sdd-pause-work`보다 더 가볍습니다 — 페이즈 상태나 계획 컨텍스트가 없습니다.
...
**명령어:** `/sdd-pr-branch [target branch]`

**목적:** `.planning/` 커밋을 필터링하여 풀 리퀘스트에 적합한 깔끔한 브랜치를 생성합니다. 검토자는 SDD 계획 산출물이 아닌 코드 변경사항만 봅니다.
...
**2. 프롬프트 주입 가드 훅** (`sdd-prompt-guard.js`)
`.planning/`을 대상으로 하는 Write/Edit 호출에서 주입 패턴을 스캔하는 PreToolUse 훅입니다. 정당한 작업을 차단하지 않고 인식을 위해 감지를 기록하는 권고 전용입니다.

**3. 워크플로우 가드 훅** (`sdd-workflow-guard.js`)
Claude가 SDD 워크플로우 컨텍스트 밖에서 파일 편집을 시도하는 것을 감지하는 PreToolUse 훅입니다. 직접 편집 대신 `/sdd-quick` 또는 `/sdd-fast` 사용을 권고합니다. `hooks.workflow_guard`로 구성 가능합니다(기본값: false).
...
**목적:** `/sdd-discuss-phase` 중에 `DISCUSSION-LOG.md`를 자동 생성하여 논의 중에 내려진 결정의 전체 감사 추적을 제공합니다.
...
**명령어:** `/sdd-forensics [description]`

**목적:** 실패하거나 막힌 SDD 워크플로우의 사후 조사.
...
**명령어:** `/sdd-milestone-summary [version]`
...
2. **전환** — 이후 SDD 명령어를 위한 활성 워크스트림 컨텍스트 변경
...
**명령어:** `/sdd-discuss-phase` with `workflow.discuss_mode: 'assumptions'`
...
**일부:** `/sdd-new-project` 및 `/sdd-progress`

**목적:** UI 중심 프로젝트를 자동으로 감지하고 `/sdd-ui-phase` 권장사항을 표시합니다.

**요구사항.**
- REQ-UI-DETECT-01: 프로젝트 설명에서 UI 신호를 감지해야 합니다(키워드, 프레임워크 참조).
- REQ-UI-DETECT-02: 해당하는 경우 ROADMAP.md 페이즈에 `ui_hint`를 주석으로 추가해야 합니다.
- REQ-UI-DETECT-03: UI 중심 페이즈의 다음 단계에서 `/sdd-ui-phase`를 제안해야 합니다.
- REQ-UI-DETECT-04: `/sdd-ui-phase`를 필수로 만들어서는 안 됩니다.

**프로세스.**
1. **감지** — UI 신호(키워드, 프레임워크 참조)에 대한 프로젝트 설명 및 기술 스택 스캔
2. **주석** — ROADMAP.md의 해당 페이즈에 `ui_hint` 표시 추가
3. **표시** — UI 중심 페이즈의 다음 단계에 `/sdd-ui-phase` 권장사항 포함

---

### 55. Multi-Runtime Installer Selection

**일부:** `npx @bhargavvc/sdd-cc`
...
3. **설치** — 단일 세션에서 선택된 모든 런타임에 SDD 구성
...
2. **변환** — SDD 프롬프트를 Windsurf 규칙 형식으로 변환
3. **설치** — Windsurf 구성 디렉토리에 SDD 설정
...
**목적:** SDD 문서를 포르투갈어, 한국어, 일본어로 제공합니다.
...
**목적:** CLI 세션 없이 프로그래밍 방식으로 SDD 워크플로우를 실행하기 위한 헤드리스 TypeScript SDK.

**요구사항.**
- REQ-SDK-01: SDK는 SDD 워크플로우 작업을 TypeScript 함수로 노출해야 합니다.
- REQ-SDK-02: SDK는 대화형 프롬프트 없이 헤드리스 실행을 지원해야 합니다.
- REQ-SDK-03: SDK는 CLI 기반 워크플로우와 동일한 아티팩트를 생성해야 합니다.

**프로세스.**
1. **임포트** — TypeScript/JavaScript 프로젝트에 SDD SDK 임포트
2. **구성** — 프로젝트 경로와 워크플로우 옵션을 프로그래밍 방식으로 설정
3. **실행** — API 호출로 SDD 페이즈(discuss, plan, execute) 실행
...
- REQ-SCHEMA-04: 감지를 재정의하는 `SDD_SKIP_SCHEMA_CHECK` 환경 변수를 지원해야 합니다.
...
**구성:** `SDD_SKIP_SCHEMA_CHECK` 환경 변수로 감지 바이패스.

---

### 60. 보안 시행

**명령어:** `/sdd-secure-phase <N>`
...
- REQ-SEC-04: 분석을 위해 `sdd-security-auditor` 에이전트를 스폰해야 합니다.
...
2. **감사** — `sdd-security-auditor`를 스폰하여 위협 모델에 대해 검증
...
**요구사항.**
- REQ-DOCS-01: 시스템은 문서 생성을 위해 `sdd-doc-writer` 에이전트를 스폰해야 합니다.
- REQ-DOCS-02: 시스템은 정확성 검사를 위해 `sdd-doc-verifier` 에이전트를 스폰해야 합니다.
...
**프로세스.**
1. **생성** — `sdd-doc-writer`를 스폰하여 구현에서 문서 생성 또는 업데이트
2. **검증** — `sdd-doc-verifier`를 스폰하여 코드베이스에 대한 문서 정확성 검사
...
**목적:** SDD 명령어를 하위 호환성을 유지하면서 Claude Code 2.1.88+ 스킬 형식으로 마이그레이션합니다.

**요구사항.**
- REQ-SKILLS-01: 설치 프로그램은 Claude Code 2.1.88+ 용 `skills/sdd-*/SKILL.md`를 작성해야 합니다.
- REQ-SKILLS-02: 설치 프로그램은 레거시 `commands/sdd/` 디렉토리를 자동 정리해야 합니다.
- REQ-SKILLS-03: Gemini 경로를 통해 이전 Claude Code 버전과의 하위 호환성을 유지해야 합니다.

**프로세스.**
1. **감지** — Claude Code 버전을 확인하여 스킬 지원 여부 판단
2. **마이그레이션** — 각 SDD 명령어에 대해 `skills/sdd-*/SKILL.md` 파일 작성
3. **정리** — 스킬이 설치되면 레거시 `commands/sdd/` 디렉토리 제거
...
**목적:** Trae IDE, Cline, Augment Code 런타임으로 SDD 설치를 확장합니다.
