...
> `sdd-tools.cjs`에 대한 프로그래밍 방식 API 레퍼런스입니다. 워크플로우와 에이전트가 내부적으로 사용합니다. 사용자 대면 명령어는 [Command Reference](COMMANDS.md)를 참조하세요.

---

## 개요

`sdd-tools.cjs`는 SDD의 약 50개 명령어, 워크플로우, 에이전트 파일에서 반복되는 인라인 bash 패턴을 대체하는 Node.js CLI 유틸리티입니다. config 파싱, 모델 해석, 단계 조회, git 커밋, 요약 검증, 상태 관리, 템플릿 작업을 중앙화합니다.

**위치:** `sdd/bin/sdd-tools.cjs`
**모듈:** `sdd/bin/lib/`의 15개 도메인 모듈
...
에이전트 이름: `sdd-planner`, `sdd-executor`, `sdd-phase-researcher`, `sdd-project-researcher`, `sdd-research-synthesizer`, `sdd-verifier`, `sdd-plan-checker`, `sdd-integration-checker`, `sdd-roadmapper`, `sdd-debugger`, `sdd-codebase-mapper`, `sdd-nyquist-auditor`
...
**대용량 페이로드 처리:** 출력이 약 50KB를 초과하면 CLI가 임시 파일에 쓰고 `@file:/tmp/sdd-init-XXXXX.json`을 반환합니다. 워크플로우는 `@file:` 접두사를 확인하고 디스크에서 읽습니다.
