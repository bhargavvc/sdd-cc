...
```bash
/sdd-new-project                    # 대화형 모드
/sdd-new-project --auto @prd.md     # PRD에서 자동 추출
```
...
**스캔 위치:** `~/sdd-workspaces/`에서 `WORKSPACE.md` 매니페스트를 탐색합니다.
...
```bash
/sdd-discuss-phase 1                # 페이즈 1 대화형 토론
/sdd-discuss-phase 3 --auto         # 페이즈 3 기본값 자동 선택
/sdd-discuss-phase --batch          # 현재 페이즈 일괄 모드
/sdd-discuss-phase 2 --analyze      # 트레이드오프 분석 포함 토론
```
...
```bash
/sdd-plan-phase 1                   # 페이즈 1 조사 + 계획 + 검증
/sdd-plan-phase 3 --skip-research   # 조사 없이 계획 (익숙한 도메인)
/sdd-plan-phase --auto              # 비대화형 계획 수립
```
...
```bash
/sdd-execute-phase 1                # 페이즈 1 실행
/sdd-execute-phase 1 --wave 2       # Wave 2만 실행
```
...
**동작 방식.**
- 프로젝트 없음 → `/sdd-new-project` 제안
- 페이즈 토론 필요 → `/sdd-discuss-phase` 실행
- 페이즈 계획 필요 → `/sdd-plan-phase` 실행
- 페이즈 실행 필요 → `/sdd-execute-phase` 실행
- 페이즈 검증 필요 → `/sdd-verify-work` 실행
...
**사전 조건:** 페이즈 검증 완료 (`/sdd-verify-work` 통과), `gh` CLI 설치 및 인증
**생성 파일:** 계획 아티팩트 기반의 풍부한 본문이 포함된 GitHub PR, STATE.md 업데이트

```bash
/sdd-ship 4                         # 페이즈 4 출시
/sdd-ship 4 --draft                 # 초안 PR로 출시
```
...
**사전 조건:** 프론트엔드 코드가 있는 프로젝트 (독립 실행 가능, SDD 프로젝트 불필요)
**생성 파일:** `{phase}-UI-REVIEW.md`, `.planning/ui-reviews/`에 스크린샷

```bash
/sdd-ui-review                      # 현재 페이즈 감사
/sdd-ui-review 3                    # 페이즈 3 감사
```
...
```bash
/sdd-milestone-summary                # 현재 마일스톤 요약
/sdd-milestone-summary v1.0           # 특정 마일스톤 요약
```
...
```bash
/sdd-new-milestone                  # 대화형 모드
/sdd-new-milestone "v2.0 Mobile"    # 이름이 지정된 마일스톤
/sdd-new-milestone --reset-phase-numbers "v2.0 Mobile"  # 마일스톤 번호를 1부터 재시작
```
...
```bash
/sdd-validate-phase 2               # 페이즈 2 테스트 커버리지 감사
```
...
```bash
/sdd-progress                       # "지금 어디 있나? 다음은 무엇인가?"
```
...
```bash
/sdd-resume-work                    # 컨텍스트 초기화 또는 새 세션 후 실행
```
...
```bash
/sdd-pause-work                     # continue-here.md 생성
```
...
```bash
/sdd-quick                          # 기본 빠른 작업
/sdd-quick --discuss --research     # 토론 + 조사 + 계획
/sdd-quick --full                   # 계획 검사 및 검증 포함
/sdd-quick --discuss --research --full  # 모든 선택적 단계 포함
```
...
```bash
/sdd-autonomous                     # 남은 모든 페이즈 실행
/sdd-autonomous --from 3            # 페이즈 3부터 시작
/sdd-autonomous --to 5              # 페이즈 5까지만 실행
/sdd-autonomous --from 3 --to 5     # 페이즈 3~5 범위 실행
...
```bash
/sdd-profile-user                   # 세션 분석 및 프로필 구축
/sdd-profile-user --questionnaire   # 대화형 설문지 대체 방법
/sdd-profile-user --refresh         # 새로운 분석으로 재생성
```
...
실패하거나 멈춘 SDD 워크플로우에 대한 사후 조사를 수행합니다.
...
```bash
/sdd-forensics                              # 대화형 — 문제 입력 프롬프트
/sdd-forensics "Phase 3 execution stalled"  # 문제 설명과 함께 실행
```
...
```bash
/sdd-workstreams                    # 모든 워크스트림 목록
/sdd-workstreams create backend-api # 새 워크스트림 생성
/sdd-workstreams switch backend-api # 활성 워크스트림 설정
/sdd-workstreams status backend-api # 상세 상태 확인
/sdd-workstreams progress           # 교차 워크스트림 진행 상황 개요
/sdd-workstreams complete backend-api  # 완료된 워크스트림 아카이브
/sdd-workstreams resume backend-api    # 워크스트림 작업 재개
```
...
```bash
/sdd-map-codebase                   # 전체 코드베이스 분석
/sdd-map-codebase auth              # 인증 영역에 집중
```
...
**`/sdd-quick`의 대체가 아닙니다.** 조사, 다단계 계획 또는 검증이 필요한 작업에는 `/sdd-quick`을 사용하세요.
...
**생성 파일:** `{phase}-REVIEWS.md` — `/sdd-plan-phase --reviews`에서 사용 가능
...
**목적:** 리뷰어에게 SDD 계획 아티팩트가 아닌 코드 변경사항만 표시합니다.

```bash
/sdd-pr-branch                     # main을 기준으로 필터링
/sdd-pr-branch develop             # develop을 기준으로 필터링
```
...
스레드는 여러 세션에 걸쳐 이어지지만 특정 페이즈에 속하지 않는 작업을 위한 경량 교차 세션 지식 저장소입니다. `/sdd-pause-work`보다 가볍습니다.

```bash
/sdd-thread                         # 모든 스레드 목록
/sdd-thread fix-deploy-key-auth     # 스레드 재개
/sdd-thread "Investigate TCP timeout in pasta service"  # 새 스레드 생성
```
