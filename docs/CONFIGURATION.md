...
| `context_window` | number | any integer | `200000` | Context window size in tokens. Set `1000000` for 1M-context models (e.g., `claude-opus-4-7[1m]`). Values `>= 500000` enable adaptive context enrichment (full-body reads of prior SUMMARY.md, deeper anti-pattern reads). Configured via `/sdd-config --advanced`. |
...
| `brave_search` | boolean | `true`/`false` | auto-detected | Override auto-detection of Brave Search API availability. When unset, SDD checks for `BRAVE_API_KEY` env var or `~/.sdd/brave_api_key` file |
| `firecrawl` | boolean | `true`/`false` | auto-detected | Override auto-detection of Firecrawl API availability. When unset, SDD checks for `FIRECRAWL_API_KEY` env var or `~/.sdd/firecrawl_api_key` file |
| `exa_search` | boolean | `true`/`false` | auto-detected | Override auto-detection of Exa Search API availability. When unset, SDD checks for `EXA_API_KEY` env var or `~/.sdd/exa_api_key` file |
...
Configured interactively via [`/sdd-config --integrations`](COMMANDS.md#sdd-config). These are *connectivity* settings — API keys and cross-tool routing — and are intentionally kept separate from `/sdd-settings` (workflow toggles).

### Search API keys

API key fields accept a string value (the key itself). They can also be set to the sentinels `true`/`false`/`null` to override auto-detection from env vars / `~/.sdd/*_api_key` files (legacy behavior, see rows above).
...
| `workflow.ui_review` | boolean | `true` | Run visual quality audit (`/sdd-ui-review`) after phase execution in autonomous mode. When `false`, the UI audit step is skipped. |
...
| `workflow.plan_review_convergence` | boolean | `false` | Enable the `/sdd-plan-review-convergence` command. Disabled by default — the command exits with an enable instruction when this key is `false`. The command automates the manual plan→review→replan loop: it spawns configured reviewers (Codex, Gemini, Claude, OpenCode, Ollama, LM Studio, llama.cpp), counts unresolved HIGH concerns via the CYCLE_SUMMARY contract, replans with `--reviews` feedback, and repeats until converged or max cycles reached. Enable with `sdd config-set workflow.plan_review_convergence true`. Added in v1.39 |
...
| `workflow.ai_integration_phase` | boolean | `true` | Enable the `/sdd-ai-integration-phase` command. When `false`, the command exits with a configuration gate message |
...
| `planning.sub_repos` | array of strings | `[]` | Paths of nested sub-repos relative to the project root. When set, SDD-aware tooling scopes phase-lookup, path-resolution, and commit operations per sub-repo instead of treating the outer repo as a monorepo |

### Project-Root Resolution in Multi-Repo Workspaces

When `sub_repos` is set and `sdd-tools.cjs` or `sdd-sdk query` is invoked from inside a listed child repo, both CLIs walk up to the parent workspace that owns `.planning/` before dispatching handlers. Resolution order (checked at each ancestor up to 10 levels, never above `$HOME`):
...
Inject custom skill files into SDD subagent prompts. Skills are read by agents at spawn time, giving them project-specific instructions beyond what CLAUDE.md provides.
...
{
  "agent_skills": {
    "sdd-executor": ["skills/testing-standards", "skills/api-conventions"],
    "sdd-planner": ["skills/architecture-rules"],
    "sdd-verifier": ["skills/acceptance-criteria"]
  }
}
```
...
Any SDD agent type can receive skills. Common types:

- `sdd-executor` -- executes implementation plans
- `sdd-planner` -- creates phase plans
- `sdd-checker` -- verifies plan quality
- `sdd-verifier` -- post-execution verification
- `sdd-researcher` -- phase research
- `sdd-project-researcher` -- new-project research
- `sdd-debugger` -- diagnostic agents
- `sdd-codebase-mapper` -- codebase analysis
- `sdd-advisor` -- discuss-phase advisors
- `sdd-ui-researcher` -- UI design contract creation
- `sdd-ui-checker` -- UI spec verification
- `sdd-roadmapper` -- roadmap creation
- `sdd-synthesizer` -- research synthesis

### How It Works

At spawn time, workflows call `sdd-sdk query agent-skills <type>` (or legacy `node sdd-tools.cjs agent-skills <type>`) to load configured skills. If skills exist for the agent type, they are injected as an `<agent_skills>` block in the Task() prompt:
...
```bash
sdd-sdk query config-set agent_skills.sdd-executor '["skills/my-skill"]'
```
...
| `intel.enabled` | boolean | `false` | Enable queryable codebase intelligence system. When `true`, `/sdd-map-codebase --query` commands build and query a JSON index in `.planning/intel/`. Added in v1.34 |
...
| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `graphify.enabled` | boolean | `false` | Enable the project knowledge graph. When `true`, `/sdd-graphify` builds and queries a graph in `.planning/graphs/`. Added in v1.36 |
| `graphify.build_timeout` | number (seconds) | `300` | Maximum seconds allowed for a `/sdd-graphify build` run before it aborts. Added in v1.36 |
...
`built_at_commit` freshness signal that `/sdd-graphify status` surfaces.

#### Commit-based staleness

`/sdd-graphify status` reports two orthogonal staleness signals:
...
```bash
# Enable a feature
sdd-sdk query config-set features.global_learnings true

# Disable a feature
sdd-sdk query config-set features.thinking_partner false
```
...
`STATE.md` carries YAML frontmatter that the status-line hook reads on every render. v1.40 adds four optional phase-lifecycle fields read by `parseStateMd()` and rendered by `formatSddState()`:
...
| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `workflow.security_enforcement` | boolean | `true` | Enable threat-model-anchored security verification via `/sdd-secure-phase`. When `false`, security checks are skipped entirely |
...
Configure per-CLI model selection for `/sdd-review`. When set, overrides the CLI's default model for that reviewer.
...
| sdd-pattern-mapper | Sonnet | Sonnet | Haiku | Inherit |
| sdd-ui-researcher | Opus | Sonnet | Haiku | Inherit |
| sdd-ui-checker | Sonnet | Sonnet | Haiku | Inherit |
| sdd-ui-auditor | Sonnet | Sonnet | Haiku | Inherit |
...
or `~/.sdd/defaults.json` (global). Per-project entries win on conflict and
...
`model_overrides` is per-**agent** (precise but verbose; you have to know that `sdd-codebase-mapper` is research and `sdd-doc-writer` is execution). The `models` block lets you say "Opus for planning and execution, Sonnet for the rest" in two lines:
...
The five layers compose top-down: `model_profile` is the base tier, `models[<phase_type>]` overrides at the phase level, `dynamic_routing` (when enabled) escalates per-attempt on soft failure, `model_overrides[<agent>]` carves per-agent exceptions at the top, and the runtime default applies when nothing else does. In the example above, all five research agents resolve to `sonnet` *except* `sdd-codebase-mapper`, which the per-agent override pins to `haiku`. `dynamic_routing` is disabled by default — when off (`enabled: false` or block omitted), this section's behavior is unchanged from today.
...
```bash
$ sdd config-set models.deployment opus
Error: 'models.deployment' is not a valid config key

# Valid:
$ sdd config-set models.research sonnet
```
...
When SDD is installed for a non-Claude runtime, the installer automatically sets `resolve_model_ids: "omit"` in `~/.sdd/defaults.json`. This causes SDD to return an empty model parameter for all agents, so each agent uses whatever model the runtime is configured with. No additional setup is needed for the default case.
...
This resolves `sdd-planner` → `gpt-5.4` (xhigh), `sdd-executor` → `gpt-5.3-codex` (medium), `sdd-codebase-mapper` → `gpt-5.4-mini` (medium). The Codex installer embeds `model = "..."` and `model_reasoning_effort = "..."` in each generated agent TOML.
...
**Unknown runtimes.** If `runtime` is set to a value with no built-in tier map and no `model_profile_overrides[<runtime>]`, SDD falls back to the Claude-alias safe default rather than emit a model ID the runtime cannot accept. To support a new runtime, populate `model_profile_overrides.<runtime>.{opus,sonnet,haiku}` with valid IDs.
