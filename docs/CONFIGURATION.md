...
| `claude_md_path` | string | any file path | (none) | Custom output path for the generated CLAUDE.md file. Useful for monorepos or projects that need CLAUDE.md in a non-root location. When set, SDD writes its CLAUDE.md content to this path instead of the project root. Added in v1.36 |
...
At spawn time, workflows call `node sdd-tools.cjs agent-skills <type>` to load configured skills. If skills exist for the agent type, they are injected as an `<agent_skills>` block in the Task() prompt:
...
```bash
node sdd-tools.cjs config-set agent_skills.sdd-executor '["skills/my-skill"]'
```
