...
first being merged with `main` — see [#2856](https://github.com/bhargavvc/sdd-cc/issues/2856)).
...
- **Manual canary release workflow** — `.github/workflows/canary.yml` publishes
  `{base}-canary.{N}` builds of `@bhargavvc/sdd-cc` under the `canary` dist-tag on
...
- **`audit-uat` parser reads `human_verification:` from frontmatter array** — the
  ...
  audit. ([#2788](https://github.com/bhargavvc/sdd-cc/issues/2788))
- **Skill description anti-patterns trimmed; ≤ 100-char budget enforced** — three
  anti-patterns eliminated across `commands/sdd/*.md`: flag documentation already in
  ...
  chars. ([#2789](https://github.com/bhargavvc/sdd-cc/issues/2789))
- **`sdd-sdk` binary collision with `@bhargavvc/sdk` resolved** — workstream-aware
  query registry now respects the `SDD_WORKSTREAM` env var; `sdd-tools` bin alias
  added. ([#2791](https://github.com/bhargavvc/sdd-cc/issues/2791))
- **`OpenCode` agents embed `model_profile_overrides.opencode.<tier>`** — per-tier
  model overrides set via `/sdd-settings-advanced` are now propagated into generated
  agent files. ([#2794](https://github.com/bhargavvc/sdd-cc/issues/2794))
- **`roadmap update-plan-progress` accepts `--phase` flag form** — SDK arg-parsing
  ...
  STATE.md corruption. ([#2796](https://github.com/bhargavvc/sdd-cc/issues/2796))
- **`context_window` added to `VALID_CONFIG_KEYS` allowlist** —
  `/sdd-settings-advanced` could not set `context_window` because the key was missing
  from the allowlist used by `config-set` validation.
  ([#2798](https://github.com/bhargavvc/sdd-cc/issues/2798))
- **`sdd-tools init` dispatches `ingest-docs` handler** — `/sdd-ingest-docs` was
  broken in v1.38.5 because the workflow called the new tool but no `ingest-docs`
  init handler was registered. ([#2801](https://github.com/bhargavvc/sdd-cc/issues/2801))
- **`config-get` honors `--default <value>` flag** — fallback for missing keys
  ported from CJS into the SDK. ([#2803](https://github.com/bhargavvc/sdd-cc/issues/2803))
...
- **SKILL.md frontmatter `name:` migrated to hyphen form** — files that still used
  the deprecated colon form (`sdd:cmd`) caused autocomplete to suggest `/sdd:command`.
  ([#2808](https://github.com/bhargavvc/sdd-cc/issues/2808))
- **`sdd-sdk` resolvable in local-mode installs** — the previous `isLocal`
  ...
  as global installs. ([#2829](https://github.com/bhargavvc/sdd-cc/issues/2829))
...
- **`sdd-sdk auto` detects Codex runtime correctly** — `auto` mode ignored
  ...
  clear error for non-Claude runtimes; `resolveModel()` honours `SDD_RUNTIME` env
...
- **`/sdd-code-review-fix` cleanup tail is transactional** — JSON recovery sentinel
  ...
  self-healing across crashes. ([#2839](https://github.com/bhargavvc/sdd-cc/issues/2839))
...
**Codex hooks migrator correctness hardening** ([#2809](https://github.com/bhargavvc/sdd-cc/issues/2809))
...
| Handler command path assertion in install tests used a regex (`/sdd-check-update\.js/`) rather than the exact absolute path | Strengthened to `assert.strictEqual` with `path.join(codexHome, 'hooks', 'sdd-check-update.js')` |
...
**`--minimal` install flag** (alias `--core-only`) ([#2762](https://github.com/bhargavvc/sdd-cc/issues/2762))
...
No `sdd-*` subagents are installed.
...
The install manifest records `mode: "minimal" | "full"`. Run `sdd update` without
`--minimal` at any time to expand to the full skill set.

### Fixed (rc.4)

**Codex install no longer corrupts `~/.codex/config.toml`** ([#2760](https://github.com/bhargavvc/sdd-cc/issues/2760))
...
```bash
npm install -g @bhargavvc/sdd-cc@1.39.0-rc.7
```
