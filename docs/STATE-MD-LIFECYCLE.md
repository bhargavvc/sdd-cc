...
> `formatSddState()` renders the in-flight / idle / progress scenes.
> SDK write-side support to maintain the fields automatically is tracked
> separately.

SDD's `STATE.md` carries YAML frontmatter that the status-line hook reads on
...
`formatSddState()` checks the lifecycle fields in the order below and emits
...
This is enforced by check order in `formatSddState()` and by tests in
...
The `formatSddState #2833 backward compatibility` test suite locks this
...
- **#1989** — *enhancement: surface SDD state in statusline.* The foundation
