...
Este plano formaliza a materialização explícita da configuração do projeto durante `/sdd:new-project`, garantindo que escolhas feitas na inicialização sejam persistidas de forma determinística em `.planning/config.json`.
...
- após `/sdd:new-project`, `config.json` reflete as escolhas feitas
