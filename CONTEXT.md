...
Compatibility Adapter Module for `sdd-tools.cjs` command families. Uses generated command metadata plus small argument shapers to route to CJS handlers, rather than calling SDK Command Topology directly. Preserves CJS compatibility startup while reducing hand-written router drift.
...
Module owning SDK-to-`@bhargavvc/sdd-cc` compatibility policy: legacy asset discovery, install-layout probing, transition-only error messaging, and thin Adapter access for CJS-era assets that native SDK Modules have not replaced yet.
...
Module owning which skills and agents are written to runtime config directories at install time (Phase 1) and at runtime via cluster-level toggles (Phase 2). Phase 1: `sdd/bin/lib/install-profiles.cjs` defines named profiles (`core`, `standard`, `full`), computes transitive closure over `requires:` frontmatter, stages skills/agents to runtime config dirs, and persists the chosen profile in a `.sdd-profile` marker. Profile resolution precedence: explicit `--profile=` flag > `.sdd-profile` marker > `full`. `--minimal`/`--core-only` are back-compat aliases for `--profile=core`. Phase 2: `sdd/bin/lib/surface.cjs` implements the `/sdd:surface` slash command for cluster-level enable/disable without reinstall; cluster definitions live in `sdd/bin/lib/clusters.cjs`; per-runtime state persists in `<runtimeConfigDir>/.sdd-surface.json` independent from the `.sdd-profile` marker. See ADR-0011.
...
### User Story
Phase-goal format under MVP Mode: `As a [role], I want to [capability], so that [outcome].` Required regex shape: `/^As a .+, I want to .+, so that .+\.$/`. Used as the framing input by `sdd-planner` (emits as bolded `## Phase Goal` header in PLAN.md) and as the verification target by `sdd-verifier` (the `[outcome]` clause is the goal-backward verification anchor). Authored interactively by `/sdd-mvp-phase`, validated by SPIDR Splitting when too large.
...
- When a command absorbs a micro-skill as a flag (e.g. `capture --backlog`), the old command's process steps must be ported to a `sdd/workflows/<name>.md` file. The routing wrapper in `commands/sdd/*.md` declares an `execution_context` `@`-reference to that workflow — if the file doesn't exist the agent loads nothing and has no steps to follow.
- **Detection**: `tests/bug-3135-capture-backlog-workflow.test.cjs` adds a broad regression — every `execution_context` `@`-reference in any `commands/sdd/*.md` must resolve to an existing file on disk. This test will catch all future gaps of this class immediately.
...
- When a flag absorbs a micro-skill, the old skill's `Invoked by` attribution in INVENTORY must move to the new parent (e.g. `add-todo.md` incorrectly claimed `/sdd-capture --backlog` until #3135 corrected it).
...
- Any `sdd-sdk query` verb implemented only in the SDK native registry (no `sdd-tools.cjs` mirror) must be added to `NO_CJS_SUBPROCESS_REASON` in `sdk/src/golden/golden-policy.ts`. Without this entry the golden-policy test fails, treating the verb as a missing implementation rather than an intentional SDK-only path.
...
- Preserved execution policy defaults by passing `allowFallbackToSubprocess` through as `undefined` when unset (no forced override in `SDDTools`).
...
`RULESET.CODERABBIT.GUARD.OPEN_PRS=gh pr list --repo bhargavvc/sdd-cc --author @me --state open; repeat near end because open PR set can change mid-run`
...
`WORKTREE.SEAM.files=[sdd/bin/lib/worktree-safety.cjs, sdd/bin/lib/core.cjs]`
...
`WORKSTREAM.NAME.POLICY.cjs-module=sdd/bin/lib/workstream-name-policy.cjs owns toWorkstreamSlug + active-name/path-segment validation`
...
`CONFIG.SEAM.loadConfig-context=loadConfig(cwd,{workstream}) replaces env-mutation fallback; no temporary process.env SDD_WORKSTREAM rewrites`
...
`RELEASE-NOTES.STANDARD.footer.hotfix=Install/upgrade: \`npx @bhargavvc/sdd-cc@latest\``
`RELEASE-NOTES.STANDARD.footer.rc=Install for testing: \`npx @bhargavvc/sdd-cc@next\` (per branch->dist-tag policy)`
`RELEASE-NOTES.STANDARD.footer.canary=Install: \`npx @bhargavvc/sdd-cc@canary\``
`RELEASE-NOTES.STANDARD.footer.full-changelog=**Full Changelog**: https://github.com/bhargavvc/sdd-cc/compare/<prev>...<this>`
...
`RELEASE-NOTES.EXAMPLE.hotfix=v1.41.1 (https://github.com/bhargavvc/sdd-cc/releases/tag/v1.41.1) - 14 fixes grouped by 6 subgroups`
`RELEASE-NOTES.EXAMPLE.rc=v1.42.0-rc1 (https://github.com/bhargavvc/sdd-cc/releases/tag/v1.42.0-rc1) - intro + Added/Changed/Fixed/Documentation taxonomy`
...
`PRED.k320.ci-paths-monitored=bin/ sdd/ agents/ commands/ hooks/ sdk/src/`
...
`WAVE.PR.3299.fix=agents/sdd-intel-updater.md layout-detection block gated on framework-repo check`
...
`WAVE.PR.3308.fix=extract sdd/bin/lib/plan-scan.cjs scanPhasePlans; port 4 call sites in init/state/roadmap/phase`
...
`WAVE.PR.3304.fix=sdd/bin/sdd-tools.cjs --json-errors flag + SDD_JSON_ERRORS env + docs/json-errors.md taxonomy + usage-string disclosure (CR k321 finding addressed)`
...
`DEFECT.PORT-DRIFT.cjs-sdk.examples=#3317 (skills missing from SDK SDD_MANAGED_DIRS), #3240 (extractFrontmatter anchor), #3226 (phase.add --dry-run), #3243 (cjs dotted canonical), #3229 (model catalog source-of-truth)`
...
`DEFECT.REMOVED-BUT-NEEDED.examples=#3316 root package-lock.json (root package.json declares deps; workflows use cache:'npm' + npm ci), e3b52c70 docs referenced removed /sdd-new-workspace`
`DEFECT.REMOVED-BUT-NEEDED.detect=before deletion, grep filename across .github/workflows, sdd/, docs/, package.json scripts, sdk/scripts; if any reference exists removal is incomplete`
...
`DEFECT.STACKED-PR-AUTO-RETARGET.examples=#3311 base fix/3255-add-json-errors-mode-sdd-tools deleted after #3304 merged`
...
`DEFECT.INVENTORY-DRIFT.symptom=new file added under sdd/references/ or sdd/workflows/ without updating docs/INVENTORY.md count + row AND docs/INVENTORY-MANIFEST.json`
...
`DEFECT.AGENT-FILE-SIZE-CAP-BREACH.symptom=adding to agents/sdd-planner.md (or other large agent files) exceeds the 45K char extraction-evidence threshold`
`DEFECT.AGENT-FILE-SIZE-CAP-BREACH.state=sdd-planner.md is already 49,121 chars on main (over 45K); test fails on main; net-new content makes it strictly worse`
...
`DEFECT.AGENT-FILE-SIZE-CAP-BREACH.fix-forward=mirror MVP mode pattern — extract full rules to sdd/references/planner-<mode>.md, leave a slim Detection section in the agent file with @-reference to the new file`
...
`DEFECT.DEFAULT-FLIP-DOCUMENTATION.fix-forward=template — "new default takes effect when .planning/config.json is rewritten (config-set, fresh project, regenerated config); existing artifacts continue to work; opt-back-in: sdd config-set <key> <old-value>"`
...
`DEFECT.SOURCE-GREP-IN-NEW-TESTS.fix-forward=replace with runSddTools(...) behavioral test capturing JSON; if asserting agent .md content (which IS the runtime contract) add // allow-test-rule: source-text-is-the-product with one-line justification`
...
- scope: `bhargavvc/sdd-cc` only; run all `gh` checks with `--repo bhargavvc/sdd-cc`.
...
- post-rebase CI drift classes to verify immediately:
  ...
  - slash namespace invariant (`tests/bug-2543-sdd-slash-namespace.test.cjs`)
- concrete regressions fixed in this session:
  ...
  - replace legacy `/sdd-review` mention with canonical `/sdd:review` in source comments

### 2026-05-13 — Phase 1 rebase + PR open (#3465)

- branch: `feat/3465-shell-projection-platform-io-seam`; PR: #3470 on `bhargavvc/sdd-cc`
...
- workflow wiring lives in `sdd/workflows/execute-phase.md` step 7; replaces single-branch "real failure" routing with class-distinct prompts (quota → wait-for-reset; classify-handoff-bug → existing spot-check; unknown → existing continue/stop)
...
- proactive-signal forward path (NOT yet usable from SDD orchestrator): Anthropic exposes `anthropic-ratelimit-{requests,tokens,input-tokens}-remaining` + `retry-after`; Agent SDK emits `RateLimitEvent` with `status: 'allowed' | 'allowed_warning' | 'rejected'`. Claude Code subprocess does NOT forward these to hooks/statusline today — tracked in upstream anthropics/claude-code#33820, #22407, #32796
...
- `cmdStateCompletePhase` in `sdd/bin/lib/state.cjs` previously wrote STATE.md unconditionally; re-invoking on a phase already superseded by a later one rewound Status / Last Activity / Current Position to stale values silently
...
- legacy-bridge banner `'state.complete-phase' not in native registry; falling back to sdd-tools.cjs` is the user-visible signal that this path runs the .cjs handler, not a typed SDK port — when fixing handler bugs, the .cjs path is canonical until ported
...
- right primitive: `git rev-parse --is-inside-work-tree` + `git rev-parse --show-toplevel`, surfaced through the existing `execGit` seam in `sdd/bin/lib/core.cjs`
