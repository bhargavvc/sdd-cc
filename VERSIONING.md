...
SDD follows [Semantic Versioning 2.0.0](https://semver.org/) with three release tiers mapped to npm dist-tags.

## Release Tiers

| Tier | What ships | Version format | npm tag | Branch | Install |
|------|-----------|---------------|---------|--------|---------|
| **Patch** | Bug fixes only | `1.27.1` | `latest` | `hotfix/1.27.1` | `npx @bhargavvc/sdd-cc@latest` |
| **Minor** | Fixes + enhancements | `1.28.0` | `latest` (after RC) | `release/1.28.0` | `npx @bhargavvc/sdd-cc@next` (RC) |
| **Major** | Fixes + enhancements + features | `2.0.0` | `latest` (after beta) | `release/2.0.0` | `npx @bhargavvc/sdd-cc@next` (beta) |
...
|-----|---------|-------------|
| `latest` | Stable production release | `npm install @bhargavvc/sdd-cc` (default) |
| `next` | Pre-release (RC or beta) | `npm install @bhargavvc/sdd-cc@next` (opt-in) |
...
3. Trigger `hotfix.yml` with `action=finalize`. The workflow:
   ...
   - Builds SDK, bundles `sdk-bundle/sdd-sdk.tgz` inside the CC tarball (parity with `release-sdk.yml`).
...
Active while the `@bhargavvc/sdk` npm token is unavailable; bundles the SDK inside the CC tarball.
...
4. Test the RC: `npx @bhargavvc/sdd-cc@next`
...
npm dist-tag ls @bhargavvc/sdd-cc
```
