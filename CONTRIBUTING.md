...
SDD accepts three types of contributions. Each type has a different process and a different bar for acceptance. **Read this section before opening anything.**
...
**Process:**
1. Open a [Bug Report issue](https://github.com/bhargavvc/sdd-cc/issues/new?template=bug_report.yml) — fill it out completely.
...
**Process:**
1. Open an [Enhancement issue](https://github.com/bhargavvc/sdd-cc/issues/new?template=enhancement.yml) with the full proposal.  The issue template requires: the problem being solved, the concrete benefit, the scope of changes, and alternatives considered.
...
**Process:**
1. **Discuss first** — check [Discussions](https://github.com/bhargavvc/sdd-cc/discussions) to see if the idea has been raised. If it has and was declined, don't open a new issue.
2. Open a [Feature Request issue](https://github.com/bhargavvc/sdd-cc/issues/new?template=feature_request.yml) with the complete spec. The template requires: the solo-developer problem being solved, what is being added, full scope of affected files and systems, user stories, acceptance criteria, and assessment of maintenance burden.
3. **Wait for maintainer approval.** A maintainer must label the issue `approved-feature` before you write a single line of code. Approval is not guaranteed — SDD is intentionally lean and many valid ideas are declined because they conflict with the project's design philosophy.
...
**Rejection reasons:** Issue not labeled `approved-feature`, spec is incomplete, scope exceeds what was approved, feature conflicts with SDD's solo-developer focus, maintenance burden too high.
...
npm run changeset -- --type Fixed --pr <YOUR_PR_NUMBER> \
  --body "**\`/sdd-foo\` no longer drops trailing slashes** — explain the user-visible change."
```
...
Fragments are consolidated into `CHANGELOG.md` at release time by the release workflow. See [`.changeset/README.md`](.changeset/README.md) for the format spec and [#2975](https://github.com/bhargavvc/sdd-cc/issues/2975) for the rationale.

**CI enforcement:** the `Changeset Required` workflow (`scripts/changeset/lint.cjs`) fails any PR that touches `bin/`, `sdd/`, `agents/`, `commands/`, `hooks/`, or `sdk/src/` without a `.changeset/*.md` fragment.
...
const configSrc = fs.readFileSync(
  path.join(SDD_ROOT, 'bin', 'lib', 'config-schema.cjs'), 'utf-8'
);
assert.ok(
...
test('config-set accepts workflow.plan_bounce', (t) => {
  ...
  const result = runSddTools('config-set workflow.plan_bounce true', tmpDir);
...
| `structural-implementation-guard` | A feature's interception or wiring point is not reachable end-to-end via `runSddTools`. Used temporarily until a behavioral path exists. |
...
const cmdContent = fs.readFileSync(path.join(tmpDir, 'sdd-sdk.cmd'), 'utf8');
...
`buildWindowsShimTriple(shimSrc)` in `bin/install.js` is the canonical IR pattern: pure function, no I/O, returns `{ invocation, eol, fileNames, render }`. `trySelfLinkSddSdkWindows` calls it and writes `triple.render[kind]()` to disk. Tests assert on `triple.invocation.target`, `triple.eol.cmd`, `Object.keys(triple).sort()` — never on the rendered text. Filesystem-level tests assert `fs.statSync(target).size === Buffer.byteLength(triple.render.cmd())` to prove the writer writes what the renderer produces, **without comparing content**.
...
if git diff --cached --name-only | grep -Eq "^sdk/src/query/command-manifest\.|^sdk/src/query/command-aliases\.generated\.ts$|^sdd/bin/lib/command-aliases\.generated\.cjs$|^sdk/scripts/gen-command-aliases\.ts$"; then
  npm run check:alias-drift
fi
EOF
...
export SDD_BLOCKED_AUTHOR_REGEX='@example-corp\\.com$'
...
blocked_regex="${SDD_BLOCKED_AUTHOR_REGEX:-}"
...
|-----|----------------|-------------|
| `lint-tests` | No source-grep tests (see above) | Replace with `runSddTools()` behavioral tests, or add `// allow-test-rule: <reason>` |
...
Only `agents/` at the repo root is tracked by git. The following directories may exist on a developer machine with SDD installed and **must not be edited** — they are install-sync outputs and will be overwritten:
...
| `.github/agents/sdd-*` | Yes (`.gitignore:37`) | Local CI-surface bundle |
