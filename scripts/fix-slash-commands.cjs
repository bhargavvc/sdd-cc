'use strict';
/**
 * One-shot script: replace retired /sdd-<cmd> with /sdd:<cmd> for known command names.
 ...
 * (see tests/bug-2543-sdd-slash-namespace.test.cjs) without needing fixture files.
 */
...
const SEARCH_DIRS = [
  path.join(__dirname, '..', 'sdd', 'bin', 'lib'),
  path.join(__dirname, '..', 'sdd', 'workflows'),
  path.join(__dirname, '..', 'sdd', 'references'),
  path.join(__dirname, '..', 'sdd', 'templates'),
  path.join(__dirname, '..', 'sdd', 'contexts'),
  path.join(__dirname, '..', 'commands', 'sdd'),
...
function buildPattern(cmdNames) {
  // Empty input would compile `/sdd-()(?=[^a-zA-Z0-9_-]|$)/g`, which the regex
  // engine still matches at any `/sdd-` token followed by a non-word boundary
  // (e.g. EOL, whitespace, punctuation) — rewriting it to a stray `/sdd:`.
  ...
  return new RegExp(`/sdd-(${sorted.join('|')})(?=[^a-zA-Z0-9_-]|$)`, 'g');
}

/**
 * Pure transform: rewrite retired `/sdd-<cmd>` to `/sdd:<cmd>` for the given command names.
 * Returns the rewritten string. Identifiers not in `cmdNames` (e.g. `/sdd-sdk`,
 * `/sdd-tools`) are left untouched.
 */
function transformContent(src, cmdNames) {
  const pattern = buildPattern(cmdNames);
  if (!pattern) return src;
  return src.replace(pattern, (_, cmd) => `/sdd:${cmd}`);
}
