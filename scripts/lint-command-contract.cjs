 ...
 *
 * Enforces the commands/sdd/*.md contract across all 65 command files:
 *
 *   1. name:        present, non-empty, matches sdd: or sdd- prefix
...
function check(filePath) {
  ...
  // 1. name: present + sdd: / sdd- prefix
  if (!fm.name || !fm.name.trim()) {
    violations.push('name: field missing or empty');
  } else if (!/^sdd[:-]/.test(fm.name.trim())) {
    violations.push(`name: must start with "sdd:" or "sdd-", got "${fm.name.trim()}"`);
  }
  ...
  for (const { token, normalized, trailingProse } of refs) {
    const absPath = path.join(SDD_ROOT, normalized);
