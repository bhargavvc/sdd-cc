#!/usr/bin/env bash
# Verify the published @bhargavvc/sdd-cc tarball actually contains
...
echo "==> Installing runtime deps inside the extracted package and invoking sdd-sdk query --help"
...
echo "==> Also verifying sdd-sdk bin shim resolves ../sdk/dist/cli.js"
SHIM="$EXTRACT_DIR/package/bin/sdd-sdk.js"
if [ ! -f "$SHIM" ]; then
  echo "::error::bin/sdd-sdk.js missing from tarball"
  exit 1
fi
if ! grep -qE "sdk.*dist.*cli\.js" "$SHIM"; then
  echo "::error::bin/sdd-sdk.js does not reference sdk/dist/cli.js"
  exit 1
fi
