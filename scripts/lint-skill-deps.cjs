 ...
 *    For each commands/sdd/*.md, parse requires: and walk the body for
 *    references to other SDD skills (pattern: /sdd:<stem> or sdd:<stem>).
 ...
 *
 * Usage:
 *   node scripts/lint-skill-deps.cjs               # scans commands/sdd/
...
const PROFILES_MODULE = path.join(__dirname, '..', 'sdd', 'bin', 'lib', 'install-profiles.cjs');
...
let commandsDir = path.join(__dirname, '..', 'commands', 'sdd');
    ...
    for (const ref of referenced) {
      ...
      if (!allStems.has(ref)) {
        violations.push({
          ...
          filePath,
          undeclared: ref,
          message: 'body references unknown skill sdd:' + ref,
        });
        continue;
      }
      if (!declaredSet.has(ref)) {
        violations.push({
          ...
          filePath,
          undeclared: ref,
          message: 'body references sdd:' + ref + ' but requires: does not list it',
        });
      }
    }
  }
