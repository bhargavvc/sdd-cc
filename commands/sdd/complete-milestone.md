...
- @~/.claude/sdd/workflows/complete-milestone.md (main workflow)
- @~/.claude/sdd/templates/milestone-archive.md (archive template)
  </execution_context>
...
0. **Check for audit:**
   ...
   - If audit status is `gaps_found`: recommend closing the gaps inline
     ...
     via `/sdd:phase --insert <N>` plus the standard
   ...
   a closure phase per gap with `/sdd:phase --insert <N>` and run the
   standard `/sdd:discuss-phase` → `/sdd:plan-phase` → `/sdd:execute-phase`
