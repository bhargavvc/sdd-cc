...
<objective>
Manage the runtime skill surface without reinstall. Reads/writes `~/.claude/skills/.sdd-surface.json`
(sibling to `.sdd-profile`) and re-stages the active commands/sdd directory in place.
...
| `reset` | Delete `.sdd-surface.json`, return to install-time profile |
| *(none)* | Treat as `list` |
...
`sdd/bin/lib/surface.cjs`. Display:
...
```
Base profile:   standard  (from .sdd-surface.json)
Install profile: standard  (from .sdd-profile)
```
...
1. Check if `.sdd-surface.json` exists.
2. Delete it.
...
COMMANDS_DIR=~/.claude/commands/sdd
...
- Missing `surface.cjs` → prompt: "Run `npm i -g sdd` to reinstall SDD."

<execution_context>
Surface state file: `~/.claude/skills/.sdd-surface.json`
Install profile marker: `~/.claude/skills/.sdd-profile`
Engine module: `~/.claude/sdd/bin/lib/surface.cjs`
Cluster definitions: `~/.claude/sdd/bin/lib/clusters.cjs`
</execution_context>
