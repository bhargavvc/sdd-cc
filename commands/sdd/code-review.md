...
- `--fix` (optional) — after review completes (or if REVIEW.md already exists), auto-apply fixes found. Spawns sdd-code-fixer agent. Accepts sub-flags:
...
Context files (CLAUDE.md, SUMMARY.md, phase state) are resolved inside the workflow via `sdd-sdk query init.phase-op` and delegated to agent via `<files_to_read>` blocks.
</context>
