...
2. **Find next backlog number:**
   ```bash
   NEXT=$(sdd-sdk query phase.next-decimal 999 --raw)
   ```
...
5. **Commit:**
   ```bash
   sdd-sdk query commit "docs: add backlog item ${NEXT} — ${ARGUMENTS}" .planning/ROADMAP.md ".planning/phases/${NEXT}-${SLUG}/.gitkeep"
   ```
