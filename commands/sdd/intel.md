...
Intel system is disabled. To activate:
  ...
  sdd-sdk query config-set intel.enabled true
...
```bash
sdd-sdk query intel.query <term>
```
...
Project root: ${CWD}
Prefer: sdd-sdk query <subcommand> (installed sdd-sdk on PATH). Legacy: node $HOME/.claude/sdd/bin/sdd-tools.cjs
...
4. Use `sdd-sdk query intel.extract-exports <file>` to analyze source files
5. Use `sdd-sdk query intel.patch-meta <file>` to update timestamps after writing
6. Use `sdd-sdk query intel.validate` to check your output
