...
**CJS-only (graphify):** `graphify` subcommands are not registered on `sdd-sdk query`. Use `node $HOME/.claude/sdd/bin/sdd-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `sdd-sdk query` where a handler exists.
...
Knowledge graph is disabled. To activate:
  ...
  node $HOME/.claude/sdd/bin/sdd-tools.cjs config-set graphify.enabled true

Then run /sdd:graphify build to create the initial graph.
```
...
```bash
node $HOME/.claude/sdd/bin/sdd-tools.cjs graphify query <term>
```

Parse the JSON output and display results:
- If the output contains `"disabled": true`, display the disabled message from Step 1 and **STOP**
- If the output contains `"error"` field, display the error message and **STOP**
- If no nodes found, display: `No graph matches for '<term>'. Try /sdd:graphify build to create or rebuild the graph.`
...
```bash
node $HOME/.claude/sdd/bin/sdd-tools.cjs graphify diff
```
...
```bash
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" graphify build
```
...
graphify update . \
  ...
  && node "$HOME/.claude/sdd/bin/sdd-tools.cjs" graphify build snapshot \
  && node "$HOME/.claude/sdd/bin/sdd-tools.cjs" graphify status
```
...
**MVP-mode rendering.** When a phase has `**Mode:** mvp` in ROADMAP.md (resolved via `sdd-sdk query roadmap.get-phase --pick mode`), render its graph node with two distinct visual signals:
...
5. DO NOT use `sdd-tools config get-value` for the config gate -- it exits on missing keys.
