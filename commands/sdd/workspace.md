---
name: sdd:workspace
description: Manage SDD workspaces — create, list, or remove isolated workspace environments
...
<objective>
Manage SDD workspaces with a single consolidated command.

Mode routing:
- **--new**: Create an isolated workspace with repo copies and independent .planning/ → new-workspace workflow
- **--list**: List active SDD workspaces and their status → list-workspaces workflow
- **--remove**: Remove a SDD workspace and clean up worktrees → remove-workspace workflow
</objective>
...
| --list | Scan ~/sdd-workspaces/, show summary table | list-workspaces |
