...
- Read frontmatter `status` field via:
  ```bash
  sdd-sdk query frontmatter.get .planning/threads/{file} status 2>/dev/null
  ```
...
3. Commit:
   ```bash
   sdd-sdk query commit "docs: resolve thread — {SLUG}" ".planning/threads/{SLUG}.md"
   ```
...
5. Commit:
   ```bash
   sdd-sdk query commit "docs: create thread — ${ARGUMENTS}" ".planning/threads/${SLUG}.md"
   ```
