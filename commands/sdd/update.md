...
<flags>
- **--sync**: Sync managed SDD skills across runtime roots so multi-runtime users stay aligned after an update. Runs the sync-skills workflow (--from, --to, --dry-run, --apply flags supported).
- **--reapply**: Reapply local modifications after a SDD update. Uses three-way comparison (pristine baseline, user-modified backup, newly installed version) to merge user customizations back. Runs the reapply-patches workflow.
...
@~/.claude/sdd/workflows/sync-skills.md
@~/.claude/sdd/workflows/reapply-patches.md
</execution_context_extended>
