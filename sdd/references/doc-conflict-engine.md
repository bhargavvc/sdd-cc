...
Display:
```
SDD > BLOCKED: {N} blockers must be resolved before {operation} can proceed.
```
...
Proceed silently or display `SDD > No conflicts detected.` Either is acceptable; workflows choose based on verbosity context.
