...
> Loaded by `execute-phase` workflow and `sdd-executor` agent only when **both** `MVP_MODE=true` AND `TDD_MODE=true` for the phase. Defines the runtime gate that blocks behavior-adding tasks until a failing-test commit exists.
...
- The user is shown the same review table, but the verdict line reads:
  > "Phase blocked: {N} TDD plan(s) violate the RED→GREEN gate sequence under MVP+TDD. Resolve and re-run /sdd execute-phase, or override with `/sdd execute-phase {phase} --force-mvp-gate` to ship anyway."
