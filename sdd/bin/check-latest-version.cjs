...
/**
 * Deterministic latest-version check for /sdd-update (#2992).
 *
 * The /sdd-update workflow's check_latest_version step was previously
 * prescribed in LLM-driven prose ("run `npm view @bhargavvc/sdd-cc
 * version`"). The executing model could shortcut the prescription and
 * invent npm queries against wrong-shaped names (`@sdd/cli`,
 * `sdd-cli`, `sdd`), all of which 404 or — worse — return an
...
const PACKAGE_NAME = '@bhargavvc/sdd-cc';
...
function checkLatestVersion(opts = {}) {
  ...
  // Bounded at 15s so a hung registry doesn't block /sdd-update (#2993 CR).
  const defaultSpawn = () => {
