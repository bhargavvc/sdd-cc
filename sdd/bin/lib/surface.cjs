 ...
 *
 * Manages the runtime enable/disable surface state (the `.sdd-surface.json` marker in
 * each runtime's skills dir) independently of the install-time profile marker
 * (`.sdd-profile`). Runtime config locations are resolved by callers.
 *
...
const SURFACE_FILE_NAME = '.sdd-surface.json';
...
function resolveSurface(runtimeConfigDir, manifest, clusterMap) {
  ...
  // Determine base profile name: from surface state or from .sdd-profile marker
 ...
 * Only touches files matching `sdd-` prefix or `*.md` in commandsDir.
 * Never touches non-`sdd-*` files.
 *
 * Steps:
 ...
 *  4. Sync: copy missing, delete superseded (sdd-only)
 *
 * @param {string} runtimeConfigDir
 * @param {string} commandsDir  runtime commands/sdd dir (resolved per-runtime by callers)
...
function applySurface(runtimeConfigDir, commandsDir, agentsDir, manifest, clusterMap) {
  ...
  _syncSddDir(stagedSkills, commandsDir, 'commands');
  ...
  if (agentsDir && fs.existsSync(agentsDir)) {
    ...
    if (srcAgentsDir) {
      ...
      _syncSddDir(stagedAgents, agentsDir, 'agents');
    }
  }
}
 ...
 * Removes sdd-prefixed .md files in dest not present in staged.
 * Never touches non-sdd files.
 *
 ...
 */
function _syncSddDir(stagedDir, destDir, context) {
  ...
  // Remove sdd-only files from dest that aren't in staged set
  // For commands dir: all .md files are sdd skills
  // For agents dir: only sdd-* files
  const destEntries = fs.readdirSync(destDir).filter(f => f.endsWith('.md'));
  for (const file of destEntries) {
    if (context === 'agents' && !file.startsWith('sdd-')) continue;
...
/**
 * Find the install source commands/sdd directory.
 * Checks the runtime's `.sdd-source` marker (sibling of the surface state file),
 * then walks up from __dirname to find the installed package source.
 *
 * @param {string} runtimeConfigDir
 * @returns {string} path to install source commands/sdd
 */
function _findInstallSource(runtimeConfigDir) {
  // Check for .sdd-source marker
  const sourceMarker = path.join(runtimeConfigDir, '.sdd-source');
  ...
  // Walk up from this module's dir to find commands/sdd
  let dir = __dirname;
  for (let i = 0; i < 6; i++) {
    const candidate = path.join(dir, 'commands', 'sdd');
  ...
  return path.join(runtimeConfigDir, '..', 'commands', 'sdd');
}
...
function _findAgentsSource(runtimeConfigDir) {
  // Prefer .sdd-source sibling marker (commands/sdd) and derive agents from it.
  const sourceMarker = path.join(runtimeConfigDir, '.sdd-source');
  if (fs.existsSync(sourceMarker)) {
    try {
