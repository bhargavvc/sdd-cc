...
function cmdInitExecutePhase(cwd, phase, raw, options = {}) {
  ...
  const result = {
    // Models
    executor_model: resolveModelInternal(cwd, 'sdd-executor'),
...
function cmdInitPlanPhase(cwd, phase, raw, options = {}) {
  ...
  const result = {
    ...
    planner_model: resolveModelInternal(cwd, 'sdd-planner'),
...
function cmdInitNewProject(cwd, raw) {
  ...
  const firecrawlKeyFile = path.join(homedir, '.sdd', 'firecrawl_api_key');
  const hasFirecrawl = !!(process.env.FIRECRAWL_API_KEY || fs.existsSync(firecrawlKeyFile));

  // Detect Exa API key availability
  const exaKeyFile = path.join(homedir, '.sdd', 'exa_api_key');
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'sdd-project-researcher'),
    synthesizer_model: resolveModelInternal(cwd, 'sdd-research-synthesizer'),
    roadmapper_model: resolveModelInternal(cwd, 'sdd-roadmapper'),
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  const result = {
    // Models
    mapper_model: resolveModelInternal(cwd, 'sdd-codebase-mapper'),
...
function cmdInitNewWorkspace(cwd, raw) {
  ...
  const defaultBase = path.join(homedir, 'sdd-workspaces');
 ...
 * @param {string} agentType - The agent type (e.g., 'sdd-executor', 'sdd-planner')
 ...
 * Used by workflows: SKILLS=$(node "$TOOLS" agent-skills sdd-executor 2>/dev/null)
 */
