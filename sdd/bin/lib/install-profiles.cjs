 ...
 *
 * Background: every installed `sdd-*` skill costs eager system-prompt tokens
 * because runtimes (Claude Code, opencode, etc.) enumerate skill descriptions
 * in `<available_skills>` on every turn. With 66 skills + 33 agents SDD alone
 ...
 *  - loadSkillsManifest: parse requires: frontmatter from commands/sdd/*.md
 ...
 *  - readActiveProfile / writeActiveProfile: .sdd-profile marker persistence
 *  - resolveEffectiveProfile: explicit flag > .sdd-profile marker > full
 *  - mostRestrictiveProfile: resolve multi-runtime disagreement to smallest set
 *
 * Companion module (Phase 2): sdd/bin/lib/surface.cjs owns the
 * runtime /sdd:surface command. It reuses stageSkillsForProfile and
 * stageAgentsForProfile from this module for cluster-level enable/disable
 * without reinstall, persisting state in <runtimeConfigDir>/.sdd-surface.json.
 *
 ...
 * since SDD enforces flow-style arrays for requires:.
 *
 ...
 * Scans the full content for `sdd-<stem>` patterns that correspond to
 * real agent files. Returns all unique `sdd-*` stems found in the body.
 *
 * The caller is responsible for filtering by which agents actually exist —
 * this function returns all syntactically valid `sdd-*` matches.
 *
 * @param {string} content full file content
 * @returns {string[]} deduplicated agent stems like ['sdd-planner', 'sdd-executor']
 */
function parseCallsAgents(content) {
  // Match word-boundary sdd-<stem> patterns; stems are lowercase letters and hyphens.
  // We use a regex that matches `sdd-` followed by one or more lowercase-alpha-or-hyphen chars.
  // This catches `sdd-planner`, `sdd-plan-checker`, etc. in prose and code.
  const matches = content.match(/\bsdd-[a-z][a-z-]*/g);
  if (!matches) return [];
  // Deduplicate
...
/**
 * Load the requires: dependency graph from a commands/sdd directory.
 * Also derives calls_agents for each skill by scanning the body text for
 * `sdd-*` agent name references. Agent stems are stored under the special
 * key `_calls_agents_<stem>` so they don't conflict with skill stems.
 *
 * @param {string} commandsDir absolute path to commands/sdd/
...
/**
 * Stage a filtered copy of commands/sdd for a resolved profile.
 * In full mode (skills === '*') returns srcDir unchanged (no-op).
 *
 * @param {string} srcDir absolute path to commands/sdd
...
function stageSkillsForProfile(srcDir, resolvedProfile) {
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-profile-skills-'));
  try {
 ...
 * For tiered profiles, copies only agents whose full stem (e.g. 'sdd-planner')
...
function stageAgentsForProfile(srcAgentsDir, resolvedProfile) {
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-profile-agents-'));
  try {
    if (resolvedProfile.agents instanceof Set && resolvedProfile.agents.size > 0) {
      ...
      for (const entry of entries) {
        if (!entry.isFile()) continue;
        if (!entry.name.endsWith('.md')) continue;
        // Agent stem is the full filename without extension, e.g. "sdd-planner"
        const stem = entry.name.slice(0, -3);
...
const PROFILE_MARKER_NAME = '.sdd-profile';
...
function resolveEffectiveProfile({ requestedProfileName, targetDir }) {
  ...
  // 2. Marker-driven (sdd update path)
...
/**
 * Stage a filtered copy of the source commands/sdd directory.
...
function stageSkillsForMode(srcDir, mode) {
  ...
  const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-minimal-skills-'));
  try {
    const entries = fs.readdirSync(srcDir, { withFileTypes: true });
    for (const entry of entries) {
      if (!entry.isFile()) continue;
