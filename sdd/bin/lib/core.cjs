...
/**
 * Remove stale sdd-* temp files/dirs older than maxAgeMs (default: 5 minutes).
 * Runs opportunistically before each new temp file write to prevent unbounded accumulation.
 * @param {string} prefix - filename prefix to match (e.g., 'sdd-')
 * @param {object} opts
...
function loadConfig(cwd) {
  ...
  const ws = process.env.SDD_WORKSTREAM || null;
  let rootParsed = null;
  if (ws) {
  ...
  try {
    ...
    if (fs.existsSync(planningDir(cwd))) {
      if (rootParsed) {
        ...
        // Temporarily clear SDD_WORKSTREAM so planningDir() returns root .planning/,
        // then reload. This is safe: rootParsed is already the root config object.
        const savedWs = process.env.SDD_WORKSTREAM;
        delete process.env.SDD_WORKSTREAM;
        try {
          return loadConfig(cwd);
        } finally {
          process.env.SDD_WORKSTREAM = savedWs;
        }
      }
      return defaults;
    }
...
/**
 * Resolve the agents directory from the SDD install location.
