 ...
 * CLI --ws > SDD_WORKSTREAM env > stored active workstream pointer.
 */
...
function resolveActiveWorkstream(cwd, args, env = process.env, deps = {}) {
  ...
  } else if (env && typeof env.SDD_WORKSTREAM === 'string' && env.SDD_WORKSTREAM.trim()) {
    ws = env.SDD_WORKSTREAM.trim();
    source = 'env';
  } else {
...
function applyResolvedWorkstreamEnv(resolution, env = process.env) {
  ...
  env.SDD_WORKSTREAM = resolution.ws;
}
