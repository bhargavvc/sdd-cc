...
const INSTALL_STATE_NAME = 'sdd-install-state.json';
const INSTALL_MIGRATION_LOCK_NAME = 'sdd-install-migration.lock';
...
function applyInstallerMigrationPlan({ configDir, plan, now = () => new Date().toISOString() }) {
  ...
  const journalRelPath = path.posix.join('sdd-migration-journal', `${runId}.json`);
  const journalPath = path.join(configDir, journalRelPath);
  const rollbackRootRelPath = path.posix.join('sdd-migration-journal', `${runId}-rollback`);
  const rollbackRoot = path.join(configDir, rollbackRootRelPath);
  const backupRootRelPath = path.posix.join('sdd-migration-journal', `${runId}-backups`);
