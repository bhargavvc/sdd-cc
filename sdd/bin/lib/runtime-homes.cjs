 ...
 * the correct global skills directory for every SDD-supported runtime.
 *
 ...
 *   hermes  — SDD skills nest under skills/sdd/<skillName>/ (not the flat
...
function getGlobalConfigDir(runtime) {
  ...
  switch (runtime) {
    ...
    // ── Hermes Agent ─────────────────────────────────────────────────────────
    // Note: skills use a nested layout (skills/sdd/<skill>/) — see getGlobalSkillDir().
    case 'hermes':
 ...
 * Hermes: <configDir>/skills/sdd  (nested category layout — #2841)
...
function getGlobalSkillsBase(runtime) {
  ...
  if (runtime === 'hermes') return path.join(configDir, 'skills', 'sdd');
 ...
 *
 * @param {string} runtime
 * @param {string} skillName - e.g. 'sdd-executor'
 * @returns {string|null}
 */
