...
/**
 * Write a WAITING.json signal file when SDD hits a decision point.
 * External watchers (fswatch, polling, orchestrators) can detect this.
 * File is written to .planning/WAITING.json (or .sdd/WAITING.json if .sdd exists).
 * Fixes #1034.
 */
function cmdSignalWaiting(cwd, type, question, options, phase, raw) {
  const sddDir = fs.existsSync(path.join(cwd, '.sdd')) ? path.join(cwd, '.sdd') : planningDir(cwd);
  const waitingPath = path.join(sddDir, 'WAITING.json');
...
function cmdSignalResume(cwd, raw) {
  const paths = [
    path.join(cwd, '.sdd', 'WAITING.json'),
