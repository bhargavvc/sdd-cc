...
function buildSection(sectionName, sourceFile, content) {
  return [
    `<!-- SDD:${sectionName}-start source:${sourceFile} -->`,
    content,
    `<!-- SDD:${sectionName}-end -->`,
  ].join('\n');
}
...
function cmdWriteProfile(cwd, options, raw) {
  ...
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'sdd', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  // commands/sdd subdirectory in favor of skills/<skill>/SKILL.md under
  // the runtime config dir. This writer was missed in the migration
  // (PR #1540 targeted SDD-shipped command files; dev-preferences is a
  // runtime-generated user artifact). Default now points at the skills/
  // location so /sdd:profile-user --refresh stops re-creating the legacy
  ...
  if (!outputPath) {
    ...
    try {
      ...
      effectiveRuntime = process.env.SDD_RUNTIME || config.runtime || 'claude';
    } catch {
      effectiveRuntime = process.env.SDD_RUNTIME || 'claude';
    }
    const skillDir = getGlobalSkillDir(effectiveRuntime, 'sdd-dev-preferences');
    if (!skillDir) {
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  try {
    ...
    // SDD_RUNTIME env var takes precedence over config.runtime, mirroring detectRuntime().
    const effectiveRuntime = process.env.SDD_RUNTIME || config.runtime || null;
  ...
  if (existingContent === null) {
    ...
    if (!options.auto && fileContent.indexOf('<!-- SDD:profile-start') === -1) {
