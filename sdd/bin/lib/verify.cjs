    ...
    if (!agentStatus.agents_installed) {
      if (agentStatus.installed_agents.length === 0) {
        addIssue('warning', 'W010',
          `No SDD agents found in ${agentStatus.agents_dir} — Task(subagent_type="sdd-*") will fall back to general-purpose`,
          'Run the SDD installer: npx @bhargavvc/sdd-cc@latest');
      } else {
        addIssue('warning', 'W010',
          `Missing ${agentStatus.missing_agents.length} SDD agents: ${agentStatus.missing_agents.join(', ')} — affected workflows will fall back to general-purpose`,
          'Run the SDD installer: npx @bhargavvc/sdd-cc@latest');
      }
    }
