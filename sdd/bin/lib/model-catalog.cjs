...
//
//   1. Co-located install path — sdd/bin/shared/model-catalog.json
...
//      bhargavvc/sdd-cc clone (the original path introduced by #3230).
//
//   3. SDD_MODEL_CATALOG env override — allows test harnesses and custom
...
const _catalogCandidates = [
  ...
  process.env.SDD_MODEL_CATALOG ? path.resolve(process.env.SDD_MODEL_CATALOG) : null,
].filter(Boolean);
