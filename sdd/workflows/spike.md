...
Saves artifacts to `.planning/spikes/`. Companion to `/sdd-spike --wrap-up`.
...
```bash
sdd-sdk query commit "docs(spike-NNN): [VERDICT] — [key finding]" --files .planning/spikes/NNN-descriptive-name/ .planning/spikes/MANIFEST.md
```
...
```bash
sdd-sdk query commit "docs(spikes): update conventions" --files .planning/spikes/CONVENTIONS.md
```
</step>
...
**Also available:**
- `/sdd-spike` — spike more ideas (or run with no argument for frontier mode)
- `/sdd-plan-phase` — start planning the real implementation
- `/sdd-explore` — continue exploring the idea
