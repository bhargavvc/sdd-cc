...
OLLAMA_HOST=$(sdd-sdk query config-get review.ollama_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
LM_STUDIO_HOST=$(sdd-sdk query config-get review.lm_studio_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
LLAMA_CPP_HOST=$(sdd-sdk query config-get review.llama_cpp_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
- Value must be a non-empty array of slug strings (configured via `sdd config-set review.default_reviewers '["gemini","codex"]'`)
...
Write to a temp file: `/tmp/sdd-review-prompt-{phase}.md`
</step>
...
OLLAMA_HOST=$(sdd-sdk query config-get review.ollama_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
if [ -z "$OLLAMA_HOST" ] || [ "$OLLAMA_HOST" = "null" ]; then OLLAMA_HOST="http://localhost:11434"; fi
OLLAMA_MODEL=$(sdd-sdk query config-get review.models.ollama 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
jq -n --rawfile content /tmp/sdd-review-prompt-{phase}.md \
  ...
  > /tmp/sdd-review-ollama-{phase}.md
if [ ! -s /tmp/sdd-review-ollama-{phase}.md ]; then
  echo "Ollama review failed or returned empty output." > /tmp/sdd-review-ollama-{phase}.md
fi
```
...
LM_STUDIO_HOST=$(sdd-sdk query config-get review.lm_studio_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
if [ -z "$LM_STUDIO_HOST" ] || [ "$LM_STUDIO_HOST" = "null" ]; then LM_STUDIO_HOST="http://localhost:1234"; fi
LM_STUDIO_MODEL=$(sdd-sdk query config-get review.models.lm_studio 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
LM_STUDIO_RESPONSE=$(jq -n --rawfile content /tmp/sdd-review-prompt-{phase}.md \
...
if [ -n "$LM_STUDIO_CONTENT" ]; then
  echo "$LM_STUDIO_CONTENT" > /tmp/sdd-review-lm_studio-{phase}.md
else
...
LLAMA_CPP_HOST=$(sdd-sdk query config-get review.llama_cpp_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
if [ -z "$LLAMA_CPP_HOST" ] || [ "$LLAMA_CPP_HOST" = "null" ]; then LLAMA_CPP_HOST="http://localhost:8080"; fi
LLAMA_CPP_MODEL=$(sdd-sdk query config-get review.models.llama_cpp 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
LLAMA_CPP_CONTENT=$(jq -n --rawfile content /tmp/sdd-review-prompt-{phase}.md \
...
if [ -n "$LLAMA_CPP_CONTENT" ]; then
  echo "$LLAMA_CPP_CONTENT" > /tmp/sdd-review-llama_cpp-{phase}.md
else
...
Commit:
```bash
sdd-sdk query commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
