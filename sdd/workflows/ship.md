...
```bash
CONFIG=$(sdd-sdk query state.load)
```
...
```bash
CUSTOM_PR_SECTIONS=$(sdd-sdk query config-get ship.pr_body_sections --default '[]' 2>/dev/null || echo '[]')
```
...
Use these sections for lean/agile PRD material that should travel with the PR without making the core `/sdd:ship` body configurable:
...
```bash
PR_BODY_FILE=$(mktemp "${TMPDIR:-/tmp}/sdd-pr-body.XXXXXX.md")
...
```bash
REVIEW_CMD=$(sdd-sdk query config-get workflow.code_review_command 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
```
...
2. **Load phase context from STATE.md:**
   ```bash
   STATE_STATUS=$(sdd-sdk query state.load 2>/dev/null | head -20)
   ```
...
3. **Build review prompt and pipe to command via stdin:**
   ...
   REVIEW_OUTPUT=$(echo "${REVIEW_PROMPT}" | timeout 120 ${REVIEW_CMD} 2>/tmp/sdd-review-stderr.log)
   REVIEW_EXIT=$?
   ```
...
4. **Handle timeout (120s) and failure:**
   ...
   if [ $REVIEW_EXIT -ne 0 ]; then
     REVIEW_STDERR=$(cat /tmp/sdd-review-stderr.log 2>/dev/null)
...
```bash
sdd-sdk query state.update "Last Activity" "$(date +%Y-%m-%d)"
sdd-sdk query state.update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```
