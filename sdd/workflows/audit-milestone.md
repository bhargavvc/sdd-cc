...
```bash
integration_checker_model=$(sdd-sdk query resolve-model sdd-integration-checker --raw)
```
...
PHASE_INFO=$(sdd-sdk query find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  sdd-sdk query summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
```bash
NYQUIST_CONFIG=$(sdd-sdk query config-get workflow.nyquist_validation --raw 2>/dev/null)
```
