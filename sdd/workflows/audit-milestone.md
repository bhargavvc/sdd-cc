...
```bash
INIT=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" init milestone-op)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_CHECKER=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" agent-skills sdd-integration-checker 2>/dev/null)
```
...
```bash
integration_checker_model=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" resolve-model sdd-integration-checker --raw)
```
...
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" phases list
```
...
PHASE_INFO=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "$HOME/.claude/sdd/bin/sdd-tools.cjs" summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
```bash
NYQUIST_CONFIG=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" config-get workflow.nyquist_validation --raw 2>/dev/null)
```
