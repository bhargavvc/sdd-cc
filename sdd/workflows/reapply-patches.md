...
**Why this matters:** `git log --diff-filter=A` returns the commit that *first added* the file, which is the wrong baseline on repos that have been through multiple SDD update cycles. The `pristine_hashes` field in `backup-meta.json` records the SHA-256 of the file as it existed in the pre-update SDD release — matching against it finds the correct baseline regardless of how many updates have occurred.
...
Run the verifier as a child process (the sdd-tools binary directory is not required — the script ships under `sdd/bin/` in the source repo and is installed to `${SDD_HOME}/sdd/bin/`; it is also exposed via the SDK at `sdk/dist/cli.js verify-reapply` when present):
...
VERIFY_OUTPUT="$(node "${SDD_HOME}/sdd/bin/verify-reapply-patches.cjs" "${VERIFY_ARGS[@]}")"
VERIFY_STATUS=$?
```
