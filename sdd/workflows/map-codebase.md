...
```bash
INIT=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" init map-codebase)
...
```bash
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" commit "docs: map existing codebase" --files .planning/codebase/*.md
```
