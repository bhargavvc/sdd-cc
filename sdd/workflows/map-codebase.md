...
post-execute codebase-drift gate in `/sdd:execute-phase` or by a user running
`/sdd:map-codebase --paths apps/accounting,packages/ui`), the workflow
operates in **incremental-remap mode**:

- Pass `--paths <p1>,<p2>,...` through to each spawned `sdd-codebase-mapper`
...
```bash
INIT=$(sdd-sdk query init.map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(sdd-sdk query agent-skills sdd-codebase-mapper)
```
...
Use Agent tool with `subagent_type="sdd-codebase-mapper"`, `model="{mapper_model}"`, and `run_in_background=true` for parallel execution.
...
```bash
sdd-sdk query commit "docs: map existing codebase" --files .planning/codebase/*.md
```
...
- If Agent tool available: 4 parallel sdd-codebase-mapper agents spawned with run_in_background=true
