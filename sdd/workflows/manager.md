...
```bash
INIT=$(sdd-sdk query init.manager)
...
These are empty strings by default. Set via: `sdd-sdk query config-set manager.flags.discuss "--auto --analyze"`
...
**Auto-refresh:** If background agents are running (`is_active` is true for any phase), set a 60-second auto-refresh cycle. After presenting the action menu, if no user input is received within 60 seconds, automatically refresh the dashboard. This interval is configurable via `manager_refresh_interval` in SDD config (default: 60 seconds, set to 0 to disable).
...
Task(
  ...
  prompt="You are running the SDD plan-phase workflow for phase {N} of the project.
...
Task(
  ...
  run_in_background=true,
  prompt="You are running the SDD execute-phase workflow for phase {N} of the project.
