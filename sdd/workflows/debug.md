...
Invoked by `/sdd-debug` (`commands/sdd/debug.md`).

Systematic debugging using the scientific method with subagent isolation.
Orchestrates symptom gathering, session creation, and delegation to `sdd-debug-session-manager`.

<available_agent_types>
Valid SDD subagent types (use exact names — do not fall back to 'general-purpose'):
- sdd-debug-session-manager — manages debug checkpoint/continuation loop in isolated context
- sdd-debugger — investigates bugs using scientific method
</available_agent_types>
...
```bash
debugger_model=$(sdd-sdk query resolve-model sdd-debugger 2>/dev/null | jq -r '.model' 2>/dev/null || true)
```

Read TDD mode from config:
```bash
TDD_MODE=$(sdd-sdk query config-get workflow.tdd_mode 2>/dev/null | jq -r 'if type == "boolean" then tostring else . end' 2>/dev/null || echo "false")
```
...
─────────────────────────────────────────────
Run `/sdd-debug continue <slug>` to resume a session.
No sessions? `/sdd-debug <description>` to start.
```

If no files exist or the glob returns nothing: print "No active debug sessions. Run `/sdd-debug <issue description>` to start one."
...
**Sanitize SLUG first:** strip whitespace, reject unless it matches `^[a-z0-9][a-z0-9-]*$`, enforce max 30 chars, reject any `..`, `/`, or `\`. If invalid, print "No active debug session found with slug: {SLUG}. Check `/sdd-debug list` for active sessions." and stop.

Check `.planning/debug/{SLUG}.md` exists. If not, print "No active debug session found with slug: {SLUG}. Check `/sdd-debug list` for active sessions." and stop.
...
""",
  subagent_type="sdd-debug-session-manager",
...
## 4. Session Management (delegated to sdd-debug-session-manager)

After initial context setup, spawn the session manager to handle the full checkpoint/continuation loop. The session manager handles specialist_hint dispatch internally: when sdd-debugger returns ROOT CAUSE FOUND it extracts the specialist_hint field and invokes the matching skill (e.g. typescript-expert, swift-concurrency) before offering fix options.
...
If summary shows `ABANDONED`: note session saved at `.planning/debug/{slug}.md` for later `/sdd-debug continue {slug}`.
...
- [ ] sdd-debug-session-manager spawned with security-hardened session_params
