...
SDD_CONFIG_PATH=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" config-path)
...
Store `$SDD_CONFIG_PATH` — all subsequent reads and writes use this path, not the hardcoded `.planning/config.json`, so active-workstream installs write to the correct location (#2282).
</step>
