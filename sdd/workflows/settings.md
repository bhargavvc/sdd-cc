<purpose>
Interactive configuration of SDD workflow agents (research, plan_check, verifier) and model profile selection via multi-question prompt. Updates .planning/config.json with user preferences. Optionally saves settings as global defaults (~/.sdd/defaults.json) for future projects.
</purpose>
...
```bash
sdd-sdk query config-ensure-section
INIT=$(sdd-sdk query state.load)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
# `state.load` returns STATE frontmatter JSON from the SDK — it does not include `config_path`. Orchestrators may set `SDD_CONFIG_PATH` from init phase-op JSON; otherwise resolve the same path sdd-tools uses for flat vs active workstream (#2282).
if [[ -z "${SDD_CONFIG_PATH:-}" ]]; then
  if [[ -f .planning/active-workstream ]]; then
    WS=$(tr -d '\n\r' < .planning/active-workstream)
    SDD_CONFIG_PATH=".planning/workstreams/${WS}/config.json"
  else
    SDD_CONFIG_PATH=".planning/config.json"
  fi
fi
```
...
Store `$SDD_CONFIG_PATH` — all subsequent reads and writes use this path, not a hardcoded `.planning/config.json`, so active-workstream installs target the correct file (#2282).
</step>
...
- `workflow.pattern_mapper` — run sdd-pattern-mapper between research and planning (default: true if absent)
...
- `workflow.code_review` — enable /sdd:code-review and /sdd:code-review --fix commands (default: true if absent)
- `workflow.code_review_depth` — default depth for /sdd:code-review: `quick`, `standard`, or `deep` (default: `"standard"` if absent; only relevant when `code_review` is on)
- `workflow.ui_review` — run visual quality audit (/sdd:ui-review) in autonomous mode (default: true if absent)
- `commit_docs` — whether `.planning/` files are committed to git (default: true if absent)
- `intel.enabled` — enable queryable codebase intelligence (/sdd:map-codebase --query) (default: false if absent)
- `graphify.enabled` — enable project knowledge graph (/sdd:graphify) (default: false if absent)
...
AskUserQuestion([
  ...
  {
    question: "Enable Code Review? (/sdd:code-review and /sdd:code-review --fix commands)",
    header: "Code Review",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "Enable /sdd:code-review commands for reviewing source files changed during a phase." },
  ...
  {
    question: "Code Review Depth? (default depth for /sdd:code-review — override per-run with --depth=)",
    header: "Review Depth",
    multiSelect: false,
    options: [
  ...
  {
    question: "Enable UI Review? (visual quality audit via /sdd:ui-review in autonomous mode)",
    header: "UI Review",
    multiSelect: false,
    options: [
  ...
  },
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "sdd-pattern-mapper runs between research and plan steps. Surfaces conventions so new code follows house style." },
  ...
  },
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "Run /sdd:ai-integration-phase before planning AI system phases. Surfaces the right framework, researches its docs, and designs the evaluation strategy." },
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Per Phase", description: "Create branch for each phase (sdd/phase-{N}-{name})" },
      { label: "Per Milestone", description: "Create branch for entire milestone (sdd/{version}-{name})" }
    ]
  },
  {
  ...
  {
    question: "Enable Intel? (queryable codebase intelligence via /sdd:map-codebase --query — builds a JSON index in .planning/intel/)",
    header: "Intel",
    multiSelect: false,
    options: [
      { label: "No (Recommended)", description: "Skip intel indexing. Use when codebase is small or intel queries are not needed." },
      { label: "Yes", description: "Enable /sdd:map-codebase --query commands. Builds and queries a JSON index of the codebase." }
    ]
  },
  {
    question: "Enable Graphify? (project knowledge graph via /sdd:graphify — builds a graph in .planning/graphs/)",
    header: "Graphify",
    multiSelect: false,
    options: [
      { label: "No (Recommended)", description: "Skip knowledge graph. Use when dependency graphs are not needed." },
      { label: "Yes", description: "Enable /sdd:graphify commands. Builds and queries a project knowledge graph." }
    ]
  }
])
```
...
**Safe merge:** Apply each chosen value via `sdd-sdk query config-set <key.path> <value>` so unrelated keys are never clobbered. `code_review_depth` is written only if the code_review question was answered `on`; otherwise leave the existing value in place.

Write updated config to `$SDD_CONFIG_PATH` (the workstream-aware path resolved in `ensure_and_load_config`). Never hardcode `.planning/config.json` — workstream installs route to `.planning/workstreams/<slug>/config.json`.
</step>
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes", description: "New projects start with these settings (saved to ~/.sdd/defaults.json)" },
...
If "Yes": write the same config object (minus project-specific fields like `brave_search`) to `~/.sdd/defaults.json`:

```bash
mkdir -p ~/.sdd
```

Write `~/.sdd/defaults.json` with:
```json
{
  "mode": <current>,
  "granularity": <current>,
...
Quick commands:
- /sdd:config --integrations — configure API keys (Brave/Firecrawl/Exa), review.models CLI routing, and agent_skills injection
- /sdd:config --profile <profile> — switch model profile
...
- /sdd:config --advanced — power-user tuning (plan bounce, timeouts, branch templates, cross-AI, context window)
```
</step>
