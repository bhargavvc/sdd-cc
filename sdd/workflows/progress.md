...
```bash
INIT=$(sdd-sdk query init.progress)
...
<step name="load">
**Use structured extraction from `sdd-sdk query` (or legacy sdd-tools.cjs):**

Instead of reading full files, use targeted tools to get only the data needed for the report:
- `ROADMAP=$(sdd-sdk query roadmap.analyze)`
- `STATE=$(sdd-sdk query state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  sdd-sdk query summary-extract <path> --fields one_liner
  ```
...
**Generate progress bar from `sdd-sdk query progress` / `progress.json`, then present rich status report:**

```bash
# Get formatted progress bar
PROGRESS_BAR=$(sdd-sdk query progress.bar --raw)
```
...
```bash
DEBT=$(sdd-sdk query audit-uat --raw 2>/dev/null)
```
...
```bash
PHASE_SECTION=$(sdd-sdk query roadmap.get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
```bash
NEXT_PHASE_SECTION=$(sdd-sdk query roadmap.get-phase "$((Z+1))" 2>/dev/null)
