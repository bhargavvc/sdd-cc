...
**Delegate ROADMAP.md and STATE.md updates to `sdd-sdk query phase.complete`:**

```bash
TRANSITION=$(sdd-sdk query phase.complete "${current_phase}")
```
...
```bash
PROGRESS=$(sdd-sdk query progress.bar --raw)
```
...
**Use the transition result from `sdd-sdk query phase.complete`:**
...
if [ -n "$SDD_WORKSTREAM" ]; then
  WS_LIST=$(sdd-sdk query workstream.list --raw)
fi
```
...
```bash
sdd-sdk query config-set workflow._auto_chain_active false
```
