...
```bash
TRANSITION=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" progress bar --raw)
```
...
if [ -n "$SDD_WORKSTREAM" ]; then
  WS_LIST=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" workstream list --raw)
fi
```
...
```bash
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" config-set workflow._auto_chain_active false
```
