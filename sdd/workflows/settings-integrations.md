<purpose>
Interactive configuration of third-party integrations for SDD — search API keys
(Brave / Firecrawl / Exa), code-review CLI routing (`review.models.<cli>`), and
agent-skill injection (`agent_skills.<agent-type>`). Writes to
`.planning/config.json` via `sdd-sdk`/`sdd-tools` so unrelated keys are
preserved, never clobbered.

This command is deliberately separate from `/sdd-settings` (workflow toggles)
and any `/sdd-settings-advanced` tuning surface. It exists because API keys and
...
Store `$SDD_CONFIG_PATH`. Every subsequent read/write uses it.
</step>
...
```bash
BRAVE=$(sdd-sdk query config-get brave_search --default null)
FIRECRAWL=$(sdd-sdk query config-get firecrawl --default null)
EXA=$(sdd-sdk query config-get exa_search --default null)
SEARCH_GITIGNORED=$(sdd-sdk query config-get search_gitignored --default false)
```
...
```bash
sdd-sdk query config-set brave_search "<value>"     # masked in output
sdd-sdk query config-set firecrawl "<value>"        # masked in output
sdd-sdk query config-set exa_search "<value>"       # masked in output
sdd-sdk query config-set search_gitignored true|false
```

For "Clear", write `null`:

```bash
sdd-sdk query config-set brave_search null
```
</step>
...
```bash
sdd-sdk query config-set review.models.<cli> "<command string>"
```
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "sdd-executor", description: "Skills injected when spawning executor agents" },
      { label: "sdd-planner", description: "Skills injected when spawning planner agents" },
      { label: "sdd-verifier", description: "Skills injected when spawning verifier agents" },
...
```bash
sdd-sdk query config-set agent_skills.<slug> "<skill-a,skill-b,skill-c>"
```
...
Quick commands:
- /sdd-settings — workflow toggles and model profile
...
<success_criteria>
- [ ] Current config read from `$SDD_CONFIG_PATH`
