<purpose>
Detect current project state and automatically advance to the next logical SDD workflow step.
...
```bash
# Get state snapshot
sdd-sdk query state.json 2>/dev/null || echo "{}"
```
...
After passing all three hard-stop gates, scan all phases that precede the current phase in ROADMAP.md order for incomplete work. For each prior phase number `N`, use `sdd-sdk query find-phase <N>` JSON (plans, summaries, incomplete_plans, etc.) to inspect that phase.
...
```bash
sdd-sdk query commit "docs: defer incomplete Phase {src} items to backlog"
```
...
⚠ Pending exploratory work:
  ...
  Resume with `/sdd-spike` or `/sdd-sketch`, or continue with phase work below.
```
