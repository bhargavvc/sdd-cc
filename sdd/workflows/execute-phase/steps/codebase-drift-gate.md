...
```bash
DRIFT=$(sdd-sdk query verify.codebase-drift 2>/dev/null || echo '{"skipped":true,"reason":"sdk-failed"}')
```
...
Run /sdd-map-codebase --paths {affected_paths} to refresh planning context.
```
...
from step `init_context` is for `sdd-executor`, not the mapper):

```bash
AGENT_SKILLS_MAPPER=$(sdd-sdk query agent-skills sdd-codebase-mapper)
```

Then spawn `sdd-codebase-mapper` agents with the `--paths` hint:
