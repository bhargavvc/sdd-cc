...
```bash
INIT=$(sdd-sdk query init.phase-op "${after_phase}")
...
**Delegate the phase insertion to `sdd-sdk query phase.insert`:**

```bash
RESULT=$(sdd-sdk query phase.insert "${after_phase}" "${description}")
```
...
1. Update STATE.md's next-phase pointer(s) to the newly inserted phase
   ...
   ```bash
   sdd-sdk query state.patch '{"Current Phase":"{decimal_phase}","Next recommended run":"/sdd-plan-phase {decimal_phase}"}'
   ```
...
2. Append a Roadmap Evolution entry via the dedicated handler. It creates the
   ...
   ```bash
   sdd-sdk query state.add-roadmap-evolution \
     --phase {decimal_phase} \
     --action inserted \
     --after {after_phase} \
     --note "{description}" \
...
- [ ] `sdd-sdk query phase.insert` executed successfully
- [ ] Phase directory created
- [ ] Roadmap updated with new phase entry (includes "(INSERTED)" marker)
- [ ] `sdd-sdk query state.add-roadmap-evolution ...` returned `{ added: true }` or `{ added: false, reason: "duplicate" }`
- [ ] `sdd-sdk query state.patch` returned matched next-phase pointer field(s)
