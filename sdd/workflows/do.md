...
| Exploring, researching, comparing, or "how does X work" | `/sdd:explore` | Socratic ideation and idea routing |
| Discussing vision, "how should X look", brainstorming | `/sdd:discuss-phase` | Needs context gathering |
| A complex task: refactoring, migration, multi-file architecture, system redesign | `/sdd:phase` | Needs a full phase with plan/build cycle |
...
| A note, idea, or "remember to..." | `/sdd:capture` | Capture for later |
...
**Requires `.planning/` directory:** All routes except `/sdd:new-project`, `/sdd:map-codebase`, `/sdd:spike`, `/sdd:sketch`, and `/sdd:help`. If the project doesn't exist and the route requires it, suggest `/sdd:new-project` first.

**Ambiguity handling:** If the text could reasonably match multiple routes, ask the user via AskUserQuestion with the top 2-3 options. For example:

```
"Refactor the authentication system" could be:
1. /sdd:phase — Full planning cycle (recommended for multi-file refactors)
