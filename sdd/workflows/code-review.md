...
```
Error: Phase ${PHASE_ARG} not found. Run /sdd:progress to see available phases.
```
...
2. Config value: `sdd-sdk query config-get workflow.code_review_depth 2>/dev/null`
3. Default: "standard"
...
if [ -n "$DEPTH_OVERRIDE" ]; then
  ...
  CONFIG_DEPTH=$(sdd-sdk query config-get workflow.code_review_depth 2>/dev/null || echo "")
...
```bash
FALLOW_ENABLED=$(sdd-sdk query config-get code_quality.fallow.enabled 2>/dev/null || echo "false")
FALLOW_SCOPE=$(sdd-sdk query config-get code_quality.fallow.scope 2>/dev/null || echo "phase")
FALLOW_PROFILE=$(sdd-sdk query config-get code_quality.fallow.profile 2>/dev/null || echo "standard")
FALLOW_MCP=$(sdd-sdk query config-get code_quality.fallow.mcp 2>/dev/null || echo "false")
```
...
```
Agent(subagent_type="sdd-code-reviewer", prompt="
<files_to_read>
${FILES_TO_READ}
</files_to_read>
...
Next steps:
  /sdd:code-review ${PHASE_NUMBER} --fix  — Auto-fix issues
