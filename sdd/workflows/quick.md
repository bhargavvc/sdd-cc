...
if ! command -v sdd-sdk &>/dev/null; then
  ...
  echo ""
  echo "Install the SDD SDK:"
  echo "  npm install -g @bhargavvc/sdk"
  echo ""
...
AGENT_SKILLS_EXECUTOR=$(sdd-sdk query agent-skills sdd-executor 2>/dev/null)
AGENT_SKILLS_CHECKER=$(sdd-sdk query agent-skills sdd-checker 2>/dev/null)
AGENT_SKILLS_VERIFIER=$(sdd-sdk query agent-skills sdd-verifier 2>/dev/null)
```
...
sdd-sdk query commit "docs(quick-${quick_id}): ${DESCRIPTION}" ${file_list}
```
