...
AGENT_SKILLS_UI=$(sdd-sdk query agent-skills sdd-ui-researcher 2>/dev/null)
AGENT_SKILLS_UI_CHECKER=$(sdd-sdk query agent-skills sdd-ui-checker 2>/dev/null)
```
...
```bash
UI_RESEARCHER_MODEL=$(sdd-sdk query resolve-model sdd-ui-researcher --raw)
UI_CHECKER_MODEL=$(sdd-sdk query resolve-model sdd-ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$(sdd-sdk query config-get workflow.ui_phase 2>/dev/null || echo "true")
```
...
⚡ Sketch findings detected: {SKETCH_FINDINGS_PATH}
   Validated design decisions from /sdd-sketch will be loaded into the UI researcher.
...
```markdown
Read ~/.claude/agents/sdd-ui-researcher.md for instructions.
...
- {SKETCH_FINDINGS_PATH} (Sketch Findings — validated design decisions, CSS patterns, visual direction from /sdd-sketch, if exists)
</files_to_read>
...
Template: ~/.claude/sdd/templates/UI-SPEC.md
</output>
...
```markdown
Read ~/.claude/agents/sdd-ui-checker.md for instructions.
...
```bash
sdd-sdk query commit "docs(${padded_phase}): UI design contract" "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
