...
```bash
SDD_WS=""
echo "$ARGUMENTS" | grep -qE -- '--ws[[:space:]]+[^[:space:]]+' && SDD_WS=$(echo "$ARGUMENTS" | grep -oE -- '--ws[[:space:]]+[^[:space:]]+')
PHASE_ARG=$(echo "$ARGUMENTS" | sed -E 's/--ws[[:space:]]+[^[:space:]]+//g' | xargs)

INIT=$(sdd-sdk query init.verify-work "${PHASE_ARG}" ${SDD_WS})
...
MVP_MODE=$(sdd-sdk query phase.mvp-mode "${phase_number}" ${SDD_WS} --pick active)
```
</step>
...
**MVP-mode UAT framing.** When `MVP_MODE=true`, follow the rules in `@~/.claude/sdd/references/verify-mvp-mode.md`. Briefly:
...
```bash
PHASE_GOAL=$(sdd-sdk query roadmap.get-phase "${phase_number}" ${SDD_WS} --pick goal)
...
if [ "$USER_STORY_VALID" != "true" ]; then
  ...
  echo "Run /sdd mvp-phase ${phase_number} to set a user-story goal before verifying."
  exit 1
fi
```
...
Commit the UAT file:
```bash
sdd-sdk query commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
- Follow @~/.claude/sdd/workflows/diagnose-issues.md
