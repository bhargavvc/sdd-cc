...
If untracked files exist, ask the user (via AskUserQuestion) which they want included:
...
If the user invoked the command with `--all`, skip the question but **still echo the file list back
and ask one final yes/no confirmation** before proceeding — `--all` is not a license to commit
secrets without showing the user.
...
Show the user the proposed message + the staged file list. Ask:
