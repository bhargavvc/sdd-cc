...
```bash
GRADUATION_ENABLED=$(sdd-sdk query config-get features.graduation 2>/dev/null || echo "true")
GRADUATION_WINDOW=$(sdd-sdk query config-get features.graduation_window 2>/dev/null || echo "5")
GRADUATION_THRESHOLD=$(sdd-sdk query config-get features.graduation_threshold 2>/dev/null || echo "3")
```
