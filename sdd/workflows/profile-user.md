...
```bash
sdd-sdk query write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
sdd-sdk query generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
sdd-sdk query generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
sdd-sdk query generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```
