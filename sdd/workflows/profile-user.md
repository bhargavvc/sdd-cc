...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing `sdd-sdk query` handlers (with legacy `sdd-tools.cjs` parity where needed) and the sdd-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
Key references:
- @$HOME/.claude/sdd/references/ui-brand.md (display patterns)
- @$HOME/.claude/agents/sdd-user-profiler.md (profiler agent definition)
- @$HOME/.claude/sdd/references/user-profiling.md (profiling reference doc)
</required_reading>
...
✓ Stores profile at $HOME/.claude/sdd/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$(sdd-sdk query scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(sdd-sdk query profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `$HOME/.claude/sdd/references/user-profiling.md`
...
Reference: @$HOME/.claude/sdd/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(sdd-sdk query profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(sdd-sdk query profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
sdd-sdk query write-profile --input "$ANALYSIS_PATH" --json
```

Display: "✓ Profile written to $HOME/.claude/sdd/USER-PROFILE.md"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/sdd/USER-PROFILE.md" and jump to step 10.
...
```bash
sdd-sdk query generate-dev-preferences --analysis "$ANALYSIS_PATH" --json
```

Display: "✓ Generated /sdd-dev-preferences at $HOME/.claude/skills/sdd-dev-preferences/SKILL.md"

**For CLAUDE.md profile section (if selected):**

```bash
sdd-sdk query generate-claude-profile --analysis "$ANALYSIS_PATH" --json
```
...
```bash
sdd-sdk query generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json
```

Display: "✓ Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any `sdd-sdk query` or sdd-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
Your profile:    $HOME/.claude/sdd/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /sdd-dev-preferences   $HOME/.claude/skills/sdd-dev-preferences/SKILL.md
...
- [ ] Artifacts generated sequentially via sdd-sdk query (or sdd-tools.cjs) subcommands
