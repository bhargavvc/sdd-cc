...
- @$HOME/.claude/sdd/agents/sdd-user-profiler.md (profiler agent definition)
...
Run session scan:
```bash
SCAN_RESULT=$(node $HOME/.claude/sdd/bin/sdd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node $HOME/.claude/sdd/bin/sdd-tools.cjs profile-sample --json 2>/dev/null)
```
...
**Get questions:**
```bash
QUESTIONS=$(node $HOME/.claude/sdd/bin/sdd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node $HOME/.claude/sdd/bin/sdd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node $HOME/.claude/sdd/bin/sdd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $HOME/.claude/sdd/bin/sdd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $HOME/.claude/sdd/bin/sdd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any sdd-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
