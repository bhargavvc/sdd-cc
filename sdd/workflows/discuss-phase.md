...
- Per-project: `sdd-sdk query config-set workflow.text_mode true`
...
AGENT_SKILLS_ADVISOR=$(sdd-sdk query agent-skills sdd-advisor 2>/dev/null)
```
...
Check if a SPEC.md (from `/sdd-spec-phase`) exists for this phase. SPEC.md locks requirements before implementation decisions — if present, this discussion focuses on HOW to implement, not WHAT to build.
...
**Note:** SPEC.md files named `AI-SPEC.md` (from `/sdd-ai-integration-phase`) are excluded — those serve a different purpose.
</step>
...
```
⚠ Unpackaged spikes/sketches detected — run `/sdd-spike-wrap-up` or `/sdd-sketch-wrap-up` to make findings available to planning agents.
```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(sdd-sdk query resolve-model sdd-advisor-researcher --raw)
   ```
...
```bash
PROFILE_CONTENT=$(cat "$HOME/.claude/sdd/USER-PROFILE.md" 2>/dev/null || true)
```
...
```bash
MAX_PASSES=$(sdd-sdk query config-get workflow.max_discuss_passes 2>/dev/null || echo "3")
```
...
```bash
sdd-sdk query commit "docs(${padded_phase}): capture phase context" "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
sdd-sdk query commit "docs(state): record phase ${PHASE} context session" .planning/STATE.md
```
</step>
