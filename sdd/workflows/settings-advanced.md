<purpose>
Interactive configuration of SDD power-user knobs — plan bounce, node repair, subagent timeouts,
...
All subsequent reads and writes go through `$SDD_CONFIG_PATH`. Never hardcode
...
- `git.phase_branch_template` (default: `sdd/phase-{phase}-{slug}`)
- `git.milestone_branch_template` (default: `sdd/{milestone}-{slug}`)
...
AskUserQuestion([
  ...
  {
    question: "Phase branch template? (current: <value or sdd/phase-{phase}-{slug}>)",
  ...
  {
    question: "Milestone branch template? (current: <value or sdd/{milestone}-{slug}>)",
...
This section lets the user inspect and override the built-in model IDs SDD resolves for each
...
`sdd-sdk query config-set runtime <value>` before proceeding to Step C.
...
```bash
sdd-sdk query config-set model_profile_overrides.<runtime>.<tier> "<model-id>"
```

For each tier where the user chose "Clear override", remove the key by setting it to null:
```bash
sdd-sdk query config-set model_profile_overrides.<runtime>.<tier> null
```
...
Merge the new settings into the existing config at `$SDD_CONFIG_PATH`. This merge is the
core correctness invariant: **preserve every unrelated key** — do not clobber siblings.

Apply each selected value via `sdd-sdk query config-set <key> <value>` so the central
...
sdd-sdk query config-set workflow.plan_bounce_passes 5
sdd-sdk query config-set workflow.subagent_timeout 900
sdd-sdk query config-set git.base_branch main
sdd-sdk query config-set context_window 1000000
# Runtime model tier examples:
sdd-sdk query config-set runtime gemini
sdd-sdk query config-set model_profile_overrides.gemini.opus gemini-3-ultra
sdd-sdk query config-set model_profile_overrides.gemini.haiku null
```
...
route each write through `sdd-sdk query config-set` so sibling preservation is handled by
the central setter.
</step>
...
<success_criteria>
- [ ] Current config read from resolved `$SDD_CONFIG_PATH`
...
- [ ] Writes routed through `sdd-sdk query config-set` so unrelated keys are preserved
