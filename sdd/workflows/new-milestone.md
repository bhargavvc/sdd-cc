...
```bash
sdd-sdk query state.milestone-switch --milestone "v[X.Y]" --name "[Name]"
```
...
```bash
sdd-sdk query phases.clear --confirm
```

```bash
sdd-sdk query commit "docs: start milestone v[X.Y] [Name]" --files .planning/PROJECT.md .planning/STATE.md
```

## 7. Load Context and Resolve Models

```bash
INIT=$(sdd-sdk query init.new-milestone)
...
Use template: ~/.claude/sdd/templates/research-project/{FILE}
</output>
...
**Commit requirements:**
```bash
sdd-sdk query commit "docs: define milestone v[X.Y] requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
sdd-sdk query commit "docs: create milestone v[X.Y] roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md
```
...
```bash
sdd-sdk query commit "docs: tag [count] pending todos with resolves_phase after milestone v[X.Y] roadmap" --files .planning/todos/pending/*.md
```
