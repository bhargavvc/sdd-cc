...
status: dormant
planted: {ISO date}
planted_during: {current milestone/phase from STATE.md, or "unknown" if not in a SDD project}
...
_To be filled in. Run `/sdd:capture --seed --enrich SEED-{PADDED}` to add context._

## When to Surface

**Trigger:** when relevant

This seed will surface during `/sdd:new-milestone` when the milestone scope matches.

## Scope Estimate

**Unknown** — run `/sdd:capture --seed --enrich SEED-{PADDED}` to estimate effort.
...
```bash
sdd-sdk query commit "docs: plant seed — {$IDEA}" --files .planning/seeds/SEED-{PADDED}-{slug}.md
```
</step>
...
Trigger and scope are set to defaults. Run `/sdd:capture --seed --enrich SEED-{PADDED}`
...
Commit the update:
```bash
sdd-sdk query commit "docs: enrich seed ${SEED_ID} — trigger + why + scope" --files "$SEED_FILE"
```
