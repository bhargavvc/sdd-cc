...
```bash
INIT=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" init execute-phase "${PHASE}")
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="sdd-executor", model=executor_model, isolation="worktree") with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
```bash
NODE_REPAIR=$(node "./.claude/sdd/bin/sdd-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```
...
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" requirements mark-complete ${REQ_IDS}
```
