...
```bash
INIT=$(sdd-sdk query init.execute-phase "${PHASE}")
...
# config settings can be fetched via sdd-sdk query config-get if needed
```
...
INLINE_THRESHOLD=$(sdd-sdk query config-get workflow.inline_plan_threshold 2>/dev/null || echo "2")
...
```bash
sdd-sdk query phases.list --type summaries --raw
...
See `~/.claude/sdd/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/sdd/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(sdd-sdk query config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/sdd/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/sdd/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/sdd/templates/summary.md`.
...
Update STATE.md using sdd-sdk query (or legacy sdd-tools) state mutations:
...
if [ "$IS_WORKTREE" != "true" ]; then
  ...
  sdd-sdk query state.advance-plan

  # Recalculate progress bar from disk state
  sdd-sdk query state.update-progress

  # Record execution metrics
  sdd-sdk query state.record-metric \
...
sdd-sdk query state.add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
sdd-sdk query state.add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using sdd-sdk query (or legacy sdd-tools):
...
if [ "$IS_WORKTREE" != "true" ]; then
  ...
  sdd-sdk query roadmap.update-plan-progress "${PHASE}"
fi
```
...
```bash
sdd-sdk query requirements.mark-complete ${REQ_IDS}
```
...
if [ "$IS_WORKTREE" = "true" ]; then
  sdd-sdk query commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/REQUIREMENTS.md
else
  sdd-sdk query commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
fi
```
</step>
...
```bash
sdd-sdk query commit "" --files .planning/codebase/*.md --amend
```
</step>
