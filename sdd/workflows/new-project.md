...
```bash
INIT=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" init new-project)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" agent-skills sdd-project-researcher 2>/dev/null)
AGENT_SKILLS_SYNTHESIZER=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" agent-skills sdd-synthesizer 2>/dev/null)
AGENT_SKILLS_ROADMAPPER=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" agent-skills sdd-roadmapper 2>/dev/null)
```
...
```bash
mkdir -p .planning
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" commit "chore: add project config" --files .planning/config.json
```
...
```bash
mkdir -p .planning
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" commit "docs: initialize project" --files .planning/PROJECT.md
```
...
```bash
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" commit "docs: define v1 requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
PHASE1_SECTION=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" roadmap get-phase 1 2>/dev/null)
