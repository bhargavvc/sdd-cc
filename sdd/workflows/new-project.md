...
AGENT_SKILLS_RESEARCHER=$(sdd-sdk query agent-skills sdd-project-researcher 2>/dev/null)
AGENT_SKILLS_SYNTHESIZER=$(sdd-sdk query agent-skills sdd-synthesizer 2>/dev/null)
AGENT_SKILLS_ROADMAPPER=$(sdd-sdk query agent-skills sdd-roadmapper 2>/dev/null)
```
...
```bash
mkdir -p .planning
sdd-sdk query commit "chore: add project config" .planning/config.json
```
...
```bash
mkdir -p .planning
sdd-sdk query commit "docs: initialize project" .planning/PROJECT.md
```
...
```bash
sdd-sdk query commit "docs: define v1 requirements" .planning/REQUIREMENTS.md
```
...
```bash
sdd-sdk query commit "docs: create roadmap ([N] phases)" .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md "$INSTRUCTION_FILE"
```
