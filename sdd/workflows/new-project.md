...
```bash
INIT=$(sdd-sdk query init.new-project)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(sdd-sdk query agent-skills sdd-project-researcher)
AGENT_SKILLS_SYNTHESIZER=$(sdd-sdk query agent-skills sdd-research-synthesizer)
AGENT_SKILLS_ROADMAPPER=$(sdd-sdk query agent-skills sdd-roadmapper)
```
...
```text
⚠ SDD agents not installed. The following agents are missing from your agents directory:
...
Subagent spawns (sdd-project-researcher, sdd-research-synthesizer, sdd-roadmapper) will fail
with "agent type not found" if `required_agents_installed` is false. Run the installer with --global to make agents available:

  npx @bhargavvc/sdd-cc@latest --global
...
Ask which optional PRD-style sections `/sdd:ship` should append to generated PR bodies. These map to `ship.pr_body_sections`; selected sections are written with `"enabled": true`, unselected seeded sections are written with `"enabled": false` so the project can enable them later without editing `ship.md`.
...
AskUserQuestion([
  {
    header: "PR Body",
    question: "Which optional PRD-style sections should /sdd:ship include in PR bodies?",
    multiSelect: true,
    options: [
...
```bash
mkdir -p .planning
sdd-sdk query commit "chore: add project config" --files .planning/config.json
```
...
{if HAS_SPIKES && !SPIKE_SKILL}  ◆ Raw spikes in .planning/spikes/ — consider `/sdd:spike --wrap-up` to package findings
{if HAS_SKETCHES && !SKETCH_SKILL}  ◆ Raw sketches in .planning/sketches/ — consider `/sdd:sketch --wrap-up` to package findings
...
```bash
mkdir -p .planning
sdd-sdk query commit "docs: initialize project" --files .planning/PROJECT.md
```
...
**Check for global defaults** at `~/.sdd/defaults.json`. If the file exists, read and display its contents before asking:

```bash
DEFAULTS_RAW=$(cat ~/.sdd/defaults.json 2>/dev/null)
```
...
```text
Your saved defaults (~/.sdd/defaults.json):
  • Mode: [value]
  • Granularity: [value]
...
**If "Configure fresh" or `~/.sdd/defaults.json` doesn't exist:** proceed with the questions below.
...
**PR body onboarding:** Ask which optional PRD-style sections `/sdd:ship` should append to generated PR bodies. Use the same `ship.pr_body_sections` mapping as Step 2a: selected sections get `enabled: true`, seeded-but-unselected sections get `enabled: false`, and selecting none writes an empty list. Prefer lean/agile PRD sections that make user value, acceptance criteria, Definition of Done, and stakeholder traceability explicit.
...
Use template: ~/.claude/sdd/templates/research-project/STACK.md
</output>
...
Use template: ~/.claude/sdd/templates/research-project/FEATURES.md
</output>
...
Use template: ~/.claude/sdd/templates/research-project/ARCHITECTURE.md
</output>
...
Use template: ~/.claude/sdd/templates/research-project/PITFALLS.md
</output>
...
Use template: ~/.claude/sdd/templates/research-project/SUMMARY.md
...
```bash
sdd-sdk query commit "docs: define v1 requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
sdd-sdk query generate-claude-md --output "$INSTRUCTION_FILE"
```
...
```bash
sdd-sdk query commit "docs: create roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md "$INSTRUCTION_FILE"
```
...
```bash
PHASE1_SECTION=$(sdd-sdk query roadmap.get-phase 1 2>/dev/null)
