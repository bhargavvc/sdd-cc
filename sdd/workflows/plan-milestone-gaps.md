...
HIGHEST=$(sdd-sdk query phases.list --pick directories[-1])
```
...
```bash
INIT=$(sdd-sdk query init.phase-op "{NN}")
...
```bash
sdd-sdk query commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
