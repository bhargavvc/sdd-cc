...
HIGHEST=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" phases list --pick directories[-1])
```
...
```bash
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
