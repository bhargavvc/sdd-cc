...
sdd-plan-phase N → sdd-review N --codex → sdd-plan-phase N --reviews → sdd-review N --codex → ...
...
@$HOME/.claude/sdd/references/revision-loop.md
@$HOME/.claude/sdd/references/gates.md
@$HOME/.claude/sdd/references/agent-contracts.md
</required_reading>
...
SDD_WS=""
echo "$ARGUMENTS" | grep -qE '\-\-ws\s+\S+' && SDD_WS=$(echo "$ARGUMENTS" | grep -oE '\-\-ws\s+\S+')
```

## 1.5. Config Gate (feature disabled by default)

```bash
CONVERGENCE_ENABLED=$(sdd-sdk query config-get workflow.plan_review_convergence 2>/dev/null || echo "false")
```

**If `CONVERGENCE_ENABLED` is not `"true"`:** Display and exit:

```text
sdd-plan-review-convergence is disabled (workflow.plan_review_convergence=false).
...
```text
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 SDD ► PLAN CONVERGENCE — Phase {phase_number}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```bash
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```

Display:
```text
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 SDD ► CONVERGENCE COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
IMPORTANT: When sdd-plan-phase outputs '## PLANNING COMPLETE', that means replanning is done. Return at that point.",
  mode="auto"
)
```
...
- [ ] Initial planning via Agent → Skill("sdd-plan-phase") if no plans exist
- [ ] Review via Agent → Skill("sdd-review") — isolated, not inline; {SDD_WS} forwarded
- [ ] Replan via Agent → Skill("sdd-plan-phase --reviews") — isolated, not inline
