...
Invoked by `/sdd-thread` (`commands/sdd/thread.md`).
...
- Read frontmatter `status` field via:
  ```bash
  sdd-sdk query frontmatter.get .planning/threads/{file} status
  ```
...
2. Update the thread file's frontmatter `status` field to `resolved` and `updated` to today's ISO date:
   ```bash
   sdd-sdk query frontmatter.set .planning/threads/{SLUG}.md status resolved
   sdd-sdk query frontmatter.set .planning/threads/{SLUG}.md updated YYYY-MM-DD
   ```

3. Commit:
   ```bash
   sdd-sdk query commit "docs: resolve thread — {SLUG}" --files ".planning/threads/{SLUG}.md"
   ```
...
2. Read the file and display a summary:
   ...
   ─────────────────────────────────────
   Resume with: /sdd-thread {SLUG}
   Close with:  /sdd-thread close {SLUG}
   ```
...
```bash
sdd-sdk query frontmatter.set .planning/threads/{SLUG}.md status in_progress
sdd-sdk query frontmatter.set .planning/threads/{SLUG}.md updated YYYY-MM-DD
```
...
5. Commit:
   ```bash
   sdd-sdk query commit "docs: create thread — ${ARGUMENTS}" --files ".planning/threads/${SLUG}.md"
   ```
...
6. Report:
   ...
   Close when done with: /sdd-thread close {slug}
   ```
</mode_create>
...
- Status fields read via sdd-sdk query frontmatter.get — never eval'd or shell-expanded
- The generate-slug call for new threads runs through sdd-sdk query (or sdd-tools) which sanitizes input — keep that pattern
</security_notes>
