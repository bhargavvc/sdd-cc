...
```bash
INIT=$(sdd-sdk query init.ingest-docs)
```
...
```
SDD > Discovered {N} docs, which exceeds the v1 cap of 50.
      Use --manifest to narrow the set to ≤ 50 files, or run
      /sdd-ingest-docs again with a narrower <path>.
```
...
For each discovered doc, spawn `sdd-doc-classifier` in parallel. In Claude Code, issue all Task calls in a single message with multiple tool uses so the harness runs them concurrently. For Copilot / sequential runtimes, fall back to sequential dispatch.
...
- `<required_reading>` — `agents/sdd-doc-classifier.md` (the agent definition itself)
...
Spawn `sdd-doc-synthesizer` once:

```
Task({
  subagent_type: "sdd-doc-synthesizer",
  prompt: "
    ...
    <required_reading>
    - agents/sdd-doc-synthesizer.md
    - sdd/references/doc-conflict-engine.md
    </required_reading>
  "
})
```
...
```
SDD > BLOCKED: {N} blockers must be resolved before ingest can proceed.
```
...
Proceed to routing silently, or optionally display `SDD > No conflicts. Auto-resolved: {N}.`
...
```bash
sdd-sdk query commit "docs: ingest {N} docs from {SCAN_PATH} (#2387)" \
  .planning/PROJECT.md \
  .planning/REQUIREMENTS.md \
  .planning/ROADMAP.md \
  .planning/STATE.md \
...
- Next step: `/sdd-plan-phase 1` (new mode) or `/sdd-plan-phase N` (merge, pointing at the first newly-added phase)
