...
`audit-open` is not registered on `sdd-sdk query` yet; use the installed CJS CLI:

```bash
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" audit-open 2>/dev/null
```
...
[R] Resolve — stop and fix items, then re-run /sdd-complete-milestone
...
SECURITY: Audit JSON output is structured data from `audit-open` (sdd-tools.cjs) — validated and sanitized at source. When writing to STATE.md, item slugs and descriptions are sanitized via `sanitizeForDisplay()` before inclusion. Never inject raw user-supplied content into STATE.md without sanitization.
</step>
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  sdd-sdk query summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `sdd-sdk query milestone.complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
**Delegate archival to `sdd-sdk query milestone.complete`:**

```bash
ARCHIVE=$(sdd-sdk query milestone.complete "v[X.Y]" --name "[Milestone Name]")
```
...
```bash
sdd-sdk query commit "chore: archive v[X.Y] milestone files" .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md .planning/ROADMAP.md
```
...
**If doesn't exist:** Create from template at `~/.claude/sdd/templates/retrospective.md`.
...
**Commit:**
```bash
sdd-sdk query commit "docs: update retrospective for v${VERSION}" .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(sdd-sdk query init.execute-phase "1")
...
Detect base branch:
```bash
BASE_BRANCH=$(sdd-sdk query config-get git.base_branch 2>/dev/null || echo "")
