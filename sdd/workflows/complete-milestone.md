...
1. Re-run `sdd-sdk query audit-open --json` to get structured data
...
```bash
sdd-sdk query commit "chore: archive v[X.Y] milestone files" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md .planning/ROADMAP.md
```
...
**Commit:**
```bash
sdd-sdk query commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
<config-check>
Read `git.create_tag` via `sdd-sdk query config-get git.create_tag 2>/dev/null || echo "true"`.
