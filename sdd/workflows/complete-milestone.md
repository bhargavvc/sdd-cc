...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  node "$HOME/.claude/sdd/bin/sdd-tools.cjs" summary-extract "$summary" --fields one_liner --pick one_liner
done
```
...
```bash
ARCHIVE=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
**Commit:**
```bash
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" init execute-phase "1")
