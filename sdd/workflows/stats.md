...
```bash
STATS=$(sdd-sdk query stats.json)
...
**MVP phase summary.** Read all phases via `sdd-sdk query roadmap.analyze` (Phase 1's `cmdRoadmapAnalyze` surfaces a `mode` field per phase). Count phases by mode:
