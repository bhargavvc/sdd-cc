<purpose>
Check for SDD updates via npm, display changelog for versions between installed and latest, obtain user confirmation, and execute clean installation with cache clearing.
</purpose>
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  ...
  if [ -f "./$dir/sdd/VERSION" ] || [ -f "./$dir/sdd/workflows/update.md" ]; then
    ...
    LOCAL_VERSION_FILE="./$dir/sdd/VERSION"
    LOCAL_MARKER_FILE="./$dir/sdd/workflows/update.md"
...
Running /sdd-update would install the npm release (A.B.C) and downgrade
...
⚠️  **Note:** The installer performs a clean install of SDD folders:
- `commands/sdd/` will be wiped and replaced
- `sdd/` will be wiped and replaced
...
SDD-managed directories. These are files that exist on disk but are NOT listed
in `sdd-file-manifest.json` — i.e., files the user added themselves that the
...
Back up each custom file to `$RUNTIME_DIR/sdd-user-files-backup/` before the
installer wipes the directories:

```bash
BACKUP_DIR="$RUNTIME_DIR/sdd-user-files-backup"
mkdir -p "$BACKUP_DIR"
...
```
⚠️  Found N custom file(s) inside SDD-managed directories.
    These have been backed up to sdd-user-files-backup/ before the update.
...
Check for sdd-local-patches/backup-meta.json in the config directory.
