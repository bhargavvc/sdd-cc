...
**Source:** [#2758](https://github.com/bhargavvc/sdd-cc/issues/2758)
...
`sdd-sdk agents render` subcommand. Conditional branches resolve at render time
...
- The `use_worktrees` branch in `sdd-executor` is resolved deterministically via
  `sdd-sdk query config-get` in bash — it is not LLM-interpreted.
...
patch-migration surface (`sdd-local-patches/` rebase logic, by the reporter's own
...
`sdd-context-monitor` output despite being asked. Without a measured baseline,
...
The `/sdd-reapply-patches` three-way-merge migration for `sdd-local-patches/`
...
- Provides measured token deltas via `sdd-context-monitor` against a
...
- `sdd/workflows/` — workflow-level config embedding before subagent
