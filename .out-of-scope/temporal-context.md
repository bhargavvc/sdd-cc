# Temporal context as a first-class SDD signal

**Source:** [#2756](https://github.com/bhargavvc/sdd-cc/issues/2756)
...
SDD. Three flavors floated across the issue:
...
Framed initially as a `claude-inject-idle-time` plugin, with a request that SDD treat
the pattern as core.

## Why SDD does not own this

- **Subagent gap unsolved.** Passive injection lands in the orchestrator's context
  only. Subagents (the workers that actually do SDD's planning, execution, verification)
  spawn fresh and never see the temporal signal. The proposal does not solve this, and
  any SDD-core integration would inherit the gap. Until the subagent boundary is
  addressed, "first-class temporal context" is at best a partial feature.
- **`HANDOFF.json` duplicates existing artifacts.** SDD already persists session
...
- **Statusline / TUI re-entry is platform-level, not SDD-level.** A statusline showing
  idle time belongs in Claude Code itself or in a thin user plugin, not in SDD's phase
  machinery.
...
- `sdd/references/` — where any future plugin-interface doc would live
