...
function readLastSlashCommand(transcriptPath) {
  ...
  // Command names in Claude Code transcripts are plain identifiers like "sdd-plan-phase"
 ...
 *
 * All new fields default to undefined when absent — formatSddState() degrades
...
function runStatusline() {
  ...
  try {
    ...
    process.stdout.write(composeStatusline({ sddUpdate, model, ctx, middle, dirname, lastCmdSuffix, position }));
  } catch (e) {
 ...
 *
 * @param {object} opts
 * @param {string} [opts.sddUpdate='']      - leading update/stale-hooks warning (already formatted)
 ...
 * @param {string|null} [opts.middle=null]  - middle segment (todo task or SDD state), null = absent
...
function composeStatusline({
  ...
  if (pos === 'front') {
    if (middle) return `${sddUpdate}${modelSeg}${ctx} │ ${middle} │ ${dirSeg}${lastCmdSuffix}`;
    return `${sddUpdate}${modelSeg}${ctx} │ ${dirSeg}${lastCmdSuffix}`;
  }
  // 'end' — preserved byte-for-byte relative to original inline templates
  if (middle) return `${sddUpdate}${modelSeg} │ ${middle} │ ${dirSeg}${ctx}${lastCmdSuffix}`;
  return `${sddUpdate}${modelSeg} │ ${dirSeg}${ctx}${lastCmdSuffix}`;
}

// Export helpers for unit tests. Harmless when run as a script.
module.exports = {
  readSddState, parseStateMd, formatSddState,
  readSddConfig, getConfigValue, readLastSlashCommand,
  composeStatusline,
};
...
function renderStatusline(data) {
  ...
  const sddStateStr = formatSddState(readSddState(dir) || {});
  const middle = sddStateStr ? `\x1b[2m${sddStateStr}\x1b[0m` : null;
