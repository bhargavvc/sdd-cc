...
// SessionStart banner that surfaces SDD update availability when SDD's
// statusline isn't installed. Reads the cache that
// sdd-check-update-worker.js writes to ~/.cache/sdd/sdd-update-check.json.
//
// Opt-in by design: bin/install.js only registers this hook when the user
// declines to install (or replace) the SDD statusline. The presence of the
...
function buildBannerOutput(state) {
  ...
  if (parseError) {
    ...
    return { systemMessage: 'SDD update check failed.' };
  }
...
function main() {
  const cacheDir = path.join(os.homedir(), '.cache', 'sdd');
  ...
  if (parseError && !suppressFailureWarning) {
    ...
    // where ~/.cache/sdd was created by check-update but the parent dir got
    // wiped between runs.
    try {
