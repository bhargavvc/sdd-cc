...
All notable changes to SDD will be documented in this file.

Format follows [Keep a Changelog](https://keepachangelog.com/en/1.1.0/).

## [Unreleased](https://github.com/bhargavvc/sdd-cc/compare/v1.42.1...HEAD)

## [1.42.1](https://github.com/bhargavvc/sdd-cc/compare/v1.41.0...v1.42.1) - 2026-05-15

### Fixed

- **`/sdd-discuss-phase` and `/sdd-plan-phase` first-touch creation now apply `project_code` prefix consistently with `phase.add`/`phase.insert`** — projects with `project_code` set in `.planning/config.json` no longer accumulate a two-headed naming convention (`01-foundation/` mixed with `XR-02.1-spike/`). `init.phase-op` and `init.plan-phase` now expose `expected_phase_dir` (with prefix) in their JSON bundle; workflow fallback mkdir calls use this value instead of constructing the path from `padded_phase`+`phase_slug`. `phase.scaffold phase-dir` (CJS and SDK) also fixed. (#3287)
...
- **Six namespace meta-skills with keyword-tag descriptions** — replace the flat 86-skill
  ...
  (`sdd:workflow`, `sdd:project`, `sdd:review`, `sdd:context`, `sdd:manage`,
  `sdd:ideate`) instead of 86 flat entries; selects a namespace, then routes to the
...
- **`/sdd-health --context` utilization guard** — context-window quality guard with two
  thresholds: 60 % warns ("consider `/sdd-thread`"), 70 % is critical ("reasoning
  quality may degrade"). Exposed via `/sdd-health --context` and as a structured
  `sdd-tools validate context` command. (#2792)
- **Phase-lifecycle status-line — read-side** — `parseStateMd()` now reads four new
  ...
  `progress` (nested completed/total/percent). `formatSddState()` gains scenes for
...
- `--minimal` install flag (alias `--core-only`) writes only the main-loop core skills
  ...
  zero `sdd-*` subagents. Cuts cold-start system-prompt overhead from ~12k tokens to
  ~700, useful for local LLMs with 32K–128K context (Sonnet 4.6 / Opus 4.7 don't need
  it). Re-run `sdd update` without `--minimal` to expand to the full surface. The
  install manifest now records `mode: "minimal" | "full"`. (#2762)
- **`/sdd-edit-phase` command** — modify any field of an existing phase in ROADMAP.md
...
- **Workstream config inheritance** — when `SDD_WORKSTREAM` is set, the root
...
- **Manual canary release workflow** — `.github/workflows/canary.yml` publishes
  `{base}-canary.{N}` builds of `@bhargavvc/sdd-cc` and `@bhargavvc/sdk` under the
...
- **Skill surface consolidated 86 → 59 `commands/sdd/*.md` entries** — four new
  ...
  `sdd:code-review --fix` (was invoking deleted `sdd:code-review-fix`). (#2790)
...
- **Skill descriptions trimmed to ≤ 100 chars across all `commands/sdd/*.md`** — three
...
- **`scripts/lint-descriptions.cjs` added** — CI lint gate that fails if any
  `commands/sdd/*.md` description exceeds 100 chars. Run via `npm run lint:descriptions`.
  (#2789)
- **Skill surface consolidated from 86 → 59 `commands/sdd/*.md` entries** — four new
...
- **`autonomous.md` corrected** — was invoking deleted `sdd:code-review-fix`; now calls
  `sdd:code-review --fix`. (#2790)
...
- **`spike --wrap-up` now dispatches correctly** — `/sdd-spike --wrap-up` was silently no-oping because the flag dispatch wiring was omitted when the micro-skill entry point was absorbed in #2790. (#2948)
...
- **Gemini slash commands namespaced as `/sdd:<cmd>` instead of `/sdd-<cmd>`** —
  Gemini CLI namespaces commands under `sdd:`, so `/sdd-plan-phase` was unexecutable.
  ...
  silent no-op'ing if `commands/sdd/` is ever missing. (#2768, #2783)
...
- **`sdd-tools` invocations use the absolute installed path** — bare `sdd-tools …`
...
- **Codex installer preserves trailing newline when stripping legacy hooks** — the
  ...
  parsers. (#2866)
- **SDD slash command namespace drift cleaned up across docs, workflows, and autocomplete** — remaining active `/sdd:<cmd>` references now use canonical `/sdd-<cmd>`, escaped workflow `Skill(skill=\"sdd:...\")` prompts now use hyphenated skill names, `scripts/fix-slash-commands.cjs` rewrites retired colon syntax to hyphen syntax, and the extract-learnings command file now uses `extract-learnings.md` so generated Claude/Qwen skill autocomplete exposes `sdd-extract-learnings` instead of `sdd-extract_learnings`. (#2855)
- **`extractCurrentMilestone` no longer truncates ROADMAP.md at heading-like lines inside fenced code blocks** — the milestone-end search now scans line-by-line while tracking ` ``` ` / `~~~` fence state, so a line like `# Ops runbook (v1.0 compat)` inside a code block no longer acts as a milestone boundary. Previously, any phase defined after such a block was invisible to `roadmap analyze`, `roadmap get-phase`, `/sdd-autonomous`, and all phase-number commands. (#2787)
- **Codex install no longer corrupts existing `~/.codex/config.toml`** — the installer
  ...
  blocks regardless of SDD marker presence (both invalid in current Codex schema), emits
  the SDD-managed hook in the user's preferred shape (`[[hooks.<Event>]]` namespaced AoT
...
- **Codex legacy `[hooks]` map format auto-migrated** — Codex 0.124.0 requires
  `[[hooks]]` array-of-tables; old SDD installs that wrote `[hooks.shell]` map-style
  now self-heal on the next `sdd install --codex`. (#2637)
- **`sdd-sdk` PATH verification tightened** — installer now probes for an executable
  `sdd-sdk` shim on PATH after confirming `sdk/dist/cli.js` is present, and attempts
  to materialize one via symlink at `~/.local/bin/sdd-sdk` when absent. Only prints
  `✓ SDD SDK ready` when the probe succeeds. (#2775, #2777)
- **USER-PROFILE.md no longer triggers false "locally modified" warning** — the file
  was both preserved across reinstalls and tracked in `sdd-file-manifest.json`, causing
...
- **All `sdd-sdk query` handlers now respect `--ws`** — 18+ handlers accepted
...
- **`sdd state complete-phase` subcommand** — was missing; unknown subcommands fell
...
- **`sdd-sdk query commit` paths passed via `--files`** — 81 invocations across 50
...
- **Orchestrators wait for subagents before continuing** — 26 SDD workflow files now
...
- **`audit-uat` parser reads `human_verification:` from frontmatter array** — the
  ...
  frontmatter, surfacing false-positive open gaps at every `/sdd-complete-milestone`
  audit. (#2788)
- **`sdd-sdk` binary collision with `@bhargavvc/sdk` resolved** — workstream-aware
  query registry now respects `SDD_WORKSTREAM` env var; `sdd-tools` bin alias added so
  the two SDK packages no longer fight over the `sdd-sdk` name in `node_modules/.bin`.
  (#2791)
- **OpenCode generated agents embed `model_profile_overrides.opencode.<tier>`** —
  per-tier model overrides set via `/sdd-settings-advanced` are now propagated into the
...
- **`context_window` added to `VALID_CONFIG_KEYS` allowlist** — `/sdd-settings-advanced`
  could not set `context_window` because the key was missing from the allowlist used by
  `config-set` validation. (#2798)
- **`sdd-tools init` dispatches `ingest-docs` handler** — `/sdd-ingest-docs` was broken
  in v1.38.5 because the workflow called `sdd-sdk` (now `sdd-tools`) but no
...
- **SKILL.md frontmatter `name:` migrated to hyphen form** — files that still used the
  deprecated colon form (`sdd:cmd`) caused autocomplete to suggest `/sdd:command`.
  Frontmatter now uses canonical `sdd-cmd` hyphen names. (#2808)
- **`sdd-sdk` resolvable in local-mode installs** — the previous `isLocal` short-circuit
...
- **OpenCode `@file` references use absolute paths on all platforms** — OpenCode does
  ...
  (#2831)
- **`sdd-sdk auto` detects Codex runtime correctly** — `auto` mode ignored
  ...
  `SDD_RUNTIME` env precedence) and never injects a Claude profile id under non-Claude
  runtimes. (#2832)
- **CR-INTEGRATION tests aligned with hyphen-form skill names** — tests previously
  asserted `sdd:code-review` (colon) against `autonomous.md` which now uses the canonical
...
- **`quick.md` / `execute-phase.md` SUMMARY rescue handles gitignored `.planning/`** —
  ...
  entirely. (#2838)
- **`/sdd-code-review-fix` cleanup tail is transactional** — JSON recovery sentinel at
...
line. Install: `npm install -g @bhargavvc/sdd-cc@latest` (or `@1.39.1` to pin).

### Fixed

- **`sdd-sdk query agent-skills` emits raw `<agent_skills>` block instead of JSON-wrapped string** — workflows that embed via `$(sdd-sdk query agent-skills <agent>)` were receiving a JSON-quoted string literal mid-prompt (e.g. `"<agent_skills>\n…"`), silently breaking all `<agent_skills>` injection into spawned subagents. The CLI dispatcher now honors an opt-in `format: 'text'` field on `QueryResult` and writes such results raw via `process.stdout.write`; `--pick` always returns JSON regardless. (#2917)
- **`sketch --wrap-up` now dispatches correctly** — `/sdd-sketch --wrap-up` was silently no-oping because the flag dispatch wiring was omitted when the micro-skill entry point was absorbed in #2790. (#2949)
- **`help.md` no longer advertises eight slash commands removed by the #2824 consolidation** — `/sdd-do`, `/sdd-note`, `/sdd-check-todos`, `/sdd-plant-seed`, `/sdd-research-phase`, `/sdd-list-phase-assumptions`, `/sdd-plan-milestone-gaps`, and `/sdd-join-discord` were removed when 86 skills were folded into 59. `help.md` was not updated alongside, so users typing the documented commands hit *Unknown command*. Each entry is now either rewritten to the surviving flag-based dispatcher (e.g., `/sdd-do …` → `/sdd-progress --do "…"`, `/sdd-note` → `/sdd-capture --note`, `/sdd-plant-seed` → `/sdd-capture --seed`, `/sdd-check-todos` → `/sdd-capture --list`) or removed for skills with no replacement. A regression test now asserts every `/sdd-*` reference in `help.md` has a matching `commands/sdd/*.md` stub. (#2954)
- **`--sdk` install on Windows now writes a callable `sdd-sdk` shim** — `npx @bhargavvc/sdd-cc@latest --claude --global --sdk` on Windows previously left `sdd-sdk` off PATH because `trySelfLinkSddSdk` returned `null` unconditionally on `win32` (a missed gap from #2775's POSIX self-link, not an intentional deferral). The function now dispatches to a Windows counterpart that writes the standard npm shim triple (`sdd-sdk.cmd`, `sdd-sdk.ps1`, and a Bash wrapper) to npm's global bin, so `sdd-sdk` resolves in a fresh shell across cmd.exe, PowerShell, and Cygwin/MSYS/Git-Bash. A new regression guard in `tests/no-unconditional-win32-skip.test.cjs` blocks any future `if (process.platform === 'win32') return null;` skip-only branches in `bin/install.js`. (#2962)
- **`/sdd-reapply-patches` Step 5 gate is now deterministic — no more silent content drops** — the prior gate parsed a Claude-generated *Hunk Verification Table* whose `verified: yes` rows were filled in without actually checking content presence, leading to merged files that lost user-added blocks (e.g., a `<visual_companion>` section, an `--execute-only` flag block) while the workflow reported success. The gate now invokes a Node script (`scripts/verify-reapply-patches.cjs`) that diffs each backup against the pristine baseline, computes the user-added significant lines, and asserts each one is present in the merged file. Exits non-zero with a per-file diagnostic on any miss; the workflow halts and surfaces the JSON output to the user. The verifier ignores low-signal lines (too short, pure whitespace, decorative comments) so trivial differences don't trigger false failures. Out of scope here: the manifest-baseline tightening described in #2969 Failure 1 — that's separate work. (#2969)
...
### Enhancement: richer architecture docs from `/sdd-map-codebase` (#2500)

`/sdd-map-codebase` (arch focus) now produces a `.planning/codebase/ARCHITECTURE.md` with the same richness as the research version created at project creation:
...
Running `/sdd-map-codebase` or `/sdd-scan --focus arch` after a major refactor now produces an up-to-date architectural reference that includes the visual diagrams previously only available in the (non-refreshable) research version.

### SDK query layer — Phase 3 (what you get)

If you use SDD **as a workflow**—milestones, phases, `.planning/` artifacts, bundled workflows, and `**/sdd:`** commands—Phase 3 is about **behavior matching what the docs and steps promise**, and **a bit less overhead** when the framework advances a phase or bootstraps a new project for you.

- **Your workflow shouldn’t silently drift from the docs** — The actions that touch **STATE**, **ROADMAP**, git commits, config, and init/bootstrap are **continuously compared** to the legacy `sdd-tools.cjs` behavior in automated tests. The point for you: fewer “the workflow said X but the tooling did Y” moments as SDD ships updates (#2302).
...
- **You can see what to run next** — Documentation now states clearly **when to use `sdd-sdk query`** and **when a step still needs the legacy script** (only a few tools). The legacy script is **marked deprecated** in source but **not removed**—existing hooks and scripts keep working while you align with current examples (#2302).

### Added

- **`sdd-sdk query check auto-mode`** — Decision-routing audit Tier 2: one JSON blob for `workflow.auto_advance` + `workflow._auto_chain_active` with `active`, `source`, and per-flag fields; workflows use `--pick active` or `--pick auto_chain_active` instead of paired `config-get` calls (#2302).
...
- **SDK Phase 3 — runner hot path uses the registry directly** — When you run **phase lifecycle** or **new-project init** through the SDK, the common STATE/roadmap/plan-index/complete/commit/config calls **skip extra subprocess overhead** on the default path (workstreams and test overrides unchanged). *Contributors:* `SDDTools` → `initPhaseOp`, `phasePlanIndex`, `phaseComplete`, `initNewProject`, `configSet`, `commit` (#2302).
- **Docs — `docs/CLI-TOOLS.md`** — New **SDK and programmatic access** section (registry-first guidance, CJS→`sdd-sdk query` examples, `SDDTools`/workstream behavior, `state load` vs registry state handlers, CLI-only commands); **See also** links to `QUERY-HANDLERS.md`, Architecture, and COMMANDS (#2302).
...
- **CJS deprecation** — `sdd/bin/sdd-tools.cjs` documents `@deprecated` in favor of `sdd-sdk query` and `@bhargavvc/sdk` (#2302).

### Fixed

- **End-of-phase routing suggestions now use `/sdd-<cmd>` (not the retired `/sdd:<cmd>`)** — All user-visible command suggestions in workflows (`execute-phase.md`, `transition.md`), tool output (`profile-output.cjs`, `init.cjs`), references, and templates have been updated from `/sdd:<cmd>` to `/sdd-<cmd>`, matching the Claude Code skill directory name and the user-typed slash-command format. Internal `Skill(skill="sdd:<cmd>")` calls (no leading slash) are preserved unchanged — those resolve by frontmatter `name:` not directory name. The namespace test (`bug-2543-sdd-slash-namespace.test.cjs`) has been updated to enforce the current invariant. Closes #2697.

- **`sdd-sdk query` now resolves parent `.planning/` root in multi-repo (`sub_repos`) workspaces** — when invoked from inside a `sub_repos`-listed child repo (e.g. `workspace/app/`), the SDK now walks up to the parent workspace that owns `.planning/`, matching the legacy `sdd-tools.cjs` `findProjectRoot` behavior. Previously `sdd-sdk query init.new-milestone` reported `project_exists: false` from the sub-repo, while `sdd-tools.cjs` resolved the parent root correctly. Resolution happens once in `cli.ts` before dispatch; if `projectDir` already owns `.planning/` (including explicit `--project-dir`), the walk is a no-op. Ported as `findProjectRoot` in `sdk/src/query/helpers.ts` with the same detection order (own `.planning/` wins, then parent `sub_repos` match, then legacy `multiRepo: true`, then `.git` heuristic), capped at 10 parent levels and never crossing `$HOME`. Closes #2623.
- **Shell hooks falsely flagged as stale on every session** — `sdd-phase-boundary.sh`, `sdd-session-state.sh`, and `sdd-validate-commit.sh` now ship with a `# sdd-hook-version: {{SDD_VERSION}}` header; the installer substitutes `{{SDD_VERSION}}` in `.sh` hooks the same way it does for `.js` hooks; and the stale-hook detector in `sdd-check-update.js` now matches bash `#` comment syntax in addition to JS `//` syntax. All three changes are required together — neither the regex fix alone nor the install fix alone is sufficient to resolve the false positive (#2136, #2206, #2209, #2210, #2212)

## [1.38.2] - 2026-04-19

### Fixed
- **SDK decoupled from build-from-source install** — replaces the fragile `tsc` + `npm install -g ./sdk` dance on user machines with a prebuilt `sdk/dist/` shipped inside the parent `@bhargavvc/sdd-cc` tarball. The `sdd-sdk` CLI is now a `bin/sdd-sdk.js` shim in the parent package that resolves `sdk/dist/cli.js` and invokes it via `node`, so npm chmods the bin entry from the tarball (not from a secondary local install) and PATH/exec-bit issues cannot occur. Repurposes `installSdkIfNeeded()` in `bin/install.js` to only verify `sdk/dist/cli.js` exists and fix its execute bit (non-fatal); deletes `resolveSddSdk()`, `detectShellRc()`, `emitSdkFatal()` and the source-build/global-install logic (162 lines removed). `release.yml` now runs `npm run build:sdk` before publish in both rc and finalize jobs, so every published tarball contains fresh SDK dist. `sdk/package.json` `prepublishOnly` is the final safety net (`rm -rf dist && tsc && chmod +x dist/cli.js`). `install-smoke.yml` adds an `smoke-unpacked` variant that installs from the unpacked dir with the exec bit stripped, so this class of regression cannot ship again. Closes #2441 and #2453.
- **`--sdk` flag semantics changed** — previously forced a rebuild of the SDK from source; now verifies the bundled `sdk/dist/` is resolvable. Users who were invoking `@bhargavvc/sdd-cc --sdk` as a "force rebuild" no longer need it — the SDK ships prebuilt.

### Added
- **`/sdd-ingest-docs` command** — Scan a repo containing mixed ADRs, PRDs, SPECs, and DOCs and bootstrap or merge the full `.planning/` setup from them in a single pass. Parallel classification (`sdd-doc-classifier`), synthesis with precedence rules and cycle detection (`sdd-doc-synthesizer`), three-bucket conflicts report (`INGEST-CONFLICTS.md`: auto-resolved, competing-variants, unresolved-blockers), and hard-block on LOCKED-vs-LOCKED ADR contradictions in both new and merge modes. Supports directory-convention discovery and `--manifest <file>` YAML override with per-doc precedence. v1 caps at 50 docs per invocation; `--resolve interactive` is reserved. Extracts shared conflict-detection contract into `references/doc-conflict-engine.md` which `/sdd-import` now also consumes (#2387)
- **`/sdd-plan-review-convergence` command** — Cross-AI plan convergence loop that automates `plan-phase → review → replan → re-review` cycles. Spawns isolated agents for `sdd-plan-phase` and `sdd-review`; orchestrator only does loop control, HIGH concern counting, stall detection, and escalation. Supports `--codex`, `--gemini`, `--claude`, `--opencode`, `--all` reviewers and `--max-cycles N` (default 3). Loop exits when no HIGH concerns remain; stall detection warns when count isn't decreasing; escalation gate asks user to proceed or review manually when max cycles reached (#2306)

### Fixed
- **`sdd-read-injection-scanner` hook now ships to users** — the scanner was added in 1.37.0 (#2201) but was never added to `scripts/build-hooks.js`' `HOOKS_TO_COPY` allowlist, so it never landed in `hooks/dist/` and `install.js` skipped it with "Skipped read injection scanner hook — sdd-read-injection-scanner.js not found at target". Effectively disabled the read-time prompt-injection scanner for every user on 1.37.0/1.37.1. Added to the build allowlist and regression test. Also dropped a redundant non-absolute `.claude/hooks/` path check that was bypassing the installer's runtime-path templating and leaking `.claude/` references into non-Claude installs (#2406)
- **SDK `checkAgentsInstalled` is now runtime-aware** — `sdk/src/query/init.ts::checkAgentsInstalled` only knew where Claude Code put agents (`~/.claude/agents`). Users running SDD on Codex, OpenCode, Gemini, Kilo, Copilot, Antigravity, Cursor, Windsurf, Augment, Trae, Qwen, CodeBuddy, or Cline got `agents_installed: false` even with a complete install, which hard-blocked any workflow that gates subagent spawning on that flag. `sdk/src/query/helpers.ts` now resolves the right directory via three-tier detection (`SDD_RUNTIME` env → `config.runtime` → `claude` fallback) and mirrors `bin/install.js::getGlobalDir()` for all 14 runtimes. `SDD_AGENTS_DIR` still short-circuits the chain. `init-runner.ts` stays Claude-only by design (#2402)
- **`init` query agents-installed check looks at the correct directory** — `checkAgentsInstalled` in `sdk/src/query/init.ts` defaulted to `~/.claude/sdd/agents/`, but the installer writes SDD agents to `~/.claude/agents/`. Every init query therefore reported `agents_installed: false` on clean installs, which made workflows refuse to spawn `sdd-executor` and other parallel subagents. The default now matches `sdk/src/init-runner.ts` and the installer (#2400)
- **Installer now installs `@bhargavvc/sdk` automatically** so `sdd-sdk` lands on PATH. Resolves `command not found: sdd-sdk` errors that affected every `/sdd-*` command after a fresh install or `/sdd-update` to 1.36+. Adds `--no-sdk` to opt out and `--sdk` to force reinstall. Implements the `--sdk` flag that was previously documented in README but never wired up (#2385)

## [1.37.1] - 2026-04-17

### Fixed
- UI-phase researcher now loads sketch findings skills, preventing re-asking questions already answered during `/sdd-sketch`

## [1.37.0] - 2026-04-17

### Added
- **`/sdd-spike` and `/sdd-sketch` commands** — First-class SDD commands for rapid feasibility spiking and UI design sketching. Each produces throwaway experiments (spikes) or HTML mockups with multi-variant exploration (sketches), saved to `.planning/spikes/` and `.planning/sketches/` with full SDD integration: banners, checkpoint boxes, `sdd-sdk query` commits, and `--quick` flag to skip intake. Neither requires `/sdd-new-project` — auto-creates `.planning/` subdirs on demand
- **`/sdd-spike-wrap-up` and `/sdd-sketch-wrap-up` commands** — Package spike/sketch findings into project-local skills at `./.claude/skills/` with a planning summary at `.planning/`. Curates each spike/sketch one-at-a-time, groups by feature/design area, and adds auto-load routing to project CLAUDE.md
...
- **`/sdd-spec-phase` command** — Socratic spec refinement with ambiguity scoring to clarify WHAT a phase delivers before discuss-phase. Produces a SPEC.md with falsifiable requirements locked before implementation decisions begin (#2213)
- **`/sdd-progress --forensic` flag** — Appends a 6-check integrity audit after the standard progress report (#2231)
- **`/sdd-discuss-phase --all` flag** — Skip area selection and discuss all gray areas interactively (#2230)
- **Parallel discuss across independent phases** — Multiple phases without dependencies can be discussed concurrently (#2268)
- **`sdd-read-injection-scanner` hook** — PostToolUse hook that scans for prompt injection attempts in read file contents (#2201)
- **SDK Phase 2 caller migration** — Workflows, agents, and commands now use `sdd-sdk query` instead of raw `sdd-tools.cjs` calls (#2179)
- **Project identity in Next Up blocks** — All Next Up blocks include workspace context for multi-project clarity (#1948)
- **Agent size-budget enforcement** — New `tests/agent-size-budget.test.cjs` enforces tiered line-count limits on every `sdd-*.md` agent (XL=1600, LARGE=1000, DEFAULT=500). Unbounded agent growth is paid in context on every subagent dispatch; the test prevents regressions and requires a deliberate PR rationale to raise a budget (#2361)
- **Shared `references/mandatory-initial-read.md`** — Extracts the `<required_reading>` enforcement block that was duplicated across 5 top agents. Agents now include it via a single `@~/.claude/sdd/references/mandatory-initial-read.md` line, using Claude Code's progressive-disclosure `@file` reference mechanism (#2361)
...
### Changed
- **`sdd-debugger` philosophy extracted to shared reference** — The 76-line `<philosophy>` block containing evergreen debugging disciplines (user-as-reporter framing, meta-debugging, foundation principles, cognitive-bias table, systematic investigation, when-to-restart protocol) is now in `sdd/references/debugger-philosophy.md` and pulled into the agent via a single `@file` include. Same content, lighter per-dispatch context footprint (#2363)
- **`sdd-planner`, `sdd-executor`, `sdd-debugger`, `sdd-verifier`, `sdd-phase-researcher`** — Migrated to `@file` includes for the mandatory-initial-read and project-skills-discovery boilerplate. Reduces per-dispatch context load without changing behavior (#2361)
- **Consolidated emphasis-marker density in top 4 agent files** — `sdd-planner.md` (23 → 15), `sdd-phase-researcher.md` (14 → 9), `sdd-doc-writer.md` (11 → 6), and `sdd-executor.md` (10 → 7). Removed `CRITICAL:` prefixes from H2/H3 headings and dropped redundant `CRITICAL:` + `MUST` / `ALWAYS:` + `NEVER:` stacking. RFC-2119 `MUST`/`NEVER` verbs inside normative sentences are preserved. Behavior-preserving; no content removed (#2368)

### Fixed
- **Broken `@planner-source-audit.md` relative references in `sdd-planner.md`** — Two locations referenced `@planner-source-audit.md` (resolves relative to working directory, almost always missing) instead of the correct absolute `@~/.claude/sdd/references/planner-source-audit.md`. The planner's source audit discipline was silently unenforced (#2361)
...
- **`/sdd-quick` crashes** — sdd-sdk pre-flight check with install hint (#2334); rescue uncommitted SUMMARY.md before worktree removal (#2296)
...
## [1.36.0](https://github.com/bhargavvc/sdd-cc/releases/tag/v1.36.0) - 2026-04-14

### SDK query layer — Phases 1 & 2 (what you get)

Day to day, SDD still revolves around **your planning tree** (ROADMAP, STATE, phase folders, config) and **following the workflow** (discuss → plan → execute → verify, milestone closes, etc.). Phases 1 and 2 introduce `**sdd-sdk query`** so those “plumbing” steps have a **supported, first-class CLI**—and so **what workflows and `/sdd:` docs tell you to paste** is closer to what actually runs.

- **Phase 1 — Unblock faster when a step fails (#2118)** — The same kinds of checks and updates your **workflows, hooks, and agents** rely on—reading phase context, roadmap, STATE, init payloads, config, validation—can go through `**sdd-sdk query`**. When something is wrong (bad path, missing file, invalid args), you get **errors you can act on**, not an opaque script dump—so a stuck phase or a bad copy-paste is easier to fix, and **your own** terminal or CI glue beside SDD is easier to keep stable.
- **Phase 2 — Trust the examples in workflows (#2122, #2008)** — The `**sdd-sdk query`** CLI **only runs commands that exist**—no accidental fallback to something else. **Workflow and agent examples** were updated to match. A few **special-case tools** (e.g. **graphify**, **from-gsd2**) still call the legacy binary until they’re brought onto the same path; `**docs/CLI-TOOLS.md`** and `**sdk/src/query/QUERY-HANDLERS.md**` list what’s in scope. Hardening (commits, locks, paths, argument parsing) mostly shows up as **fewer odd failures mid-milestone** when STATE, roadmap, and git steps run.

Technical implementation details for Phase 2 appear in the **Changed** section below.

### Added

- `**/sdd-graphify` integration** — Knowledge graph for planning agents, enabling richer context connections between project artifacts (#2164)
- `**sdd-pattern-mapper` agent** — Codebase pattern analysis agent for identifying recurring patterns and conventions (#1861)
- `**@bhargavvc/sdk` — Phase 1 typed query foundation (#2118)** — Introduces `**sdd-sdk query`** and registry-backed handlers; see **SDK query layer — Phases 1 & 2** above for how that fits the workflow.
...
- `**/sdd-quick` and `/sdd-thread` subcommands** — Added list/status/resume/close subcommands (#2159)
- **Debug skill dispatch and session manager** — Sub-orchestrator for `/sdd-debug` sessions (#2154)
- **Project skills awareness** — 9 SDD agents now discover and use project-scoped skills (#2152)
- `**/sdd-debug` session management** — TDD gate, reasoning checkpoint, and security hardening (#2146)
...
- `**/sdd-extract-learnings` command** — Phase knowledge capture workflow (#1873)
...
- **Cursor CLI self-detection** — Cursor detection and REVIEWS.md template for `/sdd-review` (#1960)
...
- `**/sdd-skill-manifest` command** — Pre-compute skill discovery for faster session starts (#2101)
...
- `**/sdd-next` prior-phase completeness scan** — Replaced consecutive-call counter with completeness scan (#2097)
...
- **PR #2179 maintainer review (Trek-e)** — Scoped SDK to Phase 2 (#2122): removed `sdd-sdk query` passthrough to `sdd-tools.cjs` and `SDD_TOOLS_PATH` override; argv routing consolidated in `resolveQueryArgv()`. `SDDTools` JSON parsing now reports `@file:` indirection read failures instead of failing opaquely. `execute-plan.md` defers Task Commit Protocol to `agents/sdd-executor.md` (single source of truth). Stale `/sdd:` scan (#1748) skips `.planning/` and root `CLAUDE.md` so local gitignored overlays do not fail CI.
...
- `**sdd-tools audit-open`** — Use `core.output()` (was undefined `output()`), and pass the artifact object for `--json` so stdout is JSON (not double-stringified).
- **SDK query layer (PR review hardening)** — `commit-to-subrepo` uses realpath-aware path containment and sanitized commit messages; `state.planned-phase` uses the STATE.md lockfile; `verifyKeyLinks` mitigates ReDoS on frontmatter patterns; frontmatter handlers resolve paths under the real project root; phase directory names reject `..` and separators; `sdd-sdk` restores strict CLI parsing by stripping `--pick` before `parseArgs`; `QueryRegistry.commands()` for enumeration; `todoComplete` uses static error imports.
- `**sdd-sdk query` routing (Phase 2 scope)** — `resolveQueryArgv()` maps argv to registered handlers (longest-prefix match on dotted and spaced command keys; optional single-token dotted split). Unregistered commands are rejected at the CLI; use `node …/sdd-tools.cjs` for CJS-only subcommands. `resolveSddToolsPath()` probes the SDK-bundled copy, then project and user `~/.claude/sdd/` installs (no `SDD_TOOLS_PATH` override). Broader “CLI parity” passthrough is explicitly out of scope for #2122 and tracked separately for a future approved issue.
...
- **Phase 2 caller migration (#2122)** — Workflows, agents, and commands prefer `sdd-sdk query` for registered handlers; extended migration to additional orchestration call sites (review, plan-phase, execute-plan, ship, extract_learnings, ai-integration-phase, eval-review, next, profile-user, autonomous, thread command) and researcher agents; dual-path and CJS-only exceptions documented in `docs/CLI-TOOLS.md` and `docs/ARCHITECTURE.md`; relaxed `tests/sdd-tools-path-refs.test.cjs` so `commands/sdd/workstreams.md` may document `sdd-sdk query` without `node` + `sdd-tools.cjs`. CJS `sdd-tools.cjs` remains on disk; graphify and other non-registry commands stay on CJS until registered. (#2008)
- **Phase 2 docs and call sites (follow-up)** — `docs/USER-GUIDE.md` now explains `sdd-sdk query` vs legacy CJS and lists CJS-only commands (`state validate`/`sync`, `audit-open`, `graphify`, `from-gsd2`). Updated `commands/sdd` (`debug`, `quick`, `intel`), `agents/sdd-debug-session-manager.md`, and workflows (`milestone-summary`, `forensics`, `next`, `complete-milestone`, `verify-work`, `discuss-phase`, `progress`, `verify-phase`, `add-phase`/`insert-phase`/`remove-phase`, `transition`, `manager`, `quick`) for `sdd-sdk query` or explicit CJS exceptions (`audit-open`).
- **Phase 2 orchestration doc pass (#2122)** — Aligned `commands/sdd` (`execute-phase`, `code-review`, `code-review-fix`, `from-gsd2`, `graphify`) and agents (`sdd-verifier`, `sdd-plan-checker`, `sdd-code-fixer`, `sdd-executor`, `sdd-planner`, researchers, debugger) so examples use `init.*` query names, correct `frontmatter.get` positional field, `state.*` positional args, and `commit` with positional file paths (not `--files`, except `commit-to-subrepo` which keeps `--files`).
- **Phase 2 `commit` example sweep (#2122)** — Normalized `sdd-sdk query commit` usage across `sdd/workflows/**/*.md`, `sdd/references/**/*.md`, and `commands/sdd/**/*.md` so file paths follow the message positionally (SDK `commit` handler); `sdd-sdk query commit-to-subrepo … --files …` unchanged. Updated `sdd/references/git-planning-commit.md` prose; adjusted workflow contract tests (`claude-md`, forensics, milestone-summary, gates taxonomy CRLF-safe `required_reading`, verifier `roadmap.analyze`) for the new examples.
...
- **SDD temp directory** — Use dedicated temp subdirectory for SDD temp files (#1975, #2100)
...
## [1.35.0](https://github.com/bhargavvc/sdd-cc/releases/tag/v1.35.0) - 2026-04-10
...
- **CodeBuddy runtime support** — Skills-based install to `~/.codebuddy/skills/sdd-*/SKILL.md`. `--codebuddy` flag.
- **Qwen Code runtime support** — Skills-based install to `~/.qwen/skills/sdd-*/SKILL.md`, same open standard as Claude Code 2.1.88+. `QWEN_CONFIG_DIR` env var for custom paths. `--qwen` flag.
- `**/sdd-from-gsd2` command** (`sdd:from-gsd2`) — Reverse migration from SDD-2 format (`.sdd/` with Milestone→Slice→Task hierarchy) back to v1 `.planning/` format. Flags: `--dry-run` (preview only), `--force` (overwrite existing `.planning/`), `--path <dir>` (specify SDD-2 root). Produces `PROJECT.md`, `REQUIREMENTS.md`, `ROADMAP.md`, `STATE.md`, and sequential phase dirs. Flattens Milestone→Slice hierarchy to sequential phase numbers (M001/S01→phase 01, M001/S02→phase 02, M002/S01→phase 03, etc.).
- `**/sdd-ai-integration-phase` command** (`sdd:ai-integration-phase`) — AI framework selection wizard for integrating AI/LLM capabilities into a project phase. Interactive decision matrix with domain-specific failure modes and eval criteria. Produces `AI-SPEC.md` with framework recommendation, implementation guidance, and evaluation strategy. Runs 3 parallel specialist agents: domain-researcher, framework-selector, ai-researcher, eval-planner.
- `**/sdd-eval-review` command** (`sdd:eval-review`) — Retroactive audit of an implemented AI phase's evaluation coverage. Checks implementation against `AI-SPEC.md` evaluation plan. Scores each eval dimension as COVERED/PARTIAL/MISSING. Produces `EVAL-REVIEW.md` with findings, gaps, and remediation guidance.
- **Review model configuration** — Per-CLI model selection for /sdd-review via `review.models.<cli>` config keys. Falls back to CLI defaults when not set. (#1849)
- **Statusline now surfaces SDD milestone/phase/status** — when no `in_progress` todo is active, `sdd-statusline.js` reads `.planning/STATE.md` (walking up from the workspace dir) and fills the middle slot with `<milestone> · <status> · <phase> (N/total)`. Gracefully degrades when fields are missing; identical to previous behavior when there is no STATE.md or an active todo wins the slot. Uses the YAML frontmatter added for #628.
- **Qwen Code and Cursor CLI peer reviewers** — Added as reviewers in `/sdd-review` with `--qwen` and `--cursor` flags. (#1966)

### Changed

- **Worktree safety — `git clean` prohibition** — `sdd-executor` now prohibits `git clean` in worktree context to prevent deletion of prior wave output. (#2075)
...
- `**Agent` tool in sdd-autonomous** — Added `Agent` to `allowed-tools` to unblock subagent spawning. (#2043)
...
## [1.34.2](https://github.com/bhargavvc/sdd-cc/releases/tag/v1.34.2) - 2026-04-06
...
## [1.34.1](https://github.com/bhargavvc/sdd-cc/releases/tag/v1.34.1) - 2026-04-06
...
## [1.34.0](https://github.com/bhargavvc/sdd-cc/releases/tag/v1.34.0) - 2026-04-06
...
## [1.33.0](https://github.com/bhargavvc/sdd-cc/releases/tag/v1.33.0) - 2026-04-05
...
## [1.30.0](https://github.com/bhargavvc/sdd-cc/releases/tag/v1.30.0) - 2026-03-26
...
## [1.29.0](https://github.com/bhargavvc/sdd-cc/releases/tag/v1.29.0) - 2026-03-25
...
- Repository references updated from `glittercowboy` to `sdd-build`
...
- Missing SDD agents detected with warning when `subagent_type` falls back to general-purpose
...
## [1.28.0](https://github.com/bhargavvc/sdd-cc/releases/tag/v1.28.0) - 2026-03-22

### Added

- **Workstream namespacing** — Parallel milestone work via `/sdd:workstreams`
- **Multi-project workspace commands** — Manage multiple SDD projects from a single root
- `**/sdd:forensics` command** — Post-mortem workflow investigation
- `**/sdd:milestone-summary` command** — Post-build onboarding for completed milestones
...
- **"Follow the Indirection" debugging technique** in sdd-debugger
- `**--reviews` flag** for `sdd:plan-phase`
...
## [1.27.0](https://github.com/glittercowboy/sdd/releases/tag/v1.27.0) - 2026-03-20
...
- `**/sdd:fast` command** — Trivial inline tasks that skip planning entirely
- `**/sdd:review` command** — Cross-AI peer review of current phase or branch
- `**/sdd:plant-seed` command** — Backlog parking lot for ideas and persistent context threads
- `**/sdd:pr-branch` command** — Clean PR branches filtering `.planning/` commits
- `**/sdd:audit-uat` command** — Verification debt tracking across phases
...
- **Security hardening** — Centralized `security.cjs` module with path traversal prevention, prompt injection detection/sanitization, safe JSON parsing, field name validation, and shell argument validation. PreToolUse `sdd-prompt-guard` hook scans writes to `.planning/` for injection patterns
...
- Stale hooks check filtered to sdd-prefixed files only
...
## [1.26.0](https://github.com/glittercowboy/sdd/releases/tag/v1.26.0) - 2026-03-18

### Added

- **Developer profiling pipeline** — `/sdd:profile-user` analyzes Claude Code session history to build behavioral profiles across 8 dimensions (communication, decisions, debugging, UX, vendor choices, frustrations, learning style, explanation depth). Generates `USER-PROFILE.md`, `/sdd:dev-preferences`, and `CLAUDE.md` profile section. Includes `--questionnaire` fallback and `--refresh` for re-analysis (#1084)
- `**/sdd:ship` command** — PR creation from verified phase work. Auto-generates rich PR body from planning artifacts, pushes branch, creates PR via `gh`, and updates STATE.md (#829)
- `**/sdd:next` command** — Automatic workflow advancement to the next logical step (#927)
...
- **Structured session handoff artifact** — `/sdd:pause-work` writes `.planning/HANDOFF.json` for machine-readable cross-session continuity (#940)
...
- **MCP tool awareness** — SDD subagents can discover and use MCP server tools (#973)
...
- `**/sdd:audit-uat` command** — Cross-phase audit of all outstanding UAT and verification items. Scans every phase for pending, skipped, blocked, and human_needed items. Cross-references against codebase to detect stale documentation. Produces prioritized human test plan grouped by testability
- **Verification debt tracking** — Five structural improvements to prevent silent loss of UAT/verification items when projects advance:
  - Cross-phase health check in `/sdd:progress` (Step 1.6) surfaces outstanding items from ALL prior phases
...
- **Advisor mode for discuss-phase** — Spawns parallel research agents during `/sdd:discuss-phase` to evaluate gray areas before user decides. Returns structured comparison tables calibrated to user's vendor philosophy. Activates only when `USER-PROFILE.md` exists (#1211)
...
- Agent suggests non-existent `/sdd:transition` — replaced with real commands (#1081, #1100)
...
## [1.25.0](https://github.com/glittercowboy/sdd/releases/tag/v1.25.0) - 2026-03-16
...
- `**/sdd:do` command** — Freeform text router that dispatches natural language to the right SDD command
- `**/sdd:note` command** — Zero-friction idea capture with append, list, and promote-to-todo subcommands
...
- `/sdd:discuss-phase` shows remaining discussion areas when asking to continue or move on
- `/sdd:plan-phase` asks user about research instead of silently deciding
...
## [1.24.0](https://github.com/glittercowboy/sdd/releases/tag/v1.24.0) - 2026-03-15

### Added

- `**/sdd:quick --research` flag** — Spawns focused research agent before planning, composable with `--discuss` and `--full` (#317)
...
- **Programmatic `/sdd:set-profile`** — runs as a script instead of LLM-driven workflow, executes in seconds instead of 30-40s
...
- `sdd-tools.cjs` uses absolute paths in all install types (#820)
- Invalid `skills:` frontmatter removed from UI agent files

## [1.23.0](https://github.com/glittercowboy/sdd/releases/tag/v1.23.0) - 2026-03-15

### Added

- `/sdd:ui-phase` + `/sdd:ui-review` — UI design contract generation and retroactive 6-pillar visual audit for frontend phases (closes #986)
- `/sdd:stats` — project statistics dashboard: phases, plans, requirements, git metrics, and timeline
- **Copilot CLI** runtime support — install with `--copilot`, maps Claude Code tools to GitHub Copilot tools
- `**sdd-autonomous` skill** for Codex runtime — enables autonomous SDD execution
...
- `/sdd:discuss-phase` supports explicit `--batch` mode for grouped question intake

### Fixed

- `/sdd:new-milestone` no longer resets `workflow.research` config during milestone transitions
- `/sdd:update` is runtime-aware and targets the correct runtime directory
...
## [1.22.4](https://github.com/glittercowboy/sdd/releases/tag/v1.22.4) - 2026-03-03

### Added

- `--discuss` flag for `/sdd:quick` — lightweight pre-planning discussion to gather context before quick tasks
...
- Missing `skills` frontmatter on sdd-nyquist-auditor agent

## [1.22.3](https://github.com/glittercowboy/sdd/releases/tag/v1.22.3) - 2026-03-03
...
## [1.22.2](https://github.com/glittercowboy/sdd/releases/tag/v1.22.2) - 2026-03-03

### Fixed

- Codex installer no longer creates duplicate `[features]` and `[agents]` sections on re-install (#902, #882)
- Context monitor hook is advisory instead of blocking non-SDD workflows
...
## [1.22.1](https://github.com/glittercowboy/sdd/releases/tag/v1.22.1) - 2026-03-02
...
## [1.22.0](https://github.com/glittercowboy/sdd/releases/tag/v1.22.0) - 2026-02-27
...
- Code-aware discuss phase with codebase scouting — `/sdd:discuss-phase` now analyzes relevant source files before asking questions
...
## [1.21.1](https://github.com/glittercowboy/sdd/releases/tag/v1.21.1) - 2026-02-27
...
## [1.21.0](https://github.com/glittercowboy/sdd/releases/tag/v1.21.0) - 2026-02-25

### Added

- YAML frontmatter sync to STATE.md for machine-readable status tracking
- `/sdd:add-tests` command for post-phase test generation
- Codex runtime support with skills-first installation
- Standard `project_context` block in sdd-verifier output
- Codex changelog and usage documentation

### Changed

- Improved onboarding UX: installer now suggests `/sdd:new-project` instead of `/sdd:help`
- Updated Discord invite to vanity URL (discord.gg/sdd)
- Compressed Nyquist validation layer to align with SDD meta-prompt conventions
...
- `/sdd:update` always installs latest package version
...
## [1.20.6](https://github.com/glittercowboy/sdd/releases/tag/v1.20.6) - 2025-02-23
...
## [1.20.5](https://github.com/glittercowboy/sdd/releases/tag/v1.20.5) - 2026-02-19

### Fixed

- `/sdd:health --repair` now creates timestamped backup before regenerating STATE.md (#657)
...
## [1.20.4](https://github.com/glittercowboy/sdd/releases/tag/v1.20.4) - 2026-02-17
...
## [1.20.3](https://github.com/glittercowboy/sdd/releases/tag/v1.20.3) - 2026-02-16
...
## [1.20.2](https://github.com/glittercowboy/sdd/releases/tag/v1.20.2) - 2026-02-16
...
## [1.20.1](https://github.com/glittercowboy/sdd/releases/tag/v1.20.1) - 2026-02-16
...
## [1.20.0](https://github.com/glittercowboy/sdd/releases/tag/v1.20.0) - 2026-02-15

### Added

- `/sdd:health` command — validates `.planning/` directory integrity with `--repair` flag for auto-fixing config.json and STATE.md
- `--full` flag for `/sdd:quick` — enables plan-checking (max 2 iterations) and post-execution verification on quick tasks
- `--auto` flag wired from `/sdd:new-project` through the full phase chain (discuss → plan → execute)
- Auto-advance chains phase execution across full milestones when `workflow.auto_advance` is enabled

### Fixed

- Plans created without user context — `/sdd:plan-phase` warns when no CONTEXT.md exists, `/sdd:discuss-phase` warns when plans already exist (#253)
- OpenCode installer converts `general-purpose` subagent type to OpenCode's `general`
- `/sdd:complete-milestone` respects `commit_docs` setting when merging branches
- Phase directories tracked in git via `.gitkeep` files

## [1.19.2](https://github.com/glittercowboy/sdd/releases/tag/v1.19.2) - 2026-02-15

### Added

- User-level default settings via `~/.sdd/defaults.json` — set SDD defaults across all projects
...
- ESM conflicts prevented by renaming sdd-tools.js to .cjs
...
## [1.19.1](https://github.com/glittercowboy/sdd/releases/tag/v1.19.1) - 2026-02-15
...
## [1.19.0](https://github.com/glittercowboy/sdd/releases/tag/v1.19.0) - 2026-02-15
...
## [1.18.0](https://github.com/glittercowboy/sdd/releases/tag/v1.18.0) - 2026-02-08

### Added

- `--auto` flag for `/sdd:new-project` — runs research → requirements → roadmap automatically after config questions. Expects idea document via @ reference (e.g., `/sdd:new-project --auto @prd.md`)
...
- Research decision from `/sdd:new-milestone` now persists to config.json

## [1.17.0](https://github.com/glittercowboy/sdd/releases/tag/v1.17.0) - 2026-02-08

### Added

- **sdd-tools verification suite**: `verify plan-structure`, `verify phase-completeness`, `verify references`, `verify commits`, `verify artifacts`, `verify key-links` — deterministic structural checks
- **sdd-tools frontmatter CRUD**: `frontmatter get/set/merge/validate` — safe YAML frontmatter operations with schema validation
- **sdd-tools template fill**: `template fill summary/plan/verification` — pre-filled document skeletons
- **sdd-tools state progression**: `state advance-plan`, `state update-progress`, `state record-metric`, `state add-decision`, `state add-blocker`, `state resolve-blocker`, `state record-session` — automates STATE.md updates
- **Local patch preservation**: Installer now detects locally modified SDD files, backs them up to `sdd-local-patches/`, and creates a manifest for restoration
- `/sdd:reapply-patches` command to merge local modifications back after SDD updates

### Changed

- Agents (executor, planner, plan-checker, verifier) now use sdd-tools for state updates and verification instead of manual markdown parsing
- `/sdd:update` workflow now notifies about backed-up local patches and suggests `/sdd:reapply-patches`
...
## [1.16.0](https://github.com/glittercowboy/sdd/releases/tag/v1.16.0) - 2026-02-08

### Added

- 10 new sdd-tools CLI commands that replace manual AI orchestration of mechanical operations:
...
- Workflows now delegate deterministic operations to sdd-tools CLI, reducing token usage and errors:
...
- Execute-phase now correctly spawns `sdd-executor` subagents instead of generic task agents
- `commit_docs=false` setting now respected in all `.planning/` commit paths (execute-plan, debugger, reference docs all route through sdd-tools CLI)
...
- Windows: Normalized backslash paths in sdd-tools invocations (contributed by @rmindel)

## [1.15.0](https://github.com/glittercowboy/sdd/releases/tag/v1.15.0) - 2026-02-08
...
## [1.14.0](https://github.com/glittercowboy/sdd/releases/tag/v1.14.0) - 2026-02-08

### Added

- Context-optimizing parsing commands in sdd-tools (`phase-plan-index`, `state-snapshot`, `summary-extract`) — reduces agent context usage by returning structured JSON instead of raw file content
...
## [1.13.0](https://github.com/glittercowboy/sdd/releases/tag/v1.13.0) - 2026-02-08

### Added

- `sdd-tools history-digest` — Compiles phase summaries into structured JSON for faster context loading
- `sdd-tools phases list` — Lists phase directories with filtering (replaces fragile `ls | sort -V` patterns)
- `sdd-tools roadmap get-phase` — Extracts phase sections from ROADMAP.md
- `sdd-tools phase next-decimal` — Calculates next decimal phase number for insert operations
- `sdd-tools state get/patch` — Atomic STATE.md field operations
- `sdd-tools template select` — Chooses summary template based on plan complexity
...
- Agents migrated from bash patterns to structured sdd-tools commands
- Nested YAML frontmatter parsing now handles `dependency-graph.provides`, `tech-stack.added` correctly

## [1.12.1](https://github.com/glittercowboy/sdd/releases/tag/v1.12.1) - 2026-02-08
...
## [1.12.0](https://github.com/glittercowboy/sdd/releases/tag/v1.12.0) - 2026-02-07
...
- **Centralized utilities** — New `sdd-tools.cjs` (11 functions) replaces repetitive bash patterns across 50+ files
...
- `sdd-tools.cjs` CLI utility with functions: state load/update, resolve-model, find-phase, commit, verify-summary, generate-slug, current-timestamp, list-todos, verify-path-exists, config-ensure-section

## [1.11.2](https://github.com/glittercowboy/sdd/releases/tag/v1.11.2) - 2026-02-05
...
- **CRITICAL:** Prevent API keys from being committed via `/sdd:map-codebase` (#429)
...
- Auto-create `config.json` when missing during `/sdd:settings` (#264)
- `/sdd:update` respects local vs global install location
...
- Removed broken sdd-gemini link (404)

## [1.11.1](https://github.com/glittercowboy/sdd/releases/tag/v1.11.0) - 2026-01-31

### Added

- Git branching strategy configuration with three options:
  - `none` (default): commit to current branch
  - `phase`: create branch per phase (`sdd/phase-{N}-{slug}`)
  - `milestone`: create branch per milestone (`sdd/{version}-{slug}`)
...
- CONTEXT.md from `/sdd:discuss-phase` now properly flows to all downstream agents (researcher, planner, checker, revision loop)

## [1.10.1](https://github.com/glittercowboy/sdd/releases/tag/v1.10.1) - 2025-01-30

### Fixed

- Gemini CLI agent loading errors that prevented commands from executing

## [1.10.0](https://github.com/glittercowboy/sdd/releases/tag/v1.10.0) - 2026-01-29
...
## [1.9.12](https://github.com/glittercowboy/sdd/releases/tag/v1.9.12) - 2025-01-23

### Removed

- `/sdd:whats-new` command — use `/sdd:update` instead (shows changelog with cancel option)

### Fixed

- Restored auto-release GitHub Actions workflow

## [1.9.11](https://github.com/glittercowboy/sdd/releases/tag/v1.9.11) - 2026-01-23
...
## [1.9.10](https://github.com/glittercowboy/sdd/releases/tag/v1.9.10) - 2026-01-23

### Added

- Discord community link shown in installer completion message

## [1.9.9](https://github.com/glittercowboy/sdd/releases/tag/v1.9.9) - 2026-01-23

### Added

- `/sdd:join-discord` command to quickly access the SDD Discord community invite link

## [1.9.8](https://github.com/glittercowboy/sdd/releases/tag/v1.9.8) - 2025-01-22

### Added

- Uninstall flag (`--uninstall`) to cleanly remove SDD from global or local installations

### Fixed

- Context file detection now matches filename variants (handles both `CONTEXT.md` and `{phase}-CONTEXT.md` patterns)

## [1.9.7](https://github.com/glittercowboy/sdd/releases/tag/v1.9.7) - 2026-01-22

### Fixed

- OpenCode installer now uses correct XDG-compliant config path (`~/.config/opencode/`) instead of `~/.opencode/`
- OpenCode commands use flat structure (`command/sdd-help.md`) matching OpenCode's expected format
- OpenCode permissions written to `~/.config/opencode/opencode.json`

## [1.9.6](https://github.com/glittercowboy/sdd/releases/tag/v1.9.6) - 2026-01-22

### Added

- Interactive runtime selection: installer now prompts to choose Claude Code, OpenCode, or both
- Native OpenCode support: `--opencode` flag converts SDD to OpenCode format automatically
- `--both` flag to install for both Claude Code and OpenCode in one command
- Auto-configures `~/.opencode.json` permissions for seamless SDD doc access
...
## [1.9.5](https://github.com/glittercowboy/sdd/releases/tag/v1.9.5) - 2025-01-22
...
- Removed stray backticks in `/sdd:new-project` output
...
## [1.9.4](https://github.com/glittercowboy/sdd/releases/tag/v1.9.4) - 2026-01-21
...
## [1.9.2](https://github.com/glittercowboy/sdd/releases/tag/v1.9.2) - 2025-01-21

### Removed

- **Codebase Intelligence System** — Removed due to overengineering concerns
  - Deleted `/sdd:analyze-codebase` command
  - Deleted `/sdd:query-intel` command
  - Removed SQLite graph database and sql.js dependency (21MB)
  - Removed intel hooks (sdd-intel-index.js, sdd-intel-session.js, sdd-intel-prune.js)
...
## [1.9.0](https://github.com/glittercowboy/sdd/releases/tag/v1.9.0) - 2025-01-20

### Added

- **Model Profiles** — `/sdd:set-profile` for quality/balanced/budget agent configurations
- **Workflow Settings** — `/sdd:settings` command for toggling workflow behaviors interactively
...
- All hooks now use `sdd-` prefix for consistency (statusline.js → sdd-statusline.js)

## [1.8.0](https://github.com/glittercowboy/sdd/releases/tag/v1.8.0) - 2026-01-19
...
- `/sdd:new-project` now asks about git tracking during initial setup, letting you opt out of committing planning docs from the start

## [1.7.1](https://github.com/glittercowboy/sdd/releases/tag/v1.7.1) - 2026-01-19
...
## [1.7.0](https://github.com/glittercowboy/sdd/releases/tag/v1.7.0) - 2026-01-19

### Added

- **Quick Mode** (`/sdd:quick`) — Execute small, ad-hoc tasks with SDD guarantees but skip optional agents (researcher, checker, verifier). Quick tasks live in `.planning/quick/` with their own tracking in STATE.md.

### Changed

- Improved progress bar calculation to clamp values within 0-100 range
- Updated documentation with comprehensive Quick Mode sections in help.md, README.md, and SDD-STYLE.md
...
- Removed dead sdd-researcher agent references
...
## [1.6.4](https://github.com/glittercowboy/sdd/releases/tag/v1.6.4) - 2026-01-17
...
- Orphaned `sdd-notify.sh` hook from previous versions is now automatically removed during install (both file and settings.json registration)

## [1.6.3](https://github.com/glittercowboy/sdd/releases/tag/v1.6.3) - 2025-01-17

### Added

- `--gaps-only` flag for `/sdd:execute-phase` — executes only gap closure plans after verify-work finds issues, eliminating redundant state discovery

## [1.6.2](https://github.com/glittercowboy/sdd/releases/tag/v1.6.2) - 2025-01-17
...
## [1.6.1](https://github.com/glittercowboy/sdd/releases/tag/v1.6.1) - 2025-01-17

### Changed

- Installer performs clean install of SDD folders, removing orphaned files from previous versions
- `/sdd:update` shows changelog and asks for confirmation before updating, with clear warning about what gets replaced

## [1.6.0](https://github.com/glittercowboy/sdd/releases/tag/v1.6.0) - 2026-01-17

### Changed

- **BREAKING:** Unified `/sdd:new-milestone` flow — now mirrors `/sdd:new-project` with questioning → research → requirements → roadmap in a single command
- Roadmapper agent now references templates instead of inline structures for easier maintenance

### Removed

- **BREAKING:** `/sdd:discuss-milestone` — consolidated into `/sdd:new-milestone`
- **BREAKING:** `/sdd:create-roadmap` — integrated into project/milestone flows
- **BREAKING:** `/sdd:define-requirements` — integrated into project/milestone flows
- **BREAKING:** `/sdd:research-project` — integrated into project/milestone flows

### Added

- `/sdd:verify-work` now includes next-step routing after verification completes

## [1.5.30](https://github.com/glittercowboy/sdd/releases/tag/v1.5.30) - 2026-01-17
...
- Next-step suggestions now consistently recommend `/sdd:discuss-phase` before `/sdd:plan-phase` across all routing paths

## [1.5.29](https://github.com/glittercowboy/sdd/releases/tag/v1.5.29) - 2025-01-16
...
- Removed blocking notification popups (sdd-notify) on all platforms

## [1.5.28](https://github.com/glittercowboy/sdd/releases/tag/v1.5.28) - 2026-01-16

### Changed

- Consolidated milestone workflow into single command
- Merged domain expertise skills into agent configurations
- **BREAKING:** Removed `/sdd:execute-plan` command (use `/sdd:execute-phase` instead)
...
## [1.5.27](https://github.com/glittercowboy/sdd/releases/tag/v1.5.27) - 2026-01-16
...
## [1.5.26](https://github.com/glittercowboy/sdd/releases/tag/v1.5.26) - 2026-01-16
...
## [1.5.25](https://github.com/glittercowboy/sdd/releases/tag/v1.5.25) - 2026-01-16
...
## [1.5.24](https://github.com/glittercowboy/sdd/releases/tag/v1.5.24) - 2026-01-16
...
## [1.5.23](https://github.com/glittercowboy/sdd/releases/tag/v1.5.23) - 2025-01-16
...
- Removed `/sdd:research-phase` from next-step suggestions (use `/sdd:plan-phase` instead)

## [1.5.22](https://github.com/glittercowboy/sdd/releases/tag/v1.5.22) - 2025-01-16

### Added

- Statusline update indicator — shows `⬆ /sdd:update` when a new version is available

### Fixed

- Planner now updates ROADMAP.md placeholders after planning completes

## [1.5.21](https://github.com/glittercowboy/sdd/releases/tag/v1.5.21) - 2026-01-16

### Added

- SDD brand system for consistent UI (checkpoint boxes, stage banners, status symbols)
- Research synthesizer agent that consolidates parallel research into SUMMARY.md

### Changed

- **Unified `/sdd:new-project` flow** — Single command now handles questions → research → requirements → roadmap (~10 min)
- Simplified README to reflect streamlined workflow: new-project → plan-phase → execute-phase
- Added optional `/sdd:discuss-phase` documentation for UI/UX/behavior decisions before planning
...
## [1.5.20](https://github.com/glittercowboy/sdd/releases/tag/v1.5.20) - 2026-01-16
...
## [1.5.19](https://github.com/glittercowboy/sdd/releases/tag/v1.5.19) - 2026-01-16

### Changed

- `/sdd:discuss-phase` redesigned with intelligent gray area analysis — analyzes phase to identify discussable areas (UI, UX, Behavior, etc.), presents multi-select for user control, deep-dives each area with focused questioning
...
- `/sdd:plan-phase` now integrates research — spawns `sdd-phase-researcher` before planning unless research exists or `--skip-research` flag used

## [1.5.18](https://github.com/glittercowboy/sdd/releases/tag/v1.5.18) - 2026-01-16

### Added

- **Plan verification loop** — Plans are now verified before execution with a planner → checker → revise cycle
  - New `sdd-plan-checker` agent (744 lines) validates plans will achieve phase goals
...
- **Dedicated planner agent** — `sdd-planner` (1,319 lines) consolidates all planning expertise
...
- `/sdd:plan-phase` refactored to thin orchestrator pattern (310 lines)
  - Spawns `sdd-planner` for planning, `sdd-plan-checker` for verification
  - User sees status between agent spawns (not a black box)
- Planning references deprecated with redirects to `sdd-planner` agent sections
  - `plan-format.md`, `scope-estimation.md`, `goal-backward.md`, `principles.md`
  - `workflows/plan-phase.md`

### Fixed

- Removed zombie `sdd-milestone-auditor` agent (was accidentally re-added after correct deletion)

### Removed

- Phase 99 throwaway test files

## [1.5.17](https://github.com/glittercowboy/sdd/releases/tag/v1.5.17) - 2026-01-15

### Added

- New `/sdd:update` command — check for updates, install, and display changelog of what changed (better UX than raw `npx @bhargavvc/sdd-cc`)

## [1.5.16](https://github.com/glittercowboy/sdd/releases/tag/v1.5.16) - 2026-01-15

### Added

- New `sdd-researcher` agent (915 lines) with comprehensive research methodology, 4 research modes (ecosystem, feasibility, implementation, comparison), source hierarchy, and verification protocols
- New `sdd-debugger` agent (990 lines) with scientific debugging methodology, hypothesis testing, and 7+ investigation techniques
- New `sdd-codebase-mapper` agent for brownfield codebase analysis
- Research subagent prompt template for context-only spawning

### Changed

- `/sdd:research-phase` refactored to thin orchestrator — now injects rich context (key insight framing, downstream consumer info, quality gates) to sdd-researcher agent
- `/sdd:research-project` refactored to spawn 4 parallel sdd-researcher agents with milestone-aware context (greenfield vs v1.1+) and roadmap implications guidance
- `/sdd:debug` refactored to thin orchestrator (149 lines) — spawns sdd-debugger agent with full debugging expertise
- `/sdd:new-milestone` now explicitly references MILESTONE-CONTEXT.md

### Deprecated

- `workflows/research-phase.md` — consolidated into sdd-researcher agent
- `workflows/research-project.md` — consolidated into sdd-researcher agent
- `workflows/debug.md` — consolidated into sdd-debugger agent
- `references/research-pitfalls.md` — consolidated into sdd-researcher agent
- `references/debugging.md` — consolidated into sdd-debugger agent
- `references/debug-investigation.md` — consolidated into sdd-debugger agent

## [1.5.15](https://github.com/glittercowboy/sdd/releases/tag/v1.5.15) - 2025-01-15

### Fixed

- **Agents now install correctly** — The `agents/` folder (sdd-executor, sdd-verifier, sdd-integration-checker, sdd-milestone-auditor) was missing from npm package, now included

### Changed

- Consolidated `/sdd:plan-fix` into `/sdd:plan-phase --gaps` for simpler workflow
- UAT file writes now batched instead of per-response for better performance

## [1.5.14](https://github.com/glittercowboy/sdd/releases/tag/v1.5.14) - 2025-01-15

### Fixed

- Plan-phase now always routes to `/sdd:execute-phase` after planning, even for single-plan phases

## [1.5.13](https://github.com/glittercowboy/sdd/releases/tag/v1.5.13) - 2026-01-15

### Fixed

- `/sdd:new-milestone` now presents research and requirements paths as equal options, matching `/sdd:new-project` format

## [1.5.12](https://github.com/glittercowboy/sdd/releases/tag/v1.5.12) - 2025-01-15
...
- `progress` now correctly routes to `/sdd:discuss-milestone` when between milestones (Route F)

## [1.5.11](https://github.com/glittercowboy/sdd/releases/tag/v1.5.11) - 2025-01-15
...
## [1.5.10](https://github.com/glittercowboy/sdd/releases/tag/v1.5.10) - 2025-01-15
...
## [1.5.9](https://github.com/glittercowboy/sdd/releases/tag/v1.5.9) - 2025-01-15

### Added

- Milestone audit system (`/sdd:audit-milestone`) for verifying milestone completion with parallel verification agents
...
- Execute-phase now recommends `/sdd:audit-milestone` when milestone completes
...
## [1.5.8](https://github.com/glittercowboy/sdd/releases/tag/v1.5.8) - 2025-01-15
...
- `sdd-executor` subagent color changed from red to blue

## [1.5.7](https://github.com/glittercowboy/sdd/releases/tag/v1.5.7) - 2025-01-15

### Added

- `sdd-executor` subagent: Dedicated agent for plan execution with full workflow logic built-in
- `sdd-verifier` subagent: Goal-backward verification that checks if phase goals are actually achieved (not just tasks completed)
...
- execute-plan and execute-phase now spawn `sdd-executor` subagent instead of using inline workflow
...
- Updated remaining `general-purpose` subagent references to use `sdd-executor`

## [1.5.6](https://github.com/glittercowboy/sdd/releases/tag/v1.5.6) - 2025-01-15
...
## [1.5.5](https://github.com/glittercowboy/sdd/releases/tag/v1.5.5) - 2025-01-15
...
## [1.5.4](https://github.com/glittercowboy/sdd/releases/tag/v1.5.4) - 2025-01-15
...
## [1.5.3](https://github.com/glittercowboy/sdd/releases/tag/v1.5.3) - 2025-01-15
...
- Dead `/sdd:status` command (referenced abandoned background agent model)
- Unused `agent-history.md` template
- `_archive/` directory with old execute-phase version

## [1.5.2](https://github.com/glittercowboy/sdd/releases/tag/v1.5.2) - 2026-01-15
...
## [1.5.1](https://github.com/glittercowboy/sdd/releases/tag/v1.5.1) - 2026-01-14
...
## [1.5.0](https://github.com/glittercowboy/sdd/releases/tag/v1.5.0) - 2026-01-14

### Added

- New `/sdd:research-project` command for pre-roadmap ecosystem research — spawns parallel agents to investigate stack, features, architecture, and pitfalls before you commit to a roadmap
- New `/sdd:define-requirements` command for scoping v1 requirements from research findings — transforms "what exists in this domain" into "what we're building"
...
## [1.4.29](https://github.com/glittercowboy/sdd/releases/tag/v1.4.29) - 2026-01-14

### Removed

- Deleted obsolete `_archive/execute-phase.md` and `status.md` commands

## [1.4.28](https://github.com/glittercowboy/sdd/releases/tag/v1.4.28) - 2026-01-14
...
## [1.4.27](https://github.com/glittercowboy/sdd/releases/tag/v1.4.27) - 2025-01-14
...
## [1.4.26](https://github.com/glittercowboy/sdd/releases/tag/v1.4.26) - 2026-01-14

### Added

- Full changelog history backfilled from git (66 historical versions from 1.0.0 to 1.4.23)

## [1.4.25](https://github.com/glittercowboy/sdd/releases/tag/v1.4.25) - 2026-01-14

### Added

- New `/sdd:whats-new` command shows changes since your installed version
- VERSION file written during installation for version tracking
- CHANGELOG.md now included in package installation

## [1.4.24](https://github.com/glittercowboy/sdd/releases/tag/v1.4.24) - 2026-01-14
...
## [1.4.23](https://github.com/glittercowboy/sdd/releases/tag/v1.4.23) - 2026-01-14

### Changed

- Removed dead ISSUES.md system code

## [1.4.22](https://github.com/glittercowboy/sdd/releases/tag/v1.4.22) - 2026-01-14
...
## [1.4.21](https://github.com/glittercowboy/sdd/releases/tag/v1.4.21) - 2026-01-14

### Fixed

- SlashCommand tool added to plan-fix allowed-tools

## [1.4.20](https://github.com/glittercowboy/sdd/releases/tag/v1.4.20) - 2026-01-14
...
## [1.4.19](https://github.com/glittercowboy/sdd/releases/tag/v1.4.19) - 2026-01-14

### Fixed

- Auto-diagnose issues instead of offering choice in plan-fix

## [1.4.18](https://github.com/glittercowboy/sdd/releases/tag/v1.4.18) - 2026-01-14

### Added

- Parallel diagnosis before plan-fix execution

## [1.4.17](https://github.com/glittercowboy/sdd/releases/tag/v1.4.17) - 2026-01-14

### Changed

- Redesigned verify-work as conversational UAT with persistent state

## [1.4.16](https://github.com/glittercowboy/sdd/releases/tag/v1.4.16) - 2026-01-13
...
## [1.4.15](https://github.com/glittercowboy/sdd/releases/tag/v1.4.15) - 2026-01-13

### Added

- Context rot explanation to README header

## [1.4.14](https://github.com/glittercowboy/sdd/releases/tag/v1.4.14) - 2026-01-13

### Changed

- YOLO mode is now recommended default in new-project

## [1.4.13](https://github.com/glittercowboy/sdd/releases/tag/v1.4.13) - 2026-01-13
...
## [1.4.12](https://github.com/glittercowboy/sdd/releases/tag/v1.4.12) - 2026-01-13

### Changed

- execute-phase is now recommended as primary execution command

## [1.4.11](https://github.com/glittercowboy/sdd/releases/tag/v1.4.11) - 2026-01-13

### Fixed

- Checkpoints now use fresh continuation agents instead of resume

## [1.4.10](https://github.com/glittercowboy/sdd/releases/tag/v1.4.10) - 2026-01-13

### Changed

- execute-plan converted to orchestrator pattern for performance

## [1.4.9](https://github.com/glittercowboy/sdd/releases/tag/v1.4.9) - 2026-01-13
...
## [1.4.8](https://github.com/glittercowboy/sdd/releases/tag/v1.4.8) - 2026-01-13

### Added

- TDD reasoning explanation restored to plan-phase docs

## [1.4.7](https://github.com/glittercowboy/sdd/releases/tag/v1.4.7) - 2026-01-13
...
## [1.4.6](https://github.com/glittercowboy/sdd/releases/tag/v1.4.6) - 2026-01-13
...
## [1.4.5](https://github.com/glittercowboy/sdd/releases/tag/v1.4.5) - 2026-01-13
...
## [1.4.4](https://github.com/glittercowboy/sdd/releases/tag/v1.4.4) - 2026-01-13

### Fixed

- Inline listing for multiple active debug sessions

## [1.4.3](https://github.com/glittercowboy/sdd/releases/tag/v1.4.3) - 2026-01-13

### Added

- `/sdd:debug` command for systematic debugging with persistent state

## [1.4.2](https://github.com/glittercowboy/sdd/releases/tag/v1.4.2) - 2026-01-13

### Fixed

- Installation verification step clarification

## [1.4.1](https://github.com/glittercowboy/sdd/releases/tag/v1.4.1) - 2026-01-13

### Added

- Parallel phase execution via `/sdd:execute-phase`
- Parallel-aware planning in `/sdd:plan-phase`
- `/sdd:status` command for parallel agent monitoring
...
## [1.4.0](https://github.com/glittercowboy/sdd/releases/tag/v1.4.0) - 2026-01-12
...
## [1.3.34](https://github.com/glittercowboy/sdd/releases/tag/v1.3.34) - 2026-01-11

### Added

- `/sdd:add-todo` and `/sdd:check-todos` for mid-session idea capture

## [1.3.33](https://github.com/glittercowboy/sdd/releases/tag/v1.3.33) - 2026-01-11
...
## [1.3.32](https://github.com/glittercowboy/sdd/releases/tag/v1.3.32) - 2026-01-10

### Added

- `/sdd:resume-task` for resuming interrupted subagent executions

## [1.3.31](https://github.com/glittercowboy/sdd/releases/tag/v1.3.31) - 2026-01-08
...
## [1.3.30](https://github.com/glittercowboy/sdd/releases/tag/v1.3.30) - 2026-01-08

### Added

- verify-work option surfaces after plan execution

## [1.3.29](https://github.com/glittercowboy/sdd/releases/tag/v1.3.29) - 2026-01-08

### Added

- `/sdd:verify-work` for conversational UAT validation
- `/sdd:plan-fix` for fixing UAT issues
- UAT issues template

## [1.3.28](https://github.com/glittercowboy/sdd/releases/tag/v1.3.28) - 2026-01-07

### Added

- `--config-dir` CLI argument for multi-account setups
- `/sdd:remove-phase` command

### Fixed

- Validation for --config-dir edge cases

## [1.3.27](https://github.com/glittercowboy/sdd/releases/tag/v1.3.27) - 2026-01-07
...
## [1.3.26](https://github.com/glittercowboy/sdd/releases/tag/v1.3.26) - 2026-01-06
...
## [1.3.25](https://github.com/glittercowboy/sdd/releases/tag/v1.3.25) - 2026-01-06

### Fixed

- Milestone discussion context persists across /clear

## [1.3.24](https://github.com/glittercowboy/sdd/releases/tag/v1.3.24) - 2026-01-06

### Added

- `CLAUDE_CONFIG_DIR` environment variable support

## [1.3.23](https://github.com/glittercowboy/sdd/releases/tag/v1.3.23) - 2026-01-06

### Added

- Non-interactive install flags (`--global`, `--local`) for Docker/CI

## [1.3.22](https://github.com/glittercowboy/sdd/releases/tag/v1.3.22) - 2026-01-05

### Changed

- Removed unused auto.md command

## [1.3.21](https://github.com/glittercowboy/sdd/releases/tag/v1.3.21) - 2026-01-05

### Changed

- TDD features use dedicated plans for full context quality

## [1.3.20](https://github.com/glittercowboy/sdd/releases/tag/v1.3.20) - 2026-01-05

### Added

- Per-task atomic commits for better AI observability

## [1.3.19](https://github.com/glittercowboy/sdd/releases/tag/v1.3.19) - 2026-01-05

### Fixed

- Clarified create-milestone.md file locations with explicit instructions

## [1.3.18](https://github.com/glittercowboy/sdd/releases/tag/v1.3.18) - 2026-01-05
...
## [1.3.17](https://github.com/glittercowboy/sdd/releases/tag/v1.3.17) - 2026-01-04

### Fixed

- Clarified depth controls compression, not inflation in planning

## [1.3.16](https://github.com/glittercowboy/sdd/releases/tag/v1.3.16) - 2026-01-04

### Added

- Depth parameter for planning thoroughness (`--depth=1-5`)

## [1.3.15](https://github.com/glittercowboy/sdd/releases/tag/v1.3.15) - 2026-01-01

### Fixed

- TDD reference loaded directly in commands

## [1.3.14](https://github.com/glittercowboy/sdd/releases/tag/v1.3.14) - 2025-12-31

### Added

- TDD integration with detection, annotation, and execution flow

## [1.3.13](https://github.com/glittercowboy/sdd/releases/tag/v1.3.13) - 2025-12-29
...
## [1.3.12](https://github.com/glittercowboy/sdd/releases/tag/v1.3.12) - 2025-12-29

### Fixed

- Restored plan-format.md as output template

## [1.3.11](https://github.com/glittercowboy/sdd/releases/tag/v1.3.11) - 2025-12-29
...
## [1.3.10](https://github.com/glittercowboy/sdd/releases/tag/v1.3.10) - 2025-12-29

### Fixed

- Explicit plan count check in offer_next step

## [1.3.9](https://github.com/glittercowboy/sdd/releases/tag/v1.3.9) - 2025-12-27

### Added

- Evolutionary PROJECT.md system with incremental updates

## [1.3.8](https://github.com/glittercowboy/sdd/releases/tag/v1.3.8) - 2025-12-18

### Added

- Brownfield/existing projects section in README

## [1.3.7](https://github.com/glittercowboy/sdd/releases/tag/v1.3.7) - 2025-12-18

### Fixed

- Improved incremental codebase map updates

## [1.3.6](https://github.com/glittercowboy/sdd/releases/tag/v1.3.6) - 2025-12-18

### Added

- File paths included in codebase mapping output

## [1.3.5](https://github.com/glittercowboy/sdd/releases/tag/v1.3.5) - 2025-12-17

### Fixed

- Removed arbitrary 100-line limit from codebase mapping

## [1.3.4](https://github.com/glittercowboy/sdd/releases/tag/v1.3.4) - 2025-12-17

### Fixed

- Inline code for Next Up commands (avoids nesting ambiguity)

## [1.3.3](https://github.com/glittercowboy/sdd/releases/tag/v1.3.3) - 2025-12-17

### Fixed

- Check PROJECT.md not .planning/ directory for existing project detection

## [1.3.2](https://github.com/glittercowboy/sdd/releases/tag/v1.3.2) - 2025-12-17

### Added

- Git commit step to map-codebase workflow

## [1.3.1](https://github.com/glittercowboy/sdd/releases/tag/v1.3.1) - 2025-12-17

### Added

- `/sdd:map-codebase` documentation in help and README

## [1.3.0](https://github.com/glittercowboy/sdd/releases/tag/v1.3.0) - 2025-12-17

### Added

- `/sdd:map-codebase` command for brownfield project analysis
...
- Brownfield integration into SDD workflows
...
## [1.2.13](https://github.com/glittercowboy/sdd/releases/tag/v1.2.13) - 2025-12-17

### Added

- Improved continuation UI with context and visual hierarchy

## [1.2.12](https://github.com/glittercowboy/sdd/releases/tag/v1.2.12) - 2025-12-17

### Fixed

- First question should be freeform, not AskUserQuestion

## [1.2.11](https://github.com/glittercowboy/sdd/releases/tag/v1.2.11) - 2025-12-17

### Fixed

- Permission errors for non-DSP users (removed shell context)

## [1.2.10](https://github.com/glittercowboy/sdd/releases/tag/v1.2.10) - 2025-12-16

### Fixed

- Inline command invocation replaced with clear-then-paste pattern

## [1.2.9](https://github.com/glittercowboy/sdd/releases/tag/v1.2.9) - 2025-12-16

### Fixed

- Git init runs in current directory

## [1.2.8](https://github.com/glittercowboy/sdd/releases/tag/v1.2.8) - 2025-12-16

### Changed

- Phase count derived from work scope, not arbitrary limits

## [1.2.7](https://github.com/glittercowboy/sdd/releases/tag/v1.2.7) - 2025-12-16

### Fixed

- AskUserQuestion mandated for all exploration questions

## [1.2.6](https://github.com/glittercowboy/sdd/releases/tag/v1.2.6) - 2025-12-16

### Changed

- Internal refactoring

## [1.2.5](https://github.com/glittercowboy/sdd/releases/tag/v1.2.5) - 2025-12-16

### Changed

- `<if mode>` tags for yolo/interactive branching

## [1.2.4](https://github.com/glittercowboy/sdd/releases/tag/v1.2.4) - 2025-12-16

### Fixed

- Stale CONTEXT.md references updated to new vision structure

## [1.2.3](https://github.com/glittercowboy/sdd/releases/tag/v1.2.3) - 2025-12-16

### Fixed

- Enterprise language removed from help and discuss-milestone

## [1.2.2](https://github.com/glittercowboy/sdd/releases/tag/v1.2.2) - 2025-12-16

### Fixed

- new-project completion presented inline instead of as question

## [1.2.1](https://github.com/glittercowboy/sdd/releases/tag/v1.2.1) - 2025-12-16

### Fixed

- AskUserQuestion restored for decision gate in questioning flow

## [1.2.0](https://github.com/glittercowboy/sdd/releases/tag/v1.2.0) - 2025-12-15

### Changed

- Research workflow implemented as Claude Code context injection

## [1.1.2](https://github.com/glittercowboy/sdd/releases/tag/v1.1.2) - 2025-12-15

### Fixed

- YOLO mode now skips confirmation gates in plan-phase

## [1.1.1](https://github.com/glittercowboy/sdd/releases/tag/v1.1.1) - 2025-12-15

### Added

- README documentation for new research workflow

## [1.1.0](https://github.com/glittercowboy/sdd/releases/tag/v1.1.0) - 2025-12-15

### Added

- Pre-roadmap research workflow
- `/sdd:research-phase` for niche domain ecosystem discovery
- `/sdd:research-project` command with workflow and templates
- `/sdd:create-roadmap` command with research-aware workflow
...
## [1.0.11](https://github.com/glittercowboy/sdd/releases/tag/v1.0.11) - 2025-12-15

### Added

- `/sdd:research-phase` for niche domain ecosystem discovery

## [1.0.10](https://github.com/glittercowboy/sdd/releases/tag/v1.0.10) - 2025-12-15

### Fixed

- Scope creep prevention in discuss-phase command

## [1.0.9](https://github.com/glittercowboy/sdd/releases/tag/v1.0.9) - 2025-12-15

### Added

- Phase CONTEXT.md loaded in plan-phase command

## [1.0.8](https://github.com/glittercowboy/sdd/releases/tag/v1.0.8) - 2025-12-15

### Changed

- PLAN.md included in phase completion commits

## [1.0.7](https://github.com/glittercowboy/sdd/releases/tag/v1.0.7) - 2025-12-15

### Added

- Path replacement for local installs

## [1.0.6](https://github.com/glittercowboy/sdd/releases/tag/v1.0.6) - 2025-12-15

### Changed

- Internal improvements

## [1.0.5](https://github.com/glittercowboy/sdd/releases/tag/v1.0.5) - 2025-12-15
...
## [1.0.4](https://github.com/glittercowboy/sdd/releases/tag/v1.0.4) - 2025-12-15

### Fixed

- Bin name and circular dependency removed

## [1.0.3](https://github.com/glittercowboy/sdd/releases/tag/v1.0.3) - 2025-12-15

### Added

- TDD guidance in planning workflow

## [1.0.2](https://github.com/glittercowboy/sdd/releases/tag/v1.0.2) - 2025-12-15

### Added

- Issue triage system to prevent deferred issue pile-up

## [1.0.1](https://github.com/glittercowboy/sdd/releases/tag/v1.0.1) - 2025-12-15

### Added

- Initial npm package release

## [1.0.0](https://github.com/glittercowboy/sdd/releases/tag/v1.0.0) - 2025-12-14

### Added

- Initial release of SDD (Spec-Driven Development) meta-prompting system
- Core slash commands: `/sdd:new-project`, `/sdd:discuss-phase`, `/sdd:plan-phase`, `/sdd:execute-phase`
...
[Unreleased]: https://github.com/bhargavvc/sdd-cc/compare/v1.42.1...HEAD
[1.42.1]: https://github.com/bhargavvc/sdd-cc/compare/v1.41.0...v1.42.1
[1.38.4]: https://github.com/bhargavvc/sdd-cc/compare/v1.38.2...v1.38.4
[1.38.2]: https://github.com/bhargavvc/sdd-cc/compare/v1.37.1...v1.38.2
[1.37.1]: https://github.com/bhargavvc/sdd-cc/compare/v1.37.0...v1.37.1
[1.37.0]: https://github.com/bhargavvc/sdd-cc/compare/v1.36.0...v1.37.0
[1.36.0]: https://github.com/bhargavvc/sdd-cc/releases/tag/v1.36.0
[1.35.0]: https://github.com/bhargavvc/sdd-cc/releases/tag/v1.35.0
