...
```bash
# Claude Code
npx @bhargavvc/sdd-cc --claude --global   # ~/.claude/ にインストール
npx @bhargavvc/sdd-cc --claude --local    # ./.claude/ にインストール

# OpenCode
npx @bhargavvc/sdd-cc --opencode --global # ~/.config/opencode/ にインストール

# Gemini CLI
npx @bhargavvc/sdd-cc --gemini --global   # ~/.gemini/ にインストール

# Kilo
npx @bhargavvc/sdd-cc --kilo --global     # ~/.config/kilo/ にインストール
npx @bhargavvc/sdd-cc --kilo --local      # ./.kilo/ にインストール

# Codex
npx @bhargavvc/sdd-cc --codex --global    # ~/.codex/ にインストール
npx @bhargavvc/sdd-cc --codex --local     # ./.codex/ にインストール

# Copilot
npx @bhargavvc/sdd-cc --copilot --global  # ~/.github/ にインストール
npx @bhargavvc/sdd-cc --copilot --local   # ./.github/ にインストール

# Cursor CLI
npx @bhargavvc/sdd-cc --cursor --global      # ~/.cursor/ にインストール
npx @bhargavvc/sdd-cc --cursor --local       # ./.cursor/ にインストール

# Antigravity
npx @bhargavvc/sdd-cc --antigravity --global # ~/.gemini/antigravity/ にインストール
npx @bhargavvc/sdd-cc --antigravity --local  # ./.agent/ にインストール

# Augment
npx @bhargavvc/sdd-cc --augment --global     # ~/.augment/ にインストール
npx @bhargavvc/sdd-cc --augment --local      # ./.augment/ にインストール

# Trae
npx @bhargavvc/sdd-cc --trae --global        # ~/.trae/ にインストール
npx @bhargavvc/sdd-cc --trae --local         # ./.trae/ にインストール

# Cline
npx @bhargavvc/sdd-cc --cline --global       # ~/.cline/ にインストール
npx @bhargavvc/sdd-cc --cline --local        # ./.clinerules にインストール

# 全ランタイム
npx @bhargavvc/sdd-cc --all --global      # すべてのディレクトリにインストール
```
...
> **既存のコードがある場合は？** まず `/sdd-map-codebase` を実行してください。並列エージェントが起動し、スタック、アーキテクチャ、規約、懸念点を分析します。その後 `/sdd-new-project` がコードベースを把握した状態で動作し、質問は追加する内容に焦点を当て、計画時にはパターンが自動的に読み込まれます。
...
> **前提モード：** 質問よりもコードベース分析を優先したい場合は、`/sdd-settings` で `workflow.discuss_mode` を `assumptions` に設定してください。システムがコードを読み、何をなぜそうするかを提示し、間違っている部分だけ修正を求めます。詳しくは[ディスカスモード](docs/ja-JP/workflow-discuss-mode.md)をご覧ください。
...
すべてパスすれば次に進みます。何か壊れていれば、手動でデバッグする必要はありません — 作成された修正プランで `/sdd-execute-phase` を再度実行するだけです。
...
ディスカッション中のインプットを速くしたい場合は、`/sdd-discuss-phase <n> --batch` で1つずつではなく小さなグループにまとめた質問に一括で回答できます。`--chain` を使うと、ディスカッションからプラン+実行まで途中で止まらずに自動チェインできます。

各フェーズであなたのインプット（discuss）、適切なリサーチ（plan）、クリーンな実行（execute）、人間による検証（verify）が行われます。コンテキストは常にフレッシュ。品質は常に高い。

すべてのフェーズが完了したら、`/sdd-complete-milestone` でマイルストーンをアーカイブしリリースをタグ付けします。

次に `/sdd-new-milestone` で次のバージョンを開始します — `new-project` と同じフローですが既存のコードベース向けです。次に構築したいものを説明し、システムがドメインを調査し、要件をスコーピングし、新しいロードマップを作成します。各マイルストーンはクリーンなサイクルです：定義 → 構築 → シップ。
...
| コマンド | 説明 |
|---------|--------------|
| `/sdd-new-project [--auto]` | フル初期化：質問 → リサーチ → 要件定義 → ロードマップ |
| `/sdd-discuss-phase [N] [--auto] [--analyze] [--chain]` | 計画前に実装の決定事項をキャプチャ（`--analyze` でトレードオフ分析を追加、`--chain` でプラン+実行へ自動チェイン） |
| `/sdd-plan-phase [N] [--auto] [--reviews]` | フェーズのリサーチ + プラン + 検証（`--reviews` でコードベースレビューの発見事項を読み込み） |
...
| `/sdd-new-milestone [name]` | 次のバージョンを開始：質問 → リサーチ → 要件定義 → ロードマップ |
| `/sdd-forensics [desc]` | 失敗したワークフロー実行の事後分析（停止ループ、欠落成果物、git異常の診断） |
| `/sdd-milestone-summary [version]` | チームオンボーディングとレビュー向けの包括的なプロジェクトサマリーを生成 |

### ワークストリーム

| コマンド | 説明 |
|---------|--------------|
| `/sdd-workstreams list` | 全ワークストリームとそのステータスを表示 |
| `/sdd-workstreams create <name>` | 並列マイルストーン作業用の名前空間付きワークストリームを作成 |
| `/sdd-workstreams switch <name>` | アクティブなワークストリームを切り替え |
| `/sdd-workstreams complete <name>` | ワークストリームを完了しマージ |
...
| `/sdd-list-workspaces` | すべてのSDDワークスペースとそのステータスを表示 |
...
| コマンド | 説明 |
|---------|--------------|
| `/sdd-ui-phase [N]` | フロントエンドフェーズ用のUIデザイン契約（UI-SPEC.md）を生成 |
| `/sdd-ui-review [N]` | 実装済みフロントエンドコードの6つの柱によるビジュアル監査（遡及的） |
...
| コマンド | 説明 |
|---------|--------------|
| `/sdd-map-codebase [area]` | new-project前に既存のコードベースを分析 |
...
| コマンド | 説明 |
|---------|--------------|
| `/sdd-workstreams` | 並列ワークストリームを管理（list、create、switch、status、progress、complete） |

### コード品質

| コマンド | 説明 |
|---------|--------------|
| `/sdd-review` | 現在のフェーズまたはブランチのクロスAIピアレビュー |
| `/sdd-pr-branch` | `.planning/` コミットをフィルタリングしたクリーンなPRブランチを作成 |
| `/sdd-audit-uat` | 検証負債を監査 — UATが未実施のフェーズを検出 |
...
これらのトグルには `/sdd-settings` を使用するか、呼び出し時にオーバーライドできます：
...
| `git.phase_branch_template` | string | `sdd/phase-{phase}-{slug}` | フェーズブランチのテンプレート |
...
- **PreToolUseプロンプトガードフック** — `sdd-prompt-guard` が `.planning/` への書き込みに埋め込まれたインジェクションベクトルをスキャン（アドバイザリー、ブロッキングではない）
...
- `~/.claude/commands/sdd/`（グローバル）または `./.claude/commands/sdd/`（ローカル）にファイルが存在するか確認してください
- Codexの場合、`~/.codex/skills/sdd-*/SKILL.md`（グローバル）または `./.codex/skills/sdd-*/SKILL.md`（ローカル）にスキルが存在するか確認してください

**コマンドが期待通りに動作しない？**
- `/sdd-help` を実行してインストールを確認してください
- `npx @bhargavvc/sdd-cc` を再実行して再インストールしてください
...
OpenCode、Gemini CLI、Kilo、Codexは `npx @bhargavvc/sdd-cc` でネイティブサポートされています。

以下のコミュニティポートがマルチランタイムサポートの先駆けとなりました：

| プロジェクト | プラットフォーム | 説明 |
|---------|----------|-------------|
| [sdd-opencode](https://github.com/rokicool/sdd-opencode) | OpenCode | オリジナルのOpenCode対応版 |
| sdd-gemini（アーカイブ済み） | Gemini CLI | uberfuzzyによるオリジナルのGemini対応版 |
