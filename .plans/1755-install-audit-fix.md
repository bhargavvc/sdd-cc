...
**Line 5485** — Change `sdd-update-check.js` to `sdd-check-update.js` and fix path from `sdd/hooks/` to `hooks/`.
**Line 5492** — Update dedup check to use `sdd-check-update`.

### Fix 3: Fix stale cache invalidation path (CRITICAL)
**Line 5406** — Change from `path.join(path.dirname(targetDir), 'cache', ...)` to `path.join(os.homedir(), '.cache', 'sdd', 'sdd-update-check.json')`.
...
- uninstall removes sdd-workflow-guard.js
