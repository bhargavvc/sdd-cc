...
SDD 解决的就是这个问题。它是让 Claude Code 变得可靠的上下文工程层。你只要描述想法，系统会自动提取它需要知道的一切，然后让 Claude Code 去干活。
...
> [!NOTE]
> Claude Code 2.1.88+ 和 Codex 以 skill 形式安装（`skills/sdd-*/SKILL.md`）。Cline 使用 `.clinerules`。安装器会自动处理所有格式。
...
```bash
# Claude Code
npx @bhargavvc/sdd-cc --claude --global   # 安装到 ~/.claude/
npx @bhargavvc/sdd-cc --claude --local    # 安装到 ./.claude/

# OpenCode
npx @bhargavvc/sdd-cc --opencode --global # 安装到 ~/.config/opencode/

# Gemini CLI
npx @bhargavvc/sdd-cc --gemini --global   # 安装到 ~/.gemini/

# Kilo
npx @bhargavvc/sdd-cc --kilo --global     # 安装到 ~/.config/kilo/
npx @bhargavvc/sdd-cc --kilo --local      # 安装到 ./.kilo/

# Codex
npx @bhargavvc/sdd-cc --codex --global    # 安装到 ~/.codex/
npx @bhargavvc/sdd-cc --codex --local     # 安装到 ./.codex/

# Copilot
npx @bhargavvc/sdd-cc --copilot --global  # 安装到 ~/.github/
npx @bhargavvc/sdd-cc --copilot --local   # 安装到 ./.github/

# Cursor CLI
npx @bhargavvc/sdd-cc --cursor --global   # 安装到 ~/.cursor/
npx @bhargavvc/sdd-cc --cursor --local    # 安装到 ./.cursor/

# Antigravity
npx @bhargavvc/sdd-cc --antigravity --global # 安装到 ~/.gemini/antigravity/
npx @bhargavvc/sdd-cc --antigravity --local  # 安装到 ./.agent/

# Augment
npx @bhargavvc/sdd-cc --augment --global     # 安装到 ~/.augment/
npx @bhargavvc/sdd-cc --augment --local      # 安装到 ./.augment/
...
# 所有运行时
npx @bhargavvc/sdd-cc --all --global      # 安装到所有目录
```
...
SDD 的设计目标是无摩擦自动化。运行 Claude Code 时建议使用：
...
> [!TIP]
> 这才是 SDD 的预期用法。连 `date` 和 `git commit` 都要来回确认 50 次，整个体验就废了。
...
> **已经有现成代码库？** 先运行 `/sdd-map-codebase`。它会并行拉起多个代理分析你的技术栈、架构、约定和风险点。之后 `/sdd-new-project` 就会真正“理解”你的代码库，提问会聚焦在你打算新增的部分，规划时也会自动加载你的现有模式。
...
如果一切通过，就进入下一步；如果哪里坏了，你不需要手动 debug，只要重新运行 `/sdd-execute-phase`，执行它自动生成的修复计划即可。
...
如果你希望在讨论阶段更快收集信息，可以用 `/sdd-discuss-phase <n> --batch`，一次回答一小组问题，而不是逐个问答。

每个阶段都会得到你的输入（discuss）、充分研究（plan）、干净执行（execute）和人工验证（verify）。上下文始终保持新鲜，质量也能持续稳定。

当所有阶段完成后，`/sdd-complete-milestone` 会归档当前里程碑并打 release tag。

接着用 `/sdd-new-milestone` 开启下一个版本。它和 `new-project` 流程相同，只是面向你现有的代码库。你描述下一步想构建什么，系统研究领域、梳理需求，再产出新的路线图。每个里程碑都是一个干净周期：定义 → 构建 → 发布。
...
快速模式保留 SDD 的核心保障（原子提交、状态跟踪），但路径更短：
...
| 命令 | 作用 |
|------|------|
| `/sdd-new-project [--auto]` | 完整初始化：提问 → 研究 → 需求 → 路线图 |
| `/sdd-discuss-phase [N] [--auto] [--analyze]` | 在规划前收集实现决策（`--analyze` 增加权衡分析） |
| `/sdd-plan-phase [N] [--auto] [--reviews]` | 为某个阶段执行研究 + 规划 + 验证（`--reviews` 加载代码库审查结果） |
| `/sdd-execute-phase <N>` | 以并行 wave 执行全部计划，完成后验证 |
...
| `/sdd-milestone-summary` | 从已完成的里程碑产物生成项目概览，用于团队上手 |
| `/sdd-forensics` | 对失败或卡住的工作流进行事后调查 |

### 工作流（Workstreams）

| 命令 | 作用 |
|------|------|
| `/sdd-workstreams list` | 显示所有工作流及其状态 |
| `/sdd-workstreams create <name>` | 创建命名空间工作流，用于并行里程碑工作 |
| `/sdd-workstreams switch <name>` | 切换当前活跃工作流 |
| `/sdd-workstreams complete <name>` | 完成并合并工作流 |
...
| 命令 | 作用 |
|------|------|
| `/sdd-ui-phase [N]` | 为前端阶段生成 UI 设计合约（UI-SPEC.md） |
| `/sdd-ui-review [N]` | 对已实现前端代码进行 6 维视觉审计 |
...
| 命令 | 作用 |
|------|------|
| `/sdd-map-codebase` | 在 `new-project` 前分析现有代码库 |
...
| 命令 | 作用 |
|------|------|
| `/sdd-review` | 对当前阶段或分支进行跨 AI 同行评审 |
| `/sdd-pr-branch` | 创建过滤 `.planning/` 提交的干净 PR 分支 |
| `/sdd-audit-uat` | 审计验证债务——找出缺少 UAT 的阶段 |
...
| `/sdd-quick [--full] [--discuss] [--research]` | 以 SDD 保障执行临时任务（`--full` 增加计划检查和验证，`--discuss` 先补上下文，`--research` 在规划前先调研） |
| `/sdd-health [--repair]` | 校验 `.planning/` 目录完整性，带 `--repair` 时自动修复 |
| `/sdd-stats` | 显示项目统计——阶段、计划、需求、git 指标 |
| `/sdd-profile-user [--questionnaire] [--refresh]` | 从会话分析生成开发者行为档案，用于个性化响应 |
...
SDD 将项目设置保存在 `.planning/config.json`。你可以在 `/sdd-new-project` 时配置，也可以稍后通过 `/sdd-settings` 修改。完整的配置 schema、工作流开关、git branching 选项以及各代理的模型分配，请查看[用户指南](docs/USER-GUIDE.md#configuration-reference)。
...
可以用 `/sdd-settings` 开关这些项，也可以在单次命令里覆盖：
...
| `git.phase_branch_template` | string | `sdd/phase-{phase}-{slug}` | phase 分支模板 |
| `git.milestone_branch_template` | string | `sdd/{milestone}-{slug}` | milestone 分支模板 |

**策略说明：**
- **`none`**：直接提交到当前分支（SDD 默认行为）
- **`phase`**：每个 phase 创建一个分支，在 phase 完成时合并
- **`milestone`**：整个里程碑只用一个分支，在里程碑完成时合并

在里程碑完成时，SDD 会提供 squash merge（推荐）或保留历史的 merge 选项。
...
SDD 的代码库映射和分析命令会读取文件来理解你的项目。**包含机密信息的文件应当加入 Claude Code 的 deny list**：
...
> [!IMPORTANT]
> SDD 内建了防止提交 secrets 的保护，但纵深防御依然是最佳实践。第一道防线应该是直接禁止读取敏感文件。
...
- 检查文件是否存在于 `~/.claude/commands/sdd/`（全局）或 `./.claude/commands/sdd/`（本地）
- 对 Codex，检查 skills 是否存在于 `~/.codex/skills/sdd-*/SKILL.md`（全局）或 `./.codex/skills/sdd-*/SKILL.md`（本地）

**命令行为不符合预期？**
- 运行 `/sdd-help` 确认安装成功
- 重新执行 `npx @bhargavvc/sdd-cc` 进行重装
...
这会移除所有 SDD 命令、代理、hooks 和设置，但会保留你其他配置。

---

## 社区移植版本

OpenCode、Gemini CLI、Kilo 和 Codex 现在都已经通过 `npx @bhargavvc/sdd-cc` 获得原生支持。

这些社区移植版本曾率先探索多运行时支持：

| Project | Platform | Description |
|---------|----------|-------------|
| [sdd-opencode](https://github.com/rokicool/sdd-opencode) | OpenCode | 最初的 OpenCode 适配版本 |
| sdd-gemini (archived) | Gemini CLI | uberfuzzy 制作的最初 Gemini 适配版本 |
