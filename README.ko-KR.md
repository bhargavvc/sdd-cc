...
[![Discord](https://img.shields.io/badge/Discord-Join-5865F2?style=for-the-badge&logo=discord&logoColor=white)](https://discord.gg/sdd)
...
> [!NOTE]
> Codex 설치는 커스텀 프롬프트 대신 스킬(`skills/sdd-*/SKILL.md`)을 사용합니다.
...
논의 중에 더 빠르게 진행하고 싶다면 `/sdd:discuss-phase <n> --batch`를 사용해 하나씩이 아닌 소그룹으로 한 번에 답할 수 있습니다.
...
| `/sdd:discuss-phase [N] [--auto] [--analyze]` | 기획 전 구현 결정 캡처 (`--analyze`는 트레이드오프 분석 추가) |
...
| `/sdd:quick [--full] [--discuss] [--research]` | SDD 보장과 함께 임시 작업 실행 (`--full`은 계획 확인 및 검증 추가, `--discuss`는 먼저 컨텍스트 수집, `--research`는 기획 전 접근법 조사) |
...
OpenCode, Gemini CLI, Codex는 이제 `npx @bhargavvc/sdd-cc`를 통해 기본 지원됩니다.
