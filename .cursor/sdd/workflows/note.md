...
- **Global scope**: `C:/Users/DS_Chitteti Bhargav/OneDrive - Data Semantics Pvt Ltd/Work/SDD-Setup/sdd-cc/.cursor/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
**`--global` flag**: Strip `--global` from anywhere in `{{SDD_ARGS}}` before parsing. When present, force global scope regardless of whether `.planning/` exists.

**Important**: Do NOT create `.planning/` if it doesn't exist. Fall back to global scope silently.
</step>

<step name="parse_subcommand">
**Parse subcommand from {{SDD_ARGS}} (after stripping --global).**
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Users/DS_Chitteti Bhargav/OneDrive - Data Semantics Pvt Ltd/Work/SDD-Setup/sdd-cc/.cursor/notes/`)
...
2. Glob `C:/Users/DS_Chitteti Bhargav/OneDrive - Data Semantics Pvt Ltd/Work/SDD-Setup/sdd-cc/.cursor/notes/*.md` (if directory exists) — global notes
...
Global (C:/Users/DS_Chitteti Bhargav/OneDrive - Data Semantics Pvt Ltd/Work/SDD-Setup/sdd-cc/.cursor/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Users/DS_Chitteti Bhargav/OneDrive - Data Semantics Pvt Ltd/Work/SDD-Setup/sdd-cc/.cursor/notes/` — works in any directory
