...
## A. Skill Invocation
- This skill is invoked when the user mentions `sdd-reapply-patches` or describes a task matching this skill.
...
if [ -z "$PATCHES_DIR" ]; then
  if [ -d "$HOME/.config/kilo/sdd-local-patches" ]; then
    ...
    PATCHES_DIR="C:/Users/DS_Chitteti Bhargav/OneDrive - Data Semantics Pvt Ltd/Work/SDD-Setup/sdd-cc/.cursor/sdd-local-patches"
  fi
fi
...
b. Identify ALL differences, then classify each as:
   - **Mechanical drift** — path substitutions (e.g. `/Users/xxx/.claude/` → `C:/Users/DS_Chitteti Bhargav/OneDrive - Data Semantics Pvt Ltd/Work/SDD-Setup/sdd-cc/.cursor/`), variable additions (`${SDD_WS}`, `${AGENT_SKILLS_*}`), error handling additions (`|| true`)
