...
## A. Skill Invocation
- This skill is invoked when the user mentions `sdd-execute-phase` or describes a task matching this skill.
...
<execution_context>
@C:/Users/DS_Chitteti Bhargav/OneDrive - Data Semantics Pvt Ltd/Work/SDD-Setup/sdd-cc/.cursor/sdd/workflows/execute-phase.md
...
- `--wave N` is active only if the literal `--wave` token is present in `{{SDD_ARGS}}`
- `--gaps-only` is active only if the literal `--gaps-only` token is present in `{{SDD_ARGS}}`
- `--interactive` is active only if the literal `--interactive` token is present in `{{SDD_ARGS}}`
...
<process>
Execute the execute-phase workflow from @C:/Users/DS_Chitteti Bhargav/OneDrive - Data Semantics Pvt Ltd/Work/SDD-Setup/sdd-cc/.cursor/sdd/workflows/execute-phase.md end-to-end.
