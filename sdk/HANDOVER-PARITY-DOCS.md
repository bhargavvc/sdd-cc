...
1. **Parity “exceptions” documentation** — A clear, maintainable description of where **full JSON equality** between `sdd-tools.cjs` and `createRegistry()` is **not** expected or not attempted, and why (stubs, structural-only tests, environment-dependent fields, ordering, etc.). Map this to **#2007 / #2302** expectations: no *undocumented* gap.
2. **CJS-only matrix** — A **single authoritative table**: each relevant `sdd-tools.cjs` surface (top-level command or documented cluster) → **registered in SDK** vs **permanent CLI-only** vs **alias / naming difference**, with a **one-line justification** where not registered.

## Parent tracking

- **Issue:** [bhargavvc/sdd-cc#2302](https://github.com/bhargavvc/sdd-cc/issues/2302) — Phase 3 SDK query parity, registry, docs (parent umbrella #2007).
...
- **Workspace:** `D:\Repos\sdd` (PBR backport); adjust path if different machine.
- **Feature branch (typical):** `feat/sdk-phase3-query-layer` — confirm with `git branch` before editing.
- **Upstream:** `bhargavvc/sdd-cc`.
...
- Implementing runner alignment (`SDDTools` → registry) — separate #2302 work.
- Adding `@deprecated` headers to `sdd-tools.cjs` — separate task.
...
- A reader can answer: **“Which `sdd-tools` top-level commands are not registered and why?”** from one table.
