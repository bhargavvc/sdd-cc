...
- **Issue:** [bhargavvc/sdd-cc#2302](https://github.com/bhargavvc/sdd-cc/issues/2302) — Phase 3 SDK query parity, registry, docs (umbrella #2007).
- **Workspace:** `D:\Repos\sdd` (PBR backport). **Upstream:** `bhargavvc/sdd-cc`. Confirm branch with `git branch` (typical: `feat/sdk-phase3-query-layer`).
...
| Role                                    | GitHub                                                                                 | Notes                                                                                                                                                                                                                                       |
| --------------------------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Product / requirements anchor**       | [#2007](https://github.com/bhargavvc/sdd-cc/issues/2007)                        | Problem statement, user stories, and target architecture for the SDK-first migration. **Do not** treat its original acceptance-checklist boxes as proof of what is merged upstream; work was split into phased PRs after maintainer review. |
| **Phase 3 execution scope**             | [#2302](https://github.com/bhargavvc/sdd-cc/issues/2302) **+ this handover**    | What this branch is actually doing now: registry/CLI parity, docs, harness gaps, runner alignment follow-ups as listed below.                                                                                                               |
| **Patch mine (if local tree is short)** | [PR #2008](https://github.com/bhargavvc/sdd-cc/pull/2008) and matching branches | Large pre-phasing PR; cherry-pick or compare when something looks missing vs that line of work.                                                                                                                                             |
...
- **”Golden parity: coverage and exceptions”** — How `golden.integration.test.ts` compares SDK vs `sdd-tools.cjs` (full `toEqual`, subset, normalized `docs-init`, `intel.update` CJS parity, time-dependent fields, etc.).
- **”CJS command surface vs SDK registry”** — Naming aliases, CLI-only rows, SDK-only rows, and a **top-level `sdd-tools` command → SDK** matrix.
...
### 2. `sdd-sdk query` tokenization (`normalizeQueryCommand`)

- **Problem:** `sdd-sdk query` used only argv[0] as the registry key, so `query state json` dispatched `state` (unregistered) instead of `state.json`.
- **Fix:** `sdk/src/query/normalize-query-command.ts` merges the same **command + subcommand** patterns as `sdd-tools` `runCommand()` (e.g. `state json` → `state.json`, `init execute-phase 9` → `init.execute-phase`, `scaffold …` → `phase.scaffold`, `progress bar` → `progress.bar`). Wired in `sdk/src/cli.ts` before `registry.dispatch()`.
- **Tests:** `sdk/src/query/normalize-query-command.test.ts`.

### 3. `phase add-batch` in the registry

- **Implementation:** `phaseAddBatch` in `sdk/src/query/phase-lifecycle.ts` — port of `cmdPhaseAddBatch` from `sdd/bin/lib/phase.cjs` (batch append under one roadmap lock; sequential or `phase_naming: custom`).
...
- `stateProjectLoad` — `sdk/src/query/state-project-load.ts`; registry key `state.load`. Uses `createRequire` to call `core.cjs` `loadConfig(projectDir)` from the same resolution paths as a normal install (bundled monorepo path, `projectDir/.claude/sdd/...`, `~/.claude/sdd/...`). `SDDTools.stateLoad()` and `formatRegistryRawStdout` for `--raw` no longer force a subprocess solely for this command.
- **Risk:** If `core.cjs` is absent (e.g. some `@bhargavvc/sdk`-only layouts), `state.load` throws `SDDError` — document; future option is a TS `loadConfig` port or bundling.
...
| `sdk/src/cli.ts`                            | `sdd-sdk query` path (uses `normalizeQueryCommand`).                                   |
| `sdk/src/query/QUERY-HANDLERS.md`           | Registry contracts, parity tiers, CJS matrix, mutation notes.                          |
| `sdk/src/golden/golden.integration.test.ts` | Golden parity vs `captureSddToolsOutput()`.                                            |
...
Work that moves **deterministic** orchestration out of AI/bash and into **SDK queries** (historically `sdd-tools.cjs`) has **two layers**. Do not confuse them:


| Layer                    | Goal                                                                                                                                                                         | What “done” looks like                                                                  |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| **Parity / migration**   | Existing CLI behavior is **stable and testable** in the registry so callers can use `sdd-sdk query` instead of `node …/sdd-tools.cjs` without silent drift.                  | Goldens + `QUERY-HANDLERS.md`; same JSON/`--raw` contracts as CJS.                      |
...
### Decision-routing audit (proposed `sdd-tools` / SDK queries)
...
|---|------------------|------|--------|
| 3.1 | `route next-action` | **1** | Next slash-command from `/sdd-next`-style routing. |
...
**SDK registry (Tier 1):** **Done** — `check.config-gates`, `check.phase-ready`, `route.next-action` in `createRegistry()` (`sdk/src/query/index.ts`). Documented in `sdk/src/query/QUERY-HANDLERS.md` § Decision routing (**SDK-only** until/unless mirrored in `sdd-tools.cjs`).

**Simple roadmap (execute in order):**

1. **Harden parity** for surfaces workflows already depend on (registry dispatch, goldens, docs) so swaps from CJS to `sdd-sdk query` stay safe.
