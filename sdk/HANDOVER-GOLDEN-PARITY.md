...
1. **Track C — Runner alignment** (not started)
   - `PhaseRunner` and `InitRunner` both take `SDDTools` (subprocess bridge) as a `tools` dependency (`phase-runner.ts:55`, `init-runner.ts:70`).
   - Issue #2302 says: "Align programmatic paths with the same contracts as query handlers (shared helpers or registry dispatch), **without** removing `SDDTools`."
   - Concretely: where runners currently shell out via `SDDTools.run('state update …')`, they could call the typed handler (`stateUpdate()`) directly or dispatch through `createRegistry()`. This eliminates subprocess overhead on the hot path while keeping `SDDTools` exported for backward compatibility.
...
3. **Track D — CJS deprecation headers** (done)
   - `sdd-tools.cjs` already has `@deprecated` JSDoc header (lines 3-6) pointing to `sdd-sdk query` and `@bhargavvc/sdk`.
...
| Runners (Track C) | **Not started** | `PhaseRunner`/`InitRunner` still use `SDDTools` subprocess bridge |
| Deprecation (Track D) | **Done** | `@deprecated` header on `sdd-tools.cjs` |
...
| `sdk/src/golden/capture.ts` | `captureSddToolsOutput` (JSON stdout); **`captureSddToolsStdout`** (raw stdout, e.g. `config-path`). |
...
| `sdk/src/golden/mutation-subprocess.integration.test.ts` | Tmp fixture + `captureSddToolsOutput` vs `registry.dispatch`; dual sandbox per comparison. |
...
1. **Find the CJS source of truth**  
   - `scan-sessions`: `sdd/bin/lib/profile-pipeline.cjs` → `cmdScanSessions`  
   - `workstream status`: `sdd/bin/lib/workstream.cjs` → `cmdWorkstreamStatus`  
   - `sdd-tools.cjs` `runCommand` switch shows the top-level command and argv.
...
3. **Add a parity row** in `read-only-golden-rows.ts` with `canonical`, `sdkArgs`, `cjs`, `cjsArgs` (must match what `execFile(node, [sddToolsPath, command, ...args])` expects).
...
**`write-profile`**, **`generate-claude-profile`**, **`generate-dev-preferences`**, **`generate-claude-md`** — implemented in **`sdk/src/query/profile-output.ts`** (templates from `sdd/templates/`, same JSON as `profile-output.cjs`); re-exported from **`profile.ts`**. **`milestone.complete`** — full port of **`cmdMilestoneComplete`** in **`phase-lifecycle.ts`**; **`readModifyWriteStateMdFull`** in **`state-mutation.ts`** for STATE writes matching CJS.
...
**Same workflow as above:** read `sdd-tools.cjs` `runCommand` for argv → implement/adjust `sdk/src/query/*.ts` → add `READ_ONLY_JSON_PARITY_ROWS` and/or a **named `describe` block** with documented omissions → `npm run build` → `read-only-parity.integration.test.ts` + `golden-policy.test.ts`.

| Priority | Command (CLI) | `sdd-tools.cjs` case / args | CJS implementation | SDK module | Notes |
| -------- | ------------- | -------------------------- | -------------------- | ---------- | ----- |
...
| ~~6~~ | ~~`validate agents`~~ | `validate` + `agents` | `verify.cjs` `cmdValidateAgents` (~L997) | `validate.ts` `validateAgents` | **Done:** strict row; `getAgentsDir` parity with `core.cjs`; `MODEL_PROFILES` includes `sdd-pattern-mapper` (sync with `model-profiles.cjs`). |
...
| `sdk/src/golden/capture.ts` | `captureSddToolsOutput`, `captureSddToolsStdout`. |
...
| `sdk/src/phase-runner.ts` | **Track C target** — currently uses `SDDTools`. |
| `sdk/src/init-runner.ts` | **Track C target** — currently uses `SDDTools`. |
| `sdk/src/sdd-tools.ts` | Subprocess bridge; **not deleted** in Phase 3 scope. |
| `sdd/bin/sdd-tools.cjs` | `runCommand` — argv routing. Has `@deprecated` header. |
| `sdd/bin/lib/*.cjs` | Per-command implementations (CJS source of truth). |
...
- **Track C:** Runner alignment — `PhaseRunner` and `InitRunner` use typed handlers where possible; `SDDTools` remains exported.
