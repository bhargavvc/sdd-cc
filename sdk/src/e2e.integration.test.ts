 ...
 * opt-in env `SDD_ENABLE_E2E=1`. Skips if env unset or CLI unavailable.
 */
...
import { SDD, parsePlanFile, SDDEventType } from './index.js';
import type { SDDEvent } from './index.js';
...
const e2eEnabled = process.env.SDD_ENABLE_E2E === '1';
...
describe.skipIf(!canRunE2E)('E2E: Single plan execution', () => {
  ...
  beforeAll(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-sdk-e2e-'));
  ...
  it('executes a single plan and returns a valid PlanResult', async () => {
    const sdd = new SDD({ projectDir: tmpDir, maxBudgetUsd: 1.0, maxTurns: 20 });
    const result = await sdd.executePlan('sample-plan.md');
    ...
    expect(outputContent).toContain('hello from sdd-sdk');
  ...
  it('proves session isolation (R014) — different session IDs for sequential runs', async () => {
    ...
    const tmpDir2 = await mkdtemp(join(tmpdir(), 'sdd-sdk-e2e-'));
    await cp(fixturesDir, tmpDir2, { recursive: true });

    try {
      const sdd1 = new SDD({ projectDir: tmpDir, maxBudgetUsd: 1.0, maxTurns: 20 });
      const sdd2 = new SDD({ projectDir: tmpDir2, maxBudgetUsd: 1.0, maxTurns: 20 });

      const result1 = await sdd1.executePlan('sample-plan.md');
      const result2 = await sdd2.executePlan('sample-plan.md');
...
describe.skipIf(!canRunE2E)('E2E: Event stream during plan execution (R007)', () => {
  ...
  beforeAll(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-sdk-e2e-stream-'));
    await cp(fixturesDir, tmpDir, { recursive: true });
  });
  ...
  it('event stream emits events during plan execution (R007)', async () => {
    ...
    const initEvents = events.filter(e => e.type === SDDEventType.SessionInit);
    expect(initEvents.length).toBeGreaterThanOrEqual(1);

    // (b) At least one tool_call event received
    const toolCallEvents = events.filter(e => e.type === SDDEventType.ToolCall);
    expect(toolCallEvents.length).toBeGreaterThanOrEqual(1);

    // (c) Exactly one session_complete event with cost >= 0
    const completeEvents = events.filter(e => e.type === SDDEventType.SessionComplete);
    expect(completeEvents).toHaveLength(1);
    const completeEvent = completeEvents[0]!;
    if (completeEvent.type === SDDEventType.SessionComplete) {
    ...
    const initIdx = events.findIndex(e => e.type === SDDEventType.SessionInit);
    const toolCallIdx = events.findIndex(e => e.type === SDDEventType.ToolCall);
    const completeIdx = events.findIndex(e => e.type === SDDEventType.SessionComplete);
    ...
    const costEvents = events.filter(e => e.type === SDDEventType.CostUpdate);
...
describe('E2E: Error handling', () => {
  it('returns failure for nonexistent plan path', async () => {
    const tmpDir = await mkdtemp(join(tmpdir(), 'sdd-sdk-e2e-err-'));

    try {
      const sdd = new SDD({ projectDir: tmpDir });
      await expect(sdd.executePlan('nonexistent-plan.md')).rejects.toThrow();
    } finally {
      await rm(tmpDir, { recursive: true, force: true });
    }
  });
