 ...
 * sessions for the sdd-sdk init workflow.
 *
...
describe.skipIf(!cliAvailable || !sddToolsAvailable || !e2eEnabled)('E2E: InitRunner.run() full workflow', () => {
  let tmpDir: string;
  let events: SDDEvent[];

  beforeAll(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-sdk-init-e2e-'));
  ...
  it('InitRunner.run() bootstraps a project without human intervention', async () => {
    ...
    const initStartEvents = events.filter(e => e.type === SDDEventType.InitStart);
    expect(initStartEvents.length).toBe(1);

    const stepCompleteEvents = events.filter(e => e.type === SDDEventType.InitStepComplete);
    expect(stepCompleteEvents.length).toBeGreaterThanOrEqual(1);

    // ── Assert: InitComplete event emitted ──
    const initCompleteEvents = events.filter(e => e.type === SDDEventType.InitComplete);
