...
describe('PromptFactory', () => {
  ...
  describe('headless prompt loading', () => {
    it('loadWorkflowFile prefers installed SDD over sdkPromptsDir', async () => {
      const sdkDir = join(tempDir, 'sdk-prompts');
      await mkdir(join(sdkDir, 'workflows'), { recursive: true });

      // Write both: installed SDD and SDK bundled version
    ...
    it('loadAgentDef prefers installed agents over sdkPromptsDir', async () => {
      ...
      await writeFile(join(agentsDir, 'sdd-executor.md'), 'user agent');
      await writeFile(join(sdkDir, 'agents', 'sdd-executor.md'), 'SDK bundled agent');
