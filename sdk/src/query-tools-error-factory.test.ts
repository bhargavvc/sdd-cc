...
import { ErrorClassification, SDDError } from './errors.js';
import {
...
describe('query tools error factory', () => {
  ...
  it('maps SDDError to failure with semantic exit code', () => {
    const err = toToolsErrorFromUnknown('state', ['load'], new SDDError('bad', ErrorClassification.Validation));
  ...
  it('maps timeout-like unknown errors to timeout classification', () => {
    const err = toToolsErrorFromUnknown('state', ['load'], new Error('sdd-tools timed out after 50ms: state load'));
