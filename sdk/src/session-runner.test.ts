  ...
  describe('resolveModel runtime awareness (#2832)', () => {
    it('does NOT pass a Claude profile model id when runtime is codex', async () => {
      await runPhaseStepSession(
        ...
        makeConfig({ runtime: 'codex', model_profile: 'balanced' } as Partial<SDDConfig>),
      );
    ...
    it('does NOT pass a Claude profile model id when resolve_model_ids is "omit"', async () => {
      await runPhaseStepSession(
        'prompt',
        PhaseStepType.Execute,
        makeConfig({ resolve_model_ids: 'omit', model_profile: 'balanced' } as Partial<SDDConfig>),
      );
      const opts = mockQueryCalls[0].options as { model?: string };
      expect(opts.model).toBeUndefined();
    });
    ...
    it('still maps profile -> Claude id when runtime is claude (no regression)', async () => {
      await runPhaseStepSession(
        'prompt',
        PhaseStepType.Execute,
        makeConfig({ runtime: 'claude', model_profile: 'balanced' } as Partial<SDDConfig>),
      );
      const opts = mockQueryCalls[0].options as { model?: string };
      expect(opts.model).toBe('claude-sonnet-4-6');
    });

    it('respects SDD_RUNTIME env precedence over config (no Claude id when env=codex)', async () => {
      const prev = process.env.SDD_RUNTIME;
      process.env.SDD_RUNTIME = 'codex';
      try {
        await runPhaseStepSession(
          'prompt',
          PhaseStepType.Execute,
          makeConfig({ model_profile: 'balanced' } as Partial<SDDConfig>),
        );
        const opts = mockQueryCalls[0].options as { model?: string };
        expect(opts.model).toBeUndefined();
      } finally {
        if (prev === undefined) delete process.env.SDD_RUNTIME;
        else process.env.SDD_RUNTIME = prev;
      }
    });
    ...
    it('explicit options.model wins for any runtime', async () => {
      await runPhaseStepSession(
        'prompt',
        PhaseStepType.Execute,
        makeConfig({ runtime: 'codex' } as Partial<SDDConfig>),
        { model: 'gpt-5.3-codex' },
      );
      const opts = mockQueryCalls[0].options as { model?: string };
