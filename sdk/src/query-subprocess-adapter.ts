...
export class QuerySubprocessAdapter {
  ...
  private commandArgs(command: string, args: string[]): string[] {
    ...
    return [this.deps.sddToolsPath, command, ...args, ...wsArgs];
  }
  ...
  private processSpawnError(command: string, args: string[], err: Error) {
    return this.deps.createFailureError(`Failed to execute sdd-tools: ${err.message}`, command, args, null, '');
  }
  ...
  private async parseOutput(raw: string): Promise<unknown> {
    ...
    if (jsonStr.startsWith('@file:')) {
      ...
      try {
        ...
        throw new Error(`Failed to read sdd-tools @file: indirection at "${resolvedPath}": ${reason}`);
      }
    }
