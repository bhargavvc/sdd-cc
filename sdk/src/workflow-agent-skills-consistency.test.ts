/**
 * Contract test: every `sdd-sdk query agent-skills <slug>` invocation in
 * `sdd/workflows/**\/*.md` must reference a slug that exists as
 ...
 *
 * Related: https://github.com/bhargavvc/sdd-cc/issues/2615
 */
...
const workflowsDir = join(repoRoot, 'sdd', 'workflows');
const agentsDir = join(repoRoot, 'agents');

/**
 * Matches a full `sdd-sdk query agent-skills <slug>` invocation and captures
 * the slug. Requires a token boundary before `sdd-sdk` and a word boundary
 ...
 */
const QUERY_KEY_PATTERN = /\bsdd-sdk\s+query\s+agent-skills\s+([a-z][a-z0-9-]*)\b/g;
...
/**
 * Extracts every `sdd-sdk query agent-skills <slug>` usage from the given
