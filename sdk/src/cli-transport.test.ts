...
import { SDDEventType, type SDDEvent, type SDDEventBase } from './types.js';
...
function makeBase(overrides: Partial<SDDEventBase> = {}): Omit<SDDEventBase, 'type'> {
  return {
...
describe('CLITransport', () => {
  ...
  it('formats PhaseStart as bold cyan banner and PhaseComplete with running cost', () => {
    ...
    expect(lines[0]).toBe(`${BOLD}${CYAN}━━━ SDD ► PHASE 01: Authentication ━━━${RESET}`);
  ...
  it('formats MilestoneStart as bold banner and MilestoneComplete with running cost', () => {
    ...
    expect(lines[1]).toBe(`${BOLD}  SDD Milestone — 3 phases${RESET}`);
