/**
 * SDD Event Stream — maps SDKMessage variants to typed SDD events.
 *
 * Extends EventEmitter to provide a typed event bus. Includes:
 * - SDKMessage → SDDEvent mapping
...
import {
  ...
  type SDDRateLimitEvent as SDDRateLimitEventType,
...
export class SDDEventStream extends EventEmitter {
  ...
  /** Emit a typed SDD event to all listeners and transports. */
  emitEvent(event: SDDEvent): void {
  ...
  /**
   * Map an SDKMessage to a SDDEvent.
   * Returns null for non-actionable message types (user messages, replays, etc.).
   */
  mapSDKMessage(msg: SDKMessage, context: EventStreamContext = {}): SDDEvent | null {
    const base = {
   ...
   */
  mapAndEmit(msg: SDKMessage, context: EventStreamContext = {}): SDDEvent | null {
