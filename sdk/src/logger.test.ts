...
import { SDDLogger } from './logger.js';
...
describe('SDDLogger', () => {
  ...
  it('outputs valid JSON on each log call', () => {
    const logger = new SDDLogger({ output, level: 'debug' });
  ...
  it('filters messages below minimum log level', () => {
    const logger = new SDDLogger({ output, level: 'warn' });
  ...
  it('defaults to info level filtering', () => {
    const logger = new SDDLogger({ output });
  ...
  it('writes to custom output stream', () => {
    ...
    const logger = new SDDLogger({ output: customOutput, level: 'debug' });
    logger.info('custom');
