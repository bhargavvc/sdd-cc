...
import { SDDTools, SDDToolsError } from './sdd-tools.js';
import {
  PhaseStepType,
  SDDEventType,
  PhaseType,
  ...
  type SDDPhaseStartEvent,
  type SDDPhaseStepStartEvent,
  type SDDPhaseStepCompleteEvent,
  type SDDPhaseCompleteEvent,
} from './types.js';
...
describe('Phase lifecycle types', () => {
  ...
  // ─── SDDEventType phase lifecycle values ───────────────────────────────

  describe('SDDEventType phase lifecycle events', () => {
    it('includes PhaseStart', () => {
      expect(SDDEventType.PhaseStart).toBe('phase_start');
    });

    it('includes PhaseStepStart', () => {
      expect(SDDEventType.PhaseStepStart).toBe('phase_step_start');
    });

    it('includes PhaseStepComplete', () => {
      expect(SDDEventType.PhaseStepComplete).toBe('phase_step_complete');
    });

    it('includes PhaseComplete', () => {
      expect(SDDEventType.PhaseComplete).toBe('phase_complete');
    });
  });
  ...
  describe('PhaseOpInfo interface', () => {
    ...
    it('matches the documented init phase-op JSON shape', () => {
      // Simulate parsing JSON from sdd-tools.cjs
  ...
  describe('Phase lifecycle event interfaces', () => {
    it('SDDPhaseStartEvent has correct shape', () => {
      const event: SDDPhaseStartEvent = {
    ...
    it('SDDPhaseStepStartEvent has correct shape', () => {
      const event: SDDPhaseStepStartEvent = {
    ...
    it('SDDPhaseStepCompleteEvent has correct shape', () => {
      const event: SDDPhaseStepCompleteEvent = {
    ...
    it('SDDPhaseStepCompleteEvent can include error', () => {
      const event: SDDPhaseStepCompleteEvent = {
        type: SDDEventType.PhaseStepComplete,
        timestamp: new Date().toISOString(),
        sessionId: 'test-session',
    ...
    it('SDDPhaseCompleteEvent has correct shape', () => {
      const event: SDDPhaseCompleteEvent = {
...
// ─── SDDTools typed methods ──────────────────────────────────────────────────

describe('SDDTools typed methods', () => {
  let tmpDir: string;
  let fixtureDir: string;

  beforeEach(async () => {
    tmpDir = join(tmpdir(), `sdd-tools-phase-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  describe('initPhaseOp()', () => {
    it('returns typed PhaseOpInfo from sdd-tools output', async () => {
  ...
  describe('configGet()', () => {
    it('returns string value from sdd-tools config', async () => {
      const scriptPath = await createScript(
        'config-get.cjs',
        `
        const args = process.argv.slice(2);
