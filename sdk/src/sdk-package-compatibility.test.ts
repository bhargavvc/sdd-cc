...
import {
  ...
  BUNDLED_SDD_AGENTS_DIR,
  BUNDLED_SDD_TEMPLATES_DIR,
...
describe('SDK Package Seam Module', () => {
  ...
  it('resolves legacy install-relative directories through one seam', () => {
    expect(resolveLegacyInstallDir(homeDir)).toBe(join(homeDir, '.claude', 'sdd'));
    expect(resolveLegacyTemplatesDir(homeDir)).toBe(join(homeDir, '.claude', 'sdd', 'templates'));
    expect(resolveLegacyWorkflowsDir(homeDir)).toBe(join(homeDir, '.claude', 'sdd', 'workflows'));
    expect(resolveLegacyUserProfilePath(homeDir)).toBe(join(homeDir, '.claude', 'sdd', 'USER-PROFILE.md'));
    expect(resolveBundledTemplatesDir()).toBe(BUNDLED_SDD_TEMPLATES_DIR);
    expect(resolveBundledAgentsDir()).toBe(BUNDLED_SDD_AGENTS_DIR);
  });

  it('probes legacy sdd-tools locations in bundled -> project -> home order', () => {
    const resolution = probeLegacySdkAsset('sdd-tools', projectDir, {
      homeDir,
      existsSync: path => path === join(projectDir, '.claude', 'sdd', 'bin', 'sdd-tools.cjs'),
    });
    ...
    expect(resolution.path).toBe(join(projectDir, '.claude', 'sdd', 'bin', 'sdd-tools.cjs'));
    expect(resolution.fallbackPath).toBe(join(homeDir, '.claude', 'sdd', 'bin', 'sdd-tools.cjs'));
  });

  it('returns concrete fallback sdd-tools path when no legacy probe exists', () => {
    const path = resolveSddToolsPath(projectDir, {
      homeDir,
      existsSync: () => false,
    });

    expect(path).toBe(join(homeDir, '.claude', 'sdd', 'bin', 'sdd-tools.cjs'));
  });
  ...
  it('reports checked core.cjs probes when legacy asset missing', () => {
    ...
    try {
      ...
      expect.fail('expected SDDError');
    } catch (error) {
      expect(error).toBeInstanceOf(SDDError);
      const message = (error as Error).message;
      expect(message).toContain('state load: sdd/bin/lib/core.cjs not found.');
      expect(message).toContain('Checked:');
      expect(message).toContain(BUNDLED_CORE_CJS_PATH);
      expect(message).toContain(join(projectDir, '.claude', 'sdd', 'bin', 'lib', 'core.cjs'));
      expect(message).toContain(join(homeDir, '.claude', 'sdd', 'bin', 'lib', 'core.cjs'));
    }
  });
});
