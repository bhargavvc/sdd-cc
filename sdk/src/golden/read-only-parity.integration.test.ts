/**
 * Read-only subprocess golden checks (SDK vs sdd-tools.cjs JSON).
 * Row data: `read-only-golden-rows.ts`. Policy: `golden-policy.ts`, `QUERY-HANDLERS.md`.
 */
import { describe, it, expect } from 'vitest';
import { captureSddToolsOutput, captureSddToolsStdout } from './capture.js';
...
describe('Read-only golden parity (JSON toEqual)', () => {
  it.each(STABLE_JSON_PARITY_ROWS)('$canonical matches sdd-tools.cjs JSON', async (row) => {

    const sddOutput = await captureSddToolsOutput(row.cjs, row.cjsArgs, REPO_ROOT);
...
describe('config-path (plain stdout vs SDK { path })', () => {
  it('SDK path matches sdd-tools.cjs plain-text stdout', async () => {
    const out = await captureSddToolsStdout('config-path', [], REPO_ROOT);
    const registry = createRegistry();
...
describe('audit-open golden parity (excluding scanned_at)', () => {
  it('SDK JSON matches sdd-tools.cjs except volatile scanned_at', async () => {
    const sddOutput = await captureSddToolsOutput('audit-open', ['--json'], REPO_ROOT);
    const registry = createRegistry();
    ...
    expect(strip(sdkResult.data)).toEqual(strip(sddOutput));
  });
});

describe('state.json golden parity (excluding last_updated)', () => {
  it('SDK rebuilt frontmatter matches sdd-tools.cjs except volatile last_updated', async () => {
    const sddOutput = await captureSddToolsOutput('state', ['json'], REPO_ROOT);
    const registry = createRegistry();
...
describe('summary.extract golden parity (with array-of-objects fix)', () => {
  it('SDK JSON matches sdd-tools.cjs except for intentional array-of-objects parsing fix', async () => {
    const sddOutput = await captureSddToolsOutput('summary-extract', ['sdk/src/golden/fixtures/summary-extract-sample.md'], REPO_ROOT);
    const registry = createRegistry();
...
describe('state.load golden parity', () => {
  it('SDK load payload matches sdd-tools.cjs state load', async () => {
    const sddOutput = await captureSddToolsOutput('state', ['load'], REPO_ROOT);
    const registry = createRegistry();
...
describe('state.get golden parity', () => {
  it('matches full STATE.md when no field (same as `state get` with no section)', async ({ skip }) => {
    ...
    const sddOutput = await captureSddToolsOutput('state', ['get'], REPO_ROOT);
    expect(sdkResult.data).toEqual(sddOutput);
  });
  ...
  it('matches single frontmatter field when `state get <field>`', async ({ skip }) => {
    ...
    if ((sdkResult.data as Record<string, unknown>)?.error === 'STATE.md not found') skip();
    const sddOutput = await captureSddToolsOutput('state', ['get', 'milestone'], REPO_ROOT);
    expect(sdkResult.data).toEqual(sddOutput);
  });
});

describe('verify.commits golden parity', () => {
  it('SDK output matches sdd-tools.cjs for two SHAs', async () => {
    ...
    const b = revs[0];
    const a = revs[1];
    const sddOutput = await captureSddToolsOutput('verify', ['commits', a, b], REPO_ROOT);
    const registry = createRegistry();
