...
const MUTATION_DEFERRED_REASON =
  'Listed in QUERY_MUTATION_COMMANDS — mutates `.planning/`, git, or profile files. Subprocess golden vs sdd-tools.cjs is covered where a tmp fixture or `--dry-run` exists in golden.integration.test.ts; otherwise handler parity lives in sdk/src/query/*-mutation.test.ts, commit.test.ts, phase-lifecycle.test.ts, workstream.test.ts, intel.test.ts, profile.test.ts, template.test.ts, docs-init.ts, or uat.test.ts as applicable.';

/** Registry commands with no `sdd-tools.cjs` analogue — cannot have subprocess JSON parity. */
const NO_CJS_SUBPROCESS_REASON: Record<string, string> = {
  'phases.archive':
    'No `sdd-tools.cjs` command for `phases archive` (SDK-only). Covered in sdk/src/query/phase-lifecycle.test.ts.',
  'check.config-gates':
  ...
  'phase.list-plans':
    'SDK-only listing helper for agents (no `sdd-tools.cjs` mirror). Covered in sdk/src/query/phase-list-queries.test.ts.',
  ...
  'commands':
    'SDK-only registry introspection (no sdd-tools.cjs equivalent — the CJS layer has no self-describing verb). Covered in sdk/src/query/commands-list.test.ts. Closes #3121.',
  'phase.mvp-mode':
...
const READ_HANDLER_ONLY_REASON = (cmd: string) =>
  `No ` +
  '`toEqual` subprocess row yet for this read-only command — handler parity is covered in sdk/src/query/*.test.ts / decomposed-handlers.test.ts; add `captureSddToolsOutput` + `registry.dispatch` in sdk/src/golden/ when JSON shapes are aligned (see QUERY-HANDLERS.md § Golden registry coverage matrix). Command: `' +
  cmd +
  '`.';
...
/**
 * Canonical commands with an explicit subprocess JSON check vs sdd-tools.cjs
