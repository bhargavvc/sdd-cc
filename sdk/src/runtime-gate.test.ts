 ...
 *
 * Regression tests for #2832: sdd-sdk auto silently routed Codex (and other
 ...
 * runtime or fall back to the in-session SDD slash commands.
 */
...
describe('assertRuntimeSupportsAutoMode', () => {
  ...
  beforeEach(() => {
    prevEnv = process.env.SDD_RUNTIME;
    delete process.env.SDD_RUNTIME;
  });
  afterEach(() => {
    if (prevEnv === undefined) delete process.env.SDD_RUNTIME;
    else process.env.SDD_RUNTIME = prevEnv;
  });
  ...
  it('throws for SDD_RUNTIME=codex even when config says claude', () => {
  ...
  it('error message references issue #2832 and slash-command workaround', () => {
    ...
    expect(caught!.message).toMatch(/sdd-discuss-phase|sdd-plan-phase|sdd-execute-phase/);
  });
  ...
  it('attributes source to config when SDD_RUNTIME is set to an unsupported value', () => {
    ...
    process.env.SDD_RUNTIME = 'unsupported-env';
    let caught: Error | undefined;
    try {
      assertRuntimeSupportsAutoMode({ runtime: 'codex' });
    } catch (err) {
    ...
    expect(caught!.message).not.toMatch(/SDD_RUNTIME=unsupported-env/);
  });
});
