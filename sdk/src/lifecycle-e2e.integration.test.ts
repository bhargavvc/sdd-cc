/**
 * E2E lifecycle integration test — proves SDD.runPhase() drives
 * the full phase lifecycle: discuss → research → plan → execute → verify → advance
 * after bootstrapping a real project via InitRunner.
 *
 * This is the capstone proof that `sdd-sdk auto` works end-to-end
 * without human intervention. InitRunner bootstraps the project,
 * then SDD.runPhase() drives Phase 1 through the complete lifecycle.
 *
...
import type { SDDEvent, PhaseRunnerResult, RoadmapAnalysis } from './types.js';
...
describe.skipIf(!cliAvailable || !sddToolsAvailable)('E2E Lifecycle: InitRunner → SDD.runPhase() full lifecycle', () => {
  let tmpDir: string;
  ...
  beforeAll(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-sdk-lifecycle-e2e-'));
    ...
    const initEventStream = new SDDEventStream();
  ...
  it('SDD.runPhase() drives Phase 1 through the full lifecycle without human intervention', async () => {
    ...
    // Construct SDD with autoMode: true
    const sdd = new SDD({
      projectDir: tmpDir,
      autoMode: true,
    });
    sdd.onEvent((e: SDDEvent) => events.push(e));

    // Run the discovered first phase with tight budget to minimize cost
    const result: PhaseRunnerResult = await sdd.runPhase(phase1Number!, {
      maxTurnsPerStep: 10,
    ...
    if (phaseComplete.type === SDDEventType.PhaseComplete) {
    ...
    const stepStartEvents = events.filter(
      (e): e is Extract<SDDEvent, { type: SDDEventType.PhaseStepStart }> =>
        e.type === SDDEventType.PhaseStepStart,
    );
    ...
    const stepCompleteEvents = events.filter(
      (e): e is Extract<SDDEvent, { type: SDDEventType.PhaseStepComplete }> =>
        e.type === SDDEventType.PhaseStepComplete,
    );
