...
export class PromptFactory {
  ...
  constructor(options?: {
    ...
    const sddInstallDir = options?.sddInstallDir ?? resolveLegacyInstallDir();
    this.workflowsDir = join(sddInstallDir, 'workflows');
    this.agentsDir = options?.agentsDir ?? join(sddInstallDir, '..', 'agents');
   ...
   * Tries installed SDD workflows first (the complete, up-to-date versions),
  ...
  async loadWorkflowFile(phaseType: PhaseType): Promise<string | undefined> {
    ...
    // Try installed SDD workflows first (complete versions)
    const paths = [
