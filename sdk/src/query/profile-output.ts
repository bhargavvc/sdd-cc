 ...
 * Ported from `sdd/bin/lib/profile-output.cjs` (`cmdWriteProfile`,
...
const TEMPLATE_DIR = join(dirname(fileURLToPath(import.meta.url)), '../../../sdd/templates');
...
function extractSectionContent(fileContent: string, sectionName: string): string | null {
  const startMarker = `<!-- SDD:${sectionName}-start`;
  const endMarker = `<!-- SDD:${sectionName}-end -->`;
...
function buildSection(sectionName: string, sourceFile: string, content: string): string {
  return [`<!-- SDD:${sectionName}-start source:${sourceFile} -->`, content, `<!-- SDD:${sectionName}-end -->`].join('\n');
}
...
function generateWorkflowSection(): { content: string; source: string; hasFallback: boolean } {
  return { content: CLAUDE_MD_WORKFLOW_ENFORCEMENT, source: 'SDD defaults', hasFallback: false };
}
...
function cmdWriteProfileLogic(
  ...
  if (!existsSync(analysisPath)) {
    throw new SDDError(`Analysis file not found: ${analysisPath}`, ErrorClassification.Validation);
  }
  ...
  try {
    ...
    throw new SDDError(`Failed to parse analysis JSON: ${msg}`, ErrorClassification.Validation);
  }

  if (!analysis.dimensions || typeof analysis.dimensions !== 'object') {
    throw new SDDError('Analysis JSON must contain a "dimensions" object', ErrorClassification.Validation);
  }
  if (!analysis.profile_version) {
    throw new SDDError('Analysis JSON must contain "profile_version"', ErrorClassification.Validation);
  }
  ...
  if (!existsSync(templatePath)) {
    throw new SDDError(`Template not found: ${templatePath}`, ErrorClassification.Validation);
  }
  ...
  if (!outputPath) {
    outputPath = join(homedir(), '.claude', 'sdd', 'USER-PROFILE.md');
...
export const writeProfile: QueryHandler = async (args, projectDir) => {
  ...
  if (!inputPath) {
    throw new SDDError('--input <analysis-json-path> is required', ErrorClassification.Validation);
  }
...
export const generateDevPreferences: QueryHandler = async (args, projectDir) => {
  ...
  if (!analysisPath) {
    throw new SDDError('--analysis <path> is required', ErrorClassification.Validation);
  }

  let ap = analysisPath;
  if (!isAbsolute(ap)) ap = join(projectDir, ap);
  if (!existsSync(ap)) {
    throw new SDDError(`Analysis file not found: ${ap}`, ErrorClassification.Validation);
  }
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock =
      'Stack preferences not available (questionnaire-only profile). Run `/sdd-profile-user --refresh` with session data to populate.';
  } else if (stackOpt) {
    stackBlock = stackOpt;
  } else {
  ...
  if (!outPath) {
    outPath = join(homedir(), '.claude', 'commands', 'sdd', 'dev-preferences.md');
...
export const generateClaudeProfile: QueryHandler = async (args, projectDir) => {
  ...
  if (existsSync(targetPath)) {
    ...
    const startMarker = '<!-- SDD:profile-start -->';
    const endMarker = '<!-- SDD:profile-end -->';
...
export const generateClaudeMd: QueryHandler = async (args, projectDir) => {
  ...
  if (existingContent === null) {
    ...
    for (const name of MANAGED_SECTIONS) {
      ...
      const hasMarkers = fileContent.indexOf(`<!-- SDD:${name}-start`) !== -1;
    ...
    if (!autoFlag && fileContent.indexOf('<!-- SDD:profile-start') === -1) {
  ...
  if (finalContent && finalContent.indexOf('<!-- SDD:profile-start') !== -1) {
    if (action === 'created' || existingContent.indexOf('<!-- SDD:profile-start') === -1) {
