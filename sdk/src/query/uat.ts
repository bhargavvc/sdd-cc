 ...
 *
 * Ported from sdd/bin/lib/uat.cjs.
...
/** Same string as `buildCheckpoint` in `sdd/bin/lib/uat.cjs`. */
 ...
 *
 * Fixes #2788: when sdd-verifier encodes human items in YAML frontmatter
...
function parseVerificationItems(content: string, status: string, fm?: Record<string, unknown>): Record<string, unknown>[] {
  ...
  if (status === 'human_needed') {
    ...
    // sdd-verifier writes items here; body-section fallback is secondary.
    if (fm) {
...
export const auditUat: QueryHandler = async (_args, projectDir, workstream) => {
  ...
  if (!existsSync(paths.phases)) {
    throw new SDDError('No phases directory found in planning directory', ErrorClassification.Blocked);
  }
