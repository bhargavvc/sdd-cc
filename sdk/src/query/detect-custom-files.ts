/**
 * Detect user-added files under SDD-managed install dirs not listed in the manifest.
 *
 * Port of `detect-custom-files` from `sdd/bin/sdd-tools.cjs` (lines 1161–1239).
 */
...
export const detectCustomFiles: QueryHandler = async (args) => {
  ...
  const manifestPath = join(resolvedConfigDir, 'sdd-file-manifest.json');
