 ...
 * `sdd/bin/lib/init.cjs` so the SDK path honors
 * `config.agent_skills[agentType]` the same way the legacy
 * `sdd-tools.cjs agent-skills <type>` path does. Project-relative skills stay
 ...
 *
 * // With config.agent_skills = { "sdd-planner": [".claude/skills/demo-skill"] }
 * await agentSkills(['sdd-planner'], '/project');
 * // { data: '<agent_skills>\nRead these user-configured skills:\n- @.claude/skills/demo-skill/SKILL.md\n</agent_skills>' }
 *
 * // No agent type → empty string (matches sdd-tools cmdAgentSkills).
...
export const agentSkills: QueryHandler = async (args, projectDir) => {
  ...
  // Match sdd-tools `cmdAgentSkills`: no agent type → empty string (JSON `""`), not a structured object.
  if (!agentType) {
    return { data: '' };
  }
  ...
  // with `$(sdd-sdk query agent-skills …)` and need the XML block verbatim, not
