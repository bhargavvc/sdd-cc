/**
 * `profile-sample` — parity with `sdd/bin/lib/profile-pipeline.cjs` `cmdProfileSample`.
 */
...
/**
 * Port of `cmdProfileSample` — same JSON + JSONL file shape as `sdd-tools profile-sample`.
 */
export async function runProfileSample(
  ...
  try {
    ...
    throw new SDDError(`Cannot read sessions directory: ${msg}`, ErrorClassification.Validation);
  }

  if (projectDirs.length === 0) {
    throw new SDDError('No project directories found in sessions directory.', ErrorClassification.Validation);
  }
  ...
  if (projectCount === 0) {
    throw new SDDError('No projects with sessions found.', ErrorClassification.Validation);
  }
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'sdd-profile-'));
