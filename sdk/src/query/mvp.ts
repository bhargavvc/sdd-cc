 ...
 *    that was previously prose-only in `references/execute-mvp-tdd.md`. The sdd-executor
 ...
 *    prose. Consumed by the verifier (phase-goal guard) and by `/sdd-mvp-phase`
 *    (interactive-prompt validation).
 *
 * Domain terms: see CONTEXT.md → MVP Mode, User Story, Behavior-Adding Task.
 * Concept index: sdd/references/mvp-concepts.md.
 */
 ...
 *
 * @example
 *   sdd-sdk query phase.mvp-mode 1                    # roadmap + config check
 *   sdd-sdk query phase.mvp-mode 1 --cli-flag         # caller saw --mvp on CLI
 */
 ...
 *
 * Canonical specification: sdd/references/execute-mvp-tdd.md.
 *
 * @example
 *   sdd-sdk query task.is-behavior-adding ./plans/01-PLAN-auth.md
 *   sdd-sdk query task.is-behavior-adding --task-content "<task>...</task>"
 */
 ...
 * Used by `sdd-verifier` (phase-goal guard) and `/sdd-mvp-phase` (interactive prompting).
 *
 * @example
 *   sdd-sdk query user-story.validate "As a user, I want to log in, so that I can see my data."
 *   sdd-sdk query user-story.validate --story "<text>"
 */
