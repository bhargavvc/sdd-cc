...
describe('verifyKeyLinks', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-validate-'));
  });
  ...
  it('throws SDDError Validation when no args', async () => {
    let caught: unknown;
    try {
...
`;
  ...
  it('reports invalid regex like sdd-tools.cjs (try/catch on new RegExp)', async () => {
    await writeFile(join(tmpDir, 'source.ts'), 'const x = 1;');
    await writeFile(join(tmpDir, 'target.ts'), 'const y = 2;');
...
describe('validateConsistency', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-consistency-'));
  });
...
describe('validateHealth', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-health-'));
  });
