 ...
 *
 * Ported from sdd/bin/lib/state.cjs.
...
export const stateUpdate: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (!field || value === undefined) {
    throw new SDDError('field and value required for state update', ErrorClassification.Validation);
  }
 ...
 *
 * @param args - Either `--field value` pairs (CLI / sdd-tools) or a single JSON object string (SDK).
 * @param projectDir - Project root directory
...
export const statePatch: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (args.length >= 2 && args[0]?.startsWith('--')) {
    ...
    if (!jsonString) {
      throw new SDDError('JSON patches required', ErrorClassification.Validation);
    }
    try {
      patches = JSON.parse(jsonString) as Record<string, string>;
    } catch {
      throw new SDDError('Invalid JSON for patches', ErrorClassification.Validation);
    }
  }
 ...
 *
 * Accepts sdd-tools-style argv: `--phase N [--name S] [--plans C]` or positional
...
export const stateBeginPhase: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (!phaseNumber) {
    throw new SDDError('phase number required', ErrorClassification.Validation);
  }
 ...
 *
 * @param args - sdd-tools argv: `--phase`, `--plan`, `--duration`, `--tasks`, `--files`
 * @param projectDir - Project root directory
 ...
 * argv matches `sdd-tools.cjs`: `--phase`, `--summary`, `--rationale`, etc.
 */
 ...
 * argv: `--text`, `--text-file` (see `sdd-tools.cjs`).
 */
 ...
 * already exists.
 *
 * Throws `SDDError` with `ErrorClassification.Validation` when required
...
export const stateAddRoadmapEvolution: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (!phase) {
    throw new SDDError('phase required for state.add-roadmap-evolution', ErrorClassification.Validation);
  }
  if (!action) {
    throw new SDDError('action required for state.add-roadmap-evolution', ErrorClassification.Validation);
  }
 ...
 *
 * The `/sdd-new-milestone` workflow only rewrote STATE.md's body (Current
 ...
 *
 * Args (named, matches sdd-tools style):
 ...
 * `state.milestone-switch` from `/sdd-new-milestone` Step 5 in place of the
 * manual body rewrite.
 */
export const stateMilestoneSwitch: QueryHandler = async (args, projectDir, workstream) => {
  // NOTE: the CLI flag is `--milestone` (not `--version`). sdd-tools reserves
  ...
  try {
    ...
    if (existingFm.sdd_state_version) {
      fm.sdd_state_version = existingFm.sdd_state_version;
    }
...
// ─── parseNamedArgs (matches sdd-tools.cjs) ───────────────────────────────
...
function parseNamedArgs(
  ...
  for (const flag of valueFlags) {
    ...
    if (value === undefined || value.startsWith('--')) {
      throw new SDDError(`missing value for --${flag}`, ErrorClassification.Validation);
    }
    result[flag] = value;
  }
 ...
 *
 * Writes `WAITING.json` under both `.sdd/` and `.planning/` so readers that only
...
export const stateSignalWaiting: QueryHandler = async (args, projectDir, _workstream) => {
  ...
  const waitingPaths = [
    join(projectDir, '.sdd', 'WAITING.json'),
  ...
  try {
    ...
    mkdirSync(join(projectDir, '.sdd'), { recursive: true });
