...
export const planTaskStructure: QueryHandler = async (args, projectDir) => {
  const rel = args[0];
  if (!rel) {
    throw new SDDError('PLAN.md path required', ErrorClassification.Validation);
  }
  ...
  try {
    ...
    if (err instanceof SDDError) {
      throw new SDDError(`cannot read plan file: ${err.message}`, ErrorClassification.Blocked);
    }
    throw err;
  }
  ...
  try {
    ...
    throw new SDDError(`cannot read plan file: ${rel}`, ErrorClassification.Blocked);
  }
