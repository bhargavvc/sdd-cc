...
describe('initMilestoneOp workstream resolution (#3196)', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-ws-milestone-op-'));
  ...
  it('reads from active-workstream file when no explicit --ws is passed', async () => {
    ...
    // Resolve context as the CLI would (no --ws arg, no SDD_WORKSTREAM env)
    const prev = process.env.SDD_WORKSTREAM;
    ...
    try {
      ...
      if (prev === undefined) delete process.env.SDD_WORKSTREAM;
      else process.env.SDD_WORKSTREAM = prev;
    }
  });
  ...
  it('SDD_WORKSTREAM env overrides active-workstream file', async () => {
    // File says other-ws, env says test-ws
    await writeFile(join(tmpDir, '.planning', 'active-workstream'), 'other-ws\n');

    const prev = process.env.SDD_WORKSTREAM;
    process.env.SDD_WORKSTREAM = 'test-ws';
    try {
      const ctx = resolveQueryRuntimeContext({ projectDir: tmpDir });
      expect(ctx.ws).toBe('test-ws');
    } finally {
...
describe('roadmapAnalyze workstream resolution (#3196)', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-ws-roadmap-analyze-'));
...
describe('resolveQueryRuntimeContext active-workstream file fallback (#3196)', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-rtctx-'));
  ...
  it('reads ws from active-workstream file when no --ws and no SDD_WORKSTREAM', async () => {
  ...
  it('SDD_WORKSTREAM env takes priority over active-workstream file', async () => {
    await writeFile(join(tmpDir, '.planning', 'active-workstream'), 'my-ws\n');
    await mkdir(join(tmpDir, '.planning', 'workstreams', 'env-ws'), { recursive: true });

    const prev = process.env.SDD_WORKSTREAM;
    process.env.SDD_WORKSTREAM = 'env-ws';
    try {
      const ctx = resolveQueryRuntimeContext({ projectDir: tmpDir });
