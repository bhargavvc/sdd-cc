 ...
 *
 * Ported from sdd/bin/lib/verify.cjs.
 ...
 * @throws SDDError with Validation classification if file path missing
 */
export const verifyPlanStructure: QueryHandler = async (args, projectDir) => {
  const filePath = args[0];
  if (!filePath) {
    throw new SDDError('file path required', ErrorClassification.Validation);
  }

  // T-12-01: Null byte rejection on file paths
  if (filePath.includes('\0')) {
    throw new SDDError('file path contains null bytes', ErrorClassification.Validation);
  }
 ...
 * @throws SDDError with Validation classification if phase number missing
 */
export const verifyPhaseCompleteness: QueryHandler = async (args, projectDir, workstream) => {
  const phase = args[0];
  if (!phase) {
    throw new SDDError('phase required', ErrorClassification.Validation);
  }
...
export const verifyArtifacts: QueryHandler = async (args, projectDir) => {
  ...
  if (!planFilePath) {
    throw new SDDError('plan file path required', ErrorClassification.Validation);
  }
 ...
 * Used by sdd-verifier agent to confirm commits mentioned in summaries
...
export const verifyCommits: QueryHandler = async (args, projectDir) => {
  if (args.length === 0) {
    throw new SDDError('At least one commit hash required', ErrorClassification.Validation);
  }
...
export const verifySummary: QueryHandler = async (args, projectDir) => {
  ...
  if (!summaryPath) {
    throw new SDDError('summary-path required', ErrorClassification.Validation);
  }
...
export const verifyPathExists: QueryHandler = async (args, projectDir) => {
  ...
  if (!targetPath) {
    throw new SDDError('path required for verification', ErrorClassification.Validation);
  }
  if (targetPath.includes('\0')) {
    throw new SDDError('path contains null bytes', ErrorClassification.Validation);
  }
...
export const verifySchemaDrift: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (!phaseArg) {
    throw new SDDError('Usage: verify schema-drift <phase> [--skip]', ErrorClassification.Validation);
  }
 ...
 * `/sdd-execute-phase` relies on this guarantee.
 *
...
export const verifyCodebaseDrift: QueryHandler = async (_args, projectDir) => {
  try {
    ...
    // sdk/src/query -> ../../../sdd/bin/sdd-tools.cjs
    // sdk/dist/query -> ../../../sdd/bin/sdd-tools.cjs
    const toolsPath = resolve(here, '..', '..', '..', 'sdd', 'bin', 'sdd-tools.cjs');
