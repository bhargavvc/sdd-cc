...
export interface QueryDispatchDeps {
  ...
  resolveSddToolsPath: (projectDir: string) => string;
  ...
  if (pickIdx !== -1) {
    ...
    if (queryArgs.length === 0 || !queryArgs[0]) {
      return {
        queryArgs,
        error: dispatchFailure(validationError({
          message: 'Error: "sdd-sdk query" requires a command',
...
export async function runQueryDispatch(deps: QueryDispatchDeps, queryArgv: string[]): Promise<QueryDispatchResult> {
  ...
  if (!normCmd || !String(normCmd).trim()) {
    return fail(validationError({ message: 'Error: "sdd-sdk query" requires a command', details: { reason: 'empty_normalized_command' } }));
  }
  ...
  if (plan.mode === 'cjs') {
    if (canUseCjsFallback({ cjsFallbackEnabled: deps.cjsFallbackEnabled })) {
      try {
        const sddPath = deps.resolveSddToolsPath(deps.projectDir);
        return await runCjsFallbackDispatch({
          projectDir: deps.projectDir,
          sddToolsPath: sddPath,
          normCmd,
          normArgs,
          ws: deps.ws,
          pickField,
  ...
  if (helpFlagPresent && matched.mutation) {
    return dispatchSuccess(
      formatSuccess(
        { help: `Usage: sdd-sdk query ${matched.canonical} [args...]` },
        undefined,
      ),
    );
  }
