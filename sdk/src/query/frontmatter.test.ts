...
describe('extractFrontmatterLeading', () => {
  it('parses only the first leading block (sdd-tools.cjs / frontmatter.cjs parity)', () => {
    const content = '---\nfirst: 1\n---\n---\nsecond: 2\n---\nbody';
...
describe('frontmatterGet', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-fm-'));
  });
  ...
  it('throws SDDError for null bytes in path', async () => {
    const { SDDError } = await import('../errors.js');
    await expect(frontmatterGet(['bad\0path.md'], tmpDir)).rejects.toThrow(SDDError);
  });
});
