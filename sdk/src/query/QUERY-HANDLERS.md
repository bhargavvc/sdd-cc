...
This document records contracts for the typed query layer consumed by `sdd-sdk query` and programmatic `createRegistry()` callers.
...
CJS routing seams mirror these families with thin adapters (`state/verify/init/phase/phases/validate/roadmap-command-router.cjs`) so `sdd-tools.cjs` stays orchestration-only.

## SDK Runtime Bridge Module (`SDDTools` path)

`SDDTools` dispatch routes through `sdk/src/query-runtime-bridge.ts`.
...
## `sdd-sdk query` routing

1. **`normalizeQueryCommand()`** (`query-command-resolution-strategy.ts`) — maps the first argv tokens to the same **command + subcommand** patterns as `sdd-tools` `runCommand()` where needed (e.g. `state json` → `state.json`, `init execute-phase 9` → `init.execute-phase` with args `['9']`, `scaffold …` → `phase.scaffold`). Re-exported from **`@bhargavvc/sdk`** and **`createRegistry`’s module** (`sdk/src/query/index.ts`) so programmatic callers can mirror CLI tokenization without importing a deep path.
...
4. **CJS fallback (CLI)**: if nothing matches a registered handler and `SDD_QUERY_FALLBACK` is not `off`/`never`/`false`/`0`, the CLI shells out to `sdd-tools.cjs` with argv derived from the normalized tokens (dotted commands are split into CJS-style segments). stderr receives a short bridge warning. Set `SDD_QUERY_FALLBACK=off` for strict mode (parity tests). CLI-only commands such as `graphify` rely on this path until native handlers exist.
...
- **Validation and programmer errors**: Handlers throw `SDDError` with an `ErrorClassification` (e.g. missing required args, invalid phase). The Dispatch Policy Module maps native failures into structured dispatch errors.
...
- `QUERY_MUTATION_COMMANDS` in `index.ts` lists every command name (including space-delimited aliases) that performs durable writes. It drives optional `SDDEventStream` wrapping so mutations emit structured events.
...
- `**intel.update`** / `**intel update**` matches CJS `intel.cjs` `intelUpdate` **JSON** (not an in-process graph refresh): when intel is enabled it returns `{ action: 'spawn_agent', message: '...' }`; when disabled, `{ disabled: true, message: '...' }`. The **sdd-intel-updater** agent performs the actual refresh after spawn. Golden tests use full `toEqual` vs `sdd-tools.cjs` on this repo’s intel config.

## Session correlation (`sessionId`)

- `createRegistry(eventStream, sessionId)` threads the optional `sessionId` string into mutation-related events emitted via `eventStream`. `SDDTools` accepts `sessionId` in its constructor and forwards it to `createRegistry`; `SDD` accepts `sessionId` in `SDDOptions` and passes it through `createTools()`. When omitted, `sessionId` is empty.
...
No `sdd-tools.cjs` mirror — agents use these instead of shell `ls`/`find`/`grep`:
...
- **`state.load`** / `state load` — port of **`cmdStateLoad`** (`state-project-load.ts` `stateProjectLoad`): `{ config, state_raw, state_exists, roadmap_exists, config_exists }`; **`config`** comes from **`sdd/bin/lib/core.cjs`** `loadConfig`, but discovery now routes through the **SDK Package Seam Module** (`sdk-package-compatibility.ts`) so install-layout probing stays behind one compatibility Adapter. Read-only golden: full `toEqual` vs `state load`. If `core.cjs` cannot be resolved, dispatch throws **`SDDError`** with the checked probe list (document for minimal `@bhargavvc/sdk`-only installs).

`stateExtractField` in `state-document.ts` (re-exported by `helpers.ts`) uses **horizontal whitespace only** after `Field:` so YAML keys such as lowercase `progress:` in frontmatter are not mistaken for the body `Progress:` line (see `sdd/bin/lib/state-document.cjs` — same rule).

## Golden parity: coverage and exceptions

Subprocess reference: `captureSddToolsOutput()` / `captureSddToolsStdout()` → `sdd/bin/sdd-tools.cjs` (`sdk/src/golden/capture.ts`). Plain-text commands (e.g. `config-path`) use stdout string comparison in `read-only-parity.integration.test.ts`.
...
| ------------------------ | ----- |
| `resolve-model` | Args e.g. `sdd-planner`; returns `reasoning_effort` when the selected runtime tier defines one. |
...
| `skill-manifest` | No args; full manifest parity with `init.cjs` `buildSkillManifest` / `cmdSkillManifest`. Handler uses `extractFrontmatterLeading` (first `---` block) like CJS `frontmatter.cjs` `extractFrontmatter` — not TS `extractFrontmatter` (last block), so skills with multiple `---` sections match CJS. Runtime-global skill roots now route through the **Runtime-Global Skills Policy Module**; legacy import-only skill root discovery (`~/.claude/sdd/skills`) routes through the **SDK Package Seam Module**. |
| `validate.agents` | No args; `agents_dir` matches `core.cjs` `getAgentsDir` (`SDD_AGENTS_DIR` or `sdk/dist/query/../../../agents` in this monorepo — same absolute path as CLI). `MODEL_PROFILES` / `expected` list stays aligned with `sdd/bin/lib/model-profiles.cjs`. |
...
These handlers implement `.planning/research/decision-routing-audit.md` — **no `sdd-tools.cjs` mirror yet** (orchestration JSON only). Invoke via `sdd-sdk query` / `registry.dispatch()` after `normalizeQueryCommand()` where argv uses `check …` / `detect …` / `route …` prefixes.
...
| `route.next-action` / `route next-action` | Suggested next slash command from `next.md`-style rules (`/sdd-discuss-phase`, `/sdd-execute-phase`, `/sdd-resume-work`, gates, etc.). |
...
Authoritative CJS entry points: `runCommand` `switch (command)` in `sdd/bin/sdd-tools.cjs`. SDK entry points: `createRegistry()` in `sdk/src/query/index.ts`.
...
| SDK dispatch                                | Notes                                                                                                                                            |
| ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `**phases.archive`** / `**phases archive**` | CJS `phases` supports only `**list**` and `**clear**`; archive behavior is available via SDK (and workflows), not as `sdd-tools phases archive`. |


### Matrix: top-level `sdd-tools` command → SDK
...
- `**detect-custom-files`**: requires `--config-dir <path>`; scans installer manifest vs SDD-managed dirs (`detect-custom-files.ts`).
