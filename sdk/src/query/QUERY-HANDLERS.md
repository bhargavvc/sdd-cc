...
- **`state.load`** / `state load` — port of **`cmdStateLoad`** (`state-project-load.ts` `stateProjectLoad`): `{ config, state_raw, state_exists, roadmap_exists, config_exists }`; **`config`** comes from **`sdd/bin/lib/core.cjs`** `loadConfig` (resolved via the same candidate paths as a normal SDD install). Read-only golden: full `toEqual` vs `state load`. If `core.cjs` cannot be resolved, dispatch throws **`SDDError`** (document for minimal `@bhargavvc/sdk`-only installs).

`stateExtractField` in `helpers.ts` uses **horizontal whitespace only** after `Field:` so YAML keys such as lowercase `progress:` in frontmatter are not mistaken for the body `Progress:` line (see `sdd/bin/lib/state.cjs` — same rule).
...
| ------------------------ | ----- |
| `resolve-model` | Args e.g. `sdd-planner`. |
