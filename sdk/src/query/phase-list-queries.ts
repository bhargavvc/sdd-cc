 ...
 * for agents (replaces shell `ls` / `find` patterns). SDK-only; no sdd-tools.cjs mirror.
 */
...
export const phaseListArtifacts: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (typeIdx === -1 || !args[typeIdx + 1]) {
    throw new SDDError('--type context|summary|verification|research required', ErrorClassification.Validation);
  }
  const phase = args[0];
  ...
  if (!allowed.includes(rawType as ArtifactType)) {
    throw new SDDError(`invalid --type ${rawType}`, ErrorClassification.Validation);
  }
...
export const phaseListPlans: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (wsIdx !== -1) {
    ...
    if (!schemaKey) {
      throw new SDDError('--with-schema requires a field name', ErrorClassification.Validation);
    }
  }
