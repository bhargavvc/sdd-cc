...
beforeEach(async () => {
  tmpDir = await mkdtemp(join(tmpdir(), 'sdd-plans-'));
...
describe('phaseListPlans', () => {
  ...
  it('throws when phase missing', async () => {
    await expect(phaseListPlans([], tmpDir)).rejects.toThrow(SDDError);
  });
});
...
describe('phaseListArtifacts', () => {
  ...
  it('throws without --type', async () => {
    await expect(phaseListArtifacts(['9'], tmpDir)).rejects.toThrow(SDDError);
  });
});
