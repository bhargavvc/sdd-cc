...
export const validateHealth: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (!existsSync(configPath)) {
    ...
    try {
      ...
      addIssue('error', 'E005', `config.json: JSON parse error - ${msg}`, 'Run /sdd-health --repair to reset to defaults', true);
...
/**
 * Default agents directory — mirrors `getAgentsDir` in `sdd/bin/lib/core.cjs`:
 * `SDD_AGENTS_DIR`, else `../../../agents` relative to this module (`sdk/dist/query` → monorepo
 * root), matching `core.cjs` (`sdd/bin/lib` → same repo `agents/`).
 */
function getAgentsDirForValidateAgents(): string {
  if (process.env.SDD_AGENTS_DIR) return process.env.SDD_AGENTS_DIR;
  return resolveBundledAgentsDir();
}

/**
 * Validate SDD agent file installation under the managed agents directory.
 *
 ...
 *   < 60%   healthy
 *   60–70%  warning   → recommend /sdd-thread
 ...
 *
 * Mirror of sdd/bin/lib/context-utilization.cjs (the legacy
 * sdd-tools.cjs path uses the CJS module). Keep both in sync.
 */
...
const CONTEXT_RECOMMENDATIONS: Record<string, string | null> = {
  healthy: null,
  warning: 'Context is approaching the fracture zone — consider /sdd-thread to continue in a fresh window.',
  critical: 'Reasoning quality may degrade past 70% utilization (fracture point). Run /sdd-thread now to preserve output quality.',
};
