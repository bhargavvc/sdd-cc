 ...
 *
 * These handlers are direct TypeScript ports of sdd-tools.cjs functions:
...
export interface QueryResult<T = unknown> {
   ...
   * Used by `agent-skills` so workflows embedding `$(sdd-sdk query …)` receive
 ...
 *
 * Port of `cmdGenerateSlug` from `sdd/bin/lib/commands.cjs`.
 ...
 * @throws SDDError with Validation classification if text is missing or empty
 */
export const generateSlug: QueryHandler = async (args, _projectDir) => {
  const text = args[0];
  if (!text) {
    throw new SDDError('text required for slug generation', ErrorClassification.Validation);
  }
 ...
 *
 * Port of `cmdCurrentTimestamp` from `sdd/bin/lib/commands.cjs`.
