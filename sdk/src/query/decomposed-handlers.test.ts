...
beforeEach(async () => {
  tmpDir = await mkdtemp(join(tmpdir(), 'sdd-stubs-'));
...
describe('agentSkills', () => {
  it('returns empty string when agent_skills config is missing', async () => {
    const result = await agentSkills(['sdd-executor'], tmpDir);
 ...
 * arg. phasesArchive read args[0] and threw SDDError('version required for
...
describe('milestoneComplete', () => {
  ...
  it('does not call phasesArchive with empty args (regression: bug #2644)', async () => {
    // If the old bug were present, this would return { completed: false, reason: 'SDDError: version required for phases archive' }
  ...
  it('throws SDDError when version arg is missing (not masked as completed: false)', async () => {
...
describe('docsInit', () => {
  it('returns docs context matching sdd-tools docs-init', async () => {
