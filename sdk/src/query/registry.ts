 ...
 * to handler functions. Unknown keys passed to `dispatch()` throw `SDDError`.
 * The `sdd-sdk query` CLI resolves argv with `resolveQueryArgv()` before dispatch;
 * there is no automatic delegation to `sdd-tools.cjs`.
 *
 * Also exports `extractField` — a TypeScript port of the `--pick` field
 * extraction logic from sdd-tools.cjs (lines 365-382).
 *
 * @example
 * ```typescript
 ...
 *
 * Direct port of `extractField()` from sdd-tools.cjs (lines 365-382).
...
/**
 * Map argv after `sdd-sdk query` to a registered handler key and remaining args.
