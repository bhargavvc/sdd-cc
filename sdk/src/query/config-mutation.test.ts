...
beforeEach(async () => {
  tmpDir = await mkdtemp(join(tmpdir(), 'sdd-cfgmut-'));
...
describe('configSet', () => {
  ...
  it('rejects invalid key with SDDError', async () => {
    const { configSet } = await import('./config-mutation.js');
    await writeFile(
      join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({}),
    );
    await expect(configSet(['totally_bogus_key', 'value'], tmpDir)).rejects.toThrow(SDDError);
  });
...
describe('configSetModelProfile', () => {
  ...
  it('rejects invalid profile with SDDError', async () => {
    const { configSetModelProfile } = await import('./config-mutation.js');
    await writeFile(
      join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({}),
    );
    await expect(configSetModelProfile(['invalid_profile'], tmpDir)).rejects.toThrow(SDDError);
  });
...
describe('configNewProject', () => {
  ...
  it('validates ship.pr_body_sections choices before writing config', async () => {
    ...
    await expect(configNewProject([choices], tmpDir)).rejects.toThrow(SDDError);
  });
