 ...
 *
 * Ported from sdd/bin/lib/roadmap.cjs and core.cjs.
...
export async function getMilestoneInfo(projectDir: string, workstream?: string): Promise<{ version: string; name: string }> {
  try {
    ...
    // List-format: in flight / active (SDD ROADMAP template uses 🟡 for current milestone)
...
export async function extractCurrentMilestone(content: string, projectDir: string, workstream?: string): Promise<string> {
  ...
  // matches the literal `Phase Details` label (canonical per SDD ROADMAP
 ...
 *
 * Delegates to sdd-tools.cjs which holds the full annotation logic.
...
export const roadmapAnnotateDependencies: QueryHandler = async (args, projectDir) => {
  ...
  if (result.error) {
    return { data: { updated: false, reason: result.error.message || 'sdd-tools invocation failed' } };
  }

  if (result.status !== 0) {
    return { data: { updated: false, reason: result.stderr?.trim() || 'sdd-tools error' } };
  }

  try {
    return { data: JSON.parse(result.stdout.trim()) };
  } catch {
    return { data: { updated: false, reason: 'failed to parse sdd-tools output' } };
  }
};
...
export const requirementsMarkComplete: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (reqIds.length === 0) {
    throw new SDDError('no valid requirement IDs found', ErrorClassification.Validation);
  }
