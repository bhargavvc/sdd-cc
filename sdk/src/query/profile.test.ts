...
describe('writeProfile', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-profile-'));
...
describe('generateDevPreferences', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-dev-preferences-'));
    await mkdir(join(tmpDir, '.planning'), { recursive: true });
  });
  ...
  it('writes to runtime-global skills dir by default', async () => {
    ...
    try {
      ...
      const data = result.data as Record<string, unknown>;
      const expectedPath = join(codexHome, 'skills', 'sdd-dev-preferences', 'SKILL.md');
...
describe('learningsCopy', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-learn-'));
    await mkdir(join(tmpDir, '.planning'), { recursive: true });
  });
