...
describe('runQueryDispatch', () => {
  ...
  it('runs cjs fallback and formats text mode', async () => {
    ...
    expect(out.stderr[0]).toContain('falling back to sdd-tools.cjs');
  });

  it('returns structured fallback failure when resolveSddToolsPath throws', async () => {
    const registry = createRegistry();
    const out = await runQueryDispatch({
      registry,
      projectDir: tmpDir,
      cjsFallbackEnabled: true,
      resolveSddToolsPath: () => { throw new Error('path boom'); },
      dispatchNative: async () => ({ data: {} }),
      topology: createCommandTopology(registry),
    }, ['unknown-cmd']);
  ...
  it('maps native timeout to native_timeout kind with details', async () => {
    ...
    const out = await runQueryDispatch({
      ...
      cjsFallbackEnabled: true,
      resolveSddToolsPath: () => '',
      dispatchNative: async () => { throw new Error('sdd-tools timed out after 30000ms: state load'); },
      topology: createCommandTopology(registry),
    }, ['state', 'load']);
  ...
  it('maps typed native timeout to native_timeout kind with details', async () => {
    ...
    const out = await runQueryDispatch({
      ...
      cjsFallbackEnabled: true,
      resolveSddToolsPath: () => '',
      dispatchNative: async () => { throw SDDToolsError.timeout('timed out', 'state', ['load'], '', 30000); },
      topology: createCommandTopology(registry),
    }, ['state', 'load']);
