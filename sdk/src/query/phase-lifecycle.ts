 ...
 *
 * Ported from sdd/bin/lib/phase.cjs and commands.cjs.
...
function assertNoNullBytes(value: string, label: string): void {
  if (value.includes('\0')) {
    throw new SDDError(`${label} contains null byte`, ErrorClassification.Validation);
  }
}
...
function assertSafePhaseDirName(dirName: string, label = 'phase directory'): void {
  if (/[/\\]|\.\./.test(dirName)) {
    throw new SDDError(`${label} contains invalid path segments`, ErrorClassification.Validation);
  }
}

function assertSafeProjectCode(code: string): void {
  if (code && /[/\\]|\.\./.test(code)) {
    throw new SDDError('project_code contains invalid characters', ErrorClassification.Validation);
  }
}
...
export const phaseAdd: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (!description) {
    throw new SDDError('description required for phase add', ErrorClassification.Validation);
  }
  ...
  await readModifyWriteRoadmapMd(projectDir, async (rawContent) => {
    ...
    if (customId || config.phase_naming === 'custom') {
      ...
      if (!newPhaseId) {
        throw new SDDError('--id required when phase_naming is "custom"', ErrorClassification.Validation);
      }
  ...
  if (!dirName) {
    throw new SDDError('Phase directory name was not computed', ErrorClassification.Execution);
  }
  if (newPhaseId === '') {
    throw new SDDError('Phase ID was not computed', ErrorClassification.Execution);
  }
...
export const phaseAddBatch: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (descIdx !== -1 && args[descIdx + 1] !== undefined) {
    try {
      ...
      if (!Array.isArray(parsed)) {
        throw new SDDError('--descriptions must be a JSON array', ErrorClassification.Validation);
      }
      descriptions = parsed.map((x) => String(x));
    } catch (e) {
      if (e instanceof SDDError) throw e;
      throw new SDDError('--descriptions must be a valid JSON array', ErrorClassification.Validation);
    }
  } else {
  ...
  if (descriptions.length === 0) {
    throw new SDDError('descriptions array required for phase add-batch', ErrorClassification.Validation);
  }

  for (const d of descriptions) {
    assertNoNullBytes(d, 'description');
    if (!d.trim()) {
      throw new SDDError('description must be non-empty', ErrorClassification.Validation);
    }
  }

  const roadmapPath = planningPaths(projectDir, workstream).roadmap;
  if (!existsSync(roadmapPath)) {
    throw new SDDError('ROADMAP.md not found', ErrorClassification.Validation);
  }
...
export const phaseInsert: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (!afterPhase || !description) {
    throw new SDDError('after-phase and description required for phase insert', ErrorClassification.Validation);
  }
  ...
  await readModifyWriteRoadmapMd(projectDir, async (rawContent) => {
    ...
    if (!targetPattern.test(content)) {
      throw new SDDError(`Phase ${afterPhase} not found in ROADMAP.md`, ErrorClassification.Validation);
    }
    ...
    if (!headerMatch) {
      throw new SDDError(`Could not find Phase ${afterPhase} header`, ErrorClassification.Execution);
    }
  ...
  if (!decimalPhase) {
    throw new SDDError('Decimal phase was not computed', ErrorClassification.Execution);
  }
  if (!dirName) {
    throw new SDDError('Phase directory name was not computed', ErrorClassification.Execution);
  }
 ...
 *
 * @param args - Positional `[type, phase, name?]` **or** sdd-tools style
...
export const phaseScaffold: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (!type) {
    throw new SDDError('type required for scaffold', ErrorClassification.Validation);
  }
  ...
  if (type === 'phase-dir') {
    if (!phase || !name) {
      throw new SDDError('phase and name required for phase-dir scaffold', ErrorClassification.Validation);
    }
  ...
  if (phase && !phaseInfo) {
    throw new SDDError(`Phase ${phase} directory not found`, ErrorClassification.Blocked);
  }
  ...
  switch (type) {
    ...
    default:
      throw new SDDError(`Unknown scaffold type: ${type}`, ErrorClassification.Validation);
  }
...
export const phaseRemove: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (!targetPhase) {
    throw new SDDError('phase number required for phase remove', ErrorClassification.Validation);
  }
  ...
  try {
    ...
    if (isDecimal) {
      ...
      if (parts.length < 2 || !parts[1]) {
        throw new SDDError(`Invalid decimal phase identifier: ${targetPhase}`, ErrorClassification.Validation);
      }
      const decimalPart = parseInt(parts[1], 10);
      if (isNaN(decimalPart)) {
        throw new SDDError(`Invalid decimal part in phase: ${targetPhase}`, ErrorClassification.Validation);
      }
...
export const phaseComplete: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (!phaseNum) {
    throw new SDDError('phase number required for phase complete', ErrorClassification.Validation);
  }
  ...
  if (!phaseInfo) {
    throw new SDDError(`Phase ${phaseNum} not found`, ErrorClassification.Validation);
  }
...
export const phaseNextDecimal: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (!basePhase) {
    throw new SDDError('base phase number required', ErrorClassification.Validation);
  }
...
export const phasesArchive: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (!version) {
    throw new SDDError('version required for phases archive', ErrorClassification.Validation);
  }
...
/** Port of `parseMultiwordArg` in `sdd-tools.cjs`. */
...
export const milestoneComplete: QueryHandler = async (args, projectDir, workstream) => {
  const version = args[0];
  if (!version) {
    throw new SDDError('version required for milestone complete (e.g., v1.0)', ErrorClassification.Validation);
  }
  assertNoNullBytes(version, 'version');
  ...
  if (existsSync(statePath)) {
    await readModifyWriteStateMdFull(projectDir, (stateContent) => {
      ...
      if (operatorPattern.test(next)) {
        next = next.replace(
          operatorPattern,
          `$1\n- Start the next milestone with /sdd-new-milestone\n\n`,
        );
      } else {
        next = `${next.trimEnd()}\n\n## Operator Next Steps\n\n- Start the next milestone with /sdd-new-milestone\n`;
      }
