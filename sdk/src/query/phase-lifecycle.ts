...
export const phaseRemove: QueryHandler = async (args, projectDir, workstream) => {
  ...
  for (const token of args) {
    ...
    if (token.startsWith('--')) {
      throw new SDDError(`phase remove does not support ${token}`, ErrorClassification.Validation);
    }
    positional.push(token);
  }

  if (positional.length > 1) {
    throw new SDDError('phase remove accepts exactly one phase number', ErrorClassification.Validation);
  }
  ...
  if (!targetDir) {
    throw new SDDError(`Phase ${targetPhase} not found`, ErrorClassification.Validation);
  }
...
export const milestoneComplete: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (version === '--help' || version === '-h') {
    throw new SDDError(
      `"${version}" is not a valid milestone version; see \`sdd-sdk query --help\` for command list`,
      ErrorClassification.Validation,
    );
  }
  assertNoNullBytes(version, 'version');
