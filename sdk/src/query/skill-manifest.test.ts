...
describe('skill-manifest', () => {
  ...
  it('resolves legacy import-only skills root through SDK Package Seam Module', () => {
    const manifest = buildSkillManifest('/tmp/project-that-does-not-exist');
    const legacyRoot = manifest.roots.find(root => root.root === '.claude/sdd/skills');
  ...
  it('discovers skills from explicit --skills-dir without scanning global roots', async () => {
    const tmpDir = await mkdtemp(join(tmpdir(), 'sdd-skill-manifest-'));
    try {
  ...
  it('legacy skills dir helper points at ~/.claude/sdd/skills', () => {
    expect(resolveLegacySkillsDir()).toBe(join(homedir(), '.claude', 'sdd', 'skills'));
  });
});
