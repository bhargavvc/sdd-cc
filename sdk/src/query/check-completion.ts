...
export const checkCompletion: QueryHandler = async (args, projectDir) => {
  const scope = args[0];
  if (!scope) {
    throw new SDDError('scope required for check completion (phase|milestone)', ErrorClassification.Validation);
  }
  ...
  if (scope === 'phase') {
    ...
    if (!phaseNum) {
      throw new SDDError('phase number required for check completion phase', ErrorClassification.Validation);
    }
