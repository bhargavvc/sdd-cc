 ...
 *
 * Ported from sdd/bin/lib/core.cjs and state.cjs.
...
export function detectRuntime(config?: { runtime?: unknown }): Runtime {
  const envValue = process.env.SDD_RUNTIME;
...
/**
 * Resolve the SDD agents directory for a given runtime.
 *
 * Precedence:
 *   1. `SDD_AGENTS_DIR` — explicit SDK override (wins over runtime selection)
...
export function planningPaths(projectDir: string, workstream?: string): PlanningPaths {
  ...
  // Validate env workstream before use: invalid SDD_WORKSTREAM falls back to
 ...
 *
 * Ported from `sdd/bin/lib/core.cjs:findProjectRoot` so that
 * `sdd-sdk query` resolves the same parent `.planning/` root as the legacy
 * `sdd-tools.cjs` CLI when invoked inside a `sub_repos`-listed child repo.
 *
...
export async function resolvePathUnderProject(projectDir: string, userPath: string): Promise<string> {
  ...
  if (rel.startsWith('..') || (isAbsolute(rel) && rel.length > 0)) {
    throw new SDDError('path escapes project directory', ErrorClassification.Validation);
  }
