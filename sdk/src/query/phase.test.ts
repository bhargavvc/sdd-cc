...
beforeEach(async () => {
  tmpDir = await mkdtemp(join(tmpdir(), 'sdd-phase-test-'));
...
describe('findPhase', () => {
  ...
  it('throws SDDError with Validation classification when no args', async () => {
    await expect(findPhase([], tmpDir)).rejects.toThrow(SDDError);
    try {
      await findPhase([], tmpDir);
    } catch (err) {
      expect((err as SDDError).classification).toBe('validation');
    }
  });
...
describe('phasePlanIndex', () => {
  ...
  it('throws SDDError with Validation classification when no args', async () => {
    await expect(phasePlanIndex([], tmpDir)).rejects.toThrow(SDDError);
    try {
  ...
  it('#3266: cycle detection throws SDDError naming the cycle nodes', async () => {
    ...
    expect(thrownError).toBeInstanceOf(SDDError);
    const msg = (thrownError as SDDError).message;
