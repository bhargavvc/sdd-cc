 ...
 *
 * Ported from sdd/bin/lib/workstream.cjs.
...
export const workstreamCreate: QueryHandler = async (args, projectDir) => {
  ...
  if (!existsSync(baseDir)) {
    return { data: { created: false, reason: '.planning/ directory not found — run /sdd-new-project first' } };
  }
...
export const workstreamStatus: QueryHandler = async (args, projectDir) => {
  const name = args[0];
  if (!name) {
    throw new SDDError('workstream name required. Usage: workstream status <name>', ErrorClassification.Validation);
  }
  if (/[/\\]/.test(name) || name === '.' || name === '..') {
    throw new SDDError('Invalid workstream name', ErrorClassification.Validation);
  }
