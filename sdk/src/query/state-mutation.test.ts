...
describe('acquireStateLock / releaseStateLock', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-lock-'));
  });
...
describe('stateUpdate', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-state-update-'));
...
describe('statePatch', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-state-patch-'));
    await setupTestProject(tmpDir);
  });
...
describe('stateBeginPhase', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-state-begin-'));
    await setupTestProject(tmpDir);
  });
...
describe('stateAdvancePlan', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-state-advance-'));
    await setupTestProject(tmpDir);
  });
...
describe('stateAddDecision', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-state-decision-'));
    await setupTestProject(tmpDir);
  });
...
describe('stateAddRoadmapEvolution', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-state-evo-'));
  });
...
`;
  ...
  it('throws SDDError(Validation) when phase is missing', async () => {
    await setupTestProject(tmpDir);
    const { stateAddRoadmapEvolution } = await import('./state-mutation.js');
    ...
    await expect(stateAddRoadmapEvolution(
      ...
      return err instanceof SDDError && err.classification === ErrorClassification.Validation;
    });
  });

  it('throws SDDError(Validation) when action is missing', async () => {
    await setupTestProject(tmpDir);
    const { stateAddRoadmapEvolution } = await import('./state-mutation.js');
    const { SDDError, ErrorClassification } = await import('../errors.js');
  ...
  it('throws SDDError(Validation) on invalid action', async () => {
    await setupTestProject(tmpDir);
    const { stateAddRoadmapEvolution } = await import('./state-mutation.js');
    const { SDDError, ErrorClassification } = await import('../errors.js');
...
describe('stateRecordSession', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-state-session-'));
    await setupTestProject(tmpDir);
  });
...
describe('Bug #2613: STATE.md frontmatter preservation through mutations', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-state-2613-'));
  });
...
describe('stateMilestoneSwitch', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-milestone-switch-'));
  });
...
describe('statePrune current phase extraction (#3471)', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-sdk-stateprune-'));
  });
