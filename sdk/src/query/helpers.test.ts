...
describe('planningPaths', () => {
  ...
  it('uses SDD_PROJECT env when no explicit workstream is provided', () => {
    process.env['SDD_PROJECT'] = 'proj-scope';
    const paths = planningPaths('/proj');
    expect(paths.planning).toContain('/proj/.planning/proj-scope');
  });

  it('explicit workstream overrides SDD_PROJECT env', () => {
    process.env['SDD_PROJECT'] = 'proj-scope';
...
describe('resolvePathUnderProject', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-path-'));
  ...
  it('rejects paths that escape the project root', async () => {
    await expect(resolvePathUnderProject(tmpDir, '../../etc/passwd')).rejects.toThrow(SDDError);
  });
});
...
const RUNTIME_ENV_VARS = [
  'SDD_AGENTS_DIR', 'SDD_RUNTIME', 'CLAUDE_CONFIG_DIR', 'OPENCODE_CONFIG_DIR',
...
describe('detectRuntime', () => {
  ...
  it('uses SDD_RUNTIME when set to a known runtime', () => {
    process.env.SDD_RUNTIME = 'codex';
    expect(detectRuntime()).toBe('codex');
  });

  it('falls back to config.runtime when SDD_RUNTIME unset', () => {
    expect(detectRuntime({ runtime: 'gemini' })).toBe('gemini');
  });

  it('SDD_RUNTIME wins over config.runtime', () => {
    process.env.SDD_RUNTIME = 'codex';
    expect(detectRuntime({ runtime: 'gemini' })).toBe('codex');
  });

  it('unknown SDD_RUNTIME falls through to config then claude', () => {
    process.env.SDD_RUNTIME = 'bogus';
    expect(detectRuntime({ runtime: 'gemini' })).toBe('gemini');
    expect(detectRuntime()).toBe('claude');
  });
...
describe('resolveAgentsDir (runtime-aware)', () => {
  ...
  it('SDD_AGENTS_DIR short-circuits regardless of runtime', () => {
    process.env.SDD_AGENTS_DIR = '/explicit/agents';
...
describe('findProjectRoot (multi-repo .planning resolution)', () => {
  ...
  beforeEach(async () => {
    workspace = await mkdtemp(join(tmpdir(), 'sdd-find-root-'));
  });
