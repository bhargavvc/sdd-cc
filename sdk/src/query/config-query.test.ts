...
import { SDDError, ErrorClassification, exitCodeFor } from '../errors.js';
...
beforeEach(async () => {
  tmpDir = await mkdtemp(join(tmpdir(), 'sdd-cfg-'));
...
describe('configGet', () => {
  ...
  it('throws SDDError when no key provided', async () => {
    const { configGet } = await import('./config-query.js');
    await expect(configGet([], tmpDir)).rejects.toThrow(SDDError);
  });

  it('throws SDDError for nonexistent key', async () => {
    const { configGet } = await import('./config-query.js');
    await writeFile(
      join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({ model_profile: 'quality' }),
    );
    await expect(configGet(['nonexistent.key'], tmpDir)).rejects.toThrow(SDDError);
  });

  it('throws SDDError that maps to exit code 1 for missing key (bug #2544)', async () => {
    const { configGet } = await import('./config-query.js');
    await writeFile(
      join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({ model_profile: 'quality' }),
    ...
    try {
      ...
      expect(err).toBeInstanceOf(SDDError);
      const sddErr = err as SDDError;
      ...
      expect(sddErr.classification).toBe(ErrorClassification.Execution);
      expect(exitCodeFor(sddErr.classification)).toBe(1);
    }
  });

  it('throws SDDError that maps to exit code 1 when traversing into non-object (bug #2544)', async () => {
    const { configGet } = await import('./config-query.js');
    await writeFile(
      join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({ model_profile: 'quality' }),
  ...
  it('reads raw config without merging defaults', async () => {
    ...
    await expect(configGet(['workflow.auto_advance'], tmpDir)).rejects.toThrow(SDDError);
  });
});
...
describe('resolveModel', () => {
  it('returns model and profile for known agent', async () => {
    ...
    const result = await resolveModel(['sdd-planner'], tmpDir);
  ...
  it('resolves shipped-but-previously-missing agents without unknown_agent (#3229)', async () => {
    ...
    const result = await resolveModel(['sdd-code-reviewer'], tmpDir);
    const data = result.data as Record<string, unknown>;
  ...
  it('throws SDDError when no agent type provided', async () => {
    const { resolveModel } = await import('./config-query.js');
    await expect(resolveModel([], tmpDir)).rejects.toThrow(SDDError);
  });
  ...
  it('runtime codex model_profile_overrides beat resolve_model_ids omit (#3358)', async () => {
    ...
    const planner = (await resolveModel(['sdd-planner'], tmpDir)).data as Record<string, unknown>;
    const executor = (await resolveModel(['sdd-executor'], tmpDir)).data as Record<string, unknown>;
  ...
  it('resolveModel uses workstream config when --ws is specified', async () => {
    const { resolveModel } = await import('./config-query.js');
    // Root config: balanced profile → sdd-executor resolves to 'sonnet'
    await writeFile(
      join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({ model_profile: 'balanced' }),
    );
    // Workstream config: quality profile → sdd-executor resolves to 'opus'
    ...
    const rootResult = await resolveModel(['sdd-executor'], tmpDir);
    ...
    const wsResult = await resolveModel(['sdd-executor'], tmpDir, 'frontend');
...
describe('MODEL_PROFILES', () => {
  it('contains every shipped sdd agent file on disk (#3229)', async () => {
