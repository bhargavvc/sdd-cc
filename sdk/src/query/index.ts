 ...
 * as they are migrated from sdd-tools.cjs.
 *
 * @example
 * ```typescript
...
import {
  SDDEventType,
  type SDDEvent,
  type SDDStateMutationEvent,
  type SDDConfigMutationEvent,
  type SDDFrontmatterMutationEvent,
  type SDDGitCommitEvent,
  type SDDTemplateFillEvent,
} from '../types.js';
 ...
 */
function buildMutationEvent(cmd: string, args: string[], result: QueryResult): SDDEvent {
  const base = {
  ...
  if (cmd.startsWith('frontmatter.') || cmd.startsWith('frontmatter ')) {
    return {
      ...
      fields: args.slice(1),
      success: true,
    } as SDDFrontmatterMutationEvent;
  }
 ...
 */
export function createRegistry(eventStream?: SDDEventStream): QueryRegistry {
