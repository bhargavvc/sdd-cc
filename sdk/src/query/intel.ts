 ...
 *
 * Ported from sdd/bin/lib/intel.cjs.
 ...
 * Returns `{ file, exports, method }` with `file` as a resolved absolute path (matches `sdd-tools.cjs`).
 */
...
/**
 * `sdd-tools intel update` entry point: returns the same JSON as `intel.cjs` `intelUpdate`.
 * Does not run the full graph refresh in-process — that work is done by the
 * **sdd-intel-updater** agent after spawn. When `.planning/intel/` is disabled in config,
