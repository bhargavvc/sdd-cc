/**
 * Next slash-command suggestion for `/sdd-next`-style routing (`route.next-action`).
 *
 * Deterministic routing from STATE.md, ROADMAP, and phase directories.
 * See `.planning/research/decision-routing-audit.md` §3.1 and `sdd/workflows/next.md`.
 */
...
export const routeNextAction: QueryHandler = async (_args, projectDir, workstream) => {
  ...
  if (sjd.error) {
    return {
      data: {
        command: '/sdd-new-project',
        args: '',
        reason: 'No STATE.md — initialize a SDD project first',
        current_phase: null,
        phase_name: null,
        gates: {
