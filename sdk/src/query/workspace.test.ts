...
describe('resolveWorkspaceContext', () => {
  ...
  it('reads SDD_WORKSTREAM from env', () => {
    process.env['SDD_WORKSTREAM'] = 'backend';
    const ctx = resolveWorkspaceContext();
    expect(ctx.workstream).toBe('backend');
  });

  it('reads SDD_PROJECT from env', () => {
    process.env['SDD_PROJECT'] = 'api-server';
    const ctx = resolveWorkspaceContext();
    expect(ctx.project).toBe('api-server');
  });

  it('reads both vars when both are set', () => {
    process.env['SDD_WORKSTREAM'] = 'ws1';
    process.env['SDD_PROJECT'] = 'proj1';
    const ctx = resolveWorkspaceContext();
