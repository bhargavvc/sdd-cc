...
async function writeManifest(configDir: string, files: Record<string, string>): Promise<void> {
  ...
  await writeFile(
    join(configDir, 'sdd-file-manifest.json'),
...
describe('detectCustomFiles — skills/ parity with CJS port (#3317)', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-3317-skills-'));
  });
  ...
  it('does not flag SDD-owned skill listed in manifest', async () => {
    await writeManifest(tmpDir, {
      'skills/sdd-planner/SKILL.md': '# SDD Planner Skill\n',
    });
    ...
    expect(result.custom_files).not.toContain('skills/sdd-planner/SKILL.md');
  });

  it('still detects custom files in sdd/workflows/ (regression guard)', async () => {
    await writeManifest(tmpDir, {
      'sdd/workflows/plan-phase.md': '# Plan Phase\n',
      'skills/sdd-planner/SKILL.md': '# SDD Planner Skill\n',
    });
    await writeCustomFile(tmpDir, 'sdd/workflows/custom-workflow.md', '# Custom\n');

    const { data } = await detectCustomFiles(['--config-dir', tmpDir], tmpDir);
    const result = data as DetectResult;

    expect(result.custom_files).toContain('sdd/workflows/custom-workflow.md');
  });
