...
describe('workstreamList', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-ws-'));
...
describe('workstreamProgress', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-ws-progress-'));
...
describe('workstreamCreate', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-ws2-'));
    await mkdir(join(tmpDir, '.planning'), { recursive: true });
    await writeFile(join(tmpDir, '.planning', 'config.json'), JSON.stringify({ model_profile: 'balanced' }));
  });
...
describe('workstreamSet root STATE.md mirror sync (#2618 gap 2)', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-ws-set-'));
    await mkdir(join(tmpDir, '.planning'), { recursive: true });
    await writeFile(
