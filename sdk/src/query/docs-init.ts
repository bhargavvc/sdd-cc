 ...
 *
 * Full port of `cmdDocsInit` and helpers from `sdd/bin/lib/docs.cjs`.
 */
...
function hasSddMarker(filePath: string): boolean {
  try {
    const buf = Buffer.alloc(500);
    const fd = openSync(filePath, 'r');
    const bytesRead = readSync(fd, buf, 0, 500, 0);
    closeSync(fd);
    return buf.subarray(0, bytesRead).toString('utf-8').includes(SDD_MARKER);
  } catch {
    return false;
  }
}
 ...
 */
export function scanExistingDocs(cwd: string): Array<{ path: string; has_sdd_marker: boolean }> {
  const MAX_DEPTH = 4;
  const results: Array<{ path: string; has_sdd_marker: boolean }> = [];
...
/**
 * Init payload for docs-update workflow — matches `sdd-tools docs-init` JSON.
...
export const docsInit: QueryHandler = async (_args, projectDir) => {
  ...
  const docModelResult = await resolveModel(['sdd-doc-writer'], projectDir);
