 ...
 *
 * Ported from sdd/bin/lib/config.cjs.
...
// with sdd/bin/lib/config-schema.cjs by a CI parity test (#2653).
...
function validateShipPrBodySections(value: unknown): void {
  ...
  value.forEach((section, index) => {
    ...
    if (!section || typeof section !== 'object' || Array.isArray(section)) {
      throw new SDDError(`${prefix}. Expected an object.`, ErrorClassification.Validation);
    }

    const record = section as Record<string, unknown>;
    const unknownKeys = Object.keys(record).filter((key) => !SHIP_PR_BODY_SECTION_KEYS.has(key));
    if (unknownKeys.length > 0) {
      throw new SDDError(`${prefix}. Unknown field(s): ${unknownKeys.join(', ')}.`, ErrorClassification.Validation);
    }

    if (typeof record.heading !== 'string' || record.heading.trim() === '') {
      throw new SDDError(`${prefix}. heading must be a non-empty string.`, ErrorClassification.Validation);
    }
    if (/[\r\n]/.test(record.heading)) {
      throw new SDDError(`${prefix}. heading must be a single line.`, ErrorClassification.Validation);
    }

    if ('enabled' in record && typeof record.enabled !== 'boolean') {
      throw new SDDError(`${prefix}. enabled must be true or false.`, ErrorClassification.Validation);
    }

    for (const field of ['source', 'fallback', 'template']) {
      if (field in record && typeof record[field] !== 'string') {
        throw new SDDError(`${prefix}. ${field} must be a string.`, ErrorClassification.Validation);
      }
    }
    ...
    if (!hasContent) {
      throw new SDDError(`${prefix}. Provide at least one of source, fallback, or template.`, ErrorClassification.Validation);
    }
    ...
    if (typeof record.template === 'string') {
      ...
      for (const match of tokens) {
        if (!SHIP_PR_BODY_TEMPLATE_TOKENS.has(match[1])) {
          throw new SDDError(`${prefix}. Unsupported template token: {${match[1]}}.`, ErrorClassification.Validation);
        }
      }
    }
  });
 ...
 * @returns QueryResult matching sdd-tools `config-set` JSON: `{ updated, key, value, previousValue }`
 * @throws SDDError with Validation if key is invalid or args missing
 */
export const configSet: QueryHandler = async (args, projectDir, workstream) => {
  const keyPath = args[0];
  const rawValue = args[1];
  if (!keyPath) {
    throw new SDDError('Usage: config-set <key.path> <value>', ErrorClassification.Validation);
  }
  ...
  // config.cjs:362-370 — without this, `sdd-sdk query config-set brave_search XXX`
 ...
 * @throws SDDError with Validation if profile is invalid
 */
...
export const configNewProject: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (args[0] && args[0].trim() !== '') {
    try {
      ...
      throw new SDDError(`Invalid JSON for config-new-project: ${msg}`, ErrorClassification.Validation);
    }
  }
  ...
  // D11: Load global defaults from ~/.sdd/defaults.json if present
  const homeDir = homedir();
  let globalDefaults: Record<string, unknown> = {};
  try {
    const defaultsPath = join(homeDir, '.sdd', 'defaults.json');
  ...
  const hasBraveSearch = !!(process.env.BRAVE_API_KEY || existsSync(join(homeDir, '.sdd', 'brave_api_key')));
  const hasFirecrawl = !!(process.env.FIRECRAWL_API_KEY || existsSync(join(homeDir, '.sdd', 'firecrawl_api_key')));
  const hasExaSearch = !!(process.env.EXA_API_KEY || existsSync(join(homeDir, '.sdd', 'exa_api_key')));
...
export const configEnsureSection: QueryHandler = async (args, projectDir, workstream) => {
  ...
  if (!sectionName) {
    throw new SDDError('Usage: config-ensure-section <section>', ErrorClassification.Validation);
  }
