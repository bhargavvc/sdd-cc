...
describe('query dispatch error mapper', () => {
  it('maps native timeout errors', () => {
    const err = mapNativeDispatchError(
      new Error('sdd-tools timed out after 30000ms: state load'),
      'state.load',
      [],
    );
  ...
  it('maps typed timeout classification from SDDToolsError', () => {
    const err = mapNativeDispatchError(
      SDDToolsError.timeout('timeout', 'state', ['load'], '', 1234),
      'state.load',
      [],
    );
    expect(err.kind).toBe('native_timeout');
  ...
  it('maps typed failure classification from SDDToolsError', () => {
    const err = mapNativeDispatchError(
      SDDToolsError.failure('boom', 'state', ['load'], 1),
      'state.load',
      [],
    );
    expect(err.kind).toBe('native_failure');
