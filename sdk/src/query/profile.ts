 ...
 *
 * Ported from sdd/bin/lib/profile-pipeline.cjs, profile-output.cjs,
 * and learnings.cjs.
 *
 * @example
 * ```typescript
 ...
 * // { data: { mode: 'interactive', questions: [...] } } — same shape as sdd-tools.cjs
 * ```
 */
...
// ─── Learnings — ~/.sdd/knowledge/ knowledge store ───────────────────────

const STORE_DIR = join(homedir(), '.sdd', 'knowledge');
...
/**
 * List all entries in the global learnings store (`~/.sdd/knowledge/`).
 *
 ...
 * Called by sdd-planner agent to inject prior learnings into plan generation.
 *
...
export const learningsPrune: QueryHandler = async (args) => {
  ...
  if (!olderThan) {
    throw new SDDError('Usage: learnings prune --older-than <duration>', ErrorClassification.Validation);
  }
  try {
    return { data: learningsPruneStore(olderThan) };
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    throw new SDDError(msg, ErrorClassification.Validation);
  }
};
...
export const learningsDelete: QueryHandler = async (args) => {
  const id = args[0];
  if (!id) {
    throw new SDDError('Usage: learnings delete <id>', ErrorClassification.Validation);
  }
  if (!/^[a-z0-9]+-[a-f0-9]+$/.test(id)) {
    throw new SDDError(`Invalid learning ID: "${id}"`, ErrorClassification.Validation);
  }
 ...
 *
 * Port of `cmdExtractMessages` from profile-pipeline.cjs — JSON matches `sdd-tools extract-messages`
...
export const extractMessages: QueryHandler = async (args) => {
  ...
  if (!projectArg || projectArg.startsWith('--')) {
    throw new SDDError(
      'Usage: sdd-tools extract-messages <project> [--session <id>] [--limit N] [--path <dir>]\nRun scan-sessions first to see available projects.',
 ...
 * JSON matches `sdd-tools profile-sample` (`output_file` JSONL + metadata).
 */
