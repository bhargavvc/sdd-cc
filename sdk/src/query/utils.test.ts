 ...
 * to sdd-tools.cjs cmdGenerateSlug and cmdCurrentTimestamp.
 */
...
describe('generateSlug', () => {
  ...
  it('throws SDDError with Validation classification for empty text', async () => {
    await expect(generateSlug([''], PROJECT_DIR)).rejects.toThrow(SDDError);
    try {
      await generateSlug([''], PROJECT_DIR);
    } catch (err) {
      expect(err).toBeInstanceOf(SDDError);
      expect((err as SDDError).classification).toBe(ErrorClassification.Validation);
    }
  });

  it('throws SDDError with Validation classification for missing text', async () => {
    await expect(generateSlug([], PROJECT_DIR)).rejects.toThrow(SDDError);
    try {
      await generateSlug([], PROJECT_DIR);
    } catch (err) {
