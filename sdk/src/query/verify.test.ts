...
describe('verifyPlanStructure', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-verify-'));
  });
...
`;
  ...
  it('throws SDDError with Validation classification when no args', async () => {
    ...
    expect(caught).toBeInstanceOf(SDDError);
    expect((caught as SDDError).classification).toBe('validation');
  });
});
...
describe('verifyPhaseCompleteness', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-verify-phase-'));
  ...
  it('throws SDDError with Validation classification when no args', async () => {
    await expect(verifyPhaseCompleteness([], tmpDir)).rejects.toThrow(SDDError);
  });
});
...
describe('verifyArtifacts', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-verify-art-'));
  });
...
body`;
  ...
  it('throws SDDError with Validation classification when no args', async () => {
    await expect(verifyArtifacts([], tmpDir)).rejects.toThrow(SDDError);
  });
});
