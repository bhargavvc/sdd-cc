 ...
 *
 * Ported from sdd/bin/lib/commands.cjs (cmdProgressRender, determinePhaseStatus).
...
export const todoMatchPhase: QueryHandler = async (args, projectDir) => {
  const phase = args[0];
  if (!phase) {
    throw new SDDError('phase required for todo match-phase', ErrorClassification.Validation);
  }
...
export const todoComplete: QueryHandler = async (args, projectDir) => {
  ...
  if (!filename) {
    throw new SDDError('filename required for todo complete', ErrorClassification.Validation);
  }
  ...
  if (!existsSync(sourcePath)) {
    throw new SDDError(`Todo not found: ${filename}`, ErrorClassification.Validation);
  }
