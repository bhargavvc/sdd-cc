 ...
 *
 * Ported from sdd/bin/lib/frontmatter.cjs.
...
export const frontmatterSet: QueryHandler = async (args, projectDir) => {
  ...
  if (hasNamedArgs) {
    if (fi === -1 || vi === -1 || !args[fi + 1] || args[vi + 1] === undefined) {
      throw new SDDError('file, --field, and --value required together', ErrorClassification.Validation);
    }
    filePath = args[0];
    field = args[fi + 1];
    value = args[vi + 1];
  ...
  if (!filePath || !field || value === undefined) {
    throw new SDDError('file, field, and value required', ErrorClassification.Validation);
  }
 ...
 *
 * @param args - `file --data <json>` (sdd-tools) or `[file, jsonString]` (SDK)
 * @param projectDir - Project root directory
...
export const frontmatterMerge: QueryHandler = async (args, projectDir) => {
  ...
  if (!filePath || !jsonString) {
    throw new SDDError('file and data required', ErrorClassification.Validation);
  }
  ...
  try {
    ...
    throw new SDDError('Invalid JSON for merge data', ErrorClassification.Validation);
  }
...
export const frontmatterValidate: QueryHandler = async (args, projectDir) => {
  ...
  if (!filePath || !schemaName) {
    throw new SDDError('file and schema required', ErrorClassification.Validation);
  }
