/**
 * `extract-messages` — parity with `sdd/bin/lib/profile-pipeline.cjs` `cmdExtractMessages`.
...
/**
 * Port of `cmdExtractMessages` — same JSON result as `sdd-tools extract-messages` (stdout object;
...
export async function runExtractMessages(
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'sdd-pipeline-'));
