 ...
 *
 * Ported from sdd/bin/lib/frontmatter.cjs and state.cjs.
 ...
 * // { data: { sdd_state_version: '1.0', milestone: 'v3.0', ... } }
 * ```
 */
...
/**
 * Parse YAML frontmatter body (between `---` fences) using the SDD stack parser.
...
/**
 * First leading frontmatter block only — parity with `sdd/bin/lib/frontmatter.cjs`
 * `extractFrontmatter` (used by `summary-extract` and `history-digest` in sdd-tools.cjs).
 */
 ...
 *
 * Port of `parseMustHavesBlock` from `sdd/bin/lib/frontmatter.cjs` lines 195-301.
