...
async function loadGateConfig(projectDir: string, workstream?: string): Promise<boolean> {
  try {
    ...
    if (typeof v === 'string') {
      ...
      console.warn(
        `[sdd] workflow.context_coverage_gate is a string "${v}" — expected boolean. Defaulting to ON.`,
      );
      return true;
    }
    if (v !== undefined && v !== null) {
      console.warn(
        `[sdd] workflow.context_coverage_gate has invalid type ${typeof v} (value: ${JSON.stringify(v)}); expected boolean. Defaulting to ON.`,
      );
    }
    ...
    for (const blockMatch of blockMatches) {
      ...
      for (const f of files) {
        ...
        if (!isInsideRoot(f, projectDir)) {
          console.warn(
            `[sdd] decision-coverage: skipping files_modified entry "${f}" — outside project root`,
          );
          continue;
        }
