...
describe('agentSkills', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-skills-'));
  });
  ...
  it('returns empty string when project has no config', async () => {
    const r = await agentSkills(['sdd-planner'], tmpDir);
    expect(r.data).toBe('');
  });

  it('returns empty string when agent type not in config.agent_skills', async () => {
    await writeConfig(tmpDir, { agent_skills: { 'sdd-executor': ['.claude/skills/foo'] } });
    const r = await agentSkills(['sdd-planner'], tmpDir);
    expect(r.data).toBe('');
  });
  ...
  it('returns <agent_skills> block for each configured skill path', async () => {
    ...
    await writeConfig(tmpDir, {
      agent_skills: {
        'sdd-planner': ['.claude/skills/skill-a', '.claude/skills/skill-b'],
      },
    });
  ...
  it('accepts a single string skill path (normalizes to array)', async () => {
    ...
    await writeConfig(tmpDir, {
      agent_skills: { 'sdd-planner': '.claude/skills/only-one' },
    });
  ...
  it('skips skills whose SKILL.md is missing', async () => {
    ...
    await writeConfig(tmpDir, {
      agent_skills: {
        'sdd-planner': ['.claude/skills/exists', '.claude/skills/missing-md'],
      },
    });
  ...
  it('rejects path traversal escaping the project root', async () => {
    await writeConfig(tmpDir, {
      agent_skills: { 'sdd-planner': ['../evil-skill'] },
    });
  ...
  it('returns empty string when agent_skills value is an empty array', async () => {
    await writeConfig(tmpDir, { agent_skills: { 'sdd-planner': [] } });
    const r = await agentSkills(['sdd-planner'], tmpDir);
    expect(r.data).toBe('');
  });
  ...
  it('signals format:"text" for non-empty blocks (used by CLI dispatcher)', async () => {
    ...
    await writeConfig(tmpDir, {
      agent_skills: { 'sdd-planner': '.claude/skills/a-skill' },
    });
  ...
  it('resolves global: skills through runtime-aware global skills dir policy', async () => {
    ...
    await writeConfig(tmpDir, {
      runtime: 'codex',
      agent_skills: { 'sdd-planner': ['global:global-skill'] },
    });
  ...
  it('returns empty string for global: skills on runtimes without a skills dir', async () => {
    await writeConfig(tmpDir, {
      runtime: 'cline',
      agent_skills: { 'sdd-planner': ['global:nope'] },
    });
...
describe('agentSkills CLI stdout', () => {
  ...
  beforeEach(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'sdd-skills-cli-'));
  });
  ...
  it('writes raw <agent_skills> block to stdout — not JSON-wrapped', async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ agent_skills: { 'sdd-planner': '.claude/skills/cli-skill' } }),
    );

    const stdout = execSync(
      `node "${CLI}" query --project-dir "${tmpDir}" agent-skills sdd-planner`,
      { encoding: 'utf-8' },
    );
  ...
  it('emits empty output (no JSON null) when agent type is unmapped', async () => {
    ...
    await writeFile(
      join(tmpDir, '.planning', 'config.json'),
      JSON.stringify({ agent_skills: { 'sdd-executor': ['.claude/skills/foo'] } }),
    );
