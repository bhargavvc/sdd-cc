 ...
 * - SDDOptions.workstream propagation
 * - SDDTools workstream-aware invocation
...
// ─── SDDOptions.workstream ──────────────────────────────────────────────────

describe('SDDOptions.workstream', () => {
  it('SDD class accepts workstream option', async () => {
...
// ─── SDDTools workstream injection ──────────────────────────────────────────

describe('SDDTools workstream injection', () => {
  let tmpDir: string;
  let fixtureDir: string;

  beforeEach(async () => {
    tmpDir = join(tmpdir(), `sdd-ws-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it('passes --ws flag to sdd-tools.cjs when workstream is set', async () => {
    const { SDDTools } = await import('./sdd-tools.js');
...
describe('loadConfig with workstream', () => {
  ...
  beforeEach(async () => {
    tmpDir = join(tmpdir(), `sdd-config-ws-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe('ContextEngine with workstream', () => {
  ...
  beforeEach(async () => {
    tmpDir = join(tmpdir(), `sdd-ctx-ws-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    await mkdir(tmpDir, { recursive: true });
  });
