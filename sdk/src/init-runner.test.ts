...
// Mock fs/promises for template reading (InitRunner reads SDD templates)
...
function makeDeps(overrides: Partial<InitRunnerDeps> & { tmpDir: string }): InitRunnerDeps & { events: SDDEvent[] } {
...
describe('InitRunner', () => {
  ...
  it('run() emits InitStart and InitComplete events', async () => {
    ...
    const startEvents = events.filter(e => e.type === SDDEventType.InitStart);
    const completeEvents = events.filter(e => e.type === SDDEventType.InitComplete);
  ...
  it('run() emits InitStepStart/Complete for each step', async () => {
    ...
    const stepStarts = events.filter(e => e.type === SDDEventType.InitStepStart);
    const stepCompletes = events.filter(e => e.type === SDDEventType.InitStepComplete);
  ...
  it('run() emits InitResearchSpawn before research sessions', async () => {
    ...
    const spawnEvents = events.filter(e => e.type === SDDEventType.InitResearchSpawn);
  ...
  describe('sdkPromptsDir preference and sanitizer integration', () => {
    ...
    beforeEach(async () => {
      ...
      await writeFile(
        join(sdkPromptsDir, 'agents', 'sdd-research-synthesizer.md'),
    ...
    it('readSDDFile falls back to SDD-1 when sdk/prompts/ file does not exist', async () => {
      ...
      // falls through to SDD-1 path. If SDD-1 also missing, gets placeholder.
      const projectPrompt = mockRunSession.mock.calls[0]![0] as string;
    ...
    it('readAgentFile falls back to SDD-1 when sdk/prompts/agents/ file does not exist', async () => {
      ...
      // Research prompt uses agent def — not in empty agents dir, falls to SDD-1
      const researchPrompt = mockRunSession.mock.calls[1]![0] as string;
