...
- **`sdd/workflows/help.md` now documents `/sdd-mvp-phase`** — the help.md ↔ commands/sdd parity test (`tests/bug-2954-help-md-slash-command-stubs.test.cjs`) requires every shipped `commands/sdd/X.md` to have a `/sdd-X` mention in help.md. PR #3180 added `/sdd-mvp-phase` to docs/COMMANDS.md but missed the in-product help that AI agents themselves load. New entry placed directly before `/sdd-plan-phase` (matches the user mental model: convert to MVP, then plan).
