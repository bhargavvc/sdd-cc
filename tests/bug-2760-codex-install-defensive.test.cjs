...
describe('#2760 defect 3 — Hooks AoT preservation across install/uninstall/reinstall', () => {
  ...
  test('fresh install emits the two-level nested AoT schema (#2773)', () => {
    ...
    const managed = sessionStartCommands.filter((cmd) => /sdd-check-update\.js/.test(cmd));
    assert.equal(managed.length, 1, 'hooks.json must contain exactly one managed sdd-check-update command');
    assert.ok(
      !parsed.hooks || !Array.isArray(parsed.hooks.SessionStart),
      'config.toml should not carry managed SessionStart hooks for SDD'
    );
  });

  test('preserves user [[hooks.SessionStart]] entries and registers managed SDD handler in hooks.json', () => {
    ...
    assert.ok(
      hooksJsonCommands.some((cmd) => typeof cmd === 'string' && /sdd-check-update\.js/.test(cmd)),
      'SDD handler must appear in hooks.json SessionStart entries: ' + JSON.stringify(hooksJsonCommands)
    );
  ...
  test('reinstall replaces flat [[hooks]] + event form with nested schema', () => {
    ...
    // Only one SDD hook entry must exist (no duplication) in hooks.json.
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const sddHandlers = hooksJsonCommands.filter((cmd) => /sdd-check-update\.js/.test(cmd));
  ...
  test('reinstall replaces single-block [[hooks.SessionStart]] (no .hooks sub-table) with nested schema', () => {
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const sddHandlers = hooksJsonCommands.filter((cmd) => /sdd-check-update\.js/.test(cmd));
    assert.strictEqual(sddHandlers.length, 1, 'exactly one managed handler after upgrade from PR-#2802-shape');
  });
  ...
  test('reinstall is idempotent: correct nested schema is stripped and re-emitted cleanly', () => {
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const sddHandlers = hooksJsonCommands.filter((cmd) => /sdd-check-update\.js/.test(cmd));
    assert.strictEqual(sddHandlers.length, 1, 'exactly one managed SessionStart handler after double install');
  });
});
...
describe('#2760 CR4 finding 2 — Legacy flat [[hooks]] block migrates to namespaced AoT on reinstall', { concurrency: false }, () => {
  ...
  test('pre-install legacy flat [[hooks]] sdd-check-update + user namespaced [[hooks.SessionStart]] → post-install converges on namespaced AoT', () => {
    ...
    assert.ok(
      hooksJsonCommands.some((cmd) => typeof cmd === 'string' && /sdd-check-update\.js/.test(cmd)),
      'SDD entry must appear in hooks.json SessionStart entries: '
    ...
    const sddEntries = hooksJsonCommands.filter((cmd) => typeof cmd === 'string' && /sdd-check-update\.js/.test(cmd));
...
describe('#2760 CR4 finding 3 / #3245 — parseTomlToObject handles edge-case value types (floats accepted; dates/trailing-garbage rejected)', () => {
  ...
  // requires f64 for tool_timeout_sec/startup_timeout_sec, so SDD's parser
    ...
    fs.renameSync = (src, dst) => {
      if (dst === configPath) {
        ...
        try {
          ...
          isHookWrite = /SDD codex_hooks ownership/.test(data);
        } catch (_) { /* ignore */ }
        if (isHookWrite) {
...
describe('#2760 CR5 finding 3 — migration emits namespaced AoT (no flat/namespaced mixing)', { concurrency: false }, () => {
  ...
  test('user has [[hooks.AfterTool]] AND legacy [hooks.SessionStart] → post-install both namespaced, no flat AoT', () => {
    ...
    assert.ok(
      hooksJsonCommands.some((cmd) => typeof cmd === 'string' && /sdd-check-update\.js/.test(cmd)),
      'managed sdd-check-update entry must appear in hooks.json SessionStart entries: ' +
        JSON.stringify(hooksJsonCommands)
    );
