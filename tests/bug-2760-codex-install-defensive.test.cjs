 ...
 *     `[[hooks.SessionStart]]` (namespaced AoT) entries in their config, SDD
 *     used to append a `[[hooks]]` (top-level AoT) block that confuses
 *     round-trip writers and produces a config Codex refuses to load.
 *     Fix: detect the user's preferred shape and emit SDD's hook in the same
 ...
 *     invalid in current Codex schema and break Codex even though SDD now
 *     emits the correct `[agents.<name>]` struct form. Fix: install-time
 *     stripping always purges these forms regardless of SDD marker presence
...
// Scope SDD_TEST_MODE to module load only — restore prior value (or unset) so
// downstream tests in the same node process never see test-only behaviour
// leak through (#2760 CR4 finding 5).
const previousSddTestMode = process.env.SDD_TEST_MODE;
...
if (previousSddTestMode === undefined) {
  delete process.env.SDD_TEST_MODE;
} else {
  process.env.SDD_TEST_MODE = previousSddTestMode;
}
...
describe('#2760 defect 3 — Hooks AoT preservation across install/uninstall/reinstall', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2760-d3-'));
  ...
  test('fresh install emits the two-level nested AoT schema (#2773)', () => {
    ...
    // The handler must have type = "command" and reference sdd-check-update.js.
    const handler = eventEntry.hooks[0];
    assert.strictEqual(handler.type, 'command', 'handler type must be "command"');
    assert.ok(
      typeof handler.command === 'string' && /sdd-check-update\.js/.test(handler.command),
      'handler command must reference sdd-check-update.js: got ' + handler.command
    );
  ...
  test('preserves user [[hooks.SessionStart]] entries and adds SDD nested handler', () => {
    // Users may have their own [[hooks.SessionStart]] entries using the new schema.
    // SDD must append its own two-level block without disturbing theirs.
    const userConfig = [
      '[[hooks.SessionStart]]',
      '',
    ...
    assert.ok(
      allCommands.some((cmd) => typeof cmd === 'string' && /sdd-check-update\.js/.test(cmd)),
      'SDD handler must appear in hooks.SessionStart[].hooks: ' + JSON.stringify(allCommands)
    );
  ...
  test('reinstall replaces flat [[hooks]] + event form with nested schema', () => {
    // Upgrade path: user has a config written by SDD 1.38.x (flat [[hooks]] form).
    const legacyConfig = [
      '[features]',
      'codex_hooks = true',
      '',
      ...
      '[[hooks]]',
      'event = "SessionStart"',
      'command = "node /old/path/to/sdd-check-update.js"',
      '',
    ].join('\n');
    ...
    assert.ok(/sdd-check-update\.js/.test(handler.command));
    // Only one SDD hook entry must exist (no duplication).
    const sessionStart = parsed.hooks?.SessionStart ?? [];
    const sddHandlers = sessionStart.flatMap((entry) =>
      Array.isArray(entry.hooks) ? entry.hooks : []
    ).filter((h) => typeof h?.command === 'string' && /sdd-check-update\.js/.test(h.command));
    assert.strictEqual(sddHandlers.length, 1, 'exactly one managed handler after upgrade');
  });
  ...
  test('reinstall replaces single-block [[hooks.SessionStart]] (no .hooks sub-table) with nested schema', () => {
    ...
    assert.strictEqual(
      handlers.filter((h) => typeof h?.command === 'string' && /sdd-check-update\.js/.test(h.command)).length,
      1,
  ...
  test('reinstall is idempotent: correct nested schema is stripped and re-emitted cleanly', () => {
    ...
    assert.ok(/sdd-check-update\.js/.test(sessionStart[0].hooks[0].command), 'managed handler command preserved');
  });
});
...
describe('#2760 fix 2 — Strip purges invalid legacy [agents] / [[agents]] regardless of marker', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2760-f2-'));
    codexHome = path.join(tmpDir, 'codex-home');
  });
  ...
  test('strips bare [agents] single-bracket block (no SDD marker, arbitrary user keys)', () => {
    writeCodexConfig(codexHome, [
      '[agents]',
    ...
    // must itself be a table (the sdd-* struct form).
    assert.ok(
    ...
    const sddAgents = Object.keys(parsed.agents).filter((k) => k.startsWith('sdd-'));
    assert.ok(
      sddAgents.length > 0 && sddAgents.every((k) => typeof parsed.agents[k] === 'object'),
      'agents.sdd-* struct form must be present: ' + JSON.stringify(Object.keys(parsed.agents))
    );
  ...
  test('strips [[agents]] sequence-form block without SDD marker (third-party / marker-edited-out)', () => {
    writeCodexConfig(codexHome, [
      '[[agents]]',
      'name = "user-helper"',
    ...
    // a table-of-tables with sdd-* struct keys.
    assert.ok(
      parsed.agents && typeof parsed.agents === 'object' && !Array.isArray(parsed.agents),
...
describe('#2760 fix 3 — Post-write Codex schema validation', { concurrency: false }, () => {
  test('passes a clean config produced by SDD install', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2760-f3a-'));
    try {
      const codexHome = path.join(tmpDir, 'codex-home');
      runCodexInstall(codexHome);
      const content = readCodexConfig(codexHome);
      const result = validateCodexConfigSchema(content);
      assert.equal(result.ok, true, 'SDD-emitted config passes schema validation');
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  test('aborts install and restores pre-install backup when post-write validation fails', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2760-f3b-'));
...
describe('#2760 fix 4 — Write-failure rollback (atomic write + snapshot restore)', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2760-f4-'));
    codexHome = path.join(tmpDir, 'codex-home');
  ...
  test('pre-install config bytes survive when fs.renameSync throws over configPath', () => {
    ...
    // user's [model] section, not a half-written SDD block.
    ...
    assert.equal(parsed.agents, undefined,
      'no SDD agents block may have leaked into the surviving file');
...
describe('#2760 CR4 finding 2 — Legacy flat [[hooks]] block migrates to namespaced AoT on reinstall', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2760-cr4-f2-'));
    codexHome = path.join(tmpDir, 'codex-home');
  });
  ...
  test('pre-install legacy flat [[hooks]] sdd-check-update + user namespaced [[hooks.SessionStart]] → post-install converges on namespaced AoT', () => {
    // Reproduce the upgrade scenario:
    //   - User has [[hooks.SessionStart]] entry of their own (signal that SDD
    //     should emit in the namespaced shape).
    //   - A previous SDD install left the legacy flat [[hooks]] managed block
    //     for sdd-check-update. The pre-CR4 strip step would short-circuit
    ...
    const userPlusLegacy = [
      ...
      '[[hooks]]',
      'event = "SessionStart"',
      'command = "node /old/path/hooks/sdd-check-update.js"',
      '',
    ].join('\n');
    ...
    // and the SDD entry is re-emitted in the namespaced AoT shape so the two
    ...
    assert.ok(
      allSessionStartCommands.some((cmd) => typeof cmd === 'string' && /sdd-check-update\.js/.test(cmd)),
      'SDD entry must appear in hooks.SessionStart array (namespaced AoT form): '
    ...
    // No duplicate sdd-check-update entries — exactly one managed entry.
    const sddEntries = allSessionStartCommands.filter(
      (cmd) => typeof cmd === 'string' && /sdd-check-update\.js/.test(cmd)
    );
    assert.equal(sddEntries.length, 1,
      'exactly one sdd-check-update entry after migration, got: ' + sddEntries.length);
  });
});
...
describe('#2760 CR4 finding 1 — atomicWriteFileSync failure aborts install (post-write fatal)', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2760-cr4-f1-'));
    codexHome = path.join(tmpDir, 'codex-home');
    originalRenameSync = fs.renameSync;
    ...
    fs.renameSync = (src, dst) => {
      if (dst === configPath) {
        ...
        try {
          ...
          isHookWrite = /sdd-check-update\.js/.test(data);
        } catch (_) { /* ignore */ }
        if (isHookWrite) {
...
describe('#2760 CR5 finding 1 — pre-write failures abort install (outer catch fatal)', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2760-cr5-f1-'));
    codexHome = path.join(tmpDir, 'codex-home');
    originalConsoleLog = console.log;
    consoleOutput = [];
    console.log = (...args) => { consoleOutput.push(args.join(' ')); };
...
describe('#2760 CR5 finding 3 — migration emits namespaced AoT (no flat/namespaced mixing)', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2760-cr5-f3-'));
    codexHome = path.join(tmpDir, 'codex-home');
  });
  ...
  test('user has [[hooks.AfterTool]] AND legacy [hooks.SessionStart] → post-install both namespaced, no flat AoT', () => {
    ...
    //    SDD-managed entry converges on namespaced too.
    const userPlusLegacy = [
      '[[hooks.AfterTool]]',
      'command = "x"',
      '',
    ...
    // SDD's managed sdd-check-update entry also lives in the namespaced array.
    assert.ok(
      ssCommands.some((cmd) => typeof cmd === 'string' && /sdd-check-update\.js/.test(cmd)),
      'managed sdd-check-update entry must appear in hooks.SessionStart array: ' +
        JSON.stringify(ssCommands)
    );
