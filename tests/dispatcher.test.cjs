/**
 * SDD Tools Tests - Dispatcher
 *
 * Tests for sdd-tools.cjs dispatch routing and error paths.
...
describe('dispatcher error paths', () => {
  ...
  test('no-command invocation prints usage and exits non-zero', () => {
    const result = runSddTools('', tmpDir);
  ...
  test('unknown command produces clear error and exits non-zero', () => {
    const result = runSddTools('nonexistent-cmd', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('--cwd= form overrides working directory', () => {
    ...
    const result = runSddTools(['--cwd=' + tmpDir, 'state', 'load'], process.cwd());
  ...
  test('--cwd= with empty value produces error', () => {
    const result = runSddTools('--cwd= state load', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('--cwd with invalid path produces error', () => {
    const result = runSddTools('--cwd /nonexistent/path/xyz state load', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('state unknown subcommand errors', () => {
    const result = runSddTools('state bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('template unknown subcommand errors', () => {
    const result = runSddTools('template bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('frontmatter unknown subcommand errors', () => {
    const result = runSddTools('frontmatter bogus file.md', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('verify unknown subcommand errors', () => {
    const result = runSddTools('verify bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('phases unknown subcommand errors', () => {
    const result = runSddTools('phases bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('roadmap unknown subcommand errors', () => {
    const result = runSddTools('roadmap bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('requirements unknown subcommand errors', () => {
    const result = runSddTools('requirements bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('phase unknown subcommand errors', () => {
    const result = runSddTools('phase bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('milestone unknown subcommand errors', () => {
    const result = runSddTools('milestone bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('validate unknown subcommand errors', () => {
    const result = runSddTools('validate bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('todo unknown subcommand errors', () => {
    const result = runSddTools('todo bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('uat unknown subcommand errors', () => {
    const result = runSddTools('uat bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
  ...
  test('init unknown workflow errors', () => {
    const result = runSddTools('init bogus', tmpDir);
    assert.strictEqual(result.success, false, 'Should exit non-zero');
...
describe('dispatcher routing branches', () => {
  ...
  test('init resume returns valid JSON', () => {
    ...
    const result = runSddTools('init resume', tmpDir);
  ...
  test('init verify-work returns valid JSON', () => {
    ...
    const result = runSddTools('init verify-work 01', tmpDir);
  ...
  test('roadmap update-plan-progress updates phase progress', () => {
    ...
    const result = runSddTools('roadmap update-plan-progress 1', tmpDir);
  ...
  test('state with no subcommand calls cmdStateLoad', () => {
    ...
    const result = runSddTools('state', tmpDir);
