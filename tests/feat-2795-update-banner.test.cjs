...
const HOOK_PATH = path.join(__dirname, '..', 'hooks', 'sdd-update-banner.js');
const {
  buildBannerOutput,
  shouldSuppressFailureWarning,
  RATE_LIMIT_SECONDS,
} = require('../hooks/sdd-update-banner.js');
...
describe('shouldSuppressFailureWarning', () => {
  function tmpDir() {
    return fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-banner-supp-'));
  }
...
describe('sdd-update-banner.js end-to-end', () => {
  function setupHome() {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-banner-home-'));
    fs.mkdirSync(path.join(home, '.cache', 'sdd'), { recursive: true });
    return home;
  }
  ...
  function writeCache(home, contents) {
    fs.writeFileSync(
      path.join(home, '.cache', 'sdd', 'sdd-update-check.json'),
  ...
  test('suppresses repeat failure diagnostic within 24h via sentinel', () => {
    ...
    try {
      ...
      const sentinel = path.join(home, '.cache', 'sdd', 'banner-failure-warned-at');
...
//
// These tests load bin/install.js as a module via SDD_TEST_MODE and assert on
...
describe('install.js update-banner wiring', () => {
  ...
  test('buildUpdateBannerHookEntry produces a SessionStart hook entry', () => {
    ...
    const entry = installExports.buildUpdateBannerHookEntry(
      '"/usr/local/bin/node" "/home/u/.claude/hooks/sdd-update-banner.js"'
    );
    ...
    assert.ok(
      entry.hooks[0].command.includes('sdd-update-banner.js'),
