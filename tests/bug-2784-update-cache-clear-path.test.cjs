 ...
 *
 * /sdd-update cache-clear step only cleared per-runtime cache paths
 * (e.g. ~/.claude/cache/sdd-update-check.json) but the SessionStart hook
 * (hooks/sdd-check-update.js) writes to the shared tool-agnostic path
 * ~/.cache/sdd/sdd-update-check.json. After a successful update, the statusline
 * kept showing the stale "⬆ /sdd-update" indicator because the actual cache
 * file was never deleted.
 *
 * Fix: add `rm -f "$HOME/.cache/sdd/sdd-update-check.json"` to the
 * run_update step's cache-clear block in sdd/workflows/update.md.
 */
...
const CHECK_UPDATE_HOOK = path.join(REPO_ROOT, 'hooks', 'sdd-check-update.js');

describe('bug-2784: update.md cache-clear covers shared cache path', () => {
  test('sdd-check-update.js hook constructs cache dir from .cache and sdd path segments', () => {
    ...
    assert.ok(
      segments.includes('sdd'),
      `hook cacheDir path.join() must include 'sdd' segment; got: ${JSON.stringify(segments)}`
    );
  });

  test('update.md run_update bash commands include rm for shared sdd cache file', () => {
    ...
    const sharedCacheClearCmds = bashLines.filter(
      (line) => /^rm\b/.test(line) && line.includes('.cache/sdd/sdd-update-check.json')
    );
    assert.ok(
      sharedCacheClearCmds.length > 0,
      [
        'run_update step bash blocks must include an `rm` command targeting .cache/sdd/sdd-update-check.json.',
