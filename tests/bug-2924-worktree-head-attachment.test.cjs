 ...
 *   3. sdd-executor.md prohibits `git update-ref refs/heads/<protected>` as a
 *      "recovery" path and includes a pre-commit HEAD assertion in the task
 *      commit protocol.
 *   4. No workflow file in sdd/workflows/ contains an unconditional
...
const EXECUTE_PHASE_PATH = path.join(REPO_ROOT, 'sdd', 'workflows', 'execute-phase.md');
const EXECUTE_PLAN_PATH = path.join(REPO_ROOT, 'sdd', 'workflows', 'execute-plan.md');
const QUICK_PATH = path.join(REPO_ROOT, 'sdd', 'workflows', 'quick.md');
const EXECUTOR_AGENT_PATH = path.join(REPO_ROOT, 'agents', 'sdd-executor.md');
const GIT_INTEGRATION_PATH = path.join(REPO_ROOT, 'sdd', 'references', 'git-integration.md');
...
describe('bug #2924: worktree HEAD attachment + destructive recovery', () => {
  ...
  describe('sdd-executor.md prohibits update-ref self-recovery', () => {
    ...
    test('destructive_git_prohibition block exists', () => {
      assert.ok(block, 'sdd-executor.md must contain a <destructive_git_prohibition> block');
    });
  ...
  describe('sdd-executor.md task_commit_protocol enforces worktree-agent-* allow-list', () => {
    const content = fs.readFileSync(EXECUTOR_AGENT_PATH, 'utf-8');
    const block = extractNamedBlock(content, 'task_commit_protocol');

    test('task_commit_protocol block exists', () => {
      assert.ok(block, 'sdd-executor.md must contain a <task_commit_protocol> block');
    });
  ...
  describe('no workflow file performs unconditional update-ref on a protected branch', () => {
    const workflowsDir = path.join(REPO_ROOT, 'sdd', 'workflows');
