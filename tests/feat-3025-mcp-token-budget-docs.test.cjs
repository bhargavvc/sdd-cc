 ...
 *
 * Verifies that sdd/references/context-budget.md contains the
 ...
 *      project-specific)
 *   4. An explicit note that SDD does NOT manage MCP enablement — this is
...
const CONTEXT_BUDGET_MD = path.join(ROOT, 'sdd', 'references', 'context-budget.md');
...
function parseMcpBudgetSection(section) {
  ...
  // backticks before phrase-matching so e.g. "SDD does **not** manage"
  // is caught by the primary `sdd does not manage` alternative below.
  ...
  // (5) Harness vs SDD distinction — must explicitly state SDD doesn't
  // own this knob and point at the harness
  const explainsHarnessNotGsd =
    /(sdd does(?:n[''’]t| not) (own|manage|control)|harness (concern|setting|controlled)|not a sdd (setting|knob))/i.test(stripped);
...
describe('#3025 context-budget.md: MCP token-budget section exists with required content', () => {
  ...
  test('explains SDD does not own MCP enablement — harness concern (#3025 requirement 4)', () => {
    const section = extractSection(CONTEXT_BUDGET_MD, 'mcp');
    const parsed = parseMcpBudgetSection(section);
    assert.equal(parsed.explainsHarnessNotGsd, true,
      'section must explicitly state SDD does not manage MCP enablement (harness concern)');
  });
