...
const {
  PROFILING_QUESTIONS,
  CLAUDE_INSTRUCTIONS,
} = require('../sdd/bin/lib/profile-output.cjs');
...
describe('write-profile command', () => {
  ...
  test('writes USER-PROFILE.md from analysis JSON', () => {
    ...
    const result = runSddTools(['write-profile', '--input', analysisPath, '--raw'], tmpDir, { HOME: tmpDir });
  ...
  test('errors when --input is missing', () => {
    const result = runSddTools('write-profile --raw', tmpDir);
...
describe('generate-claude-md command', () => {
  ...
  test('generates CLAUDE.md with --auto flag', () => {
    ...
    const result = runSddTools(['generate-claude-md', '--output', outputPath, '--auto', '--raw'], tmpDir);
    assert.ok(result.success, `Failed: ${result.error}`);
  ...
  test('skills fallback mentions the normalized project roots', () => {
    ...
    assert.ok(!content.includes('sdd/skills'));
  });
});
...
describe('generate-dev-preferences command', () => {
  ...
  test('errors when --analysis is missing', () => {
    const result = runSddTools('generate-dev-preferences --raw', tmpDir);
  ...
  test('uses runtime-aware skills dir for codex by default', () => {
    ...
    const result = runSddTools(
      ['generate-dev-preferences', '--analysis', analysisPath, '--raw'],
      tmpDir,
      { CODEX_HOME: codexHome, SDD_RUNTIME: 'codex' }
    );
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
    assert.strictEqual(out.command_path, path.join(codexHome, 'skills', 'sdd-dev-preferences', 'SKILL.md'));
