...
const WORKFLOW_PATH = path.join(ROOT, 'sdd', 'workflows', 'code-review.md');
const FIXER_PATH = path.join(ROOT, 'agents', 'sdd-code-fixer.md');
const REVIEWER_PATH = path.join(ROOT, 'agents', 'sdd-code-reviewer.md');
...
// ---------------------------------------------------------------------------
describe('Bug 3 — sdd-code-fixer BL-* inclusion in filter sets', () => {
  test('finding_parser documents BL-\\d+ as Critical-tier-equivalent', () => {
    ...
    assert.ok(parserStart !== -1, 'sdd-code-fixer.md must have a <finding_parser> block');
  ...
  test('parse_findings step documents severity as "Critical (CR-* or BL-*)"', () => {
    ...
    assert.ok(stepStart !== -1, 'sdd-code-fixer.md must have a parse_findings step');
...
// ---------------------------------------------------------------------------
// REVIEWER CONTRACT — sdd-code-reviewer.md must acknowledge BL-/blocker: as
// an accepted alternative to CR-/critical: (tier-equivalent).
// ---------------------------------------------------------------------------
describe('Reviewer contract — sdd-code-reviewer.md label-equivalence', () => {
  test('write_review step documents blocker: as accepted alternative to critical:', () => {
    ...
    const stepEnd = src.indexOf('</step>', stepStart);
    assert.ok(stepStart !== -1, 'sdd-code-reviewer.md must have a write_review step');
    const stepSection = src.slice(stepStart, stepEnd);
    assert.ok(
