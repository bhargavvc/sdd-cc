 ...
 * - If CONTEXT.md is missing: primary suggestion is /sdd-discuss-phase
 * - If CONTEXT.md exists: primary suggestion is /sdd-plan-phase
 */
...
const workflowPath = path.resolve(
  __dirname, '..', 'sdd', 'workflows', 'execute-phase.md'
);
...
describe('bug #2002: offer_next checks CONTEXT.md before suggesting next step', () => {
  ...
  test('offer_next presents /sdd-discuss-phase when CONTEXT.md does not exist', () => {
    content = content || fs.readFileSync(workflowPath, 'utf-8');
    ...
    // "does not exist" and "sdd-discuss-phase" in the offer_next step
    ...
    assert.ok(
      ...
      /sdd-discuss-phase.*recommended|recommended.*sdd-discuss-phase/i.test(offerNextSection),
      'offer_next must present /sdd-discuss-phase as primary when CONTEXT.md does not exist'
    );
  });

  test('offer_next presents /sdd-plan-phase when CONTEXT.md exists', () => {
    content = content || fs.readFileSync(workflowPath, 'utf-8');
    const offerNextIdx = content.indexOf('offer_next');
    assert.ok(offerNextIdx !== -1, 'offer_next step must exist');
    ...
    assert.ok(
      ...
      'offer_next must present /sdd-plan-phase as primary when CONTEXT.md exists'
    );
  });
