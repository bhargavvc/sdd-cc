...
const { readSurface, writeSurface, resolveSurface } = require('../sdd/bin/lib/surface.cjs');
const { resolveProfile, loadSkillsManifest, writeActiveProfile } = require('../sdd/bin/lib/install-profiles.cjs');
const { CLUSTERS } = require('../sdd/bin/lib/clusters.cjs');

const REAL_COMMANDS_DIR = path.join(__dirname, '..', 'commands', 'sdd');

function tmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-surface-resolve-'));
}
...
describe('resolveSurface', () => {
  ...
  test('surface with baseProfile overrides .sdd-profile marker', () => {
    const dir = tmpDir();
    try {
      writeActiveProfile(dir, 'core');
      writeSurface(dir, {
