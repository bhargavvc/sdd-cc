 ...
 *     (integers fail with "invalid type: integer"). SDD's strict-integer-only parser
...
// SDD_TEST_MODE must be set before require('../bin/install.js') so the module
...
describe('#3245 — install succeeds with TOML float in pre-existing config', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3245-float-'));
...
describe('#3245 — idempotent rollback reverts skills/, agents/, and VERSION', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3245-rollback-'));
    codexHome = path.join(tmpDir, 'codex-home');
  });
  ...
  test('validation failure rolls back skills/, agents/, and VERSION to pre-install state', () => {
    // Start from a clean codexHome with no pre-existing SDD content — the dirs
    ...
    // skills/ — SDD writes sdd-* subdirs here. All must be absent after rollback.
    const skillsDir = path.join(codexHome, 'skills');
    if (fs.existsSync(skillsDir)) {
      const sddSkills = fs.readdirSync(skillsDir, { withFileTypes: true })
        .filter(e => e.isDirectory() && e.name.startsWith('sdd-'));
      assert.strictEqual(
        sddSkills.length,
        0,
        'rollback must remove all sdd-* skill directories: ' + sddSkills.map(e => e.name).join(', ')
      );
    }

    // agents/ — SDD writes sdd-*.md and sdd-*.toml here. All must be absent.
    const agentsDir = path.join(codexHome, 'agents');
    if (fs.existsSync(agentsDir)) {
      const sddAgents = fs.readdirSync(agentsDir)
        .filter(f => f.startsWith('sdd-') && (f.endsWith('.md') || f.endsWith('.toml')));
      assert.strictEqual(
        sddAgents.length,
        0,
        'rollback must remove all sdd-* agent files: ' + sddAgents.join(', ')
      );
    }

    // VERSION — SDD writes sdd/VERSION. Must be absent (wasn't there before).
    const versionPath = path.join(codexHome, 'sdd', 'VERSION');
    assert.strictEqual(
  ...
  test('rollback is safe when fired before any snapshots were captured (very early failure)', () => {
    ...
    // Rollback removes all sdd-* skill dirs it wrote. Even if skills/ was
    // created during the install, no sdd-* dirs should survive after rollback.
    const skillsDir = path.join(codexHome, 'skills');
    const remainingSddSkills = fs.existsSync(skillsDir)
    ...
    assert.deepStrictEqual(
      remainingSddSkills,
      [],
      'rollback must remove all sdd-* skill dirs even when fired after minimal writes'
    );
  });

  test('rollback does not remove pre-existing user skills that SDD did not write', () => {
    // If the user has a custom skill dir (not sdd-*) it must survive rollback.
    const skillsDir = path.join(codexHome, 'skills');
    ...
    assert.strictEqual(
      ...
      true,
      'pre-existing non-sdd-* skill must survive rollback'
    );
  });
