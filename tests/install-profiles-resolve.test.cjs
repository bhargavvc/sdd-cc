...
const REAL_COMMANDS_DIR = path.join(__dirname, '..', 'commands', 'sdd');
...
describe('resolveProfile', () => {
  ...
  test('resolveProfile standard — agents Set is non-empty (plan-phase pulls sdd-planner etc)', () => {
    const manifest = loadSkillsManifest(REAL_COMMANDS_DIR);
    ...
    // plan-phase is in standard and calls sdd-planner, sdd-plan-checker, sdd-phase-researcher
    assert.ok(result.agents.has('sdd-planner'), 'standard should include sdd-planner (called by plan-phase)');
    assert.ok(result.agents.has('sdd-plan-checker'), 'standard should include sdd-plan-checker (called by plan-phase)');
  });
  ...
  test('agents are derived from skill body text — synthetic manifest', () => {
    // Build a synthetic manifest where plan-phase calls sdd-planner
    const manifest = new Map([
      ['plan-phase', []],
      ['phase', []],
    ]);
    ...
    assert.ok(result.agents.has('sdd-planner'), 'sdd-planner should be derived from plan-phase body');
  });

  test('agents transitively closed — skill requiring plan-phase also gets its agents', () => {
    const manifest = loadSkillsManifest(REAL_COMMANDS_DIR);
    // quick requires plan-phase (via requires: field or direct) and also calls sdd-planner directly
    // new-project requires plan-phase so inherits its agents
    const result = resolveProfile({ modes: ['standard'], manifest });
    // Since plan-phase is in standard, and plan-phase calls sdd-planner, sdd-planner must be present
    assert.ok(result.agents.has('sdd-planner'));
  });
});
