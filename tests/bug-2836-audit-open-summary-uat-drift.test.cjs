...
const auditModule = require('../sdd/bin/lib/audit.cjs');
...
describe('bug #2836: audit-open quick-task summary filename + UAT terminal status', () => {
  // Ensure SDD env vars do not redirect planningDir() away from our fixture.
  let prevProject, prevWorkstream;
  before(() => {
    prevProject = process.env.SDD_PROJECT;
    prevWorkstream = process.env.SDD_WORKSTREAM;
    delete process.env.SDD_PROJECT;
    delete process.env.SDD_WORKSTREAM;
  });
  after(() => {
    if (prevProject !== undefined) process.env.SDD_PROJECT = prevProject;
    if (prevWorkstream !== undefined) process.env.SDD_WORKSTREAM = prevWorkstream;
  });
...
describe('bug #2836: workflows/help.md one-liner reconciliation', () => {
  test('help.md quick-task one-liner uses ${quick_id}-SUMMARY.md pattern', () => {
    const helpPath = path.resolve(
      __dirname, '..', 'sdd', 'workflows', 'help.md'
    );
