 ...
 * boundary or usage check in `sdd-tools.cjs` carries a typed `ERROR_REASON`.
 *
...
// Run sdd-tools with SDD_JSON_ERRORS=1 (env-var activation, exercises the
...
function runJsonErrors(args, tmpDir, env = {}) {
  const result = runSddTools(args, tmpDir, { ...env, SDD_JSON_ERRORS: '1' });
  assert.strictEqual(result.success, false,
    `Expected failure with SDD_JSON_ERRORS=1 for args: ${args.join(' ')}\n` +
