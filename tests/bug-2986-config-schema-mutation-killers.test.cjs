 ...
 * score on sdd/bin/lib/config-schema.cjs (6 killed, 124 survived).
...
describe('Bug #2986: M2 -- DYNAMIC_KEY_PATTERNS.some semantic, not .every', () => {
  ...
  const patternRepresentatives = [
    { key: 'agent_skills.sdd-planner',                           topLevel: 'agent_skills' },
...
describe('Bug #2986: anchor-tightening (catches mutants that loosen ^ or $ in regexes)', () => {
  ...
  const overshoot = [
    { key: 'agent_skills.sdd-planner.extra',                     reason: 'agent_skills regex must not allow trailing dot-segment' },
