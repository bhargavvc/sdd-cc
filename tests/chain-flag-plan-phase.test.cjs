...
/**
 * SDD Tools Tests - chain flag preservation in plan-phase
 *
...
describe('plan-phase chain flag preservation (#1620)', () => {
  const planPath = path.join(__dirname, '..', 'sdd', 'workflows', 'plan-phase.md');
  const discussPath = path.join(__dirname, '..', 'sdd', 'workflows', 'discuss-phase.md');
  // After #2551, discuss-phase chain logic moved to modes/chain.md.
  const discussChainPath = path.join(__dirname, '..', 'sdd', 'workflows', 'discuss-phase', 'modes', 'chain.md');
  const readDiscuss = () => {
