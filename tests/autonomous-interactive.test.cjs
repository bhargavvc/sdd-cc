...
/**
 * SDD Tools Tests - autonomous --interactive flag
 *
...
describe('autonomous --interactive flag (#1413)', () => {
  const workflowPath = path.join(__dirname, '..', 'sdd', 'workflows', 'autonomous.md');
  const commandPath = path.join(__dirname, '..', 'commands', 'sdd', 'autonomous.md');
  ...
  test('workflow uses discuss-phase skill in interactive mode', () => {
    // Per #2697 the user-facing form is the hyphen invariant sdd-discuss-phase;
    ...
    // sdd-discuss-phase mention in prose) and falsely pass. Instead, isolate
    ...
    const skillCall = /Skill\(\s*skill\s*=\s*['"]sdd-discuss-phase['"]/.test(branch);
    assert.ok(
      skillCall,
      `INTERACTIVE branch must invoke Skill(skill="sdd-discuss-phase"). Got branch:\n${branch}`,
    );
  });
