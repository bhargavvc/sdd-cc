 ...
 *
 * (b) The parent package.json declares a `sdd-sdk` bin entry pointing at
 *     bin/sdd-sdk.js (the back-compat shim), so npm chmods it correctly.
 *
...
const SDD_SDK_SHIM = path.join(__dirname, '..', 'bin', 'sdd-sdk.js');
...
describe('fix #2441: back-compat shim — parent package bin entry', () => {
  test('root package.json declares sdd-sdk bin entry', () => {
    assert.ok(
      rootPkg.bin && rootPkg.bin['sdd-sdk'],
      'root package.json must have a bin["sdd-sdk"] entry for the back-compat shim.'
    );
  });

  test('sdd-sdk bin entry points at bin/sdd-sdk.js', () => {
    assert.equal(
      rootPkg.bin['sdd-sdk'],
      'bin/sdd-sdk.js',
      'bin["sdd-sdk"] must point at bin/sdd-sdk.js'
    );
  });

  test('bin/sdd-sdk.js shim file exists', () => {
    assert.ok(
      fs.existsSync(SDD_SDK_SHIM),
      'bin/sdd-sdk.js must exist as the back-compat PATH shim.'
    );
  });

  test('bin/sdd-sdk.js resolves sdk/dist/cli.js relative to itself', () => {
    const shimContent = fs.readFileSync(SDD_SDK_SHIM, 'utf-8');
    ...
    assert.match(
      ...
      'bin/sdd-sdk.js must call path.resolve(__dirname, "..", "sdk", "dist", "cli.js") to locate the prebuilt CLI.'
    );
  });

  test('bin/sdd-sdk.js invokes cli.js via spawnSync(process.execPath, ...)', () => {
    const shimContent = fs.readFileSync(SDD_SDK_SHIM, 'utf-8');
    ...
    assert.match(
      ...
      'bin/sdd-sdk.js must spawn node via process.execPath so the execute bit on cli.js is irrelevant (#2453).'
    );
    assert.match(
      shimContent,
      /process\.argv\.slice\(\s*2\s*\)/,
      'bin/sdd-sdk.js must forward user args via process.argv.slice(2).'
    );
    assert.match(
      shimContent,
      /process\.exit\(/,
      'bin/sdd-sdk.js must propagate the child exit status via process.exit.'
    );
  });
});
