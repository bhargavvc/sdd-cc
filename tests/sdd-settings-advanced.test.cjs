...
/**
 * Tests for `/sdd-settings-advanced` — power-user configuration command (#2528).
 *
 * Covers:
 ...
 *   - /sdd-settings confirmation output advertises /sdd-settings-advanced
...
const { VALID_CONFIG_KEYS } = require('../sdd/bin/lib/config-schema.cjs');

const ROOT = path.resolve(__dirname, '..');
// #2790: settings-advanced.md was consolidated into config.md as the --advanced flag.
const COMMAND_PATH = path.join(ROOT, 'commands', 'sdd', 'config.md');
const WORKFLOW_PATH = path.join(ROOT, 'sdd', 'workflows', 'settings-advanced.md');
const SETTINGS_WORKFLOW_PATH = path.join(ROOT, 'sdd', 'workflows', 'settings.md');
...
const SPEC_FIELDS = {
  ...
  git: [
    ...
    { key: 'git.phase_branch_template',      default: 'sdd/phase-{phase}-{slug}' },
    { key: 'git.milestone_branch_template',  default: 'sdd/{milestone}-{slug}' },
  ],
  runtime: [
...
describe('sdd-settings-advanced — file scaffolding', () => {
  ...
  test('workflow file exists at sdd/workflows/settings-advanced.md', () => {
  ...
  test('command frontmatter has name, description, allowed-tools', () => {
    ...
    const fm = fmMatch[1];
    assert.match(fm, /name:\s*sdd:config/, 'frontmatter missing name (sdd:config)');
...
describe('sdd-settings-advanced — workflow structure', () => {
  let workflow;
  try {
...
describe('sdd-settings-advanced — VALID_CONFIG_KEYS coverage', () => {
  for (const key of ALL_SPEC_KEYS) {
    test(`VALID_CONFIG_KEYS contains "${key}"`, () => {
      assert.ok(
        VALID_CONFIG_KEYS.has(key),
        `VALID_CONFIG_KEYS missing ${key} — add it to sdd/bin/lib/config-schema.cjs`
      );
    });
  }
});

// ─── /sdd-settings mentions /sdd-settings-advanced ────────────────────────────

describe('/sdd-settings advertises /sdd-settings-advanced', () => {
  test('settings workflow mentions canonical /sdd-config --advanced', () => {
    ...
    assert.ok(
      ...
      'sdd/workflows/settings.md must not mention legacy /sdd-settings-advanced variants'
    );
  });
});
...
describe('sdd-settings-advanced — config merge preserves unrelated keys', () => {
...
describe('sdd-settings-advanced — negative scenarios', () => {
  ...
  // The /sdd-settings-advanced workflow promises non-numeric input is never
  ...
  test('config-set on a numeric key stores non-numeric input verbatim as string (workflow layer must reject before reaching here)', (t) => {
    ...
    // /sdd-settings-advanced workflow. If a future change adds a numeric
