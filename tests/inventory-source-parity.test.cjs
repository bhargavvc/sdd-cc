...
/** Extract agent names from `| sdd-xxx | ...` rows (no backticks). */
function agentNames(text) {
  const re = /^\|\s*(sdd-[a-z0-9-]+)\s*\|/gm;
  const names = [];
  let m;
  while ((m = re.exec(text)) !== null) names.push(m[1]);
  return names;
...
function commandSourcePaths(text) {
  const re = /\[commands\/sdd\/[^\]]+\]\(\.\.\/(commands\/sdd\/[^)]+)\)/g;
  const paths = [];
  let m;
...
describe('INVENTORY.md declared artifacts exist on the filesystem (ghost-entry guard)', () => {
  ...
  describe('Workflows', () => {
    ...
    for (const name of names) {
      test(name, () => {
        const p = path.join(ROOT, 'sdd', 'workflows', name);
        assert.ok(
          fs.existsSync(p),
          'INVENTORY.md declares workflow "' + name + '" but sdd/workflows/' + name + ' does not exist — remove the ghost row or restore the file',
        );
      });
    }
  });
  ...
  describe('References', () => {
    ...
    for (const name of names) {
      test(name, () => {
        const p = path.join(ROOT, 'sdd', 'references', name);
        assert.ok(
          fs.existsSync(p),
          'INVENTORY.md declares reference "' + name + '" but sdd/references/' + name + ' does not exist — remove the ghost row or restore the file',
        );
      });
    }
  });
  ...
  describe('CLI Modules', () => {
    ...
    for (const name of names) {
      test(name, () => {
        const p = path.join(ROOT, 'sdd', 'bin', 'lib', name);
        assert.ok(
          fs.existsSync(p),
          'INVENTORY.md declares CLI module "' + name + '" but sdd/bin/lib/' + name + ' does not exist — remove the ghost row or restore the file',
        );
      });
    }
  });
