...
/**
 * For every `sdd/bin/lib/*.cjs`, assert the module name
...
const LIB_DIR = path.join(ROOT, 'sdd', 'bin', 'lib');
  ...
  for (const file of moduleFiles) {
    test(file, () => {
      assert.ok(
        ...
        `sdd/bin/lib/${file} has no row in docs/INVENTORY.md CLI Modules table — add one`,
      );
    });
  }
});
