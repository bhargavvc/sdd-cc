...
const {
  ...
  getActiveWorkstream,
  setActiveWorkstream,
} = require('../sdd/bin/lib/planning-workspace.cjs');
...
describe('planning-workspace: session adapter precedence', () => {
  ...
  beforeEach(() => {
    savedSession = process.env.SDD_SESSION_KEY;
  });

  afterEach(() => {
    if (savedSession !== undefined) process.env.SDD_SESSION_KEY = savedSession;
    else delete process.env.SDD_SESSION_KEY;
  });

  test('uses session adapter over shared adapter when session key exists', () => {
    process.env.SDD_SESSION_KEY = 'session-123';

    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-planning-precedence-'));
    try {
...
describe('planning-workspace: self-heal behavior', () => {
  ...
  test('clears stale pointers when workstream directory is gone', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-planning-workspace-'));
    try {
...
describe('planning-workspace: lock seam', () => {
  test('exports withPlanningLock and acquires/release lock', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-planning-lock-'));
    try {
...
describe('core compatibility adapter: planning workspace functions', () => {
  ...
  beforeEach(() => {
    savedSession = process.env.SDD_SESSION_KEY;
    delete process.env.SDD_SESSION_KEY;
  });
  ...
  test('core and planning-workspace expose matching behavior', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-core-compat-'));
    try {
