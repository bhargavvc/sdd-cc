...
const { writeSurface, listSurface } = require('../sdd/bin/lib/surface.cjs');
...
function tmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-surface-list-'));
}
...
describe('listSurface', () => {
  test('returns { enabled, disabled, tokenCost } structure', () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(dir, '.sdd-source'), REAL_COMMANDS_DIR, 'utf8');
