...
describe('scan command', () => {
  test('command file exists with correct name and description', () => {
    const p = path.join(__dirname, '..', 'commands', 'sdd', 'scan.md');
    assert.ok(fs.existsSync(p), 'commands/sdd/scan.md should exist');
    const content = fs.readFileSync(p, 'utf-8');
    assert.ok(content.includes('name: sdd:scan'), 'Command must have name: sdd:scan');
  ...
  test('workflow file exists', () => {
    const p = path.join(__dirname, '..', 'sdd', 'workflows', 'scan.md');
    assert.ok(fs.existsSync(p), 'sdd/workflows/scan.md should exist');
  });
  ...
  test('workflow references sdd-codebase-mapper', () => {
    const p = path.join(__dirname, '..', 'sdd', 'workflows', 'scan.md');
    const content = fs.readFileSync(p, 'utf-8');
    assert.ok(content.includes('sdd-codebase-mapper'),
      'Workflow should reference the sdd-codebase-mapper agent');
  });
});
