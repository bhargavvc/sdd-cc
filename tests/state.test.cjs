...
describe('state-snapshot command', () => {
  ...
  test('missing STATE.md returns error', () => {
    const result = runSddTools('state-snapshot', tmpDir);
...
`
  ...
  describe('--cwd override', () => {
    ...
    beforeEach(() => {
      outsideDir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'sdd-test-outside-'));
    });
...
`
      ...
      const result = runSddTools(`state-snapshot --cwd "${tmpDir}"`, outsideDir);
      assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for invalid --cwd path', () => {
    ...
    const result = runSddTools(`state-snapshot --cwd "${invalid}"`, tmpDir);
...
`
    ...
    const result = runSddTools(['state', 'add-blocker', '--text', 'Waiting on vendor quote $1.00 before approval'], tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const result = runSddTools(`state add-blocker --text-file "${blockerPath}"`, tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('state json command', () => {
  ...
  test('missing STATE.md returns error', () => {
    const result = runSddTools('state json', tmpDir);
    assert.ok(result.success, `Command should succeed: ${result.error}`);
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.sdd_state_version, '1.0', 'should have version 1.0');
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.sdd_state_version, '1.0', 'version from frontmatter');
...
`
    ...
    const result = runSddTools('state update Status "Executing Plan 1"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const content = fs.readFileSync(path.join(tmpDir, '.planning', 'STATE.md'), 'utf-8');
    assert.ok(content.startsWith('---\n'), 'should start with frontmatter delimiter');
    assert.ok(content.includes('sdd_state_version: 1.0'), 'should have version field');
...
`
    ...
    const result = runSddTools('state patch --Status "In progress" --"Current Plan" 04-02', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    runSddTools('state update Status "Paused"', tmpDir);
...
`
    ...
    runSddTools('state update "Current Plan" "03-03"', tmpDir);
...
`
    ...
    runSddTools('state update Status "Executing Plan 5"', tmpDir);
...
const { stateExtractField, stateReplaceField, stateReplaceFieldWithFallback } = require('../sdd/bin/lib/state.cjs');
...
describe('cmdStateLoad (state load)', () => {
  ...
  test('returns config and state when STATE.md exists', () => {
    ...
    const result = runSddTools('state load', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns raw key=value format with --raw flag', () => {
    ...
    const result = runSddTools('state load --raw', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdStateGet (state get)', () => {
  ...
  test('returns full content when no section specified', () => {
    ...
    const result = runSddTools('state get', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('extracts bold field value', () => {
    ...
    const result = runSddTools('state get Status', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('extracts markdown section content', () => {
    ...
    const result = runSddTools('state get Blockers', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for nonexistent field', () => {
    ...
    const result = runSddTools('state get Missing', tmpDir);
...
describe('cmdStatePatch and cmdStateUpdate (state patch, state update)', () => {
  ...
  test('state patch updates multiple fields at once', () => {
    ...
    const result = runSddTools('state patch --Status Complete --"Current Phase" 04', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('state patch reports failed fields that do not exist', () => {
    ...
    const result = runSddTools('state patch --Status Done --Missing value', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('state update changes a single field', () => {
    ...
    const result = runSddTools('state update Status "Phase complete"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('state update reports field not found', () => {
    ...
    const result = runSddTools('state update Missing value', tmpDir);
  ...
  test('state update returns error when STATE.md missing', () => {
    const result = runSddTools('state update Status value', tmpDir);
...
describe('cmdStateAdvancePlan (state advance-plan)', () => {
  ...
  test('advances plan counter when not on last plan', () => {
    ...
    const result = runSddTools('state advance-plan', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdStateRecordMetric (state record-metric)', () => {
  ...
  test('appends metric row to existing table', () => {
    ...
    const result = runSddTools('state record-metric --phase 2 --plan 1 --duration 5min --tasks 3 --files 4', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('replaces None yet placeholder with first metric', () => {
    ...
    const result = runSddTools('state record-metric --phase 1 --plan 1 --duration 2min --tasks 1 --files 2', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error when required fields missing', () => {
    ...
    const result = runSddTools('state record-metric --phase 1', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error}`);
  ...
  test('returns error when STATE.md missing', () => {
    const result = runSddTools('state record-metric --phase 1 --plan 1 --duration 2min', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error}`);
...
describe('cmdStateUpdateProgress (state update-progress)', () => {
  ...
  test('calculates progress from plan/summary counts', () => {
    ...
    const result = runSddTools('state update-progress', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdStateResolveBlocker (state resolve-blocker)', () => {
  ...
  test('removes matching blocker line (case-insensitive substring match)', () => {
    ...
    const result = runSddTools('state resolve-blocker --text "api credentials"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('adds None placeholder when last blocker resolved', () => {
    ...
    const result = runSddTools('state resolve-blocker --text "single blocker"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error when text not provided', () => {
    ...
    const result = runSddTools('state resolve-blocker', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error}`);
  ...
  test('returns error when STATE.md missing', () => {
    const result = runSddTools('state resolve-blocker --text "anything"', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error}`);
  ...
  test('returns resolved true even if no line matches', () => {
    ...
    const result = runSddTools('state resolve-blocker --text "nonexistent blocker text"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdStateRecordSession (state record-session)', () => {
  ...
  test('updates Last session timestamp even with no other options', () => {
    ...
    const result = runSddTools('state record-session', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('sets Resume file to None when not specified', () => {
    ...
    const result = runSddTools('state record-session --stopped-at "Phase 1 complete"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('milestone-scoped phase counting in frontmatter', () => {
  ...
  test('total_phases counts only current milestone phases', () => {
    ...
    const result = runSddTools('state update Status "Executing"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    // Read the state json to check frontmatter
    const jsonResult = runSddTools('state json', tmpDir);
  ...
  test('without ROADMAP counts all phases (pass-all filter)', () => {
    ...
    const result = runSddTools('state update Status "In progress"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`;
    ...
    const adv1 = runSddTools(['state', 'advance-plan'], tmpDir);
    ...
    const adv2 = runSddTools(['state', 'advance-plan'], tmpDir);
...
describe('progress counters correct after plan execution (#1589)', () => {
  ...
  test('percent in frontmatter is derived from disk counts, not stale Progress body field', () => {
    ...
    const result = runSddTools('state update Status "Milestone complete"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`;
    ...
    const result = runSddTools('phase complete 3', tmpDir);
...
`;
    ...
    const result = runSddTools('phase complete 4', tmpDir);
    assert.ok(result.success, `phase complete failed: ${result.error}`);
...
`;
    ...
    runSddTools('phase complete 5', tmpDir);
...
describe('state planned-phase command', () => {
  ...
  test('after call: Status is "Ready to execute"', () => {
    ...
    const result = runSddTools(['state', 'planned-phase', '--phase', '3', '--name', 'API', '--plans', '5'], tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('after call: Total Plans matches argument', () => {
    ...
    const result = runSddTools(['state', 'planned-phase', '--phase', '2', '--name', 'Core', '--plans', '7'], tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('after call: Last Activity is today\'s date', () => {
    ...
    const result = runSddTools(['state', 'planned-phase', '--phase', '1', '--name', 'Setup', '--plans', '3'], tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('missing STATE.md returns graceful error', () => {
    // No STATE.md written
    const result = runSddTools(['state', 'planned-phase', '--phase', '1', '--name', 'Test', '--plans', '3'], tmpDir);
...
describe('state validate command', () => {
  ...
  test('STATE says executing + VERIFICATION.md shows passed emits warning', () => {
    ...
    const result = runSddTools('state validate', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
...
describe('state sync command', () => {
  ...
  test('drifted STATE.md + correct filesystem: after sync, fields match disk', () => {
    ...
    const result = runSddTools('state sync', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
  ...
  test('--verify flag reports changes without writing', () => {
    ...
    const result = runSddTools('state sync --verify', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
...
`
    ...
    const syncResult = runSddTools('state sync', tmpDir);
...
describe('stale phase dirs do not corrupt phase counts (bug #2445)', () => {
  ...
  test('init new-milestone phase_dir_count excludes stale prior-milestone dirs', () => {
    ...
    const result = runSddTools('init new-milestone', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
    const output = JSON.parse(result.output);
...
describe('state complete-phase: decorated Phase fallback (#2761 nitpick)', () => {
  ...
  test('writes clean Phase identifier when only Current Position decoration is present', () => {
    ...
    const result = runSddTools('state complete-phase', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('supports explicit phase override for complete-phase disambiguation (#3063)', () => {
    ...
    const result = runSddTools('state complete-phase --phase 3.3', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
