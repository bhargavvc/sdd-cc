...
describe('plan-phase UI-SPEC missing behavior', () => {
  ...
  test('does NOT contain hard-blocking exit redirect to /sdd-ui-phase', () => {
    const text = fs.readFileSync(workflowPath, 'utf8');
    // The hard redirect pattern: AskUserQuestion option exits with "Run /sdd-ui-phase... Exit workflow."
  ...
  test('contains a primary recommendation block for missing UI-SPEC', () => {
    ...
    const hasRecommendationPattern =
      ...
      text.includes('sdd-ui-phase');
    assert.ok(
      hasRecommendationPattern,
      'plan-phase.md must include a "Recommended next step" recommendation for /sdd-ui-phase when UI-SPEC.md is missing'
    );
  });
});
