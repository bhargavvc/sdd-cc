...
/**
 * Bug #2419: sdd-project-researcher agent type not found
 *
 * When sdd-new-project spawns sdd-project-researcher subagents, it fails with
...
const NEW_PROJECT_PATH = path.join(__dirname, '..', 'sdd', 'workflows', 'new-project.md');
const NEW_MILESTONE_PATH = path.join(__dirname, '..', 'sdd', 'workflows', 'new-milestone.md');
const AGENTS_DIR = path.join(__dirname, '..', 'agents');

describe('sdd-project-researcher agent registration (#2419)', () => {
  test('sdd-project-researcher.md exists in agents source dir', () => {
    const agentFile = path.join(AGENTS_DIR, 'sdd-project-researcher.md');
    assert.ok(
      fs.existsSync(agentFile),
      'agents/sdd-project-researcher.md must exist in the source agents directory'
    );
  });

  test('sdd-project-researcher.md has correct name in frontmatter', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'sdd-project-researcher.md'), 'utf-8');
    assert.ok(
      content.includes('name: sdd-project-researcher'),
      'agents/sdd-project-researcher.md must have name: sdd-project-researcher in frontmatter'
    );
  });
  ...
  test('new-project.md lists sdd-project-researcher in available_agent_types', () => {
    const content = fs.readFileSync(NEW_PROJECT_PATH, 'utf-8');
    ...
    assert.ok(
      agentTypesMatch[1].includes('sdd-project-researcher'),
      'new-project.md <available_agent_types> must list sdd-project-researcher'
    );
  });

  test('new-milestone.md lists sdd-project-researcher in available_agent_types', () => {
    const content = fs.readFileSync(NEW_MILESTONE_PATH, 'utf-8');
    const agentTypesMatch = content.match(/<available_agent_types>([\s\S]*?)<\/available_agent_types>/);
    assert.ok(agentTypesMatch, 'new-milestone.md must have <available_agent_types> section');
    assert.ok(
      agentTypesMatch[1].includes('sdd-project-researcher'),
      'new-milestone.md <available_agent_types> must list sdd-project-researcher'
    );
  });
});
