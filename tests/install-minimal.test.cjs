...
const {
  ...
  stageSkillsForMode,
  cleanupStagedSkills,
} = require('../sdd/bin/lib/install-profiles.cjs');
...
describe('install-profiles: MINIMAL_SKILL_ALLOWLIST', () => {
  ...
  test('every allowlisted skill exists in commands/sdd/', () => {
    const commandsDir = path.join(__dirname, '..', 'commands', 'sdd');
...
describe('install-profiles: stageSkillsForMode', () => {
  function createFixtureSkillsDir() {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-stage-fixture-'));
  ...
  test('minimal mode against non-existent source returns the source path (caller handles missing)', () => {
    const ghost = path.join(os.tmpdir(), 'sdd-stage-does-not-exist-' + Date.now());
  ...
  test('minimal mode skips non-md files and subdirectories', () => {
    const src = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-stage-mixed-'));
    let staged;
    try {
...
describe('install-profiles: cleanupStagedSkills', () => {
  test('removes every staged dir created during this process', () => {
    const src = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-stage-cleanup-'));
    fs.writeFileSync(path.join(src, 'plan-phase.md'), '# plan\n');
    try {
  ...
  test('full mode does not register a staged dir (no leak source for default install)', () => {
    cleanupStagedSkills();
    const src = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-stage-fullmode-'));
    fs.writeFileSync(path.join(src, 'plan-phase.md'), '# plan\n');
    const realMkdtemp = fs.mkdtempSync;
    let minimalStageDirCreated = false;
    fs.mkdtempSync = (prefix, ...rest) => {
      if (typeof prefix === 'string' && prefix.endsWith('sdd-minimal-skills-')) {
    ...
    try {
      ...
      assert.equal(
        ...
        false,
        'full mode should not create a sdd-minimal-skills stage dir',
      );
    } finally {
  ...
  test('exit handler registers exactly once across many stageSkillsForMode calls', () => {
    cleanupStagedSkills();
    const src = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-stage-exit-handler-'));
    fs.writeFileSync(path.join(src, 'plan-phase.md'), '# plan\n');
    try {
  ...
  test('SIGINT triggers cleanup and re-raises the signal (Ctrl+C path)', () => {
    ...
    const probe = `
      const { stageSkillsForMode } = require(${JSON.stringify(
        path.join(__dirname, '..', 'sdd', 'bin', 'lib', 'install-profiles.cjs'),
      )});
      const fs = require('fs');
      const path = require('path');
      const os = require('os');
      const src = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-stage-sig-src-'));
      fs.writeFileSync(path.join(src, 'plan-phase.md'), '# plan\\n');
  ...
  test('mid-copy failure removes the partial staged dir and re-throws', () => {
    ...
    // calls stageSkillsForMode) creates and removes `sdd-minimal-skills-*`
    ...
    // parallel processes.
    cleanupStagedSkills();
    const src = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-stage-fail-'));
    fs.writeFileSync(path.join(src, 'plan-phase.md'), '# plan\n');
    ...
    fs.mkdtempSync = (prefix, ...rest) => {
      ...
      // `sdd-minimal-skills-` prefix). Don't capture our own
      // `sdd-stage-fail-` parent dir created above.
      if (typeof prefix === 'string' && prefix.endsWith('sdd-minimal-skills-')) {
        stagedDir = out;
      }
      return out;
...
function listTmpStageDirs() {
  try {
    return fs
      ...
      .filter((n) => n.startsWith('sdd-minimal-skills-'))
      .sort();
  } catch {
    return [];
  }
...
// would leave stale `agents/sdd-*.toml` files plus `[agents.sdd-*]`
// sections in `config.toml`. This test simulates a previous full Codex
// install (a few stale agent files + an existing SDD-marked config.toml)
...
describe('install: Codex full → minimal downgrade cleans stale agent state', () => {
  ...
  function makeStaleCodexInstall(targetDir) {
    ...
    fs.writeFileSync(path.join(agentsDir, 'sdd-executor.md'), 'stale\n');
    fs.writeFileSync(path.join(agentsDir, 'sdd-planner.md'), 'stale\n');
    fs.writeFileSync(path.join(agentsDir, 'sdd-executor.toml'), 'name = "sdd-executor"\n');
    fs.writeFileSync(path.join(agentsDir, 'sdd-planner.toml'), 'name = "sdd-planner"\n');
    ...
    // A previously-written codex config.toml with both SDD and user content,
    // matching the marker format produced by installCodexConfig.
    const codexConfig = [
      '# user-owned setting',
      'model = "gpt-5"',
      '',
      '# SDD Agent Configuration — managed by sdd installer',
      '[agents.sdd-executor]',
      'cmd = "stale"',
      '',
      '[agents.sdd-planner]',
  ...
  test('--minimal removes stale .toml agents and strips [agents.sdd-*] from config.toml', () => {
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-codex-downgrade-'));
    try {
      ...
      // Stale sdd-* files (.md AND .toml) must be gone:
      assert.ok(!remaining.includes('sdd-executor.md'), 'stale sdd-executor.md should be removed');
      assert.ok(!remaining.includes('sdd-planner.md'), 'stale sdd-planner.md should be removed');
      assert.ok(!remaining.includes('sdd-executor.toml'), 'stale sdd-executor.toml should be removed');
      assert.ok(!remaining.includes('sdd-planner.toml'), 'stale sdd-planner.toml should be removed');

      // User-owned agent must survive:
      assert.ok(remaining.includes('my-custom-agent.md'), 'user agent should be preserved');

      // config.toml: SDD section gone, user content preserved
      const configPath = path.join(targetDir, 'config.toml');
      if (fs.existsSync(configPath)) {
        const config = fs.readFileSync(configPath, 'utf8');
        assert.ok(!config.includes('[agents.sdd-executor]'), 'sdd-executor section stripped');
        assert.ok(!config.includes('[agents.sdd-planner]'), 'sdd-planner section stripped');
        assert.ok(config.includes('model = "gpt-5"'), 'user setting preserved');
      }
      // (If config.toml was SDD-only it'd be removed entirely, which is also acceptable —
...
describe('install: Claude full → minimal downgrade removes stale agents', () => {
  ...
  test('--minimal removes stale sdd-*.md agents but preserves user-owned agents', () => {
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-claude-downgrade-'));
    try {
      const agentsDir = path.join(targetDir, 'agents');
      fs.mkdirSync(agentsDir, { recursive: true });
      ...
      const remaining = fs.existsSync(agentsDir) ? fs.readdirSync(agentsDir) : [];
      assert.ok(!remaining.includes('sdd-executor.md'), 'stale sdd-executor.md removed');
      assert.ok(!remaining.includes('sdd-planner.md'), 'stale sdd-planner.md removed');
      assert.ok(remaining.includes('my-custom-agent.md'), 'user agent preserved');

      // No `sdd-*` files at all should remain:
      const stragglers = remaining.filter((f) => f.startsWith('sdd-'));
      assert.deepStrictEqual(stragglers, [], 'no sdd-* files should remain in agents/');
    } finally {
      fs.rmSync(targetDir, { recursive: true, force: true });
    }
  });
...
describe('install: manifest records mode for both profiles', () => {
  ...
  function manifestModeAfterInstall(extraArgs) {
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-manifest-mode-'));
    try {
      spawnSync(
        process.execPath,
      ...
      const manifestPath = path.join(targetDir, 'sdd-file-manifest.json');
...
describe('install-profiles: allowlist scope guards', () => {
  ...
  test('off-loop convenience commands are NOT in the allowlist', () => {
    // These exist in commands/sdd/ and are valid skills, but they're not part
