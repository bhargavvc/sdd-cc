...
const PLANNER_AGENT = path.join(ROOT, 'agents', 'sdd-planner.md');
const PLAN_PHASE_WORKFLOW = path.join(ROOT, 'sdd', 'workflows', 'plan-phase.md');
...
describe('bug #3320 planner action contract', () => {
  test('planner agent explicitly keeps implementation code out of action blocks', () => {
    ...
    assert.match(
      ...
      'sdd-planner.md must explicitly forbid fenced implementation code in <action>'
    );
    assert.match(
      planner,
      /Code excerpts belong in `<read_first>` source files or referenced context/,
      'sdd-planner.md must route code excerpts to context/read-first material'
    );
  });
  ...
  test('quality gate matches the reconciled planner contract', () => {
    const workflow = read('sdd/workflows/plan-phase.md');
