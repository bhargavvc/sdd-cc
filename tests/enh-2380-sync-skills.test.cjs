...
const WORKFLOW = path.join(__dirname, '../sdd/workflows/sync-skills.md');
const COMMAND = path.join(__dirname, '../commands/sdd/sync-skills.md');
  ...
  for (const { runtime, expected } of CASES) {
    test(`resolves correct skills root for ${runtime}`, () => {
      const result = spawnSync(process.execPath, [INSTALL_JS, '--skills-root', runtime], {
        encoding: 'utf-8',
        env: { ...process.env, SDD_TEST_MODE: undefined }, // ensure not in test mode
      });
...
describe('sync-skills.md — required behavioral specs', () => {
  ...
  test('only sdd-* directories are touched (non-SDD preservation)', () => {
    content = content || readWorkflow();
    assert.ok(
      content.includes('sdd-*') && (content.includes('non-SDD') || content.includes('Non-SDD') || content.includes('not starting with')),
      'workflow must document that only sdd-* dirs are modified'
    );
  });
...
// ── commands/sdd/sync-skills.md ───────────────────────────────────────────────
// #2790: sync-skills.md was consolidated into update.md as the --sync flag.

describe('commands/sdd/sync-skills.md', () => {
  test('sync-skills is now --sync flag on update.md (#2790)', () => {
    const updateCmd = path.join(__dirname, '../commands/sdd/update.md');
    assert.ok(fs.existsSync(updateCmd), 'commands/sdd/update.md must exist');
  ...
  test('sync-skills.md command file is deleted (#2790)', () => {
    assert.ok(!fs.existsSync(COMMAND), 'commands/sdd/sync-skills.md should be deleted (consolidated into update.md)');
  });
});
...
describe('INVENTORY sync', () => {
  test('INVENTORY.md lists /sdd-update --sync command (#2790: absorbed /sdd-sync-skills)', () => {
    const inventory = fs.readFileSync(path.join(__dirname, '../docs/INVENTORY.md'), 'utf-8');
    assert.ok(inventory.includes('/sdd-update --sync'), 'INVENTORY.md must list /sdd-update --sync (absorbed /sdd-sync-skills in #2790)');
  });
  ...
  test('INVENTORY-MANIFEST.json includes /sdd-update (#2790: sync-skills absorbed into update.md --sync)', () => {
    // #2790: /sdd-sync-skills was absorbed into /sdd-update as the --sync flag.
    // The manifest now records /sdd-update instead of /sdd-sync-skills.
    const manifest = JSON.parse(
      fs.readFileSync(path.join(__dirname, '../docs/INVENTORY-MANIFEST.json'), 'utf-8')
    );
    assert.ok(
      manifest.families.commands.includes('/sdd-update'),
      'INVENTORY-MANIFEST.json must include /sdd-update in commands (absorbed /sdd-sync-skills via #2790)'
    );
  });
