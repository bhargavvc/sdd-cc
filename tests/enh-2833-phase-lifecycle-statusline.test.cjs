 ...
 *
 *   2. formatSddState renders three new scenes when those fields are populated
...
const {
  parseStateMd,
  formatSddState,
} = require('../hooks/sdd-statusline.js');
...
// ─── formatSddState: new scenes ─────────────────────────────────────────────

describe('formatSddState #2833 lifecycle scenes', () => {
...
describe('formatSddState #2833 backward compatibility', () => {
...
// ─── renderProgressBar (exported indirectly via formatSddState behavior) ────
...
describe('progress bar rendering', () => {
  test('0% renders 10 empty segments', () => {
    ...
    const out = formatSddState({ milestone: 'v2.0', percent: '0' });
    assert.ok(out.includes('[░░░░░░░░░░] 0%'));
  });

  test('50% renders 5 filled + 5 empty', () => {
    const out = formatSddState({ milestone: 'v2.0', percent: '50' });
    assert.ok(out.includes('[█████░░░░░] 50%'));
  });

  test('100% renders 10 filled (and triggers Scene 3)', () => {
    const out = formatSddState({ milestone: 'v2.0', percent: '100' });
    assert.equal(out, 'v2.0 [██████████] 100% · milestone complete');
  });

  test('percent absent → no bar rendered (opt-in)', () => {
    const out = formatSddState({ milestone: 'v2.0', status: 'executing' });
  ...
  test('percent over 100 clamps to 100', () => {
    const out = formatSddState({ milestone: 'v2.0', percent: '150' });
    assert.ok(out.includes('[██████████] 100%'));
  });

  test('percent below 0 clamps to 0', () => {
    const out = formatSddState({ milestone: 'v2.0', percent: '-10' });
    assert.ok(out.includes('[░░░░░░░░░░] 0%'));
  });
});
...
describe('formatSddState #2833 scene priority', () => {
