...
// agents/sdd-code-fixer.md is the deployed agent definition the runtime
...
/**
 * Bug #2990: sdd-code-fixer worktree setup fails when current branch
 ...
 *
 * Fix: create a NEW branch `sdd-reviewfix/${padded_phase}-$$` and attach
...
const AGENT_PATH = path.join(__dirname, '..', 'agents', 'sdd-code-fixer.md');
...
describe('Bug #2990: sdd-code-fixer worktree attaches to a NEW branch, not the user-checked-out one', () => {
  ...
  test('sanity: at least one git-worktree-add invocation exists in the agent definition', () => {
    assert.ok(invocations.length > 0,
      'expected sdd-code-fixer.md to document at least one git worktree add invocation');
  });
...
describe('Bug #2990: cleanup tail fast-forwards $branch and deletes the temp branch on success', () => {
  ...
  test('cleanup tail bash block exists and is parseable', () => {
    assert.notEqual(records, null, 'expected to find a "Cleanup tail" bash block in agents/sdd-code-fixer.md');
