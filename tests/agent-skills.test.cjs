/**
 * SDD Tools Tests - Agent Skills Injection
 *
...
describe('agent-skills command', () => {
  ...
  test('returns empty when no config exists', () => {
    ...
    const result = runSddTools(['agent-skills', 'sdd-executor'], tmpDir, { HOME: tmpDir, USERPROFILE: tmpDir });
  ...
  test('returns empty for unconfigured agent type', () => {
    writeConfig(tmpDir, {
      agent_skills: {
        'sdd-executor': ['skills/test-skill'],
      },
    });
    const result = runSddTools(['agent-skills', 'sdd-planner'], tmpDir, { HOME: tmpDir, USERPROFILE: tmpDir });
    assert.strictEqual(result.output, '');
  });
  ...
  test('handles multiple skill paths', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: {
        'sdd-executor': ['skills/skill-a', 'skills/skill-b'],
      },
    });
  ...
  test('warns for nonexistent skill path but does not error', () => {
    writeConfig(tmpDir, {
      agent_skills: {
        'sdd-executor': ['skills/nonexistent'],
      },
    });
  ...
  test('returns empty when no agent type argument provided', () => {
    const result = runSddTools(['agent-skills'], tmpDir, { HOME: tmpDir, USERPROFILE: tmpDir });
...
describe('config-set agent_skills', () => {
  ...
  test('can set agent_skills via dot notation', () => {
    const result = runSddTools(
      ['config-set', 'agent_skills.sdd-executor', '["skills/my-skill"]'],
      tmpDir,
    ...
    assert.deepStrictEqual(
      config.agent_skills['sdd-executor'],
      ['skills/my-skill'],
...
describe('agent-skills global: prefix', () => {
  ...
  beforeEach(() => {
    ...
    fakeHome = fs.mkdtempSync(path.join(require('os').tmpdir(), 'sdd-1992-home-'));
  ...
  test('global:valid-skill resolves to $HOME/.claude/skills/valid-skill/SKILL.md', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: { 'sdd-executor': ['global:valid-skill'] },
    });

    const result = runSddTools(['agent-skills', 'sdd-executor'], tmpDir, { HOME: fakeHome, USERPROFILE: fakeHome });
  ...
  test('global:invalid!name is rejected by regex and skipped', () => {
    writeConfig(tmpDir, {
      agent_skills: { 'sdd-executor': ['global:invalid!name'] },
    });
  ...
  test('global:missing-skill is skipped when directory is absent', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: { 'sdd-executor': ['global:missing-skill'] },
    });
  ...
  test('mix of global: and project-relative paths both resolve correctly', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: { 'sdd-executor': ['global:shadcn', 'skills/local-skill'] },
    });
  ...
  test('global: with empty name produces clear warning and skips', () => {
    writeConfig(tmpDir, {
      agent_skills: { 'sdd-executor': ['global:'] },
    });

    const result = runSddTools(['agent-skills', 'sdd-executor'], tmpDir, { HOME: fakeHome, USERPROFILE: fakeHome });
    assert.strictEqual(result.output, '', 'should skip empty global: prefix');
    // The warning goes to stderr — cannot assert on it through runSddTools's output field,
