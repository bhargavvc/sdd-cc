 ...
 *
 * Older Codex installs carried legacy SDD SessionStart commands in hooks.json.
...
const { readInstallState } = require('../sdd/bin/lib/installer-migrations.cjs');
...
function legacySddHook(codexHome) {
  return {
    hooks: [{
      type: 'command',
      command: `node "${path.join(codexHome, 'hooks', 'sdd-check-update.js')}"`,
    }],
  };
}
...
function tomlSddHookCount(codexHome) {
  const parsed = parseTomlToObject(fs.readFileSync(path.join(codexHome, 'config.toml'), 'utf8'));
  const sessionStart = parsed.hooks?.SessionStart ?? [];
  return sessionStart
    .flatMap((entry) => Array.isArray(entry.hooks) ? entry.hooks : [])
    .filter((hook) => typeof hook.command === 'string' && hook.command.includes('sdd-check-update.js'))
    .length;
}

describe('#3357 — Codex install removes legacy SDD hooks.json entries', { concurrency: false }, () => {
  let tmpRoot;
  let codexHome;
  ...
  beforeEach(() => {
    ...
    tmpRoot = createTempDir('sdd-3357-');
  ...
  test('rewrites hooks.json to one managed SessionStart hook when file only had legacy managed entry', () => {
    fs.writeFileSync(
      ...
      JSON.stringify({ SessionStart: [legacySddHook(codexHome)] }, null, 2),
    );
    ...
    const managed = commands.filter((cmd) => typeof cmd === 'string' && cmd.includes('sdd-check-update.js'));
    assert.equal(managed.length, 1);
    assert.equal(tomlSddHookCount(codexHome), 0);
  });

  test('preserves user hooks.json entries while removing the legacy SDD hook', () => {
    const userOwnedSameBasenameHook = {
      hooks: [{
        type: 'command',
        command: 'node "/Users/example/bin/sdd-check-update.js"',
      }],
    };
    fs.writeFileSync(
      path.join(codexHome, 'hooks.json'),
      JSON.stringify({ SessionStart: [legacySddHook(codexHome), userHook(), userOwnedSameBasenameHook] }, null, 2),
    );
    ...
    assert.equal(commands.includes('node "/Users/example/bin/sdd-check-update.js"'), true);
    assert.equal(managed.length, 2);
    assert.equal(tomlSddHookCount(codexHome), 0);
  });

  test('restores migrated hooks.json and install state when later Codex validation fails', () => {
    const before = JSON.stringify({ SessionStart: [legacySddHook(codexHome)] }, null, 2);
