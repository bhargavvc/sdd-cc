/**
 * Drift guard: every `sdd-sdk query <cmd>` reference in the repo must
...
const COMMAND_ALIASES_FILE = path.join(REPO_ROOT, 'sdd', 'bin', 'lib', 'command-aliases.generated.cjs');

// Prose tokens that repeatedly appear after `sdd-sdk query` in English
...
function collectReferences() {
  ...
  const refs = [];
  const re = /sdd-sdk\s+query\s+([A-Za-z][-A-Za-z0-9._/]+)(?:\s+([A-Za-z][-A-Za-z0-9._]+))?/g;
...
describe('sdd-sdk query registry integration', () => {
  test('every referenced command resolves to a registered handler', () => {
    ...
    assert.ok(refs.length > 0, 'expected to find sdd-sdk query references');
    ...
    assert.strictEqual(
      offenders.length, 0,
      'Referenced `sdd-sdk query <cmd>` tokens with no handler in ' +
  ...
  test('informational: handlers with no in-repo caller', () => {
    ...
    if (unused.length > 0 && process.env.SDD_LOG_UNUSED_HANDLERS) {
