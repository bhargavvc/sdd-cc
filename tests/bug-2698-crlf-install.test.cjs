...
function writeCodexConfigWithStaleHooks(dir, headerEol, bodyEol) {
  ...
  test('LF config.toml: stale sdd-update-check block removed on reinstall', (t) => {
    ...
    assert.equal(
      hooksJsonCommands.some((cmd) => cmd.includes('sdd-check-update')),
      true,
      'New sdd-check-update hook must appear in hooks.json after reinstall'
    );
  });
