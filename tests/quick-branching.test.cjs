 ...
 *
 * Validates that /sdd-quick exposes branch_name from init and that the Step 2.5
...
function setupFixture(defaultBranch = 'trunk') {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-quick-branching-'));
...
function runStep(bash, cwd, branchName) {
  ...
  const scriptDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-quick-step-'));
