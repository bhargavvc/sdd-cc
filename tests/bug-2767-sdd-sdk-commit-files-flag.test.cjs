/**
 * Bug #2767: Workflows pass paths positionally to `sdd-sdk query commit`.
 *
 ...
 *
 * Primary test: invoke the actual `sdd-sdk query commit[-to-subrepo]` binary
...
/**
 * Invoke `sdd-sdk query <subcommand> <...args>` against a project dir.
...
describe('bug #2767 (behavioral): sdd-sdk query commit --files', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = createTempGitProject('sdd-2767-');
...
describe('bug #2767 (behavioral): commit-to-subrepo requires --files', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempGitProject('sdd-2767-sub-');
    fs.writeFileSync(
...
// rendered into chat, not invoked by sdd-tools.cjs), so a textual guard is
// the only available enforcement layer.

const SCAN_DIRS = ['agents', 'commands', 'sdd', 'docs', 'scripts'];
...
function extractInvocations(filePath) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const m = lines[i].match(/(sdd-sdk\s+query\s+commit(?:-to-subrepo)?\b.*)$/);
    if (!m) continue;
    const tail = m[1].match(/^sdd-sdk\s+query\s+commit(?:-to-subrepo)?(.?)/);
...
describe('bug #2767 (supplementary doc-lint): no positional invocations in shipped .md', () => {
  ...
  test('every sdd-sdk query commit invocation either uses --files or has no path args', () => {
    ...
    if (broken.length > 0) {
      ...
      assert.fail(
        `${broken.length} \`sdd-sdk query commit\` invocation(s) pass paths positionally.\n` +
