'use strict';
// allow-test-rule: reads markdown product files (sdd-executor.md, worktree-path-safety.md) to verify structural protocol — not source-grep

// Regression guards for bug #3097 and #3099.
//
// #3097: sdd-executor's worktree HEAD guard used `if [ -f .git ]` to detect
...
const executorSrc = fs.readFileSync(
  path.join(ROOT, 'agents', 'sdd-executor.md'), 'utf8',
);
const executePhaseSrc = fs.readFileSync(
  path.join(ROOT, 'sdd', 'workflows', 'execute-phase.md'), 'utf8',
);

describe('bug #3097: cwd-drift sentinel in sdd-executor.md', () => {
  test('task_commit_protocol has cwd-drift assertion step (0a)', () => {
    ...
    assert.ok(
      protocol.includes('cwd') || protocol.includes('drift') || protocol.includes('sdd-spawn-toplevel'),
  ...
  test('cwd-drift check precedes HEAD assertion', () => {
    ...
    const protocolEnd = executorSrc.indexOf('</task_commit_protocol>');
    const protocol = executorSrc.slice(protocolIdx, protocolEnd);
    const driftIdx = protocol.search(/cwd.drift|sdd-spawn-toplevel|drift.*assertion/i);
...
describe('bug #3099: absolute-path safety guidance in sdd-executor.md', () => {
  ...
  test('worktree-path-safety.md reference file exists', () => {
    assert.ok(
      fs.existsSync(path.join(ROOT, 'sdd', 'references', 'worktree-path-safety.md')),
      'sdd/references/worktree-path-safety.md does not exist',
    );
  });

  test('worktree-path-safety.md contains cwd-drift and absolute-path guards', () => {
    const safetySrc = fs.readFileSync(
      path.join(ROOT, 'sdd', 'references', 'worktree-path-safety.md'), 'utf8',
    );
    assert.ok(safetySrc.includes('sdd-spawn-toplevel') || safetySrc.includes('cwd-drift'),
