...
const HOOK_PATH = path.join(__dirname, '..', 'hooks', 'sdd-read-guard.js');
...
describe('sdd-read-guard hook', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = createTempDir('sdd-read-guard-');
  });
  ...
  test('hook is registered in build-hooks.js HOOKS_TO_COPY', () => {
    ...
    assert.ok(
      ...
      'sdd-read-guard.js must be in HOOKS_TO_COPY so it ships in hooks/dist/'
    );
  });
