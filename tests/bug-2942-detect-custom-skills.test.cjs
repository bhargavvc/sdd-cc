/**
 * SDD Tools Tests — detect-custom-files misses skills/ directory (#2942)
 *
 * After v1.39.0 skill consolidation (#2790), skills/ became a SDD-managed root.
 * SDD_MANAGED_DIRS was missing 'skills', so user-added skill directories like
...
describe('detect-custom-files — skills/ directory missing from SDD_MANAGED_DIRS (#2942)', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = createTempDir('sdd-2942-skills-');
  });
  ...
  test('detects custom skill file at skills/<name>/SKILL.md', () => {
    writeManifest(tmpDir, {
      'skills/sdd-planner/SKILL.md': '# SDD Planner Skill\n',
    });
  ...
  // Test 2: does not flag SDD-owned skills as custom (manifest-tracked path NOT in custom_files)
  test('does not flag SDD-owned skill as custom when it is tracked in manifest', () => {
    writeManifest(tmpDir, {
      'skills/sdd-planner/SKILL.md': '# SDD Planner Skill\n',
    });
    ...
    assert.ok(Array.isArray(json.custom_files), 'custom_files should be an array');
    assert.ok(
      !json.custom_files.includes('skills/sdd-planner/SKILL.md'),
      `SDD-owned skill should NOT be in custom_files; got: ${JSON.stringify(json.custom_files)}`
    );
  });

  // Test 3: regression guard — still detects custom files in sdd/workflows/
  test('regression: still detects custom files in sdd/workflows/', () => {
    writeManifest(tmpDir, {
      'sdd/workflows/plan-phase.md': '# Plan Phase\n',
      'skills/sdd-planner/SKILL.md': '# SDD Planner Skill\n',
    });

    writeCustomFile(tmpDir, 'sdd/workflows/custom-workflow.md', '# My Custom Workflow\n');
    ...
    const json = JSON.parse(result.output);
    assert.ok(
      json.custom_files.includes('sdd/workflows/custom-workflow.md'),
