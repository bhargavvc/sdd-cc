...
// commands/sdd/*.md files are the deployed skill surface. Their frontmatter
// IS the runtime contract. Checking frontmatter fields checks deployed behaviour.

/**
 * #3156 — plan-phase auto-dispatches to sdd-planner subagent on OpenCode,
 * losing Task tool access.
 *
 * Root cause: commands/sdd/plan-phase.md had `agent: sdd-planner` in its
 ...
 *
 * This test parses the YAML frontmatter of every commands/sdd/*.md file and
 * asserts:
...
// the main agent context (i.e., all SDD commands) must omit this directive.
