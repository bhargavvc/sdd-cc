 ...
 * (i.e. with SDD_TEST_MODE deleted so the real installer runs) MUST include
...
describe('sdk no-sdk guard: installer subprocess calls must include --no-sdk', () => {
  test('all subprocess installer calls in test files include --no-sdk', () => {
    ...
    for (const file of files) {
      ...
      // Only check files that explicitly delete SDD_TEST_MODE — those run
      // the real installer (not the test-mode export).
      if (!src.includes('delete env.SDD_TEST_MODE') &&
          !src.includes('delete process.env.SDD_TEST_MODE')) {
        continue;
      }
