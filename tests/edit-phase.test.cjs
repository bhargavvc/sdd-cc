...
const COMMAND_PATH = path.join(ROOT, 'commands', 'sdd', 'phase.md');
const WORKFLOW_PATH = path.join(ROOT, 'sdd', 'workflows', 'edit-phase.md');

// ─── File existence ──────────────────────────────────────────────────────────

describe('edit-phase: file existence', () => {
  test('commands/sdd/phase.md exists (absorbed edit-phase in #2790)', () => {
    assert.ok(fs.existsSync(COMMAND_PATH), 'commands/sdd/phase.md should exist (consolidates edit-phase)');
  });

  test('sdd/workflows/edit-phase.md exists', () => {
    assert.ok(fs.existsSync(WORKFLOW_PATH), 'sdd/workflows/edit-phase.md should exist');
  });
});
...
describe('edit-phase: command file structure', () => {
  test('consolidated phase.md has correct name frontmatter (#2790)', () => {
    ...
    assert.ok(/^name:\s*sdd:phase/m.test(content), 'name should be sdd:phase (consolidated)');
  });
...
describe('edit-phase: documentation registration', () => {
  test('INVENTORY.md routes edit-phase workflow through consolidated /sdd-phase --edit (#2790)', () => {
    // #2790 absorbed /sdd-edit-phase into /sdd-phase as the --edit flag. The
    ...
    // "Invoked by" column documents /sdd-phase --edit (not the deleted form).
    ...
    assert.ok(
      /\/sdd-phase\s+--edit/.test(invokedBy),
      `edit-phase.md row must list "/sdd-phase --edit" as caller; got: "${invokedBy}"`
    );
    assert.ok(
      !/\/sdd-edit-phase\b/.test(invokedBy),
      `edit-phase.md row must not still cite the deleted /sdd-edit-phase command; got: "${invokedBy}"`
    );
  });
  ...
  test('INVENTORY-MANIFEST.json contains /sdd-phase in commands (#2790: edit-phase absorbed into phase.md)', () => {
    // #2790: /sdd-edit-phase was absorbed into /sdd-phase as the --edit flag.
    // The manifest now records /sdd-phase instead of /sdd-edit-phase.
    const manifest = JSON.parse(
      fs.readFileSync(path.join(ROOT, 'docs', 'INVENTORY-MANIFEST.json'), 'utf-8')
    );
    assert.ok(
      manifest.families.commands.includes('/sdd-phase'),
      'INVENTORY-MANIFEST.json must list /sdd-phase in commands (absorbed /sdd-edit-phase via #2790)'
    );
  });
  ...
  test('docs/COMMANDS.md documents /sdd-phase (absorbed /sdd-edit-phase via --edit flag, #2790)', () => {
    const commands = fs.readFileSync(
      path.join(ROOT, 'docs', 'COMMANDS.md'),
      'utf-8'
    );
    assert.ok(
      commands.includes('/sdd-phase'),
      'docs/COMMANDS.md must document /sdd-phase (which absorbed /sdd-edit-phase via --edit flag in #2790)'
    );
  });
});
