/**
 * SDD Tools Tests - dedicated temp subdirectory
 *
 * Tests for issue #1975: SDD temp files should use a dedicated
 * subdirectory (path.join(os.tmpdir(), 'sdd')) instead of writing
...
const SDD_TEMP_DIR = path.join(os.tmpdir(), 'sdd');

// ─── Dedicated temp subdirectory ────────────────────────────────────────────

describe('dedicated sdd temp subdirectory', () => {
  describe('output() temp file placement', () => {
    ...
    test('sdd temp subdirectory path is os.tmpdir()/sdd', () => {
      // The SDD_TEMP_DIR constant should resolve to <tmpdir>/sdd
      assert.strictEqual(SDD_TEMP_DIR, path.join(os.tmpdir(), 'sdd'));
    });
  });
  ...
  describe('reapStaleTempFiles with subdirectory', () => {
    ...
    beforeEach(() => {
      testPrefix = `sdd-tempsub-test-${Date.now()}-`;
      // Ensure the sdd subdirectory exists for test setup
      fs.mkdirSync(SDD_TEMP_DIR, { recursive: true });
    });

    test('removes stale files from sdd subdirectory', () => {
      const stalePath = path.join(SDD_TEMP_DIR, `${testPrefix}stale.json`);
      ...
      assert.ok(!fs.existsSync(stalePath), 'stale file in sdd subdir should be removed');
    });

    test('preserves fresh files in sdd subdirectory', () => {
      const freshPath = path.join(SDD_TEMP_DIR, `${testPrefix}fresh.json`);
      fs.writeFileSync(freshPath, '{}');

      reapStaleTempFiles(testPrefix, { maxAgeMs: 5 * 60 * 1000 });

      assert.ok(fs.existsSync(freshPath), 'fresh file in sdd subdir should be preserved');
      // Clean up
      fs.unlinkSync(freshPath);
    });

    test('removes stale directories from sdd subdirectory', () => {
      const staleDir = path.join(SDD_TEMP_DIR, `${testPrefix}dir`);
      ...
      assert.ok(!fs.existsSync(staleDir), 'stale directory in sdd subdir should be removed');
    });

    test('creates sdd subdirectory if it does not exist', () => {
      // Use a unique nested path to avoid interfering with other tests
      const uniqueSubdir = path.join(os.tmpdir(), `sdd-creation-test-${Date.now()}`);
      ...
      assert.doesNotThrow(() => {
        reapStaleTempFiles(`sdd-creation-test-${Date.now()}-`, { maxAgeMs: 0 });
      });
    });

    test('does not scan system tmpdir root for sdd- files', () => {
      ...
      // because it now only scans the sdd subdirectory
      reapStaleTempFiles(testPrefix, { maxAgeMs: 5 * 60 * 1000 });
    ...
    test('backward compat: reapStaleTempFilesLegacy cleans old location', () => {
      ...
      const core = require('../sdd/bin/lib/core.cjs');
