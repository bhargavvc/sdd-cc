 ...
 * (the standard /sdd-new-project layout), phase complete silently skips:
...
describe('bug #2005: phase complete updates plan count when milestone is inside <details>', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2005-'));
  ...
  test('plan count is updated when current milestone is wrapped in <details>', () => {
    // This is the standard /sdd-new-project layout: every milestone in <details>
    ...
    try {
      execFileSync('node', [sddTools, 'phase', 'complete', '1'], { cwd: tmpDir, timeout: 10000 });
    } catch {
