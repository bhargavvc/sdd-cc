...
const { cmdGenerateClaudeMd } = require('../sdd/bin/lib/profile-output.cjs');

function makeTempProject(files = {}) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2415-'));
...
test('link mode falls back to embed for workflow section (no linkable source)', () => {
  ...
  assert.ok(!content.includes('@SDD defaults'), 'workflow should not write @SDD defaults');
  assert.ok(content.includes('SDD Workflow Enforcement'), 'workflow content should be embedded inline');
});
