...
describe('debug session management implementation', () => {
  test('DEBUG.md template contains reasoning_checkpoint field', () => {
    const content = fs.readFileSync(
      path.join(process.cwd(), 'sdd/templates/DEBUG.md'),
      'utf8'
    );
  ...
  test('debug command contains list subcommand logic', () => {
    const content = fs.readFileSync(
      path.join(process.cwd(), 'sdd/workflows/debug.md'),
      'utf8'
    );
    assert.ok(
  ...
  test('sdd-debugger contains structured reasoning checkpoint', () => {
    const content = fs.readFileSync(
      path.join(process.cwd(), 'agents/sdd-debugger.md'),
      'utf8'
    );
    assert.ok(content.includes('reasoning_checkpoint'), 'sdd-debugger.md must contain reasoning_checkpoint');
  });

  test('sdd-debugger contains TDD checkpoint mode', () => {
    const content = fs.readFileSync(
      path.join(process.cwd(), 'agents/sdd-debugger.md'),
      'utf8'
    );
    assert.ok(content.includes('tdd_mode'), 'sdd-debugger.md must contain tdd_mode');
    assert.ok(content.includes('TDD CHECKPOINT'), 'sdd-debugger.md must contain TDD CHECKPOINT return format');
  });

  test('sdd-debugger contains delta debugging technique', () => {
    const content = fs.readFileSync(
      path.join(process.cwd(), 'agents/sdd-debugger.md'),
      'utf8'
    );
    assert.ok(content.includes('Delta Debugging'), 'sdd-debugger.md must contain Delta Debugging technique');
  });

  test('sdd-debugger contains security note about DATA_START', () => {
    const content = fs.readFileSync(
      path.join(process.cwd(), 'agents/sdd-debugger.md'),
      'utf8'
    );
    assert.ok(content.includes('DATA_START'), 'sdd-debugger.md must contain DATA_START security reference');
  });
});

// Tests for #2148 and #2151
describe('debug skill dispatch and sub-orchestrator (#2148, #2151)', () => {
  test('sdd-debugger ROOT CAUSE FOUND format includes specialist_hint field', () => {
    const content = fs.readFileSync(path.join(process.cwd(), 'agents', 'sdd-debugger.md'), 'utf8');
    assert.ok(content.includes('specialist_hint'), 'sdd-debugger missing specialist_hint in ROOT CAUSE FOUND');
    assert.ok(content.includes('swift_concurrency'), 'sdd-debugger missing specialist_hint derivation guidance');
  });

  test('debug.md orchestrator has specialist skill dispatch step', () => {
    const content = fs.readFileSync(path.join(process.cwd(), 'sdd/workflows/debug.md'), 'utf8');
  ...
  test('sdd-debug-session-manager uses DATA_START/DATA_END for checkpoint responses', () => {
    const content = fs.readFileSync(path.join(process.cwd(), 'agents', 'sdd-debug-session-manager.md'), 'utf8');
    assert.ok(content.includes('DATA_START') && content.includes('DATA_END'),
      'sdd-debug-session-manager missing security boundaries on checkpoint responses');
  });

  test('sdd-debug-session-manager has compact summary output format', () => {
    const content = fs.readFileSync(path.join(process.cwd(), 'agents', 'sdd-debug-session-manager.md'), 'utf8');
    assert.ok(content.includes('DEBUG SESSION COMPLETE'), 'session manager missing compact summary format');
  });

  test('sdd-debug-session-manager includes anti-heredoc rule', () => {
    const content = fs.readFileSync(path.join(process.cwd(), 'agents', 'sdd-debug-session-manager.md'), 'utf8');
    assert.ok(content.includes('heredoc'), 'session manager missing anti-heredoc rule');
  });

  test('debug.md delegates to sdd-debug-session-manager', () => {
    const content = fs.readFileSync(path.join(process.cwd(), 'sdd/workflows/debug.md'), 'utf8');
    assert.ok(content.includes('sdd-debug-session-manager'),
