...
const COMMAND_PATH = path.join(ROOT, 'commands', 'sdd', 'plan-phase.md');
const WORKFLOW_PATH = path.join(ROOT, 'sdd', 'workflows', 'plan-phase.md');
...
describe('enh #3209: plan-phase ADR ingest express path', () => {
  ...
  test('docs COMMANDS advertises --ingest flag for /sdd-plan-phase', () => {
    const commands = read(DOCS_COMMANDS_PATH);
    assert.ok(commands.includes('--ingest <path-or-glob>'),
      'docs/COMMANDS.md must document --ingest for /sdd-plan-phase');
  });
});
