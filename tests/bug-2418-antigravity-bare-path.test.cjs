 ...
 *
 * Files affected: agents/sdd-debugger.md (configDir = ~/.claude) and
 * sdd/workflows/update.md (comment with e.g. ~/.claude).
 */
...
describe('convertClaudeToAntigravityContent bare path replacement (#2418)', () => {
  describe('global install', () => {
    ...
    test('still replaces ~/.claude/ (with trailing slash) correctly', () => {
      ...
      const result = convertClaudeToAntigravityContent(input, true);
      assert.ok(
        result.includes('~/.gemini/antigravity/sdd/workflows/'),
  ...
  describe('installed files contain no bare ~/.claude references after conversion', () => {
    ...
    test('sdd-debugger.md has no leaked ~/.claude after global Antigravity conversion', () => {
      const debuggerPath = path.join(repoRoot, 'agents', 'sdd-debugger.md');
      ...
      assert.strictEqual(
        matches, null,
        `sdd-debugger.md still contains leaked .claude paths after Antigravity conversion: ${matches}`
      );
    });

    test('update.md has no leaked ~/.claude after global Antigravity conversion', () => {
      const updatePath = path.join(repoRoot, 'sdd', 'workflows', 'update.md');
