...
const WORKFLOW = path.join(__dirname, '..', 'sdd', 'workflows', 'plan-phase.md');
...
function parseWorkflowContract(content) {
  ...
  const plannerLines = lines.filter(line => line.includes('planner') || line.includes('sdd-planner'));
...
describe('plan-phase workflow — --mvp flag', () => {
  ...
  test('planner spawn passes MVP_MODE to sdd-planner', () => {
...
describe('plan-phase --mvp — resolution chain integration', () => {
  ...
  test('roadmap.get-phase reports mode=mvp when set in roadmap', () => {
    ...
    const result = runSddTools('roadmap get-phase 1 --pick mode', tmpDir);
    assert.ok(result.success);
    assert.strictEqual(result.output.trim(), 'mvp');
  });

  test('config-get workflow.mvp_mode default is empty/unset', () => {
    const result = runSddTools('config-get workflow.mvp_mode', tmpDir);
