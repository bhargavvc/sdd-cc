 ...
 *
 * `--uninstall` did not remove `sdd-file-manifest.json` from the target
 ...
 * after cleaning up the rest of the SDD artefacts.
 */
...
function createFakeInstall(prefix = 'sdd-uninstall-test-') {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), prefix));

  // Simulate the minimum directory/file layout produced by the installer:
  // sdd/ directory, agents/ directory, and the manifest file.
  fs.mkdirSync(path.join(dir, 'sdd', 'workflows'), { recursive: true });
  fs.writeFileSync(path.join(dir, 'sdd', 'workflows', 'execute-phase.md'), '# stub');

  fs.mkdirSync(path.join(dir, 'agents'), { recursive: true });
  fs.writeFileSync(path.join(dir, 'agents', 'sdd-executor.md'), '# stub');

  const manifest = {
    version: '1.34.0',
    timestamp: new Date().toISOString(),
    files: {
      'sdd/workflows/execute-phase.md': 'abc123',
      'agents/sdd-executor.md': 'def456',
    },
  };
...
describe('uninstall — manifest cleanup (#1908)', () => {
  ...
  test('sdd-file-manifest.json is removed after global uninstall', () => {
  ...
  test('sdd-file-manifest.json is removed after local uninstall', () => {
    const manifestPath = path.join(tmpDir, MANIFEST_NAME);
    ...
    fs.mkdirSync(path.join(localDir, 'sdd', 'workflows'), { recursive: true });
    fs.writeFileSync(path.join(localDir, 'sdd', 'workflows', 'execute-phase.md'), '# stub');
