...
function requireStateCjs() {
  return require(path.join(ROOT, 'sdd', 'bin', 'lib', 'state.cjs'));
}

function makeTempPlanning(stateContent) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3127-'));
...
const MID_FLIGHT_STATE = `# SDD State
...
const PRE_EXECUTE_STATE = `# SDD State
