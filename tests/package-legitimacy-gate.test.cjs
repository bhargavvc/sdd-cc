...
const RESEARCHER = path.join(AGENTS, 'sdd-phase-researcher.md');
const PLANNER = path.join(AGENTS, 'sdd-planner.md');
const EXECUTOR = path.join(AGENTS, 'sdd-executor.md');
...
describe('sdd-phase-researcher.md — slopcheck invocation', () => {
  let model;
...
describe('sdd-phase-researcher.md — Package Legitimacy Audit section in template', () => {
...
describe('sdd-phase-researcher.md — ecosystem-specific package verification', () => {
  let model;
...
describe('sdd-phase-researcher.md — no npx --yes auto-download', () => {
  let model;
...
describe('sdd-phase-researcher.md — WebSearch-origin package tagging', () => {
  let model;
...
describe('sdd-planner.md — checkpoint gate for [ASSUMED]/[SUS] packages', () => {
  let model;
...
describe('sdd-planner.md — supply-chain row in threat_model template', () => {
  let planTemplate;
...
describe('sdd-planner.md — no npx --yes auto-download', () => {
  test('does not invoke npx --yes inside a code block', () => {
    const model = readModel(PLANNER);
    const found = model.codeBlocks.some((block) => hasAllTokens(block, ['npx', '--yes']));
...
describe('sdd-executor.md — package installs excluded from RULE 3 auto-fix', () => {
  let model;
