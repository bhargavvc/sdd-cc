 ...
 * `node .claude/hooks/sdd-context-monitor.js`. Claude Code persists the bash
...
describe('bug #1906: local hook commands use $CLAUDE_PROJECT_DIR', () => {
  ...
  test('local command projection for non-Gemini keeps $CLAUDE_PROJECT_DIR anchor', () => {
    ...
    const command = projectShellCommandText({
      ...
      argTokens: [`${prefix}/hooks/sdd-context-monitor.js`],
      runtime: 'claude',
      platform: 'linux',
    });
    assert.equal(
      command,
      '"/usr/local/bin/node" "$CLAUDE_PROJECT_DIR"/.claude/hooks/sdd-context-monitor.js',
    );
  });
});
