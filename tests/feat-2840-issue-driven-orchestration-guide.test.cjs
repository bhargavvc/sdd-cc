 ...
 *   - One guide explaining issue-driven orchestration using existing SDD
 *     commands.
...
function parseGuide() {
  ...
  // sdd-new-workspace, .planning/, etc. must survive).
  const stripped = content.replace(/\*{1,3}|~{2}/g, '');

  // Concept-mapping table: rows that pair a Symphony-style concept with a
  // SDD primitive. Test asserts on presence of each required pair, not on
  ...
  // guide. This prevents drift if /sdd-* command names are renamed.
  ...
  // SDD primitive that must appear in the same paragraph/row of the
  ...
  const conceptPairs = conceptMappingSection
    ? {
        ...
        workspaceCommand: /\/sdd-workspace\s+--new/.test(conceptMappingSection),
        executionCommand:
          /\/sdd-manager/.test(conceptMappingSection) ||
          /\/sdd-autonomous/.test(conceptMappingSection),
        verifyCommand: /\/sdd-verify-work/.test(conceptMappingSection),
        reviewCommand: /\/sdd-review/.test(conceptMappingSection),
        shipCommand: /\/sdd-ship/.test(conceptMappingSection),
      }
    : null;
...
describe('issue-driven-orchestration guide (#2840)', () => {
  ...
  test('every required SDD command is referenced at least once', () => {
    const ir = parseGuide();
  ...
  test('concept mapping section exists and pairs Symphony concepts with SDD primitives', () => {
    const ir = parseGuide();
    ...
    const expected = {
      ...
      workspaceCommand: '/sdd-workspace --new must appear in the concept mapping',
      executionCommand:
        '/sdd-manager or /sdd-autonomous must appear in the concept mapping',
      verifyCommand: '/sdd-verify-work must appear in the concept mapping',
      reviewCommand: '/sdd-review must appear in the concept mapping',
      shipCommand: '/sdd-ship must appear in the concept mapping',
    };
