...
function parseCodexHookBlock(block) {
  ...
  // The block always carries the "# SDD Hooks" marker, the AoT tables,
  // a type=command, and a command="<runner> <quoted-hook-path>" line.
  const hasMarker = /^# SDD Hooks$/m.test(block);
...
describe('Bug #3017 / #3440: Codex hook projection seam', () => {
  test('projectCodexHookTomlCommand renders escaped command value from shared projection module', () => {
    const commandValue = projectCodexHookTomlCommand({
      ...
      scriptPath: '/tmp/codex-test/.codex/hooks/sdd-check-update.js',
      platform: 'linux',
    });
    assert.equal(
      commandValue,
      '\\"/usr/local/bin/node\\" \\"/tmp/codex-test/.codex/hooks/sdd-check-update.js\\"',
    );
  });
});
...
describe('Bug #3017: buildCodexHookBlock emits absolute node runner', () => {
  ...
  test('emits the EXACT absolute node runner the caller supplied (#3022 CR)', () => {
    ...
    assert.equal(parsed.hasMarker, true, '# SDD Hooks marker present');
    ...
    assert.equal(parsed.hookPath, '/tmp/codex-test/.codex/hooks/sdd-check-update.js',
...
describe('Bug #3017: rewriteLegacyCodexHookBlock migrates bare-node on reinstall', () => {
  ...
  test('rewrites a bare-node managed-hook command to the absolute runner', () => {
    ...
    assert.equal(parsed.hookPath, '/Users/x/.codex/hooks/sdd-check-update.js');
    // Non-SDD content (the [model] block) must be preserved verbatim.
  ...
  test('decodes TOML-escaped quoted script paths before projection', () => {
    const before = [
      ...
      '[[hooks.SessionStart.hooks]]',
      'type = "command"',
      'command = "node \\"C:\\\\Users\\\\x\\\\.codex\\\\hooks\\\\sdd-check-update.js\\""',
      '',
    ].join('\n');
    ...
    const expected = projectCodexHookTomlCommand({
      ...
      scriptPath: 'C:\\Users\\x\\.codex\\hooks\\sdd-check-update.js',
      platform: 'win32',
    });
    ...
    assert.equal(parsed.hookPath, 'C:/Users/x/.codex/hooks/sdd-check-update.js',
  ...
  test('does NOT touch a managed-hook entry that already uses an absolute runner', () => {
    const already = [
      ...
      '[[hooks.SessionStart.hooks]]',
      'type = "command"',
      'command = "\\"/usr/local/bin/node\\" /Users/x/.codex/hooks/sdd-check-update.js"',
      '',
    ].join('\n');
  ...
  test('does NOT touch user-authored bare-node hooks (filename not in managed allowlist)', () => {
    ...
    assert.equal(result.changed, false,
      'user-authored hooks must be left alone; only managed sdd-* hooks are migrated');
    assert.equal(result.content, userOwned);
  });

  test('returns content unchanged when absoluteRunner is null', () => {
    const before = 'command = "node /path/to/sdd-check-update.js"';
