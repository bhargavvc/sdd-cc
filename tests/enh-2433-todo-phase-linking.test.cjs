...
/**
 * Tests for sdd-new-milestone todo-to-phase linking (#2433).
...
const NEW_MILESTONE = fs.readFileSync(
  path.join(ROOT, 'sdd/workflows/new-milestone.md'), 'utf-8'
);
const EXECUTE_PHASE = fs.readFileSync(
  path.join(ROOT, 'sdd/workflows/execute-phase.md'), 'utf-8'
);
...
test('new-milestone.md: step 10.5 commits tagged todos', () => {
  assert.ok(NEW_MILESTONE.includes('sdd-sdk query commit'), 'should commit tagged todos');
