 ...
 *   1. Third-person framing (no "You are a SDD X" opening in <role>)
...
const AUDIT_AGENTS = [
  ...
  'sdd-security-auditor.md',
  'sdd-verifier.md',
  'sdd-eval-auditor.md',
  'sdd-nyquist-auditor.md',
  'sdd-ui-auditor.md',
  'sdd-integration-checker.md',
  'sdd-doc-verifier.md',
];
...
describe('enh-2427 — sycophancy hardening: audit-class agents', () => {
  ...
  for (const filename of AUDIT_AGENTS) {
    ...
    describe(label, () => {
      ...
      test('(1) third-person framing — <role> does not open with "You are a SDD"', () => {
        content = content || readAgent(AGENTS_DIR, filename);
        role = role || extractRole(content);
        const firstSentence = role.trim().slice(0, 80);
        assert.ok(
          !firstSentence.startsWith('You are a SDD'),
          `${filename}: <role> must not open with "You are a SDD" — use third-person submission framing. Got: "${firstSentence}"`
        );
      });
