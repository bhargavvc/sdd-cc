 ...
 *
 * Structural assertions for the /sdd-import command and workflow files.
 */
...
const CMD_PATH = path.join(__dirname, '..', 'commands', 'sdd', 'import.md');
const WF_PATH = path.join(__dirname, '..', 'sdd', 'workflows', 'import.md');

// ─── File Existence ────────────────────────────────────────────────────────────

describe('import command file structure', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(CMD_PATH), 'commands/sdd/import.md should exist');
  });

  test('workflow file exists', () => {
    assert.ok(fs.existsSync(WF_PATH), 'sdd/workflows/import.md should exist');
  });
});
...
describe('import command frontmatter', () => {
  ...
  test('has name field', () => {
    assert.match(content, /^name:\s*sdd:import$/m);
  });
...
describe('import command references', () => {
  ...
  test('references the import workflow', () => {
    assert.ok(
      content.includes('@~/.claude/sdd/workflows/import.md'),
      'command should reference the workflow via @~/.claude/sdd/workflows/import.md'
    );
  });
});
...
describe('import workflow content', () => {
  ...
  test('includes plan-checker validation gate', () => {
    assert.ok(
      content.includes('sdd-plan-checker'),
      'workflow should delegate validation to sdd-plan-checker'
    );
  });

  test('no-args usage display is present', () => {
    assert.ok(
      content.includes('Usage: /sdd:import') || content.includes('Usage: /sdd-import'),
