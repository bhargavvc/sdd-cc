 ...
 *
 * Validates that sdd-phase-researcher maps capabilities to architectural
 * tiers before diving into framework-specific research. Also validates
 * that sdd-planner and sdd-plan-checker consume the Architectural
...
const TEMPLATES_DIR = path.join(__dirname, '..', 'sdd', 'templates');

// ─── Phase Researcher: Architectural Responsibility Mapping ─────────────────

describe('phase-researcher: Architectural Responsibility Mapping', () => {
  const researcherPath = path.join(AGENTS_DIR, 'sdd-phase-researcher.md');
  ...
  test('contains Architectural Responsibility Mapping step', () => {
    assert.ok(
      ...
      'sdd-phase-researcher.md must contain "Architectural Responsibility Map"'
    );
  });
...
describe('planner: Architectural Responsibility Map sanity check', () => {
  ...
  test('references Architectural Responsibility Map', () => {
    assert.ok(
      content.includes('Architectural Responsibility Map'),
      'sdd-planner.md must reference the Architectural Responsibility Map'
    );
  });
  ...
  test('includes sanity check against the map', () => {
    ...
    assert.ok(
      ...
      'sdd-planner.md must include a sanity check against the Architectural Responsibility Map'
    );
  });
});
...
describe('plan-checker: Architectural Tier verification dimension', () => {
  ...
  test('has verification dimension for architectural tier', () => {
    assert.ok(
      ...
      'sdd-plan-checker.md must have a verification dimension for architectural tier mapping'
    );
  });
