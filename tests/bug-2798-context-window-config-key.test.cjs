 ...
 *
 * `sdd-sdk query config-set context_window <n>` was rejected with
...
describe('bug-2798: context_window is a valid config key', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('sdd-test-2798-');
    fs.writeFileSync(
  ...
  test('config-schema CJS and SDK allowlists both include context_window', () => {
    const cjsSchema = require(path.join(REPO_ROOT, 'sdd', 'bin', 'lib', 'config-schema.cjs'));
