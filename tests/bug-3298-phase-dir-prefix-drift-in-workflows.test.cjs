'use strict';
/**
 * Regression test for #3298 — phase-dir prefix drift in /sdd-plan-milestone-gaps,
 * /sdd-import, and /sdd-capture --backlog workflows (PRED.k015 sibling audit).
 *
 ...
 * paths. PR #3292 (#3287) fixed `/sdd-discuss-phase` and `/sdd-plan-phase`.
 *
...
const PMG_WF = path.join(
  __dirname, '..', 'sdd', 'workflows', 'plan-milestone-gaps.md',
);
const IMPORT_WF = path.join(
  __dirname, '..', 'sdd', 'workflows', 'import.md',
);
const BACKLOG_WF = path.join(
  __dirname, '..', 'sdd', 'workflows', 'add-backlog.md',
);
...
describe('bug-3298 — import.md must not construct bare {NN}-{slug} phase dirs', () => {
  ...
  test('plan_convert step must call init.phase-op to resolve the prefixed dir', () => {
    ...
    assert.ok(
      ...
      'import.md must call sdd-sdk query init.phase-op to get expected_phase_dir with project_code prefix',
    );
  });
});
