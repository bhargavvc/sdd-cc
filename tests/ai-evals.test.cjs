/**
 * SDD AI Evals Framework Tests
 *
 * Validates the /sdd-ai-integration-phase + /sdd-eval-review contribution:
...
const COMMANDS_DIR   = path.join(REPO_ROOT, 'commands', 'sdd');
const WORKFLOWS_DIR  = path.join(REPO_ROOT, 'sdd', 'workflows');
const TEMPLATES_DIR  = path.join(REPO_ROOT, 'sdd', 'templates');
const REFERENCES_DIR = path.join(REPO_ROOT, 'sdd', 'references');
...
describe('CONFIG: config-set / config-get workflow.ai_integration_phase', () => {
  ...
  test('config-set workflow.ai_integration_phase false persists as boolean false', () => {
    const result = runSddTools('config-set workflow.ai_integration_phase false', tmpDir);
  ...
  test('config-set workflow.ai_integration_phase true persists as boolean true', () => {
    runSddTools('config-set workflow.ai_integration_phase false', tmpDir);
    const result = runSddTools('config-set workflow.ai_integration_phase true', tmpDir);
    assert.ok(result.success, `config-set failed: ${result.error}`);
  ...
  test('config-get workflow.ai_integration_phase returns the stored value', () => {
    runSddTools('config-set workflow.ai_integration_phase false', tmpDir);
    const result = runSddTools('config-get workflow.ai_integration_phase', tmpDir);
...
describe('HEALTH --repair: addAiIntegrationPhaseKey', () => {
  ...
  test('adds workflow.ai_integration_phase via addAiIntegrationPhaseKey repair', () => {
    ...
    const result = runSddTools('validate health --repair', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('COMMAND: ai-integration-phase and eval-review frontmatter', () => {
  ...
  for (const cmd of commands) {
    test(`${cmd}.md exists`, () => {
      ...
      assert.ok(fs.existsSync(p), `commands/sdd/${cmd}.md should exist`);
    });
  ...
  test('ai-integration-phase.md name is sdd:ai-integration-phase', () => {
    const content = fs.readFileSync(path.join(COMMANDS_DIR, 'ai-integration-phase.md'), 'utf-8');
    assert.ok(content.includes('name: sdd:ai-integration-phase'), 'ai-integration-phase command name mismatch');
  });

  test('eval-review.md name is sdd:eval-review', () => {
    const content = fs.readFileSync(path.join(COMMANDS_DIR, 'eval-review.md'), 'utf-8');
    assert.ok(content.includes('name: sdd:eval-review'), 'eval-review command name mismatch');
  });
});
...
describe('WORKFLOW: plan-phase.md AI nudge integration', () => {
  ...
  test('plan-phase.md references /sdd-ai-integration-phase nudge', () => {
    const content = fs.readFileSync(planPhasePath, 'utf-8');
    assert.ok(
...
describe('WORKFLOW: ai-integration-phase and eval-review workflow files', () => {
  ...
  test('ai-integration-phase.md orchestrates 4 agents', () => {
    ...
    for (const agent of ['sdd-framework-selector', 'sdd-ai-researcher', 'sdd-domain-researcher', 'sdd-eval-planner']) {
      assert.ok(content.includes(agent), `ai-integration-phase.md should reference ${agent}`);
    }
  });

  test('eval-review.md references sdd-eval-auditor', () => {
    const content = fs.readFileSync(path.join(WORKFLOWS_DIR, 'eval-review.md'), 'utf-8');
    assert.ok(content.includes('sdd-eval-auditor'), 'eval-review.md should reference sdd-eval-auditor');
  });
