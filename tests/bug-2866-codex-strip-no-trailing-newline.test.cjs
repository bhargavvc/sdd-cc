 ...
 *
 * The cleanup regexes in `bin/install.js` matched stale SDD hook blocks
 ...
 * saw `sdd-check-update` already present, skipped writing the new
 ...
 * helper, `stripStaleSddHookBlocks(configContent)`, exported from
...
const installPath = path.resolve(REPO_ROOT, pkg.bin['@bhargavvc/sdd-cc']);
const { stripStaleSddHookBlocks } = require(installPath);
...
const SHAPES = {
  'Shape 1 (legacy sdd-update-check)': [
    '# SDD Hooks',
    '[[hooks]]',
    'event = "SessionStart"',
    'command = "node /Users/USER/.codex/hooks/sdd-update-check.js"',
  ].join('\n'),
  'Shape 2 (flat [[hooks]] + sdd-check-update)': [
    '# SDD Hooks',
    '[[hooks]]',
    'event = "SessionStart"',
    'command = "node /Users/USER/.codex/hooks/sdd-check-update.js"',
  ].join('\n'),
...
describe('bug-2866: stripStaleSddHookBlocks handles end-of-file without trailing newline', () => {
  test('stripStaleSddHookBlocks is exported from bin/install.js', () => {
    assert.strictEqual(typeof stripStaleSddHookBlocks, 'function',
      'bin/install.js must export stripStaleSddHookBlocks');
  });
  ...
  function assertStripped(out, shape, scenario) {
    ...
    const staleCmd = [...shape_.keys.entries()].find(([_, v]) =>
      /sdd-(update-check|check-update)/.test(v));
    assert.strictEqual(staleCmd, undefined,
      `(${shape}, ${scenario}) no key may carry a stale sdd-*-update command, got: ${staleCmd && staleCmd.join('=')}`);
  ...
  for (const [shape, block] of Object.entries(SHAPES)) {
    test(`${shape}: stripped when terminated by trailing newline`, () => {
      ...
      assertStripped(stripStaleSddHookBlocks(input), shape, 'with trailing newline');
    });
    ...
    test(`${shape}: stripped when at end-of-file without trailing newline`, () => {
      ...
      const input = `[history]\npersistence = "save-all"\n${block}`;
      assertStripped(stripStaleSddHookBlocks(input), shape, 'no trailing newline');
    });
  }

  test('returns input unchanged when no SDD hook block is present', () => {
    const benign = '[history]\npersistence = "save-all"\n';
    const out = stripStaleSddHookBlocks(benign);
    assert.strictEqual(out, benign, 'helper must be a no-op when no SDD reference exists');
  ...
  for (const [variation, block] of Object.entries(VARIATIONS)) {
    test(`variation stripped: ${variation}`, () => {
      const input = `[history]\npersistence = "save-all"\n${block}\n`;
      assertStripped(stripStaleSddHookBlocks(input), variation, 'with trailing newline');
    });
    test(`variation stripped at EOF without trailing newline: ${variation}`, () => {
      const input = `[history]\npersistence = "save-all"\n${block}`;
      assertStripped(stripStaleSddHookBlocks(input), variation, 'no trailing newline');
    });
  }

  test('user-authored [[hooks.UserPromptSubmit]] is preserved', () => {
    // The structural strip must not touch hook tables that don't carry a
    // SDD-managed `sdd-(check-update|update-check).js` command.
    const input = [
      '[history]',
      'persistence = "save-all"',
    ...
    const out = stripStaleSddHookBlocks(input);
  ...
  test('Shape 4 strip does not leave an orphaned [[hooks.SessionStart]] header', () => {
    ...
    const out = stripStaleSddHookBlocks(`[history]\npersistence = "save-all"\n${block}`);
