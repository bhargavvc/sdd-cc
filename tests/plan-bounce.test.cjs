...
const SDD_ROOT = path.join(__dirname, '..', 'sdd');
const CONFIG_TEMPLATE_PATH = path.join(SDD_ROOT, 'templates', 'config.json');
const PLAN_PHASE_PATH = path.join(SDD_ROOT, 'workflows', 'plan-phase.md');
...
describe('Plan Bounce: config keys', () => {
  ...
  test('config-set accepts workflow.plan_bounce_script', () => {
    const tmpDir = createTempProject();
    try {
      const result = runSddTools('config-set workflow.plan_bounce_script ./bounce.sh', tmpDir);
  ...
  test('config-set accepts workflow.plan_bounce_passes', () => {
    const tmpDir = createTempProject();
    try {
      const result = runSddTools('config-set workflow.plan_bounce_passes 2', tmpDir);
