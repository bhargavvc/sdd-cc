// allow-test-rule: source-text-is-the-product — commands/sdd/*.md files ARE the
 ...
 *
 * Authoritative behavioral contract for every commands/sdd/*.md file.
 ...
 *
 * Contract:
 *   1. name:          present, non-empty, starts with sdd: or sdd-
...
const SDD_ROOT     = path.join(ROOT, 'sdd');
...
describe('command contract: name field (ADR-0002)', () => {
  for (const { name, full } of commandFiles) {
    test(`${name}: name: present and starts with sdd: or sdd-`, () => {
      const fm = parseFrontmatter(fs.readFileSync(full, 'utf-8'));
      assert.ok(fm.name && fm.name.trim(), `${name}: name: field missing or empty`);
      assert.ok(
        /^sdd[:-]/.test(fm.name.trim()),
        `${name}: name: must start with "sdd:" or "sdd-", got "${fm.name.trim()}"`,
      );
    });
  }
});
  ...
  for (const { name, full } of commandFiles) {
    test(`${name}: all execution_context @-refs exist on disk`, () => {
      ...
      for (const { normalized } of refs) {
        assert.ok(
          fs.existsSync(path.join(SDD_ROOT, normalized)),
