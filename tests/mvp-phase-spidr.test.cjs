...
const WORKFLOW = path.join(__dirname, '..', 'sdd', 'workflows', 'mvp-phase.md');
...
function parseMvpPhaseContract(content) {
  ...
  const planPhaseStepIndex = lowerLines.findIndex(line => line.includes('## 7. delegate to /sdd plan-phase'));
...
describe('mvp-phase workflow', () => {
  ...
  test('delegates to /sdd plan-phase after ROADMAP write', () => {
