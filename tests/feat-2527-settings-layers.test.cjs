...
/**
 * Feature test for #2527 — /sdd-settings expands to 22 settings grouped into
 ...
 * step, the confirmation table, the ~/.sdd/defaults.json save step, and
 * VALID_CONFIG_KEYS.
 *
 * Closes: #2527
 */
...
const SETTINGS_PATH = path.join(__dirname, '..', 'sdd', 'workflows', 'settings.md');
...
describe('#2527: settings.md adds grouped settings layers', () => {
    ...
    test('save_as_defaults step references every new field', () => {
      ...
      for (const field of NEW_FIELDS) {
        assert.ok(
          ...
          `save_as_defaults step must persist "${field}" into ~/.sdd/defaults.json`
        );
      }
    });
  });
  ...
  describe('Negative: config-set rejects an unknown key path', () => {
    test('config-set workflow.code_review_bogus_key fails', (t) => {
      ...
      const bad = runSddTools(['config-set', 'workflow.code_review_bogus_key', 'x'], tmpDir);
