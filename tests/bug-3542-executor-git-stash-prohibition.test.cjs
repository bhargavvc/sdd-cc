...
const EXECUTOR_PATH = path.join(__dirname, '..', 'agents', 'sdd-executor.md');

// ─── Test A — prompt content asserts the prohibition ───────────────────────

test('bug-3542: sdd-executor.md prohibits `git stash` family inside worktrees', () => {
  ...
  assert.match(
    content,
    /`git stash`/,
    'sdd-executor.md must explicitly forbid `git stash` (bare push) — see #3542',
  );
  assert.match(
    content,
    /`git stash pop`/,
    'sdd-executor.md must explicitly forbid `git stash pop` — the load-bearing footgun (#3542)',
  );
  assert.match(
    content,
    /`git stash apply`/,
    'sdd-executor.md must explicitly forbid `git stash apply` — same shared-stack hazard as pop (#3542)',
  );
  assert.match(
    content,
    /`git stash drop`/,
    'sdd-executor.md must explicitly forbid `git stash drop` — mutates the shared stack (#3542)',
  );
  ...
  assert.match(
    ...
    'sdd-executor.md must document that stash storage is shared across worktrees (#3542)',
  );
  ...
  assert.ok(
    ...
    'sdd-executor.md must document an alternative to `git stash` ' +
  ...
  assert.match(
    content,
    /#3542/,
    'sdd-executor.md must reference issue #3542 next to the stash prohibition for traceability',
  );
});
  ...
  try {
    ...
    assert.match(
      ...
      'bug #3542 invariant: stash entries pushed from any worktree (or the ' +
        ...
        'in agents/sdd-executor.md can be relaxed.',
    );
