...
/**
 * SDD Tools Tests - Hermes Agent Skills Migration
 *
 * Tests for installing SDD for Hermes Agent using the standard
...
describe('Hermes Agent: copyCommandsAsClaudeSkills', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-hermes-test-'));
  });
  ...
  test('creates skills/sdd-xxx/SKILL.md directory structure', () => {
    ...
    copyCommandsAsClaudeSkills(srcDir, skillsDir, 'sdd', '/test/prefix/', 'hermes', false);
    ...
    assert.strictEqual(fm.name, 'sdd-quick', 'frontmatter name uses hyphen form (#2808)');
  ...
  test('replaces ~/.claude/ paths with pathPrefix', () => {
    ...
    const skillsDir = path.join(tmpDir, 'dest', 'skills');
    copyCommandsAsClaudeSkills(srcDir, skillsDir, 'sdd', '$HOME/.hermes/', 'hermes', false);
  ...
  test('removes stale sdd- skills before installing new ones', () => {
    ...
    copyCommandsAsClaudeSkills(srcDir, skillsDir, 'sdd', '/test/', 'hermes', false);
  ...
  test('preserves agent field in frontmatter', () => {
    ...
    const fm = parseFrontmatter(content);
    assert.strictEqual(fm.agent, 'sdd-executor', 'agent field preserved');
  });
});
...
describe('Hermes Agent: SKILL.md format validation', () => {
  test('SKILL.md frontmatter parses with required Hermes fields', () => {
    ...
    const result = convertClaudeCommandToClaudeSkill(input, 'sdd-review', 'hermes');
    const fm = parseFrontmatter(result);

    assert.strictEqual(fm.name, 'sdd-review', 'name uses hyphen form');
    ...
    assert.strictEqual(fm.agent, 'sdd-code-reviewer', 'agent preserved');
  ...
  test('omits version field when runtime is not hermes (parity with non-Hermes skill consumers)', () => {
    ...
    assert.strictEqual(fm.name, 'sdd-plan');
  });
});
