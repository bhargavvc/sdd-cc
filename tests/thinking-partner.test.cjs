...
describe('Thinking Partner Integration (#1726)', () => {
  ...
  describe('Reference document', () => {
    const refPath = path.join(SDD_ROOT, 'references', 'thinking-partner.md');
  ...
  describe('Config integration', () => {
    test('config-set accepts features.thinking_partner', () => {
      ...
      try {
        const setResult = runSddTools('config-set features.thinking_partner true', tmpDir);
  ...
  describe('Discuss-phase integration', () => {
    function readDiscussFamily() {
      const candidates = [
        path.join(SDD_ROOT, 'workflows', 'discuss-phase.md'),
        path.join(SDD_ROOT, 'workflows', 'discuss-phase', 'modes', 'default.md'),
        path.join(SDD_ROOT, 'workflows', 'discuss-phase', 'modes', 'advisor.md'),
      ];
      return candidates
  ...
  describe('Plan-phase integration', () => {
    test('plan-phase.md contains thinking partner conditional block', () => {
      const content = fs.readFileSync(
        path.join(SDD_ROOT, 'workflows', 'plan-phase.md'),
        'utf-8'
      );
      assert.ok(
