...
} = require('../sdd/bin/lib/security.cjs');
...
// Verify that sdd-context-monitor and sdd-statusline reject session_id values
...
describe('sdd-context-monitor session_id path traversal', () => {
  const monitorPath = path.join(__dirname, '..', 'hooks', 'sdd-context-monitor.js');
...
describe('sdd-statusline session_id path traversal', () => {
  const statuslinePath = path.join(__dirname, '..', 'hooks', 'sdd-statusline.js');
  const tmpDir = os.tmpdir();
  ...
  test('does not write bridge file for session_id with ../ traversal', () => {
    const maliciousId = '../../../etc/sdd-test';
...
describe('validatePromptStructure', () => {
  ...
  test('accepts known valid tags in agent files', () => {
    const text = [
      ...
      '<available_agent_types>sdd-executor</available_agent_types>',
    ].join('\n');
