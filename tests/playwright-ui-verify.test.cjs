...
describe('Playwright-MCP UI verification integration', () => {
  test('verify-work.md mentions automated UI verification', () => {
    const content = fs.readFileSync(
      path.join(__dirname, '..', 'sdd', 'workflows', 'verify-work.md'), 'utf-8'
    );
    assert.ok(
  ...
  test('ui-review.md mentions Playwright-MCP when available', () => {
    const content = fs.readFileSync(
      path.join(__dirname, '..', 'sdd', 'workflows', 'ui-review.md'), 'utf-8'
    );
    assert.ok(
  ...
  test('sdd-ui-auditor.md includes automated screenshot guidance', () => {
    const content = fs.readFileSync(
      path.join(__dirname, '..', 'agents', 'sdd-ui-auditor.md'), 'utf-8'
    );
    assert.ok(
      content.toLowerCase().includes('playwright') || content.includes('screenshot') || content.includes('automated'),
      'sdd-ui-auditor.md should mention automated screenshot verification'
    );
  });
