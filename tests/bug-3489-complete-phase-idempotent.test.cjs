 ...
 *
 *   `sdd state complete-phase --phase <N>` was non-idempotent. Re-invoking it
...
describe('bug #3489: state complete-phase must be idempotent', () => {
  ...
  test('re-running complete-phase on an already-complete phase does not roll STATE.md back', () => {
    ...
    // (e.g. a re-run of /sdd-execute-phase) must NOT regress this content.
    const stateMd = [
      '---',
      'milestone: v1.0',
      '---',
    ...
    const result = runSddTools(['state', 'complete-phase', '--phase', '02.2'], tmpDir);
  ...
  test('completing the currently in-progress phase still works normally (no false-positive idempotency)', () => {
    ...
    const result = runSddTools(['state', 'complete-phase', '--phase', '03'], tmpDir);
