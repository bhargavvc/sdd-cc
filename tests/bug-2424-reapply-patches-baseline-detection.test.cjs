 ...
 * through multiple SDD update cycles, this returns a stale, many-versions-old
...
// sdd/workflows/update.md is the installed runtime workflow —
...
const UPDATE_MD = path.join(__dirname, '..', 'commands', 'sdd', 'update.md');
...
describe('reapply-patches pristine baseline detection (#2424)', () => {
  ...
  test('update.md workflow references backup-meta.json for pristine-hash baseline', () => {
    ...
    // baseline source) is implemented in the update.md workflow (sdd/workflows/update.md),
    // not the command file. The command delegates via --reapply flag.
    // Verify the underlying workflow has this content.
    const workflowPath = path.join(__dirname, '..', 'sdd', 'workflows', 'update.md');
    const workflowContent = fs.readFileSync(workflowPath, 'utf-8');
    assert.ok(
      workflowContent.includes('backup-meta.json'),
      'sdd/workflows/update.md must reference backup-meta.json (pristine_hashes baseline source)'
    );
  });
