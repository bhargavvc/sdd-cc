/**
 * SDD Tools Tests - config.cjs
 *
 * CLI integration tests for config-ensure-section, config-set, and config-get
 * commands exercised through sdd-tools.cjs via execSync.
 *
...
describe('config-ensure-section command', () => {
  ...
  test('creates config.json with expected structure and types', () => {
    const result = runSddTools('config-ensure-section', tmpDir);
    ...
    // Verify structure and types — exact values may vary if ~/.sdd/defaults.json exists
  ...
  test('is idempotent — returns already_exists on second call', () => {
    const first = runSddTools('config-ensure-section', tmpDir);
    assert.ok(first.success, `First call failed: ${first.error}`);
    const firstOutput = JSON.parse(first.output);
    assert.strictEqual(firstOutput.created, true);

    const second = runSddTools('config-ensure-section', tmpDir);
  ...
  test('detects Brave Search from file-based key', () => {
    // runSddTools sandboxes HOME=tmpDir, so brave_api_key is written there —
    // no real filesystem side effects, cleanup happens via afterEach.
    const sddDir = path.join(tmpDir, '.sdd');
    fs.mkdirSync(sddDir, { recursive: true });
    fs.writeFileSync(path.join(sddDir, 'brave_api_key'), 'test-key', 'utf-8');

    const result = runSddTools('config-ensure-section', tmpDir, { HOME: tmpDir, USERPROFILE: tmpDir });
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('merges user defaults from defaults.json', () => {
    // runSddTools sandboxes HOME=tmpDir, so defaults.json is written there —
    // no real filesystem side effects, cleanup happens via afterEach.
    const sddDir = path.join(tmpDir, '.sdd');
    fs.mkdirSync(sddDir, { recursive: true });
    fs.writeFileSync(path.join(sddDir, 'defaults.json'), JSON.stringify({
...
describe('config-set command', () => {
  ...
  beforeEach(() => {
    ...
    runSddTools('config-ensure-section', tmpDir);
  });
  ...
  test('sets a top-level string value', () => {
    const result = runSddTools('config-set model_profile quality', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('coerces true to boolean', () => {
    const result = runSddTools('config-set commit_docs true', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('coerces false to boolean', () => {
    const result = runSddTools('config-set commit_docs false', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('coerces numeric strings to numbers', () => {
    const result = runSddTools('config-set granularity 42', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('preserves plain strings', () => {
    const result = runSddTools('config-set model_profile hello', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('sets nested values via dot-notation', () => {
    const result = runSddTools('config-set workflow.research false', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('rejects unknown config keys', () => {
    const result = runSddTools('config-set workflow.nyquist_validation_enabled false', tmpDir);
  ...
  test('sets workflow.text_mode for remote session support', () => {
    ...
    const result = runSddTools('config-set workflow.text_mode true', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('sets workflow.use_worktrees to disable worktree isolation', () => {
    ...
    const result = runSddTools('config-set workflow.use_worktrees false', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('sets git.base_branch for non-main default branches', () => {
    ...
    const result = runSddTools('config-set git.base_branch master', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('sets intel.enabled to opt into the intel subsystem', () => {
    ...
    const result = runSddTools('config-set intel.enabled true', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('errors when no key path provided', () => {
    const result = runSddTools('config-set', tmpDir);
    assert.strictEqual(result.success, false);
  });
...
describe('config-get command', () => {
  ...
  test('gets a top-level value', () => {
    const result = runSddTools('config-get model_profile', tmpDir, { HOME: tmpDir, USERPROFILE: tmpDir });
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('gets a nested value via dot-notation', () => {
    const result = runSddTools('config-get workflow.research', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('errors for nonexistent key', () => {
    const result = runSddTools('config-get nonexistent_key', tmpDir);
    assert.strictEqual(result.success, false);
    assert.ok(
  ...
  test('errors for deeply nested nonexistent key', () => {
    const result = runSddTools('config-get workflow.nonexistent', tmpDir);
    assert.strictEqual(result.success, false);
    assert.ok(
      result.error.includes('Key not found'),
      `Expected "Key not found" in error: ${result.error}`
  ...
  describe('when config.json does not exist', () => {
    ...
    test('errors when config.json does not exist', () => {
      const result = runSddTools('config-get model_profile', emptyTmpDir);
      assert.strictEqual(result.success, false);
      assert.ok(
  ...
  test('gets git.base_branch after it is set', () => {
    runSddTools('config-set git.base_branch master', tmpDir);
    const result = runSddTools('config-get git.base_branch', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('errors when no key path provided', () => {
    const result = runSddTools('config-get', tmpDir);
    assert.strictEqual(result.success, false);
  });
});
...
describe('config-new-project command', () => {
  ...
  test('creates full config with all expected keys', () => {
    ...
    const result = runSddTools(['config-new-project', choices], tmpDir, { HOME: tmpDir, USERPROFILE: tmpDir });
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('works with empty choices — all defaults materialized', () => {
    const result = runSddTools(['config-new-project', '{}'], tmpDir, { HOME: tmpDir, USERPROFILE: tmpDir });
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('is idempotent — returns already_exists if config exists', () => {
    ...
    const first = runSddTools(['config-new-project', choices], tmpDir);
    assert.ok(first.success, `First call failed: ${first.error}`);
    const firstOut = JSON.parse(first.output);
    assert.strictEqual(firstOut.created, true);

    const second = runSddTools(['config-new-project', choices], tmpDir);
    assert.ok(second.success, `Second call failed: ${second.error}`);
  ...
  test('auto_advance in workflow choices is preserved', () => {
    ...
    const result = runSddTools(['config-new-project', choices], tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('rejects invalid JSON choices', () => {
    const result = runSddTools(['config-new-project', '{not-json}'], tmpDir);
    assert.strictEqual(result.success, false);
...
describe('config-set research_before_questions and discuss_mode', () => {
  ...
  test('workflow.research_before_questions is a valid config key', () => {
    const result = runSddTools('config-set workflow.research_before_questions true', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('workflow.discuss_mode is a valid config key', () => {
    const result = runSddTools('config-set workflow.discuss_mode assumptions', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('hooks.research_questions is rejected with suggestion', () => {
    const result = runSddTools('config-set hooks.research_questions true', tmpDir);
    assert.strictEqual(result.success, false);
    assert.ok(
      result.error.includes('Unknown config key'),
      `Expected "Unknown config key" in error: ${result.error}`
...
describe('config-set unknown key (no suggestion)', () => {
  ...
  test('rejects a key that has no suggestion', () => {
    const result = runSddTools('config-set totally.unknown.key value', tmpDir);
    assert.strictEqual(result.success, false);
    assert.ok(
      result.error.includes('Unknown config key'),
      `Expected "Unknown config key" in error: ${result.error}`
...
describe('config-get edge cases', () => {
  ...
  test('errors when traversing a dot-path through a non-object value', () => {
    ...
    const result = runSddTools('config-get model_profile.something', tmpDir);
    assert.strictEqual(result.success, false);
    assert.ok(
      result.error.includes('Key not found'),
      `Expected "Key not found" in error: ${result.error}`
  ...
  test('errors when config.json contains malformed JSON', () => {
    ...
    const result = runSddTools('config-get model_profile', tmpDir);
    assert.strictEqual(result.success, false);
    assert.ok(
...
describe('config-set-model-profile command', () => {
  ...
  test('sets a valid profile and updates config', () => {
    const result = runSddTools('config-set-model-profile quality', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('reports previous profile in output', () => {
    const result = runSddTools('config-set-model-profile budget', tmpDir, { HOME: tmpDir, USERPROFILE: tmpDir });
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('setting the same profile is a no-op on config but still succeeds', () => {
    ...
    runSddTools('config-set-model-profile quality', tmpDir);
    const result = runSddTools('config-set-model-profile quality', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('is case-insensitive', () => {
    const result = runSddTools('config-set-model-profile BALANCED', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('rejects invalid profile', () => {
    const result = runSddTools('config-set-model-profile turbo', tmpDir);
    assert.strictEqual(result.success, false);
    assert.ok(
  ...
  test('errors when no profile provided', () => {
    const result = runSddTools('config-set-model-profile', tmpDir);
    assert.strictEqual(result.success, false);
  });
  ...
  describe('when config is missing', () => {
    ...
    test('creates config if missing before setting profile', () => {
      const result = runSddTools('config-set-model-profile budget', emptyDir);
      assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('config-set workflow.skip_discuss', () => {
  ...
  test('workflow.skip_discuss is a valid config key', () => {
    const result = runSddTools('config-set workflow.skip_discuss true', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('skip_discuss can be toggled back to false', () => {
    runSddTools('config-set workflow.skip_discuss true', tmpDir);
    const result = runSddTools('config-set workflow.skip_discuss false', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  describe('skip_discuss in config-new-project', () => {
    ...
    test('skip_discuss is present in config-new-project output', () => {
      const result = runSddTools(['config-new-project', '{}'], emptyDir, { HOME: emptyDir, USERPROFILE: emptyDir });
      assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    test('skip_discuss can be set via config-new-project choices', () => {
      ...
      const result = runSddTools(['config-new-project', choices], emptyDir, { HOME: emptyDir, USERPROFILE: emptyDir });
      assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('config-get workflow.skip_discuss returns the set value', () => {
    runSddTools('config-set workflow.skip_discuss true', tmpDir);
    const result = runSddTools('config-get workflow.skip_discuss', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('config-set/config-get workflow.use_worktrees', () => {
  ...
  test('config-get workflow.use_worktrees returns false after setting to false', () => {
    runSddTools('config-set workflow.use_worktrees false', tmpDir);
    const result = runSddTools('config-get workflow.use_worktrees', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('config-get workflow.use_worktrees returns true after setting to true', () => {
    runSddTools('config-set workflow.use_worktrees true', tmpDir);
    const result = runSddTools('config-get workflow.use_worktrees', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('config-set/config-get context', () => {
  ...
  test('config set context dev succeeds', () => {
    const result = runSddTools('config-set context dev', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('config set context research succeeds', () => {
    const result = runSddTools('config-set context research', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('config set context review succeeds', () => {
    const result = runSddTools('config-set context review', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('config get context returns the set value', () => {
    runSddTools('config-set context dev', tmpDir);
    const result = runSddTools('config-get context', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('config set context rejects invalid values', () => {
    const result = runSddTools('config-set context foobar', tmpDir);
    assert.strictEqual(result.success, false);
    assert.ok(
  ...
  test('all three context profile files exist', () => {
    const contextsDir = path.join(__dirname, '..', 'sdd', 'contexts');
...
describe('config-path command (#2282)', () => {
  ...
  test('returns root config path when no workstream is active', () => {
    const result = runSddTools('config-path', tmpDir);
  ...
  test('returns workstream config path when SDD_WORKSTREAM is set', () => {
    const result = runSddTools('config-path', tmpDir, { SDD_WORKSTREAM: 'my-stream' });
    assert.ok(result.success, `config-path failed: ${result.error}`);
  ...
  test('config-path and config-get agree on the active path', () => {
    ...
    runSddTools('config-set model_profile quality', tmpDir);
    // config-path should point to a file containing that value
    const pathResult = runSddTools('config-path', tmpDir);
