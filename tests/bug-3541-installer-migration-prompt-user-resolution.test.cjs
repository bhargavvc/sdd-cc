 ...
 * `/sdd-update` unrecoverable when leftover `sdd-*` files were classified
 * as `stale-sdd-looking`.
 *
 * Fix shape (per triage brief):
 *   A. Classify-and-default for safe categories - stale SDK build
 *      artifacts default to "remove"; user-facing skills/sdd-asterisk/SKILL.md
...
describe('#3541: installer migration prompt-user non-TTY resolution', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    configDir = createTempDir('sdd-3541-');
  });
  ...
  test('Test A: non-TTY default resolution removes stale SDK artifacts and keeps user skills', () => {
    ...
    // 24 stale `sdd/sdk/{dist,src}/sdd-*` files leaked into the
    // baseline because the new manifest no longer classifies them as managed.
    writeFile(configDir, 'sdd/sdk/dist/sdd-old-bundle.js', 'stale sdk bundle\n');
    // User-facing skill: replicates `skills/sdd-roadmap/SKILL.md` from the
    // same incident — user-owned content that must be preserved.
    writeFile(configDir, 'skills/sdd-roadmap/SKILL.md', '# Roadmap skill\nuser content\n');

    // Plant an empty manifest so both files classify as `stale-sdd-looking`
    // (they look like SDD artifacts but are not manifest-managed).
    ...
    assert.deepEqual(
      blockedPaths,
      ['sdd/sdk/dist/sdd-old-bundle.js', 'skills/sdd-roadmap/SKILL.md'],
    ...
    const sdkResolution = byPath.get('sdd/sdk/dist/sdd-old-bundle.js');
    const skillResolution = byPath.get('skills/sdd-roadmap/SKILL.md');
  ...
  test('Test B: error message groups paths by reason and suggests a resolution path', () => {
    ...
    const blocked = [
      {
        type: 'prompt-user',
        relPath: 'sdd/sdk/dist/sdd-a.js',
        reason: 'SDD-looking file is not proven manifest-managed and needs explicit user choice',
        classification: 'stale-sdd-looking',
        prompt: 'Choose whether to remove this stale-looking SDD artifact or keep it as user-owned.',
        choices: ['keep', 'remove'],
      },
      {
        type: 'prompt-user',
        relPath: 'sdd/sdk/dist/sdd-b.js',
        reason: 'SDD-looking file is not proven manifest-managed and needs explicit user choice',
        classification: 'stale-sdd-looking',
        prompt: 'Choose whether to remove this stale-looking SDD artifact or keep it as user-owned.',
        choices: ['keep', 'remove'],
    ...
    assert.match(
      message,
      /SDD_INSTALLER_MIGRATION_RESOLVE/,
  ...
  test('Test C: non-TTY env override resolves otherwise-unclassified prompt-user actions', () => {
    const result = {
      blocked: [
        {
          type: 'prompt-user',
          relPath: 'skills/sdd-custom/SKILL.toml',
    ...
    const resolved = resolveInstallerMigrationPromptsForNonTty(result, {
      isTty: false,
      env: { SDD_INSTALLER_MIGRATION_RESOLVE: 'keep' },
    });
    ...
    assert.equal(resolved.resolutions[0].source, 'SDD_INSTALLER_MIGRATION_RESOLVE');
