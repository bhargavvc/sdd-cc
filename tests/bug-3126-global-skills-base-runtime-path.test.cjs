...
//
// Fix introduces sdd/bin/lib/runtime-homes.cjs with first-class
// support for all 15 supported runtimes, including:
//   - hermes: nested skills/sdd/<skillName>/ layout (#2841)
...
} = require(path.join(ROOT, 'sdd', 'bin', 'lib', 'runtime-homes.cjs'));
...
describe('bug #3126: runtime-homes getGlobalSkillsBase', () => {
  ...
  test('hermes: skills at <configDir>/skills/sdd (nested layout #2841)', () => {
    withEnv('HERMES_HOME', undefined, () => {
      assert.strictEqual(
        getGlobalSkillsBase('hermes'),
        path.join(os.homedir(), '.hermes', 'skills', 'sdd'),
      );
    });
  });
...
describe('bug #3126: runtime-homes getGlobalSkillDir', () => {
  test('cursor: <configDir>/skills/<skillName>', () => {
    withEnv('CURSOR_CONFIG_DIR', undefined, () => {
      assert.strictEqual(
        getGlobalSkillDir('cursor', 'sdd-executor'),
        path.join(os.homedir(), '.cursor', 'skills', 'sdd-executor'),
      );
    });
  });
  test('hermes: <configDir>/skills/sdd/<skillName>', () => {
    withEnv('HERMES_HOME', undefined, () => {
      assert.strictEqual(
        getGlobalSkillDir('hermes', 'sdd-executor'),
        path.join(os.homedir(), '.hermes', 'skills', 'sdd', 'sdd-executor'),
      );
    });
  });
  test('cline: returns null', () => {
    assert.strictEqual(getGlobalSkillDir('cline', 'sdd-executor'), null);
  });
});
...
describe('bug #3126: init.cjs uses runtime-homes not hardcoded .claude', () => {
  test('init.cjs has no hardcoded globalSkillsBase assignment to ~/.claude/skills', () => {
    ...
    const src = fs.readFileSync(
      path.join(ROOT, 'sdd', 'bin', 'lib', 'init.cjs'),
      'utf8',
    );
    assert.ok(
