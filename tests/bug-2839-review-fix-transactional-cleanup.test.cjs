 ...
 *
 * /sdd-code-review-fix cleanup tail is non-transactional. If the agent is
 ...
 * orphan worktree and branch — a future run, /sdd-resume-work, or
 * /sdd-progress can detect and complete the recovery.
 */
...
describe('bug-2839: /sdd-code-review-fix cleanup is transactional', () => {
  let agentPath;
  let agentContent;
  let frontmatter;

  before(() => {
    agentPath = path.join(__dirname, '..', 'agents', 'sdd-code-fixer.md');
  ...
  test('agent declares a recovery sentinel filename', () => {
    assert.ok(
      ...
      `sdd-code-fixer.md must reference the recovery sentinel ${SENTINEL_NAME} so an interrupted cleanup tail is discoverable (#2839)`
    );
  });
  ...
  test('sentinel records worktree path, branch, and padded_phase as JSON fields', () => {
    for (const key of ['worktree_path', 'branch', 'padded_phase']) {
      assert.ok(
        ...
        `recovery sentinel must record \`${key}\` so a future /sdd-resume-work or /sdd-progress can locate the orphan state (#2839)`
      );
    }
  });
