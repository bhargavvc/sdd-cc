/**
 * Tests for modular decomposition of agents/sdd-planner.md
 *
 * Verifies that:
 *   1. sdd-planner.md stays under the 100K agent file threshold
 *   2. sdd-planner.md is under 45K chars (proving the three mode sections were extracted)
 *   3. The three reference files exist
 *   4. sdd-planner.md contains reference pointers to each extracted file
...
const PLANNER_PATH = path.join(PROJECT_ROOT, 'agents', 'sdd-planner.md');
const GAP_CLOSURE_REF = path.join(PROJECT_ROOT, 'sdd', 'references', 'planner-gap-closure.md');
const REVISION_REF = path.join(PROJECT_ROOT, 'sdd', 'references', 'planner-revision.md');
const REVIEWS_REF = path.join(PROJECT_ROOT, 'sdd', 'references', 'planner-reviews.md');

// ─── sdd-planner.md size ─────────────────────────────────────────────────────

describe('sdd-planner.md size constraints', () => {
  ...
  test('planner is under 100K chars (agent file threshold)', () => {
    ...
    assert.ok(
      ...
      `sdd-planner.md is ${content.length} chars, exceeds 100K agent threshold`
    );
  });
  ...
  test('planner is under 45K chars (proves mode sections were extracted)', () => {
    ...
    assert.ok(
      ...
      `sdd-planner.md is ${content.length} chars, expected < 45K after extracting mode sections`
    );
  });
});
...
// ─── sdd-planner.md contains reference pointers ──────────────────────────────

describe('sdd-planner.md contains reference pointers to extracted files', () => {
  let plannerContent;
  ...
  test('planner references planner-gap-closure.md', () => {
    ...
    assert.ok(
      ...
      'sdd-planner.md must reference planner-gap-closure.md'
    );
  });
  ...
  test('planner references planner-revision.md', () => {
    ...
    assert.ok(
      ...
      'sdd-planner.md must reference planner-revision.md'
    );
  });
  ...
  test('planner references planner-reviews.md', () => {
    ...
    assert.ok(
      ...
      'sdd-planner.md must reference planner-reviews.md'
    );
  });
});
