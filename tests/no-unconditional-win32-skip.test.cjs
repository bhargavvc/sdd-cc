 ...
 *
 * Specifically guards against trySelfLinkSddSdk silently no-op'ing on
...
describe('Windows parity guard for trySelfLinkSddSdk (#2962)', () => {
  let tmpDir;
  let origPlatform;
  let origExecSync;

  before(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-win32-guard-'));
  ...
  test('trySelfLinkSddSdk dispatches to the Windows handler and writes shims (does NOT silently return null)', () => {
    const shimSrc = path.join(ROOT, 'bin', 'sdd-sdk.js');
    const result = installModule.trySelfLinkSddSdk(shimSrc);

    assert.notEqual(
      result,
      null,
      'trySelfLinkSddSdk must not silently return null on Windows; ' +
        'a no-op skip is a missed-parity regression (see #2962, #2775).',
    );
    assert.ok(
      fs.existsSync(path.join(tmpDir, 'sdd-sdk.cmd')),
      'Windows dispatch must materialize sdd-sdk.cmd in the npm global bin',
    );
  });
});
