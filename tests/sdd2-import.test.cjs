...
function makeSdd2Project(tmpDir, opts = {}) {
  ...
  fs.writeFileSync(path.join(sddDir, 'PROJECT.md'), '# My Project\n\nA test project.\n');
  fs.writeFileSync(path.join(sddDir, 'REQUIREMENTS.md'), [
    '# Requirements',
    '',
    '## Active',
    '',
...
/** Build a two-milestone SDD-2 project. */
function makeTwoMilestoneProject(tmpDir) {
  const sddDir = path.join(tmpDir, '.sdd');
  const m001Dir = path.join(sddDir, 'milestones', 'M001');
  const m002Dir = path.join(sddDir, 'milestones', 'M002');
  ...
  fs.writeFileSync(path.join(sddDir, 'PROJECT.md'), '# Multi-milestone Project\n');
...
describe('parseSdd2', () => {
  ...
  test('handles missing .sdd/milestones/ gracefully', () => {
    const sddDir = path.join(tmpDir, '.sdd');
...
describe('buildPlanningArtifacts', () => {
  ...
  test('SUMMARY.md strips SDD-2 frontmatter and adds v1 frontmatter', () => {
    ...
    // SDD-2 frontmatter field should not appear
