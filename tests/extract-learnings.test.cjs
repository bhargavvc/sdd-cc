...
const COMMAND_PATH = path.join(__dirname, '..', 'commands', 'sdd', 'extract_learnings.md');
const WORKFLOW_PATH = path.join(__dirname, '..', 'sdd', 'workflows', 'extract_learnings.md');

describe('extract-learnings command', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(COMMAND_PATH), 'commands/sdd/extract_learnings.md should exist');
  });

  test('command file has correct name frontmatter', () => {
    const content = fs.readFileSync(COMMAND_PATH, 'utf-8');
    assert.ok(content.includes('name: sdd:extract-learnings'), 'Command must have name: sdd:extract-learnings');
  });
