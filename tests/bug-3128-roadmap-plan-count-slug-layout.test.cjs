...
// This misses the {N}-PLAN-{NN}-{slug}.md layout that sdd-plan-phase
...
const roadmapLib = path.join(ROOT, 'sdd', 'bin', 'lib', 'roadmap.cjs');
const planScanLib = path.join(ROOT, 'sdd', 'bin', 'lib', 'plan-scan.cjs');
...
function makeTempPhase(files) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3128-'));
