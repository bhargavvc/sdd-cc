...
const REFERENCES_DIR = path.join(ROOT, 'sdd', 'references');
const TEMPLATES_DIR = path.join(ROOT, 'sdd', 'templates');
...
describe('sdd-planner.md — required structural sections (#2091, #2092)', () => {
  let plannerContent;

  test('planner file exists and is readable', () => {
    const plannerPath = path.join(AGENTS_DIR, 'sdd-planner.md');
    assert.ok(fs.existsSync(plannerPath), 'agents/sdd-planner.md must exist');
  ...
  test('contains <planner_authority_limits> section', () => {
    assert.ok(
      ...
      'sdd-planner.md must contain a <planner_authority_limits> section defining what the planner cannot decide'
    );
  });
  ...
  test('contains multi-source coverage audit (not just D-XX decisions)', () => {
    assert.ok(
      ...
      plannerContent.includes('multi-source coverage audit'),
      'sdd-planner.md must contain a multi-source coverage audit, not just D-XX decision matrix'
    );
  });
  ...
  test('coverage audit includes all four source types: GOAL, REQ, RESEARCH, CONTEXT', () => {
    ...
    const refPath = path.join(ROOT, 'sdd', 'references', 'planner-source-audit.md');
...
describe('sdd-plan-checker.md — scope reduction detection includes time/complexity (#2092)', () => {
  let checkerContent;

  test('plan-checker exists and is readable', () => {
    const checkerPath = path.join(AGENTS_DIR, 'sdd-plan-checker.md');
    assert.ok(fs.existsSync(checkerPath), 'agents/sdd-plan-checker.md must exist');
