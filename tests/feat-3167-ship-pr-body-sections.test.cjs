/**
 * Regression tests for issue #3167: configurable /sdd-ship PR body sections.
 */
...
function makeProject() {
  const tmpDir = createTempProject('sdd-3167-');
  tmpDirs.push(tmpDir);
  return tmpDir;
}
...
describe('ship.pr_body_sections config (#3167)', () => {
  test('CLI config-set accepts additional PR body section arrays', () => {
    ...
    const result = runSddTools(['config-set', 'ship.pr_body_sections', value, '--raw'], cwd, { HOME: cwd });
  ...
  test('CLI config-set rejects malformed PR body section values before writing config', () => {
    ...
    const missingHeading = runSddTools(
    ...
    const invalidEnabled = runSddTools(
  ...
  test('CLI config-new-project validates onboarded PR body sections before writing config', () => {
    ...
    const result = runSddTools(['config-new-project', choices], cwd, { HOME: cwd });
  ...
  test('ship workflow composes configured sections as append-only extensions', () => {
    const workflow = readRepoFile('sdd/workflows/ship.md');
  ...
  test('default config and documentation describe ship.pr_body_sections', () => {
    const template = JSON.parse(readRepoFile('sdd/templates/config.json'));
    ...
    const planningConfig = readRepoFile('sdd/references/planning-config.md');
  ...
  test('new-project onboarding can seed enabled or disabled PR body sections', () => {
    const workflow = readRepoFile('sdd/workflows/new-project.md');
