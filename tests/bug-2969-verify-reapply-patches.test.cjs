...
/**
 * Bug #2969: /sdd-reapply-patches Step 5 hunk verification gate reports
...
// Script lives at sdd/bin/ so the installer ships it under
// `${SDD_HOME}/sdd/bin/` (issue #2994). The top-level scripts/
// directory is not copied to user installs.
const SCRIPT = path.join(ROOT, 'sdd', 'bin', 'verify-reapply-patches.cjs');
...
before(() => {
  tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2969-'));
