/**
 * SDD Secure-Phase Tests
 *
 * Validates the security-first enforcement layer:
 * - sdd-security-auditor agent frontmatter and structure
...
// ─── 1. Agent frontmatter — sdd-security-auditor.md ─────────────────────────

describe('SECURE: sdd-security-auditor agent', () => {
  const agentPath = path.join(AGENTS_DIR, 'sdd-security-auditor.md');

  test('agent file exists', () => {
    assert.ok(
      fs.existsSync(agentPath),
      'sdd-security-auditor.md must exist in agents/'
    );
  });
  ...
  test('name is sdd-security-auditor', () => {
    const content = fs.readFileSync(agentPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('name: sdd-security-auditor'),
      'name must be sdd-security-auditor'
    );
  });
...
describe('SECURE: secure-phase command file', () => {
  ...
  test('command file exists', () => {
    assert.ok(
      ...
      'secure-phase.md must exist in commands/sdd/'
    );
  });

  test('has valid frontmatter with name sdd:secure-phase', () => {
    const content = fs.readFileSync(cmdPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('name: sdd:secure-phase'),
      'name must be sdd:secure-phase'
    );
  });
...
describe('SECURE: secure-phase workflow file', () => {
  ...
  test('workflow file exists', () => {
    assert.ok(
      ...
      'secure-phase.md must exist in sdd/workflows/'
    );
  });

  test('contains sdd-security-auditor reference', () => {
    const content = fs.readFileSync(wfPath, 'utf-8');
    assert.ok(
      content.includes('sdd-security-auditor'),
      'must reference sdd-security-auditor agent'
    );
  });
...
describe('SECURE: SECURITY.md template', () => {
  ...
  test('template exists', () => {
    assert.ok(
      ...
      'SECURITY.md must exist in sdd/templates/'
    );
  });
...
describe('SECURE: config.json security defaults', () => {
  ...
  test('config template exists', () => {
    assert.ok(
      ...
      'config.json must exist in sdd/templates/'
    );
  });
...
describe('SECURE: VALIDATION.md security columns', () => {
  ...
  test('VALIDATION.md template exists', () => {
    assert.ok(
      ...
      'VALIDATION.md must exist in sdd/templates/'
    );
  });
