...
//
// ai-integration-phase.md listed Steps 7+8 (sdd-ai-researcher +
// sdd-domain-researcher) without an explicit sequential ordering constraint.
// An orchestrator optimizing for speed could reasonably parallelize them
// since the sections appeared disjoint. When parallelized, sdd-domain-researcher's
...
const src = fs.readFileSync(
  path.join(ROOT, 'sdd', 'workflows', 'ai-integration-phase.md'),
  'utf8',
);
...
describe('bug #3096: ai-integration-phase sequential ordering and Edit-only discipline', () => {
  ...
  test('Step 7 sdd-ai-researcher prompt includes Edit-only tool discipline', () => {
    // The discipline block must appear before </objective> for sdd-ai-researcher
    const step7Idx = src.indexOf('## 7. Spawn sdd-ai-researcher');
    const step8Idx = src.indexOf('## 8. Spawn sdd-domain-researcher');
  ...
  test('Step 8 sdd-domain-researcher prompt includes Edit-only tool discipline', () => {
    const step8Idx = src.indexOf('## 8. Spawn sdd-domain-researcher');
    const step9Idx = src.indexOf('## 9. Spawn sdd-eval-planner');
    assert.ok(step8Idx !== -1, 'Step 8 not found');
