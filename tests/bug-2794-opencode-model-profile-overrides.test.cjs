 ...
 * The agent install path called `readSddEffectiveModelOverrides` (explicit
 * per-agent overrides) but never called `readSddRuntimeProfileResolver`
 * (tier-based profile overrides). When a user configured:
 *
 *   { runtime: "opencode", model_profile_overrides: { opencode: { sonnet: "..." } } }
 *
 * generated `.opencode/agents/sdd-*.md` files contained no `model:` frontmatter.
 *
 ...
 *
 * This test exercises:
 * 1. `readSddRuntimeProfileResolver` correctly resolves OpenCode tier overrides.
...
function makeTmp(prefix) {
  return fs.mkdtempSync(path.join(os.tmpdir(), `sdd-2794-${prefix}-`));
}
...
describe('bug-2794: readSddRuntimeProfileResolver resolves opencode tier overrides', () => {
  let projectDir;
  let homeDir;
  let origHome;
  ...
  test('resolves opencode sonnet tier to user-supplied model ID', () => {
    ...
    const resolver = readSddRuntimeProfileResolver(projectDir);
    assert.ok(resolver !== null, 'expected a resolver for opencode runtime');

    // sdd-roadmapper balanced tier = sonnet — should resolve to override
    const entry = resolver.resolve('sdd-roadmapper');
    assert.ok(entry !== null, 'expected entry for sdd-roadmapper');
  ...
  test('resolver returns null for agent not in MODEL_PROFILES', () => {
    ...
    const entry = resolver.resolve('sdd-nonexistent-agent');
...
describe('bug-2794: OpenCode agent install embeds model_profile_overrides model', () => {
  ...
  test('generated OpenCode agent frontmatter includes model from model_profile_overrides', () => {
    ...
    // sdd-roadmapper is balanced -> sonnet tier
    const roadmapperPath = path.join(agentsDir, 'sdd-roadmapper.md');
    assert.ok(fs.existsSync(roadmapperPath), 'sdd-roadmapper.md should exist');
    const roadmapperContent = fs.readFileSync(roadmapperPath, 'utf-8');
    assert.match(
      roadmapperContent,
      /^model: anthropic\/claude-sonnet-4-7$/m,
      'sdd-roadmapper should have sonnet model from model_profile_overrides'
    );

    // sdd-planner is balanced -> opus tier
    const plannerPath = path.join(agentsDir, 'sdd-planner.md');
    assert.ok(fs.existsSync(plannerPath), 'sdd-planner.md should exist');
    const plannerContent = fs.readFileSync(plannerPath, 'utf-8');
    assert.match(
      plannerContent,
      /^model: anthropic\/claude-opus-4-7$/m,
      'sdd-planner should have opus model from model_profile_overrides'
    );
  });
  ...
  test('explicit model_overrides[agent] wins over model_profile_overrides tier', () => {
    writeJson(path.join(projectDir, '.planning', 'config.json'), {
      ...
      model_profile: 'balanced',
      model_overrides: {
        'sdd-roadmapper': 'explicit-winner-model',
      },
      model_profile_overrides: {
        opencode: {
    ...
    const roadmapperPath = path.join(projectDir, '.opencode', 'agents', 'sdd-roadmapper.md');
  ...
  test('no model field when neither model_overrides nor model_profile_overrides is set', () => {
    ...
    if (fs.existsSync(roadmapperPath)) {
      ...
      // (sdd-roadmapper balanced = sonnet; opencode has built-in sonnet defaults)
