...
const { writeSurface, applySurface } = require('../sdd/bin/lib/surface.cjs');
const { loadSkillsManifest, writeActiveProfile } = require('../sdd/bin/lib/install-profiles.cjs');
const { CLUSTERS } = require('../sdd/bin/lib/clusters.cjs');

const REAL_COMMANDS_DIR = path.join(__dirname, '..', 'commands', 'sdd');
const REAL_AGENTS_DIR = path.join(__dirname, '..', 'agents');

function tmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-surface-apply-'));
}

/**
 * Create a minimal fixture install dir structure.
 * Returns { runtimeConfigDir, commandsDir, agentsDir }.
 * runtimeConfigDir has a .sdd-source marker pointing to REAL_COMMANDS_DIR.
 */
function createFixtureRuntime() {
  const base = tmpDir();
  const runtimeConfigDir = path.join(base, 'config');
  const commandsDir = path.join(base, 'commands', 'sdd');
  ...
  fs.writeFileSync(path.join(runtimeConfigDir, '.sdd-source'), REAL_COMMANDS_DIR, 'utf8');
...
describe('applySurface', () => {
  ...
  test('leaves non-sdd .md files alone in agentsDir', () => {
    const { base, runtimeConfigDir, commandsDir, agentsDir } = createFixtureRuntime();
    try {
      // Place a non-sdd agent file in agentsDir
      ...
      // Non-sdd file should still be there
      assert.ok(fs.existsSync(foreignAgent), 'non-sdd agent file should not be touched');
    } finally {
      fs.rmSync(base, { recursive: true, force: true });
    }
  });
