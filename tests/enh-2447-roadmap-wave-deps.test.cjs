...
`,
    ...
    const result = runSddTools('roadmap annotate-dependencies 1', tmpDir);
...
`,
    ...
    runSddTools('roadmap annotate-dependencies 1', tmpDir);
    const secondResult = runSddTools('roadmap annotate-dependencies 1', tmpDir);
  ...
  test('plan-phase.md documents annotate-dependencies step', () => {
    const planPhase = fs.readFileSync(
      path.join(__dirname, '../sdd/workflows/plan-phase.md'), 'utf-8'
    );
