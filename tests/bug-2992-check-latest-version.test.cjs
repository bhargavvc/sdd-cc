...
const { checkLatestVersion, CHECK_REASON, PACKAGE_NAME } = require(
  path.join(ROOT, 'sdd', 'bin', 'check-latest-version.cjs'),
);
...
describe('Bug #2992: deterministic latest-version check', () => {
  test('PACKAGE_NAME is the constant @bhargavvc/sdd-cc (no callers can override)', () => {
    assert.equal(PACKAGE_NAME, '@bhargavvc/sdd-cc');
  });
...
describe('Bug #2992: error paths', () => {
  const { checkLatestVersion, CHECK_REASON } = require(require('node:path').join(__dirname, '..', 'sdd', 'bin', 'check-latest-version.cjs'));
