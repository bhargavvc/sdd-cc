 ...
 * the actual number of .md files in commands/sdd/.
 *
...
/**
 * Count .md files that actually live in commands/sdd/.
 ...
 *   ├── commands/sdd/*.md               # N slash commands
 * Returns null if the pattern is not found.
 */
function docTreeCount(content) {
  const m = content.match(/commands\/sdd\/\*\.md[^\n]*#\s*(\d+)\s+slash commands/);
  return m ? parseInt(m[1], 10) : null;
}
...
describe('ARCHITECTURE.md command count sync', () => {
  ...
  test('"Total commands:" prose count matches actual commands/sdd/ file count', () => {
    const prose = docProseCount(archContent);
    assert.equal(
      prose,
      actual,
      `ARCHITECTURE.md "Total commands:" says ${prose} but commands/sdd/ has ${actual} .md files — update the doc`,
    );
  });

  test('directory-tree slash-command count matches actual commands/sdd/ file count', () => {
    const tree = docTreeCount(archContent);
    assert.equal(
      tree,
      actual,
      `ARCHITECTURE.md directory tree says ${tree} slash commands but commands/sdd/ has ${actual} .md files — update the doc`,
    );
  });
