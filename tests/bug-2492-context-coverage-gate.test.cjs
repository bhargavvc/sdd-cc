...
const PLAN_PHASE = path.join(__dirname, '..', 'sdd', 'workflows', 'plan-phase.md');
const VERIFY_PHASE = path.join(__dirname, '..', 'sdd', 'workflows', 'verify-phase.md');
...
describe('plan-phase decision-coverage gate (#2492)', () => {
  ...
  test('invokes the check.decision-coverage-plan handler', () => {
    assert.ok(
      ...
      'plan-phase.md must call sdd-sdk query check.decision-coverage-plan',
    );
  });
  ...
  test('plan-phase Decision Coverage Gate uses CONTEXT_PATH variable defined in INIT extraction (review F1)', () => {
    // The CONTEXT_PATH bash variable is defined at Step 4 (`CONTEXT_PATH=$(_sdd_field "$INIT" context_path)`).
    ...
    const defIdx = md.indexOf('CONTEXT_PATH=$(_sdd_field "$INIT" context_path)');
...
describe('verify-phase decision-coverage gate (#2492)', () => {
  ...
  test('invokes the check.decision-coverage-verify handler', () => {
    assert.ok(
      ...
      'verify-phase.md must call sdd-sdk query check.decision-coverage-verify',
    );
  });
