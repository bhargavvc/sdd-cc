...
const IOS_SCAFFOLD_REF = path.join(
  __dirname, '..', 'sdd', 'references', 'ios-scaffold.md'
);
const EXECUTOR_AGENT = path.join(
  __dirname, '..', 'agents', 'sdd-executor.md'
);
const UNIVERSAL_ANTI_PATTERNS = path.join(
  __dirname, '..', 'sdd', 'references', 'universal-anti-patterns.md'
);

describe('ios-scaffold.md reference exists and contains XcodeGen guidance', () => {
  test('reference file exists at sdd/references/ios-scaffold.md', () => {
    assert.ok(
...
describe('sdd-executor.md references ios-scaffold guidance', () => {
  test('executor agent references ios-scaffold.md', () => {
    ...
    assert.ok(
      ...
      'sdd-executor.md must reference ios-scaffold.md for iOS app scaffold guidance'
    );
  });
});
