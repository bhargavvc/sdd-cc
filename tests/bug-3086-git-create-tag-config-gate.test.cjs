...
describe('#3086: git.create_tag config key', () => {
  test('A. fresh project: config-get git.create_tag returns true (default)', (t) => {
    const tmpDir = createTempProject('sdd-3086-default-');
    t.after(() => cleanup(tmpDir));

    const result = runSddTools(['config-get', 'git.create_tag'], tmpDir, { HOME: tmpDir });
  ...
  test('B. config-set git.create_tag false → config-get returns false', (t) => {
    const tmpDir = createTempProject('sdd-3086-set-false-');
    t.after(() => cleanup(tmpDir));

    const setResult = runSddTools(['config-set', 'git.create_tag', 'false'], tmpDir, {
      HOME: tmpDir,
    });
    assert.ok(setResult.success, `config-set git.create_tag false failed:\n${setResult.error}`);

    const getResult = runSddTools(['config-get', 'git.create_tag'], tmpDir, { HOME: tmpDir });
  ...
  test('C. config-set git.create_tag with invalid value "maybe" is rejected', (t) => {
    const tmpDir = createTempProject('sdd-3086-invalid-');
    t.after(() => cleanup(tmpDir));

    const result = runSddTools(['config-set', 'git.create_tag', 'maybe'], tmpDir, {
      HOME: tmpDir,
    });
    assert.ok(
      !result.success,
