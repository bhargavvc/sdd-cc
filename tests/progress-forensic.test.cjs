/**
 * Tests for --forensic flag on /sdd-progress (#2189)
 *
...
describe('#2189: progress --forensic flag', () => {
  test('progress command argument-hint includes --forensic', () => {
    const command = fs.readFileSync(
      path.join(__dirname, '..', 'commands', 'sdd', 'progress.md'), 'utf8'
    );
  ...
  test('COMMANDS.md documents --forensic flag for sdd-progress', () => {
    const commands = fs.readFileSync(
      path.join(__dirname, '..', 'docs', 'COMMANDS.md'), 'utf8'
    );
    assert.ok(
      commands.includes('--forensic'),
      'COMMANDS.md should document --forensic flag for sdd-progress'
    );
  });
});
