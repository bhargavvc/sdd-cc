...
// Sample Claude agent frontmatter (matches actual SDD agent format)
const SAMPLE_AGENT = `---
name: sdd-executor
...
Use \${PHASE} in shell examples.
    ...
    assert.ok(frontmatter.includes('name: sdd-codebase-mapper'), 'keeps name');
    ...
    assert.ok(!frontmatter.includes('sdd-mapper-workflow'), 'drops skills list items');
