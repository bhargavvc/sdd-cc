 ...
 *
 * sdd-check-update.js contains a MANAGED_HOOKS array used to detect stale
 * hooks after a SDD update. It must list every hook file that SDD ships so
 * that all deployed hooks are checked for staleness — not just the .js ones.
 *
 * The original bug: the 3 bash hooks (sdd-phase-boundary.sh,
 * sdd-session-state.sh, sdd-validate-commit.sh) were missing from
...
const MANAGED_HOOKS_FILE = path.join(HOOKS_DIR, 'sdd-check-update-worker.js');
...
describe('bug #2136: MANAGED_HOOKS must include all shipped hook files', () => {
  ...
  assert.ok(match, 'MANAGED_HOOKS array not found in sdd-check-update-worker.js');
  ...
  // List all SDD-managed hook files in hooks/ (names starting with "sdd-")
  shippedHooks = fs.readdirSync(HOOKS_DIR)
    .filter(f => f.startsWith('sdd-') && (f.endsWith('.js') || f.endsWith('.sh')));

  test('every shipped sdd-*.js hook is in MANAGED_HOOKS', () => {
    const jsHooks = shippedHooks.filter(f => f.endsWith('.js'));
    for (const hookFile of jsHooks) {
      assert.ok(
        managedHooks.includes(hookFile),
        `${hookFile} is shipped in hooks/ but missing from MANAGED_HOOKS in sdd-check-update-worker.js`
      );
    }
  });

  test('every shipped sdd-*.sh hook is in MANAGED_HOOKS', () => {
