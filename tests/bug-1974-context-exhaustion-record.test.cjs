/**
 * Integration tests for sdd-context-monitor.js auto-record on CRITICAL (#1974).
 *
 * Verifies:
 * 1. On CRITICAL + active SDD project, subprocess is spawned and STATE.md
...
const HOOK_PATH = path.resolve(__dirname, '..', 'hooks', 'sdd-context-monitor.js');
...
describe('#1974 context exhaustion auto-record', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-1974-'));
    ...
    // Minimal config.json required by sdd-tools
  ...
  test('spawns subprocess and writes Stopped At field on CRITICAL with active SDD', () => {
  ...
  test('does NOT spawn subprocess when .planning/STATE.md is absent', () => {
    // Delete STATE.md to simulate non-SDD project
  ...
  test('hook uses __dirname-based path (runtime-agnostic)', () => {
    ...
    assert.match(
      hookSource,
      /path\.join\(__dirname,\s*'\.\.',\s*'sdd'/,
      'hook must use __dirname-based path resolution for sdd-tools.cjs'
    );
    assert.doesNotMatch(
      hookSource,
      /process\.env\.HOME.*\.claude.*sdd.*sdd-tools\.cjs/,
