...
// commands/sdd/*.md files ARE what the runtime loads — testing their
...
// skills, e.g. `sdd-map-codebase --fast` for the former `sdd-scan`).
const FLAG_FORM_PARENTS = new Set([
  'sdd-code-review',     // --fix absorbs former sdd-code-review-fix
  'sdd-map-codebase',    // --fast absorbs scan, --query absorbs intel
]);
...
describe('Namespace skill bodies carry a routing table', () => {
  for (const { file } of NAMESPACE_SKILLS) {
    ...
    test(`${file} — body has at least one Invoke target`, () => {
      const fm = readNamespaceFile(file);
      const hasInvoke = /\bsdd-[a-z-]+/i.test(fm._body);
      assert.ok(hasInvoke, `${file} body must reference at least one sdd-* sub-skill`);
    });
  }
});

// ── Context guard contract on sdd-health ──────────────────────────────
...
describe('sdd-health --context flag is wired into command + workflow', () => {
  const HEALTH_CMD = path.join(COMMANDS_DIR, 'health.md');
  const HEALTH_WORKFLOW = path.join(__dirname, '..', 'sdd', 'workflows', 'health.md');

  test('commands/sdd/health.md argument-hint advertises --context', () => {
  ...
  test('commands/sdd/health.md body documents the three-state utilization table', () => {
    const raw = fs.readFileSync(HEALTH_CMD, 'utf-8');
  ...
  test('sdd/workflows/health.md has a context_check step', () => {
  ...
  test('workflow context_check invokes sdd-sdk query validate.context', () => {
    const raw = fs.readFileSync(HEALTH_WORKFLOW, 'utf-8');
    ...
    assert.match(
      stepBody,
      /sdd-sdk\s+query\s+validate\.context/,
      'context_check must call `sdd-sdk query validate.context`',
    );
...
describe('Namespace router targets resolve to surviving skills', () => {
  ...
  for (const f of fs.readdirSync(COMMANDS_DIR)) {
    ...
    if (base === 'extract_learnings') surviving.add('sdd-extract-learnings');
  }

  for (const { file } of NAMESPACE_SKILLS) {
    test(`${file} — every routing target resolves`, () => {
      const fm = readNamespaceFile(file);
      // Extract every sdd-<name> token that appears in a table-row right column.
      // Strip flag suffixes (`sdd-foo --bar` → `sdd-foo`) before resolving.
      ...
      for (const line of fm._body.split('\n')) {
        ...
        for (const m of cells[cells.length - 1].matchAll(/\bsdd-[a-z][a-z0-9-]*/g)) {
          targets.add(m[0]);
        }
      }
      assert.ok(targets.size > 0, `${file} routing table must reference at least one sdd-* target`);
      ...
      assert.deepStrictEqual(
        unresolved,
        [],
        `${file} routes to skills that don't exist in commands/sdd/: ${unresolved.join(', ')}`,
      );
    });
  }
});
