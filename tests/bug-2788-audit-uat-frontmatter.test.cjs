 ...
 *
 * `sdd-sdk query audit-uat` returned total_items: 0 for VERIFICATION.md
 * files where human-needed items were encoded in the frontmatter
 * `human_verification:` YAML array (the format written by sdd-verifier),
...
describe('bug-2788: audit-uat reads frontmatter human_verification array', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('sdd-test-2788-');
  });
  ...
  test('frontmatter human_verification: array items are reported', () => {
    // This is the format sdd-verifier writes; before fix total_items was 0
    const content = [
      '---',
      'phase: 03-invoicing',
      'status: human_needed',
