 ...
 *
 * USER-PROFILE.md is a user-owned artifact created/refreshed by /sdd-profile-user.
 * preserveUserArtifacts() correctly preserves it across reinstalls. But writeManifest()
 * also records it under "sdd/USER-PROFILE.md" with a SHA-256 of whatever was
 ...
 * "Found N locally modified SDD file(s) — backed up to sdd-local-patches/" warning.
 *
...
describe('#2771: USER-PROFILE.md is excluded from sdd-file-manifest.json', () => {
  let tmpDir;

  beforeEach(() => { tmpDir = createTempDir('sdd-2771-manifest-'); });
  afterEach(() => { cleanup(tmpDir); });

  test('writeManifest excludes sdd/USER-PROFILE.md even when present on disk', () => {
    runInstaller(tmpDir);

    // Simulate /sdd-profile-user creating USER-PROFILE.md
    ...
    assert.ok(
      !Object.prototype.hasOwnProperty.call(manifest.files, 'sdd/USER-PROFILE.md'),
      'manifest.files must NOT contain sdd/USER-PROFILE.md — it is a user artifact, not distribution'
    );
  });
});
...
describe('#2771: USER-PROFILE.md is still preserved across reinstall', () => {
  ...
  beforeEach(() => { tmpDir = createTempDir('sdd-2771-preserve-'); });
  afterEach(() => { cleanup(tmpDir); });
  ...
  test('USER-PROFILE.md content survives reinstall (preservation regression guard)', () => {
    ...
    const profilePath = path.join(tmpDir, 'sdd', 'USER-PROFILE.md');
    const content = '# Profile\n\nUser content from /sdd-profile-user.\n';
...
describe('#2771: refreshed USER-PROFILE.md does not trigger local-patches warning', () => {
  ...
  beforeEach(() => { tmpDir = createTempDir('sdd-2771-patches-'); });
  afterEach(() => { cleanup(tmpDir); });
  ...
  test('saveLocalPatches does not classify a refreshed USER-PROFILE.md as a local patch', () => {
    ...
    // /sdd-profile-user creates USER-PROFILE.md (v1)
    const profilePath = path.join(tmpDir, 'sdd', 'USER-PROFILE.md');
    ...
    // /sdd-profile-user --refresh rewrites USER-PROFILE.md (v2 != v1)
    ...
    const patchFile = path.join(patchesDir, 'sdd', 'USER-PROFILE.md');
    assert.ok(
      !fs.existsSync(patchFile),
      'USER-PROFILE.md must NOT appear in sdd-local-patches/ — it is a user artifact, not a modified distribution file'
    );

    const offendingLine = output
      .split('\n')
      .find((line) => /locally modified SDD file/.test(line) && /USER-PROFILE/.test(line));
    assert.strictEqual(
      offendingLine,
      undefined,
      'installer output must not report USER-PROFILE.md as a locally modified SDD file on any single line. Output was:\n' + output
    );
  });
});
...
describe('#2771: legacy manifest entries for USER_OWNED_ARTIFACTS are normalized', () => {
  ...
  beforeEach(() => { tmpDir = createTempDir('sdd-2771-legacy-'); });
  afterEach(() => { cleanup(tmpDir); });
  ...
  test('pre-existing manifest entry for USER-PROFILE.md does not trigger patches warning', () => {
    ...
    manifest.files['sdd/USER-PROFILE.md'] = 'deadbeef'.repeat(8); // stale hash
    fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2));

    // /sdd-profile-user --refresh rewrites USER-PROFILE.md
    fs.writeFileSync(profilePath, '# Profile v2 — refreshed\n');
...
describe('#2771: USER_OWNED_ARTIFACTS is a single source of truth', () => {
  test('install.js exports USER_OWNED_ARTIFACTS containing USER-PROFILE.md', () => {
    ...
    try {
      ...
      if (origMode === undefined) delete process.env.SDD_TEST_MODE;
      else process.env.SDD_TEST_MODE = origMode;
    }
...
describe('manifest path safety', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('sdd-manifest-path-safety-');
