 ...
 * 2. sdd-executor.md task_commit_protocol includes post-commit deletion verification
...
describe('worktree commit safety hardening (#1977)', () => {
  ...
  test('sdd-executor.md task_commit_protocol includes post-commit deletion verification', () => {
    ...
    assert.ok(
      ...
      'sdd-executor.md must include --diff-filter=D deletion verification after each task commit'
    );

    // Must include a WARNING or notice about deletions
    assert.ok(
      content.includes('WARNING') || content.includes('DELETIONS'),
      'sdd-executor.md must warn when a commit includes file deletions'
    );
  });
