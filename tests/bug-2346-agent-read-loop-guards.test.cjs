 ...
 *
 * Multiple SDD agents (sdd-ui-checker, sdd-planner) entered unbounded Read
 ...
 * sdd-pattern-mapper was fixed in #2312; this covers the remaining agents.
 *
...
describe('bug #2346: agent read loop guards', () => {
  ...
  describe('sdd-ui-checker', () => {
    const agentPath = path.join(AGENTS_DIR, 'sdd-ui-checker.md');
    let content;

    test('agent file exists', () => {
      assert.ok(fs.existsSync(agentPath), 'agents/sdd-ui-checker.md must exist');
    ...
    test('has <critical_rules> block', () => {
      ...
      assert.ok(
        ...
        'sdd-ui-checker.md must have a <critical_rules> block to prevent unbounded read loops (#2346)'
      );
    });
  ...
  describe('sdd-planner', () => {
    const agentPath = path.join(AGENTS_DIR, 'sdd-planner.md');
    let content;

    test('agent file exists', () => {
      assert.ok(fs.existsSync(agentPath), 'agents/sdd-planner.md must exist');
      content = fs.readFileSync(agentPath, 'utf-8');
    });
    ...
    test('has <critical_rules> block', () => {
      ...
      assert.ok(
        content.includes('<critical_rules>'),
        'sdd-planner.md must have a <critical_rules> block to prevent unbounded read loops (#2346)'
      );
    });
