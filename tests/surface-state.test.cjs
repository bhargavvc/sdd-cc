...
const { readSurface, writeSurface } = require('../sdd/bin/lib/surface.cjs');

function tmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-surface-state-'));
}
...
describe('readSurface / writeSurface', () => {
  ...
  test('non-existent directory returns null', () => {
    const ghost = path.join(os.tmpdir(), 'sdd-surface-no-exist-' + Date.now());
  ...
  test('corrupt JSON returns null', () => {
    const dir = tmpDir();
    try {
      fs.writeFileSync(path.join(dir, '.sdd-surface.json'), '{not valid json', 'utf8');
      const result = readSurface(dir);
      assert.strictEqual(result, null);
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
  ...
  test('JSON missing baseProfile field returns null', () => {
    ...
    try {
      fs.writeFileSync(
        path.join(dir, '.sdd-surface.json'),
  ...
  test('atomic write: result file is never a partial tmp file', () => {
    ...
    try {
      ...
      assert.ok(files.includes('.sdd-surface.json'));
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
