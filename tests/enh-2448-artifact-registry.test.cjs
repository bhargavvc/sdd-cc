...
/**
 * Tests for canonical artifact registry and sdd-health W019 lint (#2448).
 */
...
const { isCanonicalPlanningFile, CANONICAL_EXACT } = require('../sdd/bin/lib/artifacts.cjs');
const { cmdValidateHealth } = require('../sdd/bin/lib/verify.cjs');

function makeTempProject(files = {}) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2448-'));
...
describe('artifacts.cjs — isCanonicalPlanningFile', () => {
  ...
  test('returns true for RETROSPECTIVE.md (produced by /sdd-complete-milestone)', () => {
...
describe('sdd-health W019 — unrecognized .planning/ root files', () => {
  ...
  test('templates/README.md exists and documents W019', () => {
    const readme = fs.readFileSync(
      path.join(__dirname, '../sdd/templates/README.md'), 'utf-8'
    );
