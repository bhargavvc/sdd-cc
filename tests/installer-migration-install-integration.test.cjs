...
const RUNTIME_INSTALL_CONTRACTS = {
  ...
  gemini: { surface: 'commands-sdd', settings: true, packageJson: true },
...
function assertHasSddDirectory(root, relPath) {
  assert.ok(
    listDirNames(root, relPath).some((name) => name.startsWith('sdd-')),
    `${relPath} should contain generated SDD entries`
  );
}

function assertNoSddDirectoryEntries(root, relPath) {
  assert.equal(
    listDirNames(root, relPath).some((name) => name.startsWith('sdd-')),
    false,
    `${relPath} should not contain generated SDD entries`
  );
}
...
function assertFreshInstallContract(runtime, targetDir) {
  ...
  assert.equal(
    fs.readFileSync(path.join(targetDir, 'sdd', 'VERSION'), 'utf8'),
    pkg.version,
    `${runtime} should install the package VERSION`
  );
  assert.ok(
    fs.existsSync(path.join(targetDir, 'sdd', 'bin', 'sdd-tools.cjs')),
    `${runtime} should install the SDD tool payload`
  );
  assert.ok(
    fs.existsSync(path.join(targetDir, 'sdd-file-manifest.json')),
    `${runtime} should write the install manifest`
  );

  const manifest = JSON.parse(fs.readFileSync(path.join(targetDir, 'sdd-file-manifest.json'), 'utf8'));
  ...
  assert.ok(
    manifest.files['sdd/VERSION'],
    `${runtime} manifest should track the installed VERSION file`
  );

  if (contract.surface === 'flat-skills') {
    // Pre-#3562: codex was special-cased to expect zero sdd-* skill dirs
    ...
    assertHasSddDirectory(targetDir, 'skills');
  } else if (contract.surface === 'hermes-skills') {
    assertHasSddDirectory(targetDir, path.join('skills', 'sdd'));
    assert.ok(
      fs.existsSync(path.join(targetDir, 'skills', 'sdd', 'DESCRIPTION.md')),
      'Hermes should install the nested SDD category description'
    );
  } else if (contract.surface === 'flat-command') {
    assert.ok(
      listDirNames(targetDir, 'command').some((name) => name.startsWith('sdd-') && name.endsWith('.md')),
      `${runtime} should install flattened command markdown files`
    );
  } else if (contract.surface === 'commands-sdd') {
    assert.ok(
      listDirNames(targetDir, path.join('commands', 'sdd')).length > 0,
      `${runtime} should install commands/sdd entries`
    );
  } else if (contract.surface === 'clinerules') {
    assert.match(
      fs.readFileSync(path.join(targetDir, '.clinerules'), 'utf8'),
      /SDD workflows live in `sdd\/workflows\/`/,
      'Cline should install root .clinerules guidance'
    );
  }

  assert.ok(
    listDirNames(targetDir, 'agents').some((name) => name.startsWith('sdd-')),
  ...
  if (contract.codexConfig) {
    assert.match(
      ...
      'Codex should install config.toml with the SDD marker'
    );
  }
...
describe('installer migration install integration', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    tmpRoot = createTempDir('sdd-install-migrations-');
  ...
  test('reports applied migration actions before package materialization', () => {
    ...
    assert.ok(
      plainOutput.indexOf('Installer migrations') < plainOutput.indexOf('Installed sdd'),
  ...
  test('blocks install before materialization when baseline needs explicit user choice', () => {
    writeFile(codexHome, 'hooks/sdd-retired-hook.txt', 'old sdd hook\n');
    ...
    assert.equal(fs.readFileSync(path.join(codexHome, 'hooks/sdd-retired-hook.txt'), 'utf8'), 'old sdd hook\n');
    assert.equal(fs.existsSync(path.join(codexHome, 'skills')), false);
    assert.equal(fs.existsSync(path.join(codexHome, 'sdd', 'VERSION')), false);
  });
  ...
  test('rolls back applied migrations when package materialization fails for non-Codex installs', () => {
    ...
    assert.throws(
      () => captureConsole(() =>
        withEnv('CLAUDE_CONFIG_DIR', claudeHome, () =>
          withWriteFailure(path.join(claudeHome, 'sdd', 'VERSION'), () => install(true, 'claude'))
        )
      ),
    ...
    assert.equal(fs.existsSync(path.join(claudeHome, 'sdd-install-state.json')), false);
  });
  ...
  test('rolls back completed runtime migrations when a later runtime install fails', () => {
      ...
      () => captureConsole(() =>
        withEnv('CLAUDE_CONFIG_DIR', claudeHome, () =>
          withEnv('CODEX_HOME', codexHome, () =>
            withWriteFailure(path.join(codexHome, 'sdd', 'VERSION'), () =>
    ...
    assert.equal(fs.existsSync(path.join(codexHome, 'sdd-install-state.json')), false);
  });
  ...
  for (const runtime of SUPPORTED_RUNTIMES) {
    test(`runs a full end-to-end install for ${runtime}`, () => {
      ...
      assert.match(output, /Installed sdd/);
      assert.match(output, /Done!/);
      assert.equal(fs.existsSync(path.join(targetDir, 'hooks/statusline.js')), false);

      const installState = JSON.parse(fs.readFileSync(path.join(targetDir, 'sdd-install-state.json'), 'utf8'));
      assert.ok(
    ...
    test(`blocks ambiguous SDD-looking user-choice artifacts for ${runtime}`, () => {
      const targetDir = path.join(tmpRoot, `.${runtime}-blocked`);
      fs.mkdirSync(targetDir, { recursive: true });
      writeFile(targetDir, 'sdd/sdd-retired-tool.cjs', 'old ambiguous artifact\n');
      ...
      const output = stripAnsi(`${result.stdout}\n${result.stderr}`);
      assert.match(output, /Installer migrations/);
      assert.match(output, /blocked\s+sdd\/sdd-retired-tool\.cjs/);
      assert.match(output, /installer migration blocked/);
      assert.equal(
        fs.readFileSync(path.join(targetDir, 'sdd/sdd-retired-tool.cjs'), 'utf8'),
        'old ambiguous artifact\n'
      );
      assert.equal(fs.existsSync(path.join(targetDir, 'sdd', 'VERSION')), false);
    });
  }
});
