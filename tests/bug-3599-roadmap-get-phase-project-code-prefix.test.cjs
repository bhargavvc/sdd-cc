 ...
 *
 * `phaseMarkdownRegexSource(phaseNum)` in sdd/bin/lib/core.cjs
...
describe('bug #3599: roadmap get-phase preserves project-code prefix in lookup', () => {
  ...
  test('finds ### Phase PROJ-42: when queried as PROJ-42', () => {
    ...
    const result = runSddTools('roadmap get-phase PROJ-42 --json', tmpDir);
  ...
  test('does NOT cross-match: querying 42 must not match ### Phase PROJ-42:', () => {
    ...
    const result = runSddTools('roadmap get-phase 42 --json', tmpDir);
  ...
  test('preserves #3537 contract: CK-01 directory form resolves to Phase 1 prose', () => {
    ...
    const result = runSddTools('roadmap get-phase CK-01 --json', tmpDir);
    assert.ok(result.success);
    const payload = JSON.parse(result.output);
    assert.strictEqual(
      payload.found,
  ...
  test('finds the right phase when both prefixed and bare forms coexist', () => {
    ...
    const r42 = runSddTools('roadmap get-phase 42 --json', tmpDir);
    const rProj = runSddTools('roadmap get-phase PROJ-42 --json', tmpDir);
