...
`;
    ...
    const result = convertClaudeCommandToWindsurfSkill(input, 'sdd-quick');
...
`;
    ...
    const result = convertClaudeCommandToWindsurfSkill(input, 'sdd-plan-phase');
...
`;
    ...
    const result = convertClaudeCommandToWindsurfSkill(input, 'sdd-test');
...
describe('convertClaudeToWindsurfMarkdown', () => {
  ...
  test('replaces $ARGUMENTS with {{SDD_ARGS}}', () => {
    const input = 'Pass $ARGUMENTS to the command.';
    const result = convertClaudeToWindsurfMarkdown(input);
    assert.ok(result.includes('{{SDD_ARGS}}'), '$ARGUMENTS replaced');
  });
