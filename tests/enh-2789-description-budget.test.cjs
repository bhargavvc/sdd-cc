...
// commands/sdd/*.md text IS what the runtime loads — testing description
 ...
 *
 * Verifies:
 * 1. All skill descriptions in commands/sdd/*.md are <= 100 chars
...
/**
 * Get all .md files in commands/sdd/ with their descriptions.
 */
...
describe('description length budget', () => {
  test('all commands/sdd/*.md descriptions are <= 100 chars', () => {
...
describe('lint-descriptions.cjs', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-lint-desc-test-'));
  });
