/**
 * Tests for sdd:plan-review-convergence command (#2306)
 *
...
const COMMAND_PATH = path.join(__dirname, '..', 'commands', 'sdd', 'plan-review-convergence.md');
const WORKFLOW_PATH = path.join(__dirname, '..', 'sdd', 'workflows', 'plan-review-convergence.md');
const SCHEMA_PATH = path.join(__dirname, '..', 'sdd', 'bin', 'lib', 'config-schema.cjs');
...
describe('plan-review-convergence command source (#2306)', () => {
  ...
  test('command name uses sdd: prefix (installer converts to sdd- on install)', () => {
    assert.ok(
      command.includes('name: sdd:plan-review-convergence'),
      'command name must use sdd: prefix so installer converts it to sdd-plan-review-convergence'
    );
  });
  ...
  test('command references the workflow file via execution_context', () => {
    assert.ok(
      command.includes('@$HOME/.claude/sdd/workflows/plan-review-convergence.md'),
...
describe('plan-review-convergence workflow: initialization (#2306)', () => {
  ...
  test('workflow calls sdd-tools.cjs init plan-phase for initialization', () => {
    assert.ok(
      workflow.includes('sdd-tools.cjs') && workflow.includes('init') && workflow.includes('plan-phase'),
      'workflow must initialize via sdd-tools.cjs init plan-phase'
    );
  });
...
describe('plan-review-convergence workflow: config gate (#2306-v2)', () => {
  ...
  test('workflow exits with enable instructions when config key is false', () => {
    ...
    assert.ok(
      workflow.includes('sdd config-set workflow.plan_review_convergence true') ||
...
describe('plan-review-convergence workflow: initial planning gate (#2306)', () => {
  ...
  test('workflow spawns isolated planning agent when no plans exist', () => {
    assert.ok(
      workflow.includes('sdd-plan-phase'),
      'workflow must spawn Agent → sdd-plan-phase when no plans exist'
    );
  });
...
describe('plan-review-convergence workflow: convergence loop (#2306)', () => {
  ...
  test('workflow spawns isolated review agent each cycle', () => {
    assert.ok(
      workflow.includes('sdd-review'),
      'workflow must spawn Agent → sdd-review each cycle'
    );
  });
  ...
  test('review agent spawn forwards --ws via SDD_WS (symmetric with replan agent)', () => {
    // Critical correctness bug: if SDD_WS is not forwarded to the review agent,
    // the review reads from the wrong workspace while replanning reads from the correct one.
    const reviewAgentBlock = workflow.match(/sdd-review['"`,\s][\s\S]{0,300}?SDD_WS/);
    assert.ok(
      reviewAgentBlock ||
      (workflow.includes("'sdd-review'") && workflow.includes('{SDD_WS}') &&
       workflow.indexOf('{SDD_WS}') < workflow.indexOf("'sdd-plan-phase'")),
      'review agent spawn must forward {SDD_WS} — workspace flag must reach the reviewer (#2306-v2 --ws fix)'
    );
  });
  ...
  test('workflow updates STATE.md on convergence', () => {
    assert.ok(
      ...
      'workflow must update STATE.md via sdd-tools.cjs when converged'
    );
  });
  ...
  test('workflow spawns replan agent with --reviews flag', () => {
    assert.ok(
      ...
      'replan agent must pass --reviews so sdd-plan-phase incorporates review feedback'
    );
  });
...
describe('plan-review-convergence config schema registration (#2306-v2)', () => {
  ...
  test('workflow.plan_review_convergence is registered in config-schema.cjs', () => {
    assert.ok(
      ...
      "workflow.plan_review_convergence must be registered in VALID_CONFIG_KEYS in config-schema.cjs so sdd config-set accepts it (#2306-v2)"
    );
  });
});
...
describe('plan-review-convergence local model config schema registration (#2306-local)', () => {
  ...
  test('review.ollama_host is registered in config-schema.cjs', () => {
    assert.ok(
      ...
      "review.ollama_host must be in VALID_CONFIG_KEYS so sdd config-set accepts it"
    );
  });
  ...
  test('review.lm_studio_host is registered in config-schema.cjs', () => {
    assert.ok(
      ...
      "review.lm_studio_host must be in VALID_CONFIG_KEYS so sdd config-set accepts it"
    );
  });
  ...
  test('review.llama_cpp_host is registered in config-schema.cjs', () => {
    assert.ok(
      ...
      "review.llama_cpp_host must be in VALID_CONFIG_KEYS so sdd config-set accepts it"
    );
  });
});
