 ...
 *
 * Verifies that /sdd-review includes Cursor CLI as a peer reviewer:
 *   - review.md workflow contains cursor detection, flag parsing, self-detection, invocation
 *   - commands/sdd/review.md command file mentions --cursor flag
 *   - help.md lists --cursor in the /sdd-review signature
...
describe('Cursor CLI reviewer in /sdd-review (#1960)', () => {

  // --- review.md workflow ---

  describe('review.md workflow', () => {
    const reviewPath = path.join(ROOT, 'sdd', 'workflows', 'review.md');
    let content;
  ...
  describe('commands/sdd/review.md', () => {
    const cmdPath = path.join(ROOT, 'commands', 'sdd', 'review.md');

    test('mentions --cursor flag', () => {
      const c = fs.readFileSync(cmdPath, 'utf-8');
      assert.ok(
        c.includes('--cursor'),
        'commands/sdd/review.md should mention --cursor flag'
      );
    });
    ...
    test('mentions Cursor in objective or context', () => {
      ...
      assert.ok(
        c.includes('Cursor'),
        'commands/sdd/review.md should mention Cursor'
      );
    });
  });
  ...
  describe('help.md', () => {
    const helpPath = path.join(ROOT, 'sdd', 'workflows', 'help.md');

    test('lists --cursor in /sdd-review signature', () => {
      const c = fs.readFileSync(helpPath, 'utf-8');
      assert.ok(
        c.includes('--cursor'),
        'help.md should list --cursor in the /sdd-review command signature'
      );
    });
  });
