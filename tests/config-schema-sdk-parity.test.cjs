 ...
 * MUST match the CJS allowlist at sdd/bin/lib/config-schema.cjs or
...
test('#2653 — SDK VALID_CONFIG_KEYS matches CJS VALID_CONFIG_KEYS', () => {
  ...
  assert.deepStrictEqual(
    extraInSdk,
    [],
    'SDK keys missing from sdd/bin/lib/config-schema.cjs:\n' +
...
test('#3162 — SDK RUNTIME_STATE_KEYS matches CJS RUNTIME_STATE_KEYS', () => {
  ...
  assert.deepStrictEqual(
    extraInSdk,
    [],
    'SDK runtime-state keys missing from sdd/bin/lib/config-schema.cjs:\n' +
      extraInSdk.map((k) => '  ' + k).join('\n'),
  );
});
