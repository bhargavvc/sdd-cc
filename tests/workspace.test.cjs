...
const { runSddTools, createTempProject, createTempDir, cleanup } = require('./helpers.cjs');
const { detectChildRepos } = require('../sdd/bin/lib/init.cjs');
...
describe('detectChildRepos', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('sdd-ws-test-');
  });
...
// ─── cmdInitNewWorkspace via sdd-tools ──────────────────────────────────────
...
describe('init new-workspace', () => {
  ...
  test('returns expected JSON fields', () => {
    const result = runSddTools('init new-workspace', tmpDir);
...
// ─── cmdInitListWorkspaces via sdd-tools ────────────────────────────────────
...
describe('init list-workspaces', () => {
  ...
  test('returns empty list when no workspaces exist', () => {
    const result = runSddTools('init list-workspaces', tmpDir, { HOME: tmpDir });
    assert.ok(result.success, `init failed: ${result.error}`);
    const data = JSON.parse(result.output);
  ...
  test('finds workspaces with WORKSPACE.md', () => {
    const wsBase = path.join(tmpDir, 'sdd-workspaces');
...
// ─── cmdInitRemoveWorkspace via sdd-tools ───────────────────────────────────
...
describe('init remove-workspace', () => {
  ...
  test('errors when no name provided', () => {
    const result = runSddTools('init remove-workspace', tmpDir);
    assert.strictEqual(result.success, false);
    assert.ok(result.error.includes('workspace name required'));
  });

  test('errors when workspace not found', () => {
    const result = runSddTools('init remove-workspace nonexistent', tmpDir, { HOME: tmpDir });
    assert.strictEqual(result.success, false);
  ...
  test('returns workspace info for existing workspace', () => {
    ...
    const result = runSddTools('init remove-workspace test-ws', tmpDir, { HOME: tmpDir });
    assert.ok(result.success, `init failed: ${result.error}`);
    const data = JSON.parse(result.output);
...
describe('workspace worktree integration', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('sdd-ws-integ-');
...
describe('workspace command files', () => {
   ...
   * Each line of the form `@~/.claude/sdd/workflows/foo.md` becomes
  ...
  function executionContextIncludes(body) {
    ...
    for (const blk of blocks) {
      for (const line of blk.split('\n')) {
        ...
        // Normalize away the home-prefix and the `.claude/sdd/` root
  ...
  test('consolidated workspace.md command declares correct frontmatter contract (#2790)', () => {
    ...
    const { fm } = parseCommandFile(path.join(baseDir, 'commands/sdd/workspace.md'));
    assert.equal(fm.name, 'sdd:workspace', `workspace.md frontmatter name must be "sdd:workspace"; got "${fm.name}"`);
  ...
  test('workspace.md @-includes the new-workspace workflow', () => {
    const { body } = parseCommandFile(path.join(baseDir, 'commands/sdd/workspace.md'));
...
// ─── Routing in sdd-tools ───────────────────────────────────────────────────

describe('workspace routing in sdd-tools', () => {
  let tmpDir;
  ...
  test('init new-workspace is routed correctly', () => {
    const result = runSddTools('init new-workspace test-ws', tmpDir);
  ...
  test('init list-workspaces is routed correctly', () => {
    const result = runSddTools('init list-workspaces', tmpDir);
  ...
  test('init remove-workspace is routed correctly', () => {
    const result = runSddTools('init remove-workspace nonexistent-ws', tmpDir);
    const stderr = result.error || '';
    assert.ok(
      !stderr.includes('Unknown init workflow'),
