 ...
 * `npx @bhargavvc/sdd-cc@latest --global --claude`:
 *   sdd-sdk: The term 'sdd-sdk' is not recognized
 *
 ...
 *    POSIX-style paths.
 *
 * B. isSddSdkOnPath (zero-arg fallback) reads `process.env.PATH || ''`. On
...
// ---------------------------------------------------------------------------
// B. isSddSdkOnPath — does not return true when only a Windows _npx dir has
//    sdd-sdk.cmd (using filterNpxFromPath on the passed pathString)
// ---------------------------------------------------------------------------

describe('bug #3211-B: isSddSdkOnPath rejects Windows _npx-only transient PATH', () => {
  let tmpRoot;

  before(() => {
    tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3211-b-'));
  });
  ...
  test('returns false when a sdd-sdk.cmd exists only in an _npx-style transient dir', () => {
    // On POSIX we name the dir with _npx/ to match the filter pattern.
    // We can't set process.platform, but we CAN call isSddSdkOnPath with an
    ...
    // isSddSdkOnPath. We test filterNpxFromPath(pathString) produces an
    // empty result, which means isSddSdkOnPath of the filtered path returns false.
    const npxBinDir = path.join(tmpRoot, '_npx', 'abc123', 'node_modules', '.bin');
    fs.mkdirSync(npxBinDir, { recursive: true });

    // Write a sdd-sdk shim (named .cmd for the Windows scenario — on POSIX
    // isSddSdkOnPath won't find .cmd; we validate the filter, not the exec check).
    const shimPath = path.join(npxBinDir, 'sdd-sdk.cmd');
    fs.writeFileSync(
      shimPath,
      ['@ECHO OFF', '@node "C:\\path\\to\\sdd-sdk.js" %*', ''].join('\r\n'),
    );
    ...
    // After filtering, the _npx dir must be gone so isSddSdkOnPath returns false.
    const onPath = isSddSdkOnPath(persistentPath);
    assert.equal(
      onPath,
      false,
      'isSddSdkOnPath(filterNpxFromPath(path)) must return false when only _npx dir has sdd-sdk. persistentPath=' + persistentPath,
    );
  });
});
...
//    NOT on PATH → must NOT print "SDD SDK ready"
// ---------------------------------------------------------------------------
...
describe('bug #3211-D: installSdkIfNeeded — Windows _npx false-positive', () => {
  ...
  beforeEach(() => {
    tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3211-d-'));
    sdkDir = makeSdkDir(tmpRoot);

    // Simulate: install-time PATH contains only a transient _npx dir
    // with a sdd-sdk shim. The persistent npm prefix dir is separate and
    // NOT in process.env.PATH during the npx run.
    const npxBinDir = path.join(tmpRoot, '_npx', 'abc123', 'node_modules', '.bin');
    fs.mkdirSync(npxBinDir, { recursive: true });
    // Write a sdd-sdk shim in the transient dir (executable on POSIX)
    const shimName = 'sdd-sdk';
    fs.writeFileSync(
      path.join(npxBinDir, shimName),
      ['#!/bin/sh', 'exec node /path/to/sdd-sdk.js "$@"', ''].join('\n'),
      { mode: 0o755 },
    );
  ...
  test('does NOT print "SDD SDK ready" when the only sdd-sdk entry is in a transient _npx dir and npm-prefix is off-PATH', () => {
    ...
    assert.ok(
      ...
      'installer must NOT print "SDD SDK ready" when sdd-sdk is only in a transient _npx dir. Got:\n' + combined,
    );
...
// ---------------------------------------------------------------------------
// E. isLegacySddSdkShim — Windows .cmd shim detection
// ---------------------------------------------------------------------------

describe('bug #3211-E: isLegacySddSdkShim detects legacy marker in .cmd files', () => {
  let tmpRoot;

  before(() => {
    tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3211-e-'));
  });
  ...
  test('detects @deprecated + sdd-tools.cjs in a .cmd shim', () => {
    assert.equal(typeof isLegacySddSdkShim, 'function');

    const legacyCmd = path.join(tmpRoot, 'sdd-sdk.cmd');
    fs.writeFileSync(
      legacyCmd,
      [
        '@ECHO OFF',
        ':: @deprecated — use sdd-tools.cjs directly',
        '@node "C:\\path\\to\\sdd-tools.cjs" %*',
        '',
      ].join('\r\n'),
    );
    assert.equal(isLegacySddSdkShim(legacyCmd), true, 'must detect @deprecated sdd-tools.cjs in .cmd shim');
  });

  test('returns false for a modern .cmd shim pointing at sdd-sdk.js', () => {
    const modernCmd = path.join(tmpRoot, 'sdd-sdk-modern.cmd');
    fs.writeFileSync(
      modernCmd,
      [
        '@ECHO OFF',
        '@SETLOCAL',
        '@node "C:\\path\\to\\@bhargavvc/sdd-cc\\bin\\sdd-sdk.js" %*',
        '',
      ].join('\r\n'),
    );
    assert.equal(isLegacySddSdkShim(modernCmd), false, 'must not flag modern .cmd shim as legacy');
  });

  test('returns false for a missing file', () => {
    assert.equal(isLegacySddSdkShim(path.join(tmpRoot, 'no-such-file.cmd')), false);
  });
});
