 ...
 *
 * Context: on Windows (and occasionally other platforms), sdd-planner's
...
const PLAN_PHASE = path.join(
  __dirname, '..', 'sdd', 'workflows', 'plan-phase.md'
);

const PLANNER_AGENT = path.join(__dirname, '..', 'agents', 'sdd-planner.md');
const PLANNER_CHUNKED_REF = path.join(__dirname, '..', 'sdd', 'references', 'planner-chunked.md');
const CONFIG_SCHEMA = path.join(__dirname, '..', 'sdd', 'bin', 'lib', 'config-schema.cjs');
...
describe('sdd-planner.md — references planner-chunked.md (#2310)', () => {
  const plannerContent = fs.readFileSync(PLANNER_AGENT, 'utf-8');

  test('sdd-planner.md references planner-chunked.md for chunked return formats', () => {
    assert.ok(
      plannerContent.includes('planner-chunked.md'),
      'sdd-planner.md must reference planner-chunked.md for ## OUTLINE COMPLETE / ## PLAN COMPLETE formats'
    );
  });
});
