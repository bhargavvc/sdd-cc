 ...
 * write time. SDD's status used to be mtime-only, a poor proxy for "does
...
const { graphifyStatus } = require('../sdd/bin/lib/graphify.cjs');
...
describe('enh-3170: graphifyStatus surfaces built_at_commit staleness', () => {
  ...
  describe('git-aware', () => {
    ...
    test('built_at_commit absent (pre-v0.7 graph): all four new fields null', () => {
      // No built_at_commit on the graph -- SDD must not fabricate one.
