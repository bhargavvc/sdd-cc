...
function createTaggedRepo() {
  const repo = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-release-notes-'));
  run('git', ['init', '-q'], repo);
  ...
  writeFragment(repo, 'fix-install-sdk', 'Fixed', 101, '**`sdd-sdk` now installs reliably** — persistent PATH is checked.');
  writeFragment(repo, 'remove-intel-noise', 'Removed', 102, '**`sdd-intel-updater` no longer emits layout detection noise** — ordinary projects stay quiet.');
...
describe('changeset github release notes: tag-range renderer (#3382)', () => {
  ...
  test('CLI writes a notes file suitable for gh release edit --notes-file', () => {
    ...
    const generated = renderGithubReleaseNotes({
      ...
      installCommand: 'npx @bhargavvc/sdd-cc@latest',
    });
