...
// extracts every ${SDD_HOME}/<path> script reference, and returns a
...
function fixtureRepo({ workflows, files }) {
  // workflows: { 'foo.md': '...content with ${SDD_HOME}/...' }
  // files:     [ 'sdd/bin/x.cjs', ... ]  — files to create in repo
  const repoRoot = fs.mkdtempSync(path.join(tmpRoot, 'repo-'));
  const workflowsDir = path.join(repoRoot, 'sdd', 'workflows');
...
before(() => { tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2995-')); });
...
describe('Bug #2995: post-install script-paths audit (#2995)', () => {
  ...
  test('returns { ok: true, findings: [] } when workflow refs an existing, installed-path script', () => {
    const { repoRoot, workflowsDir } = fixtureRepo({
      workflows: {
        'good.md': 'node "${SDD_HOME}/sdd/bin/foo.cjs" --json\n',
      },
      files: ['sdd/bin/foo.cjs'],
    });
    const r = auditWorkflowScriptPaths({
      workflowsDir,
      repoRoot,
      installedPrefixes: ['sdd', 'commands', 'agents', 'hooks'],
    });
...
describe('Bug #2995: detection paths', () => {
  ...
  test('reports MISSING_FROM_REPO when the referenced file does not exist in the repo', () => {
    const { repoRoot, workflowsDir } = fixtureRepo({
      workflows: {
        'foo.md': 'node "${SDD_HOME}/sdd/bin/typo.cjs" --json\n',
      },
      files: [],
    });
    const r = auditWorkflowScriptPaths({
      workflowsDir,
      repoRoot,
      installedPrefixes: ['sdd'],
    });
    assert.equal(r.ok, false);
    assert.equal(r.findings.length, 1);
    assert.deepEqual(r.findings[0], {
      workflow: 'foo.md',
      path: 'sdd/bin/typo.cjs',
  ...
  test('reports NOT_INSTALLED when first path segment is outside installedPrefixes (the #2994 case)', () => {
    const { repoRoot, workflowsDir } = fixtureRepo({
      workflows: {
        'foo.md': 'node "${SDD_HOME}/scripts/verify-reapply-patches.cjs"\n',
      },
  ...
  test('handles ${SDD_HOME:-$HOME/.claude}/... default-fallback syntax', () => {
    const { repoRoot, workflowsDir } = fixtureRepo({
      workflows: {
        'a.md': 'node "${SDD_HOME:-$HOME/.claude}/sdd/bin/x.cjs"\n',
      },
      files: ['sdd/bin/x.cjs'],
    });
    const r = auditWorkflowScriptPaths({
  ...
  test('reports both findings when one workflow has multiple problems', () => {
    const { repoRoot, workflowsDir } = fixtureRepo({
      workflows: {
        'multi.md': [
          'node "${SDD_HOME}/scripts/a.cjs"',
          'node "${SDD_HOME}/sdd/bin/b.cjs"',
          'node "${SDD_HOME}/sdd/bin/missing.cjs"',
        ].join('\n') + '\n',
      },
      files: ['scripts/a.cjs', 'sdd/bin/b.cjs'],
    });
    const r = auditWorkflowScriptPaths({
      workflowsDir,
      repoRoot,
  ...
  test('extracts no findings from a workflow without SDD_HOME script refs', () => {
    const { repoRoot, workflowsDir } = fixtureRepo({
      workflows: {
...
describe('Bug #2995: real workflow audit', () => {
  ...
  const INSTALLED_PREFIXES = [
    'sdd',  // workflows, references, bin/lib, templates
    'commands',       // commands/sdd/*.md (Claude Code local + Gemini global)
    'skills',         // skills/sdd-*/SKILL.md (Claude Code 2.1.88+ global, Codex, etc.)
    'agents',         // agents/sdd-*.md
    'hooks',          // hooks/sdd-*.{sh,js}
  ];
  ...
  // sdd/bin/ which IS an installed prefix, closing the gap.)
  const KNOWN_GAPS = new Set();

  test('no NEW workflow refs fail to resolve at the deployed path (KNOWN_GAPS allow-listed)', () => {
    const r = auditWorkflowScriptPaths({
      workflowsDir: require('node:path').join(ROOT, 'sdd', 'workflows'),
      repoRoot: ROOT,
  ...
  test('a reference that is both not-installed AND missing-from-repo emits both findings (no short-circuit)', () => {
    const { repoRoot, workflowsDir } = fixtureRepo({
      workflows: {
        'foo.md': '```bash\nnode "${SDD_HOME}/scripts/missing.cjs"\n```\n',
      },
    ...
    const r = auditWorkflowScriptPaths({
      workflowsDir,
      repoRoot,
      installedPrefixes: ['sdd', 'agents', 'hooks', 'commands'],
    });
    assert.equal(r.ok, false);
