 ...
 *
 * Original (#2643): workflows emitted Skill(skill="sdd:<cmd>") and the
 * installer registered colon form in SKILL.md name: to match.
 *
 * Updated (#2808): workflows now use Skill(skill="sdd-<cmd>") (hyphen),
 * and the installer emits name: sdd-<cmd> (hyphen). Claude Code autocomplete
 * now shows the canonical hyphen form instead of the deprecated colon form.
 * The directory name (sdd-<cmd>) is unchanged.
 */
 ...
 *      `Skill(skill="sdd:extract_learnings")` is surfaced rather than
 *      silently skipped by an over-strict character class.
 *
 * Returns `[{ name, raw }]` per call. Filtering by namespace (sdd- vs sdd:)
...
function extractSkillCalls(content) {
  ...
  // (e.g. `Skill(skill=\"sdd-plan-phase\", …)` written inside a string
...
describe('skill frontmatter name parity (#2643 / #2808)', () => {
  test('skillFrontmatterName helper emits hyphen form (#2808)', () => {
    ...
    assert.strictEqual(skillFrontmatterName('sdd-execute-phase'), 'sdd-execute-phase');
    assert.strictEqual(skillFrontmatterName('sdd-plan-phase'), 'sdd-plan-phase');
    assert.strictEqual(skillFrontmatterName('sdd-next'), 'sdd-next');
  });

  test('convertClaudeCommandToClaudeSkill emits name: sdd-<cmd> (hyphen)', () => {
    const input = '---\nname: old\ndescription: test\n---\n\nBody.';
    const result = convertClaudeCommandToClaudeSkill(input, 'sdd-execute-phase');
    ...
    assert.strictEqual(
      nameValue,
      'sdd-execute-phase',
      `frontmatter name: must be 'sdd-execute-phase' (hyphen form), got '${nameValue}'`
    );
  });

  test('no workflow uses deprecated Skill(skill="sdd:<cmd>") colon form', () => {
  ...
  test('every workflow Skill(skill="sdd-<cmd>") resolves to an emitted skill name', () => {
    const workflowFiles = collectFiles(WORKFLOWS_DIR);
    ...
    assert.ok(
      referenced.size > 0,
      `expected at least one Skill(skill="sdd-<cmd>") reference in workflows under ${WORKFLOWS_DIR}`
    );
