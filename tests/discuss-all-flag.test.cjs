/**
 * Tests for --all flag on /sdd-discuss-phase (#2188)
 *
...
describe('#2188: discuss-phase --all flag', () => {
  ...
  test('discuss-phase workflow handles --all flag in present_gray_areas', () => {
    const workflow = fs.readFileSync(
      path.join(__dirname, '..', 'sdd', 'workflows', 'discuss-phase.md'), 'utf8'
    );
  ...
  test('COMMANDS.md documents --all flag for discuss-phase', () => {
    ...
    const discussSection = commands.slice(
      commands.indexOf('sdd-discuss-phase') > -1 ? commands.indexOf('sdd-discuss-phase') : commands.indexOf('discuss-phase')
    );
