 ...
 *
 * The installer prints `✓ SDD SDK ready (sdk/dist/cli.js)` whenever
 * isSddSdkOnPath() — which reads process.env.PATH from the install
 * subprocess — finds the shim. That set is not the same as the user's
 * later interactive shell PATH:
 *
 *   - Windows cross-shell: sdd-sdk.cmd resolves under PowerShell/cmd
 *     (PATHEXT) but bare `sdd-sdk` does not resolve under Git Bash /
 *     MSYS / WSL bash.
 ...
 *
 *   isSddSdkOnPath(pathString?: string)
...
const { isSddSdkOnPath, getUserShellPath } = INSTALL;

describe('bug #3020: isSddSdkOnPath accepts an explicit PATH string', () => {
  test('exported as a function', () => {
    assert.equal(typeof isSddSdkOnPath, 'function');
  });

  test('returns true when an executable sdd-sdk exists in the supplied PATH', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3020-'));
    try {
      // Create a fake `sdd-sdk` shim with the executable bit set.
      ...
      const result = isSddSdkOnPath(tmp);
  ...
  test('returns false when the supplied PATH has no sdd-sdk', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3020-'));
    try {
      const result = isSddSdkOnPath(tmp);
  ...
  test('null pathString is type-guarded — falls back to process.env.PATH (#3028 CR)', () => {
    // Pre-fix: isSddSdkOnPath(null) threw "Cannot read properties of null
    // (reading 'split')". Post-fix: typeof check falls back to process.env.PATH.
    let threw = null;
    let result;
    try {
      result = isSddSdkOnPath(null);
    } catch (e) {
      threw = e;
    }
  ...
  test('non-string pathString (number, object) falls back to process.env.PATH (#3028 CR)', () => {
    ...
    assert.equal(typeof isSddSdkOnPath(0), 'boolean');
    assert.equal(typeof isSddSdkOnPath({}), 'boolean');
    assert.equal(typeof isSddSdkOnPath([]), 'boolean');
  });
});
...
describe('bug #3020: cross-shell PATH mismatch is detectable via the new helpers', () => {
  test('install-time PATH has shim, user-shell PATH does not → mismatch detected', () => {
    const installDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3020-install-'));
    const userDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3020-user-'));
    try {
      const shimName = process.platform === 'win32' ? 'sdd-sdk.cmd' : 'sdd-sdk';
      ...
      const installSees = isSddSdkOnPath(installDir);
      const userSees = isSddSdkOnPath(userDir);
