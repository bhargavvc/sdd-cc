...
const COMMAND_PATH = path.join(__dirname, '..', 'commands', 'sdd', 'execute-phase.md');
const WORKFLOW_PATH = path.join(__dirname, '..', 'sdd', 'workflows', 'execute-phase.md');
const COMMANDS_DOC_PATH = path.join(__dirname, '..', 'docs', 'COMMANDS.md');
const HELP_PATH = path.join(__dirname, '..', 'sdd', 'workflows', 'help.md');
...
describe('execute-phase command: --wave flag', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(COMMAND_PATH), 'commands/sdd/execute-phase.md should exist');
  });
...
describe('execute-phase docs: user-facing wave flag', () => {
  ...
  test('help workflow documents --wave behavior', () => {
    ...
    assert.ok(
      content.includes('Usage: `/sdd:execute-phase 5 --wave 2`') || content.includes('Usage: `/sdd-execute-phase 5 --wave 2`'),
...
describe('phase-plan-index: wave grouping behavior', () => {
  test('phase-plan-index groups plans by wave (DAG-bucketing: P002 depends on P001)', () => {
    // allow-test-rule: behavioral — calls sdd-tools and asserts structured output
    const fs = require('fs');
    const path = require('path');
    ...
    try {
      ...
      const result = runSddTools(['phase-plan-index', '1', '--raw'], tmpDir);
  ...
  test('phase-plan-index defaults missing wave frontmatter to wave 1', () => {
    // allow-test-rule: behavioral — exercises sdd-tools wave-defaulting logic
    const fs = require('fs');
    const path = require('path');
    const tmpDir = createTempProject();
    try {
...
describe('use_worktrees config: cross-workflow structural coverage', () => {
  ...
  test('config-set accepts workflow.use_worktrees', () => {
    ...
    const tmpDir = createTempProject();
    try {
      const result = runSddTools('config-set workflow.use_worktrees true', tmpDir);
