 ...
 *     <command-name>/sdd-plan-phase</command-name>, output includes "last: /sdd-plan-phase"
 ...
 *   - the config key is registered in the schema so /sdd-settings can surface it
 */
...
const statusline = require('../hooks/sdd-statusline.js');
...
test('default (flag absent) output has no "last:" suffix', () => {
  const transcript =
    JSON.stringify({ type: 'user', message: { content: '<command-name>/sdd-plan-phase</command-name>' } }) + '\n';
...
test('flag=true with recorded command yields "last: /<cmd>"', () => {
  ...
  try {
    const out = statusline.renderStatusline(buildInput(dir, transcriptPath));
    assert.ok(out.includes('last: /sdd-plan-phase'), `expected "last: /sdd-plan-phase" in output; got: ${out}`);
  } finally {
    cleanup();
  }
});
...
test('flag=true picks the MOST RECENT command when multiple are present', () => {
  const transcript =
    JSON.stringify({ type: 'user', message: { content: '<command-name>/sdd-discuss-phase</command-name>' } }) + '\n' +
    JSON.stringify({ type: 'user', message: { content: '<command-name>/sdd-plan-phase</command-name>' } }) + '\n' +
    JSON.stringify({ type: 'user', message: { content: '<command-name>/sdd-execute-phase</command-name>' } }) + '\n';
  const { dir, transcriptPath, cleanup } = makeProject({ flag: true, transcript });
  try {
    const out = statusline.renderStatusline(buildInput(dir, transcriptPath));
    assert.ok(out.includes('last: /sdd-execute-phase'), `expected most-recent "sdd-execute-phase"; got: ${out}`);
    assert.ok(!out.includes('last: /sdd-discuss-phase'), `should not show stale command; got: ${out}`);
  } finally {
    cleanup();
  }
});
