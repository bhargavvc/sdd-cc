...
const QUICK_MD = path.join(__dirname, '..', 'sdd', 'workflows', 'quick.md');

describe('quick.md pre-dispatch PLAN.md commit (#2432)', () => {
  let content;

  test('quick.md exists', () => {
    assert.ok(fs.existsSync(QUICK_MD), 'sdd/workflows/quick.md must exist');
  ...
  test('executor prompt references PLAN.md via relative (worktree-rooted) path', () => {
    ...
    const executorTask = content.indexOf('subagent_type="sdd-executor"');
