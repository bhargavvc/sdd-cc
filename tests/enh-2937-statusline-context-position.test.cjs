 ...
 *   - sddUpdate warning stays leftmost in both "front" and "end" modes
 */

const { test } = require('node:test');
const assert = require('node:assert/strict');

const { composeStatusline } = require('../hooks/sdd-statusline.js');
...
// ── sddUpdate leftmost invariant ─────────────────────────────────────────────

test('sddUpdate warning is leftmost in "end" mode', () => {
  const sddUpdate = '\x1b[33m⬆ /sdd:update\x1b[0m │ ';
  const out = composeStatusline({ sddUpdate, model: 'Claude', dirname: 'proj', position: 'end' });
  assert.ok(out.startsWith(sddUpdate), `sddUpdate should be leftmost in end mode; got: ${out}`);
});

test('sddUpdate warning is leftmost in "front" mode', () => {
  const sddUpdate = '\x1b[33m⬆ /sdd:update\x1b[0m │ ';
  const ctx = ' \x1b[32m████░░░░░░ 40%\x1b[0m';
  const out = composeStatusline({ sddUpdate, model: 'Claude', dirname: 'proj', ctx, position: 'front' });
  assert.ok(out.startsWith(sddUpdate), `sddUpdate should be leftmost in front mode; got: ${out}`);
});
