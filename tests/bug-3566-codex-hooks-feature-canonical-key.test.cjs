 ...
 * `hooks`. The SDD installer was still writing `codex_hooks` on every fresh
 ...
 *   2. Reinstall over a SDD-owned section-form legacy
 ...
 *   3. Reinstall over a SDD-owned root-dotted legacy
 *      `features.codex_hooks = true` migrates forward to
 *      `features.hooks = true`; user-owned legacy entries are preserved.
 *   4. Reinstall over a user-owned `[features].hooks = true` (no SDD
 ...
 *   6. Uninstall removes either SDD-owned `hooks` or SDD-owned legacy
...
describe('#3566 — Codex feature flag is canonical "hooks" (not legacy "codex_hooks")', { concurrency: false }, () => {
  ...
  beforeEach(() => {
    ...
    tmpRoot = createTempDir('sdd-3566-');
  ...
  test('install over a pre-existing legacy [features].codex_hooks line preserves it (user-owned, #2760 defensive)', () => {
    ...
    // forward-compatible without SDD rewriting user-authored content.
    const legacy = [
      '[features]',
      'codex_hooks = true',
      '',
  ...
  test('reinstall preserves user-owned [features].hooks = true (no SDD ownership marker)', () => {
    const userOwned = [
      '[features]',
      'hooks = true',
      '',
  ...
  test('uninstall removes SDD-owned canonical hooks line but preserves user-owned hooks', () => {
    // Phase 1: fresh SDD install — writes SDD-owned hooks line.
    withCodexHome(codexHome, () => install(true, 'codex'));
    ...
    assert.notStrictEqual(
      ...
      true,
      'uninstall must remove SDD-owned canonical hooks line',
    );
  });

  test('uninstall preserves user-owned hooks=true when SDD never owned it', () => {
    const userOwned = [
      '[features]',
      'hooks = true',
      '',
    ...
    assert.strictEqual(
      featuresHooks(parsed),
      true,
      'uninstall must NOT touch a hooks line SDD never claimed ownership of (#2760 defensive principle)',
    );
  });
});
