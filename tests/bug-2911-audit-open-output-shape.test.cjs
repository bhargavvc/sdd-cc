 ...
 *
 * Two bugs in the `audit-open` dispatch case in bin/sdd-tools.cjs:
 *
...
describe('audit-open — output shape (#2911)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject('sdd-bug-2911-');
  });
  ...
  test('--json mode emits parseable JSON matching auditOpenArtifacts shape', () => {
    ...
    // Shape contract from auditOpenArtifacts() in sdd/bin/lib/audit.cjs.
