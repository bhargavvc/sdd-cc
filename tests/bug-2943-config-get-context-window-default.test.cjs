 ...
 *
 * `sdd-tools.cjs config-get context_window` (and the SDK equivalent) threw
...
// "Key not found"; now the test passes `--json-errors` to sdd-tools and
...
const SDD_TOOLS = path.join(__dirname, '..', 'sdd', 'bin', 'sdd-tools.cjs');
const { ERROR_REASON } = require(path.join(__dirname, '..', 'sdd', 'bin', 'lib', 'core.cjs'));
...
describe('bug-2943: config-get returns schema default for context_window', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-test-2943-'));
  ...
  function runConfigGet(keyPath, extraArgs = []) {
    const args = [SDD_TOOLS, 'config-get', keyPath, '--raw', '--cwd', tmpDir, ...extraArgs];
    let stdout = '';
    let stderr = '';
    let exitCode = 0;
    try {
