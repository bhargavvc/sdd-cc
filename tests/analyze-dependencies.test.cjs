...
describe('analyze-dependencies command', () => {
  test('analyze-dependencies command file was consolidated away (deleted in #2790)', () => {
    // The standalone /sdd-analyze-dependencies command was removed as a dead skill in #2790.
    // The underlying workflow (workflows/analyze-dependencies.md) remains functional.
    const deleted = path.join(__dirname, '..', 'commands', 'sdd', 'analyze-dependencies.md');
  ...
  test('workflow file is sufficient without a standalone command file', () => {
    const p = path.join(__dirname, '..', 'sdd', 'workflows', 'analyze-dependencies.md');
  ...
  test('docs/COMMANDS.md does not document the consolidated-away /sdd-analyze-dependencies entry', () => {
    ...
    // from consolidated entry points (see sdd-phase / sdd-progress workflow chains).
    ...
    assert.ok(!/^### `\/sdd-analyze-dependencies`/m.test(content),
      'COMMANDS.md should not document the removed /sdd-analyze-dependencies command');
  });
});
