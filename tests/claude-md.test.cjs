...
describe('new-project workflow includes CLAUDE.md generation', () => {
  const workflowPath = path.join(__dirname, '..', 'sdd', 'workflows', 'new-project.md');
...
describe('generate-claude-md skills section', () => {
  ...
  test('discovers skills from .codex/skills/ directory and ignores deprecated import-only roots', () => {
    ...
    const homeDir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'sdd-claude-skills-home-'));
    fs.mkdirSync(path.join(homeDir, '.claude', 'sdd', 'skills', 'import-only'), { recursive: true });
    fs.writeFileSync(
      path.join(homeDir, '.claude', 'sdd', 'skills', 'import-only', 'SKILL.md'),
