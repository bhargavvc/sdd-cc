...
describe('pattern-mapper config key', () => {
  ...
  test('workflow.pattern_mapper is a valid config key', () => {
    ...
    const result = runSddTools('config-set workflow.pattern_mapper true', tmpDir, { HOME: tmpDir });
  ...
  test('default value is true in CONFIG_DEFAULTS', () => {
    ...
    const result = runSddTools('config-new-project', tmpDir, { HOME: tmpDir });
  ...
  test('config round-trip set/get', () => {
    ...
    runSddTools('config-new-project', tmpDir, { HOME: tmpDir });

    // Set to false
    const setResult = runSddTools('config-set workflow.pattern_mapper false', tmpDir, { HOME: tmpDir });
    assert.ok(setResult.success, `config-set failed: ${setResult.error}`);

    // Get should return false
    const getResult = runSddTools('config-get workflow.pattern_mapper', tmpDir, { HOME: tmpDir });
...
describe('init plan-phase patterns_path', () => {
  ...
  test('patterns_path is null when no PATTERNS.md exists', () => {
    const result = runSddTools('init plan-phase 1', tmpDir, { HOME: tmpDir });
...
describe('sdd-pattern-mapper agent prompt efficiency constraints (#2312)', () => {
  const agentPath = path.join(__dirname, '..', 'agents', 'sdd-pattern-mapper.md');
  let agentContent;
