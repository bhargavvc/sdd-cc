...
describe('bug #3442: codex legacy hooks.json migration consumes shared managed-hook policy', () => {
  test('plan prunes managed codex hook commands including legacy alias', () => {
    ...
    const hooksJson = {
      hooks: [
        { command: '"/usr/local/bin/node" "/Users/me/.codex/hooks/sdd-check-update.js"' },
        { command: '"/usr/local/bin/node" "/Users/me/.codex/hooks/sdd-update-check.js"' },
  ...
  test('plan preserves similarly named commands outside the managed hooks directory', () => {
    ...
    const hooksJson = {
      hooks: [
        { command: '"/usr/local/bin/node" "/tmp/other/hooks/sdd-check-update.js"' },
      ],
    };
