...
function makeTempSdk({ npxCache = false } = {}) {
  let root;
  if (npxCache) {
    root = path.join(os.tmpdir(), `sdd-npx-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, 'npm-cache', '_npx', 'deadbeefcafe0001', 'node_modules', '@bhargavvc/sdd-cc');
    fs.mkdirSync(root, { recursive: true });
  } else {
    root = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-clone-'));
  }
...
describe('installer SDK dist-missing fail-fast (#2649)', () => {
  ...
  test('missing dist in npx cache: fail fast, no nested npm install', () => {
    ...
    try {
      ...
      assert.strictEqual(ir.fix_command, 'npm install -g @bhargavvc/sdd-cc@latest');
