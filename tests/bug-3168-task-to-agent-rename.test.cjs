...
// commands/sdd/*.md, sdd/workflows/*.md, and agents/sdd-*.md are
 ...
 * tracker — a distinct tool set. SDD workflows were partially migrated and still
...
describe('#3168 — commands/sdd: allowed-tools must use Agent not Task', () => {
...
describe('#3168 — agents: tools frontmatter must use Agent not Task', () => {
  const agents = readMdFiles(AGENTS_DIR, 'sdd-');
