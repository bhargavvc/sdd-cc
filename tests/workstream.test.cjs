...
const { migrateToWorkstreams, getOtherActiveWorkstreams } = require('../sdd/bin/lib/workstream.cjs');
...
describe('planningDir workstream awareness via env var', () => {
  ...
  test('--ws flag overrides SDD_WORKSTREAM env var', () => {
...
describe('workstream progress', () => {
  ...
  test('clamps progress percent when completed phase dirs exceed roadmap count', () => {
    ...
    try {
      ...
      const result = runSddTools(['workstream', 'progress', '--raw'], isolatedDir);
      assert.ok(result.success, `progress failed: ${result.error}`);
      const data = JSON.parse(result.output);
  ...
  test('returns flat mode when no workstreams exist', () => {
    ...
    try {
      const result = runSddTools(['workstream', 'progress', '--raw'], emptyDir);
...
// ─── Integration: sdd-tools --ws flag ────────────────────────────────────────

describe('sdd-tools --ws flag integration', () => {
  let tmpDir;
...
describe('path traversal rejection', () => {
  ...
  describe('setActiveWorkstream rejects invalid names directly', () => {
    const { setActiveWorkstream } = require('../sdd/bin/lib/core.cjs');
    for (const name of maliciousNames) {
