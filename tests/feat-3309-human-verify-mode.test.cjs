...
describe('workflow.human_verify_mode default value', () => {
  ...
  test('defaults to end-of-phase in new project config', () => {
    const result = runSddTools('config-ensure-section', tmpDir, { HOME: tmpDir });
...
describe('workflow.human_verify_mode config round-trip', () => {
  ...
  beforeEach(() => {
    ...
    runSddTools('config-ensure-section', tmpDir, { HOME: tmpDir });
  });
  afterEach(() => { cleanup(tmpDir); });

  test('config-set end-of-phase persists to config.json', () => {
    const setResult = runSddTools('config-set workflow.human_verify_mode end-of-phase', tmpDir);
  ...
  test('config-set mid-flight overwrites end-of-phase in config.json', () => {
    runSddTools('config-set workflow.human_verify_mode end-of-phase', tmpDir);

    const setResult = runSddTools('config-set workflow.human_verify_mode mid-flight', tmpDir);
    assert.ok(setResult.success, `config-set failed: ${setResult.error}`);
  ...
  test('rejects invalid mode values', () => {
    const result = runSddTools('config-set workflow.human_verify_mode midflight', tmpDir);
...
describe('agents/sdd-planner.md acknowledges workflow.human_verify_mode', () => {
  let plannerSrc;

  test('loads', () => {
    plannerSrc = fs.readFileSync(path.join(REPO_ROOT, 'agents', 'sdd-planner.md'), 'utf-8');
    assert.ok(plannerSrc.length > 0);
  });

  test('mentions workflow.human_verify_mode by canonical name', () => {
    plannerSrc = plannerSrc || fs.readFileSync(path.join(REPO_ROOT, 'agents', 'sdd-planner.md'), 'utf-8');
    assert.ok(
...
describe('agents/sdd-verifier.md harvests deferred human verification items', () => {
  test('Step 8 mentions harvesting <verify><human-check> blocks from PLAN.md', () => {
    const verifierSrc = fs.readFileSync(path.join(REPO_ROOT, 'agents', 'sdd-verifier.md'), 'utf-8');
    assert.ok(
...
describe('references/checkpoints.md documents the flag', () => {
  test('mentions workflow.human_verify_mode in the human-verify section', () => {
    const refSrc = fs.readFileSync(
      path.join(REPO_ROOT, 'sdd', 'references', 'checkpoints.md'),
      'utf-8',
    );
    assert.ok(
