 ...
 *
 * `npx @bhargavvc/sdd-cc@latest` can refresh runtime files while an older
 * global `sdd-sdk` earlier on PATH remains the executable workflows call.
 * The installer must not report SDK readiness when the resolved `sdd-sdk`
...
const { installSdkIfNeeded, readSddSdkVersion } = require('../bin/install.js');
...
describe('bug #3359: installer detects stale sdd-sdk earlier on PATH', () => {
  let tmpRoot;
  let sdkDir;
  let pathDir;
  let homeDir;
  ...
  beforeEach(() => {
    tmpRoot = createTempDir('sdd-3359-');
  ...
  afterEach(() => {
    ...
    cleanup(tmpRoot);
	  ...
	  test('does not print ready when resolved sdd-sdk version differs from installer package version', () => {
    const staleSdk = path.join(pathDir, 'sdd-sdk');
    fs.writeFileSync(
      staleSdk,
      '#!/bin/sh\nprintf "%s\\n" "sdd-sdk v0.0.1"\n',
      { mode: 0o755 },
    ...
    assert.ok(
      ...
      `installer must warn that resolved sdd-sdk is stale. Output:\n${combined}`,
    );
    assert.ok(
      combined.includes(staleSdk),
      `warning must include resolved sdd-sdk path. Output:\n${combined}`,
    );
    assert.ok(
    ...
    assert.ok(
      ...
      `installer must not report ready while PATH resolves a stale sdd-sdk. Output:\n${combined}`,
    );
	  });

	  test('prints ready when no stale sdd-sdk is on PATH', () => {
	    const currentSdk = path.join(pathDir, 'sdd-sdk');
	    fs.writeFileSync(
	      currentSdk,
	      `#!/bin/sh\nprintf "%s\\n" "sdd-sdk v${pkg.version}"\n`,
	      { mode: 0o755 },
	    );
	    ...
	    assert.ok(
	      /SDD SDK ready/.test(combined),
	      `installer must report ready when no stale sdd-sdk exists. Output:\n${combined}`,
	    );
	  });
	  ...
	  test('reads Windows cmd shim versions through cmd.exe', () => {
	    ...
	    cp.spawnSync = (command, args, options) => {
	      ...
	      return { status: 0, stdout: `sdd-sdk v${pkg.version}\n`, stderr: '' };
	    };
	    Object.defineProperty(process, 'platform', { value: 'win32' });

	    try {
	      assert.equal(readSddSdkVersion('C:\\tools\\sdd-sdk.cmd'), pkg.version);
	    } finally {
	    ...
	    assert.deepEqual(calls.map(({ command, args }) => ({ command, args })), [{
	      command: 'cmd.exe',
	      args: ['/c', 'C:\\tools\\sdd-sdk.cmd', '--version'],
	    }]);
