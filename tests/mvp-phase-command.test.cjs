/**
 * /sdd mvp-phase command — frontmatter contract test
...
const CMD = path.join(__dirname, '..', 'commands', 'sdd', 'mvp-phase.md');
...
describe('/sdd mvp-phase command frontmatter', () => {
  ...
  test('frontmatter declares correct command name', () => {
    ...
    assert.equal(contract.name, 'sdd:mvp-phase');
  });
