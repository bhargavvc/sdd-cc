...
/**
 * Bug #2410 — /sdd:manager background execute-phase Task fails with
...
describe('bug #2410: checkpoint heartbeat format is user-documented', () => {
  ...
  test('COMMANDS.md documents the [checkpoint] format under /sdd-manager', () => {
    const managerIdx = commandsDoc.indexOf('### `/sdd-manager`');
    assert.ok(managerIdx !== -1, '/sdd-manager section should exist');
    const section = commandsDoc.slice(managerIdx, managerIdx + 4000);
    assert.ok(
      /\[checkpoint\]/.test(section),
      'COMMANDS.md /sdd-manager section should document [checkpoint] heartbeat markers'
    );
    assert.ok(
