...
//
// PR #2824 consolidated add-backlog into `sdd-capture --backlog` by creating
// a routing wrapper in commands/sdd/capture.md that delegates to
...
//
// Fix: create sdd/workflows/add-backlog.md with the full process
// ported from the deleted commands/sdd/add-backlog.md (git ref 87917131^).
//
// Also adds a broad regression: every @-reference in any commands/sdd/*.md
...
const WORKFLOW = path.join(ROOT, 'sdd', 'workflows', 'add-backlog.md');
const COMMANDS_DIR = path.join(ROOT, 'commands', 'sdd');
const WORKFLOWS_DIR = path.join(ROOT, 'sdd', 'workflows');

// ─── #3135: add-backlog workflow ─────────────────────────────────────────────

describe('#3135: sdd/workflows/add-backlog.md', () => {
  test('file exists', () => {
    assert.ok(
      fs.existsSync(WORKFLOW),
      'sdd/workflows/add-backlog.md does not exist — capture --backlog has no implementation to load',
    );
  });

  test('uses sdd-sdk query phase.next-decimal to find next 999.x slot', () => {
    const src = fs.readFileSync(WORKFLOW, 'utf8');
    assert.ok(
      src.includes('phase.next-decimal'),
      'add-backlog.md must use sdd-sdk query phase.next-decimal to find the next 999.x number',
    );
  });
  ...
  test('uses generate-slug for the directory name', () => {
    ...
    assert.ok(
      ...
      'add-backlog.md must use sdd-sdk query generate-slug to build the phase directory slug',
    );
  });

  test('commits via sdd-sdk query commit', () => {
    const src = fs.readFileSync(WORKFLOW, 'utf8');
    assert.ok(
      src.includes('sdd-sdk query commit') || src.includes('query commit'),
      'add-backlog.md must commit via sdd-sdk query commit',
    );
  });
  ...
  test('documents /sdd-review-backlog for promotion', () => {
    const src = fs.readFileSync(WORKFLOW, 'utf8');
    assert.ok(
      src.includes('review-backlog') || src.includes('sdd-review-backlog'),
      'add-backlog.md should mention /sdd-review-backlog for promoting items to active milestone',
    );
  });
});
  ...
  function executionContextIncludes(body) {
    ...
    for (const blk of blocks) {
      for (const line of blk.split('\n')) {
        ...
        const rel = t.replace(/^@~?\/?(?:\.claude\/)?(?:sdd\/)?/, '');
        targets.push(rel);
      }
    }
    return targets;
...
describe('regression: every execution_context @-reference in commands/sdd/*.md resolves to an existing workflow file', () => {
  // Extract @-references from execution_context blocks, normalised to the
  // sdd/workflows/ relative tail so we can resolve them on disk.
  function extractWorkflowRefs(filePath) {
    ...
    for (const blk of blocks) {
      for (const line of blk.split('\n')) {
        ...
        // Normalise: drop everything up to and including 'sdd/'
        const match = t.match(/sdd\/(workflows\/.+\.md)/);
  ...
  for (const cmdFile of commandFiles) {
    ...
    for (const ref of refs) {
      test(`${cmdName}: @-ref '${ref}' exists on disk`, () => {
        const absPath = path.join(ROOT, 'sdd', ref);
        assert.ok(
          fs.existsSync(absPath),
          `${cmdName} references @${ref} in execution_context but sdd/${ref} does not exist`,
        );
      });
    }
  }
