...
test('collapses first-time baseline report rows without hiding destructive actions', () => {
  const blockedAction = {
    type: 'prompt-user',
    relPath: 'hooks/sdd-ambiguous.js',
    reason: 'needs a user choice',
  };
  const result = {
    blocked: [blockedAction],
...
test('throws when installer migrations require user choice', () => {
  ...
  assert.match(captured.message, /hooks\/sdd-retired-hook\.js/);
  assert.match(captured.message, /SDD_INSTALLER_MIGRATION_RESOLVE/);
  assert.ok(captured.blockedByReason, 'error exposes grouped-by-reason data');
  assert.equal(captured.resolutionEnvVar, 'SDD_INSTALLER_MIGRATION_RESOLVE');
});
