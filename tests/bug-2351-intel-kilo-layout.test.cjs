 ...
 *
 * sdd-intel-updater used hardcoded canonical paths (`agents/*.md`,
 * `commands/sdd/*.md`, `hooks/*.js`, etc.) that assumed the standard
 * `.claude/` runtime layout. Under a `.kilo` install, the runtime root is
 * `.kilo/`, and the command directory is `command/` (not `commands/sdd/`).
...
const AGENT_PATH = path.join(__dirname, '..', 'agents', 'sdd-intel-updater.md');
...
describe('bug #2351: intel updater kilo layout support', () => {
  ...
  test('scope section includes layout detection step', () => {
    ...
    assert.ok(
      hasDetection,
      'sdd-intel-updater.md must instruct the agent to detect the runtime layout ' +
  ...
  test('scope section maps .kilo/command path (not commands/sdd)', () => {
    content = content || fs.readFileSync(AGENT_PATH, 'utf-8');
    assert.ok(
      content.includes('.kilo/command'),
      'scope section must include .kilo/command path — kilo uses "command/" not "commands/sdd/"'
    );
  });
