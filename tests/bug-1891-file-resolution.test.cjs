...
// sdd-tools.cjs @file: resolution is a low-level stdout interception that cannot be
// exercised end-to-end via runSddTools without a real workflow that emits @file: output.
 ...
 *
 * sdd-tools.cjs must transparently resolve @file: references in stdout
...
const SDD_TOOLS_SRC = path.join(__dirname, '..', 'sdd', 'bin', 'sdd-tools.cjs');

describe('bug #1891: @file: resolution in sdd-tools.cjs', () => {
  let src;

  before(() => {
    src = fs.readFileSync(SDD_TOOLS_SRC, 'utf-8');
  });
