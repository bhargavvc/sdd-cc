...
const { CLUSTERS, allClusteredSkills } = require('../sdd/bin/lib/clusters.cjs');
...
describe('CLUSTERS data structure', () => {
  ...
  test('every cluster member is a real skill stem in commands/sdd/', () => {
    ...
    for (const [cluster, members] of Object.entries(CLUSTERS)) {
      for (const stem of members) {
        if (!realStems.has(stem)) {
          mismatches.push(`${cluster}: "${stem}" not found in commands/sdd/`);
        }
      }
    }
  ...
  test('union of all clusters covers every skill in commands/sdd/', () => {
    const realStems = realSkillStems();
