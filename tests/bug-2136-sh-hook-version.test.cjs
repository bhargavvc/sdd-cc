 ...
 *
 * Root cause: three bash hooks (sdd-phase-boundary.sh, sdd-session-state.sh,
 * sdd-validate-commit.sh) shipped without a sdd-hook-version header, and the
 * stale-hook detector in sdd-check-update.js only matched JavaScript comment
 * syntax (//) — not bash comment syntax (#).
 *
 * Result: every session showed "⚠ stale hooks — run /sdd-update" immediately
 ...
 *   1. Bash hooks ship with "# sdd-hook-version: {{SDD_VERSION}}"
 *   2. install.js substitutes {{SDD_VERSION}} in .sh files at install time
 *   3. sdd-check-update.js regex matches both "//" and "#" comment styles
 *
 * Neither fix alone is sufficient:
 *   - Headers + regex fix only (no install.js fix): installed hooks contain
 *     literal "{{SDD_VERSION}}" — the {{-guard silently skips them, making
...
// NOTE: Do NOT set SDD_TEST_MODE here — the E2E install tests spawn the
// real installer subprocess, which skips all install logic when SDD_TEST_MODE=1.
...
const CHECK_UPDATE_FILE = path.join(HOOKS_DIR, 'sdd-check-update.js');
const WORKER_FILE = path.join(HOOKS_DIR, 'sdd-check-update-worker.js');
...
describe('bug #2136 part 1: bash hook sources carry sdd-hook-version placeholder', () => {
  for (const sh of SH_HOOKS) {
    test(`${sh} contains "# sdd-hook-version: {{SDD_VERSION}}"`, () => {
      const content = fs.readFileSync(path.join(HOOKS_DIR, sh), 'utf8');
      assert.ok(
        content.includes('# sdd-hook-version: {{SDD_VERSION}}'),
        `${sh} must include "# sdd-hook-version: {{SDD_VERSION}}" so the ` +
        `installer can stamp it and sdd-check-update.js can detect staleness`
      );
    });
  }
  ...
  test('version header is on line 2 (immediately after shebang)', () => {
    ...
    for (const sh of SH_HOOKS) {
      ...
      assert.ok(
        lines[1].startsWith('# sdd-hook-version:'),
        `${sh} line 2 must be the sdd-hook-version header (got: "${lines[1]}")`
      );
    }
  });
});
...
// ─────────────────────────────────────────────────────────────────────────────
// Part 2: sdd-check-update-worker.js regex handles bash "#" comment syntax
...
describe('bug #2136 part 2: stale-hook detector handles bash comment syntax', () => {
  ...
  test('version regex in source matches "#" comment syntax in addition to "//"', () => {
    ...
    assert.ok(
      hasBashAlternative,
      'sdd-check-update-worker.js version regex must include an alternative for bash "#" comments. ' +
  ...
  test('version regex does not use the old JS-only form as the sole pattern', () => {
    ...
    //   /\\/\\/ sdd-hook-version:\\s*(.+)/
    // which, when evaluated in the subprocess, produced: /\/\/ sdd-hook-version:\s*(.+)/
    // That only matched JS "//" comments — never bash "#".
    // We verify that the old exact string no longer appears.
    assert.ok(
      !src.includes('\\/\\/ sdd-hook-version'),
      'sdd-check-update-worker.js must not use the old JS-only (\\/\\/ sdd-hook-version) ' +
  ...
  test('version regex correctly matches both bash and JS hook version headers', () => {
    ...
    assert.ok(
      src.includes('sdd-hook-version'),
      'sdd-check-update-worker.js must contain a sdd-hook-version version check'
    );

    // The fixed regex that must be present: matches both comment styles
    const fixedRegex = /(?:\/\/|#) sdd-hook-version:\s*(.+)/;

    assert.ok(
      fixedRegex.test('# sdd-hook-version: 1.36.0'),
      'bash-style "# sdd-hook-version: X" must be matchable by the required regex'
    );
    assert.ok(
      fixedRegex.test('// sdd-hook-version: 1.36.0'),
      'JS-style "// sdd-hook-version: X" must still match (no regression)'
    );
    assert.ok(
      !fixedRegex.test('sdd-hook-version: 1.36.0'),
...
// ─────────────────────────────────────────────────────────────────────────────
// Part 3a: install.js bundled path substitutes {{SDD_VERSION}} in .sh hooks
// ─────────────────────────────────────────────────────────────────────────────

describe('bug #2136 part 3a: install.js bundled path substitutes {{SDD_VERSION}} in .sh hooks', () => {
  let src;
  ...
  test('.sh branch in bundled hook copy loop reads file and substitutes SDD_VERSION', () => {
    ...
    assert.ok(
      region.includes('SDD_VERSION'),
      'bundled .sh branch must reference SDD_VERSION substitution. Without this, ' +
      'installed .sh hooks contain the literal "{{SDD_VERSION}}" placeholder and ' +
...
// ─────────────────────────────────────────────────────────────────────────────
// Part 3b: install.js Codex path also substitutes {{SDD_VERSION}} in .sh hooks
// ─────────────────────────────────────────────────────────────────────────────

describe('bug #2136 part 3b: install.js Codex path substitutes {{SDD_VERSION}} in .sh hooks', () => {
  let src;
  ...
  test('.sh branch in Codex hook copy block substitutes SDD_VERSION', () => {
    ...
    assert.ok(
      region.includes('SDD_VERSION'),
      'Codex .sh branch must substitute {{SDD_VERSION}}. The bundled path was fixed ' +
...
describe('bug #2136 part 4: installed .sh hooks contain stamped concrete version', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('sdd-2136-install-');
  });
  ...
  test('installed .sh hooks contain a concrete version string, not the template placeholder', () => {
    ...
    for (const sh of SH_HOOKS) {
      ...
      assert.ok(
        content.includes('# sdd-hook-version:'),
        `installed ${sh} must contain a "# sdd-hook-version:" header`
      );
      assert.ok(
        !content.includes('{{SDD_VERSION}}'),
        `installed ${sh} must not contain literal "{{SDD_VERSION}}" — ` +
        `install.js must substitute it with the concrete package version`
      );

      const versionMatch = content.match(/# sdd-hook-version:\s*(\S+)/);
  ...
  test('stale-hook detector reports zero stale bash hooks immediately after fresh install', () => {
    ...
    // version-check logic (extracted from sdd-check-update.js) against the
    ...
    const checkScript = `
      ...
      // Use the same regex that the fixed sdd-check-update.js uses
      const versionRe = /(?:\\/\\/|#) sdd-hook-version:\\s*(.+)/;
