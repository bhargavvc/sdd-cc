 ...
 *
 * Workflow definitions in `sdd/workflows/*.md` are loaded verbatim
 * into Claude's context every time the corresponding `/sdd:*` command is
 ...
 * that belongs in `sdd/references/` or a per-mode subdirectory
  ...
  for (const workflow of ALL_WORKFLOWS) {
    ...
    test(`${workflow} (${tier}) stays under ${limit} lines`, () => {
      ...
      assert.ok(
        ...
        `to sdd/references/. See workflows/discuss-phase/ for the pattern.`
      );
    });
  }
});
  ...
  test('mode files exist for every documented mode', () => {
    ...
    for (const mode of expected) {
      ...
      assert.ok(
        ...
        `Each --flag in commands/sdd/discuss-phase.md must have a matching mode file.`
      );
    }
  });
