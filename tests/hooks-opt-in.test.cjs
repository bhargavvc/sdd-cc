// Migrated to typed-IR (#2974): the sdd-session-state.sh and
// sdd-phase-boundary.sh hooks now emit Claude Code SessionStart/PostToolUse
...
// plain text. sdd-validate-commit.sh already emitted JSON ({ decision,
// reason }). Tests parse the JSON and assert on typed fields.

/**
 * SDD Tools Tests - Community Hooks (opt-in)
 *
...
function createTempProject(prefix = 'sdd-hook-test-') {
...
describe('hook file validation', () => {
  test('sdd-session-state.sh exists', () => {
    const hookPath = path.join(HOOKS_DIR, 'sdd-session-state.sh');
    assert.ok(fs.existsSync(hookPath), 'sdd-session-state.sh should exist');
  });

  test('sdd-validate-commit.sh exists', () => {
    const hookPath = path.join(HOOKS_DIR, 'sdd-validate-commit.sh');
    assert.ok(fs.existsSync(hookPath), 'sdd-validate-commit.sh should exist');
  });

  test('sdd-phase-boundary.sh exists', () => {
    const hookPath = path.join(HOOKS_DIR, 'sdd-phase-boundary.sh');
    assert.ok(fs.existsSync(hookPath), 'sdd-phase-boundary.sh should exist');
  });

  test('sdd-session-state.sh is executable', { skip: isWindows ? 'Windows has no POSIX file permissions' : false }, () => {
    const hookPath = path.join(HOOKS_DIR, 'sdd-session-state.sh');
    const stat = fs.statSync(hookPath);
    assert.ok((stat.mode & 0o111) !== 0, 'sdd-session-state.sh should be executable');
  });

  test('sdd-validate-commit.sh is executable', { skip: isWindows ? 'Windows has no POSIX file permissions' : false }, () => {
    const hookPath = path.join(HOOKS_DIR, 'sdd-validate-commit.sh');
    const stat = fs.statSync(hookPath);
    assert.ok((stat.mode & 0o111) !== 0, 'sdd-validate-commit.sh should be executable');
  });

  test('sdd-phase-boundary.sh is executable', { skip: isWindows ? 'Windows has no POSIX file permissions' : false }, () => {
    const hookPath = path.join(HOOKS_DIR, 'sdd-phase-boundary.sh');
    const stat = fs.statSync(hookPath);
    assert.ok((stat.mode & 0o111) !== 0, 'sdd-phase-boundary.sh should be executable');
  });
});
...
describe('installer hook registration', () => {
  ...
  test('install.js contains sdd-validate-commit registration block', () => {
    assert.ok(
      installSource.includes('sdd-validate-commit'),
      'install.js should contain sdd-validate-commit hook registration'
    );
    assert.ok(
  ...
  test('install.js contains sdd-session-state registration block', () => {
    assert.ok(
      installSource.includes('sdd-session-state'),
      'install.js should contain sdd-session-state hook registration'
    );
    assert.ok(
  ...
  test('install.js contains sdd-phase-boundary registration block', () => {
    assert.ok(
      installSource.includes('sdd-phase-boundary'),
      'install.js should contain sdd-phase-boundary hook registration'
    );
    assert.ok(
...
describe('opt-in gating behavior', { skip: isWindows ? 'bash hooks require unix shell' : false }, () => {
  ...
  test('validate-commit is a no-op when config.json is absent', (t) => {
    ...
    const bareDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-hook-bare-'));
...
describe('hook execution when enabled', { skip: isWindows ? 'bash hooks require unix shell' : false }, () => {
  ...
  test('session-state exits 0 without .planning/ (in enabled project)', (t) => {
    ...
    const noStateDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-hook-nostate-'));
