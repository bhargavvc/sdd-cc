...
describe('CR-CONFIG: config key registration', () => {
  test('config-set accepts workflow.code_review', () => {
    ...
    try {
      const result = runSddTools('config-set workflow.code_review true', tmpDir);
  ...
  test('config-set accepts workflow.code_review_depth', () => {
    const tmpDir = createTempProject();
    try {
      const result = runSddTools('config-set workflow.code_review_depth standard', tmpDir);
  ...
  test('config-get workflow.code_review returns value set via config-set', (t) => {
    ...
    const setResult = runSddTools(['config-set', 'workflow.code_review', 'true'], tmpDir);
    assert.ok(setResult.success, `config-set workflow.code_review failed: ${setResult.error}`);

    const getResult = runSddTools(['config-get', 'workflow.code_review'], tmpDir);
  ...
  test('config-get workflow.code_review_depth returns value set via config-set', (t) => {
    ...
    const setResult = runSddTools(['config-set', 'workflow.code_review_depth', 'standard'], tmpDir);
    assert.ok(setResult.success, `config-set workflow.code_review_depth failed: ${setResult.error}`);

    const getResult = runSddTools(['config-get', 'workflow.code_review_depth'], tmpDir);
...
describe('CR-INTEGRATION: workflow integration points', () => {
  ...
  test('autonomous.md contains sdd-code-review skill invocation', () => {
    ...
    // (sdd-code-review); colon form (sdd:code-review) is the legacy
    ...
    assert.ok(skillNames.includes('sdd-code-review'),
      `autonomous.md must invoke Skill(skill="sdd-code-review", ...); found skills: ${JSON.stringify(skillNames)}`);
    assert.ok(!skillNames.includes('sdd:code-review'),
      'autonomous.md must not use legacy colon form sdd:code-review (canonical is hyphen form)');
  });

  test('autonomous.md auto-fix uses consolidated sdd-code-review --fix invocation (#2790)', () => {
    // After #2790, sdd-code-review-fix was absorbed into sdd-code-review as
    // the --fix flag. The autonomous workflow must invoke the consolidated
    // form, not the deleted sdd-code-review-fix skill.
    const content = fs.readFileSync(path.join(WORKFLOWS_DIR, 'autonomous.md'), 'utf-8');

    const invocations = parseWorkflowSkillInvocations(content);
    const skillNames = invocations.map(inv => inv.skill);
    assert.ok(!skillNames.includes('sdd-code-review-fix'),
      `autonomous.md must not invoke deleted sdd-code-review-fix skill (consolidated into --fix); found: ${JSON.stringify(skillNames)}`);
    assert.ok(!skillNames.includes('sdd:code-review-fix'),
      'autonomous.md must not use legacy colon form sdd:code-review-fix');

    // Find a sdd-code-review invocation that carries the --fix flag (the
    // consolidated auto-fix entry point).
    const fixInvocation = invocations.find(inv => {
      if (inv.skill !== 'sdd-code-review') return false;
      const tokens = new Set((inv.args ?? '').split(/\s+/).filter(Boolean));
      return tokens.has('--fix');
    });
    assert.ok(fixInvocation,
      `autonomous.md must invoke Skill(skill="sdd-code-review", args="... --fix ...") for auto-fix; found: ${JSON.stringify(invocations)}`);
  });

  test('autonomous.md contains --auto flag on consolidated --fix invocation (#2790)', () => {
    const content = fs.readFileSync(path.join(WORKFLOWS_DIR, 'autonomous.md'), 'utf-8');

    // Find the sdd-code-review invocation that carries --fix (the consolidated
    ...
    assert.ok(fixInvocation, 'autonomous.md missing Skill(skill="sdd-code-review", args="... --fix ...") invocation');
    const argTokens = new Set((fixInvocation.args ?? '').split(/\s+/).filter(Boolean));
    assert.ok(argTokens.has('--auto'),
      `autonomous.md sdd-code-review-fix args missing --auto flag; got args="${fixInvocation.args}"`);
  });
});
