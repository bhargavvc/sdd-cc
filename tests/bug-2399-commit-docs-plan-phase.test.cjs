...
const PLAN_PHASE_PATH = path.join(__dirname, '..', 'sdd', 'workflows', 'plan-phase.md');

describe('plan-phase commit_docs support (#2399)', () => {
  test('plan-phase.md exists', () => {
    assert.ok(fs.existsSync(PLAN_PHASE_PATH), 'sdd/workflows/plan-phase.md must exist');
  });
  ...
  test('plan-phase.md uses sdd-sdk query commit for the plan commit', () => {
    const content = fs.readFileSync(PLAN_PHASE_PATH, 'utf-8');
    // Must use sdd-sdk query commit (not raw git) so commit_docs guard in sdd-tools is respected
    assert.ok(
      content.includes('sdd-sdk query commit') || content.includes('sdd-tools') || content.includes('sdd-sdk'),
      'plan-phase.md plan commit step must use sdd-sdk query commit (not raw git commit)'
    );
  });
});
