...
/**
 * Bug #2998: sdd-pristine/ snapshot is documented but never populated by
 * the installer. saveLocalPatches declared a pristineDir variable and
 * promised "saves pristine copies (from manifest) to sdd-pristine/ to
 * enable three-way merge during reapply-patches" -- but no code ever
 * wrote to that directory. Effect: the /sdd-reapply-patches Step 5
 ...
 * out the modified-file paths into sdd-pristine/. saveLocalPatches now
...
describe('Bug #2998: populatePristineDir is exported and writes pristine for modified files', () => {
  ...
  test('returns 0 when no files are modified (no-op)', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2998-'));
    try {
      const written = INSTALL.populatePristineDir({
        packageSrc: ROOT,
        pristineDir: path.join(tmp, 'sdd-pristine'),
        modified: [],
        runtime: 'claude',
  ...
  test('writes one pristine file per modified path that exists in source', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2998-'));
    const pristineDir = path.join(tmp, 'sdd-pristine');
    try {
      // Pick a real installed-side relPath from the package source. The
      // install transforms map source `sdd/<rel>` to installed
      // `sdd/<rel>` for skills-aware runtimes (like claude),
      // so the relPath is the same on both sides.
      const candidate = path.join('sdd', 'workflows', 'reapply-patches.md');
  ...
  test('skips paths not present in source (does not corrupt pristine with stale data)', () => {
    ...
    try {
      const written = INSTALL.populatePristineDir({
        packageSrc: ROOT,
        pristineDir,
        modified: ['sdd/this-path-does-not-exist.md'],
        runtime: 'claude',
        pathPrefix: '$HOME/.claude/',
        isGlobal: true,
      });
      assert.equal(written, 0, 'expected zero pristine files for non-existent source paths');
      const out = path.join(pristineDir, 'sdd/this-path-does-not-exist.md');
  ...
  test('pristine files have stable content (transformations are deterministic)', () => {
    ...
    const tmp1 = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2998-d1-'));
    const tmp2 = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2998-d2-'));
    try {
      const candidate = path.join('sdd', 'workflows', 'reapply-patches.md');
      const ctx = {
        packageSrc: ROOT,
      ...
      INSTALL.populatePristineDir(Object.assign({ pristineDir: path.join(tmp1, 'sdd-pristine') }, ctx));
      INSTALL.populatePristineDir(Object.assign({ pristineDir: path.join(tmp2, 'sdd-pristine') }, ctx));
      const a = fs.readFileSync(path.join(tmp1, 'sdd-pristine', candidate));
      const b = fs.readFileSync(path.join(tmp2, 'sdd-pristine', candidate));
...
describe('Bug #2998 (#3004 CR): pristine expansion covers every manifest install root', () => {
  test('paths under agents/ are staged via copyWithPathReplacement, not silently skipped', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2998-multi-'));
    const pristineDir = path.join(tmp, 'sdd-pristine');
    try {
      const candidate = path.join('agents', 'sdd-planner.md');
      const sourcePath = path.join(ROOT, candidate);
      assert.equal(fs.existsSync(sourcePath), true,
        `precondition: source file exists at ${candidate}`);
      const written = INSTALL.populatePristineDir({
  ...
  test('a mix of sdd/ and agents/ paths in modified list are all staged', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2998-mix-'));
    const pristineDir = path.join(tmp, 'sdd-pristine');
    try {
      const a = path.join('sdd', 'workflows', 'reapply-patches.md');
      const b = path.join('agents', 'sdd-planner.md');
