/**
 * Stale /sdd: colon reference detection test
 *
...
/**
 * Determine whether a /sdd: match in a test file is a legitimate test input
 * (i.e., the input string fed to a colon-to-hyphen converter).
 */
function isTestInput(filePath, line) {
  const rel = path.relative(ROOT, filePath).replace(/\\/g, '/');

  // SDK test files (.ts) that test sanitizer stripping of /sdd: patterns
  ...
  // Conversion test files: input strings to convert* functions contain /sdd:
  ...
  if (conversionTestFiles.includes(rel)) {
    ...
    // JSDoc block-comment lines with /sdd: in description are stale
...
describe('No stale /sdd: colon references (#1748)', () => {
  test('all /sdd: references should be hyphenated except test inputs', () => {
    const extensions = new Set(['.md', '.js', '.cjs', '.ts', '.yml', '.sh', '.svg']);
    const files = collectFiles(ROOT, extensions);

    const staleRefs = [];
    const pattern = /\/sdd:[a-z]/g;
    ...
    if (staleRefs.length > 0) {
      assert.fail(
        `Found ${staleRefs.length} stale /sdd: colon reference(s) that should use /sdd- hyphen format:\n${staleRefs.join('\n')}`
      );
    }
  });
});
