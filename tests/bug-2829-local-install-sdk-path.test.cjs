/**
 * Regression test for #2829: `command not found: sdd-sdk` with local-mode install.
 *
 * Repro: a fresh `npx @bhargavvc/sdd-cc@latest` install with the runtime set
 * to local mode left every `sdd-sdk query …` call site unable to resolve the
 ...
 * actually carries `sdk/dist/cli.js` and `bin/sdd-sdk.js` regardless of mode,
 ...
 * `sdd-sdk` is on PATH and self-link the shim into a user-writable PATH dir
 * if not, so `/sdd-plan` and friends can call `sdd-sdk query …` directly.
 *
...
describe('bug #2829: local-mode install must materialize sdd-sdk on PATH', () => {
  let tmpRoot;
  let sdkDir;
  let pathDir;
  let homeDir;
  ...
  beforeEach(() => {
    tmpRoot = createTempDir('sdd-2829-');
  ...
  test('isLocal=true with sdk/dist/cli.js present self-links sdd-sdk into an on-PATH HOME bin dir', () => {
    ...
    // The shim must be materialized so `sdd-sdk query …` resolves.
    const linkPath = path.join(localBin, 'sdd-sdk');
    assert.ok(
      fs.existsSync(linkPath),
      `local install must self-link sdd-sdk into ${linkPath}. Output:\n${combined}`,
    );
    // And the installer must report ready (matches the global-mode UX).
    assert.ok(
      /SDD SDK ready/.test(combined),
      `local install must print "SDD SDK ready" once sdd-sdk is on PATH. Output:\n${combined}`,
    );
  ...
  test('isLocal=true with dist present and no on-PATH HOME bin still warns rather than lying about readiness', () => {
    ...
    // probe still cannot find sdd-sdk on PATH.
    const { stdout, stderr } = captureConsole(() => {
      installSdkIfNeeded({ sdkDir, isLocal: true });
    });
    const combined = `${stdout}\n${stderr}`;
    assert.ok(
      !/SDD SDK ready/.test(combined),
      `installer must not falsely claim ready when sdd-sdk is not callable. Output:\n${combined}`,
    );
    assert.ok(
