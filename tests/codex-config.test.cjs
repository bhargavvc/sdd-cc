...
describe('Codex install hook configuration (e2e)', () => {
  ...
  test('Codex install copies hook file that is referenced in config.toml (#2153)', () => {
    ...
    assert.ok(configContent.includes('sdd-check-update.js'), 'config.toml references sdd-check-update.js');
  ...
  test('fresh CODEX_HOME enables codex_hooks without draft root defaults', () => {
    ...
    assert.strictEqual(
      ...
      expectedCommand,
      'handler command must use absolute node runner pointing at sdd-check-update.js (#3017)'
    );
    ...
    assert.strictEqual(countMatches(content, /sdd-check-update\.js/g), 1, 'writes one SDD update hook');
  ...
  test('existing LF config without [features] gets one features block and preserves user content', () => {
    ...
    assert.strictEqual(countMatches(content, /sdd-check-update\.js/g), 1, 'adds one SDD update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing empty [features] and codex_hooks = false are normalized and remain idempotent', () => {
    ...
    assert.ok(content.includes('command = "echo custom"'), 'preserves custom hook');
    assert.strictEqual(countMatches(content, /sdd-check-update\.js/g), 1, 'does not duplicate SDD update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted [features] headers are recognized as the existing features table', () => {
    ...
    assert.strictEqual(countMatches(content, /sdd-check-update\.js/g), 1, 'keeps one SDD update hook');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted table headers containing # are parsed without treating # as a comment start', () => {
    ...
    assert.strictEqual(countMatches(content, /^codex_hooks = true$/gm), 1, 'adds one codex_hooks key');
    assert.strictEqual(countMatches(content, /sdd-check-update\.js/g), 1, 'remains idempotent for the SDD hook block');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing dotted features config stays dotted and does not grow a [features] table', () => {
    ...
    assert.strictEqual(countMatches(content, /sdd-check-update\.js/g), 1, 'adds one SDD update hook for dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted dotted codex_hooks keys stay dotted and are normalized without duplication', () => {
    ...
    assert.strictEqual(countMatches(content, /sdd-check-update\.js/g), 1, 'adds one SDD update hook for quoted dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('codex_hooks = true with an inline comment is treated as enabled for hook installation', () => {
    ...
    assert.ok(content.includes('other_feature = true'), 'preserves other feature keys');
    assert.strictEqual(countMatches(content, /sdd-check-update\.js/g), 1, 'adds the SDD update hook once');
    assertNoDraftRootKeys(content);
  });
  ...
  test('mixed-EOL configs use the first newline style for inserted Codex content', () => {
    ...
    assert.strictEqual(countMatches(content, /sdd-check-update\.js/g), 1, 'does not duplicate the SDD hook block');
    assertNoDraftRootKeys(content);
  });
});
