...
describe('generateCodexConfigBlock', () => {
  ...
  test('does not include feature flags or agents table header', () => {
    ...
    // Should not have bare [agents] table header (only [agents.sdd-*] sections)
  ...
  test('includes per-agent sections with relative paths (no targetDir)', () => {
    const result = generateCodexConfigBlock(agents);
    assert.ok(result.includes('[agents.sdd-executor]'), 'has executor section');
    assert.ok(result.includes('[agents.sdd-planner]'), 'has planner section');
...
describe('mergeCodexConfig', () => {
  ...
  test('case 1: creates new config.toml', () => {
    ...
    assert.ok(content.includes('[agents.sdd-executor]'), 'has agent');
  ...
  test('case 2: replaces existing SDD block', () => {
    ...
    assert.ok(content.includes('[agents.sdd-planner]'), 'has new agent');
  ...
  test('case 3 strips existing [agents.sdd-*] sections before appending fresh block', () => {
    ...
    const content = fs.readFileSync(configPath, 'utf8');
    const sddAgentCount = (content.match(/^\[agents\.sdd-executor\]\s*$/gm) || []).length;
    const markerCount = (content.match(new RegExp(SDD_CODEX_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;

    assert.ok(content.includes('[model]'), 'preserves user content');
    assert.ok(content.includes('[agents.custom-agent]'), 'preserves non-SDD agent section');
    assert.strictEqual(sddAgentCount, 1, 'keeps exactly one SDD agent section');
  ...
  test('case 2 does not inject feature keys', () => {
    ...
    assert.ok(content.includes('[agents.sdd-executor]'), 'has agent from fresh block');
  });
  ...
  test('case 2 strips leaked SDD-managed sections above marker in CRLF files', () => {
    ...
    assert.strictEqual(countMatches(content, /^\[agents\.sdd-executor\]\s*$/gm), 1, 'keeps one managed agent section');
    assertUsesOnlyEol(content, '\r\n');
  });

  test('case 2 preserves user-authored [agents] tables while stripping leaked SDD sections in CRLF files', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
      'child_agents_md = false',
    ...
    assert.strictEqual(countMatches(beforeMarker, /^\[agents\.sdd-executor\]\s*$/gm), 0, 'removes leaked SDD agent section above marker');
    assert.strictEqual(countMatches(content, /^\[agents\.sdd-executor\]\s*$/gm), 1, 'keeps one managed agent section in the SDD block');
    assertUsesOnlyEol(content, '\r\n');
  });
...
describe('installCodexConfig (integration)', () => {
  ...
  (hasAgents ? test : test.skip)('generates config.toml and agent .toml files', () => {
    ...
    assert.ok(config.includes('[agents.sdd-executor]'), 'has executor agent');
...
describe('Codex install hook configuration (e2e)', () => {
  ...
  test('fresh CODEX_HOME enables codex_hooks without draft root defaults', () => {
    ...
    assert.ok(content.includes('# SDD Hooks\n[[hooks]]\nevent = "SessionStart"\n'), 'writes SDD SessionStart hook block');
  ...
  test('existing [features] at EOF without trailing newline is updated in place', () => {
    ...
    assert.ok(content.indexOf('codex_hooks = true') < content.indexOf('[agents.sdd-codebase-mapper]'), 'keeps codex_hooks before the next real table');
    assertNoDraftRootKeys(content);
  });
  ...
  test('root inline-table features assignments are left untouched without appending invalid dotted keys or hooks', () => {
    ...
    assert.ok(content.includes('[agents.sdd-executor]'), 'still installs the managed agent block');
    assertNoDraftRootKeys(content);
  });
  ...
  test('mixed-EOL configs use the first newline style for inserted Codex content', () => {
    ...
    assert.ok(content.includes('# SDD Hooks\n[[hooks]]\nevent = "SessionStart"\n'), 'writes the SDD hook block using the first newline style');
