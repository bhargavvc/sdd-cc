...
/**
 * SDD Tools Tests - codex-config.cjs
 *
...
describe('getCodexSkillAdapterHeader', () => {
  test('contains all three sections', () => {
    const result = getCodexSkillAdapterHeader('sdd-execute-phase');
  ...
  test('includes correct invocation syntax', () => {
    const result = getCodexSkillAdapterHeader('sdd-plan-phase');
    assert.ok(result.includes('`$sdd-plan-phase`'), 'has $skillName invocation');
    assert.ok(result.includes('{{SDD_ARGS}}'), 'has SDD_ARGS variable');
  });

  test('section B maps AskUserQuestion parameters', () => {
    const result = getCodexSkillAdapterHeader('sdd-discuss-phase');
  ...
  test('section C maps Task to spawn_agent', () => {
    ...
    assert.match(
      result,
      /Resolved `reasoning_effort="low\|medium\|high\|xhigh"` \(`xhigh` is a SDD\/Codex tier, not a generic runtime enum\) → pass `reasoning_effort`\s+to `spawn_agent` when the runtime\/tool supports it/,
...
You are a SDD plan executor.
    ...
    assert.ok(result.includes('"sdd-executor"'), 'has quoted name');
    assert.ok(result.includes('"Executes SDD plans with atomic commits"'), 'has quoted description');
    ...
    assert.ok(result.includes('role: sdd-executor'), 'role matches agent name');
    assert.ok(result.includes('tools: Read, Write, Edit, Bash, Grep, Glob'), 'tools in role block');
    assert.ok(result.includes('purpose: Executes SDD plans with atomic commits'), 'purpose from description');
...
Run /sdd:execute-phase to proceed.`;

    const result = convertClaudeAgentToCodexAgent(input);
    assert.ok(result.includes('$sdd-execute-phase'), 'converts slash commands');
    assert.ok(!result.includes('/sdd:execute-phase'), 'original slash command removed');
  });
...
INIT=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" state load)
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" commit "docs: resolve"`;
...
<role>You are an executor.</role>`;
  ...
  test('sets workspace-write for executor', () => {
    const result = generateCodexAgentToml('sdd-executor', sampleAgent);
...
<role>You check plans.</role>`;
    const result = generateCodexAgentToml('sdd-plan-checker', checker);
  ...
  test('includes required name and description fields', () => {
    const result = generateCodexAgentToml('sdd-executor', sampleAgent);
    assert.ok(result.includes('name = "sdd-executor"'), 'has name');
  ...
  test('falls back to generated description when frontmatter is missing fields', () => {
    ...
    const result = generateCodexAgentToml('sdd-unknown', minimalAgent);
    assert.ok(result.includes('name = "sdd-unknown"'), 'falls back to agent name');
    assert.ok(result.includes('description = "SDD agent sdd-unknown"'), 'falls back to synthetic description');
  });

  test('defaults unknown agents to read-only', () => {
    const result = generateCodexAgentToml('sdd-unknown', sampleAgent);
  ...
  test('emits model field when modelOverrides contains an entry for the agent (#2256)', () => {
    const overrides = { 'sdd-executor': 'gpt-5.3-codex' };
    const result = generateCodexAgentToml('sdd-executor', sampleAgent, overrides);
  ...
  test('does not emit model field when modelOverrides is null (#2256)', () => {
    const result = generateCodexAgentToml('sdd-executor', sampleAgent, null);
  ...
  test('does not emit model field when modelOverrides has no entry for this agent (#2256)', () => {
    const overrides = { 'sdd-planner': 'gpt-5.4' };
    const result = generateCodexAgentToml('sdd-executor', sampleAgent, overrides);
...
describe('CODEX_AGENT_SANDBOX', () => {
  ...
  test('workspace-write agents have write tools', () => {
    const writeAgents = [
      'sdd-executor', 'sdd-planner', 'sdd-phase-researcher',
      'sdd-project-researcher', 'sdd-research-synthesizer', 'sdd-verifier',
      'sdd-codebase-mapper', 'sdd-roadmapper', 'sdd-debugger',
    ];
  ...
  test('read-only agents have no write tools', () => {
    const readOnlyAgents = ['sdd-plan-checker', 'sdd-integration-checker'];
...
describe('generateCodexConfigBlock', () => {
  const agents = [
    { name: 'sdd-executor', description: 'Executes plans' },
    { name: 'sdd-planner', description: 'Creates plans' },
  ];

  test('starts with SDD marker', () => {
    const result = generateCodexConfigBlock(agents);
    assert.ok(result.startsWith(SDD_CODEX_MARKER), 'starts with marker');
  });
  ...
  test('#2727: emits [agents.<name>] struct format (Codex 0.120.0+, replaces #2645 [[agents]])', () => {
    ...
    assert.ok(result.includes('[agents.sdd-executor]'), 'executor has struct header');
    assert.ok(result.includes('[agents.sdd-planner]'), 'planner has struct header');
    // Struct format uses the key as the name; no name = field.
    assert.ok(!result.includes('name = "sdd-executor"'), 'no name field in struct format');
    assert.ok(!result.includes('name = "sdd-planner"'), 'no name field in struct format');
  ...
  test('includes per-agent sections with relative paths (no targetDir)', () => {
    const result = generateCodexConfigBlock(agents);
    assert.ok(result.includes('[agents.sdd-executor]'), 'has executor entry');
    assert.ok(result.includes('[agents.sdd-planner]'), 'has planner entry');
    assert.ok(result.includes('config_file = "agents/sdd-executor.toml"'), 'relative config_file without targetDir');
  ...
  test('uses absolute config_file paths when targetDir is provided', () => {
    ...
    assert.ok(result.includes('config_file = "/home/user/.codex/agents/sdd-executor.toml"'), 'absolute executor path');
    assert.ok(result.includes('config_file = "/home/user/.codex/agents/sdd-planner.toml"'), 'absolute planner path');
  ...
  test('#2727: emits [agents.<name>] struct format by default (Codex 0.124.0+)', () => {
    ...
    assert.ok(result.includes('[agents.sdd-executor]'), 'executor uses struct header');
    assert.ok(result.includes('[agents.sdd-planner]'), 'planner uses struct header');
...
describe('stripSddFromCodexConfig', () => {
  ...
  test('preserves user content before marker', () => {
    ...
    assert.ok(!result.includes(SDD_CODEX_MARKER), 'removes marker');
  });
  ...
  test('strips injected keys above marker on uninstall', () => {
    ...
    assert.ok(!result.includes(SDD_CODEX_MARKER), 'strips marker');
  });

  test('removes legacy [agents.sdd-*] map sections (self-heal pre-#2645 configs)', () => {
    const content = `[agents.sdd-executor]\ndescription = "test"\nconfig_file = "agents/sdd-executor.toml"\n\n[agents.custom-agent]\ndescription = "user agent"\n`;
    const result = stripSddFromCodexConfig(content);
    assert.ok(!result.includes('[agents.sdd-executor]'), 'removes legacy SDD agent map section');
    assert.ok(result.includes('[agents.custom-agent]'), 'preserves user agent section');
  });

  test('#2645: removes [[agents]] array-of-tables entries whose name is sdd-*', () => {
    const content = `[[agents]]\nname = "sdd-executor"\ndescription = "test"\nconfig_file = "agents/sdd-executor.toml"\n\n[[agents]]\nname = "custom-agent"\ndescription = "user agent"\n`;
    const result = stripSddFromCodexConfig(content);
    assert.ok(!/name = "sdd-executor"/.test(result), 'removes managed SDD [[agents]] entry');
    assert.ok(result.includes('name = "custom-agent"'), 'preserves user [[agents]] entry');
  });

  test('#2645: handles mixed legacy + new shapes and multiple user/sdd entries in one file', () => {
    // Multiple SDD entries (both legacy map and new array-of-tables) interleaved
    // with multiple user-authored agents in both shapes — none of the user
    // entries may be removed and all SDD entries must be stripped.
    const content = [
      '[agents.sdd-executor]',
      'description = "legacy sdd"',
      'config_file = "agents/sdd-executor.toml"',
      '',
    ...
    const result = stripSddFromCodexConfig(content);
    // All SDD entries removed.
    assert.ok(!result.includes('sdd-executor'), 'removes legacy sdd-executor');
    assert.ok(!/name\s*=\s*"sdd-planner"/.test(result), 'removes new sdd-planner');
    assert.ok(!/name\s*=\s*'sdd-debugger'/.test(result), 'removes single-quoted sdd-debugger');
...
describe('migrateCodexHooksMapFormat', () => {
  test('migrates flat [[hooks]] with event key to namespaced [[hooks.<EVENT>]] form', () => {
    ...
    const content = [
      ...
      '[[hooks]]',
      'event = "SessionStart"',
      'command = "node /home/.codex/hooks/sdd-check-update.js"',
      '',
    ].join('\n');
    ...
    assert.strictEqual(parsed.hooks.SessionStart[0].hooks[0].command,
      'node /home/.codex/hooks/sdd-check-update.js');
  ...
  test('converts [hooks.shell] to namespaced AoT [[hooks.shell]] (#2760 CR5 finding 3)', () => {
    ...
    assert.strictEqual(parsed.hooks.shell[0].hooks[0].command, 'node /home/.codex/hooks/sdd-check-update.js');
  ...
  test('end-to-end: install on config with old [hooks] map format produces namespaced AoT (#2637, #2760 CR5)', () => {
    // Simulates the exact old SDD config.toml format that broke on Codex 0.124.0
    const oldContent = [
      '[features]',
      'codex_hooks = true',
      '',
...
describe('Codex hooks emit: migration produces namespaced AoT so managed-emit converges', () => {
  ...
  // install path (writes "# SDD Hooks") detects existing namespaced AoT via
  ...
  let tmpDir;
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-codex-fieldparity-'));
  });
  afterEach(() => {
...
describe('mergeCodexConfig', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-codex-merge-'));
  });
  ...
  test('case 1: creates new config.toml', () => {
    ...
    assert.ok(content.includes(SDD_CODEX_MARKER), 'has marker');
    assert.ok(content.includes('[agents.sdd-executor]'), 'has agent in struct format');
    assert.ok(!content.includes('[features]'), 'no features section');
    assert.ok(!content.includes('multi_agent'), 'no multi_agent');
  });

  test('case 2: replaces existing SDD block', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    const newBlock = generateCodexConfigBlock([
      { name: 'sdd-executor', description: 'Updated description' },
      { name: 'sdd-planner', description: 'New agent' },
    ]);
    ...
    assert.ok(content.includes('[agents.sdd-planner]'), 'has new agent in struct format');
    // Verify no duplicate markers
    const markerCount = (content.match(new RegExp(SDD_CODEX_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;
    assert.strictEqual(markerCount, 1, 'exactly one marker');
  });

  test('case 3: appends to config without SDD marker', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    const content = fs.readFileSync(configPath, 'utf8');
    assert.ok(content.includes('[model]'), 'preserves user content');
    assert.ok(content.includes(SDD_CODEX_MARKER), 'adds marker');
    assert.ok(content.includes('[agents.sdd-executor]'), 'has agent in struct format');
  });

  test('case 3 with existing [features]: preserves user features, does not inject SDD keys', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    ...
    assert.ok(content.includes(SDD_CODEX_MARKER), 'adds marker for agents block');
    assert.ok(content.includes('[agents.sdd-executor]'), 'has agent in struct format');
  });

  test('case 3 strips existing [agents.sdd-*] sections before appending fresh block', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const existing = [
      '[model]',
      'name = "o3"',
    ...
    const content = fs.readFileSync(configPath, 'utf8');
    // After merge, SDD block is after the marker. Count [agents.sdd-executor] headers:
    // exactly one should exist (the one in the freshly-written SDD block).
    const sddStructCount = (content.match(/^\[agents\.sdd-executor\]\s*$/gm) || []).length;
    const markerCount = (content.match(new RegExp(SDD_CODEX_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;
    // Struct format does not use name = field
    assert.ok(!content.match(/^name = "sdd-executor"/m), 'no name = field in struct format');

    assert.ok(content.includes('[model]'), 'preserves user content');
    assert.ok(content.includes('[agents.custom-agent]'), 'preserves non-SDD agent section');
    assert.strictEqual(sddStructCount, 1, 'keeps exactly one [agents.sdd-executor] struct entry');
    assert.strictEqual(markerCount, 1, 'adds exactly one marker block');
    assert.ok(!/\n{3,}# SDD Agent Configuration/.test(content), 'does not leave extra blank lines before marker block');
  });
  ...
  test('case 2 does not inject feature keys', () => {
    ...
    assert.ok(content.includes('[agents.sdd-executor]'), 'has agent from fresh block in struct format');
  });

  test('case 2 strips leaked [agents] and [agents.sdd-*] from before content', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
    ...
    assert.ok(!beforeMarker.includes('[agents.sdd-'), 'no leaked [agents.sdd-*] above marker');
  });

  test('case 2 strips leaked SDD-managed sections above marker in CRLF files', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
      'child_agents_md = false',
    ...
    assert.strictEqual(countMatches(beforeMarker, /^\[agents\.sdd-executor\]\s*$/gm), 0, 'removes leaked SDD agent section above marker');
    // New struct format: exactly one [agents.sdd-executor] header in the SDD block (after marker)
    assert.strictEqual(countMatches(content, /^\[agents\.sdd-executor\]\s*$/gm), 1, 'exactly one struct agent header in SDD block');
    assert.strictEqual(countMatches(content, /name = "sdd-executor"/g), 0, 'no name = field in struct format');
    assertUsesOnlyEol(content, '\r\n');
  });

  test('case 2 strips bare [agents] tables (invalid in current Codex schema, #2760) and removes leaked SDD sections in CRLF files', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    const brokenContent = [
      '[features]',
      'child_agents_md = false',
    ...
    // New struct format: exactly one [agents.sdd-executor] in the SDD block (after marker)
    assert.strictEqual(countMatches(content, /^\[agents\.sdd-executor\]\s*$/gm), 1, 'exactly one struct agent header in SDD block');
    assert.strictEqual(countMatches(content, /name = "sdd-executor"/g), 0, 'no name = field in struct format');
    assertUsesOnlyEol(content, '\r\n');
  });
  ...
  test('preserves CRLF when appending SDD block to existing config', () => {
    const configPath = path.join(tmpDir, 'config.toml');
    fs.writeFileSync(configPath, '[model]\r\nname = "o3"\r\n', 'utf8');
    ...
    assert.ok(content.includes(`${SDD_CODEX_MARKER}\r\n`), 'writes marker with CRLF');
    assertUsesOnlyEol(content, '\r\n');
  });

  test('uses the first newline style when appending SDD block to mixed-EOL configs', () => {
    const configPath = path.join(tmpDir, 'config.toml');
...
describe('installCodexConfig (integration)', () => {
  ...
  beforeEach(() => {
    tmpTarget = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-codex-install-'));
  });
  ...
  (hasAgents ? test : test.skip)('generates config.toml and agent .toml files', () => {
    ...
    assert.ok(config.includes(SDD_CODEX_MARKER), 'has SDD marker');
    assert.ok(config.includes('[agents.sdd-executor]'), 'has executor agent in struct format');
    ...
    assert.ok(fs.existsSync(path.join(agentsDir, 'sdd-executor.toml')), 'executor .toml exists');
    assert.ok(fs.existsSync(path.join(agentsDir, 'sdd-plan-checker.toml')), 'plan-checker .toml exists');

    const executorToml = fs.readFileSync(path.join(agentsDir, 'sdd-executor.toml'), 'utf8');
    assert.ok(executorToml.includes('name = "sdd-executor"'), 'executor has name');
    assert.ok(executorToml.includes('description = "Executes SDD plans with atomic commits, deviation handling, checkpoint protocols, and state management. Spawned by execute-phase orchestrator or execute-plan command."'), 'executor has description');
    ...
    const checkerToml = fs.readFileSync(path.join(agentsDir, 'sdd-plan-checker.toml'), 'utf8');
    assert.ok(checkerToml.includes('name = "sdd-plan-checker"'), 'plan-checker has name');
...
describe('Codex install hook configuration (e2e)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-codex-e2e-'));
  ...
  test('Codex install copies hook file that is referenced in hooks.json (#2153)', () => {
    // Regression test: Codex install writes sdd-check-update hook reference into
    ...
    assert.equal(
      hooksJsonCommands.some((cmd) => cmd.includes('sdd-check-update.js')),
      true,
      'hooks.json references sdd-check-update.js'
    );
    // The hook file must physically exist at the referenced path
    const hookFile = path.join(codexHome, 'hooks', 'sdd-check-update.js');
    assert.ok(
      fs.existsSync(hookFile),
      `sdd-check-update.js must exist at ${hookFile} — config.toml references it but file was not installed`
    );
  });
  ...
  test('fresh CODEX_HOME enables codex_hooks without draft root defaults', () => {
    ...
    const expectedHookPath = path.join(codexHome, 'hooks', 'sdd-check-update.js').replace(/\\/g, '/');
    const expectedCommand = `"${expectedRunner}" "${expectedHookPath}"`;
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const sddCommands = hooksJsonCommands.filter((cmd) => cmd.includes('sdd-check-update.js'));
    assert.strictEqual(sddCommands.length, 1, 'writes one SDD update hook in hooks.json');
    assert.strictEqual(sddCommands[0], expectedCommand, 'handler command must use absolute node runner pointing at sdd-check-update.js (#3017)');
  ...
  test('existing LF config without [features] gets one features block and preserves user content', () => {
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const sddEntries = hooksJsonCommands.filter((cmd) => cmd.includes('sdd-check-update.js'));
    assert.strictEqual(sddEntries.length, 1, 'adds one SDD update hook in hooks.json');
    assertNoDraftRootKeys(content);
  });
  ...
  test('bare top-level keys are NOT trapped under [features] (#1202)', () => {
    ...
    // followed by [projects] section. SDD must not prepend [features] before
  ...
  test('existing empty [features] and codex_hooks = false are normalized and remain idempotent', () => {
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const sddEntries = hooksJsonCommands.filter((cmd) => cmd.includes('sdd-check-update.js'));
    assert.strictEqual(sddEntries.length, 1, 'does not duplicate SDD update hook in hooks.json');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted [features] headers are recognized as the existing features table', () => {
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const sddEntries = hooksJsonCommands.filter((cmd) => cmd.includes('sdd-check-update.js'));
    assert.strictEqual(sddEntries.length, 1, 'keeps one SDD update hook in hooks.json');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted table headers containing # are parsed without treating # as a comment start', () => {
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const sddEntries = hooksJsonCommands.filter((cmd) => cmd.includes('sdd-check-update.js'));
    assert.strictEqual(sddEntries.length, 1, 'remains idempotent for the SDD hook block in hooks.json');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing dotted features config stays dotted and does not grow a [features] table', () => {
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const sddEntries = hooksJsonCommands.filter((cmd) => cmd.includes('sdd-check-update.js'));
    assert.strictEqual(sddEntries.length, 1, 'adds one SDD update hook for dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('root inline-table features assignments are left untouched without appending invalid dotted keys or hooks', () => {
    ...
    assert.strictEqual(countMatches(content, /sdd-check-update\.js/g), 0, 'does not add the SDD hook block when codex_hooks cannot be enabled safely');
    assert.ok(content.includes('[agents.sdd-executor]'), 'still installs the managed agent block in struct format');
    assertNoDraftRootKeys(content);
  });
  ...
  test('quoted dotted codex_hooks keys stay dotted and are normalized without duplication', () => {
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const sddEntries = hooksJsonCommands.filter((cmd) => cmd.includes('sdd-check-update.js'));
    assert.strictEqual(sddEntries.length, 1, 'adds one SDD update hook for quoted dotted codex_hooks and remains idempotent');
    assertNoDraftRootKeys(content);
  });
  ...
  test('existing CRLF codex_hooks = true stays single and preserves non-SDD hooks', () => {
    writeCodexConfig(codexHome, [
      '[features]',
      'codex_hooks = true',
      'other_feature = true',
    ...
    assert.ok(content.includes('other_feature = true'), 'preserves other feature keys');
    assert.strictEqual(countMatches(content, /echo custom-after-command/g), 1, 'preserves non-SDD hook exactly once');
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const sddEntries = hooksJsonCommands.filter((cmd) => cmd.includes('sdd-check-update.js'));
    assert.strictEqual(sddEntries.length, 1, 'keeps one SDD update hook in hooks.json');
    assertUsesOnlyEol(content, '\r\n');
  ...
  test('codex_hooks = true with an inline comment is treated as enabled for hook installation', () => {
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const sddEntries = hooksJsonCommands.filter((cmd) => cmd.includes('sdd-check-update.js'));
    assert.strictEqual(sddEntries.length, 1, 'adds the SDD update hook once in hooks.json');
    assertNoDraftRootKeys(content);
  });
  ...
  test('mixed-EOL configs use the first newline style for inserted Codex content', () => {
    ...
    assert.ok(content.includes(`# SDD Agent Configuration — managed by sdd installer\n`), 'writes the managed agent block using the first newline style');
    ...
    const hooksJsonCommands = readHooksSessionStartCommands(codexHome);
    const sddEntries = hooksJsonCommands.filter((cmd) => cmd.includes('sdd-check-update.js'));
    assert.strictEqual(sddEntries.length, 1, 'writes one managed SessionStart hook to hooks.json');
...
describe('Codex uninstall symmetry for hook-enabled configs', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-codex-uninstall-'));
    codexHome = path.join(tmpDir, 'codex-home');
  });
  ...
  test('install then uninstall removes [features].codex_hooks while preserving other feature keys, comments, hooks, and CRLF', () => {
    ...
    assert.strictEqual(countMatches(cleaned, /echo custom-after-command/g), 1, 'preserves non-SDD hooks');
    assert.strictEqual(countMatches(cleaned, /sdd-check-update\.js/g), 0, 'removes only the SDD update hook');
    assert.strictEqual(countMatches(cleaned, /\[agents\.sdd-/g), 0, 'removes managed SDD agent sections');
  ...
  test('install then uninstall keeps mixed-EOL user content stable while removing SDD hook state', () => {
