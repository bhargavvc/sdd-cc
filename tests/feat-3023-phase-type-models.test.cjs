...
function makeTmp(prefix) {
  return fs.mkdtempSync(path.join(os.tmpdir(), `sdd-3023-${prefix}-`));
}
...
describe('#3023 resolver: models.<phase_type> overrides profile-based tier', () => {
  ...
  test('phase-type alone — research agents get the phase-type tier, planner gets profile default', () => {
    ...
    // sdd-phase-researcher is a research agent — should pick up 'haiku'
    // from the phase-type slot, not 'sonnet' from the balanced profile.
    assert.equal(resolveModelInternal(projectDir, 'sdd-phase-researcher'), 'haiku');
    // sdd-codebase-mapper is also research → haiku
    assert.equal(resolveModelInternal(projectDir, 'sdd-codebase-mapper'), 'haiku');
    // sdd-planner is planning, no models.planning set → falls through to
    // profile (balanced → opus per MODEL_PROFILES).
    assert.equal(resolveModelInternal(projectDir, 'sdd-planner'), 'opus');
  });
  ...
  test('per-agent override beats phase-type (acceptance criterion b)', () => {
    writeConfig(projectDir, {
      model_profile: 'balanced',
      models: { research: 'haiku' },
      model_overrides: { 'sdd-phase-researcher': 'opus' },
    });
    // The targeted per-agent override wins for that one agent.
    assert.equal(resolveModelInternal(projectDir, 'sdd-phase-researcher'), 'opus');
    // Other research agents still pick up the phase-type tier.
    assert.equal(resolveModelInternal(projectDir, 'sdd-codebase-mapper'), 'haiku');
    assert.equal(resolveModelInternal(projectDir, 'sdd-research-synthesizer'), 'haiku');
  });
  ...
  test('phase-type beats profile (acceptance criterion c)', () => {
    ...
    assert.equal(resolveModelInternal(projectDir, 'sdd-phase-researcher'), 'haiku');
    assert.equal(resolveModelInternal(projectDir, 'sdd-codebase-mapper'), 'haiku');
    // sdd-planner is planning, no slot set, profile=quality → opus.
    assert.equal(resolveModelInternal(projectDir, 'sdd-planner'), 'opus');
  });
  ...
  test('issue example: opus for planning/discuss/execution, sonnet for research/verification/completion', () => {
    ...
    assert.equal(resolveModelInternal(projectDir, 'sdd-executor'), 'opus');
    // Research agents → sonnet
    assert.equal(resolveModelInternal(projectDir, 'sdd-phase-researcher'), 'sonnet');
    // Verification agents → sonnet
    assert.equal(resolveModelInternal(projectDir, 'sdd-verifier'), 'sonnet');
  });
  ...
  test('phase-type "inherit" is honored (preserves existing inherit semantics)', () => {
    ...
    assert.equal(resolveModelInternal(projectDir, 'sdd-phase-researcher'), 'inherit');
  });
  ...
  test('unrecognized tier value falls through to profile (typo safety) — CR follow-up', () => {
    ...
    assert.equal(resolveModelInternal(projectDir, 'sdd-phase-researcher'), 'sonnet');
    assert.equal(resolveModelInternal(projectDir, 'sdd-codebase-mapper'), 'haiku',
      'sdd-codebase-mapper at balanced is haiku per profile, unaffected by typo');
  });
  ...
  test('phase-type override wins over profile=inherit (CR Major) — model resolver', () => {
    ...
    // sdd-executor (execution) must get the phase-type opus, not inherit.
    assert.equal(resolveModelInternal(projectDir, 'sdd-executor'), 'opus');
  });
  ...
  test('phase-type "haiku" wins over profile=inherit; agents without a slot still inherit', () => {
    ...
    assert.equal(resolveModelInternal(projectDir, 'sdd-planner'), 'inherit');
  });
  ...
  test('profile=inherit with no models block still returns inherit (no regression)', () => {
    ...
    assert.equal(resolveModelInternal(projectDir, 'sdd-executor'), 'inherit');
    assert.equal(resolveModelInternal(projectDir, 'sdd-phase-researcher'), 'inherit');
  });
  ...
  test('profile=inherit with models block but agent has no slot → inherit', () => {
    ...
    // sdd-executor (execution slot) is not set → falls through to inherit.
    assert.equal(resolveModelInternal(projectDir, 'sdd-executor'), 'inherit');
  });
});
...
const { resolveReasoningEffortInternal } = require('../sdd/bin/lib/core.cjs');
...
describe('#3023 + #3030 CR: resolveReasoningEffortInternal honors phase-type tier (Codex)', () => {
  ...
  test('phase-type override flips both model AND reasoning_effort to the same tier (Codex)', () => {
    ...
    // sdd-executor's profile tier under balanced is sonnet, so without
    ...
    const effort = resolveReasoningEffortInternal(projectDir, 'sdd-executor');
    ...
    const sonnetEffort = (() => {
      ...
      try {
        ...
        return resolveReasoningEffortInternal(sonnetDir, 'sdd-executor');
      } finally {
        rmr(sonnetDir);
      }
    })();
    const opusEffort = (() => {
      ...
      try {
        ...
        return resolveReasoningEffortInternal(opusDir, 'sdd-executor');
      } finally {
        rmr(opusDir);
      }
    })();
  ...
  test('per-agent override still bypasses phase-type for reasoning_effort', () => {
    writeConfig(projectDir, {
      ...
      model_profile: 'balanced',
      models: { execution: 'opus' },
      model_overrides: { 'sdd-executor': 'openai/gpt-5' },
    });
    ...
    assert.equal(resolveReasoningEffortInternal(projectDir, 'sdd-executor'), null);
  });
  ...
  test('phase-type override wins over profile=inherit for effort (CR Major #3030)', () => {
    ...
    // Compute the expected effort by reading what sdd-executor would
    ...
    const expected = (() => {
      ...
      try {
        ...
        return resolveReasoningEffortInternal(dir, 'sdd-executor');
      } finally {
        rmr(dir);
      }
    })();
    const actual = resolveReasoningEffortInternal(projectDir, 'sdd-executor');
...
describe('#3023 config-schema: models.<phase_type> validation', () => {
  ...
  test('unknown phase-type is rejected (acceptance criterion d)', () => {
    ...
    assert.equal(isValidConfigKey('models.sdd-planner'), false,
