'use strict';
/**
 * Tests for loadSkillsManifest — parses requires: frontmatter from commands/sdd/*.md
...
function createFixtureDir() {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-manifest-fixture-'));
  return tmp;
}
...
describe('loadSkillsManifest', () => {
  ...
  test('skill with no requires: frontmatter maps to empty array', () => {
    const dir = createFixtureDir();
    try {
      writeSkill(dir, 'help', 'name: sdd:help\ndescription: Help text');
      const m = loadSkillsManifest(dir);
  ...
  test('skill with requires: single value maps to array of one', () => {
    const dir = createFixtureDir();
    try {
      writeSkill(dir, 'add-tests', 'name: sdd:add-tests\ndescription: Add tests\nrequires: [phase]');
      const m = loadSkillsManifest(dir);
  ...
  test('skill with requires: multiple values maps to full array', () => {
    const dir = createFixtureDir();
    try {
      writeSkill(dir, 'plan-phase', 'name: sdd:plan-phase\ndescription: Plan\nrequires: [discuss-phase, phase, review, update]');
      const m = loadSkillsManifest(dir);
  ...
  test('skill with requires: empty array maps to empty array', () => {
    const dir = createFixtureDir();
    try {
      writeSkill(dir, 'explore', 'name: sdd:explore\ndescription: Explore\nrequires: []');
      const m = loadSkillsManifest(dir);
  ...
  test('loads real commands/sdd/ directory without throwing', () => {
    const realDir = path.join(__dirname, '..', 'commands', 'sdd');
