...
describe('detect-custom-files — update workflow backup detection (#1997)', () => {
  ...
  // update. skills/ is now a SDD-managed directory and must be scanned so that
  // user-added skill directories are backed up before the wipe (#2942).
  // SDD-owned skills (tracked in manifest) must NOT be flagged as custom.
  test('scans skills/ directory and detects user-added skills not in manifest (#2942)', () => {
    writeManifest(tmpDir, {
      'sdd/workflows/execute-phase.md': '# Execute Phase\n',
      'skills/sdd-planner/SKILL.md': '# SDD Planner\n',
    });
    ...
    // The SDD-owned skill (in manifest) should NOT be flagged as custom
    assert.ok(
      !json.custom_files.includes('skills/sdd-planner/SKILL.md'),
      `SDD-owned skill should not be flagged as custom; got: ${JSON.stringify(json.custom_files)}`
    );
  });
