 ...
 * (`stageSkillsForMode` in sdd/bin/lib/install-profiles.cjs),
 * we found that:
 ...
 *   - HOWEVER, the install manifest only recorded `commands/sdd/` for
 ...
 * equals MINIMAL_SKILL_ALLOWLIST, and zero sdd-* agents are present.
 *
...
function runInstall({ runtime, scope, extraArgs = [] }) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `sdd-${runtime}-${scope}-`));
  try {
    let configDir;
    let cwd = process.cwd();
    ...
    const result = spawnSync(process.execPath, args, {
      ...
      // installer reads os.homedir() to detect prior global SDD installs;
 ...
 *                              CodeBuddy)
 *   command/sdd-<name>.md     (OpenCode, Kilo)
 *   commands/sdd/<name>.md    (Gemini, Claude local — fixed in #2923)
 *
...
function manifestSkillSet(manifest) {
  ...
  for (const key of Object.keys(manifest.files)) {
    if (key.startsWith('skills/')) {
      // Strip both the optional `sdd-` prefix (used by Claude/Codex/etc as
      // a per-skill subdir name) and any trailing `.md` (Codex flat layout).
      const seg = key.split('/')[1].replace(/^sdd-/, '').replace(/\.md$/, '');
      out.add(seg);
    } else if (key.startsWith('command/')) {
      const file = key.split('/')[1];
      // Strip `sdd-` prefix and `.md` suffix. Subdirs flatten with `-`,
      // but our minimal allowlist is flat (top-level files only) so this
      // is safe here.
      const base = file.replace(/^sdd-/, '').replace(/\.md$/, '');
      out.add(base);
    } else if (key.startsWith('commands/sdd/')) {
    ...
    test(`${runtime} --global --minimal emits exactly the core skill set, zero agents`, () => {
      ...
      try {
        ...
        assert.strictEqual(manifestAgentCount(manifest), 0,
          `${runtime} global --minimal should record zero sdd-* agents`);
      } finally {
        fs.rmSync(root, { recursive: true, force: true });
      }
    });
  ...
  for (const runtime of SKILL_RUNTIMES) {
    test(`${runtime} --local --minimal emits exactly the core skill set, zero agents`, () => {
      ...
      try {
        ...
        assert.strictEqual(manifestAgentCount(manifest), 0,
          `${runtime} local --minimal should record zero sdd-* agents`);
      } finally {
        fs.rmSync(root, { recursive: true, force: true });
      }
    });
  ...
  for (const scope of ['global', 'local']) {
    test(`cline --${scope} --minimal records mode=minimal and zero agents`, () => {
      ...
      try {
        ...
        assert.strictEqual(manifest.mode, 'minimal');
        assert.strictEqual(manifestAgentCount(manifest), 0,
          `cline ${scope} --minimal should record zero sdd-* agents`);
...
describe('install: directory-on-disk matches manifest for --minimal', () => {
  ...
  // dir and verify the sdd-* surface on disk equals what the manifest claims.
    ...
    for (const scope of ['global', 'local']) {
      test(`${runtime} --${scope} --minimal: on-disk skill files match manifest`, () => {
        ...
        try {
          ...
          // And no sdd-*.md agent file should exist on disk either:
          const agentsDir = path.join(configDir, 'agents');
          if (fs.existsSync(agentsDir)) {
            const sddAgents = fs.readdirSync(agentsDir).filter(
              (f) => f.startsWith('sdd-') && f.endsWith('.md'),
            );
            assert.deepStrictEqual(sddAgents, [],
              `${runtime} ${scope} --minimal should not write sdd-*.md agents on disk`);
          }
        } finally {
          fs.rmSync(root, { recursive: true, force: true });
        }
  ...
  if (fs.existsSync(skillsDir)) {
    for (const entry of fs.readdirSync(skillsDir, { withFileTypes: true })) {
      if (entry.isDirectory() && entry.name.startsWith('sdd-')) {
        out.add(entry.name.replace(/^sdd-/, ''));
      } else if (entry.isFile() && entry.name.startsWith('sdd-') && entry.name.endsWith('.md')) {
        // Codex flat skills/ layout: skills/sdd-<name>.md
        out.add(entry.name.replace(/^sdd-/, '').replace(/\.md$/, ''));
      }
    }
  }
  ...
  if (fs.existsSync(commandDir)) {
    for (const file of fs.readdirSync(commandDir)) {
      if (file.startsWith('sdd-') && file.endsWith('.md')) {
        out.add(file.replace(/^sdd-/, '').replace(/\.md$/, ''));
      }
    }
  }

  // commands/sdd/<name>.{md,toml} (Claude local emits .md; Gemini emits .toml)
  const commandsSddDir = path.join(configDir, 'commands', 'sdd');
  if (fs.existsSync(commandsSddDir)) {
    for (const file of fs.readdirSync(commandsSddDir)) {
