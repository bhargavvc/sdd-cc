 ...
 * so `npx @bhargavvc/sdd-cc@latest --sdk` produced a misleading "✓ SDD SDK ready"
...
describe('bug #3033: --sdk flag (opts.forceSdk) must be wired into installSdkIfNeeded', () => {
  ...
  beforeEach(() => {
    tmpRoot = createTempDir('sdd-3033-');
  ...
  test('forceSdk=true + isLocal=true + dist present: self-links sdd-sdk into PATH dir', () => {
    ...
    assert.ok(
      ...
      `forceSdk=true must materialize sdd-sdk shim at ${linkPath}. Output:\n${combined}`,
    );

    // Must report "SDD SDK ready" — not the legacy "Skipping SDK check" message.
    assert.ok(
      /SDD SDK ready/.test(combined),
      `forceSdk=true must print "SDD SDK ready" once shim is on PATH. Output:\n${combined}`,
    );
    assert.ok(
  ...
  test('throwOnFailure=true converts missing SDK dist into catchable error', () => {
    ...
    assert.throws(
      ...
      (error) => {
        assert.equal(error.code, 'SDD_SDK_MISSING_DIST');
