 ...
 *
 * #2046 (macOS/Linux): The three .sh hooks (sdd-validate-commit.sh,
 * sdd-session-state.sh, sdd-phase-boundary.sh) were registered in
...
const SH_HOOKS = [
  { name: 'sdd-validate-commit.sh', commandVar: 'validateCommitCommand' },
  { name: 'sdd-session-state.sh',   commandVar: 'sessionStateCommand' },
  { name: 'sdd-phase-boundary.sh',  commandVar: 'phaseBoundaryCommand' },
];
...
describe('bugs #2045 #2046: .sh hook paths must be absolute and quoted', () => {
  ...
  test('global .sh hook commands use buildHookCommand, not unquoted string concat', () => {
    ...
    //   'bash ' + targetDir.replace(/\\/g, '/') + '/hooks/sdd-*.sh'
    ...
    // wraps the path in double quotes: bash "/absolute/path/hooks/sdd-*.sh"
    ...
    assert.strictEqual(
      ...
      `('bash ' + targetDir). Use buildHookCommand(targetDir, 'sdd-*.sh') instead.`
    );
  });
  ...
  test('global .sh hook commands in source use bash with double-quoted path', () => {
    // After the fix, buildHookCommand produces: bash "/abs/path/hooks/sdd-*.sh"
