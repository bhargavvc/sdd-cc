...
const FAMILIES = [
  { label: 'Agents',      dir: 'agents',                      filter: (f) => /^sdd-.*\.md$/.test(f) },
  { label: 'Commands',    dir: 'commands/sdd',                filter: (f) => f.endsWith('.md') },
  { label: 'Workflows',   dir: 'sdd/workflows',     filter: (f) => f.endsWith('.md') },
  { label: 'References',  dir: 'sdd/references',    filter: (f) => f.endsWith('.md') },
  { label: 'CLI Modules', dir: 'sdd/bin/lib',       filter: (f) => f.endsWith('.cjs') },
