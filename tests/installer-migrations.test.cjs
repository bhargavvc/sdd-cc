...
const {
  ...
  writeInstallState,
} = require('../sdd/bin/lib/installer-migrations.cjs');
const firstTimeBaselineMigration = require('../sdd/bin/lib/installer-migrations/000-first-time-baseline.cjs');

function createTempInstall() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-installer-migrations-'));
}
...
function writeManifest(root, files) {
  fs.writeFileSync(
    path.join(root, 'sdd-file-manifest.json'),
    JSON.stringify({
      version: '1.49.0',
...
function legacyCodexHook(configDir) {
  return {
    hooks: [
      {
        type: 'command',
        command: `node "${path.join(configDir, 'hooks', 'sdd-check-update.js')}"`,
      },
    ],
  };
}
...
test('records a first-time baseline while preserving user-owned artifacts', () => {
  ...
  try {
    writeFile(configDir, 'sdd/workflows/plan.md', 'managed workflow\n');
    writeFile(configDir, 'sdd/USER-PROFILE.md', 'user profile\n');
    writeManifest(configDir, {
      'sdd/workflows/plan.md': sha256('managed workflow\n'),
    });
    ...
    assert.equal(fs.readFileSync(path.join(configDir, 'sdd/workflows/plan.md'), 'utf8'), 'managed workflow\n');
    assert.equal(fs.readFileSync(path.join(configDir, 'sdd/USER-PROFILE.md'), 'utf8'), 'user profile\n');
...
test('blocks stale SDD-looking baseline artifacts for explicit user choice', () => {
  const configDir = createTempInstall();
  try {
    writeFile(configDir, 'hooks/sdd-retired-hook.js', 'old sdd hook\n');
    writeManifest(configDir, {});
    ...
    assert.equal(fs.readFileSync(path.join(configDir, 'hooks/sdd-retired-hook.js'), 'utf8'), 'old sdd hook\n');
    assert.deepEqual(
      ...
      [
        {
          type: 'prompt-user',
          relPath: 'hooks/sdd-retired-hook.js',
          classification: 'stale-sdd-looking',
          choices: ['keep', 'remove'],
        },
      ]
    );
...
test('records known generated agent artifacts so profile cleanup can remove them', () => {
  const configDir = createTempInstall();
  try {
    writeFile(configDir, 'agents/sdd-executor.md', 'old generated agent\n');
    writeFile(configDir, 'agents/sdd-executor.toml', 'old generated agent config\n');
    writeFile(configDir, 'agents/sdd-local-experiment.md', 'user experiment\n');
    writeManifest(configDir, {});
    ...
    assert.deepEqual(
      ...
      [
        {
          type: 'record-baseline',
          relPath: 'agents/sdd-executor.md',
          classification: 'unknown',
        },
        {
          type: 'record-baseline',
          relPath: 'agents/sdd-executor.toml',
          classification: 'unknown',
        },
        {
          type: 'prompt-user',
          relPath: 'agents/sdd-local-experiment.md',
          classification: 'stale-sdd-looking',
        },
      ]
    );
    assert.deepEqual(result.blocked.map((action) => action.relPath), ['agents/sdd-local-experiment.md']);
    assert.equal(fs.readFileSync(path.join(configDir, 'agents/sdd-executor.md'), 'utf8'), 'old generated agent\n');
    assert.equal(fs.readFileSync(path.join(configDir, 'agents/sdd-executor.toml'), 'utf8'), 'old generated agent config\n');
    assert.equal(fs.readFileSync(path.join(configDir, 'agents/sdd-local-experiment.md'), 'utf8'), 'user experiment\n');
  } finally {
    cleanup(configDir);
  }
});
...
test('classifies large files without loading the whole file through readFileSync', (t) => {
  ...
  const relPath = 'skills/sdd-large/SKILL.md';
...
test('applies an unblocked plan with a journal and install-state update', () => {
  ...
  try {
    ...
    assert.match(
      ...
      /^sdd-migration-journal\/2026-05-11T00-00-01-000Z-[0-9a-f]+\.json$/
    );
...
test('stores modified-file backups under the unique migration run journal', (t) => {
  ...
  assert.match(backupRelPath, /^sdd-migration-journal\/2026-05-11T00-00-10-000Z-[0-9a-f]+-backups\/hooks\/old-hook\.js$/);
...
test('successful migration rollback removes run-scoped backup directories', (t) => {
  ...
  assert.equal(
    fs.readdirSync(path.join(configDir, 'sdd-migration-journal')).some((name) => name.includes('backups')),
    false
  );
});
...
test('refuses to run migrations while another installer owns the migration lock', (t) => {
  const configDir = createTempInstall();
  t.after(() => cleanup(configDir));
  fs.writeFileSync(path.join(configDir, 'sdd-install-migration.lock'), 'held by test\n', 'utf8');
...
test('reports lock release failures after migration work completes', (t) => {
  ...
  fs.rmSync = (targetPath, ...args) => {
    if (path.basename(String(targetPath)) === 'sdd-install-migration.lock') {
      ...
      appliedMigrations: [
        {
          ...
          journal: 'sdd-migration-journal/prior.json',
        },
      ],
    });
...
test('rolls back touched files and leaves state unchanged when apply fails', () => {
  ...
  try {
    ...
    assert.equal(
      fs.existsSync(path.join(configDir, 'sdd-migration-journal')) &&
        fs.readdirSync(path.join(configDir, 'sdd-migration-journal')).some((name) =>
  ...
  try {
    ...
    const plan = planInstallerMigrations({
      ...
      migrations: discoverInstallerMigrations({
        migrationsDir: path.join(__dirname, '..', 'sdd', 'bin', 'lib', 'installer-migrations'),
      }),
      scope: 'global',
      now: () => '2026-05-11T00:00:05.000Z',
    });
...
test('runs a Codex legacy hooks.json cleanup migration without removing user hooks', () => {
  ...
  try {
    writeFile(
      ...
      JSON.stringify({
        SessionStart: [
          ...
          userHook('node "/Users/example/bin/sdd-check-update.js"'),
        ],
      }, null, 2)
    );
    writeManifest(configDir, {});
    ...
    assert.deepEqual(commands, [
      ...
      'node "/Users/example/bin/sdd-check-update.js"',
    ]);
...
test('skips runtime-specific migration records for other runtimes', () => {
  ...
  try {
    ...
    const hooksJson = JSON.parse(fs.readFileSync(path.join(configDir, 'hooks.json'), 'utf8'));
    assert.equal(hooksJson.SessionStart[0].hooks[0].command, `node "${path.join(configDir, 'hooks', 'sdd-check-update.js')}"`);
