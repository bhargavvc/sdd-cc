 ...
 *
 * The slug is derived from the `name:` frontmatter field (e.g. `sdd-workflow`)
 ...
 * Accepts both `sdd:slug` and `sdd-slug` forms.
  ...
  for (const file of commandFiles) {
    ...
    test(`/sdd-${slug}`, () => {
      ...
      assert.ok(
        ...
        `commands/sdd/${file} (name: sdd-${slug}) is not mentioned in docs/COMMANDS.md (as a heading) or docs/INVENTORY.md (as a Commands row) — add a one-line entry to at least one`,
      );
    });
  }
});
