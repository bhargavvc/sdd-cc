 ...
 *
 * `sdd-sdk query config-set model_overrides.sdd-plan-checker opus` was
...
const { DYNAMIC_KEY_PATTERNS, isValidConfigKey } = require('../sdd/bin/lib/config-schema.cjs');

describe('#3227 — config-set accepts model_overrides.<agent-id>', () => {
  test('isValidConfigKey accepts model_overrides.sdd-plan-checker', () => {
    assert.ok(
      isValidConfigKey('model_overrides.sdd-plan-checker'),
      'model_overrides.sdd-plan-checker must be accepted by isValidConfigKey'
    );
  });

  test('isValidConfigKey accepts model_overrides with various agent-id formats', () => {
    const validKeys = [
      'model_overrides.sdd-executor',
      'model_overrides.sdd-planner',
      'model_overrides.sdd-codebase-mapper',
  ...
  test('config-set model_overrides.sdd-plan-checker opus succeeds via sdd-tools.cjs', (t) => {
    const tmpDir = createTempProject();
    t.after(() => cleanup(tmpDir));

    const result = runSddTools(
      ['config-set', 'model_overrides.sdd-plan-checker', 'opus'],
      tmpDir
    );
    assert.ok(
      result.success,
      [
        'config-set model_overrides.sdd-plan-checker opus should succeed,',
        'got:',
  ...
  test('config-set model_overrides.sdd-plan-checker opus writes to config.json', (t) => {
    const tmpDir = createTempProject();
    t.after(() => cleanup(tmpDir));

    runSddTools(['config-set', 'model_overrides.sdd-plan-checker', 'opus'], tmpDir);
    ...
    assert.ok(
      config.model_overrides !== undefined &&
        config.model_overrides['sdd-plan-checker'] === 'opus',
      [
        'Expected model_overrides["sdd-plan-checker"]: "opus" in config.json,',
        'got: ' + JSON.stringify(config),
      ].join('\n')
    );
  });

  test('config-get model_overrides.sdd-plan-checker returns opus after config-set', (t) => {
    const tmpDir = createTempProject();
    t.after(() => cleanup(tmpDir));
    ...
    const getResult = runSddTools(
      ['config-get', 'model_overrides.sdd-plan-checker'],
      tmpDir
    );
    assert.ok(
      getResult.success,
      [
        'config-get model_overrides.sdd-plan-checker should succeed,',
        'got:',
