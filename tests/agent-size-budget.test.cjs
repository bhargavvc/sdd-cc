 ...
 * Agent size budget.
 *
 * Agent definitions in `agents/sdd-*.md` are loaded verbatim into Claude's
 ...
 * that belongs in `sdd/references/`.
 *
 * See: https://github.com/bhargavvc/sdd-cc/issues/2361
 */
  ...
  for (const agent of ALL_AGENTS) {
    ...
    test(`${agent} (${tier}) stays under ${limit} lines`, () => {
      ...
      assert.ok(
        ...
        `Extract shared boilerplate to sdd/references/ or raise the budget ` +
