 ...
 *
 * - #3029: `/sdd-code-review-fix` was deleted by #2790 (consolidated into
 *   `/sdd-code-review --fix`), but the agent role cards
 *   (`agents/sdd-code-fixer.md`), several workflow offer blocks
 ...
 *
 * - #3034: `/sdd-plan-milestone-gaps` was deleted by #2790 (gap planning
 *   now happens inline as part of `/sdd-audit-milestone`'s output).
 *   `audit-milestone.md` <offer_next> blocks (lines 281, 323) and the
 *   `sdd-complete-milestone` skill (lines 46, 57) still emit it.
 *
 ...
 * A slash command is a token starting with "/sdd-" followed by hyphenated
 ...
 * shape used by audit-milestone.md. For stub files (commands/sdd/*),
...
function extractOfferBlocks(content) {
  ...
  // gaps_found marker. Used by commands/sdd/complete-milestone.md, which
  ...
  // Also include the bullet-list step-0 block in commands/sdd/
...
// ─── #3029: /sdd-code-review-fix scrub ──────────────────────────────────────

const CRF_DELETED = '/sdd-code-review-fix';
const CRF_REPLACEMENT = '/sdd-code-review --fix';
...
const CRF_USER_FACING_SURFACES = [
  'agents/sdd-code-fixer.md',
  'sdd/workflows/code-review.md',
  'sdd/workflows/execute-phase.md',
  'docs/INVENTORY.md',
...
describe('bug #3029: /sdd-code-review-fix scrubbed from user-facing surfaces', () => {
  ...
  for (const rel of CRF_REPLACEMENT_SURFACES) {
    test(`${rel}: replacement "${CRF_REPLACEMENT}" tokens present`, () => {
      const content = read(rel);
      // The replacement is a multi-token form (`/sdd-code-review` + the
      ...
      // chars of a /sdd-code-review token; that proves the flag belongs
      // to the right command rather than appearing somewhere unrelated.
      const tokens = extractSlashCommandTokens(content);
      assert.equal(
        tokens.has('/sdd-code-review'),
        true,
        `${rel}: must reference root /sdd-code-review token`
      );
      const proximityRe = /\/sdd-code-review[^\n]{0,50}--fix/;
      assert.ok(
        proximityRe.test(content),
        `${rel}: must document the "/sdd-code-review … --fix" form (root token + flag within 50 chars)`
      );
    });
  }
});

// ─── #3034: /sdd-plan-milestone-gaps scrub ──────────────────────────────────

const PMG_DELETED = '/sdd-plan-milestone-gaps';
// The closure path is the user-facing replacement for gap planning. We
// don't pin the exact prose — the sdd-ns-project SKILL.md describes it
// as inline gap planning routed through /sdd-phase --insert plus the
// standard discuss/plan/execute chain. We assert structurally:
// (a) deleted command is absent, and (b) at minimum /sdd-phase appears
// in the same offer-next block where the deleted command lived.
const PMG_FIX_SURFACES = [
  'sdd/workflows/audit-milestone.md',
  'commands/sdd/complete-milestone.md',
];

describe('bug #3034: /sdd-plan-milestone-gaps scrubbed from user-facing surfaces', () => {
  for (const rel of PMG_FIX_SURFACES) {
    test(`${rel}: deleted "${PMG_DELETED}" not in slash-command token set`, () => {
      ...
      assert.equal(
        ...
        false,
        `${rel}: parsed slash-command token set still contains "${PMG_DELETED}" — gap planning now happens inline; route via /sdd-phase --insert`
      );
    });
    ...
    test(`${rel}: replacement guidance present in the offer/pre-flight block where the deleted command lived`, () => {
      ...
      // accept either the explicit /sdd-phase --insert closure path or
      ...
      const blocksWithReplacement = blocks.filter(
        (b) => b.includes('/sdd-phase --insert') || inlineProseRe.test(b)
      );
      assert.ok(
        blocksWithReplacement.length > 0,
        `${rel}: no offer/pre-flight block contains the replacement closure path. ` +
          `Expected at least one block to mention "/sdd-phase --insert" or inline-audit prose ` +
...
// ─── Cross-issue invariant: sdd-ns-project still documents the deletion ─────

describe('cross-check: sdd-ns-project keeps the deletion note', () => {
  test('commands/sdd/ns-project.md still notes /sdd-plan-milestone-gaps was deleted', () => {
    const content = read('commands/sdd/ns-project.md');
    // sdd-ns-project legitimately mentions the deleted command name in
    // a "deleted by #2790" note for routing context. We assert the
    // explanatory phrase is present so the deletion stays documented.
    assert.ok(
      /sdd-plan-milestone-gaps.*deleted by #2790|deleted by #2790.*sdd-plan-milestone-gaps/s.test(content),
      'sdd-ns-project must keep the "deleted by #2790" note for /sdd-plan-milestone-gaps so future readers understand the inline-audit replacement'
    );
  });
});
