 ...
 * developer's real `~/.sdd/defaults.json` cannot bleed into assertions
...
const { isValidConfigKey } = require('../sdd/bin/lib/config-schema.cjs');
...
// Without this, a developer's real `~/.sdd/defaults.json` (e.g. one with
...
function isolateHome() {
  ...
  _origSddHome = process.env.SDD_HOME;
  _isolatedHome = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-home-iso-'));
  process.env.HOME = _isolatedHome;
  process.env.SDD_HOME = _isolatedHome;
}
function restoreHome() {
  if (_origHome === undefined) delete process.env.HOME; else process.env.HOME = _origHome;
  if (_origSddHome === undefined) delete process.env.SDD_HOME; else process.env.SDD_HOME = _origSddHome;
...
describe('issue #2517: backwards compat — no runtime key set', () => {
  ...
  test('balanced profile returns Claude alias when runtime absent', () => {
    ...
    // sdd-planner balanced -> opus
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'opus');
  });

  test('inherit profile still returns "inherit" with no runtime', () => {
    writeConfig(tmpDir, { model_profile: 'inherit' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'inherit');
  });
  ...
  test('resolve_model_ids:true still maps alias -> full Claude ID with no runtime', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'claude-opus-4-7');
  });
  ...
  test('resolve_model_ids:"omit" still returns "" with no runtime', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), '');
  });

  test('reasoning_effort returns null when runtime absent', () => {
    writeConfig(tmpDir, { model_profile: 'balanced' });
    assert.strictEqual(resolveReasoningEffortInternal(tmpDir, 'sdd-planner'), null);
  });

  test('adaptive profile still works without runtime (#1713/#1806)', () => {
    writeConfig(tmpDir, { model_profile: 'adaptive' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'opus');
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-codebase-mapper'), 'haiku');
  });
});
...
describe('issue #2517: runtime "codex" — Codex tier resolution', () => {
  ...
  test('opus tier -> gpt-5.4 with reasoning_effort xhigh', () => {
    ...
    // sdd-planner quality -> opus -> gpt-5.4
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'gpt-5.4');
    assert.strictEqual(resolveReasoningEffortInternal(tmpDir, 'sdd-planner'), 'xhigh');
  });

  test('sonnet tier -> gpt-5.3-codex with reasoning_effort medium', () => {
    writeConfig(tmpDir, { runtime: 'codex', model_profile: 'balanced' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-roadmapper'), 'gpt-5.3-codex');
    assert.strictEqual(resolveReasoningEffortInternal(tmpDir, 'sdd-roadmapper'), 'medium');
  });

  test('haiku tier -> gpt-5.4-mini with reasoning_effort medium', () => {
    writeConfig(tmpDir, { runtime: 'codex', model_profile: 'budget' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-codebase-mapper'), 'gpt-5.4-mini');
    assert.strictEqual(resolveReasoningEffortInternal(tmpDir, 'sdd-codebase-mapper'), 'medium');
  });

  test('adaptive profile resolves on Codex (no #1713/#1806 regression)', () => {
    writeConfig(tmpDir, { runtime: 'codex', model_profile: 'adaptive' });
    // sdd-planner adaptive -> opus -> gpt-5.4
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'gpt-5.4');
    // sdd-codebase-mapper adaptive -> haiku -> gpt-5.4-mini
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-codebase-mapper'), 'gpt-5.4-mini');
  });
...
describe('issue #2517: precedence chain', () => {
  ...
  test('per-agent model_overrides wins over runtime tier resolution', () => {
    writeConfig(tmpDir, {
      runtime: 'codex',
      model_profile: 'quality',
      model_overrides: { 'sdd-planner': 'gpt-5.4-mini' },
    });
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'gpt-5.4-mini');
  });
  ...
  test('model_profile_overrides[runtime][tier] beats built-in defaults', () => {
    ...
    // sdd-planner quality -> opus -> overridden to gpt-5-pro
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'gpt-5-pro');
    // haiku not overridden — fall back to spec defaults
    // sdd-codebase-mapper quality -> sonnet -> gpt-5.3-codex
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-codebase-mapper'), 'gpt-5.3-codex');
  });
  ...
  test('partial profile_overrides — only opus overridden, sonnet uses default', () => {
    ...
    // sdd-planner balanced -> opus -> overridden to gpt-5-pro
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'gpt-5-pro');
    // sdd-roadmapper balanced -> sonnet -> spec default
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-roadmapper'), 'gpt-5.3-codex');
  });
  ...
  test('per-agent override beats profile override beats default', () => {
    writeConfig(tmpDir, {
      ...
      model_overrides: { 'sdd-planner': 'custom-model' },
    });
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'custom-model');
  });
});
...
describe('issue #2517: field-merge of overrides with built-in defaults (finding #2)', () => {
  ...
  test('partial-object override (no model) keeps model from built-in', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'gpt-5.4');
    assert.strictEqual(resolveReasoningEffortInternal(tmpDir, 'sdd-planner'), 'low');
  });
  ...
  test('full-object override replaces both fields', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'custom-model');
    assert.strictEqual(resolveReasoningEffortInternal(tmpDir, 'sdd-planner'), 'minimal');
  });
...
describe('issue #2517: reasoning_effort allowlist gates regardless of overrides (finding #3)', () => {
  ...
  test('unknown runtime with overrides supplying reasoning_effort yields null effort', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'mystery-opus');
...
describe('issue #2517: unknown runtime + safe fallback', () => {
  ...
  test('unknown runtime falls back to Claude-alias safe default (no Codex IDs leaked)', () => {
    ...
    const resolved = resolveModelInternal(tmpDir, 'sdd-planner');
...
describe('issue #2517: install end-to-end — per-project config reaches Codex TOML (finding #1)', () => {
  ...
  const prevTestMode = process.env.SDD_TEST_MODE;
  process.env.SDD_TEST_MODE = '1';
  const installMod = require('../bin/install.js');
  if (prevTestMode === undefined) delete process.env.SDD_TEST_MODE;
  else process.env.SDD_TEST_MODE = prevTestMode;
  const { readSddRuntimeProfileResolver, generateCodexAgentToml } = installMod;

  let tmpDir;
  beforeEach(() => { isolateHome(); tmpDir = createTempProject(); _resetRuntimeWarningCacheForTests(); });
  afterEach(() => { cleanup(tmpDir); restoreHome(); });

  test('readSddRuntimeProfileResolver picks up runtime from .planning/config.json', () => {
    // No ~/.sdd/defaults.json (HOME is isolated tmpdir). Per-project config alone
    // must drive the resolver — pre-fix, it returned null.
    writeConfig(tmpDir, { runtime: 'codex', model_profile: 'quality' });
    const resolver = readSddRuntimeProfileResolver(tmpDir);
    assert.ok(resolver, 'expected a resolver from per-project config');
    assert.strictEqual(resolver.runtime, 'codex');
    const entry = resolver.resolve('sdd-planner');
    assert.deepStrictEqual(entry, { model: 'gpt-5.4', reasoning_effort: 'xhigh' });
  });

  test('per-project config wins over global ~/.sdd/defaults.json', () => {
    fs.mkdirSync(path.join(_isolatedHome, '.sdd'), { recursive: true });
    fs.writeFileSync(
      path.join(_isolatedHome, '.sdd', 'defaults.json'),
  ...
  test('inline require paths resolve relative to install.js __dirname (finding #6)', () => {
    ...
    const libDir = path.join(installDir, '..', 'sdd', 'bin', 'lib');
...
describe('issue #2612: runtime "gemini" — Gemini tier resolution', () => {
  ...
  test('opus tier -> gemini-3-pro', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'gemini-3-pro');
  });

  test('sonnet tier -> gemini-3-flash', () => {
    writeConfig(tmpDir, { runtime: 'gemini', model_profile: 'balanced' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-roadmapper'), 'gemini-3-flash');
  });

  test('haiku tier -> gemini-2.5-flash-lite', () => {
    writeConfig(tmpDir, { runtime: 'gemini', model_profile: 'budget' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-codebase-mapper'), 'gemini-2.5-flash-lite');
  });
...
describe('issue #2612: runtime "qwen" — Qwen tier resolution', () => {
  ...
  test('opus tier -> qwen3-max-2026-01-23', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'qwen3-max-2026-01-23');
  });

  test('sonnet tier -> qwen3-coder-plus', () => {
    writeConfig(tmpDir, { runtime: 'qwen', model_profile: 'balanced' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-roadmapper'), 'qwen3-coder-plus');
  });

  test('haiku tier -> qwen3-coder-next', () => {
    writeConfig(tmpDir, { runtime: 'qwen', model_profile: 'budget' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-codebase-mapper'), 'qwen3-coder-next');
  });
...
describe('issue #2612: runtime "opencode" — OpenCode tier resolution', () => {
  ...
  test('opus tier -> anthropic/claude-opus-4-7', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'anthropic/claude-opus-4-7');
  });

  test('sonnet tier -> anthropic/claude-sonnet-4-6', () => {
    writeConfig(tmpDir, { runtime: 'opencode', model_profile: 'balanced' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-roadmapper'), 'anthropic/claude-sonnet-4-6');
  });

  test('haiku tier -> anthropic/claude-haiku-4-5', () => {
    writeConfig(tmpDir, { runtime: 'opencode', model_profile: 'budget' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-codebase-mapper'), 'anthropic/claude-haiku-4-5');
  });
...
describe('issue #2612: runtime "copilot" — Copilot tier resolution', () => {
  ...
  test('sonnet tier -> claude-sonnet-4-6', () => {
    ...
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-roadmapper'), 'claude-sonnet-4-6');
  });

  test('haiku tier -> claude-haiku-4-5', () => {
    writeConfig(tmpDir, { runtime: 'copilot', model_profile: 'budget' });
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-codebase-mapper'), 'claude-haiku-4-5');
  });
...
describe('issue #2612: partial override merge for new Group A runtimes', () => {
  ...
  test('gemini.opus override wins; sonnet and haiku use built-in defaults', () => {
    ...
    // opus is overridden
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'gemini-3-ultra');
    // sonnet not overridden — built-in default (quality -> sonnet for sdd-codebase-mapper)
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-codebase-mapper'), 'gemini-3-flash');
  });
  ...
  test('qwen.opus override wins; sonnet and haiku use built-in defaults', () => {
    ...
    // opus is overridden
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'qwen3-max-custom');
    // sonnet not overridden — quality -> sonnet for sdd-codebase-mapper
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-codebase-mapper'), 'qwen3-coder-plus');
  });
  ...
  test('opencode.sonnet override wins; opus and haiku still use built-in defaults', () => {
    ...
    // sdd-planner balanced -> opus -> built-in default
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'anthropic/claude-opus-4-7');
    // sdd-roadmapper balanced -> sonnet -> overridden
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-roadmapper'), 'anthropic/claude-sonnet-4-7');
    // sdd-codebase-mapper balanced -> haiku -> built-in default (haiku not overridden)
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-codebase-mapper'), 'anthropic/claude-haiku-4-5');
  });
  ...
  test('copilot.haiku override wins; opus and sonnet still use built-in defaults', () => {
    ...
    // sdd-codebase-mapper budget -> haiku -> overridden
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-codebase-mapper'), 'claude-haiku-4-6');
    // sdd-planner budget -> sonnet -> built-in default
    assert.strictEqual(resolveModelInternal(tmpDir, 'sdd-planner'), 'claude-sonnet-4-6');
  });
});
