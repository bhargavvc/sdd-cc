...
const REFS_DIR = path.join(__dirname, '..', 'sdd', 'references', 'few-shot-examples');
...
describe('few-shot calibration examples', () => {
  ...
  describe('agent files reference few-shot examples', () => {
    test('sdd-plan-checker.md contains reference to plan-checker few-shot examples', () => {
      const content = readFile(path.join(AGENTS_DIR, 'sdd-plan-checker.md'));
      assert.match(content, /@~\/\.claude\/sdd\/references\/few-shot-examples\/plan-checker\.md/);
    });

    test('sdd-verifier.md contains reference to verifier few-shot examples', () => {
      const content = readFile(path.join(AGENTS_DIR, 'sdd-verifier.md'));
      assert.match(content, /@~\/\.claude\/sdd\/references\/few-shot-examples\/verifier\.md/);
    });
  });
