...
function createFixtureDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-lint-deps-fixture-'));
}

function writeSkillFile(dir, stem, { description = 'Test skill', requires = null, body = '' }) {
  let fm = `name: sdd:${stem}\ndescription: ${description}`;
  if (requires !== null) {
...
describe('lint-skill-deps: frontmatter ↔ body consistency', () => {
  test('exits 0 when all skills have consistent requires: and references', () => {
    ...
    try {
      ...
      writeSkillFile(dir, 'discuss-phase', {
        ...
        requires: ['phase'],
        body: 'Invoke with /sdd:phase to manage phases.',
      });
  ...
  test('exits non-zero when body references sdd:phase but requires: is absent', () => {
    const dir = createFixtureDir();
    try {
      writeSkillFile(dir, 'phase', { description: 'Phase skill', body: '' });
      writeSkillFile(dir, 'discuss-phase', {
        description: 'Discuss skill',
        // requires: absent — but body references /sdd:phase
        requires: null,
        body: 'Use /sdd:phase to manage phases.',
      });
      const result = runLint(['--dir', dir]);
  ...
  test('exits non-zero when body references sdd-phase but requires: does not include it', () => {
    const dir = createFixtureDir();
    try {
      writeSkillFile(dir, 'phase', { description: 'Phase skill', body: '' });
      writeSkillFile(dir, 'discuss-phase', {
...
describe('lint-skill-deps: profile closure satisfaction', () => {
  test('exits 0 when run against real commands/sdd (all profiles closed or full)', () => {
    // This is the most important integration test: running lint on the real
    // commands/sdd/ directory with the real PROFILES must pass.
