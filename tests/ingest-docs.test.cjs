 ...
 *
 * Structural assertions for /sdd-ingest-docs (#2387). Agents and workflows
...
const { extractFrontmatter } = require('../sdd/bin/lib/frontmatter.cjs');

const ROOT = path.join(__dirname, '..');
const CMD_PATH = path.join(ROOT, 'commands', 'sdd', 'ingest-docs.md');
const WF_PATH = path.join(ROOT, 'sdd', 'workflows', 'ingest-docs.md');
const CLASSIFIER_PATH = path.join(ROOT, 'agents', 'sdd-doc-classifier.md');
const SYNTHESIZER_PATH = path.join(ROOT, 'agents', 'sdd-doc-synthesizer.md');
const CONFLICT_ENGINE_PATH = path.join(ROOT, 'sdd', 'references', 'doc-conflict-engine.md');

// ─── File Existence ────────────────────────────────────────────────────────────

describe('ingest-docs file structure (#2387)', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(CMD_PATH), 'commands/sdd/ingest-docs.md should exist');
  });
  test('workflow file exists', () => {
    assert.ok(fs.existsSync(WF_PATH), 'sdd/workflows/ingest-docs.md should exist');
  });
  test('classifier agent exists', () => {
    assert.ok(fs.existsSync(CLASSIFIER_PATH), 'agents/sdd-doc-classifier.md should exist');
  });
  test('synthesizer agent exists', () => {
    assert.ok(fs.existsSync(SYNTHESIZER_PATH), 'agents/sdd-doc-synthesizer.md should exist');
  });
...
describe('ingest-docs command frontmatter', () => {
  ...
  test('has name field', () => {
    assert.match(content, /^name:\s*sdd:ingest-docs$/m);
  });
...
describe('ingest-docs command references', () => {
  ...
  test('references the ingest-docs workflow', () => {
    assert.ok(
      content.includes('@~/.claude/sdd/workflows/ingest-docs.md'),
      'command must @-reference its workflow'
    );
  });
  test('references the doc-conflict-engine', () => {
    assert.ok(
      content.includes('@~/.claude/sdd/references/doc-conflict-engine.md'),
      'command must load the shared conflict-engine contract'
    );
  });
  test('references gate-prompts', () => {
    assert.ok(
      content.includes('@~/.claude/sdd/references/gate-prompts.md'),
...
describe('ingest-docs workflow content', () => {
  ...
  test('spawns sdd-doc-classifier and sdd-doc-synthesizer', () => {
    assert.ok(
      content.includes('sdd-doc-classifier'),
      'workflow must spawn sdd-doc-classifier'
    );
    assert.ok(
      content.includes('sdd-doc-synthesizer'),
      'workflow must spawn sdd-doc-synthesizer'
    );
  });
  ...
  test('routes to sdd-roadmapper in new mode', () => {
    assert.ok(
      content.includes('sdd-roadmapper'),
      'new mode must delegate to sdd-roadmapper'
    );
  });
...
describe('sdd-doc-classifier agent', () => {
...
describe('sdd-doc-synthesizer agent', () => {
...
describe('import command adopts shared conflict-engine', () => {
  const cmdContent = fs.readFileSync(path.join(ROOT, 'commands', 'sdd', 'import.md'), 'utf-8');
  const wfContent = fs.readFileSync(path.join(ROOT, 'sdd', 'workflows', 'import.md'), 'utf-8');

  test('import command loads doc-conflict-engine reference', () => {
    assert.ok(
      cmdContent.includes('@~/.claude/sdd/references/doc-conflict-engine.md'),
      '/sdd-import must load the shared conflict-engine contract'
    );
  });
