...
/**
 * #2529 — /sdd-settings-integrations: configure third-party search and review integrations.
 *
 * Covers:
 ...
 *   - /sdd:settings confirmation output mentions /sdd:settings-integrations
...
const COMMAND_PATH = path.join(REPO_ROOT, 'commands', 'sdd', 'config.md');
const WORKFLOW_PATH = path.join(REPO_ROOT, 'sdd', 'workflows', 'settings-integrations.md');
const SKILL_PATH = path.join(REPO_ROOT, '.claude', 'skills', 'sdd-settings-integrations.md');
const SETTINGS_WORKFLOW_PATH = path.join(REPO_ROOT, 'sdd', 'workflows', 'settings.md');
...
describe('#2529 artifacts', () => {
  ...
  test('config.md frontmatter declares name sdd:config and routes to --integrations', () => {
    const src = fs.readFileSync(COMMAND_PATH, 'utf-8');
    // #2790: consolidated command uses sdd:config name
    assert.match(src, /name:\s*sdd:config/);
    assert.match(src, /description:\s*.+/);
    assert.match(src, /allowed-tools:/);
    assert.match(src, /AskUserQuestion/);
  });

  test('workflow exists at sdd/workflows/settings-integrations.md', () => {
...
describe('#2529 workflow — agent_skills injection', () => {
  ...
  test('agent_skills.<valid-slug> passes validator', () => {
    assert.ok(isValidConfigKey('agent_skills.sdd-executor'));
    assert.ok(isValidConfigKey('agent_skills.sdd-planner'));
...
describe('#2529 config-set round-trip', () => {
  ...
  test('config-set writes brave_search, firecrawl, exa_search values to config.json', (t) => {
    ...
    t.after(() => cleanup(tmp));
    runSddTools(['config-ensure-section'], tmp);

    const r1 = runSddTools(['config-set', 'brave_search', 'BSKY-111111112222'], tmp);
    assert.ok(r1.success, `brave_search set failed: ${r1.error}`);
    const r2 = runSddTools(['config-set', 'firecrawl', 'fc-aaaaaaaabbbbcccc'], tmp);
    assert.ok(r2.success, `firecrawl set failed: ${r2.error}`);
    const r3 = runSddTools(['config-set', 'exa_search', 'ex-000011112222dddd'], tmp);
    assert.ok(r3.success, `exa_search set failed: ${r3.error}`);
    const r4 = runSddTools(['config-set', 'search_gitignored', 'true'], tmp);
  ...
  test('config-set round-trips agent_skills.<agent-type>', (t) => {
    ...
    const r = runSddTools(
      ['config-set', 'agent_skills.sdd-executor', 'skill-a,skill-b'],
      tmp
    );
    assert.ok(r.success, `agent_skills.sdd-executor set failed: ${r.error}`);
    const cfg = JSON.parse(fs.readFileSync(path.join(tmp, '.planning', 'config.json'), 'utf-8'));
    // Accept either array or string — validator accepts both shapes today.
    const v = cfg.agent_skills?.['sdd-executor'];
    assert.ok(v === 'skill-a,skill-b' || (Array.isArray(v) && v.join(',') === 'skill-a,skill-b'),
      `expected agent_skills.sdd-executor to contain both skills, got ${JSON.stringify(v)}`);
  });
});
...
describe('#2529 config merge safety', () => {
  test('setting brave_search preserves unrelated workflow.research key', (t) => {
    ...
    t.after(() => cleanup(tmp));
    runSddTools(['config-ensure-section'], tmp);
    runSddTools(['config-set', 'workflow.research', 'false'], tmp);

    const r = runSddTools(['config-set', 'brave_search', 'BSKY-preserve-me-9999'], tmp);
  ...
  test('setting agent_skills.sdd-executor preserves unrelated review.models.codex', (t) => {
    const tmp = createTempProject();
    t.after(() => cleanup(tmp));
    runSddTools(['config-ensure-section'], tmp);
    runSddTools(['config-set', 'review.models.codex', 'codex exec'], tmp);

    const r = runSddTools(['config-set', 'agent_skills.sdd-planner', 'a,b'], tmp);
    assert.ok(r.success, `set failed: ${r.error}`);

    const cfg = JSON.parse(fs.readFileSync(path.join(tmp, '.planning', 'config.json'), 'utf-8'));
    assert.strictEqual(cfg.review?.models?.codex, 'codex exec', 'unrelated review.models.codex must be preserved');
    assert.ok(cfg.agent_skills?.['sdd-planner'], 'agent_skills.sdd-planner must be set');
  });
});

// ─── /sdd-settings mentions /sdd-settings-integrations ──────────────────────

describe('#2529 /sdd-settings mentions new command', () => {
  test('settings workflow mentions canonical /sdd-config --integrations', () => {
    ...
    assert.ok(
      ...
      'settings.md must not mention the legacy /sdd-settings-integrations variant'
    );
  });
});
...
describe('#2529 negative — invalid inputs rejected', () => {
  ...
  test('config-set rejects agent_skills with path traversal', (t) => {
    ...
    const r = runSddTools(['config-set', 'agent_skills.../etc/passwd', 'x'], tmp);
...
describe('#2529 security — plaintext containment', () => {
  test('after setting brave_search, plaintext appears only in config.json', (t) => {
    ...
    const r = runSddTools(['config-set', 'brave_search', marker], tmp);
    assert.ok(r.success, `set failed: ${r.error}`);
  ...
  test('config-get masks secrets and never echoes plaintext for brave_search/firecrawl/exa_search', (t) => {
    ...
    for (const { key, marker } of cases) {
      const set = runSddTools(['config-set', key, marker], tmp);
      assert.ok(set.success, `${key} set failed: ${set.error}`);

      const get = runSddTools(['config-get', key], tmp);
