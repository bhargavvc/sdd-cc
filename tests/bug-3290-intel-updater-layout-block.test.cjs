// allow-test-rule: source-text-is-the-product — agents/sdd-intel-updater.md IS
 ...
 *
 * The "Runtime layout detection" block in sdd-intel-updater.md ran
 * unconditionally on every project analysed, emitting:
 *
 *   Layout detection returned "unknown" — this project is not a SDD-system
 *   installation (no `.claude/sdd/` or `.kilo/` runtime root).
 *
 * for every ordinary (non-SDD-framework) user project. The verdict was already
 * ignored by Steps 2-6 on non-SDD projects. The block was dead-but-noisy.
 *
 * Fix: gate the runtime bash detection on a positive "is-this-the-framework-
 * repo" check (package.json name === "@bhargavvc/sdd-cc") so it runs ONLY when
 * analysing the SDD framework's own repo, OR remove the block entirely if no
...
const AGENT_PATH = path.join(ROOT, 'agents', 'sdd-intel-updater.md');
...
describe('bug #3290 — Group A: layout-detection block must be gated or absent', () => {
  ...
  test('agent file exists', () => {
    assert.ok(fs.existsSync(AGENT_PATH), 'agents/sdd-intel-updater.md must exist');
  ...
  test(
    ...
    () => {
      ...
      // that the current project IS the SDD framework's own repo.
      const hasFrameworkGate =
        content.includes('@bhargavvc/sdd-cc') ||
        content.includes('is-this-the-framework') ||
        content.includes('framework repo') ||
        content.includes('Only run') ||
        /if.*package\.json.*sdd/i.test(content) ||
        /Only.*layout detection.*SDD framework/i.test(content) ||
        /Only.*layout detection.*framework/i.test(content);

      assert.ok(
        hasFrameworkGate,
        'agents/sdd-intel-updater.md contains a bare unconditional layout-detection ' +
...
describe('bug #3290 — Group B: layout-detection verdict has no downstream consumers', () => {
   ...
   * of the sdd-intel-updater layout detection (not general runtime references).
   *
  ...
  test('no file contains "Layout detection returned" (the noisy verdict phrase)', () => {
    ...
    assert.strictEqual(
      ...
      'phrase from the sdd-intel-updater layout-detection block). Found:\n' +
  ...
  test('no agent or workflow instructs reading the layout-detection verdict output', () => {
    ...
    // the producing file (sdd-intel-updater.md).
    ...
    for (const dir of SOURCE_DIRS) {
      ...
      for (const file of files) {
        ...
        if (path.basename(file) === 'sdd-intel-updater.md') continue;
        const src = fs.readFileSync(file, 'utf-8');
