...
//   1. Cache-stale: bare `npx -y @bhargavvc/sdd-cc@latest` hits npx's local
//      cache and may pull an older version instead of @latest.
//   2. Token-routing: Bash-tool wrappers misroute the `@` token in
//      `@bhargavvc/sdd-cc@latest`, causing npm to error with
//      "Unknown command: @bhargavvc/sdd-cc@latest".
//
// The robust form is:
//   npx -y --package=@bhargavvc/sdd-cc@latest -- @bhargavvc/sdd-cc $ARGS
//
...
const UPDATE_WF = path.join(ROOT, 'sdd', 'workflows', 'update.md');
