 ...
 *
 * Background: SDD 1.42.2 reported the user-visible failure
 *   > Skipped Codex skill-copy generation (Codex discovers official skills directly)
 * which left users with a "successful" install but no routable `$sdd-*`
 * entrypoints in Codex CLI 0.130.0. Codex CLI does NOT auto-discover
 * commands from `~/.codex/sdd/workflows/*.md` or `agents/*.md`;
 ...
 * commands/sdd/*.md, with Claude-flavored command frontmatter rewritten
...
// SDD_TEST_MODE neutralizes side-effecting branches (auto-detection, etc.).
...
/**
 * Walk commands/sdd/**\/*.md and return the set of skill names the installer
...
function runCodexInstallCaptured() {
  const codexHome = createTempDir('sdd-3582-codex-');
  const logs = [];
  const warnings = [];
...
describe('bug-3582: Codex global install materializes the skill surface', { concurrency: false }, () => {
  ...
  test('writes the exact expected set of sdd-*/SKILL.md skills (deepEqual on name set)', () => {
    ...
    // same-count `sdd-*` directory cannot pass.
    const expected = [...expectedSkillNames()].sort();
    assert.deepStrictEqual(
      [...actualNames].sort(),
      expected,
      `installed Codex skills must exactly match commands/sdd/**/*.md (one skill per command)`,
    );
  ...
  test('representative skills named in the issue report are present', () => {
    ...
    const representative = [
      'sdd-map-codebase',     // the literal command from the bug report
      'sdd-execute-phase',
      'sdd-plan-phase',
      'sdd-new-project',
      'sdd-health',
