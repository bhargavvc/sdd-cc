...
// tests the deployed contract. The commands/sdd/*.md reads in the helper are
 ...
 *   - sdd/workflows/help.md  (deprecation-trail prose)
...
const ALLOWED_HISTORICAL_MENTIONS = new Set([
  path.join(ROOT, 'sdd', 'workflows', 'help.md'),
...
// syntax placeholders — they match the /sdd-* regex but are NOT user-typable
...
const INTERNAL_COMPONENT_SLUGS = new Set([
  ...
  // (e.g. "/sdd-command-name [args]"). It is not a registered command.
  'command-name',
  'command',

  // sdd-tools.cjs — the legacy Node CLI binary (bin/sdd-tools.cjs).
  // Docs reference it as a path component in shell examples, not as a slash command.
  // Example: node "$HOME/.claude/sdd/bin/sdd-tools.cjs" state validate
  'tools',

  // Hook scripts — internal runtime hooks, not user-invocable slash commands.
  //   hooks/sdd-statusline.js       — session statusline hook
  //   hooks/sdd-context-monitor.js  — context-window monitor hook
  //   hooks/sdd-update-banner.js    — update-available banner hook
  //   hooks/sdd-graphify-update.sh  — knowledge-graph auto-update PostToolUse hook (#3347)
  // These appear in docs as file-path references (e.g. "sdd-statusline.js reads
  ...
  // sdd-update-check.json — background update-check CACHE FILE, not a slash command.
  // ARCHITECTURE.md references "~/.cache/sdd/sdd-update-check.json" as a path;
  // the regex captures "/sdd-update-check" from the path component.
  'update-check',

  // Internal agent names referenced in ARCHITECTURE.md tables of agents.
  // These are spawned agents (sdd-planner, etc.), not user-typable slash commands.
  'planner',

  // Malformed token from SDK init reference: "/sdd-init-" appears as a truncated
  // prefix in CLI-TOOLS.md describing the sdd-sdk init command family
  // (e.g., "sdd-sdk query init.phase-op 12"). The regex captures "/sdd-init-"
  // without a following slug — this is a documentation formatting artifact, not
  // a real command token.
  'init-',

  // sdd-build — GitHub organization name: "github.com/bhargavvc/sdd-cc".
  // Every occurrence of "/sdd-build" in docs is the path component of a GitHub URL
  // (e.g., "[#2792](https://github.com/bhargavvc/sdd-cc/issues/2792)").
  ...
  // The regex captures "/sdd-workspaces" from the path component. The LIVE slash
  // command is "/sdd-workspace" (singular) — not "/sdd-workspaces" (plural).
  'workspaces',

  // Portuguese translation of "command" — pt-BR/ARCHITECTURE.md uses "/sdd-comando"
  // as the localized equivalent of the "/sdd-command-name" English placeholder
  // in an architecture flow diagram. Not a registered command.
  'comando',

  // GitHub repository name: zh-CN/README.md references "github.com/rokicool/sdd-opencode"
  // as an external community project URL. The regex captures "/sdd-opencode" from
  // the URL path. Not a user-typable slash command in this product.
  'opencode',

  // Smoke-test directory path — locale docs reference "/tmp/sdd-smoke-$(date +%s)"
  // as a temporary directory path in bash code-block examples. The regex captures
  // "/sdd-smoke-" from the filesystem path. Not a slash command.
  'smoke-',

  // Template placeholders — zh-CN/references/ui-brand.md used "/sdd-alternative-1"
  // and "/sdd-alternative-2" as unfilled placeholders in a UI template example.
...
function extractCommandTokens(content) {
  ...
  function isInternal(token) {
    ...
    const slug = token.replace(/^(?:\/sdd[:-]|\$sdd-)/, '');
  ...
  const allSlash = (stripped.match(/\/sdd-[a-z0-9][a-z0-9-]*/g) || []);
  const allColon = (stripped.match(/\/sdd:[a-z0-9][a-z0-9-]*/g) || []);
  const allDollar = (stripped.match(/\$sdd-[a-z0-9][a-z0-9-]*/g) || []);
...
describe('getLiveCommandTokens() — helper contract', () => {
  ...
  test('returns a non-empty set (commands/sdd/ has registered commands)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.size > 0, 'live registry must contain at least one token');
  });

  test('contains /sdd-help (from commands/sdd/help.md name: sdd:help)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.has('/sdd-help'), 'registry must contain /sdd-help');
  });

  test('contains /sdd:help (Gemini form)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.has('/sdd:help'), 'registry must contain /sdd:help');
  });

  test('contains $sdd-help (Codex form)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.has('$sdd-help'), 'registry must contain $sdd-help');
  });

  test('contains /sdd-plan-phase (from commands/sdd/plan-phase.md)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.has('/sdd-plan-phase'), 'registry must contain /sdd-plan-phase');
  });

  test('contains exactly 3 tokens per slug (slash, colon, dollar)', () => {
    const result = getLiveCommandTokens();
    // Every /sdd-slug should have a matching /sdd:slug and $sdd-slug
    let tokenCount = 0;
    for (const token of result) {
      if (token.startsWith('/sdd-')) tokenCount++;
    }
    const slashTokens = [...result].filter(t => t.startsWith('/sdd-'));
    for (const slash of slashTokens) {
      const slug = slash.slice('/sdd-'.length);
      assert.ok(
        result.has(`/sdd:${slug}`),
        `registry must contain Gemini form /sdd:${slug} for slash form ${slash}`
      );
      assert.ok(
        result.has(`$sdd-${slug}`),
        `registry must contain Codex form $sdd-${slug} for slash form ${slash}`
      );
    }
  });

  test('does NOT contain removed /sdd-reapply-patches', () => {
    const result = getLiveCommandTokens();
    assert.ok(!result.has('/sdd-reapply-patches'), 'registry must NOT contain removed /sdd-reapply-patches');
  });

  test('does NOT contain removed /sdd-code-review-fix', () => {
    const result = getLiveCommandTokens();
    assert.ok(!result.has('/sdd-code-review-fix'), 'registry must NOT contain removed /sdd-code-review-fix');
  });

  test('does NOT contain removed /sdd-status', () => {
    const result = getLiveCommandTokens();
    assert.ok(!result.has('/sdd-status'), 'registry must NOT contain removed /sdd-status');
  });
...
describe('getLiveCommandTokens() — fixture contract', () => {
  test('parses sdd:foo frontmatter and emits 3 canonical tokens', () => {
    // This test validates the parsing logic against a known-good fixture
    // by inspecting the live registry for commands/sdd/help.md (name: sdd:help).
    // Fixture file tests are done inline since the helper reads commands/sdd/ only.
    // The canonical token contract:
    //   name: sdd:foo → /sdd-foo, /sdd:foo, $sdd-foo
    const registry = getLiveCommandTokens();
    // We know help.md has name: sdd:help
    const slug = 'help';
    assert.ok(registry.has(`/sdd-${slug}`), `must have /sdd-${slug}`);
    assert.ok(registry.has(`/sdd:${slug}`), `must have /sdd:${slug}`);
    assert.ok(registry.has(`$sdd-${slug}`), `must have $sdd-${slug}`);
  });

  test('parses sdd-slug frontmatter (ns-* commands) and emits 3 tokens', () => {
    // ns-context.md has name: sdd-context (dash-style, no colon)
    const registry = getLiveCommandTokens();
    assert.ok(registry.has('/sdd-context'), 'must have /sdd-context (from ns-context.md)');
    assert.ok(registry.has('/sdd:context'), 'must have /sdd:context (Gemini form)');
    assert.ok(registry.has('$sdd-context'), 'must have $sdd-context (Codex form)');
  });
});
...
describe('adversarial: polarity inversion catches drift deny-list misses', () => {
  test('renaming a live command without updating docs would fail this test (demonstrated via token absence)', () => {
    // If /sdd-progress were renamed to /sdd-status-new, the old /sdd-progress
    // token would not appear in the live registry, and any doc referencing
    // /sdd-progress would fail. The deny-list shape would have passed silently
    ...
    const registry = getLiveCommandTokens();
    assert.ok(registry.has('/sdd-progress'), '/sdd-progress must be live (not renamed to /sdd-status)');
    assert.ok(!registry.has('/sdd-status'), '/sdd-status must be absent (was deleted, replaced by /sdd-progress)');
  });

  test('freshly-deleted command /sdd-check-todos is absent from registry', () => {
    const registry = getLiveCommandTokens();
    assert.ok(!registry.has('/sdd-check-todos'), '/sdd-check-todos must not be in the live registry');
  });

  test('freshly-deleted command /sdd-new-workspace is absent from registry', () => {
    const registry = getLiveCommandTokens();
    assert.ok(!registry.has('/sdd-new-workspace'), '/sdd-new-workspace must not be in the live registry');
  });

  test('freshly-deleted command /sdd-plan-milestone-gaps is absent from registry', () => {
    const registry = getLiveCommandTokens();
    assert.ok(!registry.has('/sdd-plan-milestone-gaps'), '/sdd-plan-milestone-gaps must not be in the live registry');
  });

  test('freshly-deleted command /sdd-research-phase is absent from registry', () => {
    const registry = getLiveCommandTokens();
    assert.ok(!registry.has('/sdd-research-phase'), '/sdd-research-phase must not be in the live registry');
  });
});
