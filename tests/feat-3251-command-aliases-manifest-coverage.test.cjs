...
const SDD_TOOLS = path.join(REPO_ROOT, 'sdd', 'bin', 'sdd-tools.cjs');
...
function createProject() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3251-dispatch-'));
  fs.mkdirSync(path.join(dir, '.planning', 'phases'), { recursive: true });
  return dir;
}

function runSddTools(args, cwd) {
  return spawnSync(process.execPath, [SDD_TOOLS, ...args], {
    cwd,
    encoding: 'utf8',
    timeout: 30000,
    killSignal: 'SIGKILL',
...
describe('feat-3251: generated aliases dispatch through real sdd-tools behavior', () => {
  test('phase.mvp-mode spaced alias resolves CLI flag precedence', () => {
    const projectDir = createProject();
    try {
      const result = runSddTools(['phase', 'mvp-mode', '1', '--cli-flag'], projectDir);
  ...
  test('phase.mvp-mode spaced alias resolves ROADMAP mode without mutating files', () => {
    ...
    try {
      ...
      const result = runSddTools(['phase', 'mvp-mode', '1'], projectDir);
      assert.equal(result.status, 0, result.stderr);
  ...
  test('phase.mvp-mode JSON error is typed and leaves project files untouched', () => {
    ...
    try {
      const beforeFiles = snapshotProjectState(projectDir);
      const result = runSddTools(['--json-errors', 'phase', 'mvp-mode'], projectDir);
