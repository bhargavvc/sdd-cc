 ...
 *
 * The sdd-code-fixer agent (spawned by /sdd-code-review-fix) operated directly
 ...
 *
 * This mirrors the pattern already used by every other per-issue SDD agent at
 * /private/tmp/sv-<n>.
 */
...
// The sdd-code-fixer agent's working instructions ARE the product — Claude
...
describe('bug-2686: review-fix agent worktree isolation', () => {
  ...
  before(() => {
    const agentPath = path.join(__dirname, '..', 'agents', 'sdd-code-fixer.md');
    assert.ok(fs.existsSync(agentPath), 'agents/sdd-code-fixer.md must exist');
  ...
  test('agent instructions include git worktree add before any branch-switching checkout or commit', () => {
    ...
    assert.ok(
      worktreePos !== -1,
      'sdd-code-fixer.md must include a "git worktree add" instruction to isolate operations from the main working tree (#2686)'
    );
    ...
    // either `git commit` directly or `sdd-sdk query commit`
    const commitMatch = /(?:git commit|sdd-sdk query commit)/.exec(agentContent);
    if (commitMatch) {
  ...
  test('agent instructions include worktree cleanup after completion', () => {
    assert.ok(
      ...
      'sdd-code-fixer.md must include worktree cleanup (git worktree remove) to avoid leaking tmp directories (#2686)'
    );
  });
  ...
  test('agent instructions use a /tmp path for the worktree', () => {
    ...
    assert.ok(
      hasTmpWorktreePath,
      'sdd-code-fixer.md must define a worktree variable at a /tmp/sv-... path, consistent with other SDD agents (#2686)'
    );
  });
});
