 ...
 *
 * Root cause: test code embedded space-containing paths into runSddTools()
 ...
 * runSddTools() so execFileSync receives the full path as a single argv slot.
 *
...
describe('bug-3509: frontmatter get/set/merge/validate survive spaces in file path', () => {
  ...
  test('frontmatter get returns parsed fields when file path contains spaces', () => {
    const result = runSddTools(['frontmatter', 'get', tmpFile]);
  ...
  test('frontmatter set works when file path contains spaces', () => {
    const setResult = runSddTools(['frontmatter', 'set', tmpFile, '--field', 'phase', '--value', '02']);
    ...
    const getResult = runSddTools(['frontmatter', 'get', tmpFile]);
  ...
  test('frontmatter validate works when file path contains spaces', () => {
    ...
    const result = runSddTools(['frontmatter', 'validate', tmpFile, '--schema', 'plan']);
...
describe('bug-3509: scan-sessions --path survives spaces in path', () => {
  ...
  test('scan-sessions --path with spaces returns empty array, not path-split error', () => {
    ...
    const result = runSddTools(['scan-sessions', '--path', sessionsDir, '--raw'], tmpDir);
