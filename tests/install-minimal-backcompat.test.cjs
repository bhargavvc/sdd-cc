...
describe('install-minimal-backcompat: --minimal and --profile=core produce the same manifest skill count', () => {
  function installAndGetManifest(extraArgs) {
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-backcompat-'));
    try {
      spawnSync(
        process.execPath,
      ...
      // Read marker
      const markerPath = path.join(targetDir, '.sdd-profile');
  ...
  test('--profile=standard writes .sdd-profile marker with "standard"', () => {
