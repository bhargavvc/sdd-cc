 ...
 *
 * Cline is listed in SDD documentation as a supported runtime but was
 * completely absent from bin/install.js. Running `npx @bhargavvc/sdd-cc`
 ...
 * - Installs sdd/ engine with path replacement
 */
...
describe('Cline markdown conversion', () => {
  ...
  test('replaces .claude/ paths with .cline/', () => {
    const result = convertClaudeToCliineMarkdown('See ~/.claude/sdd/');
  ...
  test('replaces .claude/skills/ with .cline/skills/', () => {
    const result = convertClaudeToCliineMarkdown('skills at .claude/skills/sdd-executor');
...
describe('Cline install (local)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('sdd-cline-test-');
  ...
  test('.clinerules contains SDD instructions', () => {
    install(false, 'cline');
    const clinerules = path.join(tmpDir, '.clinerules');
    const content = fs.readFileSync(clinerules, 'utf8');
    assert.ok(content.includes('SDD') || content.includes('sdd'), '.clinerules must reference SDD');
  });

  test('install creates sdd engine directory', () => {
    install(false, 'cline');
    const engineDir = path.join(tmpDir, 'sdd');
    assert.ok(fs.existsSync(engineDir), 'sdd directory must exist after install');
  });
    ...
    function scanDir(dir) {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          // Check for SDD install paths that should have been substituted.
          ...
          const hasLeaked = /~\/\.claude\/(?:sdd|commands|agents|hooks)|HOME\/\.claude\/(?:sdd|commands|agents|hooks)/.test(content);
          assert.ok(!hasLeaked, `Found leaked SDD .claude install path in ${fullPath}`);
        }
      }
    }
    scanDir(engineDir);
