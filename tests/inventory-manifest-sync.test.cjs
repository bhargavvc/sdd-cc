...
const FAMILIES = [
  { name: 'agents',      dir: path.join(ROOT, 'agents'),                           filter: (f) => /^sdd-.*\.md$/.test(f),  toName: (f) => f.replace(/\.md$/, '') },
  { name: 'commands',    dir: path.join(ROOT, 'commands', 'sdd'),                  filter: (f) => f.endsWith('.md'),        toName: (f) => '/sdd-' + f.replace(/\.md$/, '') },
  { name: 'workflows',   dir: path.join(ROOT, 'sdd', 'workflows'),        filter: (f) => f.endsWith('.md'),        toName: (f) => f },
  { name: 'references',  dir: path.join(ROOT, 'sdd', 'references'),       filter: (f) => f.endsWith('.md'),        toName: (f) => f },
  { name: 'cli_modules', dir: path.join(ROOT, 'sdd', 'bin', 'lib'),       filter: (f) => f.endsWith('.cjs'),       toName: (f) => f },
