...
/**
 * SDD Tests — /sdd-sketch --wrap-up silently no-ops (#2949)
 *
 * The --wrap-up flag was documented in commands/sdd/sketch.md but never dispatched.
...
const SKETCH_COMMAND = path.join(ROOT, 'commands/sdd/sketch.md');
const SKETCH_WORKFLOW = path.join(ROOT, 'sdd/workflows/sketch.md');

describe('bug-2949: sketch --wrap-up dispatch wiring', () => {
  test('commands/sdd/sketch.md contains --wrap-up dispatch logic', () => {
  ...
  test('commands/sdd/sketch.md has sketch-wrap-up in execution_context section', () => {
    const content = fs.readFileSync(SKETCH_COMMAND, 'utf8');
  ...
  test('workflows/sketch.md does NOT contain old /sdd-sketch-wrap-up form', () => {
    const content = fs.readFileSync(SKETCH_WORKFLOW, 'utf8');
    assert.ok(
      !content.includes('/sdd-sketch-wrap-up'),
      'workflows/sketch.md must not reference the old /sdd-sketch-wrap-up command'
    );
  });

  test('workflows/sketch.md DOES contain new /sdd-sketch --wrap-up form', () => {
    const content = fs.readFileSync(SKETCH_WORKFLOW, 'utf8');
    assert.ok(
      content.includes('/sdd-sketch --wrap-up'),
      'workflows/sketch.md should reference /sdd-sketch --wrap-up (the new form)'
    );
  });
});
