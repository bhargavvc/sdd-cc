...
const scanPhasePlans = require('../sdd/bin/lib/plan-scan.cjs');
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-plan-scan-'));
});
...
describe('scanPhasePlans — flat layout', () => {
  ...
  test('extended-layout root file (5-PLAN-01-setup.md style)', () => {
    ...
    // e.g. sdd-plan-phase writes "5-PLAN-01-setup.md".
    const dir = phaseDir();
