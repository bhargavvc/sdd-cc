 ...
 *
 * `/sdd-spike --wrap-up` was silently no-oping because:
 * 1. `commands/sdd/spike.md` listed `--wrap-up` as a flag but had no dispatch block.
 * 2. `workflows/spike.md` still referenced the deleted `/sdd-spike-wrap-up` entry-point
 *    instead of the correct `/sdd-spike --wrap-up` form.
 *
 * Fix:
 * - `commands/sdd/spike.md` now has a dispatch block that routes `--wrap-up` to
 *   spike-wrap-up.md, and spike-wrap-up.md is listed in execution_context so the
 *   runtime can find it.
 * - `workflows/spike.md` companion references updated from `/sdd-spike-wrap-up` to
 *   `/sdd-spike --wrap-up`.
 */
...
const SPIKE_CMD_PATH = path.join(__dirname, '..', 'commands', 'sdd', 'spike.md');
const SPIKE_WORKFLOW_PATH = path.join(__dirname, '..', 'sdd', 'workflows', 'spike.md');
...
describe('bug-2948: /sdd-spike --wrap-up dispatch wiring', () => {
  describe('commands/sdd/spike.md — frontmatter and section contract', () => {
    test('spike.md command file exists and has valid frontmatter', () => {
      assert.ok(fs.existsSync(SPIKE_CMD_PATH), 'commands/sdd/spike.md should exist');
  ...
  describe('sdd/workflows/spike.md — companion references', () => {
    test('spike workflow file exists', () => {
      assert.ok(fs.existsSync(SPIKE_WORKFLOW_PATH), 'sdd/workflows/spike.md should exist');
    });

    test('does NOT reference the deleted /sdd-spike-wrap-up entry-point', () => {
      const fm = parseFrontmatter(fs.readFileSync(SPIKE_WORKFLOW_PATH, 'utf-8'));
      assert.ok(
        !fm._body.includes('/sdd-spike-wrap-up'),
        'workflows/spike.md must not reference the deleted /sdd-spike-wrap-up command; use /sdd-spike --wrap-up instead'
      );
    });

    test('references /sdd-spike --wrap-up as the canonical wrap-up invocation', () => {
      const fm = parseFrontmatter(fs.readFileSync(SPIKE_WORKFLOW_PATH, 'utf-8'));
      assert.ok(
        fm._body.includes('/sdd-spike --wrap-up'),
        'workflows/spike.md must reference /sdd-spike --wrap-up as the canonical wrap-up command'
      );
    });
  });
});
