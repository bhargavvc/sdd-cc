...
/**
 * Bug #2962: --sdk install flag on Windows leaves sdd-sdk un-shimmed.
 *
 ...
 *
 *   2. trySelfLinkSddSdkWindows — fs/spawn driver that calls the IR builder
...
describe('Bug #2962: buildWindowsShimTriple — pure IR builder', () => {
  test('resolves shimSrc to an absolute path on the invocation.target field', () => {
    const shimSrc = path.join(ROOT, 'bin', 'sdd-sdk.js');
  ...
  test('produces a structured IR with the documented shape', () => {
    const triple = installModule.buildWindowsShimTriple(path.join(ROOT, 'bin', 'sdd-sdk.js'));
  ...
  test('declares the standard npm-style filenames for the shim triple', () => {
    const triple = installModule.buildWindowsShimTriple(path.join(ROOT, 'bin', 'sdd-sdk.js'));
    assert.deepEqual(triple.fileNames, { cmd: 'sdd-sdk.cmd', ps1: 'sdd-sdk.ps1', sh: 'sdd-sdk' });
  });
...
describe('Bug #2962: trySelfLinkSddSdkWindows — fs/spawn driver', () => {
  let tmpDir;
  let origExecSync;

  before(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2962-'));
  ...
  test('returns the .cmd path on success and writes all three shim files', () => {
    const shimSrc = path.join(ROOT, 'bin', 'sdd-sdk.js');
    const triple = installModule.buildWindowsShimTriple(shimSrc);
    const result = installModule.trySelfLinkSddSdkWindows(shimSrc);
  ...
  test('the rendered file size matches the IR renderer\'s output length (renderer drives the writer)', () => {
    ...
    const shimSrc = path.join(ROOT, 'bin', 'sdd-sdk.js');
    const triple = installModule.buildWindowsShimTriple(shimSrc);
    installModule.trySelfLinkSddSdkWindows(shimSrc);
  ...
  test('replaces stale shims atomically (mtime advances on rewrite)', () => {
    const shimSrc = path.join(ROOT, 'bin', 'sdd-sdk.js');
    installModule.trySelfLinkSddSdkWindows(shimSrc);
    const cmdPath = path.join(tmpDir, 'sdd-sdk.cmd');
  ...
  test('returns null when npm prefix -g fails', () => {
    ...
    try {
      const result = installModule.trySelfLinkSddSdkWindows(path.join(ROOT, 'bin', 'sdd-sdk.js'));
