 ...
 * commands/sdd/ for the slash menu to read on older configurations.
 *
...
describe('Bug #2957: claude+global post-install message', () => {
  test('claude+global message tells the user to restart and offers skill-name fallback', () => {
    ...
    assert.match(output, /\/sdd-new-project/, 'should still mention /sdd-new-project');
    assert.match(output, /sdd-new-project skill/i, 'should mention the skill name fallback');
    assert.doesNotMatch(
      output,
  ...
  test('claude+local message keeps the original /sdd-new-project instruction', () => {
