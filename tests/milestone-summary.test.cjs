...
const commandPath = path.join(repoRoot, 'commands', 'sdd', 'milestone-summary.md');
const workflowPath = path.join(repoRoot, 'sdd', 'workflows', 'milestone-summary.md');

describe('milestone-summary command', () => {
  test('command file exists', () => {
    assert.ok(fs.existsSync(commandPath), 'commands/sdd/milestone-summary.md should exist');
  });

  test('command has correct frontmatter name', () => {
    const content = fs.readFileSync(commandPath, 'utf-8');
    assert.ok(content.includes('name: sdd:milestone-summary'), 'should have correct command name');
  });
...
describe('milestone-summary workflow', () => {
  ...
  test('workflow updates STATE.md', () => {
    ...
    assert.ok(
      ...
      'should update STATE.md via state record-session (CJS or sdd-sdk query)'
    );
  });
...
describe('milestone-summary fixture-based artifact discovery', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-ms-test-'));
  });
