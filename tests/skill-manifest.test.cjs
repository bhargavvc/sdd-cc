...
describe('skill-manifest', () => {
  ...
  beforeEach(() => {
    ...
    homeDir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'sdd-skill-manifest-home-'));

    writeSkill(path.join(tmpDir, '.claude', 'skills'), 'project-claude', 'Project Claude skill');
    writeSkill(path.join(tmpDir, '.claude', 'skills'), 'sdd-help', 'Installed SDD skill');
    ...
    writeSkill(
      path.join(homeDir, '.claude', 'sdd', 'skills'),
      'legacy-import',
      'Deprecated import-only skill'
    );

    fs.mkdirSync(path.join(homeDir, '.claude', 'commands', 'sdd'), { recursive: true });
    fs.writeFileSync(path.join(homeDir, '.claude', 'commands', 'sdd', 'help.md'), '# legacy');
  });
  ...
  test('returns normalized inventory across canonical roots', () => {
    const result = runSddTools(['skill-manifest'], tmpDir, { HOME: homeDir });
    ...
    const sddSkill = manifest.skills.find((skill) => skill.name === 'sdd-help');
    assert.strictEqual(sddSkill.installed, true);
    ...
    assert.strictEqual(manifest.installation.sdd_skills_installed, true);
  ...
  test('writes manifest to .planning/skill-manifest.json when --write flag is used', () => {
    const result = runSddTools(['skill-manifest', '--write'], tmpDir, { HOME: homeDir });
    assert.ok(result.success, `Command should succeed: ${result.error || result.output}`);
  ...
  test('global roots honor runtime-home env overrides instead of hardcoded home paths', () => {
    const result = runSddTools(['skill-manifest'], tmpDir, {
      HOME: homeDir,
