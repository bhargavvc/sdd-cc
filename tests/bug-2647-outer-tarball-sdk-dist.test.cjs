 ...
 *
 * v1.38.3 of @bhargavvc/sdd-cc shipped with:
 *   - `files` array missing `sdk/dist`
 *   - `prepublishOnly` only running `build:hooks`, not `build:sdk`
 *
 * Result: the published tarball had no `sdk/dist/cli.js`. The `sdd-sdk`
 * bin shim in `bin/sdd-sdk.js` resolves `<pkg>/sdk/dist/cli.js`, which
 * didn't exist, so PATH fell through to the separately installed
 * `@bhargavvc/sdk@0.1.0` (predates the `query` subcommand).
 *
 * Every `sdd-sdk query <noun>` call in workflow docs thus failed on
 * fresh installs of 1.38.3.
 *
 * This test guards the OUTER package.json (@bhargavvc/sdd-cc) so future
...
const SHIM_PATH = path.join(REPO_ROOT, 'bin', 'sdd-sdk.js');

describe('bug #2647: outer tarball ships sdk/dist so sdd-sdk query works', () => {
  ...
  test('sdd-sdk bin shim resolves sdk/dist/cli.js', () => {
    assert.ok(
      pkg.bin && pkg.bin['sdd-sdk'] === 'bin/sdd-sdk.js',
      `package.json bin["sdd-sdk"] must point at bin/sdd-sdk.js. Got: ${JSON.stringify(pkg.bin)}`,
    );
    const shim = fs.readFileSync(SHIM_PATH, 'utf-8');
    assert.ok(
      /sdk['"],\s*['"]dist['"],\s*['"]cli\.js/.test(shim) ||
        /sdk\/dist\/cli\.js/.test(shim),
      'bin/sdd-sdk.js must resolve ../sdk/dist/cli.js — otherwise shipping sdk/dist does not help',
    );
  });

  test('npm pack dry-run includes sdk/dist/cli.js after build:sdk', { timeout: 180_000 }, () => {
    const npmCache = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-npm-cache-'));
    ...
    try {
      ...
      assert.ok(
        cliPresent,
        `npm pack must include sdk/dist/cli.js in the tarball (so "sdd-sdk query" resolves after install). sdk/dist entries found: ${files.filter((p) => p.startsWith('sdk/dist')).length}`,
      );
    } finally {
