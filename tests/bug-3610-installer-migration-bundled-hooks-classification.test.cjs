/**
 * Regression test for #3610: fresh `npx @bhargavvc/sdd-cc@latest --codex`
 * hard-aborts when the target ~/.codex/hooks/ contains the bundled SDD
 * hook files (`sdd-check-update-worker.js`, `sdd-prompt-guard.js`, …)
 * left over from a previous version. The installer-migration report
 * classifies them as "SDD-looking file is not proven manifest-managed
 * and needs explicit user choice" and `assertInstallerMigrationsUnblocked`
 * throws.
 *
 * The files in question are NOT user-owned — they are the SDD bundled
 * hooks shipped under `hooks/sdd-*` in the npm package. The fix adds a
 * `bundled-sdd-hook` classification to `classifyPromptUserAction` so the
...
} = require('../sdd/bin/lib/installer-migration-report.cjs');
...
// abort. Each is a real `hooks/sdd-*` file shipped under hooks/ in the npm
// package (verified by `ls hooks/`).
const BUNDLED_HOOK_RELPATHS = [
  'hooks/sdd-check-update-worker.js',
  'hooks/sdd-check-update.js',
  'hooks/sdd-context-monitor.js',
  'hooks/sdd-phase-boundary.sh',
  'hooks/sdd-prompt-guard.js',
  'hooks/sdd-read-guard.js',
  'hooks/sdd-read-injection-scanner.js',
  'hooks/sdd-session-state.sh',
  'hooks/sdd-statusline.js',
  'hooks/sdd-update-banner.js',
  'hooks/sdd-validate-commit.sh',
  'hooks/sdd-workflow-guard.js',
];

describe('bug #3610: classifyPromptUserAction recognizes bundled SDD hooks', () => {
  test('classifies hooks/sdd-*.js as bundled-sdd-hook → remove', () => {
    const result = classifyPromptUserAction({
      relPath: 'hooks/sdd-prompt-guard.js',
    });
    assert.ok(result, 'classifier returned null for a bundled SDD hook (.js)');
    assert.strictEqual(result.category, 'bundled-sdd-hook');
    assert.strictEqual(
      result.choice,
      'remove',
  ...
  test('classifies hooks/sdd-*.sh as bundled-sdd-hook → remove', () => {
    const result = classifyPromptUserAction({
      relPath: 'hooks/sdd-validate-commit.sh',
    });
    assert.ok(result);
    assert.strictEqual(result.category, 'bundled-sdd-hook');
  ...
  test('does NOT classify non-sdd hooks (preserves user-owned hook files)', () => {
    ...
    assert.strictEqual(
      result,
      null,
      'non-sdd-prefixed hook must NOT auto-classify (would clobber user files)',
    );
  });

  test('does NOT classify deeper paths under hooks/sdd-* (e.g. hooks/lib/) as bundled-sdd-hook', () => {
    // The bundled SDD distribution has hooks/lib/ (helper modules). Those
    // are managed differently — verify the classifier limits itself to
    // top-level hooks/sdd-<name>.<ext> files, not nested directories.
    const result = classifyPromptUserAction({
      relPath: 'hooks/sdd-helpers/index.js',
    });
...
describe('bug #3610: fresh upgrade with leftover bundled hooks does not throw', () => {
  ...
  beforeEach(() => {
    configDir = createTempDir('sdd-3610-');
  });
  ...
  test('end-to-end: 12 leftover bundled hooks + empty manifest → resolver clears all blockers', () => {
    // Recreate the reporter's environment: 12 bundled `sdd-*` hook files
    ...
    assert.deepStrictEqual(
      ...
      'precondition: every leftover hooks/sdd-* should be a prompt-user blocker',
    );
    ...
    for (const entry of resolved.resolutions) {
      assert.strictEqual(entry.category, 'bundled-sdd-hook');
