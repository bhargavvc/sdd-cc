...
/**
 * Bug #2973: /sdd-profile-user --refresh writes dev-preferences.md to the
 * legacy commands/sdd subdirectory, contradicting v1.39.0's skills-only
 * migration claim that "Legacy commands/sdd directory removed
 * (replaced by skills/)".
 *
 * Root cause: the writer at sdd/bin/lib/profile-output.cjs
 * fell back to commands/sdd/dev-preferences.md when no --output was passed.
 * The /sdd-profile-user workflow does not pass --output, so every refresh
 * deterministically re-creates the legacy directory.
 *
 * Fix:
 *   1. profile-output.cjs default targets skills/sdd-dev-preferences/SKILL.md
...
const PROFILE_OUTPUT = path.join(ROOT, 'sdd', 'bin', 'lib', 'profile-output.cjs');
const WORKFLOW = path.join(ROOT, 'sdd', 'workflows', 'profile-user.md');
const INSTALL = path.join(ROOT, 'bin', 'install.js');

describe('Bug #2973: dev-preferences default writer path is skills/sdd-dev-preferences/SKILL.md', () => {
  test('exercise the writer in a subprocess with HOME pointed at a tmp dir; assert the artifact lands at the skills path', () => {
    ...
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2973-'));
    try {
      ...
      const expectedPath = path.join(tmpHome, '.claude', 'skills', 'sdd-dev-preferences', 'SKILL.md');
      ...
      const legacyPath = path.join(tmpHome, '.claude', 'commands', 'sdd', 'dev-preferences.md');
...
describe('Bug #2973: profile-user.md confirmation message references the skills path', () => {
  test('the Display message points at $HOME/.claude/skills/sdd-dev-preferences/SKILL.md', () => {
    const md = fs.readFileSync(WORKFLOW, 'utf-8');
    // Match the structured Display: line; capture the path value.
    const m = md.match(/Display:\s*"[^"]*Generated\s*\/sdd-dev-preferences\s*at\s*([^"]+)"/);
    assert.notEqual(m, null, 'expected a Display: "Generated /sdd-dev-preferences at <path>" line');
    const referencedPath = m[1].trim();
    assert.equal(referencedPath, '$HOME/.claude/skills/sdd-dev-preferences/SKILL.md',
      `workflow references ${referencedPath}; expected skills path (#2973)`);
  });

  test('no occurrence of the legacy commands/sdd/dev-preferences.md path remains in profile-user.md', () => {
    const md = fs.readFileSync(WORKFLOW, 'utf-8');
    assert.equal(md.includes('commands/sdd/dev-preferences.md'), false,
      'profile-user.md still references legacy commands/sdd/dev-preferences.md (#2973)');
  });
});

describe('Bug #2973: installer migrates existing legacy dev-preferences.md to skills/sdd-dev-preferences/SKILL.md', () => {
  ...
  test('migration writes to skills/sdd-dev-preferences/SKILL.md when no skill exists yet', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2973-mig-'));
    try {
      const inst = require(INSTALL);
      ...
      const skillFile = path.join(tmpDir, 'skills', 'sdd-dev-preferences', 'SKILL.md');
  ...
  test('migration is a no-op when a SKILL.md already exists at the new location (do not clobber user-customized skill content)', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2973-skip-'));
    try {
      const inst = require(INSTALL);
      const skillDir = path.join(tmpDir, 'skills', 'sdd-dev-preferences');
...
describe('Bug #2973 (#3003 CR): copyCommandsAsClaudeSkills snapshots sdd-dev-preferences across the wipe', () => {
  test('user-customized skills/sdd-dev-preferences/SKILL.md survives a wipe-and-replace install', () => {
    const inst = require(INSTALL);
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2973-wipe-'));
    try {
      const skillsDir = path.join(tmp, 'skills');
      const userSkillDir = path.join(skillsDir, 'sdd-dev-preferences');
      fs.mkdirSync(userSkillDir, { recursive: true });
      const userContent = '# my customized dev preferences\n\nstack: rust\n';
      fs.writeFileSync(path.join(userSkillDir, 'SKILL.md'), userContent);

      // Source dir mimicking commands/sdd/ — does NOT contain dev-preferences
      ...
      // Without the CR fix, the wipe loop deletes sdd-dev-preferences/
      // and the user's content is lost (no source to restore from).
      inst.copyCommandsAsClaudeSkills(srcDir, skillsDir, 'sdd', '$HOME/.claude/', 'claude', true);

      const skillFile = path.join(userSkillDir, 'SKILL.md');
      assert.equal(fs.existsSync(skillFile), true,
        'sdd-dev-preferences/SKILL.md must survive the wipe (#3003 CR)');
  ...
  test('non-user-owned sdd-* skills are still wiped and recreated from source', () => {
    ...
    const inst = require(INSTALL);
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2973-wipe-shipped-'));
    try {
      const skillsDir = path.join(tmp, 'skills');
      const staleSkillDir = path.join(skillsDir, 'sdd-plan-phase');
