 ...
 *
 * All 85 SDD SKILL.md files declared `name: sdd:<cmd>` (colon), the deprecated
 * form. Claude Code surfaces the `name:` frontmatter field in autocomplete, so
 * users saw `/sdd:add-phase` suggestions instead of the canonical `/sdd-add-phase`.
 *
 * Root cause: skillFrontmatterName() in bin/install.js converted hyphenated
 * skill dir names to colon form (sdd-add-phase → sdd:add-phase) because
 * workflows called Skill(skill="sdd:<cmd>"). That was the original fix for
 ...
 * 2. Installed SKILL.md would emit name: sdd-<cmd> (not sdd:<cmd>).
 * 3. No workflow contains a Skill(skill="sdd:<cmd>") colon call.
 */
...
describe('bug-2808: SKILL.md name: uses hyphen form', () => {
  test('skillFrontmatterName returns hyphen form (not colon)', () => {
    assert.strictEqual(skillFrontmatterName('sdd-add-phase'), 'sdd-add-phase');
    assert.strictEqual(skillFrontmatterName('sdd-plan-phase'), 'sdd-plan-phase');
    assert.strictEqual(skillFrontmatterName('sdd-autonomous'), 'sdd-autonomous');
  });

  test('generated SKILL.md contains name: sdd-<cmd> (not sdd:<cmd>)', () => {
    const cmdFiles = fs.readdirSync(COMMANDS_DIR).filter(f => f.endsWith('.md'));
    assert.ok(cmdFiles.length > 0, 'expected SDD command files');

    for (const cmd of cmdFiles) {
      const base = cmd.replace(/\.md$/, '');
      const skillDirName = 'sdd-' + base;
      ...
      assert.ok(
        name.startsWith('sdd-'),
        `${cmd}: SKILL.md name should start with sdd-, got "${name}"`
      );
    }
  });

  test('no workflow contains Skill(skill="sdd:<cmd>") colon form', () => {
    ...
    for (const f of workflowFiles) {
      ...
      for (const line of stripped.split('\n')) {
        ...
        // and the `=` so variants like `Skill( skill = "sdd:foo" )` are still
        ...
        // `sdd:extract_learnings`. A character-class that excludes the very
        // characters we need to flag would silently let drift through.
        const colonCallRe = /Skill\(\s*skill\s*=\s*\\?['"]sdd:([^'"\s)]+)\\?['"]/gi;
        let m;
        while ((m = colonCallRe.exec(line)) !== null) {
          colonCalls.push(`${path.basename(f)}: Skill(skill="sdd:${m[1]}")`);
        }
      }
    }
    assert.deepStrictEqual(
      colonCalls,
      [],
      'deprecated colon-form Skill() calls found — update to sdd-<cmd>: ' + colonCalls.join(', ')
    );
  });

  test('generated autocomplete skill surface uses hyphen names without underscores', (t) => {
    const tmp = createTempDir('sdd-autocomplete-surface-');
    t.after(() => cleanup(tmp));
    const skillsDir = path.join(tmp, 'skills');
    copyCommandsAsClaudeSkills(COMMANDS_DIR, skillsDir, 'sdd', '$HOME/.claude/', 'claude', true);

    // Don't filter the directory listing by `startsWith('sdd-')` — that
    // would silently hide exactly the kind of drift this test exists to
    // catch (a `sdd:extract-learnings` colon variant or a bare
    ...
    for (const dir of skillDirs) {
      assert.ok(
        dir.startsWith('sdd-'),
        `${dir}: generated skill directory must start with the canonical 'sdd-' namespace`,
      );
      assert.ok(
        !dir.includes(':'),
    ...
    assert.ok(skillDirs.includes('sdd-extract-learnings'), 'autocomplete surface must include sdd-extract-learnings');
    assert.ok(!skillDirs.includes('sdd-extract_learnings'), 'autocomplete surface must not include sdd-extract_learnings');
    ...
    for (const skillDir of skillDirs) {
      ...
      assert.ok(name.startsWith('sdd-'), `${skillDir}: autocomplete name must start with sdd-, got ${name}`);
