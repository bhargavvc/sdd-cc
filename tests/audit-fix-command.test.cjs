...
describe('AUDIT-FIX: command file', () => {
  ...
  test('command file exists', () => {
    assert.ok(
      ...
      'audit-fix.md must exist in commands/sdd/'
    );
  });

  test('has valid frontmatter with name sdd:audit-fix', () => {
    const content = fs.readFileSync(cmdPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('name: sdd:audit-fix'),
      'name must be sdd:audit-fix'
    );
  });
  ...
  test('has argument-hint in frontmatter for /sdd-help discoverability', () => {
    const content = fs.readFileSync(cmdPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('argument-hint:'),
      'must have argument-hint in frontmatter for /sdd-help discoverability'
    );
  });
...
describe('AUDIT-FIX: workflow file', () => {
  ...
  test('workflow file exists', () => {
    assert.ok(
      ...
      'audit-fix.md must exist in sdd/workflows/'
    );
  });
  ...
  test('has <available_agent_types> listing sdd-executor', () => {
    const content = fs.readFileSync(wfPath, 'utf-8');
    assert.ok(
    ...
    assert.ok(
      content.includes('sdd-executor'),
      'must list sdd-executor as available agent type'
    );
  });
});
