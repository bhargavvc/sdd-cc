...
function makeTmp(prefix) {
  return fs.mkdtempSync(path.join(os.tmpdir(), `sdd-3024-${prefix}-`));
}
...
describe('#3024 resolveModelForTier: disabled mode is a no-op (acceptance criterion 1)', () => {
  ...
  test('with no dynamic_routing block, falls back to resolveModelInternal', () => {
    ...
    const baseline = resolveModelInternal(projectDir, 'sdd-phase-researcher');
    assert.equal(resolveModelForTier(projectDir, 'sdd-phase-researcher', 0), baseline);
  });
  ...
  test('with dynamic_routing.enabled=false, attempt argument is ignored — same as resolveModelInternal', () => {
    ...
    assert.equal(resolveModelForTier(projectDir, 'sdd-phase-researcher', 0), baseline);
    assert.equal(resolveModelForTier(projectDir, 'sdd-phase-researcher', 1), baseline);
  });
});
...
describe('#3024 resolveModelForTier: enabled mode picks tier_models[default_tier]', () => {
  ...
  test('attempt=0 returns tier_models[agent_default_tier] (acceptance criterion 2)', () => {
    ...
    // sdd-codebase-mapper has light default tier per AGENT_DEFAULT_TIERS.
    ...
    assert.equal(AGENT_DEFAULT_TIERS['sdd-codebase-mapper'], 'light',
      'sdd-codebase-mapper expected to be light tier');
    assert.equal(resolveModelForTier(projectDir, 'sdd-codebase-mapper', 0), 'haiku');
    assert.equal(AGENT_DEFAULT_TIERS['sdd-planner'], 'heavy',
      'sdd-planner expected to be heavy tier');
    assert.equal(resolveModelForTier(projectDir, 'sdd-planner', 0), 'opus');
  });
...
describe('#3024 precedence: per-agent override > dynamic_routing > models > profile', () => {
  ...
  test('per-agent model_overrides beats dynamic_routing (acceptance criterion: override wins)', () => {
    writeConfig(projectDir, {
      model_profile: 'balanced',
      model_overrides: { 'sdd-codebase-mapper': 'openai/gpt-5' },
      dynamic_routing: {
        enabled: true,
        tier_models: { light: 'haiku', standard: 'sonnet', heavy: 'opus' },
      },
    ...
    assert.equal(resolveModelForTier(projectDir, 'sdd-codebase-mapper', 0), 'openai/gpt-5');
    assert.equal(resolveModelForTier(projectDir, 'sdd-codebase-mapper', 1), 'openai/gpt-5');
  });
  ...
  test('dynamic_routing beats phase-type models (#3023)', () => {
    ...
    // sdd-codebase-mapper is research phase-type; phase-type would give 'opus',
    // but dynamic routing (light default → haiku) wins.
    if (AGENT_DEFAULT_TIERS['sdd-codebase-mapper'] === 'light') {
      assert.equal(resolveModelForTier(projectDir, 'sdd-codebase-mapper', 0), 'haiku');
    }
  });
});
