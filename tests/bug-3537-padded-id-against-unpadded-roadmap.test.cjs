...
const sddTools = path.resolve(__dirname, '..', 'sdd', 'bin', 'sdd-tools.cjs');

function run(args, cwd) {
  try {
    return {
      stdout: execFileSync('node', [sddTools, ...args], {
        cwd,
        timeout: 15000,
        encoding: 'utf-8',
      }),
...
function expectParity({ verbWithPadded, verbWithUnpadded, fixtureOpts }) {
  const tmpA = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3537-A-'));
  const tmpB = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3537-B-'));
  try {
...
describe('bug #3537: phase verbs accept padded ids against un-padded ROADMAP prose', () => {
  ...
  test('roadmap get-phase: padded 02.7 returns the same section as un-padded 2.7', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3537-get-'));
    try {
      setupFixture(tmp, {});
  ...
  test('phase next-decimal: padded 02 finds decimals in un-padded ROADMAP', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3537-nd-'));
    try {
      setupFixture(tmp, {
        paddedId: '02.7',
        unpaddedId: '2.7',
  ...
  test('phase insert: padded base 02 finds anchor in un-padded ROADMAP', () => {
    const tmpA = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3537-ins-A-'));
    const tmpB = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3537-ins-B-'));
    try {
  ...
  test('roadmap annotate-dependencies: padded 02.7 finds phase section', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3537-ann-'));
    try {
      ...
      const tmp2 = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-3537-ann2-'));
      try {
