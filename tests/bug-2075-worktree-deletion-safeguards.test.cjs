/**
 * Regression tests for #2075: sdd-executor worktree merge systematically
 ...
 *   Post-commit deletion check: already in sdd-executor.md (--diff-filter=D).
...
const EXECUTOR_AGENT_PATH = path.join(__dirname, '..', 'agents', 'sdd-executor.md');
...
describe('bug-2075: worktree deletion safeguards', () => {
  ...
  describe('Failure Mode B: git clean prohibition in executor agent', () => {
    test('sdd-executor.md explicitly prohibits git clean in worktree context', () => {
      ...
      assert.ok(
        prohibitsGitClean,
        'sdd-executor.md must explicitly prohibit git clean — running it inside a worktree deletes files committed on the feature branch (#2075 Failure Mode B)'
      );
    });

    test('sdd-executor.md git clean prohibition explains the worktree data-loss risk', () => {
      const content = fs.readFileSync(EXECUTOR_AGENT_PATH, 'utf-8');
      ...
      assert.ok(gitCleanIdx > -1, 'sdd-executor.md must mention git clean (to prohibit it)');
      ...
      assert.ok(
        ...
        'The git clean prohibition in sdd-executor.md must explain why: git clean in a worktree deletes files that appear untracked but are committed on the feature branch'
      );
    });
  });
  ...
  describe('Defense-in-depth: post-commit deletion check (from #1977)', () => {
    test('sdd-executor.md task_commit_protocol has post-commit deletion verification', () => {
      const content = fs.readFileSync(EXECUTOR_AGENT_PATH, 'utf-8');

      assert.ok(
        content.includes('--diff-filter=D'),
        'sdd-executor.md must include --diff-filter=D to detect accidental file deletions after each commit'
      );

      // Must have a warning about unexpected deletions
      assert.ok(
        content.includes('DELETIONS') || content.includes('WARNING'),
        'sdd-executor.md must emit a warning when a commit includes unexpected file deletions'
      );
    });
  });
