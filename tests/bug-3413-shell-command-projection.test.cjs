...
const projection = require(path.join(__dirname, '..', 'sdd', 'bin', 'lib', 'shell-command-projection.cjs'));
...
describe('bug #3439: shell projection module owns managed-hook policy and legacy rewrite projection', () => {
  test('isManagedHookBasename is surface-aware', () => {
    assert.equal(isManagedHookBasename('/x/hooks/sdd-check-update.js', { surface: 'settings-json' }), true);
    assert.equal(isManagedHookBasename('/x/hooks/sdd-statusline.js', { surface: 'settings-json' }), true);
    assert.equal(isManagedHookBasename('/x/hooks/sdd-statusline.js', { surface: 'codex-toml' }), false);
  ...
  test('projectLegacySettingsHookCommand preserves non-Windows script token shape', () => {
    const command = projectLegacySettingsHookCommand({
      ...
      scriptPath: '/x/hooks/sdd-statusline.js',
      scriptToken: "'/x/hooks/sdd-statusline.js'",
      platform: 'linux',
      runtime: 'claude',
    });
    assert.equal(command, `"/usr/local/bin/node" '/x/hooks/sdd-statusline.js'`);
  });
  ...
  test('projectLegacySettingsHookCommand normalizes Windows managed paths and runtime wrapper policy', () => {
    const command = projectLegacySettingsHookCommand({
      ...
      scriptPath: 'C:\\Users\\me\\.gemini\\hooks\\sdd-prompt-guard.js',
      scriptToken: "'C:\\Users\\me\\.gemini\\hooks\\sdd-prompt-guard.js'",
      platform: 'win32',
      runtime: 'gemini',
    });
    assert.equal(command, '& "C:/nvm4w/nodejs/node.exe" "C:/Users/me/.gemini/hooks/sdd-prompt-guard.js"');
  });
  ...
  test('isManagedHookCommand classifies managed settings hooks and leaves user commands untouched', () => {
    assert.equal(
      isManagedHookCommand('"/usr/local/bin/node" "/Users/me/.claude/hooks/sdd-statusline.js"', {
        surface: 'settings-json',
      }),
      true,
    );
    assert.equal(
      isManagedHookCommand('"C:/Program Files/Git/bin/bash.exe" "C:/Users/me/.claude/hooks/sdd-session-state.sh"', {
        surface: 'settings-json',
      }),
      true,
    );
  ...
  test('isManagedHookCommand supports codex surfaces and optional legacy alias matching', () => {
    const command = '"/usr/local/bin/node" "/Users/me/.codex/hooks/sdd-check-update.js"';
    assert.equal(
    ...
    assert.equal(
      isManagedHookCommand('"/usr/local/bin/node" "/Users/me/.codex/hooks/sdd-update-check.js"', {
        surface: 'codex-toml',
      }),
      false,
    );
