...
//
// Between v1.38.3 and v1.38.4, agents/sdd-planner.md had 10 instances of
...
let src;
try {
  src = fs.readFileSync(path.join(ROOT, 'agents', 'sdd-planner.md'), 'utf8');
} catch (err) {
  throw new Error(`agents/sdd-planner.md not found — was the file renamed? (${err.message})`);
}
...
for (const { desc, pattern } of directives) {
  test(`sdd-planner.md: ${desc}`, () => {
    assert.ok(
      pattern.test(src),
      `Directive enforcement missing from sdd-planner.md: "${desc}" — pattern ${pattern} not found. ` +
