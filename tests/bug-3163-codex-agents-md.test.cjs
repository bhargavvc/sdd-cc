 ...
 *
 * When config.runtime === 'codex' (or SDD_RUNTIME=codex), the generate-claude-md
...
describe('bug #3163: generate-claude-md uses AGENTS.md for Codex runtime', () => {
  ...
  test('writes to AGENTS.md when SDD_RUNTIME=codex env var is set (env takes precedence over config)', () => {
    const configPath = path.join(tmpDir, '.planning', 'config.json');
    ...
    const result = runSddTools('generate-claude-md', tmpDir, { HOME: tmpDir, SDD_RUNTIME: 'codex' });
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    assert.ok(fs.existsSync(expectedAgentsPath), 'AGENTS.md must exist after generation');
    assert.ok(!fs.existsSync(path.join(realTmpDir, 'CLAUDE.md')), 'CLAUDE.md must not be created when SDD_RUNTIME=codex');
  });
