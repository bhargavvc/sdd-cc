...
//
// sdd-validate-commit.sh used `[[ "$CMD" =~ ^git[[:space:]]+commit ]]` to
...
// ── sdd-validate-commit.sh source check ──────────────────────────────────────

describe('sdd-validate-commit.sh delegates to git-cmd.js', () => {
  const hookSrc = fs.readFileSync(
    path.join(ROOT, 'hooks', 'sdd-validate-commit.sh'), 'utf8',
  );

  test('hook no longer uses the stale ^git\\s+commit bash regex', () => {
    assert.ok(
      !hookSrc.includes('^git[[:space:]]+commit'),
      'sdd-validate-commit.sh still uses the bypassed regex — fix not applied',
    );
  });
  ...
  test('hook delegates to git-cmd.js isGitSubcommand', () => {
    assert.ok(
      ...
      'sdd-validate-commit.sh does not reference git-cmd.js or isGitSubcommand',
    );
  });
