 ...
 *
 * When sdd-tools is invoked with --json-errors, all error() calls emit a
 ...
 *   6. SDD_JSON_ERRORS=1 env var   → same structured output without --flag
...
// Helper: run sdd-tools with --json-errors and parse the structured stderr.
...
function runJsonErrors(args, tmpDir, env = {}) {
  ...
  const result = runSddTools(allArgs, tmpDir, env);
  // Must have failed
...
describe('feat #3255: --json-errors mode emits structured error objects', () => {
  ...
  // ── 6. SDD_JSON_ERRORS=1 env var activates structured mode ───────────────

  test('SDD_JSON_ERRORS=1 env var produces same structured error as --json-errors flag', () => {
    ...
    try {
      ...
      throw new Error(
        `SDD_JSON_ERRORS=1 must emit valid JSON on stderr.\n` +
        `stderr: ${result.error}\n` +
        `parse error: ${e.message}`
      );
    }
