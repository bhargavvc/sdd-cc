...
/**
 * Tests for sdd-health MILESTONES.md drift detection (#2446).
 */
...
function makeTempProject(files = {}) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-2446-'));
...
test('health.md mentions --backfill flag', () => {
  const healthMd = fs.readFileSync(
    path.join(__dirname, '../sdd/workflows/health.md'), 'utf-8'
  );
