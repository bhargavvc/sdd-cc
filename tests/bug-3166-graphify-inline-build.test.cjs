...
/**
 * Regression fence for #3166 — `/sdd-graphify build` lost artifacts because the
...
const SKILL_PATH = path.join(__dirname, '..', 'commands', 'sdd', 'graphify.md');
...
describe('bug-3166: /sdd-graphify build runs inline (no Task sub-agent)', () => {
  ...
  test('a bash code block invokes the inline graphify update . pipeline', () => {
    ...
    assert.ok(
      bashBlocks.some(b => /sdd-tools\.cjs["']?\s+graphify build snapshot/.test(b.content)),
      'a bash code block must invoke `sdd-tools.cjs graphify build snapshot`'
    );
  });
});
