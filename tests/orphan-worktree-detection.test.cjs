...
/**
 * SDD Tools Tests - Orphan/Stale Worktree Detection (W017)
 *
...
const { runSddTools, createTempGitProject, cleanup } = require('./helpers.cjs');
...
describe('W017: structural presence', () => {
  test('worktree-safety module exports inspectWorktreeHealth', () => {
    const modulePath = path.join(__dirname, '..', 'sdd', 'bin', 'lib', 'worktree-safety.cjs');
