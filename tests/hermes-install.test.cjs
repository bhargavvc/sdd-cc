...
describe('Hermes Agent local install/uninstall', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('sdd-hermes-install-');
  ...
  test('installs SDD into ./.hermes and removes it cleanly', () => {
    ...
    // Nested layout per spec #2841: all SDD skills collapse into a single
    // skills/sdd/ category so Hermes' system prompt sees one entry, not 86.
    assert.ok(fs.existsSync(path.join(targetDir, 'skills', 'sdd', 'sdd-help', 'SKILL.md')));
    assert.ok(fs.existsSync(path.join(targetDir, 'skills', 'sdd', 'DESCRIPTION.md')),
    ...
    assert.ok(Object.keys(manifest.files).some(file => file.startsWith('skills/sdd/sdd-help/')), manifest);

    uninstall(false, 'hermes');

    assert.ok(!fs.existsSync(path.join(targetDir, 'skills', 'sdd', 'sdd-help')), 'Hermes skill directory removed');
    assert.ok(!fs.existsSync(path.join(targetDir, 'skills', 'sdd')), 'Hermes sdd category dir removed');
  ...
  test('installed SKILL.md frontmatter conforms to Hermes spec', () => {
    ...
    const targetDir = path.join(tmpDir, '.hermes');
    // Nested layout: skills live under skills/sdd/sdd-*/SKILL.md.
    const categoryDir = path.join(targetDir, 'skills', 'sdd');
    const skillDirs = fs.readdirSync(categoryDir, { withFileTypes: true })
      .filter(e => e.isDirectory() && e.name.startsWith('sdd-'))
      .map(e => e.name);

    assert.ok(skillDirs.length > 0, 'at least one sdd-* skill installed');
    ...
    assert.strictEqual(descFm.name, 'sdd', 'category DESCRIPTION.md name is "sdd"');
  ...
  test('replaces CLAUDE.md references with HERMES.md', () => {
    ...
    walk(skillsDir);
    // Sanity: at least one skill in the SDD set references the project
...
describe('E2E: Hermes Agent uninstall skills cleanup', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('sdd-hermes-uninstall-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
  });
  ...
  test('removes all sdd-* skill directories on --hermes --uninstall', () => {
    const targetDir = path.join(tmpDir, '.hermes');
    install(false, 'hermes');

    const skillsDir = path.join(targetDir, 'skills');
    const categoryDir = path.join(skillsDir, 'sdd');
    assert.ok(fs.existsSync(categoryDir), 'skills/sdd/ category dir exists after install');
    ...
    assert.ok(!fs.existsSync(categoryDir), 'skills/sdd/ category dir removed by uninstall');
    if (fs.existsSync(skillsDir)) {
      const remainingFlat = fs.readdirSync(skillsDir, { withFileTypes: true })
        .filter(e => e.isDirectory() && e.name.startsWith('sdd-'));
      assert.strictEqual(remainingFlat.length, 0,
        `Expected 0 stray flat sdd-* skill dirs after uninstall, found: ${remainingFlat.map(e => e.name).join(', ')}`);
    }
  });

  test('preserves non-SDD skill directories during --hermes --uninstall', () => {
    const targetDir = path.join(tmpDir, '.hermes');
    install(false, 'hermes');
    ...
    assert.ok(fs.existsSync(path.join(customSkillDir, 'SKILL.md')),
      'Non-SDD skill directory should be preserved after Hermes uninstall');
  });
  ...
  test('removes engine directory on --hermes --uninstall', () => {
    ...
    assert.ok(!fs.existsSync(path.join(targetDir, 'sdd')),
      'sdd engine should be removed after Hermes uninstall');
  });
});
...
describe('Hermes install contains no leaked Claude references (parity with Qwen regression #2112)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('sdd-hermes-refs-');
    previousCwd = process.cwd();
    process.chdir(tmpDir);
    install(false, 'hermes');
  });
