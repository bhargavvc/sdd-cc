/**
 * Enhancement #2500: sdd-codebase-mapper (arch focus) rich architecture output
 *
 * The codebase/ARCHITECTURE.md produced by sdd-codebase-mapper was a sparse
 * structural inventory — file listings and module relationships. After a major
 * refactor, research/ARCHITECTURE.md (created at /sdd-new-project) goes stale
...
// The sdd-codebase-mapper ARCHITECTURE.md template is the instruction set
...
const AGENT_PATH = path.join(__dirname, '..', 'agents', 'sdd-codebase-mapper.md');

describe('enh-2500: sdd-codebase-mapper arch focus — rich architecture output', () => {
  let agentContent;
  let archTemplate;

  before(() => {
    assert.ok(fs.existsSync(AGENT_PATH), 'agents/sdd-codebase-mapper.md must exist');
