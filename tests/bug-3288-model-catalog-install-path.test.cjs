 ...
 *      sdd/bin/shared/model-catalog.json (co-located inside the
 *      sdd/ payload).
 *
...
const MODEL_CATALOG_CJS = path.join(REPO_ROOT, 'sdd', 'bin', 'lib', 'model-catalog.cjs');
...
//   <tmp>/.claude/sdd/bin/lib/model-catalog.cjs  (copy of real file)
...
describe('bug #3288: model-catalog.cjs install-layout resolution', () => {
  ...
  beforeEach(() => {
    ...
    savedExplicitConfigDir = process.env.SDD_EXPLICIT_CONFIG_DIR;
    delete process.env.SDD_EXPLICIT_CONFIG_DIR;
  });
  ...
  afterEach(() => {
    ...
    if (savedExplicitConfigDir === undefined) {
      ...
      process.env.SDD_EXPLICIT_CONFIG_DIR = savedExplicitConfigDir;
    }
    rmTmpDir(tmpRoot);
  });
  ...
  test('OLD layout (3-level __dirname, no co-located json) fails to require', () => {
    ...
    //   <tmpRoot>/.claude/sdd/bin/lib/model-catalog.cjs  (copy of the real CJS)
    //   sdk/shared/model-catalog.json                              ABSENT
    const sddLibDir = path.join(tmpRoot, '.claude', 'sdd', 'bin', 'lib');
    fs.mkdirSync(sddLibDir, { recursive: true });
...
`;
    const catalogCjsPath = path.join(sddLibDir, 'model-catalog.cjs');
  ...
  test('NEW layout (co-located bin/shared/model-catalog.json) resolves correctly', () => {
    ...
    //   <tmpRoot>/.claude/sdd/bin/lib/model-catalog.cjs (copy of real CJS)
    //   <tmpRoot>/.claude/sdd/bin/shared/model-catalog.json (co-located copy)
    const sddBinDir = path.join(tmpRoot, '.claude', 'sdd', 'bin');
    const sddLibDir = path.join(sddBinDir, 'lib');
    const sddSharedDir = path.join(sddBinDir, 'shared');
    fs.mkdirSync(sddLibDir, { recursive: true });
    fs.mkdirSync(sddSharedDir, { recursive: true });
    ...
    fs.copyFileSync(MODEL_CATALOG_JSON, path.join(sddSharedDir, 'model-catalog.json'));
