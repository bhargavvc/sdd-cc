...
const DEBUG_COMMAND_PATH = path.join(__dirname, '..', 'commands', 'sdd', 'debug.md');

function readFrontmatter(filePath) {
  return parseFrontmatter(fs.readFileSync(filePath, 'utf8'));
}

test('#3431: commands/sdd/debug.md frontmatter parses as YAML and preserves argument-hint', () => {
  const frontmatter = readFrontmatter(DEBUG_COMMAND_PATH);

  assert.equal(frontmatter.name, 'sdd:debug');
  assert.equal(
