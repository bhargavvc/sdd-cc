...
/**
 * Bug #3042 + #3044: research-only flag for /sdd-plan-phase + scrub of
 * 4 stale slash-command references across user-facing surfaces.
 *
 * #3042 (orphaned research-phase): the slash command /sdd-research-phase
 *   never had a stub registered. Per the maintainer decision, the
 *   capability moves to a flag on /sdd-plan-phase rather than restoring
 *   a separate command. Invocation:
 *
 *       /sdd-plan-phase --research-phase <N>
 *
 ...
 *   registered:
 *
 *     /sdd-check-todos             →  /sdd-capture --list
 *     /sdd-new-workspace           →  /sdd-workspace --new
 *     /sdd-plan-milestone-gaps     →  inline gap planning (#3038 partial scrub)
 *     /sdd-status                  →  /sdd-progress
 *     /sdd-research-phase          →  /sdd-plan-phase --research-phase
 *
...
/**
 * Returns the Set<string> of `/sdd-*` slash-command tokens emitted by a
...
// ─── #3042: --research-phase flag wired into /sdd-plan-phase ────────────────

describe('bug #3042: /sdd-plan-phase --research-phase flag absorbs the standalone research command', () => {
  test('commands/sdd/plan-phase.md argument-hint advertises --research-phase', () => {
    const content = read('commands/sdd/plan-phase.md');
  ...
  test('workflows/plan-phase.md parses --research-phase and sets a research-only mode', () => {
    const content = read('sdd/workflows/plan-phase.md');
  ...
  test('orphaned workflows/research-phase.md is removed', () => {
    assert.equal(
      exists('sdd/workflows/research-phase.md'),
      false,
      'workflows/research-phase.md must be removed; the capability now lives on /sdd-plan-phase --research-phase'
    );
  });
...
const DELETED_COMMANDS = [
  ...
  '/sdd-new-workspace', // → /sdd-workspace --new
  '/sdd-status', // → /sdd-progress
  '/sdd-plan-milestone-gaps', // → inline gap planning
  '/sdd-research-phase', // → /sdd-plan-phase --research-phase
];
...
const USER_FACING_SURFACES = [
  ...
  'sdd/workflows/check-todos.md',
  'sdd/workflows/add-todo.md',
  'sdd/workflows/resume-project.md',
  'sdd/workflows/progress.md',
  'sdd/workflows/code-review.md',
  'sdd/workflows/new-workspace.md',
  'sdd/workflows/list-workspaces.md',
  'sdd/workflows/transition.md',
  'sdd/workflows/discovery-phase.md',
  // Reference + template surfaces
  'sdd/references/continuation-format.md',
  'sdd/templates/state.md',
  'sdd/templates/discovery.md',
...
describe('bug #3094: progress routing does not reference removed /sdd-list-phase-assumptions', () => {
  test('sdd/workflows/progress.md has no /sdd-list-phase-assumptions token', () => {
    const content = read('sdd/workflows/progress.md');
    const tokens = extractSlashCommandTokens(content);
    assert.equal(
      tokens.has('/sdd-list-phase-assumptions'),
      false,
      'progress.md must not recommend removed /sdd-list-phase-assumptions'
    );
  });

  test('progress.md pre-planning guidance uses /sdd-discuss-phase instead', () => {
    const content = read('sdd/workflows/progress.md');
    const tokens = extractSlashCommandTokens(content);
    assert.equal(
      tokens.has('/sdd-discuss-phase'),
      true,
      'progress.md should route pre-planning assumption checks via /sdd-discuss-phase'
    );
  });
});

describe('replacement commands appear where the deleted ones used to live', () => {
  test('docs/issue-driven-orchestration.md uses /sdd-workspace --new (not /sdd-new-workspace)', () => {
    const content = read('docs/issue-driven-orchestration.md');
    const tokens = extractSlashCommandTokens(content);
    assert.equal(
      tokens.has('/sdd-workspace'),
      true,
      'issue-driven-orchestration.md must reference /sdd-workspace as the workspace command'
    );
    // The "--new" flag must appear within 60 chars of /sdd-workspace —
    // proves the flag belongs to that command rather than appearing
    // somewhere unrelated.
    const proximityRe = /\/sdd-workspace[^\n]{0,60}--new/;
    assert.ok(
      proximityRe.test(content),
      'issue-driven-orchestration.md must document the "/sdd-workspace … --new" form'
    );
  });

  test('workflows/code-review.md error path points at /sdd-progress (not /sdd-status)', () => {
    const content = read('sdd/workflows/code-review.md');
    const tokens = extractSlashCommandTokens(content);
    assert.equal(
      tokens.has('/sdd-progress'),
      true,
      'code-review.md error path must route the user at /sdd-progress'
    );
  });
});
