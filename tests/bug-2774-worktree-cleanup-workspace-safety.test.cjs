 ...
 *
 * That filter only excludes the literal `$(pwd)`. When a SDD project is itself
...
// The exact discovery pipeline from sdd/workflows/quick.md and
// sdd/workflows/execute-phase.md (line: `WORKTREES=$(git worktree
...
describe('bug #2774 — worktree cleanup pipeline must not target the parent workspace', () => {
  describe('discovery pipeline (unit)', () => {
    test('selects only the agent worktree when workspace itself is a worktree', () => {
      ...
      const porcelain = [
        'worktree /Users/dev/upstream/sdd',
        'HEAD abc123',
...
`;
  ...
  describe('end-to-end against real git worktrees', () => {
    ...
    beforeEach(() => {
      ...
      upstream = makeTempUpstreamRepo('sdd-2774-upstream-');

      workspacesParent = fs.mkdtempSync(
        path.join(os.tmpdir(), 'sdd-2774-workspaces-')
      );
