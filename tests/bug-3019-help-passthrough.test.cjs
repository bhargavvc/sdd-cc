 ...
 *
 * `sdd-sdk query <subcommand> --help` returned the top-level SDK USAGE
 ...
 * before the registry handler / sdd-tools.cjs fallback could render
 * useful help.
 *
 * Two-layer fix:
 ...
 *   2. sdd/bin/sdd-tools.cjs — render the top-level usage on
 *      --help instead of erroring. Anti-hallucination invariant from
 *      #1818 is preserved (the destructive command never executes).
 *
 * Tests the integration: invoke sdd-tools.cjs the same way the SDK
...
const { runSddTools, isUsageOutput } = require('./helpers.cjs');

// #3026 CR (Major outside-diff): the SDK fallback wraps sdd-tools.cjs.
// When sdd-tools emits plain-text help (exit 0), the SDK previously
// JSON.parsed stdout and threw "Unexpected token 'U'". Verify the fix
// by invoking the built SDK end-to-end and asserting:
//   - exit 0
//   - stdout contains the sdd-tools usage
...
describe('bug #3026 (CR Major outside-diff): SDK forwards plain-text help from sdd-tools fallback', () => {
  test('sdd-sdk query phase --help (fallback path) returns usage, not a JSON parse error', (t) => {
    ...
    // registry, so it routes through the sdd-tools.cjs fallback. That is
    ...
    // The fallback sdd-tools.cjs emits exit 0 with usage on stdout.
    ...
    assert.ok(/Usage:\s*sdd-tools/.test(result.stdout) && /Commands:/.test(result.stdout),
      `usage must reach stdout: ${result.stdout}`);
  });
});

describe('bug #3019: sdd-tools renders usage on --help instead of erroring', () => {
  test('bare sdd-tools (no args) renders usage', () => {
    ...
    assert.ok(/Usage:\s*sdd-tools/.test(result.error));
    assert.ok(/Commands:/.test(result.error));
  });

  test('sdd-tools --help renders usage on stdout, exits 0', () => {
    const result = runSddTools(['--help']);
  ...
  test('sdd-tools -h renders usage on stdout, exits 0', () => {
    const result = runSddTools(['-h']);
    assert.strictEqual(result.success, true);
    assert.ok(isUsageOutput(result.output));
  });

  test('sdd-tools <subcommand> --help renders usage (does not run subcommand)', () => {
    ...
    const result = runSddTools(['phase', 'add', '--help']);
    assert.strictEqual(result.success, true);
    assert.ok(isUsageOutput(result.output));
  });
