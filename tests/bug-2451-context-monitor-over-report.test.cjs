 ...
 *
 * The SDD context monitor hook over-reports usage by ~13 percentage points
 * compared to Claude Code's native /context command. The root cause:
 *
 * sdd-statusline.js writes two values to the bridge file:
 ...
 *     (100 - buffer) * 100
 *
 * sdd-context-monitor.js displays used_pct (78%) in warning messages.
...
const HOOK_PATH = path.join(__dirname, '..', 'hooks', 'sdd-statusline.js');
const MONITOR_PATH = path.join(__dirname, '..', 'hooks', 'sdd-context-monitor.js');
