...
const EXECUTE_PHASE = path.join(__dirname, '..', 'sdd', 'workflows', 'execute-phase.md');
const EXECUTOR_AGENT = path.join(__dirname, '..', 'agents', 'sdd-executor.md');
const PLANNER_AGENT = path.join(__dirname, '..', 'agents', 'sdd-planner.md');
const EXECUTOR_EXAMPLES_REF = path.join(__dirname, '..', 'sdd', 'references', 'executor-examples.md');
const PLANNER_ANTIPATTERNS_REF = path.join(__dirname, '..', 'sdd', 'references', 'planner-antipatterns.md');
...
describe('prompt thinning — sub-200K context window support (#1978)', () => {
  ...
  describe('sdd-executor.md — reference to extracted examples', () => {
    test('references executor-examples.md for extended examples', () => {
      const content = fs.readFileSync(EXECUTOR_AGENT, 'utf-8');
      assert.ok(
        content.includes('executor-examples.md'),
        'sdd-executor.md must reference executor-examples.md for extended deviation/checkpoint examples'
      );
    });
  });

  describe('sdd-planner.md — reference to extracted anti-patterns', () => {
    test('references planner-antipatterns.md for extended anti-patterns', () => {
      ...
      assert.ok(
        ...
        'sdd-planner.md must reference planner-antipatterns.md for extended checkpoint anti-patterns and specificity examples'
      );
    });
  });
  ...
  describe('executor-examples.md — extracted reference file', () => {
    test('file exists', () => {
      assert.ok(
        ...
        'sdd/references/executor-examples.md must exist'
      );
    });
  ...
  describe('planner-antipatterns.md — extracted reference file', () => {
    test('file exists', () => {
      assert.ok(
        ...
        'sdd/references/planner-antipatterns.md must exist'
      );
    });
