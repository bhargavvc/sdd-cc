...
Spawned by `/sdd:plan-phase` (integrated) or `/sdd:plan-phase --research-phase <N>` (standalone).
...
```bash
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" graphify query "<capability-keyword>" --budget 1500
```
...
```bash
sdd-sdk query commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
