...
```bash
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" history-digest
```
...
```bash
VALID=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node "$HOME/.claude/sdd/bin/sdd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
