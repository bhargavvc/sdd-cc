...
```bash
sdd-sdk query phase.list-plans "$phase_number"
# Research / brief artifacts (deterministic listing)
sdd-sdk query phase.list-artifacts "$phase_number" --type research
sdd-sdk query roadmap.get-phase "$phase_number"
sdd-sdk query phase.list-artifacts "$phase_number" --type summary
```
...
Use `sdd-sdk query` to validate plan structure:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(sdd-sdk query verify.plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
Extract must_haves from each plan using `sdd-sdk query`:

```bash
MUST_HAVES=$(sdd-sdk query frontmatter.get "$PLAN_PATH" must_haves)
```
...
```bash
PLAN_STRUCTURE=$(sdd-sdk query verify.plan-structure "$PLAN_PATH")
```
...
```bash
sdd-sdk query plan.task-structure "$PLAN_PATH"
```
...
```bash
sdd-sdk query plan.task-structure "$PHASE_DIR/$PHASE-01-PLAN.md"
sdd-sdk query frontmatter.get "$PHASE_DIR/$PHASE-01-PLAN.md" files_modified
```
