...
Use template from `~/.claude/sdd/templates/UI-SPEC.md`.
...
Read template: `~/.claude/sdd/templates/UI-SPEC.md`
...
```bash
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" commit "docs($PHASE): UI design contract" --files "$PHASE_DIR/$PADDED_PHASE-UI-SPEC.md"
```
