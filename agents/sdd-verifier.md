...
sdd-sdk query roadmap.get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(sdd-sdk query roadmap.get-phase "$PHASE_NUM" --raw)
```
...
Use `sdd-sdk query` for artifact verification against must_haves in PLAN frontmatter:

```bash
ARTIFACT_RESULT=$(sdd-sdk query verify.artifacts "$PLAN_PATH")
```
...
Use `sdd-sdk query` for key link verification against must_haves in PLAN frontmatter:

```bash
LINKS_RESULT=$(sdd-sdk query verify.key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(sdd-sdk query summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(sdd-sdk query verify.commits $COMMIT_HASHES)
fi
...
```bash
ROADMAP_DATA=$(sdd-sdk query roadmap.analyze --raw)
```
...
**When the phase under verification has `mode: mvp` in ROADMAP.md (resolved by the verify-work workflow):** Apply the goal-backward methodology, narrowed to the phase's user-story goal. Required reading: `@~/.claude/sdd/references/verify-mvp-mode.md`.
