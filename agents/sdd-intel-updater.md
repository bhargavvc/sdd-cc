...
- **Cross-platform.** Use Glob, Read, and Grep tools -- not Bash `ls`, `find`, or `cat`. Bash file commands fail on Windows. Only use Bash for `sdd-sdk query intel` CLI calls.
...
| Command files | `commands/sdd/*.md` | `.kilo/command/*.md` |
| CLI tooling | `sdd/bin/` | `.kilo/sdd/bin/` |
| Workflow files | `sdd/workflows/` | `.kilo/sdd/workflows/` |
| Reference docs | `sdd/references/` | `.kilo/sdd/references/` |
...
**exports constraint:** Array of ACTUAL exported symbol names extracted from `module.exports` or `export` statements. MUST be real identifiers (e.g., `"configLoad"`, `"stateUpdate"`), NOT descriptions (e.g., `"config operations"`). If an export string contains a space, it is wrong -- extract the actual symbol name instead. Use `sdd-sdk query intel.extract-exports <file>` to get accurate exports.
...
```bash
sdd-sdk query intel.patch-meta .planning/intel/stack.json --cwd <project_root>
```
...
```bash
sdd-sdk query intel.patch-meta .planning/intel/files.json --cwd <project_root>
```
...
```bash
sdd-sdk query intel.patch-meta .planning/intel/apis.json --cwd <project_root>
```
...
```bash
sdd-sdk query intel.patch-meta .planning/intel/deps.json --cwd <project_root>
```
...
Run: `sdd-sdk query intel.validate --cwd <project_root>`
...
Run: `sdd-sdk query intel.snapshot --cwd <project_root>`
