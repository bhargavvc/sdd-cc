...
<task type="checkpoint:human-verify" gate="blocking-human">
  ...
  <how-to-verify>
    ...
    3. If the package does not exist, re-run /sdd:plan-phase --research-phase <N> to find the correct package
  </how-to-verify>
...
@~/.claude/sdd/references/executor-examples.md
</deviation_rules>
...
**See @~/.claude/sdd/references/checkpoints.md**
...
**When the orchestrator passes both `MVP_MODE=true` and `TDD_MODE=true`:** Before running the implementation step of any task with `tdd="true"`, run the runtime gate from `@~/.claude/sdd/references/execute-mvp-tdd.md`. If the gate trips, halt and report — do NOT proceed to the implementation step.
...
case "$WT_GIT_DIR" in
  *.git/worktrees/*)
      SENTINEL="$WT_GIT_DIR/sdd-spawn-toplevel"
...
```bash
sdd-sdk query commit-to-subrepo "{type}({phase}-{plan}): {concise task description}" --files file1 file2 ...
```
...
**Use template:** @~/.claude/sdd/templates/summary.md
...
<state_updates>
After SUMMARY.md, update STATE.md using `sdd-sdk query` state handlers (positional args; see `sdk/src/query/QUERY-HANDLERS.md`):
...
for decision in "${DECISIONS[@]}"; do
  sdd-sdk query state.add-decision "${decision}"
done
...
sdd-sdk query roadmap.update-plan-progress "${PHASE_NUMBER}"
...
```bash
sdd-sdk query state.add-blocker "Blocker description"
```
</state_updates>

<final_commit>
```bash
sdd-sdk query commit "docs({phase}-{plan}): complete [plan-name] plan" --files \
