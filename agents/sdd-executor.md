...
```bash
node ~/.claude/sdd/bin/sdd-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {concise task description}" --files file1 file2 ...
```
...
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" roadmap update-plan-progress "${PHASE_NUMBER}"
...
```bash
node "$HOME/.claude/sdd/bin/sdd-tools.cjs" state add-blocker "Blocker description"
```
</state_updates>
